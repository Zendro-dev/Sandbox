...
module.exports = class nuc_acid_library_result extends Sequelize.Model {
    ...
    static init(sequelize, DataTypes) {
        return super.init({
            ...
            modelName: "nuc_acid_library_result",
            tableName: "nuc_acid_library_results",
            sequelize
        });
    }
    ...
    static associate(models) {
        ...
        nuc_acid_library_result.belongsTo(models.sequencing_experiment, {
    ...
    static async readById(id) {
        let item = await nuc_acid_library_result.findByPk(id);
        if (item === null) {
    ...
    static bulkAddCsv(context) {
        ...
        return `Bulk import of nuc_acid_library_result records started. You will be send an email to ${helpersAcl.getTokenFromContext(context).email} informing you about success or errors`;
    }
    ...
    static async add_sample_id(id, sample_id) {
        let updated = await sequelize.transaction(async transaction => {
            return nuc_acid_library_result.update({
                sample_id: sample_id
            }, {
                where: {
                    id: id
    ...
    /**
     * add_sequencing_experiment_id - field Mutation (model-layer) for to_one associationsArguments to add 
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Id}   sequencing_experiment_id Foreign Key (stored in "Me") of the Association to be updated. 
     */
    static async add_sequencing_experiment_id(id, sequencing_experiment_id) {
        let updated = await sequelize.transaction(async transaction => {
            return nuc_acid_library_result.update({
    ...
    /**
     * remove_sequencing_experiment_id - field Mutation (model-layer) for to_one associationsArguments to remove 
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Id}   sequencing_experiment_id Foreign Key (stored in "Me") of the Association to be updated. 
     */
    static async remove_sequencing_experiment_id(id, sequencing_experiment_id) {
        let updated = await sequelize.transaction(async transaction => {
            return nuc_acid_library_result.update({
                sequencing_experiment_id: null
            }, {
                where: {
                    id: id,
                    sequencing_experiment_id: sequencing_experiment_id
    ...
    static idAttribute() {
        return nuc_acid_library_result.definition.id.name;
    }
    ...
    static idAttributeType() {
        return nuc_acid_library_result.definition.id.type;
    }

    /**
     * getIdValue - Get the value of the idAttribute ("id", or "internalId") for an instance of nuc_acid_library_result.
     *
     * @return {type} id value
     */

    getIdValue() {
        return this[nuc_acid_library_result.idAttribute()]
    }
    ...
    stripAssociations() {
        let attributes = Object.keys(nuc_acid_library_result.definition.attributes);
        attributes.push('id');
