...
/**
 * measurement.prototype.field_plot - Return associated record
 *
 * @param  {object} search       Search argument to match the associated record
 * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
 * @return {type}         Associated record
 */
measurement.prototype.field_plot = async function({
    search
}, context) {
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
measurement.prototype.add_field_plot = async function(input, benignErrorReporter) {
    await measurement.add_field_plot_id(this.getIdValue(), input.addField_plot, benignErrorReporter);
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
measurement.prototype.remove_field_plot = async function(input, benignErrorReporter) {
    if (input.removeField_plot == this.field_plot_id) {
        await measurement.remove_field_plot_id(this.getIdValue(), input.removeField_plot, benignErrorReporter);
...
async function countAllAssociatedRecords(id, context) {
    ...
    promises_to_one.push(measurement.field_plot({}, context));
...
async function validForDeletion(id, context) {
    if (await countAllAssociatedRecords(id, context) > 0) {
        throw new Error(`measurement with id ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`);
    }
    return true;
}
...
module.exports = {
    ...
    /**
     * readOneMeasurement - Check user authorization and return one record with the specified id in the id argument.
     *
    ...
    readOneMeasurement: async function({
        ...
        if (await checkAuthorization(context, 'measurement', 'read') === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await measurement.readById(id, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    vueTableMeasurement: async function(_, context) {
        if (await checkAuthorization(context, 'measurement', 'read') === true) {
            return helper.vueTable(context.request, measurement, ["id", "method", "reference", "text_value", "unit"]);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    /**
     * deleteMeasurement - Check user authorization and delete a record with the specified id in the id argument.
     *
    ...
    deleteMeasurement: async function({
        ...
        if (await checkAuthorization(context, 'measurement', 'delete') === true) {
            if (await validForDeletion(id, context)) {
                let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
                return measurement.deleteOne(id, benignErrorReporter);
            }
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
