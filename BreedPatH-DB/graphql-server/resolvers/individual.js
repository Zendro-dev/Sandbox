...
const associationArgsDef = {
    ...
    'addMother_to': 'genotype',
    'addFather_to': 'genotype',
    'addMarker_data_snps': 'marker_data',
    'addSamples': 'sample'
}
 ...
 */
individual.prototype.genotype = async function({
    search
}, context) {
...
/**
 * individual.prototype.mother_to - Return associated record
 *
 * @param  {object} search       Search argument to match the associated record
 * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
 * @return {type}         Associated record
 */
individual.prototype.mother_to = async function({
    search
}, context) {
    //build new search filter
    let nsearch = helper.addSearchField({
    ...
    if (found) {
        if (found.length > 1) {
            let foundIds = [];
            found.forEach(genotype => {
                foundIds.push(genotype.getIdValue())
            })
            context.benignErrors.push(new Error(
                `Not unique "to_one" association Error: Found ${found.length} genotypes matching individual with id ${this.getIdValue()}. Consider making this association a "to_many", using unique constraints, or moving the foreign key into the individual model. Returning first genotype. Found genotypes ${models.genotype.idAttribute()}s: [${foundIds.toString()}]`
            ));
        }
        return found[0];
    }
...
/**
 * individual.prototype.father_to - Return associated record
 *
 * @param  {object} search       Search argument to match the associated record
 * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
 * @return {type}         Associated record
 */
individual.prototype.father_to = async function({
    search
}, context) {
    //build new search filter
    let nsearch = helper.addSearchField({
...
/**
 * individual.prototype.marker_data_snpsConnection - Check user authorization and return certain number, specified in pagination argument, of records
 * associated with the current instance, this records should also
 * holds the condition of search argument, all of them sorted as specified by the order argument.
 *
 * @param  {object} search     Search argument for filtering associated records
 ...
 */
individual.prototype.marker_data_snpsConnection = function({
    search,
    order,
    pagination
}, context) {
    ...
    return resolvers.marker_dataConnection({
        search: nsearch,
        order: order,
        pagination: pagination
    }, context);
...
individual.prototype.handleAssociations = async function(input, benignErrorReporter) {
    let promises = [];
    if (helper.isNonEmptyArray(input.addMarker_data_snps)) {
        promises.push(this.add_marker_data_snps(input, benignErrorReporter));
    }
    ...
    if (helper.isNotUndefinedAndNotNull(input.addMother_to)) {
        promises.push(this.add_mother_to(input, benignErrorReporter));
    }
    if (helper.isNotUndefinedAndNotNull(input.addFather_to)) {
        promises.push(this.add_father_to(input, benignErrorReporter));
    }
    if (helper.isNonEmptyArray(input.removeMarker_data_snps)) {
        promises.push(this.remove_marker_data_snps(input, benignErrorReporter));
    }
    ...
    if (helper.isNotUndefinedAndNotNull(input.removeMother_to)) {
        promises.push(this.remove_mother_to(input, benignErrorReporter));
    }
    if (helper.isNotUndefinedAndNotNull(input.removeFather_to)) {
        promises.push(this.remove_father_to(input, benignErrorReporter));
    }

    await Promise.all(promises);
}
/**
 * add_marker_data_snps - field Mutation for to_many associations to add
 *
 * @param {object} input   Info of input Ids to add  the association
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
individual.prototype.add_marker_data_snps = async function(input, benignErrorReporter) {
    let results = [];
    for await (associatedRecordId of input.addMarker_data_snps) {
        results.push(models.marker_data.add_individual_id(associatedRecordId, this.getIdValue(), benignErrorReporter));
    }
...
individual.prototype.add_samples = async function(input, benignErrorReporter) {
    ...
    for await (associatedRecordId of input.addSamples) {
        results.push(models.sample.add_individual_id(associatedRecordId, this.getIdValue(), benignErrorReporter));
    }
    await Promise.all(results);
}
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
individual.prototype.add_genotype = async function(input, benignErrorReporter) {
    await individual.add_genotype_id(this.getIdValue(), input.addGenotype, benignErrorReporter);
    this.genotype_id = input.addGenotype;
}

/**
 * add_mother_to - field Mutation for to_one associations to add
 *
 * @param {object} input   Info of input Ids to add  the association
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
individual.prototype.add_mother_to = async function(input, benignErrorReporter) {
    await models.genotype.add_mother_id(input.addMother_to, this.getIdValue(), benignErrorReporter);
}

/**
 * add_father_to - field Mutation for to_one associations to add
 *
 * @param {object} input   Info of input Ids to add  the association
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
individual.prototype.add_father_to = async function(input, benignErrorReporter) {
    await models.genotype.add_father_id(input.addFather_to, this.getIdValue(), benignErrorReporter);
}

/**
 * remove_marker_data_snps - field Mutation for to_many associations to remove
 *
 * @param {object} input   Info of input Ids to remove  the association
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
individual.prototype.remove_marker_data_snps = async function(input, benignErrorReporter) {
    let results = [];
    for await (associatedRecordId of input.removeMarker_data_snps) {
        results.push(models.marker_data.remove_individual_id(associatedRecordId, this.getIdValue(), benignErrorReporter));
    }
    await Promise.all(results);
}
...
individual.prototype.remove_samples = async function(input, benignErrorReporter) {
    ...
    for await (associatedRecordId of input.removeSamples) {
        results.push(models.sample.remove_individual_id(associatedRecordId, this.getIdValue(), benignErrorReporter));
    }
    await Promise.all(results);
}
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
individual.prototype.remove_genotype = async function(input, benignErrorReporter) {
    if (input.removeGenotype == this.genotype_id) {
        await individual.remove_genotype_id(this.getIdValue(), input.removeGenotype, benignErrorReporter);
        this.genotype_id = null;
    }
}

/**
 * remove_mother_to - field Mutation for to_one associations to remove
 *
 * @param {object} input   Info of input Ids to remove  the association
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
individual.prototype.remove_mother_to = async function(input, benignErrorReporter) {
    await models.genotype.remove_mother_id(input.removeMother_to, this.getIdValue(), benignErrorReporter);
}

/**
 * remove_father_to - field Mutation for to_one associations to remove
 *
 * @param {object} input   Info of input Ids to remove  the association
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
individual.prototype.remove_father_to = async function(input, benignErrorReporter) {
    await models.genotype.remove_father_id(input.removeFather_to, this.getIdValue(), benignErrorReporter);
}
...
async function countAllAssociatedRecords(id, context) {
    ...
    promises_to_many.push(individual.countFilteredMarker_data_snps({}, context));
    promises_to_many.push(individual.countFilteredSamples({}, context));
    promises_to_one.push(individual.genotype({}, context));
    promises_to_one.push(individual.mother_to({}, context));
    promises_to_one.push(individual.father_to({}, context));
