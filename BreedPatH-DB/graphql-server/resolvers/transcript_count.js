 ...
 */
transcript_count.prototype.sample = async function({
    search
}, context) {
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
transcript_count.prototype.add_sample = async function(input, benignErrorReporter) {
    await transcript_count.add_sample_id(this.getIdValue(), input.addSample, benignErrorReporter);
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote cenzontle services
 */
transcript_count.prototype.remove_sample = async function(input, benignErrorReporter) {
    if (input.removeSample == this.sample_id) {
        await transcript_count.remove_sample_id(this.getIdValue(), input.removeSample, benignErrorReporter);
        this.sample_id = null;
    }
}
...
async function countAllAssociatedRecords(id, context) {
    ...
    promises_to_one.push(transcript_count.sample({}, context));
