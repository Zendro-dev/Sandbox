...
export default {
  ...
  tableTemplate(url) {
    let query = `query {csvTableTemplateMarker_data}`
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countMarker_data(${s}) }`;
    }
    else {
      query = `{ countMarker_data }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query marker_dataConnection($pagination: paginationCursorInput)
            { marker_dataConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query marker_dataConnection($pagination: paginationCursorInput)
            { marker_dataConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query marker_dataConnection($pagination: paginationCursorInput)
          { marker_dataConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
        ...
        query =
          `query marker_dataConnection($pagination: paginationCursorInput)
          { marker_dataConnection( pagination: $pagination ) {
            ${qbody}
          }}`
      }
    }
   ...
   * createItem
   * 
   * Add new Marker_data item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Marker_data item. 
   */
   ...
   * updateItem
   * 
   * Update Marker_data item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Marker_data item.  
   */
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Marker_data item. 
   */
   ...
   * 
   * Get individuals connection (cursor based) records associated to the given marker_data record
   ...
   * 
   * Get the individual-ids associated (by cursor based connection) to the given marker_data record 
   * through association 'Individual', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
