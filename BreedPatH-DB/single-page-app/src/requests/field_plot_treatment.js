...
export default {
  ...
  tableTemplate(url) {
    let query = `query {csvTableTemplateField_plot_treatment}`
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countField_plot_treatments(${s}) }`;
    }
    else {
      query = `{ countField_plot_treatments }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query field_plot_treatmentsConnection($pagination: paginationCursorInput)
            { field_plot_treatmentsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query field_plot_treatmentsConnection($pagination: paginationCursorInput)
            { field_plot_treatmentsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query field_plot_treatmentsConnection($pagination: paginationCursorInput)
          { field_plot_treatmentsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
        ...
        query =
          `query field_plot_treatmentsConnection($pagination: paginationCursorInput)
          { field_plot_treatmentsConnection( pagination: $pagination ) {
            ${qbody}
          }}`
      }
    }
   ...
   * createItem
   * 
   * Add new Field_plot_treatment item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Field_plot_treatment item. 
   */
   ...
   * updateItem
   * 
   * Update Field_plot_treatment item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Field_plot_treatment item.  
   */
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Field_plot_treatment item. 
   */
  deleteItem(url, variables) {
    var query = 
      `mutation
        deleteField_plot_treatment(
          $id:ID! 
        ) {
          deleteField_plot_treatment(
            id:$id
   ...
   * 
   * Get field_plots connection (cursor based) records associated to the given field_plot_treatment record
  ...
  getField_plotConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    var query = 
      `{ readOneField_plot_treatment(id: ${itemId}) { 
        field_plot{ 
          id,
          field_name,
          coordinates_or_name,
   ...
   * 
   * Get the field_plot-ids associated (by cursor based connection) to the given field_plot_treatment record 
   * through association 'Field_plot', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
...
/**
  ...
  getItemsAttribute(url, attribute) {
    let query =
      `query
          { field_plot_treatmentsConnection {
            edges {
              node {
                ${attribute}
              }
