...
export default {
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query samplesConnection($pagination: paginationCursorInput)
            { samplesConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
      ...
      if (o != null) {
        ...
        query =
          `query samplesConnection($pagination: paginationCursorInput)
          { samplesConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * createItem
   * 
   * Add new Sample item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Sample item. 
   */
    ...
    var query = 
      `mutation
        addSample(
          ...
          $tissue:String,
          $addIndividual: ID,
          $addSequencing_experiment: ID,
          $addLibrary_data: [ID],
          $addTranscript_counts: [ID],
          ) { addSample(
            name:$name,
            sampling_date:$sampling_date,
            type:$type,
            biological_replicate_no:$biological_replicate_no,
            lab_code:$lab_code,
            treatment:$treatment,
            tissue:$tissue,
            addIndividual: $addIndividual,
            addSequencing_experiment: $addSequencing_experiment,
            addLibrary_data: $addLibrary_data,
            addTranscript_counts: $addTranscript_counts,
          ) {
            id,
            name,
            sampling_date,
   ...
   * updateItem
   * 
   * Update Sample item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Sample item.  
   */
    ...
    var query = 
      `mutation
        updateSample(
          ...
          $removeIndividual: ID,
          $addSequencing_experiment: ID,
          $removeSequencing_experiment: ID,
          $addLibrary_data: [ID],
          $removeLibrary_data: [ID],
          $addTranscript_counts: [ID],
          $removeTranscript_counts: [ID],
          ) { updateSample(
            id:$id,
            name: $name,
            sampling_date: $sampling_date,
            ...
            addSequencing_experiment: $addSequencing_experiment,
            removeSequencing_experiment: $removeSequencing_experiment,
            addLibrary_data: $addLibrary_data,
            removeLibrary_data: $removeLibrary_data,
            addTranscript_counts: $addTranscript_counts,
            removeTranscript_counts: $removeTranscript_counts,
          ) {
            id,
            name,
            sampling_date,
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Sample item. 
   */
   ...
   * 
   * Get individuals connection (cursor based) records associated to the given sample record
   ...
   * 
   * Get the individual-ids associated (by cursor based connection) to the given sample record 
   * through association 'Individual', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
  ...
  /**
   * getLibrary_dataConnection
   * 
   * Get nuc_acid_library_results connection (cursor based) records associated to the given sample record
   * through association 'Library_data', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   ...
   * @param {String} ops Object with adittional query options.
   */
  getLibrary_dataConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneSample($pagination: paginationCursorInput) {
        readOneSample(id: ${itemId}) { 
          library_dataConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredLibrary_data( ${s} ) 
      } }` :      
      `query readOneSample($pagination: paginationCursorInput) {
        readOneSample(id: ${itemId}) { 
          library_dataConnection( pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredLibrary_data 
      } }`;
   ...
   * 
   *  
   * getAssociatedLibrary_dataConnection
   * 
   * Get the nuc_acid_library_result-ids associated (by cursor based connection) to the given sample record 
   * through association 'Library_data', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedLibrary_dataConnection(url, itemId) {
    var query = 
      `{ readOneSample( id: ${itemId} ){ 
        library_dataConnection{ edges { node { id } } } } }`;
    /**
     * Debug
     */
    console.log("getAssociatedIds.query: gql:\n", query);
  ...
  /**
   * getSequencing_experimentConnection
   * 
   * Get sequencing_experiments connection (cursor based) records associated to the given sample record
   * through association 'Sequencing_experiment', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   ...
   * @param {String} ops Object with adittional query options.
   */
  getSequencing_experimentConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    var query = 
      `{ readOneSample(id: ${itemId}) { 
   ...
   * 
   *  
   * getAssociatedSequencing_experimentConnection
   * 
   * Get the sequencing_experiment-ids associated (by cursor based connection) to the given sample record 
   * through association 'Sequencing_experiment', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedSequencing_experimentConnection(url, itemId) {
    var query = 
    `{ readOneSample( id: ${itemId} ){ 
      sequencing_experiment{ id } } }`;
    /**
  ...
  /**
   * getTranscript_countsConnection
   * 
   * Get transcript_counts connection (cursor based) records associated to the given sample record
   * through association 'Transcript_counts', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   ...
   * @param {String} ops Object with adittional query options.
   */
  getTranscript_countsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneSample($pagination: paginationCursorInput) {
        readOneSample(id: ${itemId}) { 
          transcript_countsConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
      ...
      `query readOneSample($pagination: paginationCursorInput) {
        readOneSample(id: ${itemId}) { 
          transcript_countsConnection( pagination: $pagination ) { 
            ${qbody},
          },
   ...
   * 
   *  
   * getAssociatedTranscript_countsConnection
   * 
   * Get the transcript_count-ids associated (by cursor based connection) to the given sample record 
   * through association 'Transcript_counts', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedTranscript_countsConnection(url, itemId) {
    var query = 
      `{ readOneSample( id: ${itemId} ){ 
        transcript_countsConnection{ edges { node { id } } } } }`;
    /**
     * Debug
     */
    console.log("getAssociatedIds.query: gql:\n", query);
