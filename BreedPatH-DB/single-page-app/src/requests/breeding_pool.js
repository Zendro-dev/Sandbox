...
export default {
  ...
  tableTemplate(url) {
    let query = `query {csvTableTemplateBreeding_pool}`
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('breeding_pool', searchText, ops);
    
    var query = '';

    //if has search
    if (s !== null) {
      query = `{ countBreeding_pools(${s}) }`;
    }
    else {
      query = `{ countBreeding_pools }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query breeding_poolsConnection($pagination: paginationCursorInput)
            { breeding_poolsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query breeding_poolsConnection($pagination: paginationCursorInput)
            { breeding_poolsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query breeding_poolsConnection($pagination: paginationCursorInput)
          { breeding_poolsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
        ...
        query =
          `query breeding_poolsConnection($pagination: paginationCursorInput)
          { breeding_poolsConnection( pagination: $pagination ) {
            ${qbody}
          }}`
      }
    }
   ...
   * createItem
   * 
   * Add new Breeding_pool item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Breeding_pool item. 
   */
   ...
   * updateItem
   * 
   * Update Breeding_pool item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Breeding_pool item.  
   */
      ...
      `mutation
        updateBreeding_pool(
          ...
          $removeGenotypes: [ID],
            ...
            description: $description,
            addGenotypes: $addGenotypes,
            removeGenotypes: $removeGenotypes,
          ) {
            id,
            name,
            description,
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Breeding_pool item. 
   */
  ...
  /**
   * getGenotypesConnection
   * 
   * Get genotypes connection (cursor based) records associated to the given breeding_pool record
   * through association 'Genotypes', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  getGenotypesConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneBreeding_pool($pagination: paginationCursorInput) {
        readOneBreeding_pool(id: ${itemId}) { 
          genotypesConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredGenotypes( ${s} ) 
      } }` :      
      `query readOneBreeding_pool($pagination: paginationCursorInput) {
        readOneBreeding_pool(id: ${itemId}) { 
   ...
   * 
   *  
   * getAssociatedGenotypesConnection
   * 
   * Get the genotype-ids associated (by cursor based connection) to the given breeding_pool record 
   * through association 'Genotypes', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedGenotypesConnection(url, itemId) {
    var query = 
      `{ readOneBreeding_pool( id: ${itemId} ){ 
        genotypesConnection{ edges { node { id } } } } }`;
    /**
     * Debug
     */
