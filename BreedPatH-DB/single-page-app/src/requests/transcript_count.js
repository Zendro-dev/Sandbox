...
export default {
  ...
  tableTemplate(url) {
    let query = `query {csvTableTemplateTranscript_count}`
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countTranscript_counts(${s}) }`;
    }
    else {
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query transcript_countsConnection($pagination: paginationCursorInput)
            { transcript_countsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
      ...
      if (o != null) {
        ...
        query =
          `query transcript_countsConnection($pagination: paginationCursorInput)
          { transcript_countsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * createItem
   * 
   * Add new Transcript_count item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Transcript_count item. 
   */
   ...
   * updateItem
   * 
   * Update Transcript_count item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Transcript_count item.  
   */
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Transcript_count item. 
   */
  deleteItem(url, variables) {
    var query = 
      `mutation
        deleteTranscript_count(
          $id:ID! 
        ) {
          deleteTranscript_count(
            id:$id
   ...
   * getSampleConnection
   * 
   * Get samples connection (cursor based) records associated to the given transcript_count record
  ...
  getSampleConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    var query = 
      `{ readOneTranscript_count(id: ${itemId}) { 
        sample{ 
          id,
          name,
          sampling_date,
   ...
   * 
   * Get the sample-ids associated (by cursor based connection) to the given transcript_count record 
   * through association 'Sample', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
