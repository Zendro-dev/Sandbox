...
export default function IndividualDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.individual[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.individual[String(itemState.id)].op === "update"&&
            lastModelChanged.individual[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.individual[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.individual[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.path=['Individuals', `id:${item.id}`, 'delete'];
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Individual | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
