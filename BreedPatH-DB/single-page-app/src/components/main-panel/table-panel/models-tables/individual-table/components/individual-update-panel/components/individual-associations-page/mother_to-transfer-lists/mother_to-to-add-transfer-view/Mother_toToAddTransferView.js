...
import MotherToToAddTransferViewToolbar from './components/Mother_toToAddTransferViewToolbar';
import MotherToToAddTransferViewCursorPagination from './components/Mother_toToAddTransferViewCursorPagination';
...
export default function MotherToToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.individual.getAssociatedMother_toConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'individual', association: 'mother_to', table:'A', query: 'readOneIndividual', method: 'getData()', request: 'api.individual.getAssociatedMother_toConnection'}];
          newError.path=['add', 'mother_to'];
        ...
        || typeof readOneIndividual.mother_to!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneIndividual.mother_to;
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'individual', association: 'mother_to', table:'A', query: 'countGenotypes', method: 'getData()', request: 'api.genotype.getCountItems'}];
              newError.path=['add', 'mother_to'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'individual', association: 'mother_to', table:'A', query: 'genotypesConnection', method: 'getData()', request: 'api.genotype.getItemsConnection'}];
                  newError.path=['add', 'mother_to'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(genotypesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'individual', association: 'mother_to', table:'A', method: 'getData()'}];
                    newError.path=['add', 'mother_to'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.individual.getAssociatedMother_toConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'individual', association: 'mother_to', table:'B', query: 'countGenotypes', method: 'getDataB()', request: 'api.genotype.getCountItems'}];
          newError.path=['add', 'mother_to'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'individual', association: 'mother_to', table:'B', query: 'genotypesConnection', method: 'getDataB()', request: 'api.genotype.getItemsConnection'}];
                newError.path=['add', 'mother_to'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'individual', association: 'mother_to', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'mother_to'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'mother_to' for this item was updated.
    ...
    if(lastModelChanged.individual&&
        ...
        lastModelChanged.individual[String(item.id)].changedAssociations.mother_to&&
        (lastModelChanged.individual[String(item.id)].changedAssociations.mother_to.added ||
         lastModelChanged.individual[String(item.id)].changedAssociations.mother_to.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.individual[String(item.id)].changedAssociations.mother_to.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('mother_to', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'mother' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'genotype',
     * but this action happened on the peer relation 'mother'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.genotype) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.mother) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.mother.removed) {
              let idsRemoved = entry[1].changedAssociations.mother.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.mother.added) {
              let idsAdded = entry[1].changedAssociations.mother.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('mother_to', idAdded);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('mother_to', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('mother_to', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MotherToToAddTransferViewToolbar
                title={'Genotypes'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <MotherToToAddTransferViewCursorPagination
                count={count}
...
MotherToToAddTransferView.propTypes = {
