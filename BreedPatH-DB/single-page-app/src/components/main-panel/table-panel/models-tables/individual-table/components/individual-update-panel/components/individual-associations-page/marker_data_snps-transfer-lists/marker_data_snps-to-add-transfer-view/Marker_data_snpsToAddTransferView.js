...
import MarkerDataSnpsToAddTransferViewToolbar from './components/Marker_data_snpsToAddTransferViewToolbar';
import MarkerDataSnpsToAddTransferViewCursorPagination from './components/Marker_data_snpsToAddTransferViewCursorPagination';
...
export default function MarkerDataSnpsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.individual.getAssociatedMarker_data_snpsConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', query: 'readOneIndividual', method: 'getData()', request: 'api.individual.getAssociatedMarker_data_snpsConnection'}];
          newError.path=['add', 'marker_data_snps'];
        ...
        || typeof readOneIndividual.marker_data_snpsConnection !== 'object'
        || readOneIndividual.marker_data_snpsConnection === null
        || !Array.isArray(readOneIndividual.marker_data_snpsConnection.edges)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let idso = readOneIndividual.marker_data_snpsConnection.edges.map(o => o.node);
        ...
        /*
          API Request: countMarker_data
        */
        let cancelableApiReqB = makeCancelable(api.marker_data.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', query: 'countMarker_data', method: 'getData()', request: 'api.marker_data.getCountItems'}];
              newError.path=['add', 'marker_data_snps'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            //check: countMarker_data
            let countMarker_data = response.data.data.countMarker_data;
            if(countMarker_data === null) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countMarker_data ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
              newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', query: 'countMarker_data', method: 'getData()', request: 'api.marker_data.getCountItems'}];
              newError.path=['add', 'marker_data_snps'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //check: countMarker_data type
            if(!Number.isInteger(countMarker_data)) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countMarker_data ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
              newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', query: 'countMarker_data', method: 'getData()', request: 'api.marker_data.getCountItems'}];
              newError.path=['add', 'marker_data_snps'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            if(response.data.errors) {
              let newError = {};
              newError.message = 'countMarker_data ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', query: 'countMarker_data', method: 'getData()', request: 'api.marker_data.getCountItems'}];
              newError.path=['add', 'marker_data_snps'];
              newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //ok
            setCount(countMarker_data);


            /*
              API Request: marker_dataConnection
            */
            let variables = {
              pagination: {
            ...
            let cancelableApiReqC = makeCancelable(api.marker_data.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', query: 'marker_dataConnection', method: 'getData()', request: 'api.marker_data.getItemsConnection'}];
                  newError.path=['add', 'marker_data_snps'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                //check: marker_dataConnection
                let marker_dataConnection = response.data.data.marker_dataConnection;
                if(marker_dataConnection === null) {
                  let newError = {};
                  let withDetails=true;
                  variant.current='error';
                  newError.message = 'marker_dataConnection ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
                  newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', query: 'marker_dataConnection', method: 'getData()', request: 'api.marker_data.getItemsConnection'}];
                  newError.path=['add', 'marker_data_snps'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  ...
                  //check: marker_dataConnection type
                  if(typeof marker_dataConnection !== 'object'
                  || !Array.isArray(marker_dataConnection.edges)
                  || typeof marker_dataConnection.pageInfo !== 'object' 
                  || marker_dataConnection.pageInfo === null) {
                    let newError = {};
                    let withDetails=true;
                    variant.current='error';
                    newError.message = 'marker_dataConnection ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
                    newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', query: 'marker_dataConnection', method: 'getData()', request: 'api.marker_data.getItemsConnection'}];
                    newError.path=['add', 'marker_data_snps'];
                    newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  //get items
                  let its = marker_dataConnection.edges.map(o => o.node);
                  let pi = marker_dataConnection.pageInfo;                      

                  //check: graphql errors
                  if(response.data.errors) {
                    let newError = {};
                    newError.message = 'marker_dataConnection ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
                    newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', query: 'marker_dataConnection', method: 'getData()', request: 'api.marker_data.getItemsConnection'}];
                    newError.path=['add', 'marker_data_snps'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'A', method: 'getData()'}];
                    newError.path=['add', 'marker_data_snps'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
                ...
                //error
                .catch((err) => { //error: on api.marker_data.getItemsConnection
                  if(err.isCanceled) {
                    return;
                  } else {
                    let newError = {};
          ...
          //error
          .catch((err) => { //error: on api.marker_data.getCountItems
            if(err.isCanceled) {
              return
            } else {
              let newError = {};
      ...
      //error
      .catch((err) => { //error: on api.individual.getAssociatedMarker_data_snpsConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.marker_data.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'B', query: 'countMarker_data', method: 'getDataB()', request: 'api.marker_data.getCountItems'}];
          newError.path=['add', 'marker_data_snps'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
        ...
        setCountB(countMarker_data);
          ...
          */
          ...
          let cancelableApiReqB = makeCancelable(api.marker_data.getItemsConnection(
            graphqlServerUrl,
            searchB,
            null, //orderBy
            null, //orderDirection
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'B', query: 'marker_dataConnection', method: 'getDataB()', request: 'api.marker_data.getItemsConnection'}];
                newError.path=['add', 'marker_data_snps'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'individual', association: 'marker_data_snps', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'marker_data_snps'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'marker_data_snps' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'marker_data' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.individual&&
        ...
        lastModelChanged.individual[String(item.id)].changedAssociations.marker_data_snps&&
        (lastModelChanged.individual[String(item.id)].changedAssociations.marker_data_snps.added ||
         lastModelChanged.individual[String(item.id)].changedAssociations.marker_data_snps.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.individual[String(item.id)].changedAssociations.marker_data_snps.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('marker_data_snps', idRemoved);
            });
          }
     ...
     * That is to say that this current item was associated or dis-associated with some 'marker_data',
     * but this action happened on the peer relation 'individual'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'marker_data'.
     * B: the current item internalId is in the addedIds of the updated 'marker_data'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'marker_data' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.marker_data) {
      let oens = Object.entries(lastModelChanged.marker_data);
      oens.forEach( (entry) => {
            ...
            if(entry[1].changedAssociations.individual.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('marker_data_snps', idAdded);
    ...
    /*
     * Case 3: 
     * The attributes of some 'marker_data' were modified or the item was deleted.
     * 
     * Conditions:
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnMarker_dataRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('marker_data_snps', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('marker_data_snps', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MarkerDataSnpsToAddTransferViewToolbar
                title={'Marker_data'}
                titleIcon={1}
                search={search}
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.id;
                        let label = it.marker_name;
                        let sublabel = it.nucleotide;
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'marker_data' }>
                                <Avatar>{"marker_data".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
              ...
              {/* Pagination */}
              <MarkerDataSnpsToAddTransferViewCursorPagination
                count={count}
...
MarkerDataSnpsToAddTransferView.propTypes = {
