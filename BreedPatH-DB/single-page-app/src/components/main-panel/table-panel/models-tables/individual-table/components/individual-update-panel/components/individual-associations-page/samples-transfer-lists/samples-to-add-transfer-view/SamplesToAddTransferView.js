...
export default function SamplesToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.individual.getAssociatedSamplesConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'individual', association: 'samples', table:'A', query: 'readOneIndividual', method: 'getData()', request: 'api.individual.getAssociatedSamplesConnection'}];
        ...
        //get items
        let idso = readOneIndividual.samplesConnection.edges.map(o => o.node);
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'individual', association: 'samples', table:'A', query: 'countSamples', method: 'getData()', request: 'api.sample.getCountItems'}];
              newError.path=['add', 'samples'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'individual', association: 'samples', table:'A', query: 'samplesConnection', method: 'getData()', request: 'api.sample.getItemsConnection'}];
                  newError.path=['add', 'samples'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(samplesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'individual', association: 'samples', table:'A', method: 'getData()'}];
                    newError.path=['add', 'samples'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.individual.getAssociatedSamplesConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
