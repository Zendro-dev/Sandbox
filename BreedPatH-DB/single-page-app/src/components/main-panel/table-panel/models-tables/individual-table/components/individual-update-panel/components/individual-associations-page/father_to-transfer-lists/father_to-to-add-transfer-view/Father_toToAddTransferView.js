...
import FatherToToAddTransferViewToolbar from './components/Father_toToAddTransferViewToolbar';
import FatherToToAddTransferViewCursorPagination from './components/Father_toToAddTransferViewCursorPagination';
...
export default function FatherToToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.individual.getAssociatedFather_toConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'individual', association: 'father_to', table:'A', query: 'readOneIndividual', method: 'getData()', request: 'api.individual.getAssociatedFather_toConnection'}];
          newError.path=['add', 'father_to'];
        ...
        || typeof readOneIndividual.father_to!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneIndividual.father_to;
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'individual', association: 'father_to', table:'A', query: 'countGenotypes', method: 'getData()', request: 'api.genotype.getCountItems'}];
              newError.path=['add', 'father_to'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'individual', association: 'father_to', table:'A', query: 'genotypesConnection', method: 'getData()', request: 'api.genotype.getItemsConnection'}];
                  newError.path=['add', 'father_to'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(genotypesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'individual', association: 'father_to', table:'A', method: 'getData()'}];
                    newError.path=['add', 'father_to'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.individual.getAssociatedFather_toConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'individual', association: 'father_to', table:'B', query: 'countGenotypes', method: 'getDataB()', request: 'api.genotype.getCountItems'}];
          newError.path=['add', 'father_to'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'individual', association: 'father_to', table:'B', query: 'genotypesConnection', method: 'getDataB()', request: 'api.genotype.getItemsConnection'}];
                newError.path=['add', 'father_to'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'individual', association: 'father_to', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'father_to'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'father_to' for this item was updated.
    ...
    if(lastModelChanged.individual&&
        ...
        lastModelChanged.individual[String(item.id)].changedAssociations.father_to&&
        (lastModelChanged.individual[String(item.id)].changedAssociations.father_to.added ||
         lastModelChanged.individual[String(item.id)].changedAssociations.father_to.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.individual[String(item.id)].changedAssociations.father_to.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('father_to', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'father' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'genotype',
     * but this action happened on the peer relation 'father'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.genotype) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.father) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.father.removed) {
              let idsRemoved = entry[1].changedAssociations.father.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.father.added) {
              let idsAdded = entry[1].changedAssociations.father.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('father_to', idAdded);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('father_to', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('father_to', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <FatherToToAddTransferViewToolbar
                title={'Genotypes'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <FatherToToAddTransferViewCursorPagination
                count={count}
...
FatherToToAddTransferView.propTypes = {
