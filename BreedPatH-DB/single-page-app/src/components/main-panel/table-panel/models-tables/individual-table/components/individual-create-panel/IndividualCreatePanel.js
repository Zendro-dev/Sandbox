...
export default function IndividualCreatePanel(props) {
  ...
  const [mother_toIdsToAddState, setMother_toIdsToAddState] = useState([]);
  const mother_toIdsToAdd = useRef([]);
  const [father_toIdsToAddState, setFather_toIdsToAddState] = useState([]);
  const father_toIdsToAdd = useRef([]);
  ...
  function setAddMother_to(variables) {
  ...
  function setAddFather_to(variables) {
  ...
  function doSave(event) {
    ...
    */
    ...
    setAddMother_to(variables);
    setAddFather_to(variables);
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'genotype':
        ...
        break;
      case 'mother_to':
        if(mother_toIdsToAdd.current.indexOf(itemId) === -1) {
        ...
        break;
      case 'father_to':
        if(father_toIdsToAdd.current.indexOf(itemId) === -1) {
      ...
      case 'marker_data_snps':
        if(marker_data_snpsIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    ...
    if(associationKey === 'marker_data_snps') {
      let iof = marker_data_snpsIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        marker_data_snpsIdsToAdd.current.splice(iof, 1);
      }
      return;
