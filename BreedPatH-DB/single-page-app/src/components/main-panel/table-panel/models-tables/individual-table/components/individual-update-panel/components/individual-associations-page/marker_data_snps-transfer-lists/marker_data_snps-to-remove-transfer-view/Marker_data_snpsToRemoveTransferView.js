...
import MarkerDataSnpsToRemoveTransferViewToolbar from './components/Marker_data_snpsToRemoveTransferViewToolbar';
import MarkerDataSnpsToRemoveTransferViewCursorPagination from './components/Marker_data_snpsToRemoveTransferViewCursorPagination';
...
export default function MarkerDataSnpsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.individual.getMarker_data_snpsConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'marker_data_snps', query: 'readOneIndividual', method: 'getData()', request: 'api.individual.getMarker_data_snpsConnection'}];
          newError.path=['remove', 'marker_data_snps'];
        ...
        || !Number.isInteger(readOneIndividual.countFilteredMarker_data_snps)
        ...
        || typeof readOneIndividual.marker_data_snpsConnection.pageInfo !== 'object'
        || readOneIndividual.marker_data_snpsConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneIndividual.countFilteredMarker_data_snps;
        let its = readOneIndividual.marker_data_snpsConnection.edges.map(o => o.node);
        let pi = readOneIndividual.marker_data_snpsConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.individual.getMarker_data_snpsConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'marker_data' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MarkerDataSnpsToRemoveTransferViewToolbar
                title={'Marker_data'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <MarkerDataSnpsToRemoveTransferViewCursorPagination
                count={count}
...
MarkerDataSnpsToRemoveTransferView.propTypes = {
