...
import GenotypesToAddTransferViewToolbar from './components/GenotypesToAddTransferViewToolbar';
import GenotypesToAddTransferViewCursorPagination from './components/GenotypesToAddTransferViewCursorPagination';
...
export default function GenotypesToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    /*
      API Request: readOneBreeding_pool
    */
    let cancelableApiReq = makeCancelable(api.breeding_pool.getAssociatedGenotypesConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'readOneBreeding_pool', method: 'getData()', request: 'api.breeding_pool.getAssociatedGenotypesConnection'}];
          newError.path=['add', 'genotypes'];
        ...
        //check: readOneBreeding_pool
        let readOneBreeding_pool = response.data.data.readOneBreeding_pool;
        if(readOneBreeding_pool === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneBreeding_pool ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'readOneBreeding_pool', method: 'getData()', request: 'api.breeding_pool.getAssociatedGenotypesConnection'}];
          newError.path=['add', 'genotypes'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: readOneBreeding_pool type
        if(typeof readOneBreeding_pool !== 'object'
        || typeof readOneBreeding_pool.genotypesConnection !== 'object'
        || readOneBreeding_pool.genotypesConnection === null
        || !Array.isArray(readOneBreeding_pool.genotypesConnection.edges)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneBreeding_pool ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'readOneBreeding_pool', method: 'getData()', request: 'api.breeding_pool.getAssociatedGenotypesConnection'}];
          newError.path=['add', 'genotypes'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //get items
        let idso = readOneBreeding_pool.genotypesConnection.edges.map(o => o.node);

        //check: graphql errors
        if(response.data.errors) {
          let newError = {};
          newError.message = 'readOneBreeding_pool ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'readOneBreeding_pool', method: 'getData()', request: 'api.breeding_pool.getAssociatedGenotypesConnection'}];
          newError.path=['add', 'genotypes'];
        ...
        /*
          API Request: countGenotypes
        */
        let cancelableApiReqB = makeCancelable(api.genotype.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'countGenotypes', method: 'getData()', request: 'api.genotype.getCountItems'}];
              newError.path=['add', 'genotypes'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            //check: countGenotypes
            let countGenotypes = response.data.data.countGenotypes;
            if(countGenotypes === null) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countGenotypes ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
              newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'countGenotypes', method: 'getData()', request: 'api.genotype.getCountItems'}];
              newError.path=['add', 'genotypes'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //check: countGenotypes type
            if(!Number.isInteger(countGenotypes)) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countGenotypes ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
              newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'countGenotypes', method: 'getData()', request: 'api.genotype.getCountItems'}];
              newError.path=['add', 'genotypes'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            if(response.data.errors) {
              let newError = {};
              newError.message = 'countGenotypes ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'countGenotypes', method: 'getData()', request: 'api.genotype.getCountItems'}];
              newError.path=['add', 'genotypes'];
              newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //ok
            setCount(countGenotypes);


            /*
              API Request: genotypesConnection
            */
            let variables = {
              pagination: {
            ...
            let cancelableApiReqC = makeCancelable(api.genotype.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'genotypesConnection', method: 'getData()', request: 'api.genotype.getItemsConnection'}];
                  newError.path=['add', 'genotypes'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                //check: genotypesConnection
                let genotypesConnection = response.data.data.genotypesConnection;
                if(genotypesConnection === null) {
                  let newError = {};
                  let withDetails=true;
                  variant.current='error';
                  newError.message = 'genotypesConnection ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
                  newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'genotypesConnection', method: 'getData()', request: 'api.genotype.getItemsConnection'}];
                  newError.path=['add', 'genotypes'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  ...
                  //check: genotypesConnection type
                  if(typeof genotypesConnection !== 'object'
                  || !Array.isArray(genotypesConnection.edges)
                  || typeof genotypesConnection.pageInfo !== 'object' 
                  || genotypesConnection.pageInfo === null) {
                    let newError = {};
                    let withDetails=true;
                    variant.current='error';
                    newError.message = 'genotypesConnection ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
                    newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'genotypesConnection', method: 'getData()', request: 'api.genotype.getItemsConnection'}];
                    newError.path=['add', 'genotypes'];
                    newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  //get items
                  let its = genotypesConnection.edges.map(o => o.node);
                  let pi = genotypesConnection.pageInfo;                      

                  //check: graphql errors
                  if(response.data.errors) {
                    let newError = {};
                    newError.message = 'genotypesConnection ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
                    newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', query: 'genotypesConnection', method: 'getData()', request: 'api.genotype.getItemsConnection'}];
                    newError.path=['add', 'genotypes'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'A', method: 'getData()'}];
                    newError.path=['add', 'genotypes'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
                ...
                //error
                .catch((err) => { //error: on api.genotype.getItemsConnection
                  if(err.isCanceled) {
                    return;
                  } else {
                    let newError = {};
          ...
          //error
          .catch((err) => { //error: on api.genotype.getCountItems
            if(err.isCanceled) {
              return
            } else {
              let newError = {};
      ...
      //error
      .catch((err) => { //error: on api.breeding_pool.getAssociatedGenotypesConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.genotype.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'B', query: 'countGenotypes', method: 'getDataB()', request: 'api.genotype.getCountItems'}];
          newError.path=['add', 'genotypes'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
        ...
        setCountB(countGenotypes);
          ...
          */
          ...
          let cancelableApiReqB = makeCancelable(api.genotype.getItemsConnection(
            graphqlServerUrl,
            searchB,
            null, //orderBy
            null, //orderDirection
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'B', query: 'genotypesConnection', method: 'getDataB()', request: 'api.genotype.getItemsConnection'}];
                newError.path=['add', 'genotypes'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'breeding_pool', association: 'genotypes', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'genotypes'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'genotypes' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'genotypes' (from another place).
     * 
     * Actions:
     ...
     * - return
     */
    if(lastModelChanged.breeding_pool&&
        lastModelChanged.breeding_pool[String(item.id)]&&
        lastModelChanged.breeding_pool[String(item.id)].changedAssociations&&
        lastModelChanged.breeding_pool[String(item.id)].changedAssociations.genotypes&&
        (lastModelChanged.breeding_pool[String(item.id)].changedAssociations.genotypes.added ||
         lastModelChanged.breeding_pool[String(item.id)].changedAssociations.genotypes.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.breeding_pool[String(item.id)].changedAssociations.genotypes.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('genotypes', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'breeding_pool' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'genotype',
     * but this action happened on the peer relation 'breeding_pool'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'genotype'.
     * B: the current item internalId is in the addedIds of the updated 'genotype'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'genotype' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.genotype) {
      let oens = Object.entries(lastModelChanged.genotype);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.breeding_pool) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.breeding_pool.removed) {
              let idsRemoved = entry[1].changedAssociations.breeding_pool.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.breeding_pool.added) {
              let idsAdded = entry[1].changedAssociations.breeding_pool.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('genotypes', idAdded);
    ...
    /*
     * Case 3: 
     * The attributes of some 'genotype' were modified or the item was deleted.
     * 
     * Conditions:
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnGenotypeRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('genotypes', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('genotypes', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <GenotypesToAddTransferViewToolbar
                title={'Genotypes'}
                titleIcon={1}
                search={search}
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'genotype' }>
                                <Avatar>{"genotype".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
              ...
              {/* Pagination */}
              <GenotypesToAddTransferViewCursorPagination
                count={count}
...
GenotypesToAddTransferView.propTypes = {
