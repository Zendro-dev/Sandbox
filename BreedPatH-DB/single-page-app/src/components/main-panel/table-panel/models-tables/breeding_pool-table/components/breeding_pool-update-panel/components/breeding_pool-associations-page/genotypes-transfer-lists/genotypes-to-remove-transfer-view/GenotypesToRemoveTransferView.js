...
import GenotypesToRemoveTransferViewToolbar from './components/GenotypesToRemoveTransferViewToolbar';
import GenotypesToRemoveTransferViewCursorPagination from './components/GenotypesToRemoveTransferViewCursorPagination';
...
export default function GenotypesToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.breeding_pool.getGenotypesConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'genotypes', query: 'readOneBreeding_pool', method: 'getData()', request: 'api.breeding_pool.getGenotypesConnection'}];
          newError.path=['remove', 'genotypes'];
        ...
        || !Number.isInteger(readOneBreeding_pool.countFilteredGenotypes)
        ...
        || typeof readOneBreeding_pool.genotypesConnection.pageInfo !== 'object'
        || readOneBreeding_pool.genotypesConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneBreeding_pool.countFilteredGenotypes;
        let its = readOneBreeding_pool.genotypesConnection.edges.map(o => o.node);
        let pi = readOneBreeding_pool.genotypesConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.breeding_pool.getGenotypesConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'genotype' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <GenotypesToRemoveTransferViewToolbar
                title={'Genotypes'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <GenotypesToRemoveTransferViewCursorPagination
                count={count}
...
GenotypesToRemoveTransferView.propTypes = {
