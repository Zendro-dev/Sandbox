...
export default function BreedingPoolCreatePanel(props) {
  ...
  function doSave(event) {
    ...
    */
    ...
    /*
      API Request: addBreeding_pool
    */
    let cancelableApiReq = makeCancelable(api.breeding_pool.createItem(graphqlServerUrl, variables));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'breeding_pool', query: 'addBreeding_pool', method: 'doSave()', request: 'api.breeding_pool.createItem'}];
          newError.path=['Breeding_pools', 'add'];
        ...
        //check: addBreeding_pool
        let addBreeding_pool = response.data.data.addBreeding_pool;
        if(addBreeding_pool === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addBreeding_pool ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'breeding_pool', query: 'addBreeding_pool', method: 'doSave()', request: 'api.breeding_pool.createItem'}];
          newError.path=['Breeding_pools', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: addBreeding_pool type
        if(typeof addBreeding_pool !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addBreeding_pool ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'breeding_pool', query: 'addBreeding_pool', method: 'doSave()', request: 'api.breeding_pool.createItem'}];
          newError.path=['Breeding_pools', 'add'];
        ...
        onClose(event, true, addBreeding_pool);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.breeding_pool.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'genotypes':
        if(genotypesIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'genotypes') {
      let iof = genotypesIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        genotypesIdsToAdd.current.splice(iof, 1);
      }
      return;
    }//end: case 'genotypes'
  }
      ...
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Breeding_pool'}
          </Typography>
...
BreedingPoolCreatePanel.propTypes = {
