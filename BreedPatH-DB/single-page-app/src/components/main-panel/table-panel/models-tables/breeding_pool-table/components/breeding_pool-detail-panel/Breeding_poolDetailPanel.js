...
import BreedingPoolUpdatePanel from '../breeding_pool-update-panel/Breeding_poolUpdatePanel'
import BreedingPoolDeleteConfirmationDialog from '../Breeding_poolDeleteConfirmationDialog'
...
export default function BreedingPoolDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.breeding_pool[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.breeding_pool[String(itemState.id)].op === "update"&&
            lastModelChanged.breeding_pool[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.breeding_pool[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.breeding_pool[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteBreeding_pool
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('breeding_pool', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('breeding_pool', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Breeding_pool | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
...
BreedingPoolDetailPanel.propTypes = {
