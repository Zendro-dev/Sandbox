...
import NucAcidLibraryResultEnhancedTableHead from './components/Nuc_acid_library_resultEnhancedTableHead'
import NucAcidLibraryResultEnhancedTableToolbar from './components/Nuc_acid_library_resultEnhancedTableToolbar'
import NucAcidLibraryResultCreatePanel from './components/nuc_acid_library_result-create-panel/Nuc_acid_library_resultCreatePanel'
import NucAcidLibraryResultUpdatePanel from './components/nuc_acid_library_result-update-panel/Nuc_acid_library_resultUpdatePanel'
import NucAcidLibraryResultDetailPanel from './components/nuc_acid_library_result-detail-panel/Nuc_acid_library_resultDetailPanel'
import NucAcidLibraryResultDeleteConfirmationDialog from './components/Nuc_acid_library_resultDeleteConfirmationDialog'
import NucAcidLibraryResultUploadFileDialog from './components/Nuc_acid_library_resultUploadFileDialog'
import NucAcidLibraryResultCursorPagination from './components/Nuc_acid_library_resultCursorPagination'
...
//Plotly
import NucAcidLibraryResultPlotly from '../../../../plots/Nuc_acid_library_resultPlotly';
...
export default function NucAcidLibraryResultEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.nuc_acid_library_result.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'nuc_acid_library_result', query: 'countNuc_acid_library_results', method: 'getData()', request: 'api.nuc_acid_library_result.getCountItems'}];
          newError.path=['Nuc_acid_library_results'];
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variant.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'nuc_acid_library_result', query: 'nuc_acid_library_resultsConnection', method: 'getData()', request: 'api.nuc_acid_library_result.getItemsConnection'}];
                newError.path=['Nuc_acid_library_results'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errors.current.length > 0) {
                ...
                newError.locations=[{model: 'nuc_acid_library_result', method: 'getData()'}];
                newError.path=['Nuc_acid_library_results'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
  ...
  function doDelete(event, item) {
    ...
    /*
      API Request: deleteNuc_acid_library_result
    */
    let cancelableApiReq = makeCancelable(api.nuc_acid_library_result.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'nuc_acid_library_result', query: 'deleteNuc_acid_library_result', method: 'doDelete()', request: 'api.nuc_acid_library_result.deleteItem'}];
          newError.path=['Nuc_acid_library_results', `id:${item.id}`, 'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: deleteNuc_acid_library_result
        let deleteNuc_acid_library_result = response.data.data.deleteNuc_acid_library_result;
        if(deleteNuc_acid_library_result === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'deleteNuc_acid_library_result ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'nuc_acid_library_result', query: 'deleteNuc_acid_library_result', method: 'doDelete()', request: 'api.nuc_acid_library_result.deleteItem'}];
          newError.path=['Nuc_acid_library_results', `id:${item.id}` ,'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        /**
         * Type of deleteNuc_acid_library_result is not validated. Only not null is
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'deleteNuc_acid_library_result ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'nuc_acid_library_result', query: 'deleteNuc_acid_library_result', method: 'doDelete()', request: 'api.nuc_acid_library_result.deleteItem'}];
          newError.path=['Nuc_acid_library_results', `id:${item.id}` ,'delete'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
      ...
      //error
      .catch((err) => { //error: on api.nuc_acid_library_result.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  function getCsvTemplate() {
    ...
    /*
      API Request: csvTableTemplateNuc_acid_library_result
    */
    let cancelableApiReq = makeCancelable(api.nuc_acid_library_result.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'nuc_acid_library_result', query: 'csvTableTemplateNuc_acid_library_result', method: 'getCsvTemplate()', request: 'api.nuc_acid_library_result.tableTemplate'}];
          newError.path=['Nuc_acid_library_results', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: csvTableTemplateNuc_acid_library_result
        let csvTableTemplateNuc_acid_library_result = response.data.data.csvTableTemplateNuc_acid_library_result;
        if(csvTableTemplateNuc_acid_library_result === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateNuc_acid_library_result ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'nuc_acid_library_result', query: 'csvTableTemplateNuc_acid_library_result', method: 'getCsvTemplate()', request: 'api.nuc_acid_library_result.tableTemplate'}];
          newError.path=['Nuc_acid_library_results', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: csvTableTemplateNuc_acid_library_result type
        if(!Array.isArray(csvTableTemplateNuc_acid_library_result)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateNuc_acid_library_result ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'nuc_acid_library_result', query: 'csvTableTemplateNuc_acid_library_result', method: 'getCsvTemplate()', request: 'api.nuc_acid_library_result.tableTemplate'}];
          newError.path=['Nuc_acid_library_results', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'csvTableTemplateNuc_acid_library_result ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'nuc_acid_library_result', query: 'csvTableTemplateNuc_acid_library_result', method: 'getCsvTemplate()', request: 'api.nuc_acid_library_result.tableTemplate'}];
          newError.path=['Nuc_acid_library_results', 'csvTemplate'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateNuc_acid_library_result.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "nuc_acid_library_result-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.nuc_acid_library_result.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
...
function resetReloadData() {
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <NucAcidLibraryResultEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(NucAcidLibraryResultPlotly)}
                ...
                <SwipeableViews index={getSwipeIndex()} disabled={true}>
                  ...
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      <Table stickyHeader size='small' ref={tref}>
                        ...
                        {/* Table Head */}
                        <NucAcidLibraryResultEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                          rowCount={count}
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.nuc_acid_library_result&&Array.isArray(permissions.nuc_acid_library_result)
                                      &&(permissions.nuc_acid_library_result.includes('update') || permissions.nuc_acid_library_result.includes('*')))
                                      &&(
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.nuc_acid_library_result&&Array.isArray(permissions.nuc_acid_library_result)
                                      &&(permissions.nuc_acid_library_result.includes('delete') || permissions.nuc_acid_library_result.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.lab_code!==null)?item.lab_code:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.file_name!==null)?item.file_name:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.file_uri!==null)?item.file_uri:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.type!==null)?item.type:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.technical_replicate!==null)?item.technical_replicate:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.trimmed!==null)?item.trimmed:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.sequencing_experiment_id!==null)?item.sequencing_experiment_id:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <NucAcidLibraryResultCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(NucAcidLibraryResultPlotly)) && (
                      <NucAcidLibraryResultPlotly />
                    )}
                  </div>
                </SwipeableViews>
              </Paper>
      ...
      {/* Dialog: Create Panel */}
      {(createDialogOpen) && (
        <NucAcidLibraryResultCreatePanel
          permissions={permissions}
      ...
      {/* Dialog: Update Panel */}
      {(updateDialogOpen) && (
        <NucAcidLibraryResultUpdatePanel
          permissions={permissions}
          item={updateItem}
      ...
      {(deleteConfirmationDialogOpen) && (
        <NucAcidLibraryResultDeleteConfirmationDialog
          permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <NucAcidLibraryResultUploadFileDialog
...
NucAcidLibraryResultEnhancedTable.propTypes = {
