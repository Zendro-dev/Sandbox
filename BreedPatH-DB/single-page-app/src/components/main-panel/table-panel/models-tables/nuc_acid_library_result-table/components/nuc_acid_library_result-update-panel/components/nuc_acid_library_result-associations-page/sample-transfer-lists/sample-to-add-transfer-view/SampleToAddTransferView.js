...
import SampleToAddTransferViewToolbar from './components/SampleToAddTransferViewToolbar';
import SampleToAddTransferViewCursorPagination from './components/SampleToAddTransferViewCursorPagination';
...
export default function SampleToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.nuc_acid_library_result.getAssociatedSampleConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'nuc_acid_library_result', association: 'sample', table:'A', query: 'readOneNuc_acid_library_result', method: 'getData()', request: 'api.nuc_acid_library_result.getAssociatedSampleConnection'}];
          newError.path=['add', 'sample'];
        ...
        || typeof readOneNuc_acid_library_result.sample!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneNuc_acid_library_result.sample;
        ...
        */
        let cancelableApiReqB = makeCancelable(api.sample.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'nuc_acid_library_result', association: 'sample', table:'A', query: 'countSamples', method: 'getData()', request: 'api.sample.getCountItems'}];
              newError.path=['add', 'sample'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.sample.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'nuc_acid_library_result', association: 'sample', table:'A', query: 'samplesConnection', method: 'getData()', request: 'api.sample.getItemsConnection'}];
                  newError.path=['add', 'sample'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(samplesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'nuc_acid_library_result', association: 'sample', table:'A', method: 'getData()'}];
                    newError.path=['add', 'sample'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.nuc_acid_library_result.getAssociatedSampleConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'nuc_acid_library_result', association: 'sample', table:'B', query: 'countSamples', method: 'getDataB()', request: 'api.sample.getCountItems'}];
          newError.path=['add', 'sample'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'nuc_acid_library_result', association: 'sample', table:'B', query: 'samplesConnection', method: 'getDataB()', request: 'api.sample.getItemsConnection'}];
                newError.path=['add', 'sample'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'nuc_acid_library_result', association: 'sample', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'sample'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'sample' for this item was updated.
    ...
    if(lastModelChanged.nuc_acid_library_result&&
        ...
        lastModelChanged.nuc_acid_library_result[String(item.id)].changedAssociations.sample&&
        (lastModelChanged.nuc_acid_library_result[String(item.id)].changedAssociations.sample.added ||
         lastModelChanged.nuc_acid_library_result[String(item.id)].changedAssociations.sample.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.nuc_acid_library_result[String(item.id)].changedAssociations.sample.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('sample', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'library_data' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'sample',
     * but this action happened on the peer relation 'library_data'.
     * 
     * Conditions:
     ...
     * 
     * else if B:
     * - remove the internalId of the 'sample' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.sample) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.library_data) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.library_data.removed) {
              let idsRemoved = entry[1].changedAssociations.library_data.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.library_data.added) {
              let idsAdded = entry[1].changedAssociations.library_data.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('sample', idAdded);
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('sample', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <SampleToAddTransferViewToolbar
                title={'Samples'}
                titleIcon={1}
                search={search}
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.id===it.id) ? 'sample — ' + t('modelPanels.associatedRecord', 'Associated record') : 'sample '}>
                                <Avatar>
                                  {"sample".slice(0,1)}
                                </Avatar>
                              </Tooltip>
              ...
              {/* Pagination */}
              <SampleToAddTransferViewCursorPagination
                count={count}
...
SampleToAddTransferView.propTypes = {
