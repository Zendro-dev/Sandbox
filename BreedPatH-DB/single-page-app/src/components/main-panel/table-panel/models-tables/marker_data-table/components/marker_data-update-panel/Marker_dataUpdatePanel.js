...
import MarkerDataAttributesPage from './components/marker_data-attributes-page/Marker_dataAttributesPage'
import MarkerDataAssociationsPage from './components/marker_data-associations-page/Marker_dataAssociationsPage'
import MarkerDataTabsA from './components/Marker_dataTabsA'
import MarkerDataConfirmationDialog from './components/Marker_dataConfirmationDialog'
...
export default function MarkerDataUpdatePanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged.marker_data&&
      lastModelChanged.marker_data[String(item.id)]) {

        //updated item
        if(lastModelChanged.marker_data[String(item.id)].op === "update"&&
            lastModelChanged.marker_data[String(item.id)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.marker_data[String(item.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function getInitialValues() {
    ...
    initialValues.marker_name = item.marker_name;
    initialValues.nucleotide = item.nucleotide;
  ...
  initialValueOkStates.marker_name = (item.marker_name!==null ? 1 : 0);
  initialValueOkStates.nucleotide = (item.nucleotide!==null ? 1 : 0);
  ...
  function getInitialValueAjvStates() {
    ...
    _initialValueAjvStates.marker_name = {errors: []};
    _initialValueAjvStates.nucleotide = {errors: []};
  ...
  function areThereChangedFields() {
    if(values.current.marker_name !== item.marker_name) { return true;}
    if(values.current.nucleotide !== item.nucleotide) { return true;}
...
function setAjvErrors(err) {
  ...
  function doSave(event) {
    ...
    */
    ...
    /*
      API Request: updateMarker_data
    */
    let cancelableApiReq = makeCancelable(api.marker_data.updateItem(graphqlServerUrl, variables));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'marker_data', query: 'updateMarker_data', method: 'doSave()', request: 'api.marker_data.updateItem'}];
          newError.path=['Marker_data', `id:${item.id}`, 'update'];
        ...
        //check: updateMarker_data
        let updateMarker_data = response.data.data.updateMarker_data;
        if(updateMarker_data === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'updateMarker_data ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'marker_data', query: 'updateMarker_data', method: 'doSave()', request: 'api.marker_data.updateItem'}];
          newError.path=['Marker_data', `id:${item.id}`, 'update'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: updateMarker_data type
        if(typeof updateMarker_data !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'marker_data ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'marker_data', query: 'updateMarker_data', method: 'doSave()', request: 'api.marker_data.updateItem'}];
          newError.path=['Marker_data', `id:${item.id}`, 'update'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          newError.message = 'updateMarker_data ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'marker_data', query: 'updateMarker_data', method: 'doSave()', request: 'api.marker_data.updateItem'}];
          newError.path=['Marker_data', `id:${item.id}`, 'update'];
        ...
        onClose(event, true, updateMarker_data);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.marker_data.updateItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
      ...
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": Marker_data | id: " + item.id}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " marker_data" }>
              <Fab 
                color="secondary" 
...
MarkerDataUpdatePanel.propTypes = {
