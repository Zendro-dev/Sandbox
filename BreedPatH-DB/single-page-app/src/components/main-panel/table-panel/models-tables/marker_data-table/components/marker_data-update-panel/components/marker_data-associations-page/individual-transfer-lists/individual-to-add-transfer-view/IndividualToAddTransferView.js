...
export default function IndividualToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    /*
      API Request: readOneMarker_data
    */
    let cancelableApiReq = makeCancelable(api.marker_data.getAssociatedIndividualConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'marker_data', association: 'individual', table:'A', query: 'readOneMarker_data', method: 'getData()', request: 'api.marker_data.getAssociatedIndividualConnection'}];
        ...
        //check: readOneMarker_data
        let readOneMarker_data = response.data.data.readOneMarker_data;
        if(readOneMarker_data === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneMarker_data ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'marker_data', association: 'individual', table:'A', query: 'readOneMarker_data', method: 'getData()', request: 'api.marker_data.getAssociatedIndividualConnection'}];
          newError.path=['add', 'individual'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: readOneMarker_data type
        if(typeof readOneMarker_data !== 'object'
        || typeof readOneMarker_data.individual!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneMarker_data ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'marker_data', association: 'individual', table:'A', query: 'readOneMarker_data', method: 'getData()', request: 'api.marker_data.getAssociatedIndividualConnection'}];
          newError.path=['add', 'individual'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //get item
        let idso = readOneMarker_data.individual;

        //check: graphql errors
        if(response.data.errors) {
          let newError = {};
          newError.message = 'readOneMarker_data ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'marker_data', association: 'individual', table:'A', query: 'readOneMarker_data', method: 'getData()', request: 'api.marker_data.getAssociatedIndividualConnection'}];
          newError.path=['add', 'individual'];
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'marker_data', association: 'individual', table:'A', query: 'countIndividuals', method: 'getData()', request: 'api.individual.getCountItems'}];
              newError.path=['add', 'individual'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'marker_data', association: 'individual', table:'A', query: 'individualsConnection', method: 'getData()', request: 'api.individual.getItemsConnection'}];
                  newError.path=['add', 'individual'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(individualsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'marker_data', association: 'individual', table:'A', method: 'getData()'}];
                    newError.path=['add', 'individual'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.marker_data.getAssociatedIndividualConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'marker_data', association: 'individual', table:'B', query: 'countIndividuals', method: 'getDataB()', request: 'api.individual.getCountItems'}];
          newError.path=['add', 'individual'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'marker_data', association: 'individual', table:'B', query: 'individualsConnection', method: 'getDataB()', request: 'api.individual.getItemsConnection'}];
                newError.path=['add', 'individual'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'marker_data', association: 'individual', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'individual'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.marker_data&&
        lastModelChanged.marker_data[String(item.id)]&&
        lastModelChanged.marker_data[String(item.id)].changedAssociations&&
        lastModelChanged.marker_data[String(item.id)].changedAssociations.individual&&
        (lastModelChanged.marker_data[String(item.id)].changedAssociations.individual.added ||
         lastModelChanged.marker_data[String(item.id)].changedAssociations.individual.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.marker_data[String(item.id)].changedAssociations.individual.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'marker_data_snps' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'individual',
     * but this action happened on the peer relation 'marker_data_snps'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.individual) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.marker_data_snps) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.marker_data_snps.removed) {
              let idsRemoved = entry[1].changedAssociations.marker_data_snps.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.marker_data_snps.added) {
              let idsAdded = entry[1].changedAssociations.marker_data_snps.idsAdded;
              if(idsAdded) {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('individual', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('individual', item.id);
    }
  };
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.id===it.id) ? 'individual — ' + t('modelPanels.associatedRecord', 'Associated record') : 'individual '}>
                                <Avatar>
