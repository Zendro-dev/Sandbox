...
import MarkerDataUpdatePanel from '../marker_data-update-panel/Marker_dataUpdatePanel'
import MarkerDataDeleteConfirmationDialog from '../Marker_dataDeleteConfirmationDialog'
...
export default function MarkerDataDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.marker_data[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.marker_data[String(itemState.id)].op === "update"&&
            lastModelChanged.marker_data[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.marker_data[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.marker_data[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteMarker_data
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('marker_data', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('marker_data', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Marker_data | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
...
MarkerDataDetailPanel.propTypes = {
