...
export default function MarkerDataCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.marker_name = null;
    initialValues.nucleotide = null;
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.marker_name = 0;
    initialValueOkStates.nucleotide = 0;
  ...
  function doSave(event) {
    ...
    */
    ...
    /*
      API Request: addMarker_data
    */
    let cancelableApiReq = makeCancelable(api.marker_data.createItem(graphqlServerUrl, variables));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'marker_data', query: 'addMarker_data', method: 'doSave()', request: 'api.marker_data.createItem'}];
          newError.path=['Marker_data', 'add'];
        ...
        //check: addMarker_data
        let addMarker_data = response.data.data.addMarker_data;
        if(addMarker_data === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addMarker_data ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'marker_data', query: 'addMarker_data', method: 'doSave()', request: 'api.marker_data.createItem'}];
          newError.path=['Marker_data', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: addMarker_data type
        if(typeof addMarker_data !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addMarker_data ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'marker_data', query: 'addMarker_data', method: 'doSave()', request: 'api.marker_data.createItem'}];
          newError.path=['Marker_data', 'add'];
        ...
        onClose(event, true, addMarker_data);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.marker_data.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
...
MarkerDataCreatePanel.propTypes = {
