...
import MeasurementEnhancedTableHead from './components/MeasurementEnhancedTableHead'
import MeasurementEnhancedTableToolbar from './components/MeasurementEnhancedTableToolbar'
import MeasurementCreatePanel from './components/measurement-create-panel/MeasurementCreatePanel'
import MeasurementUpdatePanel from './components/measurement-update-panel/MeasurementUpdatePanel'
import MeasurementDetailPanel from './components/measurement-detail-panel/MeasurementDetailPanel'
import MeasurementDeleteConfirmationDialog from './components/MeasurementDeleteConfirmationDialog'
import MeasurementUploadFileDialog from './components/MeasurementUploadFileDialog'
import MeasurementCursorPagination from './components/MeasurementCursorPagination'
...
//Plotly
import MeasurementPlotly from '../../../../plots/MeasurementPlotly';
...
export default function MeasurementEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.measurement.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'measurement', query: 'countMeasurements', method: 'getData()', request: 'api.measurement.getCountItems'}];
          newError.path=['Measurements'];
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variant.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'measurement', query: 'measurementsConnection', method: 'getData()', request: 'api.measurement.getItemsConnection'}];
                newError.path=['Measurements'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errors.current.length > 0) {
                ...
                newError.locations=[{model: 'measurement', method: 'getData()'}];
                newError.path=['Measurements'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.path=['Measurements', `id:${item.id}`, 'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.measurement.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  function getCsvTemplate() {
    ...
    /*
      API Request: csvTableTemplateMeasurement
    */
    let cancelableApiReq = makeCancelable(api.measurement.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'measurement', query: 'csvTableTemplateMeasurement', method: 'getCsvTemplate()', request: 'api.measurement.tableTemplate'}];
          newError.path=['Measurements', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: csvTableTemplateMeasurement
        let csvTableTemplateMeasurement = response.data.data.csvTableTemplateMeasurement;
        if(csvTableTemplateMeasurement === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateMeasurement ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'measurement', query: 'csvTableTemplateMeasurement', method: 'getCsvTemplate()', request: 'api.measurement.tableTemplate'}];
          newError.path=['Measurements', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(!Array.isArray(csvTableTemplateMeasurement)) {
          ...
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateMeasurement ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'measurement', query: 'csvTableTemplateMeasurement', method: 'getCsvTemplate()', request: 'api.measurement.tableTemplate'}];
          newError.path=['Measurements', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'csvTableTemplateMeasurement ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'measurement', query: 'csvTableTemplateMeasurement', method: 'getCsvTemplate()', request: 'api.measurement.tableTemplate'}];
          newError.path=['Measurements', 'csvTemplate'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateMeasurement.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "measurement-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.measurement.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <MeasurementEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(MeasurementPlotly)}
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.measurement&&Array.isArray(permissions.measurement)
                                      &&(permissions.measurement.includes('update') || permissions.measurement.includes('*')))
                                      &&(
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.measurement&&Array.isArray(permissions.measurement)
                                      &&(permissions.measurement.includes('delete') || permissions.measurement.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.float_value!==null)?item.float_value:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.int_value!==null)?item.int_value:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.text_value!==null)?item.text_value:'')}
                                    </TableCell>
...
MeasurementEnhancedTable.propTypes = {
