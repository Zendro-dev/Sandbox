...
export default function MeasurementUpdatePanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.measurement[String(item.id)]) {
        ...
        //updated item
        if(lastModelChanged.measurement[String(item.id)].op === "update"&&
            lastModelChanged.measurement[String(item.id)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.measurement[String(item.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function getInitialValues() {
    ...
    initialValues.float_value = item.float_value;
    initialValues.int_value = item.int_value;
    initialValues.text_value = item.text_value;
  ...
  initialValueOkStates.float_value = (item.float_value!==null ? 1 : 0);
  initialValueOkStates.int_value = (item.int_value!==null ? 1 : 0);
  initialValueOkStates.text_value = (item.text_value!==null ? 1 : 0);
  ...
  function getInitialValueAjvStates() {
    ...
    _initialValueAjvStates.float_value = {errors: []};
    _initialValueAjvStates.int_value = {errors: []};
    _initialValueAjvStates.text_value = {errors: []};
  ...
  function areThereChangedFields() {
    ...
    if(values.current.float_value !== item.float_value) { return true;}
    if(Number(values.current.int_value) !== Number(item.int_value)) { return true;}
    if(values.current.text_value !== item.text_value) { return true;}
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.path=['Measurements', `id:${item.id}`, 'update'];
      ...
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": Measurement | id: " + item.id}
          </Typography>
