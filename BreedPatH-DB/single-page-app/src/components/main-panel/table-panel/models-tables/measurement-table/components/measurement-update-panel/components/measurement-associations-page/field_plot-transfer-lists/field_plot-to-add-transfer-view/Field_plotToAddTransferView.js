...
export default function FieldPlotToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.measurement.getAssociatedField_plotConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'measurement', association: 'field_plot', table:'A', query: 'readOneMeasurement', method: 'getData()', request: 'api.measurement.getAssociatedField_plotConnection'}];
        ...
        || typeof readOneMeasurement.field_plot!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneMeasurement.field_plot;
      ...
      //error
      .catch((err) => { //error: on api.measurement.getAssociatedField_plotConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.measurement&&
        lastModelChanged.measurement[String(item.id)]&&
        lastModelChanged.measurement[String(item.id)].changedAssociations&&
        lastModelChanged.measurement[String(item.id)].changedAssociations.field_plot&&
        (lastModelChanged.measurement[String(item.id)].changedAssociations.field_plot.added ||
         lastModelChanged.measurement[String(item.id)].changedAssociations.field_plot.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.measurement[String(item.id)].changedAssociations.field_plot.idsRemoved;
          if(idsRemoved) {
