...
export default function FieldPlotCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.measurement.getField_plotConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'field_plot', query: 'readOneMeasurement', method: 'getData()', request: 'api.measurement.getField_plotConnection'}];
        ...
        //get item
        let it = readOneMeasurement.field_plot;
      ...
      //error
      .catch((err) => { //error: on api.measurement.getField_plotConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
