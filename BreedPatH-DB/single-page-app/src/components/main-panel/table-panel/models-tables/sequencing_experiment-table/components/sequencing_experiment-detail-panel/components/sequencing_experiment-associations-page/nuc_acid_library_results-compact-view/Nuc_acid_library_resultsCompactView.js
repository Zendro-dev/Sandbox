...
import NucAcidLibraryResultsCompactViewToolbar from './components/Nuc_acid_library_resultsCompactViewToolbar';
import NucAcidLibraryResultsCompactViewCursorPagination from './components/Nuc_acid_library_resultsCompactViewCursorPagination';
...
export default function NucAcidLibraryResultsCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.sequencing_experiment.getNuc_acid_library_resultsConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'nuc_acid_library_results', query: 'readOneSequencing_experiment', method: 'getData()', request: 'api.sequencing_experiment.getNuc_acid_library_resultsConnection'}];
          newError.path=['detail', `id:${item.id}`, 'nuc_acid_library_results'];
        ...
        || !Number.isInteger(readOneSequencing_experiment.countFilteredNuc_acid_library_results)
        ...
        || typeof readOneSequencing_experiment.nuc_acid_library_resultsConnection.pageInfo !== 'object'
        || readOneSequencing_experiment.nuc_acid_library_resultsConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneSequencing_experiment.countFilteredNuc_acid_library_results;
        let its = readOneSequencing_experiment.nuc_acid_library_resultsConnection.edges.map(o => o.node);
        let pi = readOneSequencing_experiment.nuc_acid_library_resultsConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.sequencing_experiment.getNuc_acid_library_resultsConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
        ...
        <Grid item xs={12} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <NucAcidLibraryResultsCompactViewToolbar 
              ...
              {(true) && (
                ...
                <NucAcidLibraryResultsCompactViewCursorPagination
                  count={count}
...
NucAcidLibraryResultsCompactView.propTypes = {
  item: PropTypes.object.isRequired,
  handleClickOnNuc_acid_library_resultsRow: PropTypes.func,
};