...
export default function SequencingExperimentCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.end_date = null;
    initialValues.protocol = null;
    initialValues.platform = null;
    initialValues.data_type = null;
    initialValues.library_type = null;
    initialValues.library_preparation = null;
    initialValues.aimed_coverage = null;
    initialValues.resulting_coverage = null;
    initialValues.insert_size = null;
    initialValues.aimed_read_length = null;
    initialValues.genome_complexity_reduction = null;
    initialValues.contamination = null;
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.start_date = 0;
    initialValueOkStates.end_date = 0;
    initialValueOkStates.protocol = 0;
    initialValueOkStates.platform = 0;
    initialValueOkStates.data_type = 0;
    initialValueOkStates.library_type = 0;
    initialValueOkStates.library_preparation = 0;
    initialValueOkStates.aimed_coverage = 0;
    initialValueOkStates.resulting_coverage = 0;
    initialValueOkStates.insert_size = 0;
    initialValueOkStates.aimed_read_length = 0;
    initialValueOkStates.genome_complexity_reduction = 0;
    initialValueOkStates.contamination = 0;
  ...
  function doSave(event) {
    ...
    */
    ...
    /*
      API Request: addSequencing_experiment
    */
    let cancelableApiReq = makeCancelable(api.sequencing_experiment.createItem(graphqlServerUrl, variables));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'sequencing_experiment', query: 'addSequencing_experiment', method: 'doSave()', request: 'api.sequencing_experiment.createItem'}];
          newError.path=['Sequencing_experiments', 'add'];
        ...
        //check: addSequencing_experiment
        let addSequencing_experiment = response.data.data.addSequencing_experiment;
        if(addSequencing_experiment === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addSequencing_experiment ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'sequencing_experiment', query: 'addSequencing_experiment', method: 'doSave()', request: 'api.sequencing_experiment.createItem'}];
          newError.path=['Sequencing_experiments', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: addSequencing_experiment type
        if(typeof addSequencing_experiment !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addSequencing_experiment ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'sequencing_experiment', query: 'addSequencing_experiment', method: 'doSave()', request: 'api.sequencing_experiment.createItem'}];
          newError.path=['Sequencing_experiments', 'add'];
        ...
        onClose(event, true, addSequencing_experiment);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.sequencing_experiment.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'nuc_acid_library_results':
        if(nuc_acid_library_resultsIdsToAdd.current.indexOf(itemId) === -1) {
        ...
        break;
      case 'samples':
        if(samplesIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'nuc_acid_library_results') {
      let iof = nuc_acid_library_resultsIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        nuc_acid_library_resultsIdsToAdd.current.splice(iof, 1);
      }
      return;
    }//end: case 'nuc_acid_library_results'
    if(associationKey === 'samples') {
      let iof = samplesIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        samplesIdsToAdd.current.splice(iof, 1);
      }
      return;
    }//end: case 'samples'
  }
      ...
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Sequencing_experiment'}
          </Typography>
