...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.sequencing_experiment.getSamplesConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'samples', query: 'readOneSequencing_experiment', method: 'getData()', request: 'api.sequencing_experiment.getSamplesConnection'}];
        ...
        || !Number.isInteger(readOneSequencing_experiment.countFilteredSamples)
        ...
        || typeof readOneSequencing_experiment.samplesConnection.pageInfo !== 'object'
        || readOneSequencing_experiment.samplesConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneSequencing_experiment.countFilteredSamples;
        let its = readOneSequencing_experiment.samplesConnection.edges.map(o => o.node);
        let pi = readOneSequencing_experiment.samplesConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.sequencing_experiment.getSamplesConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
