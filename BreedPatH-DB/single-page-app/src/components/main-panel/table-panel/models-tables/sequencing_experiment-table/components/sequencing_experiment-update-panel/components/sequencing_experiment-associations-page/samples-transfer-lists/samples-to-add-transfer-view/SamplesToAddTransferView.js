...
import SamplesToAddTransferViewToolbar from './components/SamplesToAddTransferViewToolbar';
import SamplesToAddTransferViewCursorPagination from './components/SamplesToAddTransferViewCursorPagination';
...
export default function SamplesToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.sequencing_experiment.getAssociatedSamplesConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'sequencing_experiment', association: 'samples', table:'A', query: 'readOneSequencing_experiment', method: 'getData()', request: 'api.sequencing_experiment.getAssociatedSamplesConnection'}];
          newError.path=['add', 'samples'];
        ...
        || typeof readOneSequencing_experiment.samplesConnection !== 'object'
        || readOneSequencing_experiment.samplesConnection === null
        || !Array.isArray(readOneSequencing_experiment.samplesConnection.edges)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let idso = readOneSequencing_experiment.samplesConnection.edges.map(o => o.node);
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'sequencing_experiment', association: 'samples', table:'A', query: 'countSamples', method: 'getData()', request: 'api.sample.getCountItems'}];
              newError.path=['add', 'samples'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'sequencing_experiment', association: 'samples', table:'A', query: 'samplesConnection', method: 'getData()', request: 'api.sample.getItemsConnection'}];
                  newError.path=['add', 'samples'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(samplesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'sequencing_experiment', association: 'samples', table:'A', method: 'getData()'}];
                    newError.path=['add', 'samples'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.sequencing_experiment.getAssociatedSamplesConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'sequencing_experiment', association: 'samples', table:'B', query: 'countSamples', method: 'getDataB()', request: 'api.sample.getCountItems'}];
          newError.path=['add', 'samples'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'sequencing_experiment', association: 'samples', table:'B', query: 'samplesConnection', method: 'getDataB()', request: 'api.sample.getItemsConnection'}];
                newError.path=['add', 'samples'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'sequencing_experiment', association: 'samples', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'samples'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.sequencing_experiment&&
        ...
        lastModelChanged.sequencing_experiment[String(item.id)].changedAssociations.samples&&
        (lastModelChanged.sequencing_experiment[String(item.id)].changedAssociations.samples.added ||
         lastModelChanged.sequencing_experiment[String(item.id)].changedAssociations.samples.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.sequencing_experiment[String(item.id)].changedAssociations.samples.idsRemoved;
          if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.sequencing_experiment.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('samples', idAdded);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <SamplesToAddTransferViewToolbar
                title={'Samples'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <SamplesToAddTransferViewCursorPagination
                count={count}
...
SamplesToAddTransferView.propTypes = {
