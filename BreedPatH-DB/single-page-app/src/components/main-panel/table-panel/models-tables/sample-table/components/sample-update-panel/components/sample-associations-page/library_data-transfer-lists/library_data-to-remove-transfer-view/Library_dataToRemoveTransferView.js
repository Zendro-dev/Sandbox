...
import LibraryDataToRemoveTransferViewToolbar from './components/Library_dataToRemoveTransferViewToolbar';
import LibraryDataToRemoveTransferViewCursorPagination from './components/Library_dataToRemoveTransferViewCursorPagination';
...
export default function LibraryDataToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.sample.getLibrary_dataConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'library_data', query: 'readOneSample', method: 'getData()', request: 'api.sample.getLibrary_dataConnection'}];
          newError.path=['remove', 'library_data'];
        ...
        || !Number.isInteger(readOneSample.countFilteredLibrary_data)
        ...
        || typeof readOneSample.library_dataConnection.pageInfo !== 'object'
        || readOneSample.library_dataConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneSample.countFilteredLibrary_data;
        let its = readOneSample.library_dataConnection.edges.map(o => o.node);
        let pi = readOneSample.library_dataConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.sample.getLibrary_dataConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'nuc_acid_library_result' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <LibraryDataToRemoveTransferViewToolbar
              ...
              {/* Pagination */}
              <LibraryDataToRemoveTransferViewCursorPagination
                count={count}
...
LibraryDataToRemoveTransferView.propTypes = {
