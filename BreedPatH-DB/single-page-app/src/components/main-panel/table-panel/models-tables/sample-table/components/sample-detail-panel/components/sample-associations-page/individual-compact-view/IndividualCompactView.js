...
export default function IndividualCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.sample.getIndividualConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'individual', query: 'readOneSample', method: 'getData()', request: 'api.sample.getIndividualConnection'}];
        ...
        //get item
        let it = readOneSample.individual;
      ...
      //error
      .catch((err) => { //error: on api.sample.getIndividualConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
