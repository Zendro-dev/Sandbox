...
import TranscriptCountsToAddTransferViewToolbar from './components/Transcript_countsToAddTransferViewToolbar';
import TranscriptCountsToAddTransferViewCursorPagination from './components/Transcript_countsToAddTransferViewCursorPagination';
...
export default function TranscriptCountsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    /*
      API Request: readOneSample
    */
    let cancelableApiReq = makeCancelable(api.sample.getAssociatedTranscript_countsConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'readOneSample', method: 'getData()', request: 'api.sample.getAssociatedTranscript_countsConnection'}];
          newError.path=['add', 'transcript_counts'];
        ...
        //check: readOneSample
        let readOneSample = response.data.data.readOneSample;
        if(readOneSample === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneSample ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'readOneSample', method: 'getData()', request: 'api.sample.getAssociatedTranscript_countsConnection'}];
          newError.path=['add', 'transcript_counts'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: readOneSample type
        if(typeof readOneSample !== 'object'
        || typeof readOneSample.transcript_countsConnection !== 'object'
        || readOneSample.transcript_countsConnection === null
        || !Array.isArray(readOneSample.transcript_countsConnection.edges)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneSample ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'readOneSample', method: 'getData()', request: 'api.sample.getAssociatedTranscript_countsConnection'}];
          newError.path=['add', 'transcript_counts'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //get items
        let idso = readOneSample.transcript_countsConnection.edges.map(o => o.node);

        //check: graphql errors
        if(response.data.errors) {
          let newError = {};
          newError.message = 'readOneSample ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'readOneSample', method: 'getData()', request: 'api.sample.getAssociatedTranscript_countsConnection'}];
          newError.path=['add', 'transcript_counts'];
        ...
        /*
          API Request: countTranscript_counts
        */
        let cancelableApiReqB = makeCancelable(api.transcript_count.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'countTranscript_counts', method: 'getData()', request: 'api.transcript_count.getCountItems'}];
              newError.path=['add', 'transcript_counts'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            //check: countTranscript_counts
            let countTranscript_counts = response.data.data.countTranscript_counts;
            if(countTranscript_counts === null) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countTranscript_counts ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
              newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'countTranscript_counts', method: 'getData()', request: 'api.transcript_count.getCountItems'}];
              newError.path=['add', 'transcript_counts'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //check: countTranscript_counts type
            if(!Number.isInteger(countTranscript_counts)) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countTranscript_counts ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
              newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'countTranscript_counts', method: 'getData()', request: 'api.transcript_count.getCountItems'}];
              newError.path=['add', 'transcript_counts'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            if(response.data.errors) {
              let newError = {};
              newError.message = 'countTranscript_counts ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'countTranscript_counts', method: 'getData()', request: 'api.transcript_count.getCountItems'}];
              newError.path=['add', 'transcript_counts'];
              newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //ok
            setCount(countTranscript_counts);


            /*
              API Request: transcript_countsConnection
            */
            let variables = {
              pagination: {
            ...
            let cancelableApiReqC = makeCancelable(api.transcript_count.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'transcript_countsConnection', method: 'getData()', request: 'api.transcript_count.getItemsConnection'}];
                  newError.path=['add', 'transcript_counts'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                //check: transcript_countsConnection
                let transcript_countsConnection = response.data.data.transcript_countsConnection;
                if(transcript_countsConnection === null) {
                  let newError = {};
                  let withDetails=true;
                  variant.current='error';
                  newError.message = 'transcript_countsConnection ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
                  newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'transcript_countsConnection', method: 'getData()', request: 'api.transcript_count.getItemsConnection'}];
                  newError.path=['add', 'transcript_counts'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  ...
                  //check: transcript_countsConnection type
                  if(typeof transcript_countsConnection !== 'object'
                  || !Array.isArray(transcript_countsConnection.edges)
                  || typeof transcript_countsConnection.pageInfo !== 'object' 
                  || transcript_countsConnection.pageInfo === null) {
                    let newError = {};
                    let withDetails=true;
                    variant.current='error';
                    newError.message = 'transcript_countsConnection ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
                    newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'transcript_countsConnection', method: 'getData()', request: 'api.transcript_count.getItemsConnection'}];
                    newError.path=['add', 'transcript_counts'];
                    newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  //get items
                  let its = transcript_countsConnection.edges.map(o => o.node);
                  let pi = transcript_countsConnection.pageInfo;                      

                  //check: graphql errors
                  if(response.data.errors) {
                    let newError = {};
                    newError.message = 'transcript_countsConnection ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
                    newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', query: 'transcript_countsConnection', method: 'getData()', request: 'api.transcript_count.getItemsConnection'}];
                    newError.path=['add', 'transcript_counts'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'sample', association: 'transcript_counts', table:'A', method: 'getData()'}];
                    newError.path=['add', 'transcript_counts'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
                ...
                //error
                .catch((err) => { //error: on api.transcript_count.getItemsConnection
                  if(err.isCanceled) {
                    return;
                  } else {
                    let newError = {};
          ...
          //error
          .catch((err) => { //error: on api.transcript_count.getCountItems
            if(err.isCanceled) {
              return
            } else {
              let newError = {};
      ...
      //error
      .catch((err) => { //error: on api.sample.getAssociatedTranscript_countsConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.transcript_count.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'sample', association: 'transcript_counts', table:'B', query: 'countTranscript_counts', method: 'getDataB()', request: 'api.transcript_count.getCountItems'}];
          newError.path=['add', 'transcript_counts'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
        ...
        //ok
        setCountB(countTranscript_counts);
          ...
          */
          ...
          let cancelableApiReqB = makeCancelable(api.transcript_count.getItemsConnection(
            graphqlServerUrl,
            searchB,
            null, //orderBy
            null, //orderDirection
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'sample', association: 'transcript_counts', table:'B', query: 'transcript_countsConnection', method: 'getDataB()', request: 'api.transcript_count.getItemsConnection'}];
                newError.path=['add', 'transcript_counts'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'sample', association: 'transcript_counts', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'transcript_counts'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'transcript_counts' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'transcript_counts' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.sample&&
        ...
        lastModelChanged.sample[String(item.id)].changedAssociations&&
        lastModelChanged.sample[String(item.id)].changedAssociations.transcript_counts&&
        (lastModelChanged.sample[String(item.id)].changedAssociations.transcript_counts.added ||
         lastModelChanged.sample[String(item.id)].changedAssociations.transcript_counts.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.sample[String(item.id)].changedAssociations.transcript_counts.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('transcript_counts', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'sample' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'transcript_count',
     * but this action happened on the peer relation 'sample'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'transcript_count'.
     * B: the current item internalId is in the addedIds of the updated 'transcript_count'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'transcript_count' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.transcript_count) {
      let oens = Object.entries(lastModelChanged.transcript_count);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.sample) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.sample.removed) {
              let idsRemoved = entry[1].changedAssociations.sample.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.sample.added) {
              let idsAdded = entry[1].changedAssociations.sample.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('transcript_counts', idAdded);
    ...
    /*
     * Case 3: 
     * The attributes of some 'transcript_count' were modified or the item was deleted.
     * 
     * Conditions:
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnTranscript_countRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('transcript_counts', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('transcript_counts', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <TranscriptCountsToAddTransferViewToolbar
                title={'Transcript_counts'}
                titleIcon={1}
                search={search}
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'transcript_count' }>
                                <Avatar>{"transcript_count".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
              ...
              {/* Pagination */}
              <TranscriptCountsToAddTransferViewCursorPagination
                count={count}
...
TranscriptCountsToAddTransferView.propTypes = {
