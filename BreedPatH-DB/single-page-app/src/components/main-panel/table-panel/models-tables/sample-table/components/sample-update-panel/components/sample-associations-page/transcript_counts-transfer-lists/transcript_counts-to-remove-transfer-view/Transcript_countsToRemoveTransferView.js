...
import TranscriptCountsToRemoveTransferViewToolbar from './components/Transcript_countsToRemoveTransferViewToolbar';
import TranscriptCountsToRemoveTransferViewCursorPagination from './components/Transcript_countsToRemoveTransferViewCursorPagination';
...
export default function TranscriptCountsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.sample.getTranscript_countsConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'transcript_counts', query: 'readOneSample', method: 'getData()', request: 'api.sample.getTranscript_countsConnection'}];
          newError.path=['remove', 'transcript_counts'];
        ...
        || !Number.isInteger(readOneSample.countFilteredTranscript_counts)
        ...
        || typeof readOneSample.transcript_countsConnection.pageInfo !== 'object'
        || readOneSample.transcript_countsConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneSample.countFilteredTranscript_counts;
        let its = readOneSample.transcript_countsConnection.edges.map(o => o.node);
        let pi = readOneSample.transcript_countsConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.sample.getTranscript_countsConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'transcript_count' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <TranscriptCountsToRemoveTransferViewToolbar
              ...
              {/* Pagination */}
              <TranscriptCountsToRemoveTransferViewCursorPagination
                count={count}
...
TranscriptCountsToRemoveTransferView.propTypes = {
