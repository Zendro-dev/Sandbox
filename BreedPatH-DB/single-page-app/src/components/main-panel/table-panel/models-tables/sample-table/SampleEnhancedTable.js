...
import SampleEnhancedTableHead from './components/SampleEnhancedTableHead'
import SampleEnhancedTableToolbar from './components/SampleEnhancedTableToolbar'
import SampleCreatePanel from './components/sample-create-panel/SampleCreatePanel'
import SampleUpdatePanel from './components/sample-update-panel/SampleUpdatePanel'
import SampleDetailPanel from './components/sample-detail-panel/SampleDetailPanel'
import SampleDeleteConfirmationDialog from './components/SampleDeleteConfirmationDialog'
import SampleUploadFileDialog from './components/SampleUploadFileDialog'
import SampleCursorPagination from './components/SampleCursorPagination'
...
//Plotly
import SamplePlotly from '../../../../plots/SamplePlotly';
...
export default function SampleEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.sample.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'sample', query: 'countSamples', method: 'getData()', request: 'api.sample.getCountItems'}];
          newError.path=['Samples'];
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variant.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'sample', query: 'samplesConnection', method: 'getData()', request: 'api.sample.getItemsConnection'}];
                newError.path=['Samples'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errors.current.length > 0) {
                ...
                newError.locations=[{model: 'sample', method: 'getData()'}];
                newError.path=['Samples'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on api.sample.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  function getCsvTemplate() {
    ...
    /*
      API Request: csvTableTemplateSample
    */
    let cancelableApiReq = makeCancelable(api.sample.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'sample', query: 'csvTableTemplateSample', method: 'getCsvTemplate()', request: 'api.sample.tableTemplate'}];
        ...
        //check: csvTableTemplateSample
        let csvTableTemplateSample = response.data.data.csvTableTemplateSample;
        if(csvTableTemplateSample === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateSample ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'sample', query: 'csvTableTemplateSample', method: 'getCsvTemplate()', request: 'api.sample.tableTemplate'}];
          newError.path=['Samples', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: csvTableTemplateSample type
        if(!Array.isArray(csvTableTemplateSample)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateSample ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'sample', query: 'csvTableTemplateSample', method: 'getCsvTemplate()', request: 'api.sample.tableTemplate'}];
          newError.path=['Samples', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'csvTableTemplateSample ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'sample', query: 'csvTableTemplateSample', method: 'getCsvTemplate()', request: 'api.sample.tableTemplate'}];
          newError.path=['Samples', 'csvTemplate'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateSample.join("\n");
      ...
      //error
      .catch((err) => { //error: on api.sample.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <SampleEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(SamplePlotly)}
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.sample&&Array.isArray(permissions.sample)
                                      &&(permissions.sample.includes('update') || permissions.sample.includes('*')))
                                      &&(
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.sample&&Array.isArray(permissions.sample)
                                      &&(permissions.sample.includes('delete') || permissions.sample.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.sampling_date!==null)?item.sampling_date:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.biological_replicate_no!==null)?item.biological_replicate_no:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.treatment!==null)?item.treatment:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.tissue!==null)?item.tissue:'')}
                                    </TableCell>
