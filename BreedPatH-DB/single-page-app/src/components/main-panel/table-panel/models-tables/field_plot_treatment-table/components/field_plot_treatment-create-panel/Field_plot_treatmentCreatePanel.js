...
export default function FieldPlotTreatmentCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.pesticide_type = null;
    initialValues.field_plot_id = null;
  ...
  function setAddField_plot(variables) {
  ...
  function doSave(event) {
    ...
    */
    ...
    /*
      API Request: addField_plot_treatment
    */
    let cancelableApiReq = makeCancelable(api.field_plot_treatment.createItem(graphqlServerUrl, variables));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'field_plot_treatment', query: 'addField_plot_treatment', method: 'doSave()', request: 'api.field_plot_treatment.createItem'}];
          newError.path=['Field_plot_treatments', 'add'];
        ...
        //check: addField_plot_treatment
        let addField_plot_treatment = response.data.data.addField_plot_treatment;
        if(addField_plot_treatment === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addField_plot_treatment ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'field_plot_treatment', query: 'addField_plot_treatment', method: 'doSave()', request: 'api.field_plot_treatment.createItem'}];
          newError.path=['Field_plot_treatments', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: addField_plot_treatment type
        if(typeof addField_plot_treatment !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addField_plot_treatment ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'field_plot_treatment', query: 'addField_plot_treatment', method: 'doSave()', request: 'api.field_plot_treatment.createItem'}];
          newError.path=['Field_plot_treatments', 'add'];
        ...
        onClose(event, true, addField_plot_treatment);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.field_plot_treatment.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
      ...
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Field_plot_treatment'}
          </Typography>
...
FieldPlotTreatmentCreatePanel.propTypes = {
