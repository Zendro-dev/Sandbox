...
export default function FieldPlotToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    /*
      API Request: readOneField_plot_treatment
    */
    let cancelableApiReq = makeCancelable(api.field_plot_treatment.getAssociatedField_plotConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'A', query: 'readOneField_plot_treatment', method: 'getData()', request: 'api.field_plot_treatment.getAssociatedField_plotConnection'}];
        ...
        //check: readOneField_plot_treatment
        let readOneField_plot_treatment = response.data.data.readOneField_plot_treatment;
        if(readOneField_plot_treatment === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneField_plot_treatment ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'A', query: 'readOneField_plot_treatment', method: 'getData()', request: 'api.field_plot_treatment.getAssociatedField_plotConnection'}];
          newError.path=['add', 'field_plot'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: readOneField_plot_treatment type
        if(typeof readOneField_plot_treatment !== 'object'
        || typeof readOneField_plot_treatment.field_plot!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneField_plot_treatment ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'A', query: 'readOneField_plot_treatment', method: 'getData()', request: 'api.field_plot_treatment.getAssociatedField_plotConnection'}];
          newError.path=['add', 'field_plot'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //get item
        let idso = readOneField_plot_treatment.field_plot;

        //check: graphql errors
        if(response.data.errors) {
          let newError = {};
          newError.message = 'readOneField_plot_treatment ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'A', query: 'readOneField_plot_treatment', method: 'getData()', request: 'api.field_plot_treatment.getAssociatedField_plotConnection'}];
          newError.path=['add', 'field_plot'];
        ...
        */
        let cancelableApiReqB = makeCancelable(api.field_plot.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'A', query: 'countField_plots', method: 'getData()', request: 'api.field_plot.getCountItems'}];
              newError.path=['add', 'field_plot'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.field_plot.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'A', query: 'field_plotsConnection', method: 'getData()', request: 'api.field_plot.getItemsConnection'}];
                  newError.path=['add', 'field_plot'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(field_plotsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'A', method: 'getData()'}];
                    newError.path=['add', 'field_plot'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.field_plot_treatment.getAssociatedField_plotConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'B', query: 'countField_plots', method: 'getDataB()', request: 'api.field_plot.getCountItems'}];
          newError.path=['add', 'field_plot'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'B', query: 'field_plotsConnection', method: 'getDataB()', request: 'api.field_plot.getItemsConnection'}];
                newError.path=['add', 'field_plot'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'field_plot_treatment', association: 'field_plot', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'field_plot'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'field_plot' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'field_plots' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.field_plot_treatment&&
        ...
        lastModelChanged.field_plot_treatment[String(item.id)].changedAssociations&&
        lastModelChanged.field_plot_treatment[String(item.id)].changedAssociations.field_plot&&
        (lastModelChanged.field_plot_treatment[String(item.id)].changedAssociations.field_plot.added ||
         lastModelChanged.field_plot_treatment[String(item.id)].changedAssociations.field_plot.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.field_plot_treatment[String(item.id)].changedAssociations.field_plot.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'field_plot_treatment' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'field_plot',
     * but this action happened on the peer relation 'field_plot_treatment'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'field_plot'.
     * B: the current item internalId is in the addedIds of the updated 'field_plot'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'field_plot' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.field_plot) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.field_plot_treatment) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.field_plot_treatment.removed) {
              let idsRemoved = entry[1].changedAssociations.field_plot_treatment.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.field_plot_treatment.added) {
              let idsAdded = entry[1].changedAssociations.field_plot_treatment.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('field_plot', idAdded);
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.id===it.id) ? 'field_plot — ' + t('modelPanels.associatedRecord', 'Associated record') : 'field_plot '}>
                                <Avatar>
                                  {"field_plot".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
