...
import FieldPlotCompactViewToolbar from './components/Field_plotCompactViewToolbar';
import FieldPlotCompactViewCursorPagination from './components/Field_plotCompactViewCursorPagination';
...
export default function FieldPlotCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let sublabel = 'coordinates_or_name';
    let variables = {
      pagination: {
    ...
    let cancelableApiReq = makeCancelable(api.field_plot_treatment.getField_plotConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'field_plot', query: 'readOneField_plot_treatment', method: 'getData()', request: 'api.field_plot_treatment.getField_plotConnection'}];
          newError.path=['detail', `id:${item.id}`, 'field_plot'];
        ...
        //get item
        let it = readOneField_plot_treatment.field_plot;
      ...
      //error
      .catch((err) => { //error: on api.field_plot_treatment.getField_plotConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * 
     * Conditions:
     * A: the current item 'internalId' attribute is in the removedIds of the updated 'field_plot'.
     * B: the current item 'internalId' attribute is in the addedIds of the updated 'field_plot'.
     * 
     * Actions:
     * if A:
     * - reload table.
          ...
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {(false) && (
                ...
                <FieldPlotCompactViewCursorPagination
                  count={count}
...
FieldPlotCompactView.propTypes = {
  item: PropTypes.object.isRequired,
  handleClickOnField_plotRow: PropTypes.func,
};