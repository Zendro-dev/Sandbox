...
import FieldPlotTreatmentUpdatePanel from '../field_plot_treatment-update-panel/Field_plot_treatmentUpdatePanel'
import FieldPlotTreatmentDeleteConfirmationDialog from '../Field_plot_treatmentDeleteConfirmationDialog'
...
export default function FieldPlotTreatmentDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.field_plot_treatment[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.field_plot_treatment[String(itemState.id)].op === "update"&&
            lastModelChanged.field_plot_treatment[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.field_plot_treatment[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.field_plot_treatment[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteField_plot_treatment
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('field_plot_treatment', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('field_plot_treatment', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Field_plot_treatment | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
...
FieldPlotTreatmentDetailPanel.propTypes = {
