...
export default function SampleCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.transcript_count.getSampleConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'sample', query: 'readOneTranscript_count', method: 'getData()', request: 'api.transcript_count.getSampleConnection'}];
        ...
        //get item
        let it = readOneTranscript_count.sample;
      ...
      //error
      .catch((err) => { //error: on api.transcript_count.getSampleConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
