...
export default function SampleToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    /*
      API Request: readOneTranscript_count
    */
    let cancelableApiReq = makeCancelable(api.transcript_count.getAssociatedSampleConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'transcript_count', association: 'sample', table:'A', query: 'readOneTranscript_count', method: 'getData()', request: 'api.transcript_count.getAssociatedSampleConnection'}];
        ...
        //check: readOneTranscript_count
        let readOneTranscript_count = response.data.data.readOneTranscript_count;
        if(readOneTranscript_count === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneTranscript_count ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'transcript_count', association: 'sample', table:'A', query: 'readOneTranscript_count', method: 'getData()', request: 'api.transcript_count.getAssociatedSampleConnection'}];
          newError.path=['add', 'sample'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: readOneTranscript_count type
        if(typeof readOneTranscript_count !== 'object'
        || typeof readOneTranscript_count.sample!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneTranscript_count ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'transcript_count', association: 'sample', table:'A', query: 'readOneTranscript_count', method: 'getData()', request: 'api.transcript_count.getAssociatedSampleConnection'}];
          newError.path=['add', 'sample'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //get item
        let idso = readOneTranscript_count.sample;

        //check: graphql errors
        if(response.data.errors) {
          let newError = {};
          newError.message = 'readOneTranscript_count ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'transcript_count', association: 'sample', table:'A', query: 'readOneTranscript_count', method: 'getData()', request: 'api.transcript_count.getAssociatedSampleConnection'}];
          newError.path=['add', 'sample'];
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'transcript_count', association: 'sample', table:'A', query: 'countSamples', method: 'getData()', request: 'api.sample.getCountItems'}];
              newError.path=['add', 'sample'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'transcript_count', association: 'sample', table:'A', query: 'samplesConnection', method: 'getData()', request: 'api.sample.getItemsConnection'}];
                  newError.path=['add', 'sample'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(samplesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'transcript_count', association: 'sample', table:'A', method: 'getData()'}];
                    newError.path=['add', 'sample'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.transcript_count.getAssociatedSampleConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'transcript_count', association: 'sample', table:'B', query: 'countSamples', method: 'getDataB()', request: 'api.sample.getCountItems'}];
          newError.path=['add', 'sample'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'transcript_count', association: 'sample', table:'B', query: 'samplesConnection', method: 'getDataB()', request: 'api.sample.getItemsConnection'}];
                newError.path=['add', 'sample'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'transcript_count', association: 'sample', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'sample'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.transcript_count&&
        lastModelChanged.transcript_count[String(item.id)]&&
        lastModelChanged.transcript_count[String(item.id)].changedAssociations&&
        lastModelChanged.transcript_count[String(item.id)].changedAssociations.sample&&
        (lastModelChanged.transcript_count[String(item.id)].changedAssociations.sample.added ||
         lastModelChanged.transcript_count[String(item.id)].changedAssociations.sample.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.transcript_count[String(item.id)].changedAssociations.sample.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'transcript_counts' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'sample',
     * but this action happened on the peer relation 'transcript_counts'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.sample) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.transcript_counts) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.transcript_counts.removed) {
              let idsRemoved = entry[1].changedAssociations.transcript_counts.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.transcript_counts.added) {
              let idsAdded = entry[1].changedAssociations.transcript_counts.idsAdded;
              if(idsAdded) {
