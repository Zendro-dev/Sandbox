...
import TranscriptCountAttributesPage from './components/transcript_count-attributes-page/Transcript_countAttributesPage'
import TranscriptCountAssociationsPage from './components/transcript_count-associations-page/Transcript_countAssociationsPage'
import TranscriptCountUpdatePanel from '../transcript_count-update-panel/Transcript_countUpdatePanel'
import TranscriptCountDeleteConfirmationDialog from '../Transcript_countDeleteConfirmationDialog'
...
export default function TranscriptCountDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged.transcript_count&&
      lastModelChanged.transcript_count[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.transcript_count[String(itemState.id)].op === "update"&&
            lastModelChanged.transcript_count[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.transcript_count[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.transcript_count[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteTranscript_count
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  function getInitialValueOkStates(item) {
    ...
    */
    ...
    initialValueOkStates.gene = (item.gene!==null ? 1 : 0);
    initialValueOkStates.value = (item.value!==null ? 1 : 0);
    initialValueOkStates.method = (item.method!==null ? 1 : 0);
    initialValueOkStates.reference_genome = (item.reference_genome!==null ? 1 : 0);
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('transcript_count', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('transcript_count', 'delete', item, null))
        ...
        >
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Transcript_count | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
          ...
          <div className={classes.root}>
            <Grid container justify='center' alignItems='flex-start' alignContent='flex-start' spacing={0}>
              ...
              <Grid item xs={12}>
                {/* Associations Page */}
                <TranscriptCountAssociationsPage
                  item={itemState}
                  deleted={deleted}
...
TranscriptCountDetailPanel.propTypes = {
