...
import TranscriptCountTabsA from './components/Transcript_countTabsA'
import TranscriptCountConfirmationDialog from './components/Transcript_countConfirmationDialog'
...
export default function TranscriptCountCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.gene = null;
    initialValues.value = null;
    initialValues.method = null;
    initialValues.reference_genome = null;
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.gene = 0;
    initialValueOkStates.value = 0;
    initialValueOkStates.method = 0;
    initialValueOkStates.reference_genome = 0;
  ...
  function getInitialValueAjvStates() {
    ...
    _initialValueAjvStates.gene = {errors: []};
    _initialValueAjvStates.value = {errors: []};
    _initialValueAjvStates.method = {errors: []};
    _initialValueAjvStates.reference_genome = {errors: []};
  ...
  function doSave(event) {
    ...
    */
    ...
    /*
      API Request: addTranscript_count
    */
    let cancelableApiReq = makeCancelable(api.transcript_count.createItem(graphqlServerUrl, variables));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'transcript_count', query: 'addTranscript_count', method: 'doSave()', request: 'api.transcript_count.createItem'}];
          newError.path=['Transcript_counts', 'add'];
        ...
        //check: addTranscript_count
        let addTranscript_count = response.data.data.addTranscript_count;
        if(addTranscript_count === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addTranscript_count ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'transcript_count', query: 'addTranscript_count', method: 'doSave()', request: 'api.transcript_count.createItem'}];
          newError.path=['Transcript_counts', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: addTranscript_count type
        if(typeof addTranscript_count !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'transcript_count ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'transcript_count', query: 'addTranscript_count', method: 'doSave()', request: 'api.transcript_count.createItem'}];
          newError.path=['Transcript_counts', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addTranscript_count ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'transcript_count', query: 'addTranscript_count', method: 'doSave()', request: 'api.transcript_count.createItem'}];
          newError.path=['Transcript_counts', 'add'];
        ...
        onClose(event, true, addTranscript_count);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.transcript_count.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  return (
    ...
    >
      ...
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Transcript_count'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " transcript_count" }>
            <Fab 
              color="secondary" 
      ...
      <div className={classes.root}>
        ...
        {/* Confirmation Dialog */}
        <TranscriptCountConfirmationDialog
...
TranscriptCountCreatePanel.propTypes = {
