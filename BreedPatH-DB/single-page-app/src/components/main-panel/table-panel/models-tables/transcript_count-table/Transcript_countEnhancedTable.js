...
import TranscriptCountEnhancedTableHead from './components/Transcript_countEnhancedTableHead'
import TranscriptCountEnhancedTableToolbar from './components/Transcript_countEnhancedTableToolbar'
import TranscriptCountCreatePanel from './components/transcript_count-create-panel/Transcript_countCreatePanel'
import TranscriptCountUpdatePanel from './components/transcript_count-update-panel/Transcript_countUpdatePanel'
import TranscriptCountDetailPanel from './components/transcript_count-detail-panel/Transcript_countDetailPanel'
import TranscriptCountDeleteConfirmationDialog from './components/Transcript_countDeleteConfirmationDialog'
import TranscriptCountUploadFileDialog from './components/Transcript_countUploadFileDialog'
import TranscriptCountCursorPagination from './components/Transcript_countCursorPagination'
...
//Plotly
import TranscriptCountPlotly from '../../../../plots/Transcript_countPlotly';
...
export default function TranscriptCountEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.transcript_count.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'transcript_count', query: 'countTranscript_counts', method: 'getData()', request: 'api.transcript_count.getCountItems'}];
          newError.path=['Transcript_counts'];
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variant.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'transcript_count', query: 'transcript_countsConnection', method: 'getData()', request: 'api.transcript_count.getItemsConnection'}];
                newError.path=['Transcript_counts'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errors.current.length > 0) {
                ...
                newError.locations=[{model: 'transcript_count', method: 'getData()'}];
                newError.path=['Transcript_counts'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
  ...
  function doDelete(event, item) {
    ...
    /*
      API Request: deleteTranscript_count
    */
    let cancelableApiReq = makeCancelable(api.transcript_count.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'transcript_count', query: 'deleteTranscript_count', method: 'doDelete()', request: 'api.transcript_count.deleteItem'}];
          newError.path=['Transcript_counts', `id:${item.id}`, 'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: deleteTranscript_count
        let deleteTranscript_count = response.data.data.deleteTranscript_count;
        if(deleteTranscript_count === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'deleteTranscript_count ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'transcript_count', query: 'deleteTranscript_count', method: 'doDelete()', request: 'api.transcript_count.deleteItem'}];
          newError.path=['Transcript_counts', `id:${item.id}` ,'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        /**
         * Type of deleteTranscript_count is not validated. Only not null is
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'deleteTranscript_count ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'transcript_count', query: 'deleteTranscript_count', method: 'doDelete()', request: 'api.transcript_count.deleteItem'}];
          newError.path=['Transcript_counts', `id:${item.id}` ,'delete'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
      ...
      //error
      .catch((err) => { //error: on api.transcript_count.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  function getCsvTemplate() {
    ...
    /*
      API Request: csvTableTemplateTranscript_count
    */
    let cancelableApiReq = makeCancelable(api.transcript_count.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'transcript_count', query: 'csvTableTemplateTranscript_count', method: 'getCsvTemplate()', request: 'api.transcript_count.tableTemplate'}];
          newError.path=['Transcript_counts', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: csvTableTemplateTranscript_count
        let csvTableTemplateTranscript_count = response.data.data.csvTableTemplateTranscript_count;
        if(csvTableTemplateTranscript_count === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateTranscript_count ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'transcript_count', query: 'csvTableTemplateTranscript_count', method: 'getCsvTemplate()', request: 'api.transcript_count.tableTemplate'}];
          newError.path=['Transcript_counts', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: csvTableTemplateTranscript_count type
        if(!Array.isArray(csvTableTemplateTranscript_count)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateTranscript_count ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'transcript_count', query: 'csvTableTemplateTranscript_count', method: 'getCsvTemplate()', request: 'api.transcript_count.tableTemplate'}];
          newError.path=['Transcript_counts', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'csvTableTemplateTranscript_count ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'transcript_count', query: 'csvTableTemplateTranscript_count', method: 'getCsvTemplate()', request: 'api.transcript_count.tableTemplate'}];
          newError.path=['Transcript_counts', 'csvTemplate'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateTranscript_count.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "transcript_count-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.transcript_count.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <TranscriptCountEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(TranscriptCountPlotly)}
                ...
                <SwipeableViews index={getSwipeIndex()} disabled={true}>
                  ...
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      <Table stickyHeader size='small' ref={tref}>
                        ...
                        {/* Table Head */}
                        <TranscriptCountEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                          rowCount={count}
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.transcript_count&&Array.isArray(permissions.transcript_count)
                                      &&(permissions.transcript_count.includes('update') || permissions.transcript_count.includes('*')))
                                      &&(
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.transcript_count&&Array.isArray(permissions.transcript_count)
                                      &&(permissions.transcript_count.includes('delete') || permissions.transcript_count.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.gene!==null)?item.gene:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.reference_genome!==null)?item.reference_genome:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.sample_id!==null)?item.sample_id:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <TranscriptCountCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(TranscriptCountPlotly)) && (
                      <TranscriptCountPlotly />
                    )}
                  </div>
                </SwipeableViews>
              </Paper>
      ...
      {(deleteConfirmationDialogOpen) && (
        <TranscriptCountDeleteConfirmationDialog
          permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <TranscriptCountUploadFileDialog
...
TranscriptCountEnhancedTable.propTypes = {
