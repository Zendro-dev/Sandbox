...
export default function FieldPlotToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.genotype.getAssociatedField_plotConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'genotype', association: 'field_plot', table:'A', query: 'readOneGenotype', method: 'getData()', request: 'api.genotype.getAssociatedField_plotConnection'}];
        ...
        //get items
        let idso = readOneGenotype.field_plotConnection.edges.map(o => o.node);
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'genotype', association: 'field_plot', table:'A', query: 'countField_plots', method: 'getData()', request: 'api.field_plot.getCountItems'}];
              newError.path=['add', 'field_plot'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'genotype', association: 'field_plot', table:'A', query: 'field_plotsConnection', method: 'getData()', request: 'api.field_plot.getItemsConnection'}];
                  newError.path=['add', 'field_plot'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(field_plotsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'genotype', association: 'field_plot', table:'A', method: 'getData()'}];
                    newError.path=['add', 'field_plot'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.genotype.getAssociatedField_plotConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
