...
import FieldPlotToRemoveTransferViewToolbar from './components/Field_plotToRemoveTransferViewToolbar';
import FieldPlotToRemoveTransferViewCursorPagination from './components/Field_plotToRemoveTransferViewCursorPagination';
...
export default function FieldPlotToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.genotype.getField_plotConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'field_plot', query: 'readOneGenotype', method: 'getData()', request: 'api.genotype.getField_plotConnection'}];
          newError.path=['remove', 'field_plot'];
        ...
        || !Number.isInteger(readOneGenotype.countFilteredField_plot)
        || typeof readOneGenotype.field_plotConnection !== 'object'
        || readOneGenotype.field_plotConnection === null
        || !Array.isArray(readOneGenotype.field_plotConnection.edges)
        || typeof readOneGenotype.field_plotConnection.pageInfo !== 'object'
        || readOneGenotype.field_plotConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneGenotype.countFilteredField_plot;
        let its = readOneGenotype.field_plotConnection.edges.map(o => o.node);
        let pi = readOneGenotype.field_plotConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.genotype.getField_plotConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'genotype', association: 'field_plot', table:'B', query: 'countField_plots', method: 'getDataB()', request: 'api.field_plot.getCountItems'}];
          newError.path=['remove', 'field_plot'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'genotype', association: 'field_plot', table:'B', query: 'field_plotsConnection', method: 'getDataB()', request: 'api.field_plot.getItemsConnection'}];
                newError.path=['remove', 'field_plot'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'genotype', association: 'field_plot', table:'B', method: 'getDataB()'}];
                newError.path=['remove', 'field_plot'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.genotype&&
        ...
        lastModelChanged.genotype[String(item.id)].changedAssociations.field_plot&&
        (lastModelChanged.genotype[String(item.id)].changedAssociations.field_plot.added ||
         lastModelChanged.genotype[String(item.id)].changedAssociations.field_plot.removed)) {
          
          //remove any dis-associated id from idsToRemove[]
          let idsRemoved = lastModelChanged.genotype[String(item.id)].changedAssociations.field_plot.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'genotype' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'field_plot',
     * but this action happened on the peer relation 'genotype'.
     * 
     * Conditions:
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'field_plot' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
    ...
    if(lastModelChanged.field_plot) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.genotype) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.genotype.removed) {
              let idsRemoved = entry[1].changedAssociations.genotype.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.genotype.added) {
              let idsAdded = entry[1].changedAssociations.genotype.idsAdded;
              if(idsAdded) {
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <FieldPlotToRemoveTransferViewToolbar
                title={'Field_plots'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <FieldPlotToRemoveTransferViewCursorPagination
                count={count}
...
FieldPlotToRemoveTransferView.propTypes = {
