...
import GenotypeUpdatePanel from '../genotype-update-panel/GenotypeUpdatePanel'
import GenotypeDeleteConfirmationDialog from '../GenotypeDeleteConfirmationDialog'
...
export default function GenotypeDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.genotype[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.genotype[String(itemState.id)].op === "update"&&
            lastModelChanged.genotype[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.genotype[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.genotype[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteGenotype
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('genotype', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('genotype', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Genotype | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
