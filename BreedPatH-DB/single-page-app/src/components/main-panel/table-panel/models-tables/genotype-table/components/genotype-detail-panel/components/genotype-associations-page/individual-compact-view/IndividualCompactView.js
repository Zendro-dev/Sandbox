...
export default function IndividualCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.genotype.getIndividualConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'individual', query: 'readOneGenotype', method: 'getData()', request: 'api.genotype.getIndividualConnection'}];
        ...
        //get item
        let it = readOneGenotype.individual;
      ...
      //error
      .catch((err) => { //error: on api.genotype.getIndividualConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
