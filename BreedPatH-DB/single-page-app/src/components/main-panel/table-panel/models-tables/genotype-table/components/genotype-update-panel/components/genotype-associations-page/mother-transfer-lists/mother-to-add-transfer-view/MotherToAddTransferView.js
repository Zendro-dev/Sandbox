...
import MotherToAddTransferViewToolbar from './components/MotherToAddTransferViewToolbar';
import MotherToAddTransferViewCursorPagination from './components/MotherToAddTransferViewCursorPagination';
...
export default function MotherToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.genotype.getAssociatedMotherConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'genotype', association: 'mother', table:'A', query: 'readOneGenotype', method: 'getData()', request: 'api.genotype.getAssociatedMotherConnection'}];
          newError.path=['add', 'mother'];
        ...
        || typeof readOneGenotype.mother!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneGenotype.mother;
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'genotype', association: 'mother', table:'A', query: 'countIndividuals', method: 'getData()', request: 'api.individual.getCountItems'}];
              newError.path=['add', 'mother'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'genotype', association: 'mother', table:'A', query: 'individualsConnection', method: 'getData()', request: 'api.individual.getItemsConnection'}];
                  newError.path=['add', 'mother'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(individualsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'genotype', association: 'mother', table:'A', method: 'getData()'}];
                    newError.path=['add', 'mother'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.genotype.getAssociatedMotherConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'genotype', association: 'mother', table:'B', query: 'countIndividuals', method: 'getDataB()', request: 'api.individual.getCountItems'}];
          newError.path=['add', 'mother'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'genotype', association: 'mother', table:'B', query: 'individualsConnection', method: 'getDataB()', request: 'api.individual.getItemsConnection'}];
                newError.path=['add', 'mother'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'genotype', association: 'mother', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'mother'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'mother' for this item was updated.
    ...
    if(lastModelChanged.genotype&&
        ...
        lastModelChanged.genotype[String(item.id)].changedAssociations.mother&&
        (lastModelChanged.genotype[String(item.id)].changedAssociations.mother.added ||
         lastModelChanged.genotype[String(item.id)].changedAssociations.mother.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.genotype[String(item.id)].changedAssociations.mother.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('mother', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'mother_to' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'individual',
     * but this action happened on the peer relation 'mother_to'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.individual) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.mother_to) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.mother_to.removed) {
              let idsRemoved = entry[1].changedAssociations.mother_to.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.mother_to.added) {
              let idsAdded = entry[1].changedAssociations.mother_to.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('mother', idAdded);
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('mother', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MotherToAddTransferViewToolbar
                title={'Individuals'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <MotherToAddTransferViewCursorPagination
                count={count}
...
MotherToAddTransferView.propTypes = {
