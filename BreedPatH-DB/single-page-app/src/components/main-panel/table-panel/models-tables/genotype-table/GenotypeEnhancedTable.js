...
import GenotypeEnhancedTableHead from './components/GenotypeEnhancedTableHead'
import GenotypeEnhancedTableToolbar from './components/GenotypeEnhancedTableToolbar'
import GenotypeCreatePanel from './components/genotype-create-panel/GenotypeCreatePanel'
import GenotypeUpdatePanel from './components/genotype-update-panel/GenotypeUpdatePanel'
import GenotypeDetailPanel from './components/genotype-detail-panel/GenotypeDetailPanel'
import GenotypeDeleteConfirmationDialog from './components/GenotypeDeleteConfirmationDialog'
import GenotypeUploadFileDialog from './components/GenotypeUploadFileDialog'
import GenotypeCursorPagination from './components/GenotypeCursorPagination'
...
//Plotly
import GenotypePlotly from '../../../../plots/GenotypePlotly';
...
export default function GenotypeEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.genotype.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'genotype', query: 'countGenotypes', method: 'getData()', request: 'api.genotype.getCountItems'}];
          newError.path=['Genotypes'];
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variant.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'genotype', query: 'genotypesConnection', method: 'getData()', request: 'api.genotype.getItemsConnection'}];
                newError.path=['Genotypes'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errors.current.length > 0) {
                ...
                newError.locations=[{model: 'genotype', method: 'getData()'}];
                newError.path=['Genotypes'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
  ...
  function doDelete(event, item) {
    ...
    /*
      API Request: deleteGenotype
    */
    let cancelableApiReq = makeCancelable(api.genotype.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'genotype', query: 'deleteGenotype', method: 'doDelete()', request: 'api.genotype.deleteItem'}];
          newError.path=['Genotypes', `id:${item.id}`, 'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: deleteGenotype
        let deleteGenotype = response.data.data.deleteGenotype;
        if(deleteGenotype === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'deleteGenotype ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'genotype', query: 'deleteGenotype', method: 'doDelete()', request: 'api.genotype.deleteItem'}];
          newError.path=['Genotypes', `id:${item.id}` ,'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        /**
         * Type of deleteGenotype is not validated. Only not null is
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'deleteGenotype ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'genotype', query: 'deleteGenotype', method: 'doDelete()', request: 'api.genotype.deleteItem'}];
          newError.path=['Genotypes', `id:${item.id}` ,'delete'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
      ...
      //error
      .catch((err) => { //error: on api.genotype.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  function getCsvTemplate() {
    ...
    /*
      API Request: csvTableTemplateGenotype
    */
    let cancelableApiReq = makeCancelable(api.genotype.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'genotype', query: 'csvTableTemplateGenotype', method: 'getCsvTemplate()', request: 'api.genotype.tableTemplate'}];
          newError.path=['Genotypes', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: csvTableTemplateGenotype
        let csvTableTemplateGenotype = response.data.data.csvTableTemplateGenotype;
        if(csvTableTemplateGenotype === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateGenotype ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'genotype', query: 'csvTableTemplateGenotype', method: 'getCsvTemplate()', request: 'api.genotype.tableTemplate'}];
          newError.path=['Genotypes', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: csvTableTemplateGenotype type
        if(!Array.isArray(csvTableTemplateGenotype)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateGenotype ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'genotype', query: 'csvTableTemplateGenotype', method: 'getCsvTemplate()', request: 'api.genotype.tableTemplate'}];
          newError.path=['Genotypes', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'csvTableTemplateGenotype ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'genotype', query: 'csvTableTemplateGenotype', method: 'getCsvTemplate()', request: 'api.genotype.tableTemplate'}];
          newError.path=['Genotypes', 'csvTemplate'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateGenotype.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "genotype-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.genotype.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <GenotypeEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(GenotypePlotly)}
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.genotype&&Array.isArray(permissions.genotype)
                                      &&(permissions.genotype.includes('update') || permissions.genotype.includes('*')))
                                      &&(
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.genotype&&Array.isArray(permissions.genotype)
                                      &&(permissions.genotype.includes('delete') || permissions.genotype.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.pedigree_type!==null)?item.pedigree_type:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.mother_id!==null)?item.mother_id:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.father_id!==null)?item.father_id:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.breeding_pool_id!==null)?item.breeding_pool_id:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <GenotypeDeleteConfirmationDialog
          permissions={permissions}
...
GenotypeEnhancedTable.propTypes = {
