...
import BreedingPoolToAddTransferViewToolbar from './components/Breeding_poolToAddTransferViewToolbar';
import BreedingPoolToAddTransferViewCursorPagination from './components/Breeding_poolToAddTransferViewCursorPagination';
...
export default function BreedingPoolToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.genotype.getAssociatedBreeding_poolConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'genotype', association: 'breeding_pool', table:'A', query: 'readOneGenotype', method: 'getData()', request: 'api.genotype.getAssociatedBreeding_poolConnection'}];
          newError.path=['add', 'breeding_pool'];
        ...
        || typeof readOneGenotype.breeding_pool!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneGenotype.breeding_pool;
        ...
        */
        let cancelableApiReqB = makeCancelable(api.breeding_pool.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'genotype', association: 'breeding_pool', table:'A', query: 'countBreeding_pools', method: 'getData()', request: 'api.breeding_pool.getCountItems'}];
              newError.path=['add', 'breeding_pool'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.breeding_pool.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'genotype', association: 'breeding_pool', table:'A', query: 'breeding_poolsConnection', method: 'getData()', request: 'api.breeding_pool.getItemsConnection'}];
                  newError.path=['add', 'breeding_pool'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(breeding_poolsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'genotype', association: 'breeding_pool', table:'A', method: 'getData()'}];
                    newError.path=['add', 'breeding_pool'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.genotype.getAssociatedBreeding_poolConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.breeding_pool.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'genotype', association: 'breeding_pool', table:'B', query: 'countBreeding_pools', method: 'getDataB()', request: 'api.breeding_pool.getCountItems'}];
          newError.path=['add', 'breeding_pool'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'genotype', association: 'breeding_pool', table:'B', query: 'breeding_poolsConnection', method: 'getDataB()', request: 'api.breeding_pool.getItemsConnection'}];
                newError.path=['add', 'breeding_pool'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'genotype', association: 'breeding_pool', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'breeding_pool'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'breeding_pool' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'breeding_pools' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.genotype&&
        ...
        lastModelChanged.genotype[String(item.id)].changedAssociations.breeding_pool&&
        (lastModelChanged.genotype[String(item.id)].changedAssociations.breeding_pool.added ||
         lastModelChanged.genotype[String(item.id)].changedAssociations.breeding_pool.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.genotype[String(item.id)].changedAssociations.breeding_pool.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('breeding_pool', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'genotypes' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'breeding_pool',
     * but this action happened on the peer relation 'genotypes'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'breeding_pool'.
     * B: the current item internalId is in the addedIds of the updated 'breeding_pool'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'breeding_pool' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.breeding_pool) {
      let oens = Object.entries(lastModelChanged.breeding_pool);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.genotypes) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.genotypes.removed) {
              let idsRemoved = entry[1].changedAssociations.genotypes.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.genotypes.added) {
              let idsAdded = entry[1].changedAssociations.genotypes.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('breeding_pool', idAdded);
    ...
    /*
     * Case 3: 
     * The attributes of some 'breeding_pool' were modified or the item was deleted.
     * 
     * Conditions:
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnBreeding_poolRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('breeding_pool', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('breeding_pool', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <BreedingPoolToAddTransferViewToolbar
                title={'Breeding_pools'}
                titleIcon={1}
                search={search}
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.id===it.id) ? 'breeding_pool — ' + t('modelPanels.associatedRecord', 'Associated record') : 'breeding_pool '}>
                                <Avatar>
                                  {"breeding_pool".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
              ...
              {/* Pagination */}
              <BreedingPoolToAddTransferViewCursorPagination
                count={count}
...
BreedingPoolToAddTransferView.propTypes = {
