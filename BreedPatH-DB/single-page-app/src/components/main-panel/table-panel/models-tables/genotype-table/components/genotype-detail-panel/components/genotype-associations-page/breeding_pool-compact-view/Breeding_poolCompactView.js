...
import BreedingPoolCompactViewToolbar from './components/Breeding_poolCompactViewToolbar';
import BreedingPoolCompactViewCursorPagination from './components/Breeding_poolCompactViewCursorPagination';
...
export default function BreedingPoolCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.genotype.getBreeding_poolConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'breeding_pool', query: 'readOneGenotype', method: 'getData()', request: 'api.genotype.getBreeding_poolConnection'}];
          newError.path=['detail', `id:${item.id}`, 'breeding_pool'];
        ...
        //get item
        let it = readOneGenotype.breeding_pool;
      ...
      //error
      .catch((err) => { //error: on api.genotype.getBreeding_poolConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * 
     * Conditions:
     * A: the current item 'internalId' attribute is in the removedIds of the updated 'breeding_pool'.
     * B: the current item 'internalId' attribute is in the addedIds of the updated 'breeding_pool'.
     * 
     * Actions:
     * if A:
     * - reload table.
        ...
        <Grid item xs={12} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <BreedingPoolCompactViewToolbar 
                title={'Breeding_pool'}
                search={search}
              ...
              {(false) && (
                ...
                <BreedingPoolCompactViewCursorPagination
                  count={count}
...
BreedingPoolCompactView.propTypes = {
  item: PropTypes.object.isRequired,
  handleClickOnBreeding_poolRow: PropTypes.func,
};