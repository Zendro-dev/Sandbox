...
import FieldPlotUpdatePanel from '../field_plot-update-panel/Field_plotUpdatePanel'
import FieldPlotDeleteConfirmationDialog from '../Field_plotDeleteConfirmationDialog'
...
export default function FieldPlotDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.field_plot[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.field_plot[String(itemState.id)].op === "update"&&
            lastModelChanged.field_plot[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.field_plot[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.field_plot[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteField_plot
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('field_plot', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('field_plot', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Field_plot | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
...
FieldPlotDetailPanel.propTypes = {
