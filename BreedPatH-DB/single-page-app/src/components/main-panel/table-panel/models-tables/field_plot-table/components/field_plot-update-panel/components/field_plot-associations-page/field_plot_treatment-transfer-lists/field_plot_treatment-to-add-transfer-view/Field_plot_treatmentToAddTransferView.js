...
import FieldPlotTreatmentToAddTransferViewToolbar from './components/Field_plot_treatmentToAddTransferViewToolbar';
import FieldPlotTreatmentToAddTransferViewCursorPagination from './components/Field_plot_treatmentToAddTransferViewCursorPagination';
...
export default function FieldPlotTreatmentToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.field_plot.getAssociatedField_plot_treatmentConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'field_plot', association: 'field_plot_treatment', table:'A', query: 'readOneField_plot', method: 'getData()', request: 'api.field_plot.getAssociatedField_plot_treatmentConnection'}];
          newError.path=['add', 'field_plot_treatment'];
        ...
        */
        let cancelableApiReqB = makeCancelable(api.field_plot_treatment.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'field_plot', association: 'field_plot_treatment', table:'A', query: 'countField_plot_treatments', method: 'getData()', request: 'api.field_plot_treatment.getCountItems'}];
              newError.path=['add', 'field_plot_treatment'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.field_plot_treatment.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'field_plot', association: 'field_plot_treatment', table:'A', query: 'field_plot_treatmentsConnection', method: 'getData()', request: 'api.field_plot_treatment.getItemsConnection'}];
                  newError.path=['add', 'field_plot_treatment'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(field_plot_treatmentsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'field_plot', association: 'field_plot_treatment', table:'A', method: 'getData()'}];
                    newError.path=['add', 'field_plot_treatment'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.field_plot.getAssociatedField_plot_treatmentConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.field_plot_treatment.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'field_plot', association: 'field_plot_treatment', table:'B', query: 'countField_plot_treatments', method: 'getDataB()', request: 'api.field_plot_treatment.getCountItems'}];
          newError.path=['add', 'field_plot_treatment'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
        ...
        //ok
        setCountB(countField_plot_treatments);
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'field_plot', association: 'field_plot_treatment', table:'B', query: 'field_plot_treatmentsConnection', method: 'getDataB()', request: 'api.field_plot_treatment.getItemsConnection'}];
                newError.path=['add', 'field_plot_treatment'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'field_plot', association: 'field_plot_treatment', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'field_plot_treatment'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'field_plot_treatment' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'field_plot_treatments' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.field_plot&&
        ...
        lastModelChanged.field_plot[String(item.id)].changedAssociations.field_plot_treatment&&
        (lastModelChanged.field_plot[String(item.id)].changedAssociations.field_plot_treatment.added ||
         lastModelChanged.field_plot[String(item.id)].changedAssociations.field_plot_treatment.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.field_plot[String(item.id)].changedAssociations.field_plot_treatment.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('field_plot_treatment', idRemoved);
            });
          }
     ...
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'field_plot_treatment'.
     * B: the current item internalId is in the addedIds of the updated 'field_plot_treatment'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'field_plot_treatment' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
            ...
            if(entry[1].changedAssociations.field_plot.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('field_plot_treatment', idAdded);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('field_plot_treatment', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('field_plot_treatment', item.id);
    }
  };
...
FieldPlotTreatmentToAddTransferView.propTypes = {
