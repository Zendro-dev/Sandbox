...
export default function MeasurementsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.field_plot.getAssociatedMeasurementsConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'field_plot', association: 'measurements', table:'A', query: 'readOneField_plot', method: 'getData()', request: 'api.field_plot.getAssociatedMeasurementsConnection'}];
        ...
        //get items
        let idso = readOneField_plot.measurementsConnection.edges.map(o => o.node);
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'field_plot', association: 'measurements', table:'A', query: 'countMeasurements', method: 'getData()', request: 'api.measurement.getCountItems'}];
              newError.path=['add', 'measurements'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'field_plot', association: 'measurements', table:'A', query: 'measurementsConnection', method: 'getData()', request: 'api.measurement.getItemsConnection'}];
                  newError.path=['add', 'measurements'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(measurementsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'field_plot', association: 'measurements', table:'A', method: 'getData()'}];
                    newError.path=['add', 'measurements'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.field_plot.getAssociatedMeasurementsConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
