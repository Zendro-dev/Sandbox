...
import FieldPlotEnhancedTableHead from './components/Field_plotEnhancedTableHead'
import FieldPlotEnhancedTableToolbar from './components/Field_plotEnhancedTableToolbar'
import FieldPlotCreatePanel from './components/field_plot-create-panel/Field_plotCreatePanel'
import FieldPlotUpdatePanel from './components/field_plot-update-panel/Field_plotUpdatePanel'
import FieldPlotDetailPanel from './components/field_plot-detail-panel/Field_plotDetailPanel'
import FieldPlotDeleteConfirmationDialog from './components/Field_plotDeleteConfirmationDialog'
import FieldPlotUploadFileDialog from './components/Field_plotUploadFileDialog'
import FieldPlotCursorPagination from './components/Field_plotCursorPagination'
...
//Plotly
import FieldPlotPlotly from '../../../../plots/Field_plotPlotly';
...
export default function FieldPlotEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.field_plot.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'field_plot', query: 'countField_plots', method: 'getData()', request: 'api.field_plot.getCountItems'}];
          newError.path=['Field_plots'];
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variant.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'field_plot', query: 'field_plotsConnection', method: 'getData()', request: 'api.field_plot.getItemsConnection'}];
                newError.path=['Field_plots'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errors.current.length > 0) {
                ...
                newError.locations=[{model: 'field_plot', method: 'getData()'}];
                newError.path=['Field_plots'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
  ...
  function doDelete(event, item) {
    ...
    /*
      API Request: deleteField_plot
    */
    let cancelableApiReq = makeCancelable(api.field_plot.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'field_plot', query: 'deleteField_plot', method: 'doDelete()', request: 'api.field_plot.deleteItem'}];
          newError.path=['Field_plots', `id:${item.id}`, 'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: deleteField_plot
        let deleteField_plot = response.data.data.deleteField_plot;
        if(deleteField_plot === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'deleteField_plot ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'field_plot', query: 'deleteField_plot', method: 'doDelete()', request: 'api.field_plot.deleteItem'}];
          newError.path=['Field_plots', `id:${item.id}` ,'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        /**
         * Type of deleteField_plot is not validated. Only not null is
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'deleteField_plot ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'field_plot', query: 'deleteField_plot', method: 'doDelete()', request: 'api.field_plot.deleteItem'}];
          newError.path=['Field_plots', `id:${item.id}` ,'delete'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
      ...
      //error
      .catch((err) => { //error: on api.field_plot.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  function getCsvTemplate() {
    ...
    /*
      API Request: csvTableTemplateField_plot
    */
    let cancelableApiReq = makeCancelable(api.field_plot.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'field_plot', query: 'csvTableTemplateField_plot', method: 'getCsvTemplate()', request: 'api.field_plot.tableTemplate'}];
          newError.path=['Field_plots', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: csvTableTemplateField_plot
        let csvTableTemplateField_plot = response.data.data.csvTableTemplateField_plot;
        if(csvTableTemplateField_plot === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateField_plot ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'field_plot', query: 'csvTableTemplateField_plot', method: 'getCsvTemplate()', request: 'api.field_plot.tableTemplate'}];
          newError.path=['Field_plots', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: csvTableTemplateField_plot type
        if(!Array.isArray(csvTableTemplateField_plot)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateField_plot ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'field_plot', query: 'csvTableTemplateField_plot', method: 'getCsvTemplate()', request: 'api.field_plot.tableTemplate'}];
          newError.path=['Field_plots', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'csvTableTemplateField_plot ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'field_plot', query: 'csvTableTemplateField_plot', method: 'getCsvTemplate()', request: 'api.field_plot.tableTemplate'}];
          newError.path=['Field_plots', 'csvTemplate'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateField_plot.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "field_plot-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.field_plot.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <FieldPlotEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(FieldPlotPlotly)}
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.field_plot&&Array.isArray(permissions.field_plot)
                                      &&(permissions.field_plot.includes('update') || permissions.field_plot.includes('*')))
                                      &&(
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.field_plot&&Array.isArray(permissions.field_plot)
                                      &&(permissions.field_plot.includes('delete') || permissions.field_plot.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.field_name!==null)?item.field_name:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.coordinates_or_name!==null)?item.coordinates_or_name:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.year!==null)?item.year:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.area_sqm!==null)?item.area_sqm:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.field_plot_treatment_id!==null)?item.field_plot_treatment_id:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <FieldPlotDeleteConfirmationDialog
          permissions={permissions}
...
FieldPlotEnhancedTable.propTypes = {
