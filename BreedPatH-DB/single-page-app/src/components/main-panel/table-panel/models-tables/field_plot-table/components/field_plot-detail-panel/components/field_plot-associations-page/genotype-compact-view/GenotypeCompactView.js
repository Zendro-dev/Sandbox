...
export default function GenotypeCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.field_plot.getGenotypeConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'genotype', query: 'readOneField_plot', method: 'getData()', request: 'api.field_plot.getGenotypeConnection'}];
        ...
        //get item
        let it = readOneField_plot.genotype;
      ...
      //error
      .catch((err) => { //error: on api.field_plot.getGenotypeConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
