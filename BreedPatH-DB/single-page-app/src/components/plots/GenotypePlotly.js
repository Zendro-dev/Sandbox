...
export default function GenotypePlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.genotype.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.genotypesConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.genotypesConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'genotype', query: 'genotypesConnection', method: 'getItemsAttribute()', request: 'api.genotype.getItemsAttribute'}];
