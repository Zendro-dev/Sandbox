...
export default function MeasurementPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.measurement.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.measurementsConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.measurementsConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'measurement', query: 'measurementsConnection', method: 'getItemsAttribute()', request: 'api.measurement.getItemsAttribute'}];
    ...
    .catch((error) => {
      ...
      newError.locations=[{model: 'measurement', query: 'measurementsConnection', method: 'getItemsAttribute()', request: 'api.measurement.getItemsAttribute'}];
      newError.path=['Measurements', 'Plot'];
