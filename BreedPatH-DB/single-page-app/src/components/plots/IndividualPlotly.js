...
export default function IndividualPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.individual.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.individualsConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.individualsConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'individual', query: 'individualsConnection', method: 'getItemsAttribute()', request: 'api.individual.getItemsAttribute'}];
    ...
    .catch((error) => {
      ...
      newError.locations=[{model: 'individual', query: 'individualsConnection', method: 'getItemsAttribute()', request: 'api.individual.getItemsAttribute'}];
      newError.path=['Individuals', 'Plot'];
