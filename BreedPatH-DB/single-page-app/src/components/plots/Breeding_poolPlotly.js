...
export default function BreedingPoolPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.breeding_pool.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.breeding_poolsConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.breeding_poolsConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'breeding_pool', query: 'breeding_poolsConnection', method: 'getItemsAttribute()', request: 'api.breeding_pool.getItemsAttribute'}];
    ...
    .catch((error) => {
      ...
      newError.locations=[{model: 'breeding_pool', query: 'breeding_poolsConnection', method: 'getItemsAttribute()', request: 'api.breeding_pool.getItemsAttribute'}];
      newError.path=['Breeding_pools', 'Plot'];
