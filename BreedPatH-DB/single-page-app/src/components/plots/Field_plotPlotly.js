...
export default function FieldPlotPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.field_plot.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.field_plotsConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.field_plotsConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'field_plot', query: 'field_plotsConnection', method: 'getItemsAttribute()', request: 'api.field_plot.getItemsAttribute'}];
    ...
    .catch((error) => {
      ...
      newError.locations=[{model: 'field_plot', query: 'field_plotsConnection', method: 'getItemsAttribute()', request: 'api.field_plot.getItemsAttribute'}];
      newError.path=['Field_plots', 'Plot'];
