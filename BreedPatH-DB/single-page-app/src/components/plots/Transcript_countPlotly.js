...
export default function TranscriptCountPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.transcript_count.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.transcript_countsConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.transcript_countsConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'transcript_count', query: 'transcript_countsConnection', method: 'getItemsAttribute()', request: 'api.transcript_count.getItemsAttribute'}];
    ...
    .catch((error) => {
      ...
      newError.locations=[{model: 'transcript_count', query: 'transcript_countsConnection', method: 'getItemsAttribute()', request: 'api.transcript_count.getItemsAttribute'}];
      newError.path=['Transcript_counts', 'Plot'];
