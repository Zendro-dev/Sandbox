...
export default function MarkerDataPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.marker_data.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.marker_dataConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.marker_dataConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'marker_data', query: 'marker_dataConnection', method: 'getItemsAttribute()', request: 'api.marker_data.getItemsAttribute'}];
    ...
    .catch((error) => {
      ...
      newError.locations=[{model: 'marker_data', query: 'marker_dataConnection', method: 'getItemsAttribute()', request: 'api.marker_data.getItemsAttribute'}];
      newError.path=['Marker_data', 'Plot'];
