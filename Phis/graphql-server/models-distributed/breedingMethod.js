...
const definition = {
    ...
    registry: [
        'breedingMethod_PHENOMIS',
        'breedingMethod_PHIS',
        'breedingMethod_PIPPA'
    ],
    attributes: {
        abbreviation: 'String',
...
let registry = ["breedingMethod_PHENOMIS", "breedingMethod_PHIS", "breedingMethod_PIPPA"];

module.exports = class breedingMethod {
    ...
    static get registeredAdapters() {
        return ["breedingMethod_PHENOMIS", "breedingMethod_PHIS", "breedingMethod_PIPPA"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readById(id) {
        if (id !== null) {
            ...
            return adapters[responsibleAdapter[0]].readById(id).then(result => new breedingMethod(result));
        }
    }
    ...
    static idAttribute() {
        let internalId = breedingMethod.definition.id.name;
        return internalId;
    }
    ...
    static addOne(input) {
        ...
        let responsibleAdapter = this.adapterForIri(input.breedingMethodDbId);
        return adapters[responsibleAdapter].addOne(input).then(result => new breedingMethod(result));
    }
    ...
    static updateOne(input) {
        this.assertInputHasId(input);
        let responsibleAdapter = this.adapterForIri(input.breedingMethodDbId);
        return adapters[responsibleAdapter].updateOne(input).then(result => new breedingMethod(result));
    }
    ...
    static bulkAddCsv(context) {
        throw new Error("breedingMethod.bulkAddCsv is not implemented.")
    }

    static csvTableTemplate() {
        return helper.csvTableTemplate(breedingMethod);
    }
}