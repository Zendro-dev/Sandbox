...
let registry = ["program_PHENOMIS", "program_PHIS", "program_PIPPA"];
...
module.exports = class program {
    ...
    static get registeredAdapters() {
        return ["program_PHENOMIS", "program_PHIS", "program_PIPPA"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readById(id) {
        if (id !== null) {
            ...
            return adapters[responsibleAdapter[0]].readById(id).then(result => new program(result));
        }
    }
    ...
    static idAttribute() {
        let internalId = program.definition.id.name;
        return internalId;
    }
    ...
    static addOne(input) {
        ...
        let responsibleAdapter = this.adapterForIri(input.programDbId);
        return adapters[responsibleAdapter].addOne(input).then(result => new program(result));
    }
    ...
    static updateOne(input) {
        this.assertInputHasId(input);
        let responsibleAdapter = this.adapterForIri(input.programDbId);
        return adapters[responsibleAdapter].updateOne(input).then(result => new program(result));
    }
