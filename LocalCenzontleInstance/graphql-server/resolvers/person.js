...
async function checkCount(search, context, query) {
    if (await person.countRecords(search) > context.recordsLimit) {
...
async function validForDeletion(id, context) {
    if (await countAllAssociatedRecords(id, context) > 0) {
        throw new Error(`Person with internalPId ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`);
    }
    return true;
}
...
module.exports = {
    ...
    people: function({
        ...
        return checkAuthorization(context, 'Person', 'read').then(async authorization => {
            if (authorization === true) {
                await checkCount(search, context, "people");
                let resultRecords = await person.readAll(search, order, pagination);
                checkCountAgainAndAdaptLimit(context, resultRecords.length, "people");
    ...
    peopleConnection: function({
        ...
        return checkAuthorization(context, 'Person', 'read').then(async authorization => {
            if (authorization === true) {
                await checkCount(search, context, "peopleConnection");
                let resultRecords = await person.readAll(search, order, pagination);
                checkCountAgainAndAdaptLimit(context, resultRecords.length, "peopleConnection");
    ...
    /**
     * readOnePerson - Check user authorization and return one record with the specified internalPId in the internalPId argument.
     *
     * @param  {number} {internalPId}    internalPId of the record to retrieve
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     * @return {object}         Record with internalPId requested
     */
    readOnePerson: function({
        ...
        return checkAuthorization(context, 'Person', 'read').then(authorization => {
            if (authorization === true) {
                ...
                let resultRecords = person.readById(internalPId);
                checkCountForOne(context);
    ...
    /**
     * deletePerson - Check user authorization and delete a record with the specified internalPId in the internalPId argument.
     *
     * @param  {number} {internalPId}    internalPId of the record to delete
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
    ...
    deletePerson: function({
        ...
        return checkAuthorization(context, 'Person', 'delete').then(async authorization => {
            if (authorization === true) {
                if (await validForDeletion(internalPId, context)) {
                    return person.deleteOne(internalPId);
                }
            } else {
                throw new Error("You don't have authorization to perform this action");
            }
