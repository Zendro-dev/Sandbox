...
module.exports = {
    ...
    booksConnection: async function({
        ...
        try {
            ...
            //let aa = authorizationCheck.authorizedAdapters.pop();
            //authorizationCheck.authorizationErrors = authorizationCheck.authorizationErrors.concat([new Error("You don't have authorization"),new Error("still no authorization")]);
            if (authorizationCheck.authorizedAdapters.length > 0) {
                let connectionObj = await book.readAllCursor(search, order, pagination, authorizationCheck.authorizedAdapters);
                if (authorizationCheck.authorizationErrors.length > 0) {
                    //connectionObj.edges.push({node: authorizationCheck.authorizationErrors[0], cursor:null});
                ...
                if ( connectionObj.errors !== undefined && Array.isArray(connectionObj.errors) && connectionObj.errors.length > 0 ) {
                    ...
                    // is not neccessary, because GraphQL ignores all fields not declared in the schema:
