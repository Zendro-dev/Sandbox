...
const definition = {
    ...
    adapterName: 'eventParameter_PIPPA',
    regex: 'pippa',
    attributes: {
        key: 'String',
        rdfValue: 'String',
...
module.exports = class eventParameter_PIPPA extends Sequelize.Model {
    ...
    static async readById(id) {
        let item = await eventParameter_PIPPA.findByPk(id);
        if (item === null) {
    ...
    static readAllCursor(search, order, pagination) {
        ...
        return super.count(options).then(countA => {
            ...
            if (!options['order'].map(orderItem => {
                    return orderItem[0]
                }).includes("eventParameterDbId")) {
                options['order'] = [...options['order'], ...[
                    ["eventParameterDbId", "ASC"]
                ]];
            }
            ...
            if (pagination) {
                ...
                if (isForwardPagination) {
                    if (pagination.after) {
                        ...
                        options['where'] = {
                            ...options['where'],
                            ...helper.parseOrderCursor(options['order'], decoded_cursor, "eventParameterDbId", pagination.includeCursor)
                        };
                    }
                } else { //backward
                    if (pagination.before) {
                        ...
                        options['where'] = {
                            ...options['where'],
                            ...helper.parseOrderCursorBefore(options['order'], decoded_cursor, "eventParameterDbId", pagination.includeCursor)
                        };
                    }
                }
            }
            ...
            return super.count(woptions).then(countB => {
                ...
                if (globals.LIMIT_RECORDS < options['limit']) {
                    throw new Error(`Request of total eventParameters exceeds max limit of ${globals.LIMIT_RECORDS}. Please use pagination.`);
                }
    ...
    static idAttribute() {
        return eventParameter_PIPPA.definition.id.name;
    }
    ...
    static idAttributeType() {
        return eventParameter_PIPPA.definition.id.type;
    }
    ...
    getIdValue() {
        return this[eventParameter_PIPPA.idAttribute()]
    }
    ...
    stripAssociations() {
        let attributes = Object.keys(eventParameter_PIPPA.definition.attributes);
