...
const event = require(path.join(__dirname, '..', 'models_index.js')).event;
...
const associationArgsDef = {
    ...
    'addEventToObservationUnits': 'observationUnit_to_event'
}
...
/**
 * event.prototype.countFilteredEventToObservationUnits - Count number of associated records that holds the conditions specified in the search argument
 *
 * @param  {object} {search} description
 * @param  {object} context  Provided to every resolver holds contextual information like the resquest query and user info.
 * @return {type}          Number of associated records that holds the conditions specified in the search argument
 */
event.prototype.countFilteredEventToObservationUnits = function({
    search
}, context) {
    try {
...
/**
 * event.prototype.eventToObservationUnitsConnection - Check user authorization and return certain number, specified in pagination argument, of records
 * associated with the current instance, this records should also
 * holds the condition of search argument, all of them sorted as specified by the order argument.
 *
 * @param  {object} search     Search argument for filtering associated records
 ...
 * @return {array}             Array of records as grapqhql connections holding conditions specified by search, order and pagination argument
 */
event.prototype.eventToObservationUnitsConnection = function({
    search,
    order,
    pagination
}, context) {
 ...
 * @param {object} context Provided to every resolver holds contextual information like the resquest query and user info.
 */
event.prototype.handleAssociations = async function(input, context) {
    try {
        let promises = [];
        if (helper.isNonEmptyArray(input.addEventParameters)) {
            promises.push(this.add_eventParameters(input, context));
        }
        if (helper.isNonEmptyArray(input.addEventToObservationUnits)) {
            promises.push(this.add_eventToObservationUnits(input, context));
        }
        ...
        if (helper.isNonEmptyArray(input.removeEventParameters)) {
            promises.push(this.remove_eventParameters(input, context));
        }
        if (helper.isNonEmptyArray(input.removeEventToObservationUnits)) {
            promises.push(this.remove_eventToObservationUnits(input, context));
        }
 ...
 */
event.prototype.add_eventParameters = async function(input) {
    let results = [];
    for await (associatedRecordId of input.addEventParameters) {
        results.push(models.eventParameter.add_eventDbId(associatedRecordId, this.getIdValue()));
    }
    await Promise.all(results);
}

/**
 * add_eventToObservationUnits - field Mutation for to_many associations to add
 *
 * @param {object} input   Info of input Ids to add  the association
 */
event.prototype.add_eventToObservationUnits = async function(input) {
    let results = [];
    for await (associatedRecordId of input.addEventToObservationUnits) {
        results.push(models.observationUnit_to_event.add_eventDbId(associatedRecordId, this.getIdValue()));
    }
    await Promise.all(results);
}
 ...
 * @param {object} input   Info of input Ids to add  the association
 */
event.prototype.add_study = async function(input) {
    await event.add_studyDbId(this.getIdValue(), input.addStudy);
 ...
 */
event.prototype.remove_eventParameters = async function(input) {
    let results = [];
    for await (associatedRecordId of input.removeEventParameters) {
        results.push(models.eventParameter.remove_eventDbId(associatedRecordId, this.getIdValue()));
    }
    await Promise.all(results);
}

/**
 * remove_eventToObservationUnits - field Mutation for to_many associations to remove
 *
 * @param {object} input   Info of input Ids to remove  the association
 */
event.prototype.remove_eventToObservationUnits = async function(input) {
    let results = [];
    for await (associatedRecordId of input.removeEventToObservationUnits) {
        results.push(models.observationUnit_to_event.remove_eventDbId(associatedRecordId, this.getIdValue()));
    }
    await Promise.all(results);
}
 ...
 * @param {object} input   Info of input Ids to remove  the association
 */
event.prototype.remove_study = async function(input) {
    if (input.removeStudy == this.studyDbId) {
        await event.remove_studyDbId(this.getIdValue(), input.removeStudy);
        this.studyDbId = null;
    }
}
...
async function checkCount(search, context, query) {
    if (await event.countRecords(search).sum > context.recordsLimit) {
...
function checkCountForOne(context) {
    if (1 > context.recordsLimit) {
        throw new Error(errorMessageForRecordsLimit("readOneEvent"));
    }
}
...
async function countAllAssociatedRecords(id, context) {
    ...
    promises_to_many.push(event.countFilteredEventParameters({}, context));
    promises_to_many.push(event.countFilteredEventToObservationUnits({}, context));
...
async function validForDeletion(id, context) {
    if (await countAllAssociatedRecords(id, context) > 0) {
        throw new Error(`event with eventType ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`);
    }
...
module.exports = {
    ...
    eventsConnection: async function({
        ...
        try {
            ...
            if (authorizationCheck.authorizedAdapters.length > 0) {
                let connectionObj = await event.readAllCursor(search, order, pagination, authorizationCheck.authorizedAdapters);
    ...
    readOneEvent: async function({
        ...
        //check: adapters auth
        try {
            let authorizationCheck = await checkAuthorization(context, event.adapterForIri(eventType), 'read');
            if (authorizationCheck === true) {
                return event.readById(eventType);
    ...
    addEvent: async function(input, context) {
        ...
        //check: adapters auth
        try {
            let authorizationCheck = await checkAuthorization(context, event.adapterForIri(input.eventType), 'create');
            if (authorizationCheck === true) {
                ...
                let createdRecord = await event.addOne(inputSanitized);
     ...
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     */
    bulkAddEventCsv: function(_, context) {
        return checkAuthorization(context, 'event', 'create').then(authorization => {
            if (authorization === true) {
                return event.bulkAddCsv(context);
            } else {
    ...
    deleteEvent: async function({
        ...
        //check: adapters auth
        try {
            let authorizationCheck = await checkAuthorization(context, event.adapterForIri(eventType), 'delete');
            if (authorizationCheck === true) {
                if (await validForDeletion(eventType, context)) {
                    return event.deleteOne(eventType);
                }
            } else { //adapter not auth
                throw new Error("You don't have authorization to perform this action on adapter");
            }
    ...
    updateEvent: async function(input, context) {
        ...
        //check: adapters auth
        try {
            let authorizationCheck = await checkAuthorization(context, event.adapterForIri(input.eventType), 'update');
            if (authorizationCheck === true) {
                let inputSanitized = helper.sanitizeAssociationArguments(input, [Object.keys(associationArgsDef)]);
                await helper.checkAuthorizationOnAssocArgs(inputSanitized, context, associationArgsDef, ['read', 'update'], models);
                await helper.checkAndAdjustRecordLimitForCreateUpdate(inputSanitized, context, associationArgsDef);
                ...
                let updatedRecord = await event.updateOne(inputSanitized);
    ...
    countEvents: async function({
        ...
        try {
            ...
            if (authorizationCheck.authorizedAdapters.length > 0) {
                ...
                let countObj = await event.countRecords(search, authorizationCheck.authorizedAdapters);
                //check adapter authorization Errors
                if (authorizationCheck.authorizationErrors.length > 0) {
                    context.benignErrors = context.benignErrors.concat(authorizationCheck.authorizationErrors);
                }
     ...
     */
    csvTableTemplateEvent: function(_, context) {
        return checkAuthorization(context, 'event', 'read').then(authorization => {
            if (authorization === true) {
                return event.csvTableTemplate();
            } else {
                throw new Error("You don't have authorization to perform this action");
            }
        }).catch(error => {
