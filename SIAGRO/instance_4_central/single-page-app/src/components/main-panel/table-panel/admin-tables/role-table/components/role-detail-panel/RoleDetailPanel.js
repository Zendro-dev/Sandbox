...
import RoleUpdatePanel from '../role-update-panel/RoleUpdatePanel'
...
export default function RoleDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.role[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.role[String(itemState.id)].op === "update"&&
            lastModelChanged.role[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.role[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.role[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
