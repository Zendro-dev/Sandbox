  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated accessions ids
      */
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneLocation.accessionsConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.accession_id}) : [];
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.locationId, search, rowsPerPage]);
