...
export default {
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('taxon', searchText, ops);
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query taxonsConnection($pagination: paginationCursorInput)
            { taxonsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query taxonsConnection($pagination: paginationCursorInput)
            { taxonsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query taxonsConnection($pagination: paginationCursorInput)
          { taxonsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * 
   * Get accessions connection (cursor based) records associated to the given taxon record
  ...
  getAccessionsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    ...
    var query = (s) ?
      `query readOneTaxon($pagination: paginationCursorInput) {
        readOneTaxon(id: "${itemId}") { 
   ...
   * 
   * Get the accession-ids associated (by cursor based connection) to the given taxon record 
   * through association 'Accessions', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
