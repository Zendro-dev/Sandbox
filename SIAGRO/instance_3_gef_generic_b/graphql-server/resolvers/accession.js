...
async function checkCount(search, context, query) {
    if (await accession.countRecords(search).sum > context.recordsLimit) {
...
module.exports = {
    ...
    countAccessions: async function({
        ...
        return await checkAuthorization(context, 'Accession', 'read').then(async authorization => {
            if (authorization === true) {
                return await accession.countRecords(search);
            } else {
                throw new Error("You don't have authorization to perform this action");
            }
        }).catch(error => {
    ...
    addAccession: async function(input, context) {
        try {
            ...
            if (authorization === true) {
                ...
                if (createdAccession) await createdAccession.handleAssociations(inputSanitized, context);
    ...
    updateAccession: async function(input, context) {
        try {
            ...
            if (authorization === true) {
                ...
                if (updatedAccession) await updatedAccession.handleAssociations(inputSanitized, context);
