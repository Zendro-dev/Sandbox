...
async function checkCount(search, context, query) {
    if (await measurement.countRecords(search).sum > context.recordsLimit) {
...
module.exports = {
    ...
    countMeasurements: async function({
        ...
        return await checkAuthorization(context, 'Measurement', 'read').then(async authorization => {
            if (authorization === true) {
                return await measurement.countRecords(search);
            } else {
                throw new Error("You don't have authorization to perform this action");
            }
        }).catch(error => {
    ...
    addMeasurement: async function(input, context) {
        try {
            ...
            if (authorization === true) {
                ...
                if (createdMeasurement) await createdMeasurement.handleAssociations(inputSanitized, context);
    ...
    updateMeasurement: async function(input, context) {
        try {
            ...
            if (authorization === true) {
                ...
                if (updatedMeasurement) await updatedMeasurement.handleAssociations(inputSanitized, context);
