...
import InformacionTaxonomicaToAddTransferViewToolbar from './components/Informacion_taxonomicaToAddTransferViewToolbar';
import InformacionTaxonomicaToAddTransferViewCursorPagination from './components/Informacion_taxonomicaToAddTransferViewCursorPagination';
...
export default function InformacionTaxonomicaToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.conabio_id;

    /*
      API Request: api.registro.getInformacion_taxonomica
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.getInformacion_taxonomica(graphqlServerUrl, item.conabio_id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'registro', association: 'informacion_taxonomica', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.registro.getInformacion_taxonomica'}];
            newError.path=['update', `conabio_id:${currentId}`, 'add', 'informacion_taxonomica'];
      ...
      //error
      .catch((err) => { //error: on api.registro.getInformacion_taxonomica
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.conabio_id]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.registro.getNotAssociatedInformacion_taxonomicaCount
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.getNotAssociatedInformacion_taxonomicaCount(graphqlServerUrl, item.conabio_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'informacion_taxonomica', table:'A', method: 'getCount()', request: 'api.registro.getNotAssociatedInformacion_taxonomicaCount'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'informacion_taxonomica'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.registro.getNotAssociatedInformacion_taxonomicaCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.registro.getNotAssociatedInformacion_taxonomica
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.getNotAssociatedInformacion_taxonomica(
      graphqlServerUrl,
      item.conabio_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'informacion_taxonomica', table:'A', method: 'getData()', request: 'api.registro.getNotAssociatedInformacion_taxonomica'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'informacion_taxonomica'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.registro.getNotAssociatedInformacion_taxonomica
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.conabio_id, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.taxon.getItems
    */
    let api = await loadApi("taxon");
    let cancelableApiReq = makeCancelable(api.taxon.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'informacion_taxonomica', table:'B', method: 'getDataB()', request: 'api.taxon.getItems'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'informacion_taxonomica'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.taxon.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.conabio_id, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'informacion_taxonomica' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'taxons', 
    ...
    if(lastModelChanged.registro&&
        ...
        lastModelChanged.registro[String(item.conabio_id)].changedAssociations.registro_taxon_id&&
        (lastModelChanged.registro[String(item.conabio_id)].changedAssociations.registro_taxon_id.added ||
         lastModelChanged.registro[String(item.conabio_id)].changedAssociations.registro_taxon_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.registro[String(item.conabio_id)].changedAssociations.registro_taxon_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('informacion_taxonomica', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('informacion_taxonomica', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.registro[String(item.conabio_id)].changedAssociations.registro_taxon_id.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('informacion_taxonomica', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'informacion_taxonomica' for this item was updated from the target model (in the peer relation).
     ...
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'Taxon'.
     * B: the current item internalId is in the addedIds of the updated 'Taxon'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'Taxon' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('informacion_taxonomica', idAdded);
                  } 
    ...
    if(lastModelChanged.Taxon) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('informacion_taxonomica', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.conabio_id, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      if(associatedItem && checked) {
        ...
        handleDisassociateItem('informacion_taxonomica', associatedItem.id);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('informacion_taxonomica', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('informacion_taxonomica', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <InformacionTaxonomicaToAddTransferViewToolbar
                title={'Taxons'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InformacionTaxonomicaToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='InformacionTaxonomicaToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'InformacionTaxonomicaToAddTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.id===it.id) ? 'Taxon — ' + t('modelPanels.associatedRecord', 'Associated record') : 'Taxon '}>
                                <Avatar>
                                  {"taxon".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'InformacionTaxonomicaToAddTransferView-listA-listItem-id-'+it.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'InformacionTaxonomicaToAddTransferView-listA-listItem-'+it.id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <InformacionTaxonomicaToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InformacionTaxonomicaToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InformacionTaxonomicaToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='InformacionTaxonomicaToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = it.taxon;
                        ...
                        return (
                          <ListItem 
                            id={'InformacionTaxonomicaToAddTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'InformacionTaxonomicaToAddTransferView-listB-listItem-id-'+it.id}
                                        variant="body1" display="block" noWrap={true}>{it.id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'InformacionTaxonomicaToAddTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='InformacionTaxonomicaToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.id;
                        let label = associatedItem.taxon;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'InformacionTaxonomicaToAddTransferView-associatedItem-list-listItem-'+associatedItem.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'Taxon — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"taxon".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'InformacionTaxonomicaToAddTransferView-listB-listItem-id-'+associatedItem.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'InformacionTaxonomicaToAddTransferView-associatedItem-list-listItem-'+associatedItem.id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      lidsToRemove.current = [];
                                      handleReassociateItem('informacion_taxonomica', associatedItem.id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.id];
                                      handleDisassociateItem('informacion_taxonomica', associatedItem.id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InformacionTaxonomicaToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
InformacionTaxonomicaToAddTransferView.propTypes = {
