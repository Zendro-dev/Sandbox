...
import AlimentosToAddTransferViewToolbar from './components/AlimentosToAddTransferViewToolbar';
import AlimentosToAddTransferViewCursorPagination from './components/AlimentosToAddTransferViewCursorPagination';
...
export default function AlimentosToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.referencia.getNotAssociatedAlimentosCount
    */
    let api = await loadApi("referencia");
    let cancelableApiReq = makeCancelable(api.referencia.getNotAssociatedAlimentosCount(graphqlServerUrl, item.referencia_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'referencia', association: 'alimentos', table:'A', method: 'getCount()', request: 'api.referencia.getNotAssociatedAlimentosCount'}];
            newError.path=['update', `referencia_id:${item.referencia_id}`, 'add', 'alimentos'];
      ...
      //error
      .catch((err) => { //error: on api.referencia.getNotAssociatedAlimentosCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.referencia_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.referencia.getNotAssociatedAlimentos
    */
    let api = await loadApi("referencia");
    let cancelableApiReq = makeCancelable(api.referencia.getNotAssociatedAlimentos(
      graphqlServerUrl,
      item.referencia_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'referencia', association: 'alimentos', table:'A', method: 'getData()', request: 'api.referencia.getNotAssociatedAlimentos'}];
            newError.path=['update', `referencia_id:${item.referencia_id}`, 'add', 'alimentos'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.referencia.getNotAssociatedAlimentos
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.referencia_id, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'referencia', association: 'alimentos', table:'B', method: 'getCountB()', request: 'api.registro.getCountItems'}];
            newError.path=['update', `referencia_id:${item.referencia_id}`, 'add', 'alimentos'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.referencia_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'referencia', association: 'alimentos', table:'B', method: 'getDataB()', request: 'api.registro.getItems'}];
            newError.path=['update', `referencia_id:${item.referencia_id}`, 'add', 'alimentos'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.referencia_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.referencia&&
        lastModelChanged.referencia[String(item.referencia_id)]&&
        lastModelChanged.referencia[String(item.referencia_id)].changedAssociations&&
        lastModelChanged.referencia[String(item.referencia_id)].changedAssociations.registro_referencias_ids&&
        (lastModelChanged.referencia[String(item.referencia_id)].changedAssociations.registro_referencias_ids.added ||
         lastModelChanged.referencia[String(item.referencia_id)].changedAssociations.registro_referencias_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.referencia[String(item.referencia_id)].changedAssociations.registro_referencias_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('alimentos', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.referencia[String(item.referencia_id)].changedAssociations.registro_referencias_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'registro_referencias_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.registro) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.registro_referencias_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.registro_referencias_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.registro_referencias_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.referencia_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.registro_referencias_ids.added) {
              let idsAdded = entry[1].changedAssociations.registro_referencias_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.referencia_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.conabio_id;
    ...
    if(lastModelChanged.registro) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.conabio_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.referencia_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.conabio_id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('alimentos', item.conabio_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.conabio_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('alimentos', item.conabio_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <AlimentosToAddTransferViewToolbar
                title={'Registros'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AlimentosToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='AlimentosToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'AlimentosToAddTransferView-listA-listItem-'+it.conabio_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'conabio_id' }>
                                        <Typography 
                                        id={'AlimentosToAddTransferView-listA-listItem-id-'+it.conabio_id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'AlimentosToAddTransferView-listA-listItem-'+it.conabio_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <AlimentosToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AlimentosToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AlimentosToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='AlimentosToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.conabio_id;
                        let label = it.descripcion_alimento;
                        ...
                        return (
                          <ListItem 
                            id={'AlimentosToAddTransferView-listB-listItem-'+it.conabio_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'conabio_id' }>
                                        <Typography 
                                        id={'AlimentosToAddTransferView-listB-listItem-id-'+it.conabio_id}
                                        variant="body1" display="block" noWrap={true}>{it.conabio_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'AlimentosToAddTransferView-listB-listItem-'+it.conabio_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
AlimentosToAddTransferView.propTypes = {
