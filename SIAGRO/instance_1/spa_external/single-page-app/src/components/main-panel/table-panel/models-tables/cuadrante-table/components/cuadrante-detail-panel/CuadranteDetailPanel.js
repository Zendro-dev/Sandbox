...
export default function CuadranteDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.cuadrante[String(itemState.cuadrante_id)]) {
          
        //updated item
        if(lastModelChanged.cuadrante[String(itemState.cuadrante_id)].op === "update"&&
            lastModelChanged.cuadrante[String(itemState.cuadrante_id)].newItem) {
              //replace item
              setItemState(lastModelChanged.cuadrante[String(itemState.cuadrante_id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.cuadrante[String(itemState.cuadrante_id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.cuadrante_id]);
