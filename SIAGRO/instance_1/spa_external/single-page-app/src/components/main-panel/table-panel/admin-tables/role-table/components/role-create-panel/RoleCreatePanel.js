...
//lazy loading
const RoleAttributesPage = lazy(() => import(/* webpackChunkName: "Create-AttributesRole" */ './components/role-attributes-page/RoleAttributesPage'));
const RoleAssociationsPage = lazy(() => import(/* webpackChunkName: "Create-AssociationsRole" */ './components/role-associations-page/RoleAssociationsPage'));
const RoleConfirmationDialog = lazy(() => import(/* webpackChunkName: "Create-ConfirmationRole" */ './components/RoleConfirmationDialog'));
const UserDetailPanel = lazy(() => import(/* webpackChunkName: "Create-DetailUser" */ '../../../user-table/components/user-detail-panel/UserDetailPanel'));
...
export default function RoleCreatePanel(props) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.role.createItem
    */
    let api = await loadApi("role");
    let cancelableApiReq = makeCancelable(api.role.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'role', method: 'doSave()', request: 'api.role.createItem'}];
            newError.path=['Roles', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.role.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'users':
        if(usersIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'users') {
      let iof = usersIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        usersIdsToAdd.current.splice(iof, 1);
      }
      return;
    }//end: case 'users'
  }
  ...
  return (
    ...
    <Dialog id='RoleCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='RoleCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Role'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " role" }>
            <Fab
              id='RoleCreatePanel-fabButton-save' 
              color="secondary" 
