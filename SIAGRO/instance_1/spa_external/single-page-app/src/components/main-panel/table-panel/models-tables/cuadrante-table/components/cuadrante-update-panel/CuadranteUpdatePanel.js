...
export default function CuadranteUpdatePanel(props) {
  ...
  const informacion_taxonomicaIdsToAdd = useRef((item.informacion_taxonomica&& item.informacion_taxonomica.id) ? [item.informacion_taxonomica.id] : []);
  const [informacion_taxonomicaIdsToAddState, setInformacion_taxonomicaIdsToAddState] = useState((item.informacion_taxonomica&& item.informacion_taxonomica.id) ? [item.informacion_taxonomica.id] : []);
  const tipo_plantaIdsToAdd = useRef((item.tipo_planta&& item.tipo_planta.tipo_planta_id) ? [item.tipo_planta.tipo_planta_id] : []);
  const [tipo_plantaIdsToAddState, setTipo_plantaIdsToAddState] = useState((item.tipo_planta&& item.tipo_planta.tipo_planta_id) ? [item.tipo_planta.tipo_planta_id] : []);
  ...
  function getInitialForeignKeys() {
    ...
    initialForeignKeys.tipo_planta_id = item.tipo_planta_id;
  ...
  function getInitialValueAjvStates() {
    ...
    _initialValueAjvStates.tipo_planta_id = {errors: []}; //FK
  ...
  function setAddRemoveGrupo_enfoque(variables) {
    ...
    changedAssociations.current.cuadrante_grupo_enfoque_id = {added: false, removed: false};
    ...
    if(item.grupo_enfoque&&item.grupo_enfoque.grupo_id) {
      ...
      if(grupo_enfoqueIdsToAdd.current.length>0) {
        ...
        if(item.grupo_enfoque.grupo_id!== grupo_enfoqueIdsToAdd.current[0]) {
          ...
          changedAssociations.current.cuadrante_grupo_enfoque_id.added = true;
          changedAssociations.current.cuadrante_grupo_enfoque_id.idsAdded = grupo_enfoqueIdsToAdd.current;
          changedAssociations.current.cuadrante_grupo_enfoque_id.removed = true;
          changedAssociations.current.cuadrante_grupo_enfoque_id.idsRemoved = [item.grupo_enfoque.grupo_id];
        } else {
          /*
  ...
  function setAddRemoveInformacion_taxonomica(variables) {
    //data to notify changes
    changedAssociations.current.cuadrante_taxon_id = {added: false, removed: false};
    
    /*
     * Case I: Currently, this record is associated.
     */
    if(item.informacion_taxonomica&&item.informacion_taxonomica.id) {
      /*
       * Case I.a: The toAdd list isn't empty.
       */      
      if(informacion_taxonomicaIdsToAdd.current.length>0) {
         ...
         * Case I.a.1: There is a new ID (current association changed).
         */
        if(item.informacion_taxonomica.id!== informacion_taxonomicaIdsToAdd.current[0]) {
          //set id to add
          variables.addInformacion_taxonomica = informacion_taxonomicaIdsToAdd.current[0];
          
          changedAssociations.current.cuadrante_taxon_id.added = true;
          changedAssociations.current.cuadrante_taxon_id.idsAdded = informacion_taxonomicaIdsToAdd.current;
          changedAssociations.current.cuadrante_taxon_id.removed = true;
          changedAssociations.current.cuadrante_taxon_id.idsRemoved = [item.informacion_taxonomica.id];
        } else {
          /*
           * Case I.a.2: The ID on toAdd list es equal to the current associated ID.
           */
         ...
         */
        //set id to remove
        variables.removeInformacion_taxonomica = item.informacion_taxonomica.id;
  ...
  function setAddRemoveTipo_planta(variables) {
    //data to notify changes
     ...
     * Case I: Currently, this record is associated.
     */
    if(item.tipo_planta&&item.tipo_planta.tipo_planta_id) {
      /*
       * Case I.a: The toAdd list isn't empty.
       */      
      if(tipo_plantaIdsToAdd.current.length>0) {
        /*
         * Case I.a.1: There is a new ID (current association changed).
         */
        if(item.tipo_planta.tipo_planta_id!== tipo_plantaIdsToAdd.current[0]) {
          //set id to add
          variables.addTipo_planta = tipo_plantaIdsToAdd.current[0];
          
          changedAssociations.current.cuadrante_tipo_planta_id.added = true;
          changedAssociations.current.cuadrante_tipo_planta_id.idsAdded = tipo_plantaIdsToAdd.current;
          changedAssociations.current.cuadrante_tipo_planta_id.removed = true;
          changedAssociations.current.cuadrante_tipo_planta_id.idsRemoved = [item.tipo_planta.tipo_planta_id];
        } else {
          /*
           * Case I.a.2: The ID on toAdd list es equal to the current associated ID.
           */
         ...
         */
        //set id to remove
        variables.removeTipo_planta = item.tipo_planta.tipo_planta_id;
...
function setAjvErrors(err) {
  ...
  function doSave(event) {
    ...
    */
    ...
    delete variables.grupo_enfoque_id; //FK
    delete variables.taxon_id; //FK
    delete variables.tipo_planta_id; //FK

    //add & remove: to_one's
    setAddRemoveGrupo_enfoque(variables);
    setAddRemoveInformacion_taxonomica(variables);
    setAddRemoveTipo_planta(variables);
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'grupo_enfoque':
        ...
        handleSetValue(itemId, -2, 'grupo_enfoque_id');
      ...
      case 'informacion_taxonomica':
        ...
        break;
      case 'tipo_planta':
        tipo_plantaIdsToAdd.current = [];
        tipo_plantaIdsToAdd.current.push(itemId);
        setTipo_plantaIdsToAddState(tipo_plantaIdsToAdd.current);
        handleSetValue(itemId, -2, 'tipo_planta_id');
        setForeignKeys({...foreignKeys, tipo_planta_id: itemId});
        break;
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'grupo_enfoque') {
      ...
      handleSetValue(null, -2, 'grupo_enfoque_id');
    ...
    if(associationKey === 'tipo_planta') {
      tipo_plantaIdsToAdd.current = [];
      setTipo_plantaIdsToAddState([]);
      handleSetValue(null, -2, 'tipo_planta_id');
      setForeignKeys({...foreignKeys, tipo_planta_id: null});
      return;
    }//end: case 'tipo_planta'
  }
  ...
  return (
    <Dialog id='CuadranteUpdatePanel-dialog' 
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='CuadranteUpdatePanel-button-cancel'
              edge="start" 
              color="inherit" 
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " cuadrante" }>
              <Fab
                id='CuadranteUpdatePanel-fabButton-save' 
                color="secondary" 
          ...
          {(tabsValue === 1 && !deleted) && (
            <Grid item xs={12}>
              ...
              <CuadranteAssociationsPage
                ...
                tipo_plantaIdsToAdd={tipo_plantaIdsToAddState}
