...
import ReferenciaEnhancedTableHead from './components/ReferenciaEnhancedTableHead'
import ReferenciaEnhancedTableToolbar from './components/ReferenciaEnhancedTableToolbar'
import ReferenciaUploadFileDialog from './components/ReferenciaUploadFileDialog'
import ReferenciaCursorPagination from './components/ReferenciaCursorPagination'
...
//lazy loading
const ReferenciaCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Referencia" */ './components/referencia-create-panel/ReferenciaCreatePanel'));
const ReferenciaUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Referencia" */ './components/referencia-update-panel/ReferenciaUpdatePanel'));
const ReferenciaDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Referencia" */ './components/referencia-detail-panel/ReferenciaDetailPanel'));
const ReferenciaDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Referencia" */ './components/ReferenciaDeleteConfirmationDialog'));
//Plotly
const ReferenciaPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Referencia" */ '../../../../plots/ReferenciaPlotly'));
...
export default function ReferenciaEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('referencia_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.referencia.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'referencia', method: 'getCount()', request: 'api.referencia.getCountItems'}];
            newError.path=['Referencia'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'referencia', method: 'getData()', request: 'api.referencia.getItems'}];
            newError.path=['Referencia'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['referencia']
    &&(Object.keys(changes.lastModelChanged['referencia']).length===1)
    &&(changes.lastModelChanged['referencia'][Object.keys(changes.lastModelChanged['referencia'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set referencia_id (internalId)
    variables.referencia_id = item.referencia_id;

    /*
      API Request: api.referencia.deleteItem
    */
    let api = await loadApi("referencia");
    let cancelableApiReq = makeCancelable(api.referencia.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'referencia', method: 'doDelete()', request: 'api.referencia.deleteItem'}];
            newError.path=['Referencia', `referencia_id:${item.referencia_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.referencia.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.referencia.tableTemplate
    */
    let api = await loadApi("referencia");
    let cancelableApiReq = makeCancelable(api.referencia.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'referencia', method: 'getCsvTemplate()', request: 'api.referencia.tableTemplate'}];
            newError.path=['Referencia', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateReferencia.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "referencia-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.referencia.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <ReferenciaEnhancedTableToolbar
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ReferenciaEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ReferenciaEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ReferenciaEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ReferenciaEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ReferenciaEnhancedTable-row-'+item.referencia_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ReferenciaEnhancedTable-row-iconButton-detail-'+item.referencia_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.referencia&&Array.isArray(permissions.referencia)
                                      &&(permissions.referencia.includes('update') || permissions.referencia.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ReferenciaEnhancedTable-row-iconButton-edit-'+item.referencia_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.referencia&&Array.isArray(permissions.referencia)
                                      &&(permissions.referencia.includes('delete') || permissions.referencia.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ReferenciaEnhancedTable-row-iconButton-delete-'+item.referencia_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'referencia_id: ' + item.referencia_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.referencia_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.referencia!==null)?item.referencia:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.registros_ids!==null)?item.registros_ids:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <ReferenciaDeleteConfirmationDialog
            permissions={permissions}
...
ReferenciaEnhancedTable.propTypes = {
