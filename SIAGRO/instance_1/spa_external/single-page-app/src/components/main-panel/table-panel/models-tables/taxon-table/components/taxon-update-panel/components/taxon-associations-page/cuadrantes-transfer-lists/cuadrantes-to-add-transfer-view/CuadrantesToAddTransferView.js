...
export default function CuadrantesToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.taxon.getAssociatedCuadrantesConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'taxon', association: 'cuadrantes', table:'A', query: 'readOneTaxon', method: 'getData()', request: 'api.taxon.getAssociatedCuadrantesConnection'}];
        ...
        //get items
        let idso = readOneTaxon.cuadrantesConnection.edges.map(o => o.node);
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'taxon', association: 'cuadrantes', table:'A', query: 'countCuadrantes', method: 'getData()', request: 'api.cuadrante.getCountItems'}];
              newError.path=['add', 'cuadrantes'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'taxon', association: 'cuadrantes', table:'A', query: 'cuadrantesConnection', method: 'getData()', request: 'api.cuadrante.getItemsConnection'}];
                  newError.path=['add', 'cuadrantes'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(cuadrantesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'taxon', association: 'cuadrantes', table:'A', method: 'getData()'}];
                    newError.path=['add', 'cuadrantes'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.taxon.getAssociatedCuadrantesConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
