...
//lazy loading
const RoleToUserAttributesPage = lazy(() => import(/* webpackChunkName: "Detail-Attributes-RoleToUser" */ './components/role_to_user-attributes-page/Role_to_userAttributesPage'));
const RoleToUserAssociationsPage = lazy(() => import(/* webpackChunkName: "Detail-Associations-RoleToUser" */ './components/role_to_user-associations-page/Role_to_userAssociationsPage'));
const RoleToUserUpdatePanel = lazy(() => import(/* webpackChunkName: "Detail-Update-RoleToUser" */ '../role_to_user-update-panel/Role_to_userUpdatePanel'));
const RoleToUserDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Detail-Delete-RoleToUser" */ '../Role_to_userDeleteConfirmationDialog'));
...
export default function RoleToUserDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged['role_to_user']&&
      lastModelChanged['role_to_user'][String(item['id'])]) {
          
        //updated item
        if(lastModelChanged['role_to_user'][String(item['id'])].op === "update"&&
            lastModelChanged['role_to_user'][String(item['id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['role_to_user'][String(item['id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged['role_to_user'][String(item['id'])].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function getInitialValueOkStates(item) {
    ...
    */
    ...
    initialValueOkStates.userId = (item.userId!==null ? 1 : 0);
    initialValueOkStates.roleId = (item.roleId!==null ? 1 : 0);
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('role_to_user', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('role_to_user', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='RoleToUserDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Role_to_user | id: " + itemState['id'] }
              </Typography>
              {/*
                Actions:
                - Edit
...
RoleToUserDetailPanel.propTypes = {
