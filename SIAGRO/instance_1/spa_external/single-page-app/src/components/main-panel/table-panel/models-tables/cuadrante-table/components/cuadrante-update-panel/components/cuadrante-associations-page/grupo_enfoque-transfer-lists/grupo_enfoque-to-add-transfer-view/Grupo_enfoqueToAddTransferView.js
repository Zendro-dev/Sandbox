  ...
  */
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.cuadrante&&
        ...
        lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.grupo_enfoque&&
        (lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.grupo_enfoque.added ||
         lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.grupo_enfoque.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.grupo_enfoque.idsRemoved;
          if(idsRemoved) {
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem.grupo_id===it.grupo_id) ? 'grupo_enfoque — ' + t('modelPanels.associatedRecord', 'Associated record') : 'grupo_enfoque '}>
                                <Avatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    ...
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem.grupo_id===it.grupo_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem.grupo_id===it.grupo_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
