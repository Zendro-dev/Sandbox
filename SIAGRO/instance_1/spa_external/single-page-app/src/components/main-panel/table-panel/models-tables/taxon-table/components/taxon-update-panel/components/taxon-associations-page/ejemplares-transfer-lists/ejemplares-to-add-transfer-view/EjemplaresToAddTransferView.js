...
import EjemplaresToAddTransferViewToolbar from './components/EjemplaresToAddTransferViewToolbar';
import EjemplaresToAddTransferViewCursorPagination from './components/EjemplaresToAddTransferViewCursorPagination';
...
export default function EjemplaresToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.taxon.getNotAssociatedEjemplaresCount
    */
    let api = await loadApi("taxon");
    let cancelableApiReq = makeCancelable(api.taxon.getNotAssociatedEjemplaresCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Taxon', association: 'ejemplares', table:'A', method: 'getCount()', request: 'api.taxon.getNotAssociatedEjemplaresCount'}];
            newError.path=['update', `id:${item.id}`, 'add', 'ejemplares'];
      ...
      //error
      .catch((err) => { //error: on api.taxon.getNotAssociatedEjemplaresCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.taxon.getNotAssociatedEjemplares
    */
    let api = await loadApi("taxon");
    let cancelableApiReq = makeCancelable(api.taxon.getNotAssociatedEjemplares(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Taxon', association: 'ejemplares', table:'A', method: 'getData()', request: 'api.taxon.getNotAssociatedEjemplares'}];
            newError.path=['update', `id:${item.id}`, 'add', 'ejemplares'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.taxon.getNotAssociatedEjemplares
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.ejemplar.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Taxon', association: 'ejemplares', table:'B', method: 'getCountB()', request: 'api.ejemplar.getCountItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'ejemplares'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Taxon', association: 'ejemplares', table:'B', method: 'getDataB()', request: 'api.ejemplar.getItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'ejemplares'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'ejemplares' for this item was updated.
    ...
    if(lastModelChanged.Taxon&&
        ...
        lastModelChanged.Taxon[String(item.id)].changedAssociations.Taxon_ejemplares&&
        (lastModelChanged.Taxon[String(item.id)].changedAssociations.Taxon_ejemplares.added ||
         lastModelChanged.Taxon[String(item.id)].changedAssociations.Taxon_ejemplares.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.Taxon[String(item.id)].changedAssociations.Taxon_ejemplares.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('ejemplares', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.Taxon[String(item.id)].changedAssociations.Taxon_ejemplares.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'ejemplares' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'Ejemplar',
     * but this action happened on the peer relation, identified by 'Taxon_ejemplares'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.Ejemplar) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.Taxon_ejemplares) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.Taxon_ejemplares.removed) {
              let idsRemoved = entry[1].changedAssociations.Taxon_ejemplares.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.Taxon_ejemplares.added) {
              let idsAdded = entry[1].changedAssociations.Taxon_ejemplares.idsAdded;
              if(idsAdded) {
    ...
    if(lastModelChanged.Ejemplar) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('ejemplares', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('ejemplares', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('ejemplares', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <EjemplaresToAddTransferViewToolbar
                title={'Ejemplars'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EjemplaresToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='EjemplaresToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'EjemplaresToAddTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'EjemplaresToAddTransferView-listA-listItem-id-'+it.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'EjemplaresToAddTransferView-listA-listItem-'+it.id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <EjemplaresToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EjemplaresToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EjemplaresToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='EjemplaresToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'EjemplaresToAddTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'EjemplaresToAddTransferView-listB-listItem-id-'+it.id}
                                        variant="body1" display="block" noWrap={true}>{it.id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'EjemplaresToAddTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
EjemplaresToAddTransferView.propTypes = {
