...
import CaracteristicasCuantitativasToAddTransferViewToolbar from './components/Caracteristicas_cuantitativasToAddTransferViewToolbar';
import CaracteristicasCuantitativasToAddTransferViewCursorPagination from './components/Caracteristicas_cuantitativasToAddTransferViewCursorPagination';
...
export default function CaracteristicasCuantitativasToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.registro.getNotAssociatedCaracteristicas_cuantitativasCount
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.getNotAssociatedCaracteristicas_cuantitativasCount(graphqlServerUrl, item.conabio_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'registro', association: 'caracteristicas_cuantitativas', table:'A', method: 'getCount()', request: 'api.registro.getNotAssociatedCaracteristicas_cuantitativasCount'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'caracteristicas_cuantitativas'];
      ...
      //error
      .catch((err) => { //error: on api.registro.getNotAssociatedCaracteristicas_cuantitativasCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.registro.getNotAssociatedCaracteristicas_cuantitativas
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.getNotAssociatedCaracteristicas_cuantitativas(
      graphqlServerUrl,
      item.conabio_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'caracteristicas_cuantitativas', table:'A', method: 'getData()', request: 'api.registro.getNotAssociatedCaracteristicas_cuantitativas'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'caracteristicas_cuantitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.registro.getNotAssociatedCaracteristicas_cuantitativas
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cuantitativa.getCountItems
    */
    let api = await loadApi("caracteristica_cuantitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cuantitativa.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'caracteristicas_cuantitativas', table:'B', method: 'getCountB()', request: 'api.caracteristica_cuantitativa.getCountItems'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'caracteristicas_cuantitativas'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.caracteristica_cuantitativa.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cuantitativa.getItems
    */
    let api = await loadApi("caracteristica_cuantitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cuantitativa.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'caracteristicas_cuantitativas', table:'B', method: 'getDataB()', request: 'api.caracteristica_cuantitativa.getItems'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'caracteristicas_cuantitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cuantitativa.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'caracteristicas_cuantitativas' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'caracteristica_cuantitativas', 
    ...
    if(lastModelChanged.registro&&
        ...
        lastModelChanged.registro[String(item.conabio_id)].changedAssociations.caracteristica_cuantitativa_registro_id&&
        (lastModelChanged.registro[String(item.conabio_id)].changedAssociations.caracteristica_cuantitativa_registro_id.added ||
         lastModelChanged.registro[String(item.conabio_id)].changedAssociations.caracteristica_cuantitativa_registro_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.registro[String(item.conabio_id)].changedAssociations.caracteristica_cuantitativa_registro_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('caracteristicas_cuantitativas', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.registro[String(item.conabio_id)].changedAssociations.caracteristica_cuantitativa_registro_id.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'caracteristicas_cuantitativas' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'caracteristica_cuantitativa',
     * but this action happened on the peer relation, identified by 'caracteristica_cuantitativa_registro_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'caracteristica_cuantitativa'.
     * B: the current item internalId is in the addedIds of the updated 'caracteristica_cuantitativa'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'caracteristica_cuantitativa' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.caracteristica_cuantitativa) {
      let oens = Object.entries(lastModelChanged.caracteristica_cuantitativa);
      oens.forEach( (entry) => {
    ...
    /*
     * Case 3: 
     * The attributes of some 'caracteristica_cuantitativa' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.caracteristica_cuantitativa) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('caracteristicas_cuantitativas', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnCaracteristica_cuantitativaRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('caracteristicas_cuantitativas', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('caracteristicas_cuantitativas', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <CaracteristicasCuantitativasToAddTransferViewToolbar
                title={'Caracteristica_cuantitativas'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CaracteristicasCuantitativasToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='CaracteristicasCuantitativasToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.id;
                        let label = it.nombre_corto;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'CaracteristicasCuantitativasToAddTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'caracteristica_cuantitativa' }>
                                <Avatar>{"caracteristica_cuantitativa".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'CaracteristicasCuantitativasToAddTransferView-listA-listItem-id-'+it.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'CaracteristicasCuantitativasToAddTransferView-listA-listItem-'+it.id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <CaracteristicasCuantitativasToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CaracteristicasCuantitativasToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CaracteristicasCuantitativasToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='CaracteristicasCuantitativasToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = it.nombre_corto;
                        ...
                        return (
                          <ListItem 
                            id={'CaracteristicasCuantitativasToAddTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'CaracteristicasCuantitativasToAddTransferView-listB-listItem-id-'+it.id}
                                        variant="body1" display="block" noWrap={true}>{it.id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'CaracteristicasCuantitativasToAddTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
CaracteristicasCuantitativasToAddTransferView.propTypes = {
