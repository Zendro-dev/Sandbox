...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['caracteristica_cualitativa'][String(itemId['id'])]) {

        //updated password
        if(lastModelChanged['caracteristica_cualitativa'][String(itemId['id'])].op === "updatePassword"&&
            lastModelChanged['caracteristica_cualitativa'][String(itemId['id'])].newItem) {
              //update password value
              setValue(lastModelChanged['caracteristica_cualitativa'][String(itemId['id'])].newItem[itemKey]);
        }
    }//end: Case 1
  ...
  /**
    ...
    */
    ...
    return await api.caracteristica_cualitativa.updateItem(graphqlServerUrl, variables)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['Caracteristica_cualitativas', itemId, 'update', 'change password'];
  ...
  return (
    <div>
      <Grid container justify='flex-start' alignItems='center' spacing={0}>
        <Grid item xs={12}>
          <form noValidate autoComplete="off">
            <TextField
              id={'PasswordField-CaracteristicaCualitativa-'+name}
              label={label}
              value={value}
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'PasswordField-CaracteristicaCualitativa-button-showPassword'+name}
                    ...
                    {(valueOk!==undefined&&valueOk===1) 
                      ...
                      <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                        <Typography id={'PasswordField-CaracteristicaCualitativa-exists-'+name} variant="caption" color="primary">
                          &#8707;
                        </Typography>
                      </Tooltip>
                      : 
                      <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                        <Typography id={'PasswordField-CaracteristicaCualitativa-notExists-'+name} variant="caption" color="textSecondary">
                          &#8708;
                        </Typography>
                      </Tooltip>
                    }
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'PasswordField-CaracteristicaCualitativa-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-CaracteristicaCualitativa-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-CaracteristicaCualitativa-ChangePasswordDialog-textField-newPassword2-'+name}
