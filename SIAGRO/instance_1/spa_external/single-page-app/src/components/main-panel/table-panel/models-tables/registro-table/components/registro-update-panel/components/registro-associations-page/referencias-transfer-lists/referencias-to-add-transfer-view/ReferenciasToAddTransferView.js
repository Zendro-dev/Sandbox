...
import ReferenciasToAddTransferViewToolbar from './components/ReferenciasToAddTransferViewToolbar';
import ReferenciasToAddTransferViewCursorPagination from './components/ReferenciasToAddTransferViewCursorPagination';
...
export default function ReferenciasToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"referencia_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.registro.getNotAssociatedReferenciasCount
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.getNotAssociatedReferenciasCount(graphqlServerUrl, item.conabio_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'registro', association: 'referencias', table:'A', method: 'getCount()', request: 'api.registro.getNotAssociatedReferenciasCount'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'referencias'];
      ...
      //error
      .catch((err) => { //error: on api.registro.getNotAssociatedReferenciasCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.conabio_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.registro.getNotAssociatedReferencias
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.getNotAssociatedReferencias(
      graphqlServerUrl,
      item.conabio_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'referencias', table:'A', method: 'getData()', request: 'api.registro.getNotAssociatedReferencias'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'referencias'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.registro.getNotAssociatedReferencias
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.conabio_id, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.referencia.getCountItems
    */
    let api = await loadApi("referencia");
    let cancelableApiReq = makeCancelable(api.referencia.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'referencias', table:'B', method: 'getCountB()', request: 'api.referencia.getCountItems'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'referencias'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.referencia.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.conabio_id, searchB]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.referencia.getItems
    */
    let api = await loadApi("referencia");
    let cancelableApiReq = makeCancelable(api.referencia.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'referencias', table:'B', method: 'getDataB()', request: 'api.referencia.getItems'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'add', 'referencias'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.referencia.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.conabio_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'referencias' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'referencia', 
    ...
    if(lastModelChanged.registro&&
        ...
        lastModelChanged.registro[String(item.conabio_id)].changedAssociations&&
        lastModelChanged.registro[String(item.conabio_id)].changedAssociations.referencia_registros_ids&&
        (lastModelChanged.registro[String(item.conabio_id)].changedAssociations.referencia_registros_ids.added ||
         lastModelChanged.registro[String(item.conabio_id)].changedAssociations.referencia_registros_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.registro[String(item.conabio_id)].changedAssociations.referencia_registros_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('referencias', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.registro[String(item.conabio_id)].changedAssociations.referencia_registros_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'referencias' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'referencia',
     * but this action happened on the peer relation, identified by 'referencia_registros_ids'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'referencia'.
     * B: the current item internalId is in the addedIds of the updated 'referencia'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'referencia' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.referencia) {
      let oens = Object.entries(lastModelChanged.referencia);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.referencia_registros_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.referencia_registros_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.referencia_registros_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.conabio_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.referencia_registros_ids.added) {
              let idsAdded = entry[1].changedAssociations.referencia_registros_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.conabio_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.referencia_id;
    ...
    /*
     * Case 3: 
     * The attributes of some 'referencia' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.referencia) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.referencia_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.referencia_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.referencia_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.referencia_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.referencia_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.referencia_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('referencias', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.conabio_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnReferenciaRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.referencia_id) === -1) {
      lidsToAdd.current.push(item.referencia_id);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('referencias', item.referencia_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.referencia_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('referencias', item.referencia_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ReferenciasToAddTransferViewToolbar
                title={'Referencia'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ReferenciasToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ReferenciasToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.referencia_id;
                        let label = it.referencia;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'ReferenciasToAddTransferView-listA-listItem-'+it.referencia_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'referencia' }>
                                <Avatar>{"referencia".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'referencia_id' }>
                                        <Typography 
                                        id={'ReferenciasToAddTransferView-listA-listItem-id-'+it.referencia_id}
                                        variant="body1" display="block" noWrap={true}>{it.referencia_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ReferenciasToAddTransferView-listA-listItem-'+it.referencia_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ReferenciasToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ReferenciasToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ReferenciasToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ReferenciasToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.referencia_id;
                        let label = it.referencia;
                        ...
                        return (
                          <ListItem 
                            id={'ReferenciasToAddTransferView-listB-listItem-'+it.referencia_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'referencia_id' }>
                                        <Typography 
                                        id={'ReferenciasToAddTransferView-listB-listItem-id-'+it.referencia_id}
                                        variant="body1" display="block" noWrap={true}>{it.referencia_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ReferenciasToAddTransferView-listB-listItem-'+it.referencia_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ReferenciasToAddTransferView.propTypes = {
