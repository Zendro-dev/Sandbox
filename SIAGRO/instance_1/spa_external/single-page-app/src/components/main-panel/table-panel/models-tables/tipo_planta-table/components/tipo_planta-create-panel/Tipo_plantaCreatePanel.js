...
export default function TipoPlantaCreatePanel(props) {
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.tipo_planta_id = 0;
  ...
  function doSave(event) {
    ...
    */
    ...
    /*
      API Request: addTipo_planta
    */
    let cancelableApiReq = makeCancelable(api.tipo_planta.createItem(graphqlServerUrl, variables));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'tipo_planta', query: 'addTipo_planta', method: 'doSave()', request: 'api.tipo_planta.createItem'}];
          newError.path=['Tipo_planta', 'add'];
        ...
        //check: addTipo_planta
        let addTipo_planta = response.data.data.addTipo_planta;
        if(addTipo_planta === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addTipo_planta ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'tipo_planta', query: 'addTipo_planta', method: 'doSave()', request: 'api.tipo_planta.createItem'}];
          newError.path=['Tipo_planta', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: addTipo_planta type
        if(typeof addTipo_planta !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addTipo_planta ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'tipo_planta', query: 'addTipo_planta', method: 'doSave()', request: 'api.tipo_planta.createItem'}];
          newError.path=['Tipo_planta', 'add'];
        ...
        onClose(event, true, addTipo_planta);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.tipo_planta.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  return (
    ...
    <Dialog id='TipoPlantaCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='TipoPlantaCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Tipo_planta'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " tipo_planta" }>
            <Fab
              id='TipoPlantaCreatePanel-fabButton-save' 
              color="secondary" 
...
TipoPlantaCreatePanel.propTypes = {
