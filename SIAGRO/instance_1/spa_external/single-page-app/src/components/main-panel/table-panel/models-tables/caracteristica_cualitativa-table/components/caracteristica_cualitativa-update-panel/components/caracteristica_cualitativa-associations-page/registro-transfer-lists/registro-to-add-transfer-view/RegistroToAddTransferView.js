...
export default function RegistroToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cualitativa.getRegistro
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.getRegistro(graphqlServerUrl, item.id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'caracteristica_cualitativa', association: 'registro', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.caracteristica_cualitativa.getRegistro'}];
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cualitativa.getRegistro
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cualitativa.getNotAssociatedRegistroCount
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.getNotAssociatedRegistroCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'caracteristica_cualitativa', association: 'registro', table:'A', method: 'getCount()', request: 'api.caracteristica_cualitativa.getNotAssociatedRegistroCount'}];
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cualitativa.getNotAssociatedRegistroCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cualitativa.getNotAssociatedRegistro
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.getNotAssociatedRegistro(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'caracteristica_cualitativa', association: 'registro', table:'A', method: 'getData()', request: 'api.caracteristica_cualitativa.getNotAssociatedRegistro'}];
            newError.path=['update', `id:${item.id}`, 'add', 'registro'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cualitativa.getNotAssociatedRegistro
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'caracteristica_cualitativa', association: 'registro', table:'B', method: 'getDataB()', request: 'api.ejemplar.getItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'registro'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.caracteristica_cualitativa&&
        ...
        lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id&&
        (lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id.added ||
         lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id.idsRemoved;
          if(idsRemoved) {
            //increment A
