...
import EjemplaresToRemoveTransferViewToolbar from './components/EjemplaresToRemoveTransferViewToolbar';
import EjemplaresToRemoveTransferViewCursorPagination from './components/EjemplaresToRemoveTransferViewCursorPagination';
...
export default function EjemplaresToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.taxon.getEjemplaresCount
    */
    let api = await loadApi("taxon");
    let cancelableApiReq = makeCancelable(api.taxon.getEjemplaresCount(
      graphqlServerUrl, 
      item.id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'ejemplares', method: 'getCount()', request: 'api.taxon.getEjemplaresCount'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'ejemplares'];
      ...
      //error
      .catch((err) => { //error: on api.taxon.getEjemplaresCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.taxon.getEjemplares
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("taxon");
    let cancelableApiReq = makeCancelable(api.taxon.getEjemplares(
      graphqlServerUrl, 
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'ejemplares', method: 'getData()', request: 'api.taxon.getEjemplares'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'ejemplares'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.taxon.getEjemplares
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'Ejemplar' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <EjemplaresToRemoveTransferViewToolbar
                title={'Ejemplars'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EjemplaresToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='EjemplaresToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'EjemplaresToRemoveTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'EjemplaresToRemoveTransferView-listA-listItem-'+it.id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <EjemplaresToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EjemplaresToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EjemplaresToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='EjemplaresToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'EjemplaresToRemoveTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'EjemplaresToRemoveTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
EjemplaresToRemoveTransferView.propTypes = {
