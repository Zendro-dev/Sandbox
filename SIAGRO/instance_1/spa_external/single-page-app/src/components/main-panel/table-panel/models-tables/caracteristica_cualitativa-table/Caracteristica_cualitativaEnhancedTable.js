...
import CaracteristicaCualitativaEnhancedTableHead from './components/Caracteristica_cualitativaEnhancedTableHead'
import CaracteristicaCualitativaEnhancedTableToolbar from './components/Caracteristica_cualitativaEnhancedTableToolbar'
import CaracteristicaCualitativaUploadFileDialog from './components/Caracteristica_cualitativaUploadFileDialog'
import CaracteristicaCualitativaCursorPagination from './components/Caracteristica_cualitativaCursorPagination'
...
//lazy loading
const CaracteristicaCualitativaCreatePanel = lazy(() => import(/* webpackChunkName: "Create-CaracteristicaCualitativa" */ './components/caracteristica_cualitativa-create-panel/Caracteristica_cualitativaCreatePanel'));
const CaracteristicaCualitativaUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-CaracteristicaCualitativa" */ './components/caracteristica_cualitativa-update-panel/Caracteristica_cualitativaUpdatePanel'));
const CaracteristicaCualitativaDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-CaracteristicaCualitativa" */ './components/caracteristica_cualitativa-detail-panel/Caracteristica_cualitativaDetailPanel'));
const CaracteristicaCualitativaDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-CaracteristicaCualitativa" */ './components/Caracteristica_cualitativaDeleteConfirmationDialog'));
//Plotly
const CaracteristicaCualitativaPlotly = lazy(() => import(/* webpackChunkName: "Plotly-CaracteristicaCualitativa" */ '../../../../plots/Caracteristica_cualitativaPlotly'));
...
export default function CaracteristicaCualitativaEnhancedTable(props) {
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'caracteristica_cualitativa', method: 'getCount()', request: 'api.caracteristica_cualitativa.getCountItems'}];
            newError.path=['Caracteristica_cualitativas'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'caracteristica_cualitativa', method: 'getData()', request: 'api.caracteristica_cualitativa.getItems'}];
            newError.path=['Caracteristica_cualitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['caracteristica_cualitativa']
    &&(Object.keys(changes.lastModelChanged['caracteristica_cualitativa']).length===1)
    &&(changes.lastModelChanged['caracteristica_cualitativa'][Object.keys(changes.lastModelChanged['caracteristica_cualitativa'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    /*
      API Request: api.caracteristica_cualitativa.deleteItem
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'caracteristica_cualitativa', method: 'doDelete()', request: 'api.caracteristica_cualitativa.deleteItem'}];
            newError.path=['Caracteristica_cualitativas', `id:${item.id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cualitativa.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.caracteristica_cualitativa.tableTemplate
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'caracteristica_cualitativa', method: 'getCsvTemplate()', request: 'api.caracteristica_cualitativa.tableTemplate'}];
            newError.path=['Caracteristica_cualitativas', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateCaracteristica_cualitativa.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "caracteristica_cualitativa-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cualitativa.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <CaracteristicaCualitativaEnhancedTableToolbar
                ...
                >
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='CaracteristicaCualitativaEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='CaracteristicaCualitativaEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <CaracteristicaCualitativaEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='CaracteristicaCualitativaEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'CaracteristicaCualitativaEnhancedTable-row-'+item.id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'CaracteristicaCualitativaEnhancedTable-row-iconButton-detail-'+item.id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.caracteristica_cualitativa&&Array.isArray(permissions.caracteristica_cualitativa)
                                      &&(permissions.caracteristica_cualitativa.includes('update') || permissions.caracteristica_cualitativa.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'CaracteristicaCualitativaEnhancedTable-row-iconButton-edit-'+item.id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.caracteristica_cualitativa&&Array.isArray(permissions.caracteristica_cualitativa)
                                      &&(permissions.caracteristica_cualitativa.includes('delete') || permissions.caracteristica_cualitativa.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'CaracteristicaCualitativaEnhancedTable-row-iconButton-delete-'+item.id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.nombre!==null)?item.nombre:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.valor!==null)?item.valor:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.nombre_corto!==null)?item.nombre_corto:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.comentarios!==null)?item.comentarios:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.metodo_id!==null)?item.metodo_id:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.registro_id!==null)?item.registro_id:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <CaracteristicaCualitativaCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(CaracteristicaCualitativaPlotly)) && (
                      <Suspense fallback={<div />}>
                        <CaracteristicaCualitativaPlotly />
                      </Suspense>
                    )}
                  </div>
      ...
      {(createDialogOpen) && (
        <Suspense fallback={<div />}>
          <CaracteristicaCualitativaCreatePanel
            permissions={permissions}
      ...
      {(updateDialogOpen) && (
        <Suspense fallback={<div />}>
          <CaracteristicaCualitativaUpdatePanel
            permissions={permissions}
            item={updateItem}
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <CaracteristicaCualitativaDeleteConfirmationDialog
            permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <CaracteristicaCualitativaUploadFileDialog
...
CaracteristicaCualitativaEnhancedTable.propTypes = {
