  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.referencia.getCountItems(graphqlServerUrl, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'registro', association: 'referencias', table:'A', method: 'getCount()', request: 'api.referencia.getCountItems'}];
            newError.path=['add', 'referencias'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'referencias', table:'A', method: 'getData()', request: 'api.referencia.getItems'}];
            newError.path=['add', 'referencias'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
