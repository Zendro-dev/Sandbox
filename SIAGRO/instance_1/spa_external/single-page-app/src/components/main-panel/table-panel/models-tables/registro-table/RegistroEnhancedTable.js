...
import RegistroEnhancedTableHead from './components/RegistroEnhancedTableHead'
import RegistroEnhancedTableToolbar from './components/RegistroEnhancedTableToolbar'
import RegistroUploadFileDialog from './components/RegistroUploadFileDialog'
import RegistroCursorPagination from './components/RegistroCursorPagination'
...
//lazy loading
const RegistroCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Registro" */ './components/registro-create-panel/RegistroCreatePanel'));
const RegistroUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Registro" */ './components/registro-update-panel/RegistroUpdatePanel'));
const RegistroDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Registro" */ './components/registro-detail-panel/RegistroDetailPanel'));
const RegistroDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Registro" */ './components/RegistroDeleteConfirmationDialog'));
//Plotly
const RegistroPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Registro" */ '../../../../plots/RegistroPlotly'));
...
export default function RegistroEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('conabio_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.registro.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'registro', method: 'getCount()', request: 'api.registro.getCountItems'}];
            newError.path=['Registros'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', method: 'getData()', request: 'api.registro.getItems'}];
            newError.path=['Registros'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['registro']
    &&(Object.keys(changes.lastModelChanged['registro']).length===1)
    &&(changes.lastModelChanged['registro'][Object.keys(changes.lastModelChanged['registro'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set conabio_id (internalId)
    variables.conabio_id = item.conabio_id;

    /*
      API Request: api.registro.deleteItem
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', method: 'doDelete()', request: 'api.registro.deleteItem'}];
            newError.path=['Registros', `conabio_id:${item.conabio_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.registro.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.registro.tableTemplate
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', method: 'getCsvTemplate()', request: 'api.registro.tableTemplate'}];
            newError.path=['Registros', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateRegistro.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "registro-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.registro.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='RegistroEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='RegistroEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <RegistroEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='RegistroEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'RegistroEnhancedTable-row-'+item.conabio_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'RegistroEnhancedTable-row-iconButton-detail-'+item.conabio_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.registro&&Array.isArray(permissions.registro)
                                      &&(permissions.registro.includes('update') || permissions.registro.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'RegistroEnhancedTable-row-iconButton-edit-'+item.conabio_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.registro&&Array.isArray(permissions.registro)
                                      &&(permissions.registro.includes('delete') || permissions.registro.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'RegistroEnhancedTable-row-iconButton-delete-'+item.conabio_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'conabio_id: ' + item.conabio_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.conabio_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.clave_original!==null)?item.clave_original:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.tipo_alimento!==null)?item.tipo_alimento:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.food_type!==null)?item.food_type:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.descripcion_alimento!==null)?item.descripcion_alimento:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.food_description!==null)?item.food_description:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.procedencia!==null)?item.procedencia:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.referencias_ids!==null)?item.referencias_ids:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <RegistroDeleteConfirmationDialog
            permissions={permissions}
...
RegistroEnhancedTable.propTypes = {
