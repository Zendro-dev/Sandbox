...
import CaracteristicasCualitativasToRemoveTransferViewToolbar from './components/Caracteristicas_cualitativasToRemoveTransferViewToolbar';
import CaracteristicasCualitativasToRemoveTransferViewCursorPagination from './components/Caracteristicas_cualitativasToRemoveTransferViewCursorPagination';
...
export default function CaracteristicasCualitativasToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.metodo.getCaracteristicas_cualitativasCount
    */
    let api = await loadApi("metodo");
    let cancelableApiReq = makeCancelable(api.metodo.getCaracteristicas_cualitativasCount(
      graphqlServerUrl, 
      item.id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'caracteristicas_cualitativas', method: 'getCount()', request: 'api.metodo.getCaracteristicas_cualitativasCount'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'caracteristicas_cualitativas'];
      ...
      //error
      .catch((err) => { //error: on api.metodo.getCaracteristicas_cualitativasCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.metodo.getCaracteristicas_cualitativas
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("metodo");
    let cancelableApiReq = makeCancelable(api.metodo.getCaracteristicas_cualitativas(
      graphqlServerUrl, 
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'caracteristicas_cualitativas', method: 'getData()', request: 'api.metodo.getCaracteristicas_cualitativas'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'caracteristicas_cualitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.metodo.getCaracteristicas_cualitativas
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'caracteristica_cualitativa' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <CaracteristicasCualitativasToRemoveTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CaracteristicasCualitativasToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='CaracteristicasCualitativasToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'CaracteristicasCualitativasToRemoveTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'CaracteristicasCualitativasToRemoveTransferView-listA-listItem-'+it.id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <CaracteristicasCualitativasToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CaracteristicasCualitativasToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CaracteristicasCualitativasToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='CaracteristicasCualitativasToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = it.nombre_corto;
                        ...
                        return (
                          <ListItem 
                            id={'CaracteristicasCualitativasToRemoveTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'CaracteristicasCualitativasToRemoveTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
CaracteristicasCualitativasToRemoveTransferView.propTypes = {
