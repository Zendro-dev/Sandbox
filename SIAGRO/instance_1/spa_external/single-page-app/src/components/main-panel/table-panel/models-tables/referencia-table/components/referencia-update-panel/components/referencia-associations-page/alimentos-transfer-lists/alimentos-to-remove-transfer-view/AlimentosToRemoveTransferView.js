...
import AlimentosToRemoveTransferViewCursorPagination from './components/AlimentosToRemoveTransferViewCursorPagination';
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.referencia.getAlimentosCount
    */
    let api = await loadApi("referencia");
    let cancelableApiReq = makeCancelable(api.referencia.getAlimentosCount(
      graphqlServerUrl, 
      item.referencia_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'alimentos', method: 'getCount()', request: 'api.referencia.getAlimentosCount'}];
            newError.path=['update', `referencia_id:${item.referencia_id}`, 'remove', 'alimentos'];
      ...
      //error
      .catch((err) => { //error: on api.referencia.getAlimentosCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {conabio_id: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.referencia.getAlimentos
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("referencia");
    let cancelableApiReq = makeCancelable(api.referencia.getAlimentos(
      graphqlServerUrl, 
      item.referencia_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'alimentos', method: 'getData()', request: 'api.referencia.getAlimentos'}];
            newError.path=['update', `referencia_id:${item.referencia_id}`, 'remove', 'alimentos'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.referencia.getAlimentos
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.referencia_id, search, getCount, configurePagination, onEmptyPage]);
            ...
            if(entry[1].changedAssociations.registro_referencias_ids.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.conabio_id;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.conabio_id) === -1) {
      lidsToRemove.current.push(item.conabio_id);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.conabio_id);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    <List id='AlimentosToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        let disabled = lidsToRemove.current.find(conabio_id=> conabio_id=== it.conabio_id) !== undefined;

                        return (
                          <ListItem 
                            id={'AlimentosToRemoveTransferView-listA-listItem-'+it.conabio_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'AlimentosToRemoveTransferView-listA-listItem-'+it.conabio_id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <AlimentosToRemoveTransferViewCursorPagination
                count={count}
                  ...
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='AlimentosToRemoveTransferView-list-listB'
                      ...
                      {itemsB.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'AlimentosToRemoveTransferView-listB-listItem-'+it.conabio_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'AlimentosToRemoveTransferView-listB-listItem-'+it.conabio_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
