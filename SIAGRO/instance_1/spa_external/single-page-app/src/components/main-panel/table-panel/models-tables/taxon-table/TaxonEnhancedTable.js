...
//lazy loading
const TaxonCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Taxon" */ './components/taxon-create-panel/TaxonCreatePanel'));
const TaxonUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Taxon" */ './components/taxon-update-panel/TaxonUpdatePanel'));
const TaxonDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Taxon" */ './components/taxon-detail-panel/TaxonDetailPanel'));
const TaxonDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Taxon" */ './components/TaxonDeleteConfirmationDialog'));
//Plotly
const TaxonPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Taxon" */ '../../../../plots/TaxonPlotly'));
...
export default function TaxonEnhancedTable(props) {
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Taxon', method: 'getCount()', request: 'api.taxon.getCountItems'}];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Taxon', method: 'getData()', request: 'api.taxon.getItems'}];
            newError.path=['Taxons'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['Taxon']
    &&(Object.keys(changes.lastModelChanged['Taxon']).length===1)
    &&(changes.lastModelChanged['Taxon'][Object.keys(changes.lastModelChanged['Taxon'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.taxon.tableTemplate
    */
    let api = await loadApi("taxon");
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Taxon', method: 'getCsvTemplate()', request: 'api.taxon.tableTemplate'}];
                                  ...
                                  >
                                    ...
                                    */}
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.categoriaSorter!==null)?item.categoriaSorter:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.bibliografia!==null)?item.bibliografia:'')}
                                    </TableCell>
