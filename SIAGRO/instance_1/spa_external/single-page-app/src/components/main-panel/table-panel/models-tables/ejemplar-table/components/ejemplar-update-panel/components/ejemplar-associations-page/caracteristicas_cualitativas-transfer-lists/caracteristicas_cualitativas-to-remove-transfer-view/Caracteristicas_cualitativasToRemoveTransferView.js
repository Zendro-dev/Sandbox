...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ejemplar.getCaracteristicas_cualitativasCount
    */
    let api = await loadApi("ejemplar");
    let cancelableApiReq = makeCancelable(api.ejemplar.getCaracteristicas_cualitativasCount(
      graphqlServerUrl, 
      item.id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'caracteristicas_cualitativas', method: 'getCount()', request: 'api.ejemplar.getCaracteristicas_cualitativasCount'}];
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getCaracteristicas_cualitativasCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ejemplar.getCaracteristicas_cualitativas
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("ejemplar");
    let cancelableApiReq = makeCancelable(api.ejemplar.getCaracteristicas_cualitativas(
      graphqlServerUrl, 
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'caracteristicas_cualitativas', method: 'getData()', request: 'api.ejemplar.getCaracteristicas_cualitativas'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'caracteristicas_cualitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getCaracteristicas_cualitativas
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
