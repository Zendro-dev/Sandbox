...
import ReferenciasToRemoveTransferViewToolbar from './components/ReferenciasToRemoveTransferViewToolbar';
import ReferenciasToRemoveTransferViewCursorPagination from './components/ReferenciasToRemoveTransferViewCursorPagination';
...
export default function ReferenciasToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.registro.getReferenciasCount
    */
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.getReferenciasCount(
      graphqlServerUrl, 
      item.conabio_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'referencias', method: 'getCount()', request: 'api.registro.getReferenciasCount'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'remove', 'referencias'];
      ...
      //error
      .catch((err) => { //error: on api.registro.getReferenciasCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {referencia_id: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.registro.getReferencias
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("registro");
    let cancelableApiReq = makeCancelable(api.registro.getReferencias(
      graphqlServerUrl, 
      item.conabio_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'referencias', method: 'getData()', request: 'api.registro.getReferencias'}];
            newError.path=['update', `conabio_id:${item.conabio_id}`, 'remove', 'referencias'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.registro.getReferencias
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.conabio_id, search, getCount, configurePagination, onEmptyPage]);
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'referencia' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.referencia_registros_ids.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.referencia_id;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.referencia_id) === -1) {
      lidsToRemove.current.push(item.referencia_id);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.referencia_id);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ReferenciasToRemoveTransferViewToolbar
                title={'Referencia'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ReferenciasToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ReferenciasToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.referencia_id;
                        let label = it.referencia;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(referencia_id=> referencia_id=== it.referencia_id) !== undefined;

                        return (
                          <ListItem 
                            id={'ReferenciasToRemoveTransferView-listA-listItem-'+it.referencia_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'referencia_id' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.referencia_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'ReferenciasToRemoveTransferView-listA-listItem-'+it.referencia_id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <ReferenciasToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ReferenciasToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ReferenciasToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ReferenciasToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.referencia_id;
                        let label = it.referencia;
                        ...
                        return (
                          <ListItem 
                            id={'ReferenciasToRemoveTransferView-listB-listItem-'+it.referencia_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'ReferenciasToRemoveTransferView-listB-listItem-'+it.referencia_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ReferenciasToRemoveTransferView.propTypes = {
