...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.metodo.getCaracteristicas_cuantitativasCount
    */
    let api = await loadApi("metodo");
    let cancelableApiReq = makeCancelable(api.metodo.getCaracteristicas_cuantitativasCount(
      graphqlServerUrl, 
      item.id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'caracteristicas_cuantitativas', method: 'getCount()', request: 'api.metodo.getCaracteristicas_cuantitativasCount'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'caracteristicas_cuantitativas'];
      ...
      //error
      .catch((err) => { //error: on api.metodo.getCaracteristicas_cuantitativasCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.metodo.getCaracteristicas_cuantitativas
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("metodo");
    let cancelableApiReq = makeCancelable(api.metodo.getCaracteristicas_cuantitativas(
      graphqlServerUrl, 
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'caracteristicas_cuantitativas', method: 'getData()', request: 'api.metodo.getCaracteristicas_cuantitativas'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'caracteristicas_cuantitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.metodo.getCaracteristicas_cuantitativas
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
