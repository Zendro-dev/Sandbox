...
import TaxonToAddTransferViewToolbar from './components/TaxonToAddTransferViewToolbar';
import TaxonToAddTransferViewCursorPagination from './components/TaxonToAddTransferViewCursorPagination';
...
export default function TaxonToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.ejemplar.getTaxon
    */
    let api = await loadApi("ejemplar");
    let cancelableApiReq = makeCancelable(api.ejemplar.getTaxon(graphqlServerUrl, item.id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Ejemplar', association: 'Taxon', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.ejemplar.getTaxon'}];
            newError.path=['update', `id:${currentId}`, 'add', 'Taxon'];
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getTaxon
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ejemplar.getNotAssociatedTaxonCount
    */
    let api = await loadApi("ejemplar");
    let cancelableApiReq = makeCancelable(api.ejemplar.getNotAssociatedTaxonCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Ejemplar', association: 'Taxon', table:'A', method: 'getCount()', request: 'api.ejemplar.getNotAssociatedTaxonCount'}];
            newError.path=['update', `id:${item.id}`, 'add', 'Taxon'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getNotAssociatedTaxonCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ejemplar.getNotAssociatedTaxon
    */
    let api = await loadApi("ejemplar");
    let cancelableApiReq = makeCancelable(api.ejemplar.getNotAssociatedTaxon(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Ejemplar', association: 'Taxon', table:'A', method: 'getData()', request: 'api.ejemplar.getNotAssociatedTaxon'}];
            newError.path=['update', `id:${item.id}`, 'add', 'Taxon'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getNotAssociatedTaxon
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Ejemplar', association: 'Taxon', table:'B', method: 'getDataB()', request: 'api.taxon.getItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'Taxon'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'Taxon' for this item was updated.
    ...
    if(lastModelChanged.Ejemplar&&
        ...
        lastModelChanged.Ejemplar[String(item.id)].changedAssociations.Ejemplar_Taxon&&
        (lastModelChanged.Ejemplar[String(item.id)].changedAssociations.Ejemplar_Taxon.added ||
         lastModelChanged.Ejemplar[String(item.id)].changedAssociations.Ejemplar_Taxon.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.Ejemplar[String(item.id)].changedAssociations.Ejemplar_Taxon.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('Taxon', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('Taxon', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.Ejemplar[String(item.id)].changedAssociations.Ejemplar_Taxon.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('Taxon', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'Taxon' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'Taxon',
     * but this action happened on the peer relation, identified by 'Ejemplar_Taxon'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.Taxon) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.Ejemplar_Taxon) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.Ejemplar_Taxon.removed) {
              let idsRemoved = entry[1].changedAssociations.Ejemplar_Taxon.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.Ejemplar_Taxon.added) {
              let idsAdded = entry[1].changedAssociations.Ejemplar_Taxon.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('Taxon', idAdded);
                  } 
    ...
    if(lastModelChanged.Taxon) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('Taxon', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      if(associatedItem && checked) {
        ...
        handleDisassociateItem('Taxon', associatedItem.id);
      }
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('Taxon', item.id);
    }
  };
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TaxonToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='TaxonToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'TaxonToAddTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'TaxonToAddTransferView-listA-listItem-id-'+it.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'TaxonToAddTransferView-listA-listItem-'+it.id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <TaxonToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TaxonToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TaxonToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='TaxonToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'TaxonToAddTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'TaxonToAddTransferView-listB-listItem-id-'+it.id}
                                        variant="body1" display="block" noWrap={true}>{it.id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'TaxonToAddTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='TaxonToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        ...
                        return (
                          <ListItem 
                            id={'TaxonToAddTransferView-associatedItem-list-listItem-'+associatedItem.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'TaxonToAddTransferView-listB-listItem-id-'+associatedItem.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'TaxonToAddTransferView-associatedItem-list-listItem-'+associatedItem.id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      lidsToRemove.current = [];
                                      handleReassociateItem('Taxon', associatedItem.id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.id];
                                      handleDisassociateItem('Taxon', associatedItem.id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TaxonToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
TaxonToAddTransferView.propTypes = {
