...
export default function CaracteristicasCuantitativasToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.metodo.getNotAssociatedCaracteristicas_cuantitativasCount
    */
    let api = await loadApi("metodo");
    let cancelableApiReq = makeCancelable(api.metodo.getNotAssociatedCaracteristicas_cuantitativasCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'metodo', association: 'caracteristicas_cuantitativas', table:'A', method: 'getCount()', request: 'api.metodo.getNotAssociatedCaracteristicas_cuantitativasCount'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cuantitativas'];
      ...
      //error
      .catch((err) => { //error: on api.metodo.getNotAssociatedCaracteristicas_cuantitativasCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.metodo.getNotAssociatedCaracteristicas_cuantitativas
    */
    let api = await loadApi("metodo");
    let cancelableApiReq = makeCancelable(api.metodo.getNotAssociatedCaracteristicas_cuantitativas(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'metodo', association: 'caracteristicas_cuantitativas', table:'A', method: 'getData()', request: 'api.metodo.getNotAssociatedCaracteristicas_cuantitativas'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cuantitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.metodo.getNotAssociatedCaracteristicas_cuantitativas
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'metodo', association: 'caracteristicas_cuantitativas', table:'B', method: 'getCountB()', request: 'api.caracteristica_cuantitativa.getCountItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cuantitativas'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'metodo', association: 'caracteristicas_cuantitativas', table:'B', method: 'getDataB()', request: 'api.caracteristica_cuantitativa.getItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cuantitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.metodo&&
        ...
        lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cuantitativa_metodo_id&&
        (lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cuantitativa_metodo_id.added ||
         lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cuantitativa_metodo_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cuantitativa_metodo_id.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cuantitativa_metodo_id.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'caracteristica_cuantitativa_metodo_id'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.caracteristica_cuantitativa) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.caracteristica_cuantitativa_metodo_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.caracteristica_cuantitativa_metodo_id.removed) {
              let idsRemoved = entry[1].changedAssociations.caracteristica_cuantitativa_metodo_id.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.caracteristica_cuantitativa_metodo_id.added) {
              let idsAdded = entry[1].changedAssociations.caracteristica_cuantitativa_metodo_id.idsAdded;
              if(idsAdded) {
