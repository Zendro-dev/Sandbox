...
export default function GrupoEnfoqueUpdatePanel(props) {
  ...
  function setAddRemoveSitio(variables) {
    ...
    changedAssociations.current.sitio = {added: false, removed: false};
    ...
    if(item.sitio&&item.sitio.sitio_id) {
      ...
      if(sitioIdsToAdd.current.length>0) {
        ...
        if(item.sitio.sitio_id!== sitioIdsToAdd.current[0]) {
          ...
          changedAssociations.current.sitio.added = true;
          changedAssociations.current.sitio.idsAdded = sitioIdsToAdd.current;
          changedAssociations.current.sitio.removed = true;
          changedAssociations.current.sitio.idsRemoved = [item.sitio.sitio_id];
        } else {
          /*
