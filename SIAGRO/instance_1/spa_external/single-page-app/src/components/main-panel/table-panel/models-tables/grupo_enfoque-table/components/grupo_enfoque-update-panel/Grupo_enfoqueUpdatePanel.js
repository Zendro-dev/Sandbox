...
export default function GrupoEnfoqueUpdatePanel(props) {
  ...
  const sitioIdsToAdd = useRef((item.sitio&& item.sitio.sitio_id) ? [item.sitio.sitio_id] : []);
  const [sitioIdsToAddState, setSitioIdsToAddState] = useState((item.sitio&& item.sitio.sitio_id) ? [item.sitio.sitio_id] : []);
  ...
  function getInitialForeignKeys() {
    ...
    initialForeignKeys.sitio_id = item.sitio_id;
  ...
  function setAddRemoveSitio(variables) {
     ...
     */
    if(item.sitio&&item.sitio.sitio_id) {
      /*
      ...
      if(sitioIdsToAdd.current.length>0) {
         ...
         */
        if(item.sitio.sitio_id!== sitioIdsToAdd.current[0]) {
          //set id to add
          variables.addSitio = sitioIdsToAdd.current[0];
          
          changedAssociations.current.grupo_enfoque_sitio_id.added = true;
          changedAssociations.current.grupo_enfoque_sitio_id.idsAdded = sitioIdsToAdd.current;
          changedAssociations.current.grupo_enfoque_sitio_id.removed = true;
          changedAssociations.current.grupo_enfoque_sitio_id.idsRemoved = [item.sitio.sitio_id];
        } else {
          /*
         ...
         */
        //set id to remove
        variables.removeSitio = item.sitio.sitio_id;
...
function setAjvErrors(err) {
  ...
  function doSave(event) {
    ...
    */
    ...
    delete variables.sitio_id; //FK

    //add & remove: to_one's
    setAddRemoveSitio(variables);
    ...
    if(cuadrantesIdsToAdd.current.length>0) {
      ...
      changedAssociations.current.cuadrante_grupo_enfoque_id.idsAdded = cuadrantesIdsToAdd.current;
    }
    if(cuadrantesIdsToRemove.current.length>0) {
      ...
      changedAssociations.current.cuadrante_grupo_enfoque_id.idsRemoved = cuadrantesIdsToRemove.current;
    }
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      ...
      case 'sitio':
        ...
        handleSetValue(itemId, -2, 'sitio_id');
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    ...
    if(associationKey === 'sitio') {
      ...
      handleSetValue(null, -2, 'sitio_id');
  ...
  return (
    <Dialog id='GrupoEnfoqueUpdatePanel-dialog' 
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='GrupoEnfoqueUpdatePanel-button-cancel'
              edge="start" 
              color="inherit" 
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " grupo_enfoque" }>
              <Fab
                id='GrupoEnfoqueUpdatePanel-fabButton-save' 
                color="secondary" 
