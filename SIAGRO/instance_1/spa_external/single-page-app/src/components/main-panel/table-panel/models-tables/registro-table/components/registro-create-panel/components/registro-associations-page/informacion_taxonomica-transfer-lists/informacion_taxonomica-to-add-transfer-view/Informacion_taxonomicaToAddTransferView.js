...
const cancelableCountingPromises = useRef([]);
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.taxon.getCountItems
    */
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'registro', association: 'informacion_taxonomica', table:'A', method: 'getCount()', request: 'api.taxon.getCountItems'}];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'registro', association: 'informacion_taxonomica', table:'A', method: 'getData()', request: 'api.taxon.getItems'}];
            newError.path=['add', 'informacion_taxonomica'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
