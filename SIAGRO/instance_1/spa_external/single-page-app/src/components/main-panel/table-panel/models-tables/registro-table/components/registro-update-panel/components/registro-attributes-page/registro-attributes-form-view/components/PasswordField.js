...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['registro'][String(itemId['conabio_id'])]) {

        //updated password
        if(lastModelChanged['registro'][String(itemId['conabio_id'])].op === "updatePassword"&&
            lastModelChanged['registro'][String(itemId['conabio_id'])].newItem) {
              //update password value
              setValue(lastModelChanged['registro'][String(itemId['conabio_id'])].newItem[itemKey]);
        }
    }//end: Case 1
  ...
  /**
    ...
    */
    ...
    return await api.registro.updateItem(graphqlServerUrl, variables)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['Registros', itemId, 'update', 'change password'];
  ...
  return (
    <div>
      <Grid container justify='flex-start' alignItems='center' spacing={0}>
        <Grid item xs={12}>
          <form noValidate autoComplete="off">
            <TextField
              id={'PasswordField-Registro-'+name}
              label={label}
              value={value}
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'PasswordField-Registro-button-showPassword'+name}
                    ...
                    {(valueOk!==undefined&&valueOk===1) 
                      ...
                      <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                        <Typography id={'PasswordField-Registro-exists-'+name} variant="caption" color="primary">
                          &#8707;
                        </Typography>
                      </Tooltip>
                      : 
                      <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                        <Typography id={'PasswordField-Registro-notExists-'+name} variant="caption" color="textSecondary">
                          &#8708;
                        </Typography>
                      </Tooltip>
                    }
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'PasswordField-Registro-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-Registro-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-Registro-ChangePasswordDialog-textField-newPassword2-'+name}
