...
export default function CaracteristicasCualitativasToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ejemplar.getNotAssociatedCaracteristicas_cualitativasCount
    */
    let api = await loadApi("ejemplar");
    let cancelableApiReq = makeCancelable(api.ejemplar.getNotAssociatedCaracteristicas_cualitativasCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Ejemplar', association: 'caracteristicas_cualitativas', table:'A', method: 'getCount()', request: 'api.ejemplar.getNotAssociatedCaracteristicas_cualitativasCount'}];
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getNotAssociatedCaracteristicas_cualitativasCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ejemplar.getNotAssociatedCaracteristicas_cualitativas
    */
    let api = await loadApi("ejemplar");
    let cancelableApiReq = makeCancelable(api.ejemplar.getNotAssociatedCaracteristicas_cualitativas(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Ejemplar', association: 'caracteristicas_cualitativas', table:'A', method: 'getData()', request: 'api.ejemplar.getNotAssociatedCaracteristicas_cualitativas'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cualitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getNotAssociatedCaracteristicas_cualitativas
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Ejemplar', association: 'caracteristicas_cualitativas', table:'B', method: 'getCountB()', request: 'api.caracteristica_cualitativa.getCountItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cualitativas'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Ejemplar', association: 'caracteristicas_cualitativas', table:'B', method: 'getDataB()', request: 'api.caracteristica_cualitativa.getItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cualitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.Ejemplar&&
        ...
        lastModelChanged.Ejemplar[String(item.id)].changedAssociations&&
        lastModelChanged.Ejemplar[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id&&
        (lastModelChanged.Ejemplar[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id.added ||
         lastModelChanged.Ejemplar[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.Ejemplar[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.Ejemplar[String(item.id)].changedAssociations.caracteristica_cualitativa_registro_id.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'caracteristica_cualitativa_registro_id'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.caracteristica_cualitativa) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.caracteristica_cualitativa_registro_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.caracteristica_cualitativa_registro_id.removed) {
              let idsRemoved = entry[1].changedAssociations.caracteristica_cualitativa_registro_id.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.caracteristica_cualitativa_registro_id.added) {
              let idsAdded = entry[1].changedAssociations.caracteristica_cualitativa_registro_id.idsAdded;
              if(idsAdded) {
