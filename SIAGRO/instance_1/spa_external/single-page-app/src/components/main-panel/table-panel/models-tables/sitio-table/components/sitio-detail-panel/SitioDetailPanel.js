...
import SitioUpdatePanel from '../sitio-update-panel/SitioUpdatePanel'
import SitioDeleteConfirmationDialog from '../SitioDeleteConfirmationDialog'
...
export default function SitioDetailPanel(props) {
      ...
      lastModelChanged['sitio'][String(item['sitio_id'])]) {
        ...
        if(lastModelChanged['sitio'][String(item['sitio_id'])].op === "update"&&
            lastModelChanged['sitio'][String(item['sitio_id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['sitio'][String(item['sitio_id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteSitio
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('sitio', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('sitio', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='SitioDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Sitio | sitio_id: " + itemState['sitio_id'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='SitioDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='SitioDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
