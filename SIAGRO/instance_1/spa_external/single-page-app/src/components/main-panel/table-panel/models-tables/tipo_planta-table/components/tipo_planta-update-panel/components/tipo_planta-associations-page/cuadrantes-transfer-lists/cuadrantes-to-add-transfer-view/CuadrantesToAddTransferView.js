...
import CuadrantesToAddTransferViewToolbar from './components/CuadrantesToAddTransferViewToolbar';
import CuadrantesToAddTransferViewCursorPagination from './components/CuadrantesToAddTransferViewCursorPagination';
...
export default function CuadrantesToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    /*
      API Request: readOneTipo_planta
    */
    let cancelableApiReq = makeCancelable(api.tipo_planta.getAssociatedCuadrantesConnection(graphqlServerUrl, item.tipo_planta_id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'A', query: 'readOneTipo_planta', method: 'getData()', request: 'api.tipo_planta.getAssociatedCuadrantesConnection'}];
          newError.path=['add', 'cuadrantes'];
        ...
        //check: readOneTipo_planta
        let readOneTipo_planta = response.data.data.readOneTipo_planta;
        if(readOneTipo_planta === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneTipo_planta ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'A', query: 'readOneTipo_planta', method: 'getData()', request: 'api.tipo_planta.getAssociatedCuadrantesConnection'}];
          newError.path=['add', 'cuadrantes'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: readOneTipo_planta type
        if(typeof readOneTipo_planta !== 'object'
        || typeof readOneTipo_planta.cuadrantesConnection !== 'object'
        || readOneTipo_planta.cuadrantesConnection === null
        || !Array.isArray(readOneTipo_planta.cuadrantesConnection.edges)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneTipo_planta ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'A', query: 'readOneTipo_planta', method: 'getData()', request: 'api.tipo_planta.getAssociatedCuadrantesConnection'}];
          newError.path=['add', 'cuadrantes'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //get items
        let idso = readOneTipo_planta.cuadrantesConnection.edges.map(o => o.node);

        //check: graphql errors
        if(response.data.errors) {
          let newError = {};
          newError.message = 'readOneTipo_planta ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'A', query: 'readOneTipo_planta', method: 'getData()', request: 'api.tipo_planta.getAssociatedCuadrantesConnection'}];
          newError.path=['add', 'cuadrantes'];
        ...
        //set associated ids
        lidsAssociated.current = idso.map(function(item){ return item.cuadrante_id});
        ...
        */
        let cancelableApiReqB = makeCancelable(api.cuadrante.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'A', query: 'countCuadrantes', method: 'getData()', request: 'api.cuadrante.getCountItems'}];
              newError.path=['add', 'cuadrantes'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.cuadrante.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'A', query: 'cuadrantesConnection', method: 'getData()', request: 'api.cuadrante.getItemsConnection'}];
                  newError.path=['add', 'cuadrantes'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(cuadrantesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'A', method: 'getData()'}];
                    newError.path=['add', 'cuadrantes'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.tipo_planta.getAssociatedCuadrantesConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, t, dataTrigger, item.tipo_planta_id, search, rowsPerPage]);
  ...
  const getDataB = useCallback(() => {
    ...
    if(lidsToAdd.current !== undefined && lidsToAdd.current.length > 0) {
      ops = {
        only: [{
          type: 'String',
          values: {cuadrante_id: lidsToAdd.current}
        }]
      };
    } else {
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'B', query: 'countCuadrantes', method: 'getDataB()', request: 'api.cuadrante.getCountItems'}];
          newError.path=['add', 'cuadrantes'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'B', query: 'cuadrantesConnection', method: 'getDataB()', request: 'api.cuadrante.getItemsConnection'}];
                newError.path=['add', 'cuadrantes'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'tipo_planta', association: 'cuadrantes', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'cuadrantes'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'cuadrantes', 
    ...
    if(lastModelChanged.tipo_planta&&
        ...
        lastModelChanged.tipo_planta[String(item.tipo_planta_id)].changedAssociations&&
        lastModelChanged.tipo_planta[String(item.tipo_planta_id)].changedAssociations.cuadrante_tipo_planta_id&&
        (lastModelChanged.tipo_planta[String(item.tipo_planta_id)].changedAssociations.cuadrante_tipo_planta_id.added ||
         lastModelChanged.tipo_planta[String(item.tipo_planta_id)].changedAssociations.cuadrante_tipo_planta_id.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.tipo_planta[String(item.tipo_planta_id)].changedAssociations.cuadrante_tipo_planta_id.idsRemoved;
          if(idsRemoved) {
     ...
     * 
     * else if B:
     * - remove the internalId of the 'cuadrante' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.cuadrante_tipo_planta_id) {
            ...
            if(entry[1].changedAssociations.cuadrante_tipo_planta_id.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.tipo_planta_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.cuadrante_tipo_planta_id.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.tipo_planta_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.cuadrante_id;
                  ...
                  handleUntransfer('cuadrantes', idAdded);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.tipo_planta_id, handleUntransfer]);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.cuadrante_id) === -1) {
      lidsToAdd.current.push(item.cuadrante_id);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.cuadrante_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <CuadrantesToAddTransferViewToolbar
                title={'Cuadrantes'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady || !isCountReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CuadrantesToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady && isCountReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='CuadrantesToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'CuadrantesToAddTransferView-listA-listItem-'+it.cuadrante_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'cuadrante_id' }>
                                        <Typography 
                                        id={'CuadrantesToAddTransferView-listA-listItem-id-'+it.cuadrante_id}
                                        variant="body1" display="block" noWrap={true}>{it.cuadrante_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'CuadrantesToAddTransferView-listA-listItem-'+it.cuadrante_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <CuadrantesToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CuadrantesToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB || !isCountReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CuadrantesToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB && isCountReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='CuadrantesToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.cuadrante_id;
                        let label = it.nombre_comun_grupo_enfoque;
                        ...
                        return (
                          <ListItem 
                            id={'CuadrantesToAddTransferView-listB-listItem-'+it.cuadrante_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'cuadrante_id' }>
                                        <Typography 
                                        id={'CuadrantesToAddTransferView-listB-listItem-id-'+it.cuadrante_id}
                                        variant="body1" display="block" noWrap={true}>{it.cuadrante_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'CuadrantesToAddTransferView-listB-listItem-'+it.cuadrante_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
CuadrantesToAddTransferView.propTypes = {
