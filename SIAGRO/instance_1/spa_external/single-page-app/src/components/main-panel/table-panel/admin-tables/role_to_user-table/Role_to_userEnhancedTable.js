...
//lazy loading
const RoleToUserCreatePanel = lazy(() => import(/* webpackChunkName: "Create-RoleToUser" */ './components/role_to_user-create-panel/Role_to_userCreatePanel'));
const RoleToUserUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-RoleToUser" */ './components/role_to_user-update-panel/Role_to_userUpdatePanel'));
const RoleToUserDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-RoleToUser" */ './components/role_to_user-detail-panel/Role_to_userDetailPanel'));
const RoleToUserDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-RoleToUser" */ './components/Role_to_userDeleteConfirmationDialog'));
//Plotly
const RoleToUserPlotly = lazy(() => import(/* webpackChunkName: "Plotly-RoleToUser" */ '../../../../plots/Role_to_userPlotly'));
...
export default function RoleToUserEnhancedTable(props) {
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.role_to_user.getCountItems
    */
    let api = await loadApi("role_to_user");
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'role_to_user', method: 'getCount()', request: 'api.role_to_user.getCountItems'}];
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.role_to_user.getItems
    */
    let api = await loadApi("role_to_user");
    let cancelableApiReq = makeCancelable(api.role_to_user.getItems(
      graphqlServerUrl,
      search,
      orderBy,
      order,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'role_to_user', method: 'getData()', request: 'api.role_to_user.getItems'}];
            newError.path=['Role_to_users'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.role_to_user.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['role_to_user']
    &&(Object.keys(changes.lastModelChanged['role_to_user']).length===1)
    &&(changes.lastModelChanged['role_to_user'][Object.keys(changes.lastModelChanged['role_to_user'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    /*
      API Request: api.role_to_user.deleteItem
    */
    let api = await loadApi("role_to_user");
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'role_to_user', method: 'doDelete()', request: 'api.role_to_user.deleteItem'}];
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.role_to_user.tableTemplate
    */
    let api = await loadApi("role_to_user");
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'role_to_user', method: 'getCsvTemplate()', request: 'api.role_to_user.tableTemplate'}];
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='RoleToUserEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='RoleToUserEnhancedTable-table' stickyHeader size='small' ref={tref}>
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='RoleToUserEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'RoleToUserEnhancedTable-row-'+item.id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'RoleToUserEnhancedTable-row-iconButton-detail-'+item.id}
                                          color="default"
                                          onClick={event => {
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'RoleToUserEnhancedTable-row-iconButton-edit-'+item.id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'RoleToUserEnhancedTable-row-iconButton-delete-'+item.id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
