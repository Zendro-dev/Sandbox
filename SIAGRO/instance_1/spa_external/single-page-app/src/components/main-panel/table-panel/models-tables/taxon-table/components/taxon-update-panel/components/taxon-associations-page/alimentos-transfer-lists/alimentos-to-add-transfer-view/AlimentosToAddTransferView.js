...
export default function AlimentosToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.taxon.getNotAssociatedAlimentosCount
    */
    let api = await loadApi("taxon");
    let cancelableApiReq = makeCancelable(api.taxon.getNotAssociatedAlimentosCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Taxon', association: 'alimentos', table:'A', method: 'getCount()', request: 'api.taxon.getNotAssociatedAlimentosCount'}];
            newError.path=['update', `id:${item.id}`, 'add', 'alimentos'];
      ...
      //error
      .catch((err) => { //error: on api.taxon.getNotAssociatedAlimentosCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.taxon.getNotAssociatedAlimentos
    */
    let api = await loadApi("taxon");
    let cancelableApiReq = makeCancelable(api.taxon.getNotAssociatedAlimentos(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Taxon', association: 'alimentos', table:'A', method: 'getData()', request: 'api.taxon.getNotAssociatedAlimentos'}];
            newError.path=['update', `id:${item.id}`, 'add', 'alimentos'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.taxon.getNotAssociatedAlimentos
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
