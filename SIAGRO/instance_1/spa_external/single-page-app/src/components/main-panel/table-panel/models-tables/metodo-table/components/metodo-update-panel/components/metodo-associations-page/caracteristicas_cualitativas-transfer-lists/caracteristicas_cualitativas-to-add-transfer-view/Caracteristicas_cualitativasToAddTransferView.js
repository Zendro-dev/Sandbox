...
import CaracteristicasCualitativasToAddTransferViewToolbar from './components/Caracteristicas_cualitativasToAddTransferViewToolbar';
import CaracteristicasCualitativasToAddTransferViewCursorPagination from './components/Caracteristicas_cualitativasToAddTransferViewCursorPagination';
...
export default function CaracteristicasCualitativasToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.metodo.getNotAssociatedCaracteristicas_cualitativasCount
    */
    let api = await loadApi("metodo");
    let cancelableApiReq = makeCancelable(api.metodo.getNotAssociatedCaracteristicas_cualitativasCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'metodo', association: 'caracteristicas_cualitativas', table:'A', method: 'getCount()', request: 'api.metodo.getNotAssociatedCaracteristicas_cualitativasCount'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cualitativas'];
      ...
      //error
      .catch((err) => { //error: on api.metodo.getNotAssociatedCaracteristicas_cualitativasCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.metodo.getNotAssociatedCaracteristicas_cualitativas
    */
    let api = await loadApi("metodo");
    let cancelableApiReq = makeCancelable(api.metodo.getNotAssociatedCaracteristicas_cualitativas(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'metodo', association: 'caracteristicas_cualitativas', table:'A', method: 'getData()', request: 'api.metodo.getNotAssociatedCaracteristicas_cualitativas'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cualitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.metodo.getNotAssociatedCaracteristicas_cualitativas
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.id, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cualitativa.getCountItems
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'metodo', association: 'caracteristicas_cualitativas', table:'B', method: 'getCountB()', request: 'api.caracteristica_cualitativa.getCountItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cualitativas'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.caracteristica_cualitativa.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cualitativa.getItems
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'metodo', association: 'caracteristicas_cualitativas', table:'B', method: 'getDataB()', request: 'api.caracteristica_cualitativa.getItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'caracteristicas_cualitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cualitativa.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'caracteristicas_cualitativas' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'caracteristica_cualitativas', 
     ...
     * - return
     */
    if(lastModelChanged.metodo&&
        lastModelChanged.metodo[String(item.id)]&&
        lastModelChanged.metodo[String(item.id)].changedAssociations&&
        lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id&&
        (lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id.added ||
         lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('caracteristicas_cualitativas', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.metodo[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'caracteristicas_cualitativas' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'caracteristica_cualitativa',
     * but this action happened on the peer relation, identified by 'caracteristica_cualitativa_metodo_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'caracteristica_cualitativa'.
     * B: the current item internalId is in the addedIds of the updated 'caracteristica_cualitativa'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'caracteristica_cualitativa' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.caracteristica_cualitativa) {
      let oens = Object.entries(lastModelChanged.caracteristica_cualitativa);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.caracteristica_cualitativa_metodo_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.caracteristica_cualitativa_metodo_id.removed) {
              let idsRemoved = entry[1].changedAssociations.caracteristica_cualitativa_metodo_id.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.caracteristica_cualitativa_metodo_id.added) {
              let idsAdded = entry[1].changedAssociations.caracteristica_cualitativa_metodo_id.idsAdded;
              if(idsAdded) {
    ...
    /*
     * Case 3: 
     * The attributes of some 'caracteristica_cualitativa' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.caracteristica_cualitativa) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('caracteristicas_cualitativas', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnCaracteristica_cualitativaRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('caracteristicas_cualitativas', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('caracteristicas_cualitativas', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <CaracteristicasCualitativasToAddTransferViewToolbar
                title={'Caracteristica_cualitativas'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CaracteristicasCualitativasToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='CaracteristicasCualitativasToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.id;
                        let label = it.nombre_corto;
                        let sublabel = it.valor;
                        
                        return (
                          <ListItem 
                            id={'CaracteristicasCualitativasToAddTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'caracteristica_cualitativa' }>
                                <Avatar>{"caracteristica_cualitativa".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'CaracteristicasCualitativasToAddTransferView-listA-listItem-id-'+it.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'CaracteristicasCualitativasToAddTransferView-listA-listItem-'+it.id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <CaracteristicasCualitativasToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CaracteristicasCualitativasToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CaracteristicasCualitativasToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='CaracteristicasCualitativasToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = it.nombre_corto;
                        ...
                        return (
                          <ListItem 
                            id={'CaracteristicasCualitativasToAddTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'CaracteristicasCualitativasToAddTransferView-listB-listItem-id-'+it.id}
                                        variant="body1" display="block" noWrap={true}>{it.id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'CaracteristicasCualitativasToAddTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
CaracteristicasCualitativasToAddTransferView.propTypes = {
