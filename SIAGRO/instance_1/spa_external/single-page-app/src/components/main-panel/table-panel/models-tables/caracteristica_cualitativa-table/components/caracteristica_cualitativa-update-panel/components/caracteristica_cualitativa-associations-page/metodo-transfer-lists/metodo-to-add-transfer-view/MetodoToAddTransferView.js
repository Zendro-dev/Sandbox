...
import MetodoToAddTransferViewToolbar from './components/MetodoToAddTransferViewToolbar';
import MetodoToAddTransferViewCursorPagination from './components/MetodoToAddTransferViewCursorPagination';
...
export default function MetodoToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cualitativa.getMetodo
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.getMetodo(graphqlServerUrl, item.id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'caracteristica_cualitativa', association: 'metodo', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.caracteristica_cualitativa.getMetodo'}];
            newError.path=['update', `id:${currentId}`, 'add', 'metodo'];
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cualitativa.getMetodo
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cualitativa.getNotAssociatedMetodoCount
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.getNotAssociatedMetodoCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'caracteristica_cualitativa', association: 'metodo', table:'A', method: 'getCount()', request: 'api.caracteristica_cualitativa.getNotAssociatedMetodoCount'}];
            newError.path=['update', `id:${item.id}`, 'add', 'metodo'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cualitativa.getNotAssociatedMetodoCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.caracteristica_cualitativa.getNotAssociatedMetodo
    */
    let api = await loadApi("caracteristica_cualitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cualitativa.getNotAssociatedMetodo(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'caracteristica_cualitativa', association: 'metodo', table:'A', method: 'getData()', request: 'api.caracteristica_cualitativa.getNotAssociatedMetodo'}];
            newError.path=['update', `id:${item.id}`, 'add', 'metodo'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cualitativa.getNotAssociatedMetodo
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'caracteristica_cualitativa', association: 'metodo', table:'B', method: 'getDataB()', request: 'api.metodo.getItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'metodo'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'metodo' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'metodos', 
    ...
    if(lastModelChanged.caracteristica_cualitativa&&
        lastModelChanged.caracteristica_cualitativa[String(item.id)]&&
        lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations&&
        lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id&&
        (lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id.added ||
         lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('metodo', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('metodo', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.caracteristica_cualitativa[String(item.id)].changedAssociations.caracteristica_cualitativa_metodo_id.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('metodo', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'metodo' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'metodo',
     * but this action happened on the peer relation, identified by 'caracteristica_cualitativa_metodo_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'metodo'.
     * B: the current item internalId is in the addedIds of the updated 'metodo'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'metodo' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.metodo) {
      let oens = Object.entries(lastModelChanged.metodo);
      oens.forEach( (entry) => {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('metodo', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'metodo' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.metodo) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('metodo', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnMetodoRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      if(associatedItem && checked) {
        ...
        handleDisassociateItem('metodo', associatedItem.id);
      }
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('metodo', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MetodoToAddTransferViewToolbar
                title={'Metodos'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MetodoToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='MetodoToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'MetodoToAddTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.id===it.id) ? 'metodo — ' + t('modelPanels.associatedRecord', 'Associated record') : 'metodo '}>
                                <Avatar>
                                  {"metodo".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'MetodoToAddTransferView-listA-listItem-id-'+it.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'MetodoToAddTransferView-listA-listItem-'+it.id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <MetodoToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MetodoToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MetodoToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='MetodoToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'MetodoToAddTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'MetodoToAddTransferView-listB-listItem-id-'+it.id}
                                        variant="body1" display="block" noWrap={true}>{it.id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'MetodoToAddTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='MetodoToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        ...
                        return (
                          <ListItem 
                            id={'MetodoToAddTransferView-associatedItem-list-listItem-'+associatedItem.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'metodo — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"metodo".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'MetodoToAddTransferView-listB-listItem-id-'+associatedItem.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'MetodoToAddTransferView-associatedItem-list-listItem-'+associatedItem.id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      lidsToRemove.current = [];
                                      handleReassociateItem('metodo', associatedItem.id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.id];
                                      handleDisassociateItem('metodo', associatedItem.id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MetodoToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
MetodoToAddTransferView.propTypes = {
