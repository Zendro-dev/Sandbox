...
import CaracteristicaCuantitativaTabsA from './components/Caracteristica_cuantitativaTabsA';
...
export default function CaracteristicaCuantitativaUpdatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.unidad = item.unidad;
  ...
  function getInitialValueAjvStates() {
    ...
    _initialValueAjvStates.unidad = {errors: []};
  ...
  function areThereChangedFields() {
    ...
    if(values.current.unidad !== item.unidad) { return true;}
  ...
  function setAddRemoveOneMetodo(variables) {
    ...
    if(metodoIdsToAdd.current.length>0) {
      ...
      changedAssociations.current.caracteristica_cuantitativa_metodo_id.idsAdded = [metodoIdsToAdd.current[0]];
    }
    /**
...
function setAjvErrors(err) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.caracteristica_cuantitativa.updateItem
    */
    let api = await loadApi("caracteristica_cuantitativa");
    let cancelableApiReq = makeCancelable(api.caracteristica_cuantitativa.updateItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'caracteristica_cuantitativa', method: 'doSave()', request: 'api.caracteristica_cuantitativa.updateItem'}];
            newError.path=['Caracteristica_cuantitativas', `id:${item.id}`, 'update'];
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cuantitativa.updateItem
        if(err.isCanceled) {
          return;
        } else {
          //set ajv errors
  ...
  return (
    <Dialog id='CaracteristicaCuantitativaUpdatePanel-dialog' 
    ...
    >
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='CaracteristicaCuantitativaUpdatePanel-button-cancel'
              edge="start" 
              color="inherit" 
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": Caracteristica_cuantitativa | id: " + item.id}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " caracteristica_cuantitativa" }>
              <Fab
                id='CaracteristicaCuantitativaUpdatePanel-fabButton-save' 
                color="secondary" 
      ...
      <div className={classes.root}>
        <Grid container justify='center' alignItems='flex-start' alignContent='flex-start'>
          ...
          {/* TabsA: Menú */}
          <Grid item xs={12}>
            <CaracteristicaCuantitativaTabsA
              value={tabsValue}
        ...
        {/* Confirmation Dialog */}
        <Suspense fallback={<div />}>
          <CaracteristicaCuantitativaConfirmationDialog
...
CaracteristicaCuantitativaUpdatePanel.propTypes = {
