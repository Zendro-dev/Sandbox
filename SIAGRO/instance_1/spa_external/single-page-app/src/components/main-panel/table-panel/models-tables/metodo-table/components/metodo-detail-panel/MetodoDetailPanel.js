...
//lazy loading
const MetodoAttributesPage = lazy(() => import(/* webpackChunkName: "Detail-Attributes-Metodo" */ './components/metodo-attributes-page/MetodoAttributesPage'));
...
export default function MetodoDetailPanel(props) {
      ...
      lastModelChanged['metodo'][String(item['id'])]) {
        ...
        if(lastModelChanged['metodo'][String(item['id'])].op === "update"&&
            lastModelChanged['metodo'][String(item['id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['metodo'][String(item['id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('metodo', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('metodo', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='MetodoDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Metodo | id: " + itemState['id'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='MetodoDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='MetodoDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
