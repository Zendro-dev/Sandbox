...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ejemplar.getCaracteristicas_cuantitativasCount
    */
    let api = await loadApi("ejemplar");
    let cancelableApiReq = makeCancelable(api.ejemplar.getCaracteristicas_cuantitativasCount(
      graphqlServerUrl, 
      item.id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'caracteristicas_cuantitativas', method: 'getCount()', request: 'api.ejemplar.getCaracteristicas_cuantitativasCount'}];
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getCaracteristicas_cuantitativasCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ejemplar.getCaracteristicas_cuantitativas
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("ejemplar");
    let cancelableApiReq = makeCancelable(api.ejemplar.getCaracteristicas_cuantitativas(
      graphqlServerUrl, 
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'caracteristicas_cuantitativas', method: 'getData()', request: 'api.ejemplar.getCaracteristicas_cuantitativas'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'caracteristicas_cuantitativas'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getCaracteristicas_cuantitativas
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
