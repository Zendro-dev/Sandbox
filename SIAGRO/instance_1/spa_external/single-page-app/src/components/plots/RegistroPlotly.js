...
export default function RegistroPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.registro.getBarchartData
    */
    let api = await loadApi("registro");
    await api.registro.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'registro', method: 'getBarchartData()', request: 'api.registro.getBarchartData'}];
            newError.path=['Registros', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.registro.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
