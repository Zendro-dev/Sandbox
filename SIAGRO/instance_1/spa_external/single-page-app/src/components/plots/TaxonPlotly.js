...
export default function TaxonPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.taxon.getBarchartData
    */
    let api = await loadApi("taxon");
    await api.taxon.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Taxon', method: 'getBarchartData()', request: 'api.taxon.getBarchartData'}];
      ...
      //error
      .catch((err) => { //error: on api.taxon.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
