...
export default function TaxonPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.taxon.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.taxonsConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.taxonsConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'taxon', query: 'taxonsConnection', method: 'getItemsAttribute()', request: 'api.taxon.getItemsAttribute'}];
