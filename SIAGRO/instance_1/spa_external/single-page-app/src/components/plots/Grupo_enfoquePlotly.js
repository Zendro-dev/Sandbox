...
export default function GrupoEnfoquePlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.grupo_enfoque.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.grupo_enfoquesConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.grupo_enfoquesConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'grupo_enfoque', query: 'grupo_enfoquesConnection', method: 'getItemsAttribute()', request: 'api.grupo_enfoque.getItemsAttribute'}];
    ...
    .catch((error) => {
      ...
      newError.locations=[{model: 'grupo_enfoque', query: 'grupo_enfoquesConnection', method: 'getItemsAttribute()', request: 'api.grupo_enfoque.getItemsAttribute'}];
      newError.path=['Grupo_enfoques', 'Plot'];
