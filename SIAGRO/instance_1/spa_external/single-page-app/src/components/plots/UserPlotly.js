...
export default function UserPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.user.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.usersConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.usersConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'user', query: 'usersConnection', method: 'getItemsAttribute()', request: 'api.user.getItemsAttribute'}];
