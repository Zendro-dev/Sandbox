...
export default function EjemplarPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.ejemplar.getBarchartData
    */
    let api = await loadApi("ejemplar");
    await api.ejemplar.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Ejemplar', method: 'getBarchartData()', request: 'api.ejemplar.getBarchartData'}];
            newError.path=['Ejemplars', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.ejemplar.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
