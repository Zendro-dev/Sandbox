...
export default function MetodoPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.metodo.getBarchartData
    */
    let api = await loadApi("metodo");
    await api.metodo.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'metodo', method: 'getBarchartData()', request: 'api.metodo.getBarchartData'}];
            newError.path=['Metodos', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.metodo.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
