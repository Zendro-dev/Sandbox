...
export default function CaracteristicaCuantitativaPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.caracteristica_cuantitativa.getBarchartData
    */
    let api = await loadApi("caracteristica_cuantitativa");
    await api.caracteristica_cuantitativa.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'caracteristica_cuantitativa', method: 'getBarchartData()', request: 'api.caracteristica_cuantitativa.getBarchartData'}];
            newError.path=['Caracteristica_cuantitativas', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.caracteristica_cuantitativa.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
