...
export default function ReferenciaPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.referencia.getBarchartData
    */
    let api = await loadApi("referencia");
    await api.referencia.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'referencia', method: 'getBarchartData()', request: 'api.referencia.getBarchartData'}];
            newError.path=['Referencia', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.referencia.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
