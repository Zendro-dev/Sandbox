 ...
 */
   ...
   * tableTemplate
   *
   * Get referenciatable template from GraphQL Server.
   * (root query)
   *
  ...
  async tableTemplate(url) {
    let graphqlErrors = [];
    let query = `query {csvTableTemplateReferencia}`;
    ...
    let headers = null;
    //check
    let check = checkResponse(response, graphqlErrors, "csvTableTemplateReferencia");
    if(check === 'ok') {
      //check type
      if(!Array.isArray(response.data.data["csvTableTemplateReferencia"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      headers = response.data.data["csvTableTemplateReferencia"];
 ...
 */
   ...
   * getCountItems
   *
   * Get referenciaitems count from GraphQL Server.
   * (root query)
   *
   * @param {String} url GraphQL Server url
...
`;
    ...
    //query
    let query =
      `query referenciaConnection($order: [orderReferenciaInput], $search: searchReferenciaInput, $pagination: paginationCursorInput!) { 
             referenciaConnection( order: $order, search: $search, pagination: $pagination ) {
 ...
 */
   ...
   * createItem
   *
   * Add new Referencia item on GraphQL Server.
   * (root mutation)
   *
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Referencia item.
   */
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "addReferencia");
    if(check === 'ok') {
        //check type
        if(!response.data.data["addReferencia"]
        || typeof response.data.data["addReferencia"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["addReferencia"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * updateItem
   *
   * Update Referencia item on GraphQL Server.
   * (root mutation)
   *
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Referencia item.
   */
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "updateReferencia");
    if(check === 'ok') {
        //check type
        if(!response.data.data["updateReferencia"]
        || typeof response.data.data["updateReferencia"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["updateReferencia"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   *
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Referencia item.
   */
  async deleteItem(url, variables) {
    ...
    let result = null;
    //check
    let check = checkResponse(response, graphqlErrors, "deleteReferencia");
    if(check === 'ok') {
      //check type
      if(response.data.data["deleteReferencia"] === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      result = response.data.data["deleteReferencia"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getAlimentos   *
   * Get registros records associated to the given referencia record
...
`;
    ...
    variables["referencia_id"] = itemId;
    //set search
    ...
    let query =
      `query readOneReferencia($referencia_id:ID!, $search: searchRegistroInput, $pagination: paginationCursorInput!) {
             readOneReferencia(referencia_id:$referencia_id) {
...
//request
    ...
    let items = null;
    //check
    let check = checkResponse(response, graphqlErrors, "readOneReferencia");
    if(check === 'ok') {
      //check type
      if(!response.data.data["readOneReferencia"]
      || typeof response.data.data["readOneReferencia"] !== 'object'
      || !response.data.data["readOneReferencia"]["alimentosConnection"]
      || typeof response.data.data["readOneReferencia"]["alimentosConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneReferencia"]["alimentosConnection"].edges)
      || typeof response.data.data["readOneReferencia"]["alimentosConnection"].pageInfo !== 'object' 
      || response.data.data["readOneReferencia"]["alimentosConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneReferencia"]["alimentosConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * getAlimentosCount
   * 
   * Get registros records count associated to the given referencia record
   * through association 'Alimentos', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  async getAlimentosCount(url, itemId, searchText, ops) {
    ...
    let variables = {"referencia_id": itemId};
    //search
    let s = getSearchArgument('registro', searchText, ops, 'object');
    if(s) variables.search = s.search;
    //query
    let query =
      `query readOneReferencia($referencia_id:ID!, $search: searchRegistroInput) { 
             readOneReferencia(referencia_id:$referencia_id) {
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneReferencia"]
      || typeof response.data.data["readOneReferencia"] !== 'object'
      || !Number.isInteger(response.data.data["readOneReferencia"]["countFilteredAlimentos"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneReferencia"]["countFilteredAlimentos"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  let f1 = {field: "referencias_ids", valueType: "Array", value: itemId, operator: "notIn"};
  let f2 = {field: "referencias_ids", valueType: "String", value: null, operator: "eq"};
 ...
 */
  ...
  async getBarchartData(url, attribute) {
    ...
    //query
    let batchQuery = 
          `query referenciaConnection($pagination: paginationCursorInput!) {
                 referenciaConnection(pagination: $pagination) {
                    pageInfo {startCursor endCursor hasPreviousPage hasNextPage}
