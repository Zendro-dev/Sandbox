...
export default function CuadranteCreatePanel(props) {
  ...
  const [taxonomic_informationIdsToAddState, setTaxonomic_informationIdsToAddState] = useState([]);
  const taxonomic_informationIdsToAdd = useRef([]);
  ...
  function setAddTaxonomic_information(variables) {
  ...
  function doSave(event) {
    ...
    */
    ...
    setAddTaxonomic_information(variables);
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      ...
      case 'taxonomic_information':
        if(taxonomic_informationIdsToAdd.current.indexOf(itemId) === -1) {
