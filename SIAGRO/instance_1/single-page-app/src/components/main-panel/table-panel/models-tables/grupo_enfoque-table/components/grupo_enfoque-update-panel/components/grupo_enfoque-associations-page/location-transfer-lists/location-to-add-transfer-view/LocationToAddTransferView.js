...
export default function LocationToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.grupo_enfoque.getAssociatedLocationConnection(graphqlServerUrl, item.grupo_id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'grupo_enfoque', association: 'location', table:'A', query: 'readOneGrupo_enfoque', method: 'getData()', request: 'api.grupo_enfoque.getAssociatedLocationConnection'}];
        ...
        || typeof readOneGrupo_enfoque.location!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneGrupo_enfoque.location;
        ...
        //ok
        //set associated id
        lidsAssociated.current = (idso !== null) ? [idso.location_id] : [];
      ...
      //error
      .catch((err) => { //error: on api.grupo_enfoque.getAssociatedLocationConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.grupo_enfoque&&
        ...
        lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.location&&
        (lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.location.added ||
         lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.location.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.location.idsRemoved;
          if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.grupo_enfoque.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.location_id;
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem.location_id===it.location_id) ? 'location — ' + t('modelPanels.associatedRecord', 'Associated record') : 'location '}>
                                <Avatar>
                                  {"location".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    ...
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem.location_id===it.location_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem.location_id===it.location_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
