...
import TaxonomicInformationCompactViewToolbar from './components/Taxonomic_informationCompactViewToolbar';
import TaxonomicInformationCompactViewCursorPagination from './components/Taxonomic_informationCompactViewCursorPagination';
...
export default function TaxonomicInformationCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.cuadrante.getTaxonomic_informationConnection(
      graphqlServerUrl, 
      item.cuadrante_id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'taxonomic_information', query: 'readOneCuadrante', method: 'getData()', request: 'api.cuadrante.getTaxonomic_informationConnection'}];
          newError.path=['detail', `cuadrante_id:${item.cuadrante_id}`, 'taxonomic_information'];
        ...
        //get item
        let it = readOneCuadrante.taxonomic_information;
      ...
      //error
      .catch((err) => { //error: on api.cuadrante.getTaxonomic_informationConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
        ...
        <Grid item xs={12} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <TaxonomicInformationCompactViewToolbar 
                title={'Taxon'}
                search={search}
              ...
              {(false) && (
                ...
                <TaxonomicInformationCompactViewCursorPagination
                  count={count}
...
TaxonomicInformationCompactView.propTypes = {
  item: PropTypes.object.isRequired,
  handleClickOnTaxonomic_informationRow: PropTypes.func,
};