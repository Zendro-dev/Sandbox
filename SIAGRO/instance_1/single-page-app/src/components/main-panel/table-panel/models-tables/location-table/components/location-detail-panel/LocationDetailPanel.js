...
export default function LocationDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.location[String(itemState.location_id)]) {
          
        //updated item
        if(lastModelChanged.location[String(itemState.location_id)].op === "update"&&
            lastModelChanged.location[String(itemState.location_id)].newItem) {
              //replace item
              setItemState(lastModelChanged.location[String(itemState.location_id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.location[String(itemState.location_id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.location_id]);
  ...
  function doDelete(event, item) {
    ...
    let variables = {};
    //location_id
    variables.location_id = item.location_id;
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.path=['Locations', `location_id:${item.location_id}`, 'delete'];
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Location | location_id: " + itemState.location_id}
              </Typography>
              {/*
                Actions:
                - Edit
