...
import GrupoEnfoqueToRemoveTransferViewToolbar from './components/Grupo_enfoqueToRemoveTransferViewToolbar';
import GrupoEnfoqueToRemoveTransferViewCursorPagination from './components/Grupo_enfoqueToRemoveTransferViewCursorPagination';
...
export default function GrupoEnfoqueToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {grupo_id: lidsToRemove.current}
    //     }]
    //   };
    // }
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.location.getGrupo_enfoqueConnection(
      graphqlServerUrl, 
      item.location_id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'grupo_enfoque', query: 'readOneLocation', method: 'getData()', request: 'api.location.getGrupo_enfoqueConnection'}];
          newError.path=['remove', 'grupo_enfoque'];
        ...
        || !Number.isInteger(readOneLocation.countFilteredGrupo_enfoque)
        ...
        || typeof readOneLocation.grupo_enfoqueConnection.pageInfo !== 'object'
        || readOneLocation.grupo_enfoqueConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneLocation.countFilteredGrupo_enfoque;
        let its = readOneLocation.grupo_enfoqueConnection.edges.map(o => o.node);
        let pi = readOneLocation.grupo_enfoqueConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.location.getGrupo_enfoqueConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'grupo_enfoque' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.location.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.grupo_id;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.grupo_id) === -1) {
      lidsToRemove.current.push(item.grupo_id);
      setThereAreItemsToAdd(true);
      updateHeights();
      //reload A
      reloadDataA();
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.grupo_id);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <GrupoEnfoqueToRemoveTransferViewToolbar
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        let disabled = lidsToRemove.current.find(grupo_id=> grupo_id=== it.grupo_id) !== undefined;
              ...
              {/* Pagination */}
              <GrupoEnfoqueToRemoveTransferViewCursorPagination
                count={count}
...
GrupoEnfoqueToRemoveTransferView.propTypes = {
