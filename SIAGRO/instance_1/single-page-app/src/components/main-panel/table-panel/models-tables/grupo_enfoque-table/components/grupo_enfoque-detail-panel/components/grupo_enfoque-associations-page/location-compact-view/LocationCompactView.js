...
export default function LocationCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.grupo_enfoque.getLocationConnection(
      graphqlServerUrl, 
      item.grupo_id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'location', query: 'readOneGrupo_enfoque', method: 'getData()', request: 'api.grupo_enfoque.getLocationConnection'}];
          newError.path=['detail', `grupo_id:${item.grupo_id}`, 'location'];
        ...
        //get item
        let it = readOneGrupo_enfoque.location;
      ...
      //error
      .catch((err) => { //error: on api.grupo_enfoque.getLocationConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
