...
import GrupoEnfoqueUpdatePanel from '../grupo_enfoque-update-panel/Grupo_enfoqueUpdatePanel'
import GrupoEnfoqueDeleteConfirmationDialog from '../Grupo_enfoqueDeleteConfirmationDialog'
...
export default function GrupoEnfoqueDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.grupo_enfoque[String(itemState.grupo_id)]) {
          
        //updated item
        if(lastModelChanged.grupo_enfoque[String(itemState.grupo_id)].op === "update"&&
            lastModelChanged.grupo_enfoque[String(itemState.grupo_id)].newItem) {
              //replace item
              setItemState(lastModelChanged.grupo_enfoque[String(itemState.grupo_id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.grupo_enfoque[String(itemState.grupo_id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.grupo_id]);
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteGrupo_enfoque
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('grupo_enfoque', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('grupo_enfoque', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Grupo_enfoque | grupo_id: " + itemState.grupo_id}
              </Typography>
              {/*
                Actions:
                - Edit
...
GrupoEnfoqueDetailPanel.propTypes = {
