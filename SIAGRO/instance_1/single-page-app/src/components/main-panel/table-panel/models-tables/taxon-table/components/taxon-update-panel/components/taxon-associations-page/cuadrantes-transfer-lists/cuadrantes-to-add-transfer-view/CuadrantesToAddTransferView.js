...
export default function CuadrantesToAddTransferView(props) {
  ...
  */
  ...
  useEffect(() => {
    ...
    /*
     * Case 2: 
     * The peer relation 'taxonomic_information' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'cuadrante',
     * but this action happened on the peer relation 'taxonomic_information'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.cuadrante) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.taxonomic_information) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.taxonomic_information.removed) {
              let idsRemoved = entry[1].changedAssociations.taxonomic_information.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.taxonomic_information.added) {
              let idsAdded = entry[1].changedAssociations.taxonomic_information.idsAdded;
              if(idsAdded) {
