...
export default function GrupoEnfoqueToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    /*
      API Request: readOneLocation
    */
    let cancelableApiReq = makeCancelable(api.location.getAssociatedGrupo_enfoqueConnection(graphqlServerUrl, item.location_id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'A', query: 'readOneLocation', method: 'getData()', request: 'api.location.getAssociatedGrupo_enfoqueConnection'}];
        ...
        //check: readOneLocation
        let readOneLocation = response.data.data.readOneLocation;
        if(readOneLocation === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneLocation ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'A', query: 'readOneLocation', method: 'getData()', request: 'api.location.getAssociatedGrupo_enfoqueConnection'}];
          newError.path=['add', 'grupo_enfoque'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: readOneLocation type
        if(typeof readOneLocation !== 'object'
        || typeof readOneLocation.grupo_enfoqueConnection !== 'object'
        || readOneLocation.grupo_enfoqueConnection === null
        || !Array.isArray(readOneLocation.grupo_enfoqueConnection.edges)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneLocation ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'A', query: 'readOneLocation', method: 'getData()', request: 'api.location.getAssociatedGrupo_enfoqueConnection'}];
          newError.path=['add', 'grupo_enfoque'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //get items
        let idso = readOneLocation.grupo_enfoqueConnection.edges.map(o => o.node);

        //check: graphql errors
        if(response.data.errors) {
          let newError = {};
          newError.message = 'readOneLocation ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'A', query: 'readOneLocation', method: 'getData()', request: 'api.location.getAssociatedGrupo_enfoqueConnection'}];
          newError.path=['add', 'grupo_enfoque'];
        ...
        //set associated ids
        lidsAssociated.current = idso.map(function(item){ return item.grupo_id});
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'A', query: 'countGrupo_enfoques', method: 'getData()', request: 'api.grupo_enfoque.getCountItems'}];
              newError.path=['add', 'grupo_enfoque'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'A', query: 'grupo_enfoquesConnection', method: 'getData()', request: 'api.grupo_enfoque.getItemsConnection'}];
                  newError.path=['add', 'grupo_enfoque'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(grupo_enfoquesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'A', method: 'getData()'}];
                    newError.path=['add', 'grupo_enfoque'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.location.getAssociatedGrupo_enfoqueConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, t, dataTrigger, item.location_id, search, rowsPerPage]);
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'B', query: 'countGrupo_enfoques', method: 'getDataB()', request: 'api.grupo_enfoque.getCountItems'}];
          newError.path=['add', 'grupo_enfoque'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'B', query: 'grupo_enfoquesConnection', method: 'getDataB()', request: 'api.grupo_enfoque.getItemsConnection'}];
                newError.path=['add', 'grupo_enfoque'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'location', association: 'grupo_enfoque', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'grupo_enfoque'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'grupo_enfoques' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.location&&
        ...
        lastModelChanged.location[String(item.location_id)].changedAssociations&&
        lastModelChanged.location[String(item.location_id)].changedAssociations.grupo_enfoque&&
        (lastModelChanged.location[String(item.location_id)].changedAssociations.grupo_enfoque.added ||
         lastModelChanged.location[String(item.location_id)].changedAssociations.grupo_enfoque.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.location[String(item.location_id)].changedAssociations.grupo_enfoque.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'location' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'grupo_enfoque',
     * but this action happened on the peer relation 'location'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.grupo_enfoque) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.location) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.location.removed) {
              let idsRemoved = entry[1].changedAssociations.location.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.location_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.location.added) {
              let idsAdded = entry[1].changedAssociations.location.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.location_id);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.location_id, handleUntransfer]);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.grupo_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'grupo_id' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.grupo_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
