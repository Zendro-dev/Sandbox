...
export default {
   ...
   * 
   * Get grupo_enfoques connection (cursor based) records associated to the given location record
  ...
  getGrupo_enfoqueConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    ...
    var query = (s) ?
      `query readOneLocation($pagination: paginationCursorInput) {
        readOneLocation(location_id: "${itemId}") { 
   ...
   * 
   * Get the grupo_enfoque-ids associated (by cursor based connection) to the given location record 
   * through association 'Grupo_enfoque', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
