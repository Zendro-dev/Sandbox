...
export default {
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query rolesConnection($pagination: paginationCursorInput)
            { rolesConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query rolesConnection($pagination: paginationCursorInput)
            { rolesConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query rolesConnection($pagination: paginationCursorInput)
          { rolesConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * getUsersConnection
   * 
   * Get users connection (cursor based) records associated to the given role record
   ...
   * @param {String} ops Object with adittional query options.
   */
  getUsersConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneRole($pagination: paginationCursorInput) {
        readOneRole(id: ${itemId}) { 
          usersConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredUsers( ${s} ) 
      } }` :      
      `query readOneRole($pagination: paginationCursorInput) {
        readOneRole(id: ${itemId}) { 
   ...
   * 
   *  
   * getAssociatedUsersConnection
   * 
   * Get the user-ids associated (by cursor based connection) to the given role record 
   * through association 'Users', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedUsersConnection(url, itemId) {
    var query = 
      `{ readOneRole( id: ${itemId} ){ 
        usersConnection{ edges { node { id } } } } }`;
    /**
     * Debug
     */
