...
export default function CuadrantePlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.cuadrante.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.cuadrantesConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.cuadrantesConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'cuadrante', query: 'cuadrantesConnection', method: 'getItemsAttribute()', request: 'api.cuadrante.getItemsAttribute'}];
