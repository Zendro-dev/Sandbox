...
export default function SitioPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.sitio.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.sitiosConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.sitiosConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'sitio', query: 'sitiosConnection', method: 'getItemsAttribute()', request: 'api.sitio.getItemsAttribute'}];
