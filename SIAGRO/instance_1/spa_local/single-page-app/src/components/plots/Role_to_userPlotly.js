...
export default function RoleToUserPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.role_to_user.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.role_to_usersConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.role_to_usersConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'role_to_user', query: 'role_to_usersConnection', method: 'getItemsAttribute()', request: 'api.role_to_user.getItemsAttribute'}];
    ...
    .catch((error) => {
      ...
      newError.locations=[{model: 'role_to_user', query: 'role_to_usersConnection', method: 'getItemsAttribute()', request: 'api.role_to_user.getItemsAttribute'}];
      newError.path=['Role_to_users', 'Plot'];
