...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['tipo_planta'][String(itemId['tipo_planta_id'])]) {

        //updated password
        if(lastModelChanged['tipo_planta'][String(itemId['tipo_planta_id'])].op === "updatePassword"&&
            lastModelChanged['tipo_planta'][String(itemId['tipo_planta_id'])].newItem) {
              //update password value
              setValue(lastModelChanged['tipo_planta'][String(itemId['tipo_planta_id'])].newItem[itemKey]);
        }
    }//end: Case 1
  ...
  /**
    ...
    */
    return await api.tipo_planta.updateItem(graphqlServerUrl, variables)
      .then(
      //resolved
      (response) => {
        //check: response data
        if(!response.data ||!response.data.data) {
          ...
          newError.path=['Tipo_planta', itemId, 'update', 'change password'];
  ...
  return (
    <div>
      <Grid container justify='flex-start' alignItems='center' spacing={0}>
        <Grid item xs={12}>
          <form noValidate autoComplete="off">
            <TextField
              id={'PasswordField-TipoPlanta-'+name}
              label={label}
              value={value}
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'PasswordField-TipoPlanta-button-showPassword'+name}
                    ...
                    {(valueOk!==undefined&&valueOk===1) 
                      ...
                      <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                        <Typography id={'PasswordField-TipoPlanta-exists-'+name} variant="caption" color="primary">
                          &#8707;
                        </Typography>
                      </Tooltip>
                      : 
                      <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                        <Typography id={'PasswordField-TipoPlanta-notExists-'+name} variant="caption" color="textSecondary">
                          &#8708;
                        </Typography>
                      </Tooltip>
                    }
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'PasswordField-TipoPlanta-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-TipoPlanta-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-TipoPlanta-ChangePasswordDialog-textField-newPassword2-'+name}
