...
import SitioToAddTransferViewToolbar from './components/SitioToAddTransferViewToolbar';
import SitioToAddTransferViewCursorPagination from './components/SitioToAddTransferViewCursorPagination';
...
export default function SitioToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.grupo_enfoque.getAssociatedSitioConnection(graphqlServerUrl, item.grupo_id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'grupo_enfoque', association: 'sitio', table:'A', query: 'readOneGrupo_enfoque', method: 'getData()', request: 'api.grupo_enfoque.getAssociatedSitioConnection'}];
          newError.path=['add', 'sitio'];
        ...
        //get item
        let idso = readOneGrupo_enfoque.sitio;
        ...
        //ok
        //set associated id
        lidsAssociated.current = (idso !== null) ? [idso.sitio_id] : [];
        ...
        */
        let cancelableApiReqB = makeCancelable(api.sitio.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'grupo_enfoque', association: 'sitio', table:'A', query: 'countSitios', method: 'getData()', request: 'api.sitio.getCountItems'}];
              newError.path=['add', 'sitio'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.sitio.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'grupo_enfoque', association: 'sitio', table:'A', query: 'sitiosConnection', method: 'getData()', request: 'api.sitio.getItemsConnection'}];
                  newError.path=['add', 'sitio'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(sitiosConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'grupo_enfoque', association: 'sitio', table:'A', method: 'getData()'}];
                    newError.path=['add', 'sitio'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.grupo_enfoque.getAssociatedSitioConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    if(lidsToAdd.current !== undefined && lidsToAdd.current.length > 0) {
      ops = {
        only: [{
          type: 'String',
          values: {sitio_id: lidsToAdd.current}
        }]
      };
    } else {
    ...
    */
    let cancelableApiReq = makeCancelable(api.sitio.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'grupo_enfoque', association: 'sitio', table:'B', query: 'countSitios', method: 'getDataB()', request: 'api.sitio.getCountItems'}];
          newError.path=['add', 'sitio'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'grupo_enfoque', association: 'sitio', table:'B', query: 'sitiosConnection', method: 'getDataB()', request: 'api.sitio.getItemsConnection'}];
                newError.path=['add', 'sitio'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'grupo_enfoque', association: 'sitio', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'sitio'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'sitio' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'sitios', 
    ...
    if(lastModelChanged.grupo_enfoque&&
        ...
        lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.grupo_enfoque_sitio_id&&
        (lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.grupo_enfoque_sitio_id.added ||
         lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.grupo_enfoque_sitio_id.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.grupo_enfoque_sitio_id.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('sitio', idRemoved);
            });
          }
     ...
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'sitio'.
     * B: the current item internalId is in the addedIds of the updated 'sitio'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'sitio' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
            ...
            if(entry[1].changedAssociations.grupo_enfoque_sitio_id.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.sitio_id;
                  ...
                  handleUntransfer('sitio', idAdded);
    ...
    if(lastModelChanged.sitio) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          ...
          let iofB = nitemsB.findIndex((item) => item.sitio_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          ...
          let iofB = itemsB.findIndex((item) => item.sitio_id===idRemoved);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.sitio_id) === -1) {
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.sitio_id);
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('sitio', item.sitio_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('sitio', item.sitio_id);
    }
  };
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady || !isCountReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SitioToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady && isCountReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='SitioToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'SitioToAddTransferView-listA-listItem-'+it.sitio_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.sitio_id===it.sitio_id) ? 'sitio — ' + t('modelPanels.associatedRecord', 'Associated record') : 'sitio '}>
                                <Avatar>
                                  {"sitio".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'sitio_id' }>
                                        <Typography 
                                        id={'SitioToAddTransferView-listA-listItem-id-'+it.sitio_id}
                                        variant="body1" display="block" noWrap={true}>{it.sitio_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.sitio_id===it.sitio_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.sitio_id===it.sitio_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'SitioToAddTransferView-listA-listItem-'+it.sitio_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <SitioToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SitioToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB || !isCountReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SitioToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB && isCountReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='SitioToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.sitio_id;
                        let label = it.estado;
                        ...
                        return (
                          <ListItem 
                            id={'SitioToAddTransferView-listB-listItem-'+it.sitio_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'sitio_id' }>
                                        <Typography 
                                        id={'SitioToAddTransferView-listB-listItem-id-'+it.sitio_id}
                                        variant="body1" display="block" noWrap={true}>{it.sitio_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'SitioToAddTransferView-listB-listItem-'+it.sitio_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
SitioToAddTransferView.propTypes = {
