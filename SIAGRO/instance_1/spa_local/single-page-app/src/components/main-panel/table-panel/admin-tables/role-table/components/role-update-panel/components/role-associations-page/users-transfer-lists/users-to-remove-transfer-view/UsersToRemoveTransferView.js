...
import UsersToRemoveTransferViewToolbar from './components/UsersToRemoveTransferViewToolbar';
import UsersToRemoveTransferViewCursorPagination from './components/UsersToRemoveTransferViewCursorPagination';
...
export default function UsersToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.role.getUsersConnection(
      graphqlServerUrl, 
      item.id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'users', query: 'readOneRole', method: 'getData()', request: 'api.role.getUsersConnection'}];
          newError.path=['remove', 'users'];
        ...
        //check: readOneRole
        let readOneRole = response.data.data.readOneRole;
        if(readOneRole === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneRole ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{association: 'users', query: 'readOneRole', method: 'getData()', request: 'api.role.getUsersConnection'}];
          newError.path=['remove', 'users'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: readOneRole type
        if(typeof readOneRole !== 'object'
        || !Number.isInteger(readOneRole.countFilteredUsers)
        || typeof readOneRole.usersConnection !== 'object'
        || readOneRole.usersConnection === null
        || !Array.isArray(readOneRole.usersConnection.edges)
        || typeof readOneRole.usersConnection.pageInfo !== 'object'
        || readOneRole.usersConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneRole ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{association: 'users', query: 'readOneRole', method: 'getData()', request: 'api.role.getUsersConnection'}];
          newError.path=['remove', 'users'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //get items
        let newCount = readOneRole.countFilteredUsers;
        let its = readOneRole.usersConnection.edges.map(o => o.node);
        let pi = readOneRole.usersConnection.pageInfo;
        ...
        if(response.data.errors) {
          ...
          newError.message = 'readOneRole ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{association: 'users', query: 'readOneRole', method: 'getData()', request: 'api.role.getUsersConnection'}];
          newError.path=['remove', 'users'];
      ...
      //error
      .catch((err) => { //error: on api.role.getUsersConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'users' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'users' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.role&&
        ...
        lastModelChanged.role[String(item.id)].changedAssociations&&
        lastModelChanged.role[String(item.id)].changedAssociations.role_to_user&&
        (lastModelChanged.role[String(item.id)].changedAssociations.role_to_user.added ||
         lastModelChanged.role[String(item.id)].changedAssociations.role_to_user.removed)) {
          
          //remove any dis-associated id from idsToRemove[]
          let idsRemoved = lastModelChanged.role[String(item.id)].changedAssociations.role_to_user.idsRemoved;
          if(idsRemoved) {
     ...
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'user'.
     * B: the current item internalId is in the addedIds of the updated 'user'.
     * 
     * Actions:
     * if A:
     * - remove the internalId of the 'user' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <UsersToRemoveTransferViewToolbar
                title={'Users'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady || !isCountReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UsersToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady && isCountReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='UsersToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'UsersToRemoveTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'UsersToRemoveTransferView-listA-listItem-'+it.id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <UsersToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UsersToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB || !isCountReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UsersToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB && isCountReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='UsersToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = it.email;
                        ...
                        return (
                          <ListItem 
                            id={'UsersToRemoveTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'UsersToRemoveTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
UsersToRemoveTransferView.propTypes = {
