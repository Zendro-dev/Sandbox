...
import TaxonUpdatePanel from '../taxon-update-panel/TaxonUpdatePanel'
import TaxonDeleteConfirmationDialog from '../TaxonDeleteConfirmationDialog'
...
export default function TaxonDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.taxon[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.taxon[String(itemState.id)].op === "update"&&
            lastModelChanged.taxon[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.taxon[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.taxon[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteTaxon
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
