  ...
  */
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.taxon&&
        ...
        lastModelChanged.taxon[String(item.id)].changedAssociations.cuadrantes&&
        (lastModelChanged.taxon[String(item.id)].changedAssociations.cuadrantes.added ||
         lastModelChanged.taxon[String(item.id)].changedAssociations.cuadrantes.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.taxon[String(item.id)].changedAssociations.cuadrantes.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'informacion_taxonomica' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'cuadrante',
     * but this action happened on the peer relation 'informacion_taxonomica'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.cuadrante) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.informacion_taxonomica) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.informacion_taxonomica.removed) {
              let idsRemoved = entry[1].changedAssociations.informacion_taxonomica.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.informacion_taxonomica.added) {
              let idsAdded = entry[1].changedAssociations.informacion_taxonomica.idsAdded;
              if(idsAdded) {
