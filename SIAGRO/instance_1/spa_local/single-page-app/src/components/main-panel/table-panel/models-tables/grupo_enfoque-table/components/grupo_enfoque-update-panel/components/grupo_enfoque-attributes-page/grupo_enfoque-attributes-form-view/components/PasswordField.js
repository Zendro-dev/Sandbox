...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['grupo_enfoque'][String(itemId['grupo_id'])]) {

        //updated password
        if(lastModelChanged['grupo_enfoque'][String(itemId['grupo_id'])].op === "updatePassword"&&
            lastModelChanged['grupo_enfoque'][String(itemId['grupo_id'])].newItem) {
              //update password value
              setValue(lastModelChanged['grupo_enfoque'][String(itemId['grupo_id'])].newItem[itemKey]);
        }
    }//end: Case 1
  ...
  /**
    ...
    */
    return await api.grupo_enfoque.updateItem(graphqlServerUrl, variables)
      .then(
      //resolved
      (response) => {
        //check: response data
        if(!response.data ||!response.data.data) {
          ...
          newError.path=['Grupo_enfoques', itemId, 'update', 'change password'];
  ...
  return (
    <div>
      <Grid container justify='flex-start' alignItems='center' spacing={0}>
        <Grid item xs={12}>
          <form noValidate autoComplete="off">
            <TextField
              id={'PasswordField-GrupoEnfoque-'+name}
              label={label}
              value={value}
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'PasswordField-GrupoEnfoque-button-showPassword'+name}
                    ...
                    {(valueOk!==undefined&&valueOk===1) 
                      ...
                      <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                        <Typography id={'PasswordField-GrupoEnfoque-exists-'+name} variant="caption" color="primary">
                          &#8707;
                        </Typography>
                      </Tooltip>
                      : 
                      <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                        <Typography id={'PasswordField-GrupoEnfoque-notExists-'+name} variant="caption" color="textSecondary">
                          &#8708;
                        </Typography>
                      </Tooltip>
                    }
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'PasswordField-GrupoEnfoque-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-GrupoEnfoque-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-GrupoEnfoque-ChangePasswordDialog-textField-newPassword2-'+name}
