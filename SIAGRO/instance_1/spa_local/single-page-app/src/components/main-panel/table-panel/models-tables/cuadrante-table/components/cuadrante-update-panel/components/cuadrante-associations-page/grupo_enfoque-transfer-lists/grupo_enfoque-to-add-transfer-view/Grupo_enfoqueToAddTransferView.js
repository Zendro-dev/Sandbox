...
import GrupoEnfoqueToAddTransferViewToolbar from './components/Grupo_enfoqueToAddTransferViewToolbar';
import GrupoEnfoqueToAddTransferViewCursorPagination from './components/Grupo_enfoqueToAddTransferViewCursorPagination';
...
export default function GrupoEnfoqueToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.cuadrante.getAssociatedGrupo_enfoqueConnection(graphqlServerUrl, item.cuadrante_id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'cuadrante', association: 'grupo_enfoque', table:'A', query: 'readOneCuadrante', method: 'getData()', request: 'api.cuadrante.getAssociatedGrupo_enfoqueConnection'}];
          newError.path=['add', 'grupo_enfoque'];
        ...
        || typeof readOneCuadrante.grupo_enfoque!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneCuadrante.grupo_enfoque;
        ...
        //ok
        //set associated id
        lidsAssociated.current = (idso !== null) ? [idso.grupo_id] : [];
        ...
        */
        let cancelableApiReqB = makeCancelable(api.grupo_enfoque.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'cuadrante', association: 'grupo_enfoque', table:'A', query: 'countGrupo_enfoques', method: 'getData()', request: 'api.grupo_enfoque.getCountItems'}];
              newError.path=['add', 'grupo_enfoque'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.grupo_enfoque.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'cuadrante', association: 'grupo_enfoque', table:'A', query: 'grupo_enfoquesConnection', method: 'getData()', request: 'api.grupo_enfoque.getItemsConnection'}];
                  newError.path=['add', 'grupo_enfoque'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(grupo_enfoquesConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'cuadrante', association: 'grupo_enfoque', table:'A', method: 'getData()'}];
                    newError.path=['add', 'grupo_enfoque'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.cuadrante.getAssociatedGrupo_enfoqueConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    if(lidsToAdd.current !== undefined && lidsToAdd.current.length > 0) {
      ops = {
        only: [{
          type: 'String',
          values: {grupo_id: lidsToAdd.current}
        }]
      };
    } else {
    ...
    */
    let cancelableApiReq = makeCancelable(api.grupo_enfoque.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'cuadrante', association: 'grupo_enfoque', table:'B', query: 'countGrupo_enfoques', method: 'getDataB()', request: 'api.grupo_enfoque.getCountItems'}];
          newError.path=['add', 'grupo_enfoque'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'cuadrante', association: 'grupo_enfoque', table:'B', query: 'grupo_enfoquesConnection', method: 'getDataB()', request: 'api.grupo_enfoque.getItemsConnection'}];
                newError.path=['add', 'grupo_enfoque'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'cuadrante', association: 'grupo_enfoque', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'grupo_enfoque'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'grupo_enfoque' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'grupo_enfoques', 
    ...
    if(lastModelChanged.cuadrante&&
        ...
        lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.cuadrante_grupo_enfoque_id&&
        (lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.cuadrante_grupo_enfoque_id.added ||
         lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.cuadrante_grupo_enfoque_id.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.cuadrante_grupo_enfoque_id.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('grupo_enfoque', idRemoved);
            });
          }
     ...
     * but this action happened on the peer relation, identified by 'cuadrante_grupo_enfoque_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'grupo_enfoque'.
     * B: the current item internalId is in the addedIds of the updated 'grupo_enfoque'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'grupo_enfoque' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.grupo_enfoque) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.cuadrante_grupo_enfoque_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.cuadrante_grupo_enfoque_id.removed) {
              let idsRemoved = entry[1].changedAssociations.cuadrante_grupo_enfoque_id.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.cuadrante_grupo_enfoque_id.added) {
              let idsAdded = entry[1].changedAssociations.cuadrante_grupo_enfoque_id.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.cuadrante_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.grupo_id;
                  ...
                  handleUntransfer('grupo_enfoque', idAdded);
    ...
    if(lastModelChanged.grupo_enfoque) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          ...
          let iofB = nitemsB.findIndex((item) => item.grupo_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          ...
          let iofB = itemsB.findIndex((item) => item.grupo_id===idRemoved);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.grupo_id) === -1) {
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.grupo_id);
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('grupo_enfoque', item.grupo_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('grupo_enfoque', item.grupo_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <GrupoEnfoqueToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady || !isCountReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GrupoEnfoqueToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady && isCountReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='GrupoEnfoqueToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'GrupoEnfoqueToAddTransferView-listA-listItem-'+it.grupo_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.grupo_id===it.grupo_id) ? 'grupo_enfoque — ' + t('modelPanels.associatedRecord', 'Associated record') : 'grupo_enfoque '}>
                                <Avatar>
                                  {"grupo_enfoque".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'grupo_id' }>
                                        <Typography 
                                        id={'GrupoEnfoqueToAddTransferView-listA-listItem-id-'+it.grupo_id}
                                        variant="body1" display="block" noWrap={true}>{it.grupo_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.grupo_id===it.grupo_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.grupo_id===it.grupo_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'GrupoEnfoqueToAddTransferView-listA-listItem-'+it.grupo_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <GrupoEnfoqueToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GrupoEnfoqueToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB || !isCountReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GrupoEnfoqueToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB && isCountReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='GrupoEnfoqueToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.grupo_id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'GrupoEnfoqueToAddTransferView-listB-listItem-'+it.grupo_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'grupo_id' }>
                                        <Typography 
                                        id={'GrupoEnfoqueToAddTransferView-listB-listItem-id-'+it.grupo_id}
                                        variant="body1" display="block" noWrap={true}>{it.grupo_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'GrupoEnfoqueToAddTransferView-listB-listItem-'+it.grupo_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
GrupoEnfoqueToAddTransferView.propTypes = {
