...
export default function InformacionTaxonomicaToAddTransferView(props) {
  ...
  */
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'taxons' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.cuadrante&&
        ...
        lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.informacion_taxonomica&&
        (lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.informacion_taxonomica.added ||
         lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.informacion_taxonomica.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.informacion_taxonomica.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'cuadrantes' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'taxon',
     * but this action happened on the peer relation 'cuadrantes'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.taxon) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.cuadrantes) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.cuadrantes.removed) {
              let idsRemoved = entry[1].changedAssociations.cuadrantes.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.cuadrantes.added) {
              let idsAdded = entry[1].changedAssociations.cuadrantes.idsAdded;
              if(idsAdded) {
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem.id===it.id) ? 'taxon — ' + t('modelPanels.associatedRecord', 'Associated record') : 'taxon '}>
                                <Avatar>
                                  {"taxon".slice(0,1)}
                                </Avatar>
                              </Tooltip>
