...
export default function InformacionTaxonomicaToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.cuadrante.getAssociatedInformacion_taxonomicaConnection(graphqlServerUrl, item.cuadrante_id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', query: 'readOneCuadrante', method: 'getData()', request: 'api.cuadrante.getAssociatedInformacion_taxonomicaConnection'}];
          newError.path=['add', 'informacion_taxonomica'];
        ...
        || typeof readOneCuadrante.informacion_taxonomica!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneCuadrante.informacion_taxonomica;
        ...
        /*
          API Request: countTaxons
        */
        let cancelableApiReqB = makeCancelable(api.taxon.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', query: 'countTaxons', method: 'getData()', request: 'api.taxon.getCountItems'}];
              newError.path=['add', 'informacion_taxonomica'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            //check: countTaxons
            let countTaxons = response.data.data.countTaxons;
            if(countTaxons === null) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countTaxons ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
              newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', query: 'countTaxons', method: 'getData()', request: 'api.taxon.getCountItems'}];
              newError.path=['add', 'informacion_taxonomica'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //check: countTaxons type
            if(!Number.isInteger(countTaxons)) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countTaxons ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
              newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', query: 'countTaxons', method: 'getData()', request: 'api.taxon.getCountItems'}];
              newError.path=['add', 'informacion_taxonomica'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            if(response.data.errors) {
              let newError = {};
              newError.message = 'countTaxons ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', query: 'countTaxons', method: 'getData()', request: 'api.taxon.getCountItems'}];
              newError.path=['add', 'informacion_taxonomica'];
              newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //ok
            setCount(countTaxons);


            /*
              API Request: taxonsConnection
            */
            let variables = {
              pagination: {
            ...
            let cancelableApiReqC = makeCancelable(api.taxon.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', query: 'taxonsConnection', method: 'getData()', request: 'api.taxon.getItemsConnection'}];
                  newError.path=['add', 'informacion_taxonomica'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                //check: taxonsConnection
                let taxonsConnection = response.data.data.taxonsConnection;
                if(taxonsConnection === null) {
                  let newError = {};
                  let withDetails=true;
                  variant.current='error';
                  newError.message = 'taxonsConnection ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
                  newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', query: 'taxonsConnection', method: 'getData()', request: 'api.taxon.getItemsConnection'}];
                  newError.path=['add', 'informacion_taxonomica'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  ...
                  //check: taxonsConnection type
                  if(typeof taxonsConnection !== 'object'
                  || !Array.isArray(taxonsConnection.edges)
                  || typeof taxonsConnection.pageInfo !== 'object' 
                  || taxonsConnection.pageInfo === null) {
                    let newError = {};
                    let withDetails=true;
                    variant.current='error';
                    newError.message = 'taxonsConnection ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
                    newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', query: 'taxonsConnection', method: 'getData()', request: 'api.taxon.getItemsConnection'}];
                    newError.path=['add', 'informacion_taxonomica'];
                    newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  //get items
                  let its = taxonsConnection.edges.map(o => o.node);
                  let pi = taxonsConnection.pageInfo;                      

                  //check: graphql errors
                  if(response.data.errors) {
                    let newError = {};
                    newError.message = 'taxonsConnection ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
                    newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', query: 'taxonsConnection', method: 'getData()', request: 'api.taxon.getItemsConnection'}];
                    newError.path=['add', 'informacion_taxonomica'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'A', method: 'getData()'}];
                    newError.path=['add', 'informacion_taxonomica'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
                ...
                //error
                .catch((err) => { //error: on api.taxon.getItemsConnection
                  if(err.isCanceled) {
                    return;
                  } else {
                    let newError = {};
          ...
          //error
          .catch((err) => { //error: on api.taxon.getCountItems
            if(err.isCanceled) {
              return
            } else {
              let newError = {};
      ...
      //error
      .catch((err) => { //error: on api.cuadrante.getAssociatedInformacion_taxonomicaConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.taxon.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'B', query: 'countTaxons', method: 'getDataB()', request: 'api.taxon.getCountItems'}];
          newError.path=['add', 'informacion_taxonomica'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'B', query: 'taxonsConnection', method: 'getDataB()', request: 'api.taxon.getItemsConnection'}];
                newError.path=['add', 'informacion_taxonomica'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'cuadrante', association: 'informacion_taxonomica', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'informacion_taxonomica'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.cuadrante&&
        ...
        lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.cuadrante_taxon_id&&
        (lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.cuadrante_taxon_id.added ||
         lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.cuadrante_taxon_id.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.cuadrante[String(item.cuadrante_id)].changedAssociations.cuadrante_taxon_id.idsRemoved;
          if(idsRemoved) {
