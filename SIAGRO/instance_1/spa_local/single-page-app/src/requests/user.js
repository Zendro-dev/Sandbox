...
export default {
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query usersConnection($pagination: paginationCursorInput)
            { usersConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
      ...
      if (o != null) {
        ...
        query =
          `query usersConnection($pagination: paginationCursorInput)
          { usersConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * getRolesConnection
   * 
   * Get roles connection (cursor based) records associated to the given user record
   ...
   * @param {String} ops Object with adittional query options.
   */
  getRolesConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneUser($pagination: paginationCursorInput) {
   ...
   * 
   *  
   * getAssociatedRolesConnection
   * 
   * Get the role-ids associated (by cursor based connection) to the given user record 
   * through association 'Roles', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedRolesConnection(url, itemId) {
    var query = 
      `{ readOneUser( id: ${itemId} ){ 
        rolesConnection{ edges { node { id } } } } }`;
    /**
     * Debug
     */
