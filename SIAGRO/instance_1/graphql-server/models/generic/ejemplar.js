...
module.exports = class Ejemplar {
    ...
    static async readById(id, benignErrorReporter) {
        ...
        try {
            ...
            return new Ejemplar(response.data.data.ejemplar);
            } else {
    ...
    async TaxonImpl({
        ...
        let ejemplar_id = this.getIdValue();

        let query = `query taxon { ejemplar(id:"${ejemplar_id}"){
            idnombrecatvalido{
                id,
                taxon,
                categoria,
        ...
        try {
            ...
            // console.log("RESPONSE: ", response)
            if (response && response.data && response.data.data) {
            return new models.taxon(response.data.data.ejemplar.idnombrecatvalido);
            } else {
            throw new Error(`Invalid response from remote cenz-server: Zacatuche`);
            }
        } catch (error) {
