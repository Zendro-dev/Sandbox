...
module.exports = class sitio extends Sequelize.Model {
    ...
    static associate(models) {
        sitio.hasMany(models.grupo_enfoque, {
            as: 'grupo_enfoque',
            foreignKey: 'sitio_id'
        });
    }
    ...
    static async readById(id) {
        let item = await sitio.findByPk(id);
        if (item === null) {
    ...
    static readAll(search, order, pagination, benignErrorReporter) {
        ...
        return super.count(options).then(async items => {
            ...
            if (globals.LIMIT_RECORDS < options['limit']) {
                throw new Error(`Request of total sitios exceeds max limit of ${globals.LIMIT_RECORDS}. Please use pagination.`);
            }
            ...
            if (pagination) {
                ...
                if (isForwardPagination) {
                    if (pagination.after) {
                        ...
                        options['where'] = {
                            ...options['where'],
                            ...helper.parseOrderCursor(options['order'], decoded_cursor, "sitio_id", pagination.includeCursor)
                        };
                    }
                } else { //backward
                    if (pagination.before) {
                        ...
                        options['where'] = {
                            ...options['where'],
                            ...helper.parseOrderCursorBefore(options['order'], decoded_cursor, "sitio_id", pagination.includeCursor)
                        };
                    }
                }
            }
    ...
    static bulkAddCsv(context) {
        ...
        return `Bulk import of sitio records started. You will be send an email to ${helpersAcl.getTokenFromContext(context).email} informing you about success or errors`;
    }
