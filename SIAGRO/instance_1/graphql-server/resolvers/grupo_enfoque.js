...
const grupo_enfoque = require(path.join(__dirname, '..', 'models', 'index.js')).grupo_enfoque;
...
grupo_enfoque.prototype.add_cuadrantes = async function(input, benignErrorReporter) {
    let bulkAssociationInput = input.addCuadrantes.map(associatedRecordId => {
        return {
            grupo_enfoque_id: this.getIdValue(),
            [models.cuadrante.idAttribute()]: associatedRecordId
        }
    });
    await models.cuadrante.bulkAssociateCuadranteWithGrupo_enfoque_id(bulkAssociationInput, benignErrorReporter);
}
...
grupo_enfoque.prototype.remove_cuadrantes = async function(input, benignErrorReporter) {
    ...
    await models.cuadrante.bulkDisAssociateCuadranteWithGrupo_enfoque_id(bulkAssociationInput, benignErrorReporter);
}
...
async function checkCountAndReduceRecordsLimit({
    search,
    pagination
}, context, resolverName, modelName = 'grupo_enfoque') {
    //defaults
    ...
    let paginationValues = helper.getGenericPaginationValues(pagination, "grupo_id", inputPaginationValues);
    //get records count
...
function checkCountForOneAndReduceRecordsLimit(context) {
    helper.checkCountAndReduceRecordLimitHelper(1, context, "readOneGrupo_enfoque")
}
/**
...
module.exports = {
    ...
    grupo_enfoquesConnection: async function({
        ...
        if (await checkAuthorization(context, 'grupo_enfoque', 'read') === true) {
            await checkCountAndReduceRecordsLimit({
                search,
                pagination
            }, context, "grupo_enfoquesConnection");
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
    ...
    vueTableGrupo_enfoque: async function(_, context) {
        if (await checkAuthorization(context, 'grupo_enfoque', 'read') === true) {
            return helper.vueTable(context.request, grupo_enfoque, ["id", "grupo_id", "tipo_grupo", "lista_especies", "sitio_id"]);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    addGrupo_enfoque: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            await createdGrupo_enfoque.handleAssociations(inputSanitized, benignErrorReporter);
    ...
    updateGrupo_enfoque: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            await updatedGrupo_enfoque.handleAssociations(inputSanitized, benignErrorReporter);
    ...
    /**
     * bulkAssociateGrupo_enfoqueWithSitio_id - bulkAssociaton resolver of given ids
     *
     * @param  {array} bulkAssociationInput Array of associations to add , 
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     * @return {string} returns message on success
     */
    bulkAssociateGrupo_enfoqueWithSitio_id: async function(bulkAssociationInput, context) {
        let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
        // if specified, check existence of the unique given ids
        if (!bulkAssociationInput.skipAssociationsExistenceChecks) {
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
                sitio_id
            }) => sitio_id)), models.sitio);
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
                grupo_id
            }) => grupo_id)), grupo_enfoque);
        }
        return await grupo_enfoque.bulkAssociateGrupo_enfoqueWithSitio_id(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
    /**
     * bulkDisAssociateGrupo_enfoqueWithSitio_id - bulkDisAssociaton resolver of given ids
     *
     * @param  {array} bulkAssociationInput Array of associations to remove , 
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     * @return {string} returns message on success
     */
    bulkDisAssociateGrupo_enfoqueWithSitio_id: async function(bulkAssociationInput, context) {
        let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
        // if specified, check existence of the unique given ids
        if (!bulkAssociationInput.skipAssociationsExistenceChecks) {
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
        ...
        return await grupo_enfoque.bulkDisAssociateGrupo_enfoqueWithSitio_id(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
