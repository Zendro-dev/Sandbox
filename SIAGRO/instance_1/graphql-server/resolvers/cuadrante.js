...
const cuadrante = require(path.join(__dirname, '..', 'models_index.js')).cuadrante;
...
async function checkCountAndReduceRecordsLimit(search, context, query) {
    let count = (await cuadrante.countRecords(search));
...
function checkCountForOneAndReduceRecordsLimit(context) {
    if (1 > context.recordsLimit) {
        throw new Error(errorMessageForRecordsLimit("readOneCuadrante"));
    }
...
module.exports = {
    ...
    cuadrantes: async function({
        ...
        if (await checkAuthorization(context, 'cuadrante', 'read') === true) {
            await checkCountAndReduceRecordsLimit(search, context, "cuadrantes");
    ...
    cuadrantesConnection: async function({
        ...
        if (await checkAuthorization(context, 'cuadrante', 'read') === true) {
            await checkCountAndReduceRecordsLimit(search, context, "cuadrantesConnection");
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
    ...
    vueTableCuadrante: async function(_, context) {
        if (await checkAuthorization(context, 'cuadrante', 'read') === true) {
            return helper.vueTable(context.request, cuadrante, ["id", "cuadrante_id", "tipo_planta", "produccion_etiqueta", "autoconsumo_etiqueta", "compra_etiqueta", "venta_etiqueta", "nombre_comun_grupo_enfoque", "grupo_enfoque_id", "taxon_id"]);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    addCuadrante: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            await createdCuadrante.handleAssociations(inputSanitized, context);
    ...
    updateCuadrante: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            await updatedCuadrante.handleAssociations(inputSanitized, context);
