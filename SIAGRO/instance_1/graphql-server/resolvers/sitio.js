...
sitio.prototype.add_grupo_enfoque = async function(input, benignErrorReporter) {
    let bulkAssociationInput = input.addGrupo_enfoque.map(associatedRecordId => {
        return {
            sitio_id: this.getIdValue(),
            [models.grupo_enfoque.idAttribute()]: associatedRecordId
        }
    });
    await models.grupo_enfoque.bulkAssociateGrupo_enfoqueWithSitio_id(bulkAssociationInput, benignErrorReporter);
}
...
sitio.prototype.remove_grupo_enfoque = async function(input, benignErrorReporter) {
    let bulkAssociationInput = input.removeGrupo_enfoque.map(associatedRecordId => {
        return {
            sitio_id: this.getIdValue(),
            [models.grupo_enfoque.idAttribute()]: associatedRecordId
        }
    });
    await models.grupo_enfoque.bulkDisAssociateGrupo_enfoqueWithSitio_id(bulkAssociationInput, benignErrorReporter);
}
...
async function checkCountAndReduceRecordsLimit({
    search,
    pagination
}, context, resolverName, modelName = 'sitio') {
    //defaults
    ...
    let paginationValues = helper.getGenericPaginationValues(pagination, "sitio_id", inputPaginationValues);
    //get records count
...
function checkCountForOneAndReduceRecordsLimit(context) {
    helper.checkCountAndReduceRecordLimitHelper(1, context, "readOneSitio")
}
/**
...
module.exports = {
    ...
    vueTableSitio: async function(_, context) {
        if (await checkAuthorization(context, 'sitio', 'read') === true) {
            return helper.vueTable(context.request, sitio, ["id", "sitio_id", "pais", "estado", "clave_estado", "municipio", "clave_municipio", "localidad", "clave_localidad"]);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
