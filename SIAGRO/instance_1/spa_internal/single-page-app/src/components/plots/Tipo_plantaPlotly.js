...
export default function TipoPlantaPlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.tipo_planta.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.tipo_plantaConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.tipo_plantaConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'tipo_planta', query: 'tipo_plantaConnection', method: 'getItemsAttribute()', request: 'api.tipo_planta.getItemsAttribute'}];
        newError.path=['Tipo_planta', 'csvTemplate'];
    ...
    .catch((error) => {
      ...
      newError.locations=[{model: 'tipo_planta', query: 'tipo_plantaConnection', method: 'getItemsAttribute()', request: 'api.tipo_planta.getItemsAttribute'}];
      newError.path=['Tipo_planta', 'Plot'];
