...
export default function RolePlotly(props) {
  ...
  async function getItemsAttribute(attribute) {
    /**
      * API Request
      */
      api.role.getItemsAttribute(graphqlServerUrl, attribute)
    .then((response) => {
      
      //check data
      if(response.data.data.rolesConnection) {
        //reduce to {x1:y1, x2:y2, ...}
        let result = response.data.data.rolesConnection.edges.reduce((acc, item) => {
      ...
      if(response.data.errors) {
        ...
        newError.locations=[{model: 'role', query: 'rolesConnection', method: 'getItemsAttribute()', request: 'api.role.getItemsAttribute'}];
