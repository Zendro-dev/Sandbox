...
export default function SitioDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.sitio[String(itemState.sitio_id)]) {
          
        //updated item
        if(lastModelChanged.sitio[String(itemState.sitio_id)].op === "update"&&
            lastModelChanged.sitio[String(itemState.sitio_id)].newItem) {
              //replace item
              setItemState(lastModelChanged.sitio[String(itemState.sitio_id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.sitio[String(itemState.sitio_id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.sitio_id]);
