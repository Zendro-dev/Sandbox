...
export default function CuadranteCreatePanel(props) {
  ...
  const [tipo_plantaIdsToAddState, setTipo_plantaIdsToAddState] = useState([]);
  const tipo_plantaIdsToAdd = useRef([]);
  ...
  function getInitialValues() {
    ...
    initialValues.tipo_planta_id = null;
  ...
  function setAddTipo_planta(variables) {
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      ...
      case 'informacion_taxonomica':
        ...
        break;
      case 'tipo_planta':
        if(tipo_plantaIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  return (
    ...
    <Dialog id='CuadranteCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='CuadranteCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Tooltip title={ t('modelPanels.save') + " cuadrante" }>
            <Fab
              id='CuadranteCreatePanel-fabButton-save' 
              color="secondary" 
