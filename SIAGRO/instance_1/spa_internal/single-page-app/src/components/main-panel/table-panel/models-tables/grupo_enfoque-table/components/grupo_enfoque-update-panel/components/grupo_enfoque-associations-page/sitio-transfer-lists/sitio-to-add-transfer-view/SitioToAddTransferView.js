...
export default function SitioToAddTransferView(props) {
  ...
  */
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'sitios' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.grupo_enfoque&&
        ...
        lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.sitio&&
        (lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.sitio.added ||
         lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.sitio.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.grupo_enfoque[String(item.grupo_id)].changedAssociations.sitio.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'grupo_enfoque' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'sitio',
     * but this action happened on the peer relation 'grupo_enfoque'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.sitio) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.grupo_enfoque) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.grupo_enfoque.removed) {
              let idsRemoved = entry[1].changedAssociations.grupo_enfoque.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.grupo_enfoque.added) {
              let idsAdded = entry[1].changedAssociations.grupo_enfoque.idsAdded;
              if(idsAdded) {
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem.sitio_id===it.sitio_id) ? 'sitio — ' + t('modelPanels.associatedRecord', 'Associated record') : 'sitio '}>
                                <Avatar>
                                  {"sitio".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'sitio_id' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.sitio_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem.sitio_id===it.sitio_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem.sitio_id===it.sitio_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
