  ...
  */
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.sitio&&
        ...
        lastModelChanged.sitio[String(item.sitio_id)].changedAssociations.grupo_enfoque&&
        (lastModelChanged.sitio[String(item.sitio_id)].changedAssociations.grupo_enfoque.added ||
         lastModelChanged.sitio[String(item.sitio_id)].changedAssociations.grupo_enfoque.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.sitio[String(item.sitio_id)].changedAssociations.grupo_enfoque.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'sitio' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'grupo_enfoque',
     * but this action happened on the peer relation 'sitio'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.grupo_enfoque) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.sitio) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.sitio.removed) {
              let idsRemoved = entry[1].changedAssociations.sitio.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.sitio.added) {
              let idsAdded = entry[1].changedAssociations.sitio.idsAdded;
              if(idsAdded) {
