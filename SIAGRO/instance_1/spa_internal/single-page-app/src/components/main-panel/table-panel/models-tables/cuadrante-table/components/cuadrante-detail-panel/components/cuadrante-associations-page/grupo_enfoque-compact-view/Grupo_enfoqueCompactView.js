...
export default function GrupoEnfoqueCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.cuadrante.getGrupo_enfoqueConnection(
      graphqlServerUrl, 
      item.cuadrante_id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'grupo_enfoque', query: 'readOneCuadrante', method: 'getData()', request: 'api.cuadrante.getGrupo_enfoqueConnection'}];
          newError.path=['detail', `cuadrante_id:${item.cuadrante_id}`, 'grupo_enfoque'];
        ...
        //get item
        let it = readOneCuadrante.grupo_enfoque;
      ...
      //error
      .catch((err) => { //error: on api.cuadrante.getGrupo_enfoqueConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
