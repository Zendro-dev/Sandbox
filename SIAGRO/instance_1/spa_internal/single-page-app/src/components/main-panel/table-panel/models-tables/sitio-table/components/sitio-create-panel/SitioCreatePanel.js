...
export default function SitioCreatePanel(props) {
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.sitio_id = 0;
  ...
  function doSave(event) {
    ...
    */
    ...
    /*
      API Request: addSitio
    */
    let cancelableApiReq = makeCancelable(api.sitio.createItem(graphqlServerUrl, variables));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'sitio', query: 'addSitio', method: 'doSave()', request: 'api.sitio.createItem'}];
          newError.path=['Sitios', 'add'];
        ...
        //check: addSitio
        let addSitio = response.data.data.addSitio;
        if(addSitio === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addSitio ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'sitio', query: 'addSitio', method: 'doSave()', request: 'api.sitio.createItem'}];
          newError.path=['Sitios', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addSitio ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'sitio', query: 'addSitio', method: 'doSave()', request: 'api.sitio.createItem'}];
          newError.path=['Sitios', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.sitio.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
