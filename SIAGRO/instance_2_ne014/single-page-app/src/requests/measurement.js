...
export default {
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countMeasurements(${s}) }`;
    }
    else {
      query = `{ countMeasurements }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query measurementsConnection($pagination: paginationCursorInput)
            { measurementsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
      ...
      if (o != null) {
        ...
        query =
          `query measurementsConnection($pagination: paginationCursorInput)
          { measurementsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * createItem
   * 
   * Add new Measurement item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Measurement item. 
   */
   ...
   * updateItem
   * 
   * Update Measurement item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Measurement item.  
   */
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Measurement item. 
   */
  ...
  /**
   * getAccessionConnection
   * 
   * Get accessions connection (cursor based) records associated to the given measurement record
   * through association 'Accession', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  getAccessionConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    var query = 
      `{ readOneMeasurement(measurement_id: "${itemId}") { 
        accession{ 
          accession_id,
          collectors_name,
          collectors_initials,
   ...
   * 
   *  
   * getAssociatedAccessionConnection
   * 
   * Get the accession-ids associated (by cursor based connection) to the given measurement record 
   * through association 'Accession', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedAccessionConnection(url, itemId) {
    var query = 
    `{ readOneMeasurement( measurement_id: "${itemId}" ){ 
      accession{ accession_id } } }`;
    /**
   ...
   * 
   * Get individuals connection (cursor based) records associated to the given measurement record
   ...
   * 
   * Get the individual-ids associated (by cursor based connection) to the given measurement record 
   * through association 'Individual', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
