...
const showMessageB = useCallback((message, withDetail) => {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.role&&
        ...
        lastModelChanged.role[String(item.id)].changedAssociations.users&&
        (lastModelChanged.role[String(item.id)].changedAssociations.users.added ||
         lastModelChanged.role[String(item.id)].changedAssociations.users.removed)) {
          
          //remove any dis-associated id from idsToRemove[]
          let idsRemoved = lastModelChanged.role[String(item.id)].changedAssociations.users.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'roles' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'user',
     * but this action happened on the peer relation 'roles'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.user) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.roles) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.roles.removed) {
              let idsRemoved = entry[1].changedAssociations.roles.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.roles.added) {
              let idsAdded = entry[1].changedAssociations.roles.idsAdded;
              if(idsAdded) {
