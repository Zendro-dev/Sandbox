...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let sublabel = 'institution_deposited';
    let variables = {
      pagination: {
    ...
    let cancelableApiReq = makeCancelable(api.location.getAccessionsConnection(
      graphqlServerUrl, 
      item.locationId,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'accessions', query: 'readOneLocation', method: 'getData()', request: 'api.location.getAccessionsConnection'}];
        ...
        || !Number.isInteger(readOneLocation.countFilteredAccessions)
        ...
        || typeof readOneLocation.accessionsConnection.pageInfo !== 'object'
        || readOneLocation.accessionsConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneLocation.countFilteredAccessions;
        let its = readOneLocation.accessionsConnection.edges.map(o => o.node);
        let pi = readOneLocation.accessionsConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.location.getAccessionsConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
