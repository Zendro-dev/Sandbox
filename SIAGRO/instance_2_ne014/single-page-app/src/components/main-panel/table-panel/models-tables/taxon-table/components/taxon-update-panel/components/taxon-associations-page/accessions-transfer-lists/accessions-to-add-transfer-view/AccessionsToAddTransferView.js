...
export default function AccessionsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.taxon.getAssociatedAccessionsConnection(graphqlServerUrl, item.id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Taxon', association: 'accessions', table:'A', query: 'readOneTaxon', method: 'getData()', request: 'api.taxon.getAssociatedAccessionsConnection'}];
        ...
        //get items
        let idso = readOneTaxon.accessionsConnection.edges.map(o => o.node);
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'Taxon', association: 'accessions', table:'A', query: 'countAccessions', method: 'getData()', request: 'api.accession.getCountItems'}];
              newError.path=['add', 'accessions'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'Taxon', association: 'accessions', table:'A', query: 'accessionsConnection', method: 'getData()', request: 'api.accession.getItemsConnection'}];
                  newError.path=['add', 'accessions'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(accessionsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'Taxon', association: 'accessions', table:'A', method: 'getData()'}];
                    newError.path=['add', 'accessions'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.taxon.getAssociatedAccessionsConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
