...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {measurement_id: lidsToRemove.current}
    //     }]
    //   };
    // }
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.accession.getMeasurementsConnection(
      graphqlServerUrl, 
      item.accession_id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'measurements', query: 'readOneAccession', method: 'getData()', request: 'api.accession.getMeasurementsConnection'}];
        ...
        || !Number.isInteger(readOneAccession.countFilteredMeasurements)
        ...
        || typeof readOneAccession.measurementsConnection.pageInfo !== 'object'
        || readOneAccession.measurementsConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneAccession.countFilteredMeasurements;
        let its = readOneAccession.measurementsConnection.edges.map(o => o.node);
        let pi = readOneAccession.measurementsConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.accession.getMeasurementsConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
            ...
            if(entry[1].changedAssociations.accession.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.measurement_id;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.measurement_id) === -1) {
      lidsToRemove.current.push(item.measurement_id);
      setThereAreItemsToAdd(true);
      updateHeights();
      //reload A
      reloadDataA();
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.measurement_id);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        let disabled = lidsToRemove.current.find(measurement_id=> measurement_id=== it.measurement_id) !== undefined;
