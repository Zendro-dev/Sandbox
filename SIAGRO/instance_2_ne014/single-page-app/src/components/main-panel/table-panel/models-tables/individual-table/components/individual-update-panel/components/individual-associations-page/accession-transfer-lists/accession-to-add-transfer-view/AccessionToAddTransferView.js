...
import AccessionToAddTransferViewToolbar from './components/AccessionToAddTransferViewToolbar';
import AccessionToAddTransferViewCursorPagination from './components/AccessionToAddTransferViewCursorPagination';
...
export default function AccessionToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.individual.getAssociatedAccessionConnection(graphqlServerUrl, item.name));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Individual', association: 'accession', table:'A', query: 'readOneIndividual', method: 'getData()', request: 'api.individual.getAssociatedAccessionConnection'}];
          newError.path=['add', 'accession'];
        ...
        || typeof readOneIndividual.accession!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneIndividual.accession;
        ...
        //ok
        //set associated id
        lidsAssociated.current = (idso !== null) ? [idso.accession_id] : [];
        ...
        */
        let cancelableApiReqB = makeCancelable(api.accession.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'Individual', association: 'accession', table:'A', query: 'countAccessions', method: 'getData()', request: 'api.accession.getCountItems'}];
              newError.path=['add', 'accession'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.accession.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'Individual', association: 'accession', table:'A', query: 'accessionsConnection', method: 'getData()', request: 'api.accession.getItemsConnection'}];
                  newError.path=['add', 'accession'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(accessionsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'Individual', association: 'accession', table:'A', method: 'getData()'}];
                    newError.path=['add', 'accession'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.individual.getAssociatedAccessionConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, t, dataTrigger, item.name, search, rowsPerPage]);
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'Individual', association: 'accession', table:'B', query: 'countAccessions', method: 'getDataB()', request: 'api.accession.getCountItems'}];
          newError.path=['add', 'accession'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'Individual', association: 'accession', table:'B', query: 'accessionsConnection', method: 'getDataB()', request: 'api.accession.getItemsConnection'}];
                newError.path=['add', 'accession'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'Individual', association: 'accession', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'accession'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'accession' for this item was updated.
    ...
    if(lastModelChanged.Individual&&
        ...
        lastModelChanged.Individual[String(item.name)].changedAssociations&&
        lastModelChanged.Individual[String(item.name)].changedAssociations.accession&&
        (lastModelChanged.Individual[String(item.name)].changedAssociations.accession.added ||
         lastModelChanged.Individual[String(item.name)].changedAssociations.accession.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.Individual[String(item.name)].changedAssociations.accession.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('accession', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'individuals' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'Accession',
     * but this action happened on the peer relation 'individuals'.
     * 
     * Conditions:
     ...
     * 
     * else if B:
     * - remove the internalId of the 'Accession' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.Accession) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.individuals) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.individuals.removed) {
              let idsRemoved = entry[1].changedAssociations.individuals.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.name);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.individuals.added) {
              let idsAdded = entry[1].changedAssociations.individuals.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.name);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.accession_id;
                  ...
                  handleUntransfer('accession', idAdded);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.name, handleUntransfer]);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.accession_id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('accession', item.accession_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('accession', item.accession_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <AccessionToAddTransferViewToolbar
                title={'Accessions'}
                titleIcon={1}
                search={search}
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem.accession_id===it.accession_id) ? 'Accession — ' + t('modelPanels.associatedRecord', 'Associated record') : 'Accession '}>
                                <Avatar>
                                  {"accession".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            ...
                            <ListItemText
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    ...
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem.accession_id===it.accession_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem.accession_id===it.accession_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                              ...
                              secondary={
                                <React.Fragment>
                                  ...
                                  {/* Sublabel */}
                                  {(sublabel) && (
                                    <Tooltip title={ 'institution_deposited' }>
              ...
              {/* Pagination */}
              <AccessionToAddTransferViewCursorPagination
                count={count}
...
AccessionToAddTransferView.propTypes = {
