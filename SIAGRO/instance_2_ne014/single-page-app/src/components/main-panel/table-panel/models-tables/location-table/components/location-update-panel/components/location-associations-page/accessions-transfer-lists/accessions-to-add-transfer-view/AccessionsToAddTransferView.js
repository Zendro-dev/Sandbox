...
export default function AccessionsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.location.getAssociatedAccessionsConnection(graphqlServerUrl, item.locationId));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Location', association: 'accessions', table:'A', query: 'readOneLocation', method: 'getData()', request: 'api.location.getAssociatedAccessionsConnection'}];
        ...
        || typeof readOneLocation.accessionsConnection !== 'object'
        || readOneLocation.accessionsConnection === null
        || !Array.isArray(readOneLocation.accessionsConnection.edges)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let idso = readOneLocation.accessionsConnection.edges.map(o => o.node);
        ...
        //set associated ids
        lidsAssociated.current = idso.map(function(item){ return item.accession_id});
      ...
      //error
      .catch((err) => { //error: on api.location.getAssociatedAccessionsConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, t, dataTrigger, item.locationId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.Location&&
        lastModelChanged.Location[String(item.locationId)]&&
        lastModelChanged.Location[String(item.locationId)].changedAssociations&&
        lastModelChanged.Location[String(item.locationId)].changedAssociations.accessions&&
        (lastModelChanged.Location[String(item.locationId)].changedAssociations.accessions.added ||
         lastModelChanged.Location[String(item.locationId)].changedAssociations.accessions.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.Location[String(item.locationId)].changedAssociations.accessions.idsRemoved;
          if(idsRemoved) {
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.location) {
            ...
            if(entry[1].changedAssociations.location.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.locationId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.location.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.locationId);
                if(iof !== -1) {
                  ...
                  handleUntransfer('accessions', idAdded);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.locationId, handleUntransfer]);
