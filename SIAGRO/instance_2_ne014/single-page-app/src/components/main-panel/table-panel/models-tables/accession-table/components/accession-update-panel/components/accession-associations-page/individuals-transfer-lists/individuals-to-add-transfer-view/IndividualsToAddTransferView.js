...
import IndividualsToAddTransferViewToolbar from './components/IndividualsToAddTransferViewToolbar';
import IndividualsToAddTransferViewCursorPagination from './components/IndividualsToAddTransferViewCursorPagination';
...
export default function IndividualsToAddTransferView(props) {
  ...
  */
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.name);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <IndividualsToAddTransferViewToolbar
                title={'Individuals'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <IndividualsToAddTransferViewCursorPagination
                count={count}
...
IndividualsToAddTransferView.propTypes = {
