...
import IndividualsToRemoveTransferViewToolbar from './components/IndividualsToRemoveTransferViewToolbar';
import IndividualsToRemoveTransferViewCursorPagination from './components/IndividualsToRemoveTransferViewCursorPagination';
...
export default function IndividualsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {name: lidsToRemove.current}
    //     }]
    //   };
    // }
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.accession.getIndividualsConnection(
      graphqlServerUrl, 
      item.accession_id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'individuals', query: 'readOneAccession', method: 'getData()', request: 'api.accession.getIndividualsConnection'}];
          newError.path=['remove', 'individuals'];
        ...
        || !Number.isInteger(readOneAccession.countFilteredIndividuals)
        ...
        || typeof readOneAccession.individualsConnection.pageInfo !== 'object'
        || readOneAccession.individualsConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneAccession.countFilteredIndividuals;
        let its = readOneAccession.individualsConnection.edges.map(o => o.node);
        let pi = readOneAccession.individualsConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.accession.getIndividualsConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'individuals' for this item was updated.
    ...
    if(lastModelChanged.Accession&&
        ...
        lastModelChanged.Accession[String(item.accession_id)].changedAssociations.individuals&&
        (lastModelChanged.Accession[String(item.accession_id)].changedAssociations.individuals.added ||
         lastModelChanged.Accession[String(item.accession_id)].changedAssociations.individuals.removed)) {
          
          //remove any dis-associated id from idsToRemove[]
          let idsRemoved = lastModelChanged.Accession[String(item.accession_id)].changedAssociations.individuals.idsRemoved;
          if(idsRemoved) {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'Individual' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.accession.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.name;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.name) === -1) {
      lidsToRemove.current.push(item.name);
      setThereAreItemsToAdd(true);
      updateHeights();
      //reload A
      reloadDataA();
      //reload B (full)
      resetReloadDataB();
      handleTransfer('individuals', item.name);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.name);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('individuals', item.name);
    }
  }
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <IndividualsToRemoveTransferViewToolbar
                title={'Individuals'}
                titleIcon={1}
                search={search}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'Individual' }>
                                <Avatar>{"individual".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
              ...
              {/* Pagination */}
              <IndividualsToRemoveTransferViewCursorPagination
                count={count}
...
IndividualsToRemoveTransferView.propTypes = {
