...
export default function LocationToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.accession.getAssociatedLocationConnection(graphqlServerUrl, item.accession_id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Accession', association: 'location', table:'A', query: 'readOneAccession', method: 'getData()', request: 'api.accession.getAssociatedLocationConnection'}];
        ...
        || typeof readOneAccession.location!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneAccession.location;
        ...
        //ok
        //set associated id
        lidsAssociated.current = (idso !== null) ? [idso.locationId] : [];
        ...
        */
        let cancelableApiReqB = makeCancelable(api.location.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'Accession', association: 'location', table:'A', query: 'countLocations', method: 'getData()', request: 'api.location.getCountItems'}];
              newError.path=['add', 'location'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.location.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'Accession', association: 'location', table:'A', query: 'locationsConnection', method: 'getData()', request: 'api.location.getItemsConnection'}];
                  newError.path=['add', 'location'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(locationsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'Accession', association: 'location', table:'A', method: 'getData()'}];
                    newError.path=['add', 'location'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.accession.getAssociatedLocationConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    if(lidsToAdd.current !== undefined && lidsToAdd.current.length > 0) {
      ops = {
        only: [{
          type: 'String',
          values: {locationId: lidsToAdd.current}
        }]
      };
    } else {
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'Accession', association: 'location', table:'B', query: 'countLocations', method: 'getDataB()', request: 'api.location.getCountItems'}];
          newError.path=['add', 'location'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'Accession', association: 'location', table:'B', query: 'locationsConnection', method: 'getDataB()', request: 'api.location.getItemsConnection'}];
                newError.path=['add', 'location'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'Accession', association: 'location', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'location'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'locations' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.Accession&&
        ...
        lastModelChanged.Accession[String(item.accession_id)].changedAssociations.location&&
        (lastModelChanged.Accession[String(item.accession_id)].changedAssociations.location.added ||
         lastModelChanged.Accession[String(item.accession_id)].changedAssociations.location.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.Accession[String(item.accession_id)].changedAssociations.location.idsRemoved;
          if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.accessions.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.locationId;
    ...
    if(lastModelChanged.Location) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.locationId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.locationId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.locationId===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.locationId;

          //lookup item on any tables or associated ids
          let iofA = items.findIndex((item) => item.locationId===idRemoved);
          let iofB = itemsB.findIndex((item) => item.locationId===idRemoved);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.locationId) === -1) {
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.locationId);
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('location', item.locationId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('location', item.locationId);
    }
  };
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.locationId;
                        let label = it.country;
                        let sublabel = it.state;
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem.locationId===it.locationId) ? 'Location — ' + t('modelPanels.associatedRecord', 'Associated record') : 'Location '}>
                                <Avatar>
                                  {"location".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'locationId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.locationId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem.locationId===it.locationId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem.locationId===it.locationId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
