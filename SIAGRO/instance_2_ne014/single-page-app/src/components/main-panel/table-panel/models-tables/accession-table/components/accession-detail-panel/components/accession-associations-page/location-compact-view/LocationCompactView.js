...
export default function LocationCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.accession.getLocationConnection(
      graphqlServerUrl, 
      item.accession_id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'location', query: 'readOneAccession', method: 'getData()', request: 'api.accession.getLocationConnection'}];
          newError.path=['detail', `accession_id:${item.accession_id}`, 'location'];
        ...
        //get item
        let it = readOneAccession.location;
      ...
      //error
      .catch((err) => { //error: on api.accession.getLocationConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
