...
const showMessageB = useCallback((message, withDetail) => {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.individual.getMeasurementsConnection(
      graphqlServerUrl, 
      item.name,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'measurements', query: 'readOneIndividual', method: 'getData()', request: 'api.individual.getMeasurementsConnection'}];
        ...
        || !Number.isInteger(readOneIndividual.countFilteredMeasurements)
        ...
        || typeof readOneIndividual.measurementsConnection.pageInfo !== 'object'
        || readOneIndividual.measurementsConnection.pageInfo === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let newCount = readOneIndividual.countFilteredMeasurements;
        let its = readOneIndividual.measurementsConnection.edges.map(o => o.node);
        let pi = readOneIndividual.measurementsConnection.pageInfo;
      ...
      //error
      .catch((err) => { //error: on api.individual.getMeasurementsConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
