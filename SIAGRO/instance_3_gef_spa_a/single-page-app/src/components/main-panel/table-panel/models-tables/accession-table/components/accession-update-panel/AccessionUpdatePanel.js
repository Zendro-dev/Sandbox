    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Accession', query: 'updateAccession', method: 'doSave()', request: 'api.accession.updateItem'}];
          newError.path=['Accessions', `accession_id:${item.accession_id}`, 'update'];
        ...
        //check: updateAccession
        let updateAccession = response.data.data.updateAccession;
        if(updateAccession === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'updateAccession ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'Accession', query: 'updateAccession', method: 'doSave()', request: 'api.accession.updateItem'}];
          newError.path=['Accessions', `accession_id:${item.accession_id}`, 'update'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: updateAccession type
        if(typeof updateAccession !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'Accession ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'Accession', query: 'updateAccession', method: 'doSave()', request: 'api.accession.updateItem'}];
          newError.path=['Accessions', `accession_id:${item.accession_id}`, 'update'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          newError.message = 'updateAccession ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'Accession', query: 'updateAccession', method: 'doSave()', request: 'api.accession.updateItem'}];
          newError.path=['Accessions', `accession_id:${item.accession_id}`, 'update'];
        ...
        onClose(event, true, updateAccession);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.accession.updateItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
