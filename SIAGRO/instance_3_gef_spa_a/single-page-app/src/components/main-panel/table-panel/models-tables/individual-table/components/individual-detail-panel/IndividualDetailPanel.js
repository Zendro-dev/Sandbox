...
import IndividualUpdatePanel from '../individual-update-panel/IndividualUpdatePanel'
import IndividualDeleteConfirmationDialog from '../IndividualDeleteConfirmationDialog'
...
export default function IndividualDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.Individual[String(itemState.name)]) {
          
        //updated item
        if(lastModelChanged.Individual[String(itemState.name)].op === "update"&&
            lastModelChanged.Individual[String(itemState.name)].newItem) {
              //replace item
              setItemState(lastModelChanged.Individual[String(itemState.name)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.Individual[String(itemState.name)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.name]);
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deleteIndividual
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('Individual', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('Individual', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Individual | name: " + itemState.name}
              </Typography>
              {/*
                Actions:
                - Edit
...
IndividualDetailPanel.propTypes = {
