    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'taxon', query: 'readOneAccession', method: 'getData()', request: 'api.accession.getTaxonConnection'}];
          newError.path=['detail', `accession_id:${item.accession_id}`, 'taxon'];
      ...
      //error
      .catch((err) => { //error: on api.accession.getTaxonConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
