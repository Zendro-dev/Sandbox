...
import MeasurementsToAddTransferViewToolbar from './components/MeasurementsToAddTransferViewToolbar';
import MeasurementsToAddTransferViewCursorPagination from './components/MeasurementsToAddTransferViewCursorPagination';
...
export default function MeasurementsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.accession.getAssociatedMeasurementsConnection(graphqlServerUrl, item.accession_id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Accession', association: 'measurements', table:'A', query: 'readOneAccession', method: 'getData()', request: 'api.accession.getAssociatedMeasurementsConnection'}];
        ...
        || typeof readOneAccession.measurementsConnection !== 'object'
        || readOneAccession.measurementsConnection === null
        || !Array.isArray(readOneAccession.measurementsConnection.edges)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get items
        let idso = readOneAccession.measurementsConnection.edges.map(o => o.node);
        ...
        //set associated ids
        lidsAssociated.current = idso.map(function(item){ return item.measurement_id});
        ...
        */
        let cancelableApiReqB = makeCancelable(api.measurement.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'Accession', association: 'measurements', table:'A', query: 'countMeasurements', method: 'getData()', request: 'api.measurement.getCountItems'}];
              newError.path=['add', 'measurements'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.measurement.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'Accession', association: 'measurements', table:'A', query: 'measurementsConnection', method: 'getData()', request: 'api.measurement.getItemsConnection'}];
                  newError.path=['add', 'measurements'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(measurementsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'Accession', association: 'measurements', table:'A', method: 'getData()'}];
                    newError.path=['add', 'measurements'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.accession.getAssociatedMeasurementsConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    if(lidsToAdd.current !== undefined && lidsToAdd.current.length > 0) {
      ops = {
        only: [{
          type: 'String',
          values: {measurement_id: lidsToAdd.current}
        }]
      };
    } else {
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'Accession', association: 'measurements', table:'B', query: 'countMeasurements', method: 'getDataB()', request: 'api.measurement.getCountItems'}];
          newError.path=['add', 'measurements'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'Accession', association: 'measurements', table:'B', query: 'measurementsConnection', method: 'getDataB()', request: 'api.measurement.getItemsConnection'}];
                newError.path=['add', 'measurements'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'Accession', association: 'measurements', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'measurements'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.Accession&&
        ...
        lastModelChanged.Accession[String(item.accession_id)].changedAssociations.measurements&&
        (lastModelChanged.Accession[String(item.accession_id)].changedAssociations.measurements.added ||
         lastModelChanged.Accession[String(item.accession_id)].changedAssociations.measurements.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.Accession[String(item.accession_id)].changedAssociations.measurements.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'accession' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'Measurement',
     * but this action happened on the peer relation 'accession'.
     * 
     * Conditions:
     ...
     * 
     * else if B:
     * - remove the internalId of the 'Measurement' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.Measurement) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.accession) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.accession.removed) {
              let idsRemoved = entry[1].changedAssociations.accession.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.accession.added) {
              let idsAdded = entry[1].changedAssociations.accession.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.accession_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.measurement_id;
                  ...
                  handleUntransfer('measurements', idAdded);
    ...
    if(lastModelChanged.Measurement) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.measurement_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.measurement_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.measurement_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.measurement_id;

          //lookup item on any tables or associated ids
          let iofA = items.findIndex((item) => item.measurement_id===idRemoved);
          let iofB = itemsB.findIndex((item) => item.measurement_id===idRemoved);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.measurement_id) === -1) {
      lidsToAdd.current.push(item.measurement_id);
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('measurements', item.measurement_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.measurement_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('measurements', item.measurement_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MeasurementsToAddTransferViewToolbar
                title={'Measurements'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <MeasurementsToAddTransferViewCursorPagination
                count={count}
...
MeasurementsToAddTransferView.propTypes = {
