    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Accession', query: 'addAccession', method: 'doSave()', request: 'api.accession.createItem'}];
          newError.path=['Accessions', 'add'];
        ...
        //check: addAccession
        let addAccession = response.data.data.addAccession;
        if(addAccession === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addAccession ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'Accession', query: 'addAccession', method: 'doSave()', request: 'api.accession.createItem'}];
          newError.path=['Accessions', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: addAccession type
        if(typeof addAccession !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addAccession ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'Accession', query: 'addAccession', method: 'doSave()', request: 'api.accession.createItem'}];
          newError.path=['Accessions', 'add'];
        ...
        onClose(event, true, addAccession);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.accession.createItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
