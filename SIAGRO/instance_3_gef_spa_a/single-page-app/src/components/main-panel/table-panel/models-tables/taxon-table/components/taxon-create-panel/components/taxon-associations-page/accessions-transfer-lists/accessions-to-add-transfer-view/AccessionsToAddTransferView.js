    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'Taxon', association: 'accessions', table:'B', query: 'countAccessions', method: 'getData()', request: 'api.accession.getCountItems'}];
          newError.path=['add', 'accessions'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'Taxon', association: 'accessions', table:'B', query: 'accessionsConnection', method: 'getData()', request: 'api.accession.getItemsConnection'}];
                newError.path=['add', 'accessions'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
