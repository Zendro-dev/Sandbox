...
export default function IndividualCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.measurement.getIndividualConnection(
      graphqlServerUrl, 
      item.measurement_id,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'individual', query: 'readOneMeasurement', method: 'getData()', request: 'api.measurement.getIndividualConnection'}];
          newError.path=['detail', `measurement_id:${item.measurement_id}`, 'individual'];
        ...
        //get item
        let it = readOneMeasurement.individual;
      ...
      //error
      .catch((err) => { //error: on api.measurement.getIndividualConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
