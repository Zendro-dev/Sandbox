...
export default function AccessionCompactView(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.individual.getAccessionConnection(
      graphqlServerUrl, 
      item.name,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{association: 'accession', query: 'readOneIndividual', method: 'getData()', request: 'api.individual.getAccessionConnection'}];
          newError.path=['detail', `name:${item.name}`, 'accession'];
        ...
        //get item
        let it = readOneIndividual.accession;
      ...
      //error
      .catch((err) => { //error: on api.individual.getAccessionConnection
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
