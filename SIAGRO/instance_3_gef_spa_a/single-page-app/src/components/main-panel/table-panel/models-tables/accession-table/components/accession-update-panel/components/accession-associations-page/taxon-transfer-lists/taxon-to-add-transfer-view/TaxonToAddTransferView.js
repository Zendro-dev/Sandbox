...
export default function TaxonToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.accession.getAssociatedTaxonConnection(graphqlServerUrl, item.accession_id));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Accession', association: 'taxon', table:'A', query: 'readOneAccession', method: 'getData()', request: 'api.accession.getAssociatedTaxonConnection'}];
          newError.path=['add', 'taxon'];
        ...
        || typeof readOneAccession.taxon!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        //get item
        let idso = readOneAccession.taxon;
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'Accession', association: 'taxon', table:'A', query: 'countTaxons', method: 'getData()', request: 'api.taxon.getCountItems'}];
              newError.path=['add', 'taxon'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'Accession', association: 'taxon', table:'A', query: 'taxonsConnection', method: 'getData()', request: 'api.taxon.getItemsConnection'}];
                  newError.path=['add', 'taxon'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(taxonsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'Accession', association: 'taxon', table:'A', method: 'getData()'}];
                    newError.path=['add', 'taxon'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.accession.getAssociatedTaxonConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'Accession', association: 'taxon', table:'B', query: 'countTaxons', method: 'getDataB()', request: 'api.taxon.getCountItems'}];
          newError.path=['add', 'taxon'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'Accession', association: 'taxon', table:'B', query: 'taxonsConnection', method: 'getDataB()', request: 'api.taxon.getItemsConnection'}];
                newError.path=['add', 'taxon'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'Accession', association: 'taxon', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'taxon'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.Accession&&
        ...
        lastModelChanged.Accession[String(item.accession_id)].changedAssociations&&
        lastModelChanged.Accession[String(item.accession_id)].changedAssociations.taxon&&
        (lastModelChanged.Accession[String(item.accession_id)].changedAssociations.taxon.added ||
         lastModelChanged.Accession[String(item.accession_id)].changedAssociations.taxon.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.Accession[String(item.accession_id)].changedAssociations.taxon.idsRemoved;
          if(idsRemoved) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.accession_id, handleUntransfer]);
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem.id===it.id) ? 'Taxon — ' + t('modelPanels.associated', 'Associated') : 'Taxon '}>
                                <Avatar>
                                  {"taxon".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    ...
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem.id===it.id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associated', 'Associated') : t('modelPanels.internalId', 'Unique Identifier')}>
