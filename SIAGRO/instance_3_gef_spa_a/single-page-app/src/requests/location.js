...
export default {
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('location', searchText, ops);
    
    var query = '';

    //if has search
    if (s !== null) {
      query = `{ countLocations(${s}) }`;
    }
    else {
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query locationsConnection($pagination: paginationCursorInput)
            { locationsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query locationsConnection($pagination: paginationCursorInput)
            { locationsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query locationsConnection($pagination: paginationCursorInput)
          { locationsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
  ...
  /**
   * getAccessionsConnection
   * 
   * Get accessions connection (cursor based) records associated to the given location record
   * through association 'Accessions', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  getAccessionsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneLocation($pagination: paginationCursorInput) {
        readOneLocation(locationId: "${itemId}") { 
          accessionsConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredAccessions( ${s} ) 
      } }` :      
      `query readOneLocation($pagination: paginationCursorInput) {
        readOneLocation(locationId: "${itemId}") { 
   ...
   * 
   *  
   * getAssociatedAccessionsConnection
   * 
   * Get the accession-ids associated (by cursor based connection) to the given location record 
   * through association 'Accessions', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedAccessionsConnection(url, itemId) {
    var query = 
      `{ readOneLocation( locationId: "${itemId}" ){ 
        accessionsConnection{ edges { node { accession_id } } } } }`;
    /**
     * Debug
     */
