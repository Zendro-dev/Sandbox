...
module.exports = class book_server2 extends Sequelize.Model {
    ...
    static async readById(id) {
        let item = await book_server2.findByPk(id);
        if (item === null) {
            ...
            if (pagination) {
                ...
                if (isForwardPagination) {
                    if (pagination.after) {
                        ...
                        options['where'] = {
                            ...options['where'],
                            ...helper.parseOrderCursor(options['order'], decoded_cursor, "book_id", pagination.includeCursor)
                        };
                    }
                } else { //backward
                    if (pagination.before) {
                        ...
                        options['where'] = {
                            ...options['where'],
                            ...helper.parseOrderCursorBefore(options['order'], decoded_cursor, "book_id", pagination.includeCursor)
                        };
                    }
                }
            }
    ...
    static bulkAddCsv(context) {
        ...
        return `Bulk import of book_server2 records started. You will be send an email to ${helpersAcl.getTokenFromContext(context).email} informing you about success or errors`;
    }
     ...
     * @return {string} returns message on success
     */
    static async bulkDisAssociateBookWithAuthor_id(bulkAssociationInput) {
        let mappedForeignKeys = helper.mapForeignKeysToPrimaryKeyArray(bulkAssociationInput, "book_id", "author_id");
        var promises = [];
        mappedForeignKeys.forEach(({
            author_id,
    ...
    static idAttribute() {
        return book_server2.definition.id.name;
    }
    ...
    static idAttributeType() {
        return book_server2.definition.id.type;
    }
    ...
    getIdValue() {
        return this[book_server2.idAttribute()]
    }
    ...
    stripAssociations() {
        let attributes = Object.keys(book_server2.definition.attributes);
