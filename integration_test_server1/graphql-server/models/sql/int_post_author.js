...
module.exports = class int_post_author extends Sequelize.Model {
    ...
    static async readAll(search, order, pagination, benignErrorReporter) {
        ...
        records = records.map(x => {return int_post_author.postReadCast(x)})
  ...
  static preCreateCast(record){
    let clone = _.cloneDeep(record);
    for(let attr in definition.attributes){
      let type = definition.attributes[ attr ].replace(/\s+/g, '');
      if(type[0]==='[' && clone[ attr ]!== undefined && clone[ attr ]!== null){
        clone[ attr ] = JSON.stringify(clone[attr]);
      }
    }
