...
module.exports = class int_post_book extends Sequelize.Model {
    ...
    static async readById(id) {
        ...
        item = int_post_book.postReadCast(item)
        return validatorUtil.validateData('validateAfterRead', this, item);
    }

    static async countRecords(search) {
        let options = {}
        options['where'] = helper.searchConditionsToSequelize(search, int_post_book.definition.attributes);
    ...
    static async readAll(search, order, pagination, benignErrorReporter) {
        ...
        let options = helper.buildLimitOffsetSequelizeOptions(search, order, pagination, this.idAttribute(), int_post_book.definition.attributes);
        let records = await super.findAll(options);
        records = records.map(x => int_post_book.postReadCast(x))
    ...
    static async readAllCursor(search, order, pagination, benignErrorReporter) {
        ...
        let options = helper.buildCursorBasedSequelizeOptions(search, order, pagination, this.idAttribute(), int_post_book.definition.attributes);
        let records = await super.findAll(options);
        ...
        if (pagination && (pagination.after || pagination.before)) {
            let oppOptions = helper.buildOppositeSearchSequelize(search, order, {
                ...pagination,
                includeCursor: false
            }, this.idAttribute(), int_post_book.definition.attributes);
    ...
    static async addOne(input) {
        ...
        input = int_post_book.preWriteCast(input)
        try {
            ...
            int_post_book.postReadCast(result.dataValues)
            int_post_book.postReadCast(result._previousDataValues)
            return result;
        } catch (error) {
            throw error;
        }
    ...
    static async add_author_ids(id, author_ids, benignErrorReporter, handle_inverse = true) {
        ...
        if (handle_inverse) {
            ...
            author_ids.forEach(idx => {
                promises.push(models.int_post_author.add_book_ids(idx, [`${id}`], benignErrorReporter, false));
            });
    ...
    static async remove_author_ids(id, author_ids, benignErrorReporter, handle_inverse = true) {
        ...
        if (handle_inverse) {
            let promises = [];
            author_ids.forEach(idx => {
                promises.push(models.int_post_author.remove_book_ids(idx, [`${id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
