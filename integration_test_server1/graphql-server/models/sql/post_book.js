...
module.exports = class post_book extends Sequelize.Model {
    ...
    static async readById(id) {
        let item = await post_book.findByPk(id);
        if (item === null) {
    ...
    static bulkAddCsv(context) {
        ...
        return `Bulk import of post_book records started. You will be send an email to ${helpersAcl.getTokenFromContext(context).email} informing you about success or errors`;
    }
    ...
    static async add_author_ids(id, author_ids, benignErrorReporter, handle_inverse = true) {
        ...
        if (handle_inverse) {
            ...
            author_ids.forEach(idx => {
                promises.push(models.post_author.add_book_ids(idx, [`${id}`], benignErrorReporter, false));
            });
    ...
    static async remove_author_ids(id, author_ids, benignErrorReporter, handle_inverse = true) {
        ...
        if (handle_inverse) {
            let promises = [];
            author_ids.forEach(idx => {
                promises.push(models.post_author.remove_book_ids(idx, [`${id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
