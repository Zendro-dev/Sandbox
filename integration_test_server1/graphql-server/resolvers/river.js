...
river.prototype.countriesFilter = async function({
    ...
    if (await checkAuthorization(context, 'country', 'read') === true) {
        await checkCountAndReduceRecordsLimit(search, context, 'countriesFilter', 'country');
...
river.prototype.countriesConnection = async function({
    ...
    if (await checkAuthorization(context, 'country', 'read') === true) {
        await checkCountAndReduceRecordsLimit(search, context, 'countriesConnection', 'country');
 ...
 */
async function checkCountAndReduceRecordsLimit(search, context, resolverName, modelName = 'river') {
...
module.exports = {
    ...
    rivers: async function({
        ...
        if (await checkAuthorization(context, 'river', 'read') === true) {
            await checkCountAndReduceRecordsLimit(search, context, "rivers");
    ...
    riversConnection: async function({
        ...
        if (await checkAuthorization(context, 'river', 'read') === true) {
            await checkCountAndReduceRecordsLimit(search, context, "riversConnection");
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
    ...
    addRiver: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            await createdRiver.handleAssociations(inputSanitized, context);
            return createdRiver;
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    ...
    updateRiver: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            await updatedRiver.handleAssociations(inputSanitized, context);
            return updatedRiver;
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
