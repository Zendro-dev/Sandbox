...
const definition = {
    ...
    registry: [
        'study_to_contact_PHENOMIS',
        'study_to_contact_PHIS',
        'study_to_contact_PIPPA'
    ],
    attributes: {
        studyDbId: 'String',
        contactDbId: 'String'
...
let registry = ["study_to_contact_PHENOMIS", "study_to_contact_PHIS", "study_to_contact_PIPPA"];

module.exports = class study_to_contact {
    ...
    static get registeredAdapters() {
        return ["study_to_contact_PHENOMIS", "study_to_contact_PHIS", "study_to_contact_PIPPA"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readById(id) {
        if (id !== null) {
            ...
            return adapters[responsibleAdapter[0]].readById(id).then(result => new study_to_contact(result));
        }
    }
    ...
    stripAssociations() {
        let attributes = Object.keys(study_to_contact.definition.attributes);
        attributes.push('id');
    ...
    static idAttribute() {
        let internalId = study_to_contact.definition.id.name;
        return internalId;
    }
    ...
    static idAttributeType() {
        return study_to_contact.definition.id.type;
    }

    /**
     * getIdValue - Get the value of the idAttribute ("id", or "internalId") for an instance of study_to_contact.
     *
     * @return {type} id value
     */

    getIdValue() {
        return this[study_to_contact.idAttribute()]
    }
    ...
    static addOne(input) {
        ...
        return adapters[responsibleAdapter].addOne(input).then(result => new study_to_contact(result));
    }
    ...
    static updateOne(input) {
        this.assertInputHasId(input);
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input).then(result => new study_to_contact(result));
    }
    ...
    static bulkAddCsv(context) {
        throw new Error("study_to_contact.bulkAddCsv is not implemented.")
    }

    static csvTableTemplate() {
        return helper.csvTableTemplate(study_to_contact);
    }
}