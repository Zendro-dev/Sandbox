...
let registry = ["season_PHENOMIS", "season_PHIS", "season_PIPPA"];
...
module.exports = class season {
    ...
    static get registeredAdapters() {
        return ["season_PHENOMIS", "season_PHIS", "season_PIPPA"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readById(id) {
        if (id !== null) {
            ...
            return adapters[responsibleAdapter[0]].readById(id).then(result => new season(result));
        }
    }
    ...
    static idAttribute() {
        let internalId = season.definition.id.name;
        return internalId;
    }
    ...
    static addOne(input) {
        ...
        let responsibleAdapter = this.adapterForIri(input.seasonDbId);
        return adapters[responsibleAdapter].addOne(input).then(result => new season(result));
    }
    ...
    static updateOne(input) {
        this.assertInputHasId(input);
        let responsibleAdapter = this.adapterForIri(input.seasonDbId);
        return adapters[responsibleAdapter].updateOne(input).then(result => new season(result));
    }
