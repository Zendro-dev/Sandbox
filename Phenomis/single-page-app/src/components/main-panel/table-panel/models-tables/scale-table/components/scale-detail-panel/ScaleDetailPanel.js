...
export default function ScaleDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.scale[String(itemState.scaleDbId)]) {
          
        //updated item
        if(lastModelChanged.scale[String(itemState.scaleDbId)].op === "update"&&
            lastModelChanged.scale[String(itemState.scaleDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.scale[String(itemState.scaleDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.scale[String(itemState.scaleDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.scaleDbId]);
