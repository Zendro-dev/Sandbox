  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.scale.getAssociatedObservationVariablesConnection(graphqlServerUrl, item.scaleDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneScale.observationVariablesConnection.edges.map(o => o.node);
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.scaleDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.scale&&
        ...
        lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.observationVariables&&
        (lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.observationVariables.added ||
         lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.observationVariables.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.observationVariables.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'scale' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationVariable',
     * but this action happened on the peer relation 'scale'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.scale) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.scale.removed) {
              let idsRemoved = entry[1].changedAssociations.scale.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.scale.added) {
              let idsAdded = entry[1].changedAssociations.scale.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.scaleDbId, handleUntransfer]);
