  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.scale.getAssociatedOntologyReferenceConnection(graphqlServerUrl, item.scaleDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneScale.ontologyReference;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.scale&&
        ...
        lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.ontologyReference&&
        (lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.ontologyReference.added ||
         lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.ontologyReference.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.ontologyReference.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'scales' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'ontologyReference',
     * but this action happened on the peer relation 'scales'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyReference) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.scales) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.scales.removed) {
              let idsRemoved = entry[1].changedAssociations.scales.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.scales.added) {
              let idsAdded = entry[1].changedAssociations.scales.idsAdded;
              if(idsAdded) {
