...
import StudyToContactEnhancedTableHead from './components/Study_to_contactEnhancedTableHead'
import StudyToContactEnhancedTableToolbar from './components/Study_to_contactEnhancedTableToolbar'
import StudyToContactCreatePanel from './components/study_to_contact-create-panel/Study_to_contactCreatePanel'
import StudyToContactUpdatePanel from './components/study_to_contact-update-panel/Study_to_contactUpdatePanel'
import StudyToContactDetailPanel from './components/study_to_contact-detail-panel/Study_to_contactDetailPanel'
import StudyToContactDeleteConfirmationDialog from './components/Study_to_contactDeleteConfirmationDialog'
import StudyToContactUploadFileDialog from './components/Study_to_contactUploadFileDialog'
import StudyToContactCursorPagination from './components/Study_to_contactCursorPagination'
...
export default function StudyToContactEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.study_to_contact.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if(
          ...
          //save response data
          var newCount = response.data.data.countStudy_to_contacts;
  ...
  function doDelete(event, item) {
    ...
    */
    let cancelableApiReq = makeCancelable(api.study_to_contact.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  function getCsvTemplate() {
    /*
      API Request: deleteItem
    */
    let cancelableApiReq = makeCancelable(api.study_to_contact.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
        ...
        if(
          response.data &&
          response.data.data &&
          response.data.data.csvTableTemplateStudy_to_contact        ) {
          //notify graphql errors
          if(response.data.errors) {
            actionText.current = t('modelPanels.gotIt', "Got it");
            enqueueSnackbar( t('modelPanels.errors.e3', "The GraphQL query returned a response with errors. Please contact your administrator."), {
            ...
            //download
            let file = response.data.data.csvTableTemplateStudy_to_contact.join("\n");
            const url = window.URL.createObjectURL(new Blob([file]));
            const link = document.createElement("a");
            link.href = url;
            link.setAttribute("download", "study_to_contact-template.csv");
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12} md={11}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <StudyToContactEnhancedTableToolbar
                ...
                <div className={classes.tableWrapper} ref={twref}>
                  ...
                  <Table stickyHeader size='small' ref={tref}>
                    ...
                    {/* Table Head */}
                    <StudyToContactEnhancedTableHead
                      permissions={permissions}
                      order={order}
                      orderBy={orderBy}
                      rowCount={count}
                                ...
                                */}
                                {
                                  /* acl check */
                                  (permissions&&permissions.study_to_contact&&Array.isArray(permissions.study_to_contact)
                                  &&(permissions.study_to_contact.includes('update') || permissions.study_to_contact.includes('*')))
                                  &&(
...

                                {
                                  /* acl check */
                                  (permissions&&permissions.study_to_contact&&Array.isArray(permissions.study_to_contact)
                                  &&(permissions.study_to_contact.includes('delete') || permissions.study_to_contact.includes('*')))
                                  &&(
                                    <TableCell padding='checkbox' align='center'>
                                ...
                                <TableCell
                                  ...
                                  padding="default"
                                >
                                  {String((item.contactDbId!==null)?item.contactDbId:'')}
                                </TableCell>
                ...
                {/*
                  Pagination
                */}
                <StudyToContactCursorPagination
                  count={count}
      ...
      {(deleteConfirmationDialogOpen) && (
        <StudyToContactDeleteConfirmationDialog
          permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <StudyToContactUploadFileDialog
...
StudyToContactEnhancedTable.propTypes = {
