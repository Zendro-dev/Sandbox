  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.study_to_contact.getAssociatedStudyConnection(graphqlServerUrl, item.id));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneStudy_to_contact.study;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study_to_contact&&
        ...
        lastModelChanged.study_to_contact[String(item.id)].changedAssociations.study&&
        (lastModelChanged.study_to_contact[String(item.id)].changedAssociations.study.added ||
         lastModelChanged.study_to_contact[String(item.id)].changedAssociations.study.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study_to_contact[String(item.id)].changedAssociations.study.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'studyToContacts' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'study',
     * but this action happened on the peer relation 'studyToContacts'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.studyToContacts) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.studyToContacts.removed) {
              let idsRemoved = entry[1].changedAssociations.studyToContacts.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.studyToContacts.added) {
              let idsAdded = entry[1].changedAssociations.studyToContacts.idsAdded;
              if(idsAdded) {
