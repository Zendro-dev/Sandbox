...
export default function StudyToContactCreatePanel(props) {
  ...
  const [contactIdsToAddState, setContactIdsToAddState] = useState([]);
  const contactIdsToAdd = useRef([]);
  ...
  function setAddContact(variables) {
  ...
  function doSave(event) {
    ...
    */
    let cancelableApiReq = makeCancelable(api.study_to_contact.createItem(graphqlServerUrl, variables));
      ...
      .then(response => {
        ...
        if (
          ...
          if(response.data.errors) {
            ...
            onClose(event, true, response.data.data.addStudy_to_contact);
          }
          return;
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'contact':
        if(contactIdsToAdd.current.indexOf(itemId) === -1) {
      ...
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Study_to_contact'}
          </Typography>
...
StudyToContactCreatePanel.propTypes = {
