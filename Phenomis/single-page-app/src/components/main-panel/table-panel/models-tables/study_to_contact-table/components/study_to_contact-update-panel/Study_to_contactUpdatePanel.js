...
import StudyToContactAttributesPage from './components/study_to_contact-attributes-page/Study_to_contactAttributesPage'
import StudyToContactAssociationsPage from './components/study_to_contact-associations-page/Study_to_contactAssociationsPage'
import StudyToContactTabsA from './components/Study_to_contactTabsA'
import StudyToContactConfirmationDialog from './components/Study_to_contactConfirmationDialog'
...
export default function StudyToContactUpdatePanel(props) {
  ...
  const contactIdsToAdd = useRef((item.contact&& item.contact.contactDbId) ? [item.contact.contactDbId] : []);
  const [contactIdsToAddState, setContactIdsToAddState] = useState((item.contact&& item.contact.contactDbId) ? [item.contact.contactDbId] : []);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged.study_to_contact&&
      lastModelChanged.study_to_contact[String(item.id)]) {

        //updated item
        if(lastModelChanged.study_to_contact[String(item.id)].op === "update"&&
            lastModelChanged.study_to_contact[String(item.id)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.study_to_contact[String(item.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function getInitialForeignKeys() {
    ...
    initialForeignKeys.contactDbId = item.contactDbId;
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.contactDbId = -2; //FK
  ...
  function setAddRemoveContact(variables) {
    //data to notify changes
    changedAssociations.current.contact = {added: false, removed: false};
    
    /*
     * Case I: Currently, this record is associated.
     */
    if(item.contact&&item.contact.contactDbId) {
      /*
       * Case I.a: The toAdd list isn't empty.
       */      
      if(contactIdsToAdd.current.length>0) {
        /*
         * Case I.a.1: There is a new ID (current association changed).
         */
        if(item.contact.contactDbId!== contactIdsToAdd.current[0]) {
          //set id to add
          variables.addContact = contactIdsToAdd.current[0];
          
          changedAssociations.current.contact.added = true;
          changedAssociations.current.contact.idsAdded = contactIdsToAdd.current;
          changedAssociations.current.contact.removed = true;
          changedAssociations.current.contact.idsRemoved = [item.contact.contactDbId];
        } else {
          /*
         ...
         */
        //set id to remove
        variables.removeContact = item.contact.contactDbId;
  ...
  function doSave(event) {
    ...
    */
    ...
    delete variables.contactDbId;

    //add & remove: to_one's
    setAddRemoveContact(variables);
    ...
    */
    let cancelableApiReq = makeCancelable(api.study_to_contact.updateItem(graphqlServerUrl, variables));
      ...
      .then(response => {
        ...
        if (
          ...
          if(response.data.errors) {
            ...
            onClose(event, true, response.data.data.updateStudy_to_contact);
          }
          return;
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'contact':
        contactIdsToAdd.current = [];
        contactIdsToAdd.current.push(itemId);
        setContactIdsToAddState(contactIdsToAdd.current);
        handleSetValue(itemId, 1, 'contactDbId');
        setForeignKeys({...foreignKeys, contactDbId: itemId});
        break;
      case 'study':
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'contact') {
      contactIdsToAdd.current = [];
      setContactIdsToAddState([]);
      handleSetValue(null, 0, 'contactDbId');
      setForeignKeys({...foreignKeys, contactDbId: null});
      return;
    }//end: case 'contact'
    ...
    >
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": Study_to_contact | id: " + item.id}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " study_to_contact" }>
              <Fab 
                color="secondary" 
      ...
      <div className={classes.root}>
        <Grid container justify='center' alignItems='flex-start' alignContent='flex-start'>
          ...
          <Grid item xs={12}>
            {/* Associations Page [1] */}
            <StudyToContactAssociationsPage
              hidden={tabsValue !== 1 || deleted}
              item={item}
              contactIdsToAdd={contactIdsToAddState}
        ...
        {/* Confirmation Dialog */}
        <StudyToContactConfirmationDialog
...
StudyToContactUpdatePanel.propTypes = {
