...
import StudyToContactUpdatePanel from '../study_to_contact-update-panel/Study_to_contactUpdatePanel'
import StudyToContactDeleteConfirmationDialog from '../Study_to_contactDeleteConfirmationDialog'
...
export default function StudyToContactDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.study_to_contact[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.study_to_contact[String(itemState.id)].op === "update"&&
            lastModelChanged.study_to_contact[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.study_to_contact[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.study_to_contact[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('study_to_contact', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('study_to_contact', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Study_to_contact | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
...
StudyToContactDetailPanel.propTypes = {
