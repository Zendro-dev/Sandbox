...
export default function EventParameterDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.eventParameter[String(itemState.eventParameterDbId)]) {
          
        //updated item
        if(lastModelChanged.eventParameter[String(itemState.eventParameterDbId)].op === "update"&&
            lastModelChanged.eventParameter[String(itemState.eventParameterDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.eventParameter[String(itemState.eventParameterDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.eventParameter[String(itemState.eventParameterDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.eventParameterDbId]);
