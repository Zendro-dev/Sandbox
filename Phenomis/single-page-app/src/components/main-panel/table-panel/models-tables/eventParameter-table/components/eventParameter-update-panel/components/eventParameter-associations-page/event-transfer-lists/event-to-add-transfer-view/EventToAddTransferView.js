  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated event ids
      */
      let cancelableApiReq = makeCancelable(api.eventParameter.getAssociatedEventConnection(graphqlServerUrl, item.eventParameterDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneEventParameter.event;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('eventType')) ? [idso.eventType] : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.event.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countEvents;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.event.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.eventsConnection.edges.map(o => o.node);
                        let pi = response.data.data.eventsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.event.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.event.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.eventParameterDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'events' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.eventParameter&&
        ...
        lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations.event&&
        (lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations.event.added ||
         lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations.event.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations.event.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'eventParameters' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'event',
     * but this action happened on the peer relation 'eventParameters'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.event) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.eventParameters) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.eventParameters.removed) {
              let idsRemoved = entry[1].changedAssociations.eventParameters.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.eventParameters.added) {
              let idsAdded = entry[1].changedAssociations.eventParameters.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.eventParameterDbId, handleUntransfer]);
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.eventType}</Typography>
                                  </Grid>
