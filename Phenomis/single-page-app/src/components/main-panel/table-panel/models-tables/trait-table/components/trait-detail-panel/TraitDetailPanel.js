...
export default function TraitDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.trait[String(itemState.traitDbId)]) {
          
        //updated item
        if(lastModelChanged.trait[String(itemState.traitDbId)].op === "update"&&
            lastModelChanged.trait[String(itemState.traitDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.trait[String(itemState.traitDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.trait[String(itemState.traitDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.traitDbId]);
