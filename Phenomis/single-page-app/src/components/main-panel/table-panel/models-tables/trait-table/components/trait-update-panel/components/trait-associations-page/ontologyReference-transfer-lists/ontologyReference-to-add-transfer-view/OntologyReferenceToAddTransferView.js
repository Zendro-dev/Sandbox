  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.trait.getAssociatedOntologyReferenceConnection(graphqlServerUrl, item.traitDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneTrait.ontologyReference;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trait&&
        ...
        lastModelChanged.trait[String(item.traitDbId)].changedAssociations.ontologyReference&&
        (lastModelChanged.trait[String(item.traitDbId)].changedAssociations.ontologyReference.added ||
         lastModelChanged.trait[String(item.traitDbId)].changedAssociations.ontologyReference.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.trait[String(item.traitDbId)].changedAssociations.ontologyReference.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'traits' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'ontologyReference',
     * but this action happened on the peer relation 'traits'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyReference) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.traits) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.traits.removed) {
              let idsRemoved = entry[1].changedAssociations.traits.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.traits.added) {
              let idsAdded = entry[1].changedAssociations.traits.idsAdded;
              if(idsAdded) {
