...
export default function TraitUpdatePanel(props) {
  ...
  function doSave(event) {
    ...
    */
    ...
    //data to notify changes
    changedAssociations.current.observationVariables = {added: false, removed: false};
    
    if(observationVariablesIdsToAdd.current.length>0) {
      variables.addObservationVariables = observationVariablesIdsToAdd.current;
      
      changedAssociations.current.observationVariables.added = true;
      changedAssociations.current.observationVariables.idsAdded = observationVariablesIdsToAdd.current;
    }
    if(observationVariablesIdsToRemove.current.length>0) {
      ...
      changedAssociations.current.observationVariables.removed = true;
      changedAssociations.current.observationVariables.idsRemoved = observationVariablesIdsToRemove.current;
    }
      ...
      .then(response => {
        ...
        if (
          ...
          if(response.data.errors) {
            ...
            onClose(event, true, response.data.data.updateTrait);
          }
          return;
