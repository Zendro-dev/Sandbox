  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.trait.getAssociatedObservationVariablesConnection(graphqlServerUrl, item.traitDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneTrait.observationVariablesConnection.edges.map(o => o.node);
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.traitDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trait&&
        ...
        lastModelChanged.trait[String(item.traitDbId)].changedAssociations.observationVariables&&
        (lastModelChanged.trait[String(item.traitDbId)].changedAssociations.observationVariables.added ||
         lastModelChanged.trait[String(item.traitDbId)].changedAssociations.observationVariables.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.trait[String(item.traitDbId)].changedAssociations.observationVariables.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'trait' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationVariable',
     * but this action happened on the peer relation 'trait'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.trait) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.trait.removed) {
              let idsRemoved = entry[1].changedAssociations.trait.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.trait.added) {
              let idsAdded = entry[1].changedAssociations.trait.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.traitDbId, handleUntransfer]);
