  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.program.getObservationUnitsConnection(
      graphqlServerUrl, 
      item.programDbId,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if (
          ...
          //save response data
          let newCount = response.data.data.readOneProgram.countFilteredObservationUnits;
          let its = response.data.data.readOneProgram.observationUnitsConnection.edges.map(o => o.node);
          let pi = response.data.data.readOneProgram.observationUnitsConnection.pageInfo;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.program&&
        ...
        lastModelChanged.program[String(item.programDbId)].changedAssociations.observationUnits&&
        (lastModelChanged.program[String(item.programDbId)].changedAssociations.observationUnits.added ||
         lastModelChanged.program[String(item.programDbId)].changedAssociations.observationUnits.removed)) {
          
          //remove any dis-associated id from idsToRemove[]
          let idsRemoved = lastModelChanged.program[String(item.programDbId)].changedAssociations.observationUnits.idsRemoved;
          if(idsRemoved) {
