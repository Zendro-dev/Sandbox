...
export default function ProgramDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.program[String(itemState.programDbId)]) {
          
        //updated item
        if(lastModelChanged.program[String(itemState.programDbId)].op === "update"&&
            lastModelChanged.program[String(itemState.programDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.program[String(itemState.programDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.program[String(itemState.programDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.programDbId]);
