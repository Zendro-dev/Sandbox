  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated trials ids
      */
      let cancelableApiReq = makeCancelable(api.program.getAssociatedTrialsConnection(graphqlServerUrl, item.programDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneProgram.trialsConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.trialDbId}) : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.trial.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countTrials;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.trial.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.trial.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.programDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'trials' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.program&&
        ...
        lastModelChanged.program[String(item.programDbId)].changedAssociations.trials&&
        (lastModelChanged.program[String(item.programDbId)].changedAssociations.trials.added ||
         lastModelChanged.program[String(item.programDbId)].changedAssociations.trials.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.program[String(item.programDbId)].changedAssociations.trials.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'program' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'trial',
     * but this action happened on the peer relation 'program'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.trial) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.program) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.program.removed) {
              let idsRemoved = entry[1].changedAssociations.program.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.program.added) {
              let idsAdded = entry[1].changedAssociations.program.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.programDbId, handleUntransfer]);
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.trialDbId}</Typography>
                                  </Grid>
