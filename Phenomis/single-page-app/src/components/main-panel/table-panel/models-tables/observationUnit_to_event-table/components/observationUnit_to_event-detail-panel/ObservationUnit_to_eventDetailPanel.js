...
import ObservationUnitToEventUpdatePanel from '../observationUnit_to_event-update-panel/ObservationUnit_to_eventUpdatePanel'
import ObservationUnitToEventDeleteConfirmationDialog from '../ObservationUnit_to_eventDeleteConfirmationDialog'
...
export default function ObservationUnitToEventDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.observationUnit_to_event[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.observationUnit_to_event[String(itemState.id)].op === "update"&&
            lastModelChanged.observationUnit_to_event[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.observationUnit_to_event[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.observationUnit_to_event[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('observationUnit_to_event', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('observationUnit_to_event', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": ObservationUnit_to_event | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
...
ObservationUnitToEventDetailPanel.propTypes = {
