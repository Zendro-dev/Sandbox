...
import ObservationUnitToEventEnhancedTableHead from './components/ObservationUnit_to_eventEnhancedTableHead'
import ObservationUnitToEventEnhancedTableToolbar from './components/ObservationUnit_to_eventEnhancedTableToolbar'
import ObservationUnitToEventCreatePanel from './components/observationUnit_to_event-create-panel/ObservationUnit_to_eventCreatePanel'
import ObservationUnitToEventUpdatePanel from './components/observationUnit_to_event-update-panel/ObservationUnit_to_eventUpdatePanel'
import ObservationUnitToEventDetailPanel from './components/observationUnit_to_event-detail-panel/ObservationUnit_to_eventDetailPanel'
import ObservationUnitToEventDeleteConfirmationDialog from './components/ObservationUnit_to_eventDeleteConfirmationDialog'
import ObservationUnitToEventUploadFileDialog from './components/ObservationUnit_to_eventUploadFileDialog'
import ObservationUnitToEventCursorPagination from './components/ObservationUnit_to_eventCursorPagination'
...
export default function ObservationUnitToEventEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.observationUnit_to_event.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if(
          ...
          //save response data
          var newCount = response.data.data.countObservationUnit_to_events;
  ...
  function doDelete(event, item) {
    ...
    */
    let cancelableApiReq = makeCancelable(api.observationUnit_to_event.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  function getCsvTemplate() {
    /*
      API Request: deleteItem
    */
    let cancelableApiReq = makeCancelable(api.observationUnit_to_event.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
        ...
        if(
          response.data &&
          response.data.data &&
          response.data.data.csvTableTemplateObservationUnit_to_event        ) {
          //notify graphql errors
          if(response.data.errors) {
            actionText.current = t('modelPanels.gotIt', "Got it");
            enqueueSnackbar( t('modelPanels.errors.e3', "The GraphQL query returned a response with errors. Please contact your administrator."), {
            ...
            //download
            let file = response.data.data.csvTableTemplateObservationUnit_to_event.join("\n");
            const url = window.URL.createObjectURL(new Blob([file]));
            const link = document.createElement("a");
            link.href = url;
            link.setAttribute("download", "observationUnit_to_event-template.csv");
...
function resetReloadData() {
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12} md={11}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <ObservationUnitToEventEnhancedTableToolbar
                ...
                <div className={classes.tableWrapper} ref={twref}>
                  ...
                  <Table stickyHeader size='small' ref={tref}>
                    ...
                    {/* Table Head */}
                    <ObservationUnitToEventEnhancedTableHead
                      permissions={permissions}
                      order={order}
                      orderBy={orderBy}
                      rowCount={count}
                                ...
                                */}
                                {
                                  /* acl check */
                                  (permissions&&permissions.observationUnit_to_event&&Array.isArray(permissions.observationUnit_to_event)
                                  &&(permissions.observationUnit_to_event.includes('update') || permissions.observationUnit_to_event.includes('*')))
                                  &&(
...

                                {
                                  /* acl check */
                                  (permissions&&permissions.observationUnit_to_event&&Array.isArray(permissions.observationUnit_to_event)
                                  &&(permissions.observationUnit_to_event.includes('delete') || permissions.observationUnit_to_event.includes('*')))
                                  &&(
                                    <TableCell padding='checkbox' align='center'>
                ...
                {/*
                  Pagination
                */}
                <ObservationUnitToEventCursorPagination
                  count={count}
      ...
      {/* Dialog: Create Panel */}
      {(createDialogOpen) && (
        <ObservationUnitToEventCreatePanel
          permissions={permissions}
      ...
      {/* Dialog: Update Panel */}
      {(updateDialogOpen) && (
        <ObservationUnitToEventUpdatePanel
          permissions={permissions}
          item={updateItem}
      ...
      {(deleteConfirmationDialogOpen) && (
        <ObservationUnitToEventDeleteConfirmationDialog
          permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <ObservationUnitToEventUploadFileDialog
...
ObservationUnitToEventEnhancedTable.propTypes = {
