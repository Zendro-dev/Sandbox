  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.observationUnit_to_event.getAssociatedObservationUnitConnection(graphqlServerUrl, item.id));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationUnit_to_event.observationUnit;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationUnit_to_event&&
        ...
        lastModelChanged.observationUnit_to_event[String(item.id)].changedAssociations.observationUnit&&
        (lastModelChanged.observationUnit_to_event[String(item.id)].changedAssociations.observationUnit.added ||
         lastModelChanged.observationUnit_to_event[String(item.id)].changedAssociations.observationUnit.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit_to_event[String(item.id)].changedAssociations.observationUnit.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'observationUnitToEvents' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationUnit',
     * but this action happened on the peer relation 'observationUnitToEvents'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationUnit) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnitToEvents) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnitToEvents.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnitToEvents.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnitToEvents.added) {
              let idsAdded = entry[1].changedAssociations.observationUnitToEvents.idsAdded;
              if(idsAdded) {
