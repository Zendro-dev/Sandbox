  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.observationUnit_to_event.getAssociatedEventConnection(graphqlServerUrl, item.id));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationUnit_to_event.event;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationUnit_to_event&&
        ...
        lastModelChanged.observationUnit_to_event[String(item.id)].changedAssociations&&
        lastModelChanged.observationUnit_to_event[String(item.id)].changedAssociations.event&&
        (lastModelChanged.observationUnit_to_event[String(item.id)].changedAssociations.event.added ||
         lastModelChanged.observationUnit_to_event[String(item.id)].changedAssociations.event.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit_to_event[String(item.id)].changedAssociations.event.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'eventToObservationUnits' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'event',
     * but this action happened on the peer relation 'eventToObservationUnits'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.event) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.eventToObservationUnits) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.eventToObservationUnits.removed) {
              let idsRemoved = entry[1].changedAssociations.eventToObservationUnits.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.eventToObservationUnits.added) {
              let idsAdded = entry[1].changedAssociations.eventToObservationUnits.idsAdded;
              if(idsAdded) {
