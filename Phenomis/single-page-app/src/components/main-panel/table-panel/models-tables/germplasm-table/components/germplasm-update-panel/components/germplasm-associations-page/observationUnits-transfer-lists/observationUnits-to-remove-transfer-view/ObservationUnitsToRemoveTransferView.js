  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.germplasm.getObservationUnitsConnection(
      graphqlServerUrl, 
      item.germplasmDbId,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if (
          ...
          //save response data
          let newCount = response.data.data.readOneGermplasm.countFilteredObservationUnits;
          let its = response.data.data.readOneGermplasm.observationUnitsConnection.edges.map(o => o.node);
          let pi = response.data.data.readOneGermplasm.observationUnitsConnection.pageInfo;
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'observationUnits' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.germplasm&&
        ...
        lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observationUnits&&
        (lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observationUnits.added ||
         lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observationUnits.removed)) {
          
          //remove any dis-associated id from idsToRemove[]
          let idsRemoved = lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observationUnits.idsRemoved;
          if(idsRemoved) {
