  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated observationUnits ids
      */
      let cancelableApiReq = makeCancelable(api.germplasm.getAssociatedObservationUnitsConnection(graphqlServerUrl, item.germplasmDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneGermplasm.observationUnitsConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.observationUnitDbId}) : [];
            ...
            if(exIds.length > 0) {
              ops = {
                exclude: [{
                  type: 'String',
                  values: {observationUnitDbId: exIds}
                }]
              };
            }
            ...
            */
            let cancelableApiReqB = makeCancelable(api.observationUnit.getCountItems(graphqlServerUrl, search, ops));
              ...
              .then(response => {
                ...
                if (
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.observationUnit.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
