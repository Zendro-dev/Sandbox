  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated breedingMethod ids
      */
      let cancelableApiReq = makeCancelable(api.germplasm.getAssociatedBreedingMethodConnection(graphqlServerUrl, item.germplasmDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneGermplasm.breedingMethod;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('breedingMethodDbId')) ? [idso.breedingMethodDbId] : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.breedingMethod.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countBreedingMethods;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.breedingMethod.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.breedingMethodsConnection.edges.map(o => o.node);
                        let pi = response.data.data.breedingMethodsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.breedingMethod.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.breedingMethod.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.germplasmDbId, search, rowsPerPage]);
  ...
  const getDataB = useCallback(() => {
    ...
    /*
      API Request: countItems
    */
    let cancelableApiReq = makeCancelable(api.breedingMethod.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'breedingMethods' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.germplasm&&
        ...
        lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.breedingMethod&&
        (lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.breedingMethod.added ||
         lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.breedingMethod.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.breedingMethod.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'germplasm' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'breedingMethod',
     * but this action happened on the peer relation 'germplasm'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.breedingMethod) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.germplasm) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.germplasm.removed) {
              let idsRemoved = entry[1].changedAssociations.germplasm.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.germplasm.added) {
              let idsAdded = entry[1].changedAssociations.germplasm.idsAdded;
              if(idsAdded) {
    ...
    if(lastModelChanged.breedingMethod) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          let iofB = itemsB.findIndex((item) => item.breedingMethodDbId===idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.germplasmDbId, handleUntransfer]);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.breedingMethodDbId) === -1) {
      lidsToAdd.current = [];
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.breedingMethodDbId}</Typography>
                                  </Grid>
