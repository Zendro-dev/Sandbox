  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.germplasm.getAssociatedObservationsConnection(graphqlServerUrl, item.germplasmDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneGermplasm.observationsConnection.edges.map(o => o.node);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.germplasm&&
        ...
        lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observations&&
        (lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observations.added ||
         lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observations.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observations.idsRemoved;
          if(idsRemoved) {
