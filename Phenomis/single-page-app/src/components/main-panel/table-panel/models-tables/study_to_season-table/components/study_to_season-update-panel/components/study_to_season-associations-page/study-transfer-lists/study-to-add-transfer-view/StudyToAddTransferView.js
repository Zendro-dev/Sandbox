  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.study_to_season.getAssociatedStudyConnection(graphqlServerUrl, item.id));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneStudy_to_season.study;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study_to_season&&
        ...
        lastModelChanged.study_to_season[String(item.id)].changedAssociations.study&&
        (lastModelChanged.study_to_season[String(item.id)].changedAssociations.study.added ||
         lastModelChanged.study_to_season[String(item.id)].changedAssociations.study.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study_to_season[String(item.id)].changedAssociations.study.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'studyToSeasons' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'study',
     * but this action happened on the peer relation 'studyToSeasons'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.studyToSeasons) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.studyToSeasons.removed) {
              let idsRemoved = entry[1].changedAssociations.studyToSeasons.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.studyToSeasons.added) {
              let idsAdded = entry[1].changedAssociations.studyToSeasons.idsAdded;
              if(idsAdded) {
