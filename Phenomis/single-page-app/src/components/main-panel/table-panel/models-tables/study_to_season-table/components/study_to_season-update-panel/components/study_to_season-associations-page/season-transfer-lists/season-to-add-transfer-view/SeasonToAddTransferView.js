  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.study_to_season.getAssociatedSeasonConnection(graphqlServerUrl, item.id));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneStudy_to_season.season;
  ...
  useEffect(() => {
     ...
     * - return
     */
    if(lastModelChanged.study_to_season&&
        lastModelChanged.study_to_season[String(item.id)]&&
        lastModelChanged.study_to_season[String(item.id)].changedAssociations&&
        lastModelChanged.study_to_season[String(item.id)].changedAssociations.season&&
        (lastModelChanged.study_to_season[String(item.id)].changedAssociations.season.added ||
         lastModelChanged.study_to_season[String(item.id)].changedAssociations.season.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study_to_season[String(item.id)].changedAssociations.season.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'seasonToStudies' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'season',
     * but this action happened on the peer relation 'seasonToStudies'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.season) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.seasonToStudies) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.seasonToStudies.removed) {
              let idsRemoved = entry[1].changedAssociations.seasonToStudies.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.seasonToStudies.added) {
              let idsAdded = entry[1].changedAssociations.seasonToStudies.idsAdded;
              if(idsAdded) {
