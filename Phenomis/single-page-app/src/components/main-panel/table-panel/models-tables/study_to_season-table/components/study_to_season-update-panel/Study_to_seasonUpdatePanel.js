...
import StudyToSeasonTabsA from './components/Study_to_seasonTabsA'
import StudyToSeasonConfirmationDialog from './components/Study_to_seasonConfirmationDialog'
...
export default function StudyToSeasonUpdatePanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.study_to_season[String(item.id)]) {
        ...
        //updated item
        if(lastModelChanged.study_to_season[String(item.id)].op === "update"&&
            lastModelChanged.study_to_season[String(item.id)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.study_to_season[String(item.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  function doSave(event) {
    ...
    */
    let cancelableApiReq = makeCancelable(api.study_to_season.updateItem(graphqlServerUrl, variables));
      ...
      .then(response => {
        ...
        if (
          ...
          if(response.data.errors) {
            ...
            onClose(event, true, response.data.data.updateStudy_to_season);
          }
          return;
  ...
  return (
    ...
    >
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": Study_to_season | id: " + item.id}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " study_to_season" }>
              <Fab 
                color="secondary" 
      ...
      <div className={classes.root}>
        ...
        {/* Confirmation Dialog */}
        <StudyToSeasonConfirmationDialog
...
StudyToSeasonUpdatePanel.propTypes = {
