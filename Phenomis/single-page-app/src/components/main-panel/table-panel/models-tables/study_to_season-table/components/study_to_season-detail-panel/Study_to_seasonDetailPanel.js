...
import StudyToSeasonAttributesPage from './components/study_to_season-attributes-page/Study_to_seasonAttributesPage'
import StudyToSeasonAssociationsPage from './components/study_to_season-associations-page/Study_to_seasonAssociationsPage'
import StudyToSeasonUpdatePanel from '../study_to_season-update-panel/Study_to_seasonUpdatePanel'
import StudyToSeasonDeleteConfirmationDialog from '../Study_to_seasonDeleteConfirmationDialog'
...
export default function StudyToSeasonDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged.study_to_season&&
      lastModelChanged.study_to_season[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.study_to_season[String(itemState.id)].op === "update"&&
            lastModelChanged.study_to_season[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.study_to_season[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.study_to_season[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('study_to_season', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('study_to_season', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Study_to_season | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
...
StudyToSeasonDetailPanel.propTypes = {
