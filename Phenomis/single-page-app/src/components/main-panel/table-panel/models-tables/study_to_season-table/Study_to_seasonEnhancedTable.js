...
import StudyToSeasonEnhancedTableHead from './components/Study_to_seasonEnhancedTableHead'
import StudyToSeasonEnhancedTableToolbar from './components/Study_to_seasonEnhancedTableToolbar'
import StudyToSeasonCreatePanel from './components/study_to_season-create-panel/Study_to_seasonCreatePanel'
import StudyToSeasonUpdatePanel from './components/study_to_season-update-panel/Study_to_seasonUpdatePanel'
import StudyToSeasonDetailPanel from './components/study_to_season-detail-panel/Study_to_seasonDetailPanel'
import StudyToSeasonDeleteConfirmationDialog from './components/Study_to_seasonDeleteConfirmationDialog'
import StudyToSeasonUploadFileDialog from './components/Study_to_seasonUploadFileDialog'
import StudyToSeasonCursorPagination from './components/Study_to_seasonCursorPagination'
...
export default function StudyToSeasonEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.study_to_season.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if(
          ...
          //save response data
          var newCount = response.data.data.countStudy_to_seasons;
  ...
  function doDelete(event, item) {
    ...
    */
    let cancelableApiReq = makeCancelable(api.study_to_season.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  function getCsvTemplate() {
    /*
      API Request: deleteItem
    */
    let cancelableApiReq = makeCancelable(api.study_to_season.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
        ...
        if(
          response.data &&
          response.data.data &&
          response.data.data.csvTableTemplateStudy_to_season        ) {
          //notify graphql errors
          if(response.data.errors) {
            actionText.current = t('modelPanels.gotIt', "Got it");
            enqueueSnackbar( t('modelPanels.errors.e3', "The GraphQL query returned a response with errors. Please contact your administrator."), {
            ...
            //download
            let file = response.data.data.csvTableTemplateStudy_to_season.join("\n");
            const url = window.URL.createObjectURL(new Blob([file]));
            const link = document.createElement("a");
            link.href = url;
            link.setAttribute("download", "study_to_season-template.csv");
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12} md={11}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <StudyToSeasonEnhancedTableToolbar
                ...
                <div className={classes.tableWrapper} ref={twref}>
                  ...
                  <Table stickyHeader size='small' ref={tref}>
                    ...
                    {/* Table Head */}
                    <StudyToSeasonEnhancedTableHead
                      permissions={permissions}
                      order={order}
                      orderBy={orderBy}
                      rowCount={count}
                                ...
                                */}
                                {
                                  /* acl check */
                                  (permissions&&permissions.study_to_season&&Array.isArray(permissions.study_to_season)
                                  &&(permissions.study_to_season.includes('update') || permissions.study_to_season.includes('*')))
                                  &&(
...

                                {
                                  /* acl check */
                                  (permissions&&permissions.study_to_season&&Array.isArray(permissions.study_to_season)
                                  &&(permissions.study_to_season.includes('delete') || permissions.study_to_season.includes('*')))
                                  &&(
                                    <TableCell padding='checkbox' align='center'>
      ...
      {(deleteConfirmationDialogOpen) && (
        <StudyToSeasonDeleteConfirmationDialog
          permissions={permissions}
...
StudyToSeasonEnhancedTable.propTypes = {
