  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated observationUnit ids
      */
      let cancelableApiReq = makeCancelable(api.observationUnitPosition.getAssociatedObservationUnitConnection(graphqlServerUrl, item.observationUnitPositionDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationUnitPosition.observationUnit;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('observationUnitDbId')) ? [idso.observationUnitDbId] : [];
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.observationUnitPositionDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationUnitPosition&&
        ...
        lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations.observationUnit&&
        (lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations.observationUnit.added ||
         lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations.observationUnit.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations.observationUnit.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'observationUnitPosition' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationUnit',
     * but this action happened on the peer relation 'observationUnitPosition'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationUnit) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnitPosition) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnitPosition.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnitPosition.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnitPosition.added) {
              let idsAdded = entry[1].changedAssociations.observationUnitPosition.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationUnitPositionDbId, handleUntransfer]);
