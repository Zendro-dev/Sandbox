  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.image.getAssociatedObservationsConnection(graphqlServerUrl, item.imageDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneImage.observationsConnection.edges.map(o => o.node);
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.imageDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.image&&
        ...
        lastModelChanged.image[String(item.imageDbId)].changedAssociations.observations&&
        (lastModelChanged.image[String(item.imageDbId)].changedAssociations.observations.added ||
         lastModelChanged.image[String(item.imageDbId)].changedAssociations.observations.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.image[String(item.imageDbId)].changedAssociations.observations.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'image' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observation',
     * but this action happened on the peer relation 'image'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.image) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.image.removed) {
              let idsRemoved = entry[1].changedAssociations.image.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.image.added) {
              let idsAdded = entry[1].changedAssociations.image.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.imageDbId, handleUntransfer]);
