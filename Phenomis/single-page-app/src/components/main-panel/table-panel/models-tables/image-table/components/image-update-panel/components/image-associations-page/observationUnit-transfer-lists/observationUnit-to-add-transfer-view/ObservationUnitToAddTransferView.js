  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.image.getAssociatedObservationUnitConnection(graphqlServerUrl, item.imageDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneImage.observationUnit;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.image&&
        ...
        lastModelChanged.image[String(item.imageDbId)].changedAssociations.observationUnit&&
        (lastModelChanged.image[String(item.imageDbId)].changedAssociations.observationUnit.added ||
         lastModelChanged.image[String(item.imageDbId)].changedAssociations.observationUnit.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.image[String(item.imageDbId)].changedAssociations.observationUnit.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'images' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationUnit',
     * but this action happened on the peer relation 'images'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationUnit) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.images) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.images.removed) {
              let idsRemoved = entry[1].changedAssociations.images.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.images.added) {
              let idsAdded = entry[1].changedAssociations.images.idsAdded;
              if(idsAdded) {
