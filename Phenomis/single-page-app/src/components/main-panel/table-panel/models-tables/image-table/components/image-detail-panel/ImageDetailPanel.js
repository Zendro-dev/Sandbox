...
import ImageUpdatePanel from '../image-update-panel/ImageUpdatePanel'
...
export default function ImageDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.image[String(itemState.imageDbId)]) {
          
        //updated item
        if(lastModelChanged.image[String(itemState.imageDbId)].op === "update"&&
            lastModelChanged.image[String(itemState.imageDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.image[String(itemState.imageDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.image[String(itemState.imageDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.imageDbId]);
