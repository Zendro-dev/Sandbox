  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated germplasm ids
      */
      let cancelableApiReq = makeCancelable(api.breedingMethod.getAssociatedGermplasmConnection(graphqlServerUrl, item.breedingMethodDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneBreedingMethod.germplasmConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.germplasmDbId}) : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.germplasm.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countGermplasms;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.germplasm.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.germplasmsConnection.edges.map(o => o.node);
                        let pi = response.data.data.germplasmsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.germplasm.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.germplasm.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.breedingMethodDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'germplasms' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.breedingMethod&&
        ...
        lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations.germplasm&&
        (lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations.germplasm.added ||
         lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations.germplasm.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations.germplasm.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'breedingMethod' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'germplasm',
     * but this action happened on the peer relation 'breedingMethod'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.germplasm) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.breedingMethod) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.breedingMethod.removed) {
              let idsRemoved = entry[1].changedAssociations.breedingMethod.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.breedingMethod.added) {
              let idsAdded = entry[1].changedAssociations.breedingMethod.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.breedingMethodDbId, handleUntransfer]);
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.germplasmDbId}</Typography>
                                  </Grid>
