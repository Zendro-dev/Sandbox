...
import BreedingMethodUpdatePanel from '../breedingMethod-update-panel/BreedingMethodUpdatePanel'
import BreedingMethodDeleteConfirmationDialog from '../BreedingMethodDeleteConfirmationDialog'
...
export default function BreedingMethodDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.breedingMethod[String(itemState.breedingMethodDbId)]) {
          
        //updated item
        if(lastModelChanged.breedingMethod[String(itemState.breedingMethodDbId)].op === "update"&&
            lastModelChanged.breedingMethod[String(itemState.breedingMethodDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.breedingMethod[String(itemState.breedingMethodDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.breedingMethod[String(itemState.breedingMethodDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.breedingMethodDbId]);
