  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.study.getAssociatedTrialConnection(graphqlServerUrl, item.studyDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneStudy.trial;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.trial&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.trial.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.trial.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.trial.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'studies' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'trial',
     * but this action happened on the peer relation 'studies'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.trial) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.studies) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.studies.removed) {
              let idsRemoved = entry[1].changedAssociations.studies.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.studies.added) {
              let idsAdded = entry[1].changedAssociations.studies.idsAdded;
              if(idsAdded) {
