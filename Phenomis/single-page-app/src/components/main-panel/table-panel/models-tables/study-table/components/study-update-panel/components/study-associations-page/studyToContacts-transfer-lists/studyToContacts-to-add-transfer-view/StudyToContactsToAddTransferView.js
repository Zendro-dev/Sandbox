...
import StudyToContactsToAddTransferViewToolbar from './components/StudyToContactsToAddTransferViewToolbar';
import StudyToContactsToAddTransferViewCursorPagination from './components/StudyToContactsToAddTransferViewCursorPagination';
...
export default function StudyToContactsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated studyToContacts ids
      */
      let cancelableApiReq = makeCancelable(api.study.getAssociatedStudyToContactsConnection(graphqlServerUrl, item.studyDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneStudy.studyToContactsConnection.edges.map(o => o.node);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'studyToContacts' for this item was updated.
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.studyToContacts&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.studyToContacts.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.studyToContacts.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.studyToContacts.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('studyToContacts', idRemoved);
            });
          }
            ...
            if(entry[1].changedAssociations.study.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('studyToContacts', idAdded);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('studyToContacts', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('studyToContacts', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <StudyToContactsToAddTransferViewToolbar
              ...
              {/* Pagination */}
              <StudyToContactsToAddTransferViewCursorPagination
                count={count}
...
StudyToContactsToAddTransferView.propTypes = {
