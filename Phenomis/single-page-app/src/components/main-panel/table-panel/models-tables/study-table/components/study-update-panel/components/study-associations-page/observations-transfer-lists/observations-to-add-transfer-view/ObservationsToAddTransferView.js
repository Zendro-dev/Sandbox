  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.study.getAssociatedObservationsConnection(graphqlServerUrl, item.studyDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneStudy.observationsConnection.edges.map(o => o.node);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.observations&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.observations.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.observations.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.observations.idsRemoved;
          if(idsRemoved) {
