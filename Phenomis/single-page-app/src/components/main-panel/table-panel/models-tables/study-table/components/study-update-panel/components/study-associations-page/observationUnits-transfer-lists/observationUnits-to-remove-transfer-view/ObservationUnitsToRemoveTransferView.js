  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.study.getObservationUnitsConnection(
      graphqlServerUrl, 
      item.studyDbId,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if (
          ...
          //save response data
          let newCount = response.data.data.readOneStudy.countFilteredObservationUnits;
          let its = response.data.data.readOneStudy.observationUnitsConnection.edges.map(o => o.node);
          let pi = response.data.data.readOneStudy.observationUnitsConnection.pageInfo;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.observationUnits&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.observationUnits.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.observationUnits.removed)) {
          
          //remove any dis-associated id from idsToRemove[]
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.observationUnits.idsRemoved;
          if(idsRemoved) {
