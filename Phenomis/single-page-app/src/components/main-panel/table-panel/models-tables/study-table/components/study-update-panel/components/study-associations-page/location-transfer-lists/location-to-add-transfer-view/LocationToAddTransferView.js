  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.study.getAssociatedLocationConnection(graphqlServerUrl, item.studyDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneStudy.location;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.location&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.location.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.location.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.location.idsRemoved;
          if(idsRemoved) {
