  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated environmentParameters ids
      */
      let cancelableApiReq = makeCancelable(api.study.getAssociatedEnvironmentParametersConnection(graphqlServerUrl, item.studyDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneStudy.environmentParametersConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.environmentParameterDbId}) : [];
            ...
            if(exIds.length > 0) {
              ops = {
                exclude: [{
                  type: 'String',
                  values: {environmentParameterDbId: exIds}
                }]
              };
            }
            ...
            */
            let cancelableApiReqB = makeCancelable(api.environmentParameter.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countEnvironmentParameters;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.environmentParameter.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.environmentParametersConnection.edges.map(o => o.node);
                        let pi = response.data.data.environmentParametersConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.environmentParameter.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.environmentParameter.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.studyDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'environmentParameters' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.environmentParameters&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.environmentParameters.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.environmentParameters.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.environmentParameters.idsRemoved;
          if(idsRemoved) {
    ...
    if(lastModelChanged.environmentParameter) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study.removed) {
              let idsRemoved = entry[1].changedAssociations.study.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study.added) {
              let idsAdded = entry[1].changedAssociations.study.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.studyDbId, handleUntransfer]);
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.environmentParameterDbId}</Typography>
                                  </Grid>
