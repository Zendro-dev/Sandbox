...
import StudyToSeasonsToAddTransferViewToolbar from './components/StudyToSeasonsToAddTransferViewToolbar';
import StudyToSeasonsToAddTransferViewCursorPagination from './components/StudyToSeasonsToAddTransferViewCursorPagination';
...
export default function StudyToSeasonsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated studyToSeasons ids
      */
      let cancelableApiReq = makeCancelable(api.study.getAssociatedStudyToSeasonsConnection(graphqlServerUrl, item.studyDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneStudy.studyToSeasonsConnection.edges.map(o => o.node);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'studyToSeasons' for this item was updated.
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.studyToSeasons&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.studyToSeasons.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.studyToSeasons.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.studyToSeasons.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('studyToSeasons', idRemoved);
            });
          }
            ...
            if(entry[1].changedAssociations.study.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('studyToSeasons', idAdded);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('studyToSeasons', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('studyToSeasons', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <StudyToSeasonsToAddTransferViewToolbar
              ...
              {/* Pagination */}
              <StudyToSeasonsToAddTransferViewCursorPagination
                count={count}
...
StudyToSeasonsToAddTransferView.propTypes = {
