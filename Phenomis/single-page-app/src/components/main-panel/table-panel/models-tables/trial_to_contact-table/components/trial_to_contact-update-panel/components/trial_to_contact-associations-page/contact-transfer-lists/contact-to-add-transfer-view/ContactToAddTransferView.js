  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.trial_to_contact.getAssociatedContactConnection(graphqlServerUrl, item.id));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneTrial_to_contact.contact;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trial_to_contact&&
        ...
        lastModelChanged.trial_to_contact[String(item.id)].changedAssociations.contact&&
        (lastModelChanged.trial_to_contact[String(item.id)].changedAssociations.contact.added ||
         lastModelChanged.trial_to_contact[String(item.id)].changedAssociations.contact.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.trial_to_contact[String(item.id)].changedAssociations.contact.idsRemoved;
          if(idsRemoved) {
