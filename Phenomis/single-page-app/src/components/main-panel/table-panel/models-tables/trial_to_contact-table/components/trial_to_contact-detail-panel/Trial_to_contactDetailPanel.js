...
import TrialToContactUpdatePanel from '../trial_to_contact-update-panel/Trial_to_contactUpdatePanel'
import TrialToContactDeleteConfirmationDialog from '../Trial_to_contactDeleteConfirmationDialog'
...
export default function TrialToContactDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged.trial_to_contact&&
      lastModelChanged.trial_to_contact[String(itemState.id)]) {
          
        //updated item
        if(lastModelChanged.trial_to_contact[String(itemState.id)].op === "update"&&
            lastModelChanged.trial_to_contact[String(itemState.id)].newItem) {
              //replace item
              setItemState(lastModelChanged.trial_to_contact[String(itemState.id)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.trial_to_contact[String(itemState.id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('trial_to_contact', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('trial_to_contact', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Trial_to_contact | id: " + itemState.id}
              </Typography>
              {/*
                Actions:
                - Edit
...
TrialToContactDetailPanel.propTypes = {
