...
import TrialToContactEnhancedTableHead from './components/Trial_to_contactEnhancedTableHead'
import TrialToContactEnhancedTableToolbar from './components/Trial_to_contactEnhancedTableToolbar'
import TrialToContactCreatePanel from './components/trial_to_contact-create-panel/Trial_to_contactCreatePanel'
import TrialToContactUpdatePanel from './components/trial_to_contact-update-panel/Trial_to_contactUpdatePanel'
import TrialToContactDetailPanel from './components/trial_to_contact-detail-panel/Trial_to_contactDetailPanel'
import TrialToContactDeleteConfirmationDialog from './components/Trial_to_contactDeleteConfirmationDialog'
import TrialToContactUploadFileDialog from './components/Trial_to_contactUploadFileDialog'
import TrialToContactCursorPagination from './components/Trial_to_contactCursorPagination'
...
export default function TrialToContactEnhancedTable(props) {
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.trial_to_contact.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if(
          ...
          //save response data
          var newCount = response.data.data.countTrial_to_contacts;
  ...
  function doDelete(event, item) {
    ...
    */
    let cancelableApiReq = makeCancelable(api.trial_to_contact.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  function getCsvTemplate() {
    /*
      API Request: deleteItem
    */
    let cancelableApiReq = makeCancelable(api.trial_to_contact.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
        ...
        if(
          response.data &&
          response.data.data &&
          response.data.data.csvTableTemplateTrial_to_contact        ) {
          //notify graphql errors
          if(response.data.errors) {
            actionText.current = t('modelPanels.gotIt', "Got it");
            enqueueSnackbar( t('modelPanels.errors.e3', "The GraphQL query returned a response with errors. Please contact your administrator."), {
            ...
            //download
            let file = response.data.data.csvTableTemplateTrial_to_contact.join("\n");
            const url = window.URL.createObjectURL(new Blob([file]));
            const link = document.createElement("a");
            link.href = url;
            link.setAttribute("download", "trial_to_contact-template.csv");
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12} md={11}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <TrialToContactEnhancedTableToolbar
                ...
                <div className={classes.tableWrapper} ref={twref}>
                  ...
                  <Table stickyHeader size='small' ref={tref}>
                    ...
                    {/* Table Head */}
                    <TrialToContactEnhancedTableHead
                      permissions={permissions}
                      order={order}
                      orderBy={orderBy}
                      rowCount={count}
                                ...
                                */}
                                {
                                  /* acl check */
                                  (permissions&&permissions.trial_to_contact&&Array.isArray(permissions.trial_to_contact)
                                  &&(permissions.trial_to_contact.includes('update') || permissions.trial_to_contact.includes('*')))
                                  &&(
...

                                {
                                  /* acl check */
                                  (permissions&&permissions.trial_to_contact&&Array.isArray(permissions.trial_to_contact)
                                  &&(permissions.trial_to_contact.includes('delete') || permissions.trial_to_contact.includes('*')))
                                  &&(
                                    <TableCell padding='checkbox' align='center'>
                ...
                {/*
                  Pagination
                */}
                <TrialToContactCursorPagination
                  count={count}
      ...
      {(deleteConfirmationDialogOpen) && (
        <TrialToContactDeleteConfirmationDialog
          permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <TrialToContactUploadFileDialog
...
TrialToContactEnhancedTable.propTypes = {
