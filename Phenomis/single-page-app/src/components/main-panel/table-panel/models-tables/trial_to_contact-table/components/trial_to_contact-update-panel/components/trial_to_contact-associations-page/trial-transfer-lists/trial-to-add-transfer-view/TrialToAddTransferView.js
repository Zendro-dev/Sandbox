  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.trial_to_contact.getAssociatedTrialConnection(graphqlServerUrl, item.id));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneTrial_to_contact.trial;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trial_to_contact&&
        lastModelChanged.trial_to_contact[String(item.id)]&&
        lastModelChanged.trial_to_contact[String(item.id)].changedAssociations&&
        lastModelChanged.trial_to_contact[String(item.id)].changedAssociations.trial&&
        (lastModelChanged.trial_to_contact[String(item.id)].changedAssociations.trial.added ||
         lastModelChanged.trial_to_contact[String(item.id)].changedAssociations.trial.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.trial_to_contact[String(item.id)].changedAssociations.trial.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'trialToContacts' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'trial',
     * but this action happened on the peer relation 'trialToContacts'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.trial) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.trialToContacts) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.trialToContacts.removed) {
              let idsRemoved = entry[1].changedAssociations.trialToContacts.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.trialToContacts.added) {
              let idsAdded = entry[1].changedAssociations.trialToContacts.idsAdded;
              if(idsAdded) {
