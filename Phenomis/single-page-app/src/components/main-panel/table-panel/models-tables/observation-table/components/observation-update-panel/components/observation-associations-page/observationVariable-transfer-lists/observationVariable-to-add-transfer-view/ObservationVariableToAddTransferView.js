  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated observationVariable ids
      */
      let cancelableApiReq = makeCancelable(api.observation.getAssociatedObservationVariableConnection(graphqlServerUrl, item.observationDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservation.observationVariable;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('observationVariableDbId')) ? [idso.observationVariableDbId] : [];
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.observationDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observationVariable&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observationVariable.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observationVariable.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observationVariable.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'observations' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationVariable',
     * but this action happened on the peer relation 'observations'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observations) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observations.removed) {
              let idsRemoved = entry[1].changedAssociations.observations.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observations.added) {
              let idsAdded = entry[1].changedAssociations.observations.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationDbId, handleUntransfer]);
