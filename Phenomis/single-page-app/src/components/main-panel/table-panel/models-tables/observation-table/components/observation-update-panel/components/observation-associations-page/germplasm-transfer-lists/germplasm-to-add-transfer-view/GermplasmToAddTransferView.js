  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.observation.getAssociatedGermplasmConnection(graphqlServerUrl, item.observationDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservation.germplasm;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('germplasmDbId')) ? [idso.germplasmDbId] : [];
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.germplasm&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.germplasm.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.germplasm.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.germplasm.idsRemoved;
          if(idsRemoved) {
