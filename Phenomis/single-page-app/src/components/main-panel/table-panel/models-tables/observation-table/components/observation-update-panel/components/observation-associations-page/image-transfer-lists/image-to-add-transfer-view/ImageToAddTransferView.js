  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated image ids
      */
      let cancelableApiReq = makeCancelable(api.observation.getAssociatedImageConnection(graphqlServerUrl, item.observationDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservation.image;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('imageDbId')) ? [idso.imageDbId] : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.image.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countImages;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.image.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.imagesConnection.edges.map(o => o.node);
                        let pi = response.data.data.imagesConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.image.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.image.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'images' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.image&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.image.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.image.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.image.idsRemoved;
          if(idsRemoved) {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.imageDbId) === -1) {
      lidsToAdd.current = [];
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.imageDbId}</Typography>
                                  </Grid>
