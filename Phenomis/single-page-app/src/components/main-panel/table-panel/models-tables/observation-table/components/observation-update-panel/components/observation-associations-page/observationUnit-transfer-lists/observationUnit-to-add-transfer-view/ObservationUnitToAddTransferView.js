  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.observation.getAssociatedObservationUnitConnection(graphqlServerUrl, item.observationDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservation.observationUnit;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observationUnit&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observationUnit.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observationUnit.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observationUnit.idsRemoved;
          if(idsRemoved) {
