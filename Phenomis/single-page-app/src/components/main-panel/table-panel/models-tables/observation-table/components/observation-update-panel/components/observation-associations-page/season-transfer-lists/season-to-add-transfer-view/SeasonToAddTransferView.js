  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated season ids
      */
      let cancelableApiReq = makeCancelable(api.observation.getAssociatedSeasonConnection(graphqlServerUrl, item.observationDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservation.season;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('seasonDbId')) ? [idso.seasonDbId] : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.season.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countSeasons;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.season.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.seasonsConnection.edges.map(o => o.node);
                        let pi = response.data.data.seasonsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.season.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.season.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'seasons' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.season&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.season.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.season.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.season.idsRemoved;
          if(idsRemoved) {
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.seasonDbId}</Typography>
                                  </Grid>
