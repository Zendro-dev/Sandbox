  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated studies ids
      */
      let cancelableApiReq = makeCancelable(api.location.getAssociatedStudiesConnection(graphqlServerUrl, item.locationDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneLocation.studiesConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.studyDbId}) : [];
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.locationDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.location&&
        ...
        lastModelChanged.location[String(item.locationDbId)].changedAssociations.studies&&
        (lastModelChanged.location[String(item.locationDbId)].changedAssociations.studies.added ||
         lastModelChanged.location[String(item.locationDbId)].changedAssociations.studies.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.location[String(item.locationDbId)].changedAssociations.studies.idsRemoved;
          if(idsRemoved) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.locationDbId, handleUntransfer]);
