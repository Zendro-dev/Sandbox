...
export default function LocationDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.location[String(itemState.locationDbId)]) {
          
        //updated item
        if(lastModelChanged.location[String(itemState.locationDbId)].op === "update"&&
            lastModelChanged.location[String(itemState.locationDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.location[String(itemState.locationDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.location[String(itemState.locationDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.locationDbId]);
