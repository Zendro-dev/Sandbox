  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.location.getObservationUnitsConnection(
      graphqlServerUrl, 
      item.locationDbId,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if (
          ...
          //save response data
          let newCount = response.data.data.readOneLocation.countFilteredObservationUnits;
          let its = response.data.data.readOneLocation.observationUnitsConnection.edges.map(o => o.node);
          let pi = response.data.data.readOneLocation.observationUnitsConnection.pageInfo;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.location&&
        ...
        lastModelChanged.location[String(item.locationDbId)].changedAssociations.observationUnits&&
        (lastModelChanged.location[String(item.locationDbId)].changedAssociations.observationUnits.added ||
         lastModelChanged.location[String(item.locationDbId)].changedAssociations.observationUnits.removed)) {
          
          //remove any dis-associated id from idsToRemove[]
          let idsRemoved = lastModelChanged.location[String(item.locationDbId)].changedAssociations.observationUnits.idsRemoved;
          if(idsRemoved) {
