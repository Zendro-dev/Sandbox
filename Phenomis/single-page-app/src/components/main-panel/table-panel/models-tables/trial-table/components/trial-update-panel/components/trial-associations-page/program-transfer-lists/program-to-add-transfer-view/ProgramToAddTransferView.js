  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.trial.getAssociatedProgramConnection(graphqlServerUrl, item.trialDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneTrial.program;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trial&&
        ...
        lastModelChanged.trial[String(item.trialDbId)].changedAssociations.program&&
        (lastModelChanged.trial[String(item.trialDbId)].changedAssociations.program.added ||
         lastModelChanged.trial[String(item.trialDbId)].changedAssociations.program.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.program.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'trials' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'program',
     * but this action happened on the peer relation 'trials'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.program) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.trials) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.trials.removed) {
              let idsRemoved = entry[1].changedAssociations.trials.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.trials.added) {
              let idsAdded = entry[1].changedAssociations.trials.idsAdded;
              if(idsAdded) {
