  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.trial.getObservationUnitsConnection(
      graphqlServerUrl, 
      item.trialDbId,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if (
          ...
          //save response data
          let newCount = response.data.data.readOneTrial.countFilteredObservationUnits;
          let its = response.data.data.readOneTrial.observationUnitsConnection.edges.map(o => o.node);
          let pi = response.data.data.readOneTrial.observationUnitsConnection.pageInfo;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trial&&
        ...
        lastModelChanged.trial[String(item.trialDbId)].changedAssociations.observationUnits&&
        (lastModelChanged.trial[String(item.trialDbId)].changedAssociations.observationUnits.added ||
         lastModelChanged.trial[String(item.trialDbId)].changedAssociations.observationUnits.removed)) {
          
          //remove any dis-associated id from idsToRemove[]
          let idsRemoved = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.observationUnits.idsRemoved;
          if(idsRemoved) {
