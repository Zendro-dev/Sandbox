  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.trial.getAssociatedStudiesConnection(graphqlServerUrl, item.trialDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneTrial.studiesConnection.edges.map(o => o.node);
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.trialDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trial&&
        ...
        lastModelChanged.trial[String(item.trialDbId)].changedAssociations.studies&&
        (lastModelChanged.trial[String(item.trialDbId)].changedAssociations.studies.added ||
         lastModelChanged.trial[String(item.trialDbId)].changedAssociations.studies.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.studies.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'trial' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'study',
     * but this action happened on the peer relation 'trial'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.trial) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.trial.removed) {
              let idsRemoved = entry[1].changedAssociations.trial.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.trial.added) {
              let idsAdded = entry[1].changedAssociations.trial.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.trialDbId, handleUntransfer]);
