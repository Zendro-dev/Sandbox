...
export default function TrialDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.trial[String(itemState.trialDbId)]) {
          
        //updated item
        if(lastModelChanged.trial[String(itemState.trialDbId)].op === "update"&&
            lastModelChanged.trial[String(itemState.trialDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.trial[String(itemState.trialDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.trial[String(itemState.trialDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.trialDbId]);
