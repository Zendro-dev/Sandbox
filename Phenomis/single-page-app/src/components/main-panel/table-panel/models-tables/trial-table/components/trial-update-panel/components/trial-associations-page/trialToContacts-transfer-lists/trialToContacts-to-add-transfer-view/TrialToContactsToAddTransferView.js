...
import TrialToContactsToAddTransferViewToolbar from './components/TrialToContactsToAddTransferViewToolbar';
import TrialToContactsToAddTransferViewCursorPagination from './components/TrialToContactsToAddTransferViewCursorPagination';
...
export default function TrialToContactsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated trialToContacts ids
      */
      let cancelableApiReq = makeCancelable(api.trial.getAssociatedTrialToContactsConnection(graphqlServerUrl, item.trialDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneTrial.trialToContactsConnection.edges.map(o => o.node);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'trialToContacts' for this item was updated.
    ...
    if(lastModelChanged.trial&&
        ...
        lastModelChanged.trial[String(item.trialDbId)].changedAssociations.trialToContacts&&
        (lastModelChanged.trial[String(item.trialDbId)].changedAssociations.trialToContacts.added ||
         lastModelChanged.trial[String(item.trialDbId)].changedAssociations.trialToContacts.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.trialToContacts.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('trialToContacts', idRemoved);
            });
          }
            ...
            if(entry[1].changedAssociations.trial.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('trialToContacts', idAdded);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('trialToContacts', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('trialToContacts', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <TrialToContactsToAddTransferViewToolbar
              ...
              {/* Pagination */}
              <TrialToContactsToAddTransferViewCursorPagination
                count={count}
...
TrialToContactsToAddTransferView.propTypes = {
