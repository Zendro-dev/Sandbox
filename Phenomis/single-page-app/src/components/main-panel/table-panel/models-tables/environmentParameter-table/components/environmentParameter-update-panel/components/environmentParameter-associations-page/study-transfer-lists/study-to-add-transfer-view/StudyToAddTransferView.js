  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated study ids
      */
      let cancelableApiReq = makeCancelable(api.environmentParameter.getAssociatedStudyConnection(graphqlServerUrl, item.environmentParameterDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneEnvironmentParameter.study;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('studyDbId')) ? [idso.studyDbId] : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.study.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countStudies;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.study.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.studiesConnection.edges.map(o => o.node);
                        let pi = response.data.data.studiesConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.study.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.study.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.environmentParameterDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'studies' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.environmentParameter&&
        ...
        lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations.study&&
        (lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations.study.added ||
         lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations.study.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations.study.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'environmentParameters' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'study',
     * but this action happened on the peer relation 'environmentParameters'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.environmentParameters) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.environmentParameters.removed) {
              let idsRemoved = entry[1].changedAssociations.environmentParameters.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.environmentParameters.added) {
              let idsAdded = entry[1].changedAssociations.environmentParameters.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.environmentParameterDbId, handleUntransfer]);
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.studyDbId}</Typography>
                                  </Grid>
