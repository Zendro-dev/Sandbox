...
export default function EnvironmentParameterDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.environmentParameter[String(itemState.environmentParameterDbId)]) {
          
        //updated item
        if(lastModelChanged.environmentParameter[String(itemState.environmentParameterDbId)].op === "update"&&
            lastModelChanged.environmentParameter[String(itemState.environmentParameterDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.environmentParameter[String(itemState.environmentParameterDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.environmentParameter[String(itemState.environmentParameterDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.environmentParameterDbId]);
