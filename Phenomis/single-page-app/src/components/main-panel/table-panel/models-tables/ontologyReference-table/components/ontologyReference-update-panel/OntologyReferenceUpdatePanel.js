...
import OntologyReferenceAttributesPage from './components/ontologyReference-attributes-page/OntologyReferenceAttributesPage'
import OntologyReferenceAssociationsPage from './components/ontologyReference-associations-page/OntologyReferenceAssociationsPage'
import OntologyReferenceTabsA from './components/OntologyReferenceTabsA'
import OntologyReferenceConfirmationDialog from './components/OntologyReferenceConfirmationDialog'
...
export default function OntologyReferenceUpdatePanel(props) {
  ...
  function doSave(event) {
    ...
    */
    ...
    changedAssociations.current.methods = {added: false, removed: false};
    
    if(methodsIdsToAdd.current.length>0) {
      variables.addMethods = methodsIdsToAdd.current;
      
      changedAssociations.current.methods.added = true;
      changedAssociations.current.methods.idsAdded = methodsIdsToAdd.current;
    }
    if(methodsIdsToRemove.current.length>0) {
      variables.removeMethods = methodsIdsToRemove.current;
      
      changedAssociations.current.methods.removed = true;
      changedAssociations.current.methods.idsRemoved = methodsIdsToRemove.current;
    }
    //data to notify changes
    ...
    //data to notify changes
    changedAssociations.current.scales = {added: false, removed: false};
    
    if(scalesIdsToAdd.current.length>0) {
      variables.addScales = scalesIdsToAdd.current;
      
      changedAssociations.current.scales.added = true;
      changedAssociations.current.scales.idsAdded = scalesIdsToAdd.current;
    }
    if(scalesIdsToRemove.current.length>0) {
      variables.removeScales = scalesIdsToRemove.current;
      
      changedAssociations.current.scales.removed = true;
      changedAssociations.current.scales.idsRemoved = scalesIdsToRemove.current;
    }
    //data to notify changes
    changedAssociations.current.traits = {added: false, removed: false};
    
    if(traitsIdsToAdd.current.length>0) {
      variables.addTraits = traitsIdsToAdd.current;
      
      changedAssociations.current.traits.added = true;
      changedAssociations.current.traits.idsAdded = traitsIdsToAdd.current;
    }
    if(traitsIdsToRemove.current.length>0) {
      variables.removeTraits = traitsIdsToRemove.current;
      
      changedAssociations.current.traits.removed = true;
      changedAssociations.current.traits.idsRemoved = traitsIdsToRemove.current;
    }
      ...
      .then(response => {
        ...
        if (
          ...
          if(response.data.errors) {
            ...
            onClose(event, true, response.data.data.updateOntologyReference);
          }
          return;
