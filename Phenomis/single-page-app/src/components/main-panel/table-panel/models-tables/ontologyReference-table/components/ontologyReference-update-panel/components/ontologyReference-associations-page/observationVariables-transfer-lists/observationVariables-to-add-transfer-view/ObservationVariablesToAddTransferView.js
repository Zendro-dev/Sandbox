  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated observationVariables ids
      */
      let cancelableApiReq = makeCancelable(api.ontologyReference.getAssociatedObservationVariablesConnection(graphqlServerUrl, item.ontologyDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneOntologyReference.observationVariablesConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.observationVariableDbId}) : [];
            ...
            if(exIds.length > 0) {
              ops = {
                exclude: [{
                  type: 'String',
                  values: {observationVariableDbId: exIds}
                }]
              };
            }
            ...
            */
            let cancelableApiReqB = makeCancelable(api.observationVariable.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countObservationVariables;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.observationVariable.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.observationVariablesConnection.edges.map(o => o.node);
                        let pi = response.data.data.observationVariablesConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.observationVariable.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.observationVariable.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.ontologyDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'observationVariables' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.ontologyReference&&
        ...
        lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.observationVariables&&
        (lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.observationVariables.added ||
         lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.observationVariables.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.observationVariables.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'ontologyReference' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationVariable',
     * but this action happened on the peer relation 'ontologyReference'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.ontologyReference) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.ontologyReference.removed) {
              let idsRemoved = entry[1].changedAssociations.ontologyReference.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.ontologyReference.added) {
              let idsAdded = entry[1].changedAssociations.ontologyReference.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.ontologyDbId, handleUntransfer]);
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.observationVariableDbId}</Typography>
                                  </Grid>
