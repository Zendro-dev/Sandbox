...
export default function OntologyReferenceDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.ontologyReference[String(itemState.ontologyDbId)]) {
          
        //updated item
        if(lastModelChanged.ontologyReference[String(itemState.ontologyDbId)].op === "update"&&
            lastModelChanged.ontologyReference[String(itemState.ontologyDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.ontologyReference[String(itemState.ontologyDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.ontologyReference[String(itemState.ontologyDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.ontologyDbId]);
