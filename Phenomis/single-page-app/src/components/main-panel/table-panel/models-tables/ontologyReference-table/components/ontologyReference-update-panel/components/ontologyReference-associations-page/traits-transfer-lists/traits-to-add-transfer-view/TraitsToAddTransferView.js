  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated traits ids
      */
      let cancelableApiReq = makeCancelable(api.ontologyReference.getAssociatedTraitsConnection(graphqlServerUrl, item.ontologyDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneOntologyReference.traitsConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.traitDbId}) : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.trait.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countTraits;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.trait.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.traitsConnection.edges.map(o => o.node);
                        let pi = response.data.data.traitsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.trait.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.trait.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'traits' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.ontologyReference&&
        ...
        lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.traits&&
        (lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.traits.added ||
         lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.traits.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.traits.idsRemoved;
          if(idsRemoved) {
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.traitDbId}</Typography>
                                  </Grid>
