...
export default function EventDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.event[String(itemState.eventType)]) {
          
        //updated item
        if(lastModelChanged.event[String(itemState.eventType)].op === "update"&&
            lastModelChanged.event[String(itemState.eventType)].newItem) {
              //replace item
              setItemState(lastModelChanged.event[String(itemState.eventType)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.event[String(itemState.eventType)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.eventType]);
