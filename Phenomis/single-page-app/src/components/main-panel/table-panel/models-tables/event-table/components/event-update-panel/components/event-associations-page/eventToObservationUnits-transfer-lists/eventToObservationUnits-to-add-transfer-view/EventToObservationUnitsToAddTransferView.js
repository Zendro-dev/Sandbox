...
import EventToObservationUnitsToAddTransferViewToolbar from './components/EventToObservationUnitsToAddTransferViewToolbar';
import EventToObservationUnitsToAddTransferViewCursorPagination from './components/EventToObservationUnitsToAddTransferViewCursorPagination';
...
export default function EventToObservationUnitsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated eventToObservationUnits ids
      */
      let cancelableApiReq = makeCancelable(api.event.getAssociatedEventToObservationUnitsConnection(graphqlServerUrl, item.eventType));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneEvent.eventToObservationUnitsConnection.edges.map(o => o.node);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'eventToObservationUnits' for this item was updated.
    ...
    if(lastModelChanged.event&&
        ...
        lastModelChanged.event[String(item.eventType)].changedAssociations.eventToObservationUnits&&
        (lastModelChanged.event[String(item.eventType)].changedAssociations.eventToObservationUnits.added ||
         lastModelChanged.event[String(item.eventType)].changedAssociations.eventToObservationUnits.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.event[String(item.eventType)].changedAssociations.eventToObservationUnits.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('eventToObservationUnits', idRemoved);
            });
          }
            ...
            if(entry[1].changedAssociations.event.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('eventToObservationUnits', idAdded);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('eventToObservationUnits', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('eventToObservationUnits', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <EventToObservationUnitsToAddTransferViewToolbar
              ...
              {/* Pagination */}
              <EventToObservationUnitsToAddTransferViewCursorPagination
                count={count}
...
EventToObservationUnitsToAddTransferView.propTypes = {
