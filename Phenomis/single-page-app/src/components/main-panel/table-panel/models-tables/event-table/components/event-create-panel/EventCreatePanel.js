...
export default function EventCreatePanel(props) {
      ...
      .then(response => {
        ...
        if (
          ...
          if(response.data.errors) {
            ...
            onClose(event, true, response.data.data.addEvent);
          }
          return;
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      ...
      case 'eventToObservationUnits':
        if(eventToObservationUnitsIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    ...
    if(associationKey === 'eventToObservationUnits') {
      let iof = eventToObservationUnitsIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        eventToObservationUnitsIdsToAdd.current.splice(iof, 1);
      }
      return;
