  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated eventParameters ids
      */
      let cancelableApiReq = makeCancelable(api.event.getAssociatedEventParametersConnection(graphqlServerUrl, item.eventType));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneEvent.eventParametersConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.eventParameterDbId}) : [];
            ...
            if(exIds.length > 0) {
              ops = {
                exclude: [{
                  type: 'String',
                  values: {eventParameterDbId: exIds}
                }]
              };
            }
            ...
            */
            let cancelableApiReqB = makeCancelable(api.eventParameter.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countEventParameters;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.eventParameter.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.eventParametersConnection.edges.map(o => o.node);
                        let pi = response.data.data.eventParametersConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.eventParameter.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.eventParameter.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.eventType, search, rowsPerPage]);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'eventParameters' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.event&&
        ...
        lastModelChanged.event[String(item.eventType)].changedAssociations.eventParameters&&
        (lastModelChanged.event[String(item.eventType)].changedAssociations.eventParameters.added ||
         lastModelChanged.event[String(item.eventType)].changedAssociations.eventParameters.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.event[String(item.eventType)].changedAssociations.eventParameters.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'event' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'eventParameter',
     * but this action happened on the peer relation 'event'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.eventParameter) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.event) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.event.removed) {
              let idsRemoved = entry[1].changedAssociations.event.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.event.added) {
              let idsAdded = entry[1].changedAssociations.event.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.eventType, handleUntransfer]);
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.eventParameterDbId}</Typography>
                                  </Grid>
