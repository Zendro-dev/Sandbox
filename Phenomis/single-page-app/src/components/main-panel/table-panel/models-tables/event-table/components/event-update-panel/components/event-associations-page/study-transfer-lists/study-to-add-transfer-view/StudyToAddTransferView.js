  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.event.getAssociatedStudyConnection(graphqlServerUrl, item.eventType));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneEvent.study;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.event&&
        ...
        lastModelChanged.event[String(item.eventType)].changedAssociations.study&&
        (lastModelChanged.event[String(item.eventType)].changedAssociations.study.added ||
         lastModelChanged.event[String(item.eventType)].changedAssociations.study.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.event[String(item.eventType)].changedAssociations.study.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'events' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'study',
     * but this action happened on the peer relation 'events'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.events) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.events.removed) {
              let idsRemoved = entry[1].changedAssociations.events.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.events.added) {
              let idsAdded = entry[1].changedAssociations.events.idsAdded;
              if(idsAdded) {
