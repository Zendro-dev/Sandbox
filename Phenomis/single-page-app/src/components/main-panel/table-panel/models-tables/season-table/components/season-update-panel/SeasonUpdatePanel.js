...
export default function SeasonUpdatePanel(props) {
  ...
  const [seasonToStudiesIdsToRemoveState, setSeasonToStudiesIdsToRemoveState] = useState([]);
  const seasonToStudiesIdsToRemove = useRef([]);
  ...
  function doSave(event) {
    ...
    */
    ...
    //data to notify changes
    changedAssociations.current.seasonToStudies = {added: false, removed: false};
    
    if(seasonToStudiesIdsToAdd.current.length>0) {
      variables.addSeasonToStudies = seasonToStudiesIdsToAdd.current;
      
      changedAssociations.current.seasonToStudies.added = true;
      changedAssociations.current.seasonToStudies.idsAdded = seasonToStudiesIdsToAdd.current;
    }
    if(seasonToStudiesIdsToRemove.current.length>0) {
      variables.removeSeasonToStudies = seasonToStudiesIdsToRemove.current;
      
      changedAssociations.current.seasonToStudies.removed = true;
      changedAssociations.current.seasonToStudies.idsRemoved = seasonToStudiesIdsToRemove.current;
    }
      ...
      .then(response => {
        ...
        if (
          ...
          if(response.data.errors) {
            ...
            onClose(event, true, response.data.data.updateSeason);
          }
          return;
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    ...
    if(associationKey === 'seasonToStudies') {
      for(let i=0; i<seasonToStudiesIdsToAdd.current.length; ++i)
      {
        if(seasonToStudiesIdsToAdd.current[i] === itemId) {
          seasonToStudiesIdsToAdd.current.splice(i, 1);
          setSeasonToStudiesIdsToAddState(seasonToStudiesIdsToAdd.current);
          return;
        }
      }
  ...
  const handleTransferToRemove = (associationKey, itemId) => {
    switch(associationKey) {
      case 'observations':
        ...
        break;
      case 'seasonToStudies':
        seasonToStudiesIdsToRemove.current.push(itemId);
        setSeasonToStudiesIdsToRemoveState(seasonToStudiesIdsToRemove.current);
        break;
  ...
  const handleUntransferFromRemove =(associationKey, itemId) => {
    ...
    if(associationKey === 'seasonToStudies') {
      for(let i=0; i<seasonToStudiesIdsToRemove.current.length; ++i)
      {
        if(seasonToStudiesIdsToRemove.current[i] === itemId) {
          seasonToStudiesIdsToRemove.current.splice(i, 1);
          setSeasonToStudiesIdsToRemoveState(seasonToStudiesIdsToRemove.current);
          return;
        }
      }
        ...
        <Grid container justify='center' alignItems='flex-start' alignContent='flex-start'>
          ...
          <Grid item xs={12}>
            ...
            <SeasonAssociationsPage
              ...
              seasonToStudiesIdsToRemove={seasonToStudiesIdsToRemoveState}
