...
import SeasonToStudiesToAddTransferViewToolbar from './components/SeasonToStudiesToAddTransferViewToolbar';
import SeasonToStudiesToAddTransferViewCursorPagination from './components/SeasonToStudiesToAddTransferViewCursorPagination';
...
export default function SeasonToStudiesToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated seasonToStudies ids
      */
      let cancelableApiReq = makeCancelable(api.season.getAssociatedSeasonToStudiesConnection(graphqlServerUrl, item.seasonDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneSeason.seasonToStudiesConnection.edges.map(o => o.node);
            ...
            */
            let cancelableApiReqB = makeCancelable(api.study_to_season.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countStudy_to_seasons;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.study_to_season.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.study_to_seasonsConnection.edges.map(o => o.node);
                        let pi = response.data.data.study_to_seasonsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.study_to_season.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.study_to_season.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  const getDataB = useCallback(() => {
    ...
    /*
      API Request: countItems
    */
    let cancelableApiReq = makeCancelable(api.study_to_season.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'seasonToStudies' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'study_to_seasons' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.season&&
        ...
        lastModelChanged.season[String(item.seasonDbId)].changedAssociations.seasonToStudies&&
        (lastModelChanged.season[String(item.seasonDbId)].changedAssociations.seasonToStudies.added ||
         lastModelChanged.season[String(item.seasonDbId)].changedAssociations.seasonToStudies.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.season[String(item.seasonDbId)].changedAssociations.seasonToStudies.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('seasonToStudies', idRemoved);
            });
          }
     ...
     * That is to say that this current item was associated or dis-associated with some 'study_to_season',
     * but this action happened on the peer relation 'season'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'study_to_season'.
     * B: the current item internalId is in the addedIds of the updated 'study_to_season'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'study_to_season' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.study_to_season) {
      let oens = Object.entries(lastModelChanged.study_to_season);
      oens.forEach( (entry) => {
            ...
            if(entry[1].changedAssociations.season.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('seasonToStudies', idAdded);
    ...
    /*
     * Case 3: 
     * The attributes of some 'study_to_season' were modified or the item was deleted.
     * 
     * Conditions:
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnStudy_to_seasonRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('seasonToStudies', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('seasonToStudies', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <SeasonToStudiesToAddTransferViewToolbar
                title={'Study_to_seasons'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <SeasonToStudiesToAddTransferViewCursorPagination
                count={count}
...
SeasonToStudiesToAddTransferView.propTypes = {
