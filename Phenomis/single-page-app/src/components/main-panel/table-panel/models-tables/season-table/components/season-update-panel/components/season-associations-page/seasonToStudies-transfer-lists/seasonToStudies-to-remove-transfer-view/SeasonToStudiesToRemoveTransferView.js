...
import SeasonToStudiesToRemoveTransferViewToolbar from './components/SeasonToStudiesToRemoveTransferViewToolbar';
import SeasonToStudiesToRemoveTransferViewCursorPagination from './components/SeasonToStudiesToRemoveTransferViewCursorPagination';
...
export default function SeasonToStudiesToRemoveTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.season.getSeasonToStudiesConnection(
      graphqlServerUrl, 
      item.seasonDbId,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if (
          ...
          //save response data
          let newCount = response.data.data.readOneSeason.countFilteredSeasonToStudies;
          let its = response.data.data.readOneSeason.seasonToStudiesConnection.edges.map(o => o.node);
          let pi = response.data.data.readOneSeason.seasonToStudiesConnection.pageInfo;
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'study_to_season' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <SeasonToStudiesToRemoveTransferViewToolbar
              ...
              {/* Pagination */}
              <SeasonToStudiesToRemoveTransferViewCursorPagination
                count={count}
...
SeasonToStudiesToRemoveTransferView.propTypes = {
