  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.season.getAssociatedObservationsConnection(graphqlServerUrl, item.seasonDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneSeason.observationsConnection.edges.map(o => o.node);
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.seasonDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.season&&
        ...
        lastModelChanged.season[String(item.seasonDbId)].changedAssociations.observations&&
        (lastModelChanged.season[String(item.seasonDbId)].changedAssociations.observations.added ||
         lastModelChanged.season[String(item.seasonDbId)].changedAssociations.observations.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.season[String(item.seasonDbId)].changedAssociations.observations.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'season' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observation',
     * but this action happened on the peer relation 'season'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.season) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.season.removed) {
              let idsRemoved = entry[1].changedAssociations.season.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.season.added) {
              let idsAdded = entry[1].changedAssociations.season.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.seasonDbId, handleUntransfer]);
