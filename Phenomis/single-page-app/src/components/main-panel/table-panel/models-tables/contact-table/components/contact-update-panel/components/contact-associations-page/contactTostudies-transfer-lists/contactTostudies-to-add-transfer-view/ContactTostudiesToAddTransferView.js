...
import ContactTostudiesToAddTransferViewToolbar from './components/ContactTostudiesToAddTransferViewToolbar';
import ContactTostudiesToAddTransferViewCursorPagination from './components/ContactTostudiesToAddTransferViewCursorPagination';
...
export default function ContactTostudiesToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated contactTostudies ids
      */
      let cancelableApiReq = makeCancelable(api.contact.getAssociatedContactTostudiesConnection(graphqlServerUrl, item.contactDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneContact.contactTostudiesConnection.edges.map(o => o.node);
            ...
            */
            let cancelableApiReqB = makeCancelable(api.study_to_contact.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countStudy_to_contacts;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.study_to_contact.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.study_to_contactsConnection.edges.map(o => o.node);
                        let pi = response.data.data.study_to_contactsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.study_to_contact.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.study_to_contact.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  const getDataB = useCallback(() => {
    ...
    /*
      API Request: countItems
    */
    let cancelableApiReq = makeCancelable(api.study_to_contact.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'ContactTostudies' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'study_to_contacts' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.contact&&
        ...
        lastModelChanged.contact[String(item.contactDbId)].changedAssociations.ContactTostudies&&
        (lastModelChanged.contact[String(item.contactDbId)].changedAssociations.ContactTostudies.added ||
         lastModelChanged.contact[String(item.contactDbId)].changedAssociations.ContactTostudies.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.contact[String(item.contactDbId)].changedAssociations.ContactTostudies.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('ContactTostudies', idRemoved);
            });
          }
     ...
     * That is to say that this current item was associated or dis-associated with some 'study_to_contact',
     * but this action happened on the peer relation 'study'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'study_to_contact'.
     * B: the current item internalId is in the addedIds of the updated 'study_to_contact'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'study_to_contact' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.study_to_contact) {
      let oens = Object.entries(lastModelChanged.study_to_contact);
      oens.forEach( (entry) => {
            ...
            if(entry[1].changedAssociations.study.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('ContactTostudies', idAdded);
    ...
    /*
     * Case 3: 
     * The attributes of some 'study_to_contact' were modified or the item was deleted.
     * 
     * Conditions:
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnStudy_to_contactRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('ContactTostudies', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('ContactTostudies', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ContactTostudiesToAddTransferViewToolbar
                title={'Study_to_contacts'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <ContactTostudiesToAddTransferViewCursorPagination
                count={count}
...
ContactTostudiesToAddTransferView.propTypes = {
