...
export default function ContactDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.contact[String(itemState.contactDbId)]) {
          
        //updated item
        if(lastModelChanged.contact[String(itemState.contactDbId)].op === "update"&&
            lastModelChanged.contact[String(itemState.contactDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.contact[String(itemState.contactDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.contact[String(itemState.contactDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.contactDbId]);
