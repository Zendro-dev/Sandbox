...
import ContactToTrialsToAddTransferViewToolbar from './components/ContactToTrialsToAddTransferViewToolbar';
import ContactToTrialsToAddTransferViewCursorPagination from './components/ContactToTrialsToAddTransferViewCursorPagination';
...
export default function ContactToTrialsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated contactToTrials ids
      */
      let cancelableApiReq = makeCancelable(api.contact.getAssociatedContactToTrialsConnection(graphqlServerUrl, item.contactDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneContact.contactToTrialsConnection.edges.map(o => o.node);
            ...
            */
            let cancelableApiReqB = makeCancelable(api.trial_to_contact.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countTrial_to_contacts;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.trial_to_contact.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.trial_to_contactsConnection.edges.map(o => o.node);
                        let pi = response.data.data.trial_to_contactsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.trial_to_contact.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.trial_to_contact.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.contactDbId, search, rowsPerPage]);
  ...
  const getDataB = useCallback(() => {
    ...
    /*
      API Request: countItems
    */
    let cancelableApiReq = makeCancelable(api.trial_to_contact.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'ContactToTrials' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'trial_to_contacts' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.contact&&
        ...
        lastModelChanged.contact[String(item.contactDbId)].changedAssociations.ContactToTrials&&
        (lastModelChanged.contact[String(item.contactDbId)].changedAssociations.ContactToTrials.added ||
         lastModelChanged.contact[String(item.contactDbId)].changedAssociations.ContactToTrials.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.contact[String(item.contactDbId)].changedAssociations.ContactToTrials.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('ContactToTrials', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'contact' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'trial_to_contact',
     * but this action happened on the peer relation 'contact'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'trial_to_contact'.
     * B: the current item internalId is in the addedIds of the updated 'trial_to_contact'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'trial_to_contact' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.trial_to_contact) {
      let oens = Object.entries(lastModelChanged.trial_to_contact);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.contact) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.contact.removed) {
              let idsRemoved = entry[1].changedAssociations.contact.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.contact.added) {
              let idsAdded = entry[1].changedAssociations.contact.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('ContactToTrials', idAdded);
    ...
    /*
     * Case 3: 
     * The attributes of some 'trial_to_contact' were modified or the item was deleted.
     * 
     * Conditions:
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.contactDbId, handleUntransfer]);
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnTrial_to_contactRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('ContactToTrials', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('ContactToTrials', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ContactToTrialsToAddTransferViewToolbar
                title={'Trial_to_contacts'}
                titleIcon={1}
                search={search}
              ...
              {/* Pagination */}
              <ContactToTrialsToAddTransferViewCursorPagination
                count={count}
...
ContactToTrialsToAddTransferView.propTypes = {
