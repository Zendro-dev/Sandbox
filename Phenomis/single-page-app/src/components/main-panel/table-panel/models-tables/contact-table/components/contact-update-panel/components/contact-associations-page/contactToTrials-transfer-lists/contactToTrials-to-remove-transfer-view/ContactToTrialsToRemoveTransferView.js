...
import ContactToTrialsToRemoveTransferViewToolbar from './components/ContactToTrialsToRemoveTransferViewToolbar';
import ContactToTrialsToRemoveTransferViewCursorPagination from './components/ContactToTrialsToRemoveTransferViewCursorPagination';
...
export default function ContactToTrialsToRemoveTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.contact.getContactToTrialsConnection(
      graphqlServerUrl, 
      item.contactDbId,
      label,
      sublabel,
    ...
    cancelableApiReq
      ...
      .then(response => {
        ...
        if (
          ...
          //save response data
          let newCount = response.data.data.readOneContact.countFilteredContactToTrials;
          let its = response.data.data.readOneContact.contactToTrialsConnection.edges.map(o => o.node);
          let pi = response.data.data.readOneContact.contactToTrialsConnection.pageInfo;
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'trial_to_contact' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ContactToTrialsToRemoveTransferViewToolbar
              ...
              {/* Pagination */}
              <ContactToTrialsToRemoveTransferViewCursorPagination
                count={count}
...
ContactToTrialsToRemoveTransferView.propTypes = {
