...
export default function ObservationTreatmentDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.observationTreatment[String(itemState.observationTreatmentDbId)]) {
          
        //updated item
        if(lastModelChanged.observationTreatment[String(itemState.observationTreatmentDbId)].op === "update"&&
            lastModelChanged.observationTreatment[String(itemState.observationTreatmentDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.observationTreatment[String(itemState.observationTreatmentDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.observationTreatment[String(itemState.observationTreatmentDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.observationTreatmentDbId]);
