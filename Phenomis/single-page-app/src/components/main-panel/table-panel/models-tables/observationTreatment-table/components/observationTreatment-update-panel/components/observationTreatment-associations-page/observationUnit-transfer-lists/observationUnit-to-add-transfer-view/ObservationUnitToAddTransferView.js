  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.observationTreatment.getAssociatedObservationUnitConnection(graphqlServerUrl, item.observationTreatmentDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationTreatment.observationUnit;
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.observationTreatmentDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationTreatment&&
        ...
        lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations.observationUnit&&
        (lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations.observationUnit.added ||
         lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations.observationUnit.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations.observationUnit.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'observationTreatments' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationUnit',
     * but this action happened on the peer relation 'observationTreatments'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationUnit) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationTreatments) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationTreatments.removed) {
              let idsRemoved = entry[1].changedAssociations.observationTreatments.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationTreatments.added) {
              let idsAdded = entry[1].changedAssociations.observationTreatments.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationTreatmentDbId, handleUntransfer]);
