...
import ObservationUnitToEventsToAddTransferViewToolbar from './components/ObservationUnitToEventsToAddTransferViewToolbar';
import ObservationUnitToEventsToAddTransferViewCursorPagination from './components/ObservationUnitToEventsToAddTransferViewCursorPagination';
...
export default function ObservationUnitToEventsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated observationUnitToEvents ids
      */
      let cancelableApiReq = makeCancelable(api.observationUnit.getAssociatedObservationUnitToEventsConnection(graphqlServerUrl, item.observationUnitDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneObservationUnit.observationUnitToEventsConnection.edges.map(o => o.node);
            ...
            */
            let cancelableApiReqB = makeCancelable(api.observationUnit_to_event.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countObservationUnit_to_events;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.observationUnit_to_event.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.observationUnit_to_eventsConnection.edges.map(o => o.node);
                        let pi = response.data.data.observationUnit_to_eventsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.observationUnit_to_event.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.observationUnit_to_event.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  const getDataB = useCallback(() => {
    ...
    /*
      API Request: countItems
    */
    let cancelableApiReq = makeCancelable(api.observationUnit_to_event.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'observationUnitToEvents' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'observationUnit_to_events' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitToEvents&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitToEvents.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitToEvents.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitToEvents.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('observationUnitToEvents', idRemoved);
            });
          }
     ...
     * That is to say that this current item was associated or dis-associated with some 'observationUnit_to_event',
     * but this action happened on the peer relation 'observationUnit'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'observationUnit_to_event'.
     * B: the current item internalId is in the addedIds of the updated 'observationUnit_to_event'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'observationUnit_to_event' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.observationUnit_to_event) {
      let oens = Object.entries(lastModelChanged.observationUnit_to_event);
      oens.forEach( (entry) => {
            ...
            if(entry[1].changedAssociations.observationUnit.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  handleUntransfer('observationUnitToEvents', idAdded);
    ...
    /*
     * Case 3: 
     * The attributes of some 'observationUnit_to_event' were modified or the item was deleted.
     * 
     * Conditions:
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnObservationUnit_to_eventRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('observationUnitToEvents', item.id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('observationUnitToEvents', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationUnitToEventsToAddTransferViewToolbar
                title={'ObservationUnit_to_events'}
                titleIcon={1}
                search={search}
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.id;
                        let label = it.observationUnitDbId;
              ...
              {/* Pagination */}
              <ObservationUnitToEventsToAddTransferViewCursorPagination
                count={count}
...
ObservationUnitToEventsToAddTransferView.propTypes = {
