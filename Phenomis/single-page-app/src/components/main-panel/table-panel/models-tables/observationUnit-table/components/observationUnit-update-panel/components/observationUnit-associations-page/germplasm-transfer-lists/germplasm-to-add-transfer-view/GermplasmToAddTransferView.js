  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.observationUnit.getAssociatedGermplasmConnection(graphqlServerUrl, item.observationUnitDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationUnit.germplasm;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.germplasm&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.germplasm.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.germplasm.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.germplasm.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'observationUnits' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'germplasm',
     * but this action happened on the peer relation 'observationUnits'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.germplasm) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnits) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnits.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnits.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnits.added) {
              let idsAdded = entry[1].changedAssociations.observationUnits.idsAdded;
              if(idsAdded) {
