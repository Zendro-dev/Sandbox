  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated location ids
      */
      let cancelableApiReq = makeCancelable(api.observationUnit.getAssociatedLocationConnection(graphqlServerUrl, item.observationUnitDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationUnit.location;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('locationDbId')) ? [idso.locationDbId] : [];
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countLocations;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.location&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.location.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.location.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.location.idsRemoved;
          if(idsRemoved) {
    ...
    if(lastModelChanged.location) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          let iofB = itemsB.findIndex((item) => item.locationDbId===idRemoved);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.locationDbId) === -1) {
      lidsToAdd.current = [];
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.locationDbId}</Typography>
                                  </Grid>
