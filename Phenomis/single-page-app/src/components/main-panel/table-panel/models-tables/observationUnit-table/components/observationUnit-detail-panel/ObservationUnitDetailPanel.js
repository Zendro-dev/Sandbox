...
export default function ObservationUnitDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.observationUnit[String(itemState.observationUnitDbId)]) {
          
        //updated item
        if(lastModelChanged.observationUnit[String(itemState.observationUnitDbId)].op === "update"&&
            lastModelChanged.observationUnit[String(itemState.observationUnitDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.observationUnit[String(itemState.observationUnitDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.observationUnit[String(itemState.observationUnitDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.observationUnitDbId]);
