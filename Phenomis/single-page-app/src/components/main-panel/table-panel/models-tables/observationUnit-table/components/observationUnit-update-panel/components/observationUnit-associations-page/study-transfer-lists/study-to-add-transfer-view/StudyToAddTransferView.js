  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.observationUnit.getAssociatedStudyConnection(graphqlServerUrl, item.observationUnitDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationUnit.study;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.study&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.study.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.study.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.study.idsRemoved;
          if(idsRemoved) {
