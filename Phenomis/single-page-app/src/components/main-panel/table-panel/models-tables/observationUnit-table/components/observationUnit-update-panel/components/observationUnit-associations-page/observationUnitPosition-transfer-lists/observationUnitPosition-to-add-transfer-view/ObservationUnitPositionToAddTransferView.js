  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated observationUnitPosition ids
      */
      let cancelableApiReq = makeCancelable(api.observationUnit.getAssociatedObservationUnitPositionConnection(graphqlServerUrl, item.observationUnitDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationUnit.observationUnitPosition;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('observationUnitPositionDbId')) ? [idso.observationUnitPositionDbId] : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.observationUnitPosition.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countObservationUnitPositions;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.observationUnitPosition.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.observationUnitPositionsConnection.edges.map(o => o.node);
                        let pi = response.data.data.observationUnitPositionsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.observationUnitPosition.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.observationUnitPosition.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.observationUnitDbId, search, rowsPerPage]);
  ...
  const getDataB = useCallback(() => {
    ...
    /*
      API Request: countItems
    */
    let cancelableApiReq = makeCancelable(api.observationUnitPosition.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'observationUnitPositions' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitPosition&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitPosition.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitPosition.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitPosition.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'observationUnit' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationUnitPosition',
     * but this action happened on the peer relation 'observationUnit'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationUnitPosition) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnit) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnit.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnit.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnit.added) {
              let idsAdded = entry[1].changedAssociations.observationUnit.idsAdded;
              if(idsAdded) {
    ...
    if(lastModelChanged.observationUnitPosition) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          let iofB = itemsB.findIndex((item) => item.observationUnitPositionDbId===idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationUnitDbId, handleUntransfer]);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.observationUnitPositionDbId) === -1) {
      lidsToAdd.current = [];
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.observationUnitPositionDbId}</Typography>
                                  </Grid>
