  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated observationTreatments ids
      */
      let cancelableApiReq = makeCancelable(api.observationUnit.getAssociatedObservationTreatmentsConnection(graphqlServerUrl, item.observationUnitDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneObservationUnit.observationTreatmentsConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.observationTreatmentDbId}) : [];
            ...
            if(exIds.length > 0) {
              ops = {
                exclude: [{
                  type: 'String',
                  values: {observationTreatmentDbId: exIds}
                }]
              };
            }
            ...
            */
            let cancelableApiReqB = makeCancelable(api.observationTreatment.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countObservationTreatments;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.observationTreatment.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.observationTreatmentsConnection.edges.map(o => o.node);
                        let pi = response.data.data.observationTreatmentsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.observationTreatment.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.observationTreatment.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'observationTreatments' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationTreatments&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationTreatments.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationTreatments.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationTreatments.idsRemoved;
          if(idsRemoved) {
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.observationTreatmentDbId}</Typography>
                                  </Grid>
