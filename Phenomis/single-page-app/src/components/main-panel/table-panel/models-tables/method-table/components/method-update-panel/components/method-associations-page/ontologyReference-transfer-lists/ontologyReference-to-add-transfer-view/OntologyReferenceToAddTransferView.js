  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.method.getAssociatedOntologyReferenceConnection(graphqlServerUrl, item.methodDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneMethod.ontologyReference;
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.method&&
        ...
        lastModelChanged.method[String(item.methodDbId)].changedAssociations.ontologyReference&&
        (lastModelChanged.method[String(item.methodDbId)].changedAssociations.ontologyReference.added ||
         lastModelChanged.method[String(item.methodDbId)].changedAssociations.ontologyReference.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.method[String(item.methodDbId)].changedAssociations.ontologyReference.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'methods' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'ontologyReference',
     * but this action happened on the peer relation 'methods'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyReference) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.methods) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.methods.removed) {
              let idsRemoved = entry[1].changedAssociations.methods.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.methods.added) {
              let idsAdded = entry[1].changedAssociations.methods.idsAdded;
              if(idsAdded) {
