  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      ...
      */
      let cancelableApiReq = makeCancelable(api.method.getAssociatedObservationVariablesConnection(graphqlServerUrl, item.methodDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneMethod.observationVariablesConnection.edges.map(o => o.node);
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.methodDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.method&&
        ...
        lastModelChanged.method[String(item.methodDbId)].changedAssociations.observationVariables&&
        (lastModelChanged.method[String(item.methodDbId)].changedAssociations.observationVariables.added ||
         lastModelChanged.method[String(item.methodDbId)].changedAssociations.observationVariables.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.method[String(item.methodDbId)].changedAssociations.observationVariables.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'method' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observationVariable',
     * but this action happened on the peer relation 'method'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.method) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.method.removed) {
              let idsRemoved = entry[1].changedAssociations.method.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.method.added) {
              let idsAdded = entry[1].changedAssociations.method.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.methodDbId, handleUntransfer]);
