...
export default function MethodDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.method[String(itemState.methodDbId)]) {
          
        //updated item
        if(lastModelChanged.method[String(itemState.methodDbId)].op === "update"&&
            lastModelChanged.method[String(itemState.methodDbId)].newItem) {
              //replace item
              setItemState(lastModelChanged.method[String(itemState.methodDbId)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.method[String(itemState.methodDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.methodDbId]);
