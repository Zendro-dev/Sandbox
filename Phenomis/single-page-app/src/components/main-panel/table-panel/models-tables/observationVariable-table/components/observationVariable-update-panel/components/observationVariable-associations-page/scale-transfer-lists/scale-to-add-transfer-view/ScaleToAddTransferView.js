  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated scale ids
      */
      let cancelableApiReq = makeCancelable(api.observationVariable.getAssociatedScaleConnection(graphqlServerUrl, item.observationVariableDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationVariable.scale;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('scaleDbId')) ? [idso.scaleDbId] : [];
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationVariable&&
        ...
        lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.scale&&
        (lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.scale.added ||
         lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.scale.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.scale.idsRemoved;
          if(idsRemoved) {
