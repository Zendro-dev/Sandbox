  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated ontologyReference ids
      */
      let cancelableApiReq = makeCancelable(api.observationVariable.getAssociatedOntologyReferenceConnection(graphqlServerUrl, item.observationVariableDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationVariable.ontologyReference;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('ontologyDbId')) ? [idso.ontologyDbId] : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.ontologyReference.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countOntologyReferences;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.ontologyReference.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.ontologyReferencesConnection.edges.map(o => o.node);
                        let pi = response.data.data.ontologyReferencesConnection.pageInfo;
        ...
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.ontologyReference.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  const getDataB = useCallback(() => {
    ...
    /*
      API Request: countItems
    */
    let cancelableApiReq = makeCancelable(api.ontologyReference.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(response => {
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'ontologyReferences' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.observationVariable&&
        ...
        lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.ontologyReference&&
        (lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.ontologyReference.added ||
         lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.ontologyReference.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.ontologyReference.idsRemoved;
          if(idsRemoved) {
    ...
    if(lastModelChanged.ontologyReference) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          let iofB = itemsB.findIndex((item) => item.ontologyDbId===idRemoved);
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.ontologyDbId}</Typography>
                                  </Grid>
