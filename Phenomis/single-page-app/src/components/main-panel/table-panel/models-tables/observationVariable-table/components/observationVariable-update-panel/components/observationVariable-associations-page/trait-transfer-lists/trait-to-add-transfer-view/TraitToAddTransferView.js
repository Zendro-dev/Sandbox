  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated trait ids
      */
      let cancelableApiReq = makeCancelable(api.observationVariable.getAssociatedTraitConnection(graphqlServerUrl, item.observationVariableDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationVariable.trait;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('traitDbId')) ? [idso.traitDbId] : [];
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationVariable&&
        ...
        lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.trait&&
        (lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.trait.added ||
         lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.trait.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.trait.idsRemoved;
          if(idsRemoved) {
