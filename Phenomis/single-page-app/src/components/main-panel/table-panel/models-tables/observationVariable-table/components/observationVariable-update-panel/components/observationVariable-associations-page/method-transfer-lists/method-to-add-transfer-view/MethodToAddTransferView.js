  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated method ids
      */
      let cancelableApiReq = makeCancelable(api.observationVariable.getAssociatedMethodConnection(graphqlServerUrl, item.observationVariableDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set internalId
            let idso = response.data.data.readOneObservationVariable.method;
            lidsAssociated.current = (idso&&typeof idso==='object'&&idso.hasOwnProperty('methodDbId')) ? [idso.methodDbId] : [];
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationVariable&&
        ...
        lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.method&&
        (lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.method.added ||
         lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.method.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.method.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'observationVariables' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'method',
     * but this action happened on the peer relation 'observationVariables'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.method) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationVariables) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationVariables.removed) {
              let idsRemoved = entry[1].changedAssociations.observationVariables.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationVariables.added) {
              let idsAdded = entry[1].changedAssociations.observationVariables.idsAdded;
              if(idsAdded) {
