  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    if(lidsAssociated.current === undefined) {
      /*
        API Request: associated observations ids
      */
      let cancelableApiReq = makeCancelable(api.observationVariable.getAssociatedObservationsConnection(graphqlServerUrl, item.observationVariableDbId));  
      ...
      cancelableApiReq
        ...
        .then(response => {
          ...
          if (
            ...
            //set ids
            let idso = response.data.data.readOneObservationVariable.observationsConnection.edges.map(o => o.node);
            lidsAssociated.current = (idso&&Array.isArray(idso)) ? idso.map(function(item){ return item.observationDbId}) : [];
            ...
            */
            let cancelableApiReqB = makeCancelable(api.observation.getCountItems(graphqlServerUrl, search, ops));
            ...
            cancelableApiReqB
              ...
              .then(response => {
                ...
                if (
                  ...
                  //save response data
                  let newCount = response.data.data.countObservations;
                  ...
                  */
                  ...
                  let cancelableApiReqC = makeCancelable(api.observation.getItemsConnection(
                    graphqlServerUrl,
                    search,
                    null, //orderBy
                    null, //orderDirection
                  ...
                  cancelableApiReqC
                    ...
                    .then(response => {
                      ...
                      if (
                        ...
                        //save response data
                        let its = response.data.data.observationsConnection.edges.map(o => o.node);
                        let pi = response.data.data.observationsConnection.pageInfo;
      ...
      /*
        API Request: countItems
      */
      let cancelableApiReq = makeCancelable(api.observation.getCountItems(graphqlServerUrl, search, ops));
      cancelablePromises.current.push(cancelableApiReq);
      cancelableApiReq
        .promise
        .then(response => {
          ...
          if (
            ...
            */
            ...
            let cancelableApiReqB = makeCancelable(api.observation.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
  ...
  }, [graphqlServerUrl, enqueueSnackbar, t, dataTrigger, item.observationVariableDbId, search, rowsPerPage]);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'observations' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.observationVariable&&
        ...
        lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observations&&
        (lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observations.added ||
         lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observations.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observations.idsRemoved;
          if(idsRemoved) {
    ...
    /*
     * Case 2: 
     * The peer relation 'observationVariable' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'observation',
     * but this action happened on the peer relation 'observationVariable'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationVariable) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationVariable.removed) {
              let idsRemoved = entry[1].changedAssociations.observationVariable.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationVariable.added) {
              let idsAdded = entry[1].changedAssociations.observationVariable.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationVariableDbId, handleUntransfer]);
                            ...
                            <Grid container justify='center' alignItems='center'>
                              <Grid item xs={12}>
                                <Grid container justify='center' alignItems='center' wrap='nowrap'>
                                  ...
                                  {/* InternalId */}
                                  <Grid item>
                                    <Typography className={classes.id} variant="caption" display="block" noWrap={true}>{it.observationDbId}</Typography>
                                  </Grid>
