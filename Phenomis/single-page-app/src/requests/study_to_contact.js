...
export default {
  ...
  tableTemplate(url) {
    let query = `query {csvTableTemplateStudy_to_contact}`
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countStudy_to_contacts(${s}) }`;
    }
    else {
      query = `{ countStudy_to_contacts }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query study_to_contactsConnection($pagination: paginationCursorInput)
            { study_to_contactsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query study_to_contactsConnection($pagination: paginationCursorInput)
            { study_to_contactsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query study_to_contactsConnection($pagination: paginationCursorInput)
          { study_to_contactsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
        ...
        query =
          `query study_to_contactsConnection($pagination: paginationCursorInput)
          { study_to_contactsConnection( pagination: $pagination ) {
            ${qbody}
          }}`
      }
    }
   ...
   * createItem
   * 
   * Add new Study_to_contact item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Study_to_contact item. 
   */
   ...
   * updateItem
   * 
   * Update Study_to_contact item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Study_to_contact item.  
   */
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Study_to_contact item. 
   */
  deleteItem(url, variables) {
    var query = 
      `mutation
        deleteStudy_to_contact(
          $id:ID! 
        ) {
          deleteStudy_to_contact(
            id:$id
   ...
   * 
   * Get contacts connection (cursor based) records associated to the given study_to_contact record
   * through association 'Contact', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  getContactConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    var query = 
      `{ readOneStudy_to_contact(id: ${itemId}) { 
        contact{ 
          contactDbId,
          email,
          instituteName,
   ...
   * 
   *  
   * getAssociatedContactConnection
   * 
   * Get the contact-ids associated (by cursor based connection) to the given study_to_contact record 
   * through association 'Contact', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedContactConnection(url, itemId) {
    var query = 
    `{ readOneStudy_to_contact( id: ${itemId} ){ 
      contact{ contactDbId } } }`;
    /**
   ...
   * getStudyConnection
   * 
   * Get studies connection (cursor based) records associated to the given study_to_contact record
   ...
   * 
   * Get the study-ids associated (by cursor based connection) to the given study_to_contact record 
   * through association 'Study', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
