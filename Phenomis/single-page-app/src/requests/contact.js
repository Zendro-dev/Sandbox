...
export default {
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('contact', searchText, ops);
    
    var query = '';

    //if has search
    if (s !== null) {
      query = `{ countContacts(${s}) }`;
    }
    else {
      query = `{ countContacts }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query contactsConnection($pagination: paginationCursorInput)
            { contactsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query contactsConnection($pagination: paginationCursorInput)
            { contactsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query contactsConnection($pagination: paginationCursorInput)
          { contactsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
    ...
    var query = 
      `mutation
        addContact(
          ...
          $orcid:String,
          $type:String,
          $addContactTostudies: [ID],
          $addContactToTrials: [ID],
          ) { addContact(
            ...
            orcid:$orcid,
            type:$type,
            addContactTostudies: $addContactTostudies,
            addContactToTrials: $addContactToTrials,
          ) {
            contactDbId,
            email,
            instituteName,
    ...
    var query = 
      `mutation
        updateContact(
          ...
          $type:String,
          $addContactTostudies: [ID],
          $removeContactTostudies: [ID],
          $addContactToTrials: [ID],
          $removeContactToTrials: [ID],
          ) { updateContact(
            contactDbId: $contactDbId
            email: $email,
            instituteName: $instituteName,
            ...
            type: $type,
            addContactTostudies: $addContactTostudies,
            removeContactTostudies: $removeContactTostudies,
            addContactToTrials: $addContactToTrials,
            removeContactToTrials: $removeContactToTrials,
          ) {
            contactDbId,
            email,
            instituteName,
  ...
  /**
   * getContactTostudiesConnection
   * 
   * Get study_to_contacts connection (cursor based) records associated to the given contact record
   * through association 'ContactTostudies', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  getContactTostudiesConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneContact($pagination: paginationCursorInput) {
        readOneContact(contactDbId: "${itemId}") { 
          contactTostudiesConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredContactTostudies( ${s} ) 
      } }` :      
      `query readOneContact($pagination: paginationCursorInput) {
        readOneContact(contactDbId: "${itemId}") { 
          contactTostudiesConnection( pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredContactTostudies 
      } }`;
   ...
   * 
   *  
   * getAssociatedContactTostudiesConnection
   * 
   * Get the study_to_contact-ids associated (by cursor based connection) to the given contact record 
   * through association 'ContactTostudies', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedContactTostudiesConnection(url, itemId) {
    var query = 
      `{ readOneContact( contactDbId: "${itemId}" ){ 
        contactTostudiesConnection{ edges { node { id } } } } }`;
    /**
     * Debug
     */
  ...
  /**
   * getContactToTrialsConnection
   * 
   * Get trial_to_contacts connection (cursor based) records associated to the given contact record
   * through association 'ContactToTrials', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   ...
   * @param {String} ops Object with adittional query options.
   */
  getContactToTrialsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneContact($pagination: paginationCursorInput) {
        readOneContact(contactDbId: "${itemId}") { 
          contactToTrialsConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredContactToTrials( ${s} ) 
      } }` :      
      `query readOneContact($pagination: paginationCursorInput) {
        readOneContact(contactDbId: "${itemId}") { 
          contactToTrialsConnection( pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredContactToTrials 
      } }`;
   ...
   * 
   *  
   * getAssociatedContactToTrialsConnection
   * 
   * Get the trial_to_contact-ids associated (by cursor based connection) to the given contact record 
   * through association 'ContactToTrials', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedContactToTrialsConnection(url, itemId) {
    var query = 
      `{ readOneContact( contactDbId: "${itemId}" ){ 
        contactToTrialsConnection{ edges { node { id } } } } }`;
    /**
     * Debug
     */
    console.log("getAssociatedIds.query: gql:\n", query);
