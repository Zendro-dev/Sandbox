...
export default {
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countEventParameters(${s}) }`;
    }
    else {
      query = `{ countEventParameters }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query eventParametersConnection($pagination: paginationCursorInput)
            { eventParametersConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
      ...
      if (o != null) {
        ...
        query =
          `query eventParametersConnection($pagination: paginationCursorInput)
          { eventParametersConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * createItem
   * 
   * Add new EventParameter item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new EventParameter item. 
   */
   ...
   * updateItem
   * 
   * Update EventParameter item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given EventParameter item.  
   */
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the EventParameter item. 
   */
   ...
   * getEventConnection
   * 
   * Get events connection (cursor based) records associated to the given eventParameter record
  ...
  getEventConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    var query = 
      `{ readOneEventParameter(eventParameterDbId: "${itemId}") { 
        event{ 
          eventDbId,
          eventDescription,
          eventType,
   ...
   * 
   * Get the event-ids associated (by cursor based connection) to the given eventParameter record 
   * through association 'Event', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
