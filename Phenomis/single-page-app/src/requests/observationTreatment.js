...
export default {
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countObservationTreatments(${s}) }`;
    }
    else {
      query = `{ countObservationTreatments }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query observationTreatmentsConnection($pagination: paginationCursorInput)
            { observationTreatmentsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
      ...
      if (o != null) {
        ...
        query =
          `query observationTreatmentsConnection($pagination: paginationCursorInput)
          { observationTreatmentsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * 
   * Get observationUnits connection (cursor based) records associated to the given observationTreatment record
   ...
   * 
   * Get the observationUnit-ids associated (by cursor based connection) to the given observationTreatment record 
   * through association 'ObservationUnit', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
