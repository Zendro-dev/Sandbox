...
export default {
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('observationUnitPosition', searchText, ops);
    
    var query = '';

    //if has search
    if (s !== null) {
      query = `{ countObservationUnitPositions(${s}) }`;
    }
    else {
      query = `{ countObservationUnitPositions }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query observationUnitPositionsConnection($pagination: paginationCursorInput)
            { observationUnitPositionsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query observationUnitPositionsConnection($pagination: paginationCursorInput)
            { observationUnitPositionsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query observationUnitPositionsConnection($pagination: paginationCursorInput)
          { observationUnitPositionsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * 
   * Get observationUnits connection (cursor based) records associated to the given observationUnitPosition record
   ...
   * 
   * Get the observationUnit-ids associated (by cursor based connection) to the given observationUnitPosition record 
   * through association 'ObservationUnit', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
