...
export default {
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countEnvironmentParameters(${s}) }`;
    }
    else {
      query = `{ countEnvironmentParameters }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query environmentParametersConnection($pagination: paginationCursorInput)
            { environmentParametersConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
      ...
      if (o != null) {
        ...
        query =
          `query environmentParametersConnection($pagination: paginationCursorInput)
          { environmentParametersConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * createItem
   * 
   * Add new EnvironmentParameter item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new EnvironmentParameter item. 
   */
   ...
   * updateItem
   * 
   * Update EnvironmentParameter item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given EnvironmentParameter item.  
   */
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the EnvironmentParameter item. 
   */
   ...
   * getStudyConnection
   * 
   * Get studies connection (cursor based) records associated to the given environmentParameter record
  ...
  getStudyConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    var query = 
      `{ readOneEnvironmentParameter(environmentParameterDbId: "${itemId}") { 
        study{ 
          active,
          commonCropName,
          culturalPractices,
   ...
   * 
   * Get the study-ids associated (by cursor based connection) to the given environmentParameter record 
   * through association 'Study', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
