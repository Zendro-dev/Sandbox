...
export default {
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('ontologyReference', searchText, ops);
    
    var query = '';

    //if has search
    if (s !== null) {
      query = `{ countOntologyReferences(${s}) }`;
    }
    else {
      query = `{ countOntologyReferences }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query ontologyReferencesConnection($pagination: paginationCursorInput)
            { ontologyReferencesConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query ontologyReferencesConnection($pagination: paginationCursorInput)
            { ontologyReferencesConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query ontologyReferencesConnection($pagination: paginationCursorInput)
          { ontologyReferencesConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
  ...
  /**
   * getMethodsConnection
   * 
   * Get methods connection (cursor based) records associated to the given ontologyReference record
   ...
   * @param {String} ops Object with adittional query options.
   */
  getMethodsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    var s = getSearchArgument('method', searchText, ops); 
    ...
    var query = (s) ?
      `query readOneOntologyReference($pagination: paginationCursorInput) {
        readOneOntologyReference(ontologyDbId: "${itemId}") { 
          methodsConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredMethods( ${s} ) 
      } }` :      
      `query readOneOntologyReference($pagination: paginationCursorInput) {
        readOneOntologyReference(ontologyDbId: "${itemId}") { 
   ...
   * 
   *  
   * getAssociatedMethodsConnection
   * 
   * Get the method-ids associated (by cursor based connection) to the given ontologyReference record 
   * through association 'Methods', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedMethodsConnection(url, itemId) {
    var query = 
      `{ readOneOntologyReference( ontologyDbId: "${itemId}" ){ 
        methodsConnection{ edges { node { methodDbId } } } } }`;
    /**
     * Debug
     */
  ...
  /**
   * getObservationVariablesConnection
   * 
   * Get observationVariables connection (cursor based) records associated to the given ontologyReference record
   ...
   * @param {String} ops Object with adittional query options.
   */
  getObservationVariablesConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneOntologyReference($pagination: paginationCursorInput) {
        readOneOntologyReference(ontologyDbId: "${itemId}") { 
          ...
          countFilteredObservationVariables( ${s} ) 
      } }` :      
      `query readOneOntologyReference($pagination: paginationCursorInput) {
        readOneOntologyReference(ontologyDbId: "${itemId}") { 
          ...
          countFilteredObservationVariables 
      } }`;
   ...
   * 
   *  
   * getAssociatedObservationVariablesConnection
   * 
   * Get the observationVariable-ids associated (by cursor based connection) to the given ontologyReference record 
   * through association 'ObservationVariables', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedObservationVariablesConnection(url, itemId) {
    var query = 
      `{ readOneOntologyReference( ontologyDbId: "${itemId}" ){ 
        observationVariablesConnection{ edges { node { observationVariableDbId } } } } }`;
    /**
     * Debug
     */
    console.log("getAssociatedIds.query: gql:\n", query);
   ...
   * getScalesConnection
   * 
   * Get scales connection (cursor based) records associated to the given ontologyReference record
   ...
   * @param {String} ops Object with adittional query options.
   */
  getScalesConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    var s = getSearchArgument('scale', searchText, ops); 
    ...
    var query = (s) ?
      `query readOneOntologyReference($pagination: paginationCursorInput) {
        readOneOntologyReference(ontologyDbId: "${itemId}") { 
          scalesConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredScales( ${s} ) 
      } }` :      
      `query readOneOntologyReference($pagination: paginationCursorInput) {
        readOneOntologyReference(ontologyDbId: "${itemId}") { 
   ...
   * 
   *  
   * getAssociatedScalesConnection
   * 
   * Get the scale-ids associated (by cursor based connection) to the given ontologyReference record 
   * through association 'Scales', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedScalesConnection(url, itemId) {
    var query = 
      `{ readOneOntologyReference( ontologyDbId: "${itemId}" ){ 
        scalesConnection{ edges { node { scaleDbId } } } } }`;
    /**
     * Debug
     */
    console.log("getAssociatedIds.query: gql:\n", query);
   ...
   * getTraitsConnection
   * 
   * Get traits connection (cursor based) records associated to the given ontologyReference record
   ...
   * @param {String} ops Object with adittional query options.
   */
  getTraitsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    var s = getSearchArgument('trait', searchText, ops); 
    ...
    var query = (s) ?
      `query readOneOntologyReference($pagination: paginationCursorInput) {
        readOneOntologyReference(ontologyDbId: "${itemId}") { 
          traitsConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredTraits( ${s} ) 
      } }` :      
      `query readOneOntologyReference($pagination: paginationCursorInput) {
        readOneOntologyReference(ontologyDbId: "${itemId}") { 
   ...
   * 
   *  
   * getAssociatedTraitsConnection
   * 
   * Get the trait-ids associated (by cursor based connection) to the given ontologyReference record 
   * through association 'Traits', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedTraitsConnection(url, itemId) {
    var query = 
      `{ readOneOntologyReference( ontologyDbId: "${itemId}" ){ 
        traitsConnection{ edges { node { traitDbId } } } } }`;
    /**
     * Debug
     */
    console.log("getAssociatedIds.query: gql:\n", query);
