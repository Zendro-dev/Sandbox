...
export default {
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('breedingMethod', searchText, ops);
    
    var query = '';

    //if has search
    if (s !== null) {
      query = `{ countBreedingMethods(${s}) }`;
    }
    else {
      query = `{ countBreedingMethods }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query breedingMethodsConnection($pagination: paginationCursorInput)
            { breedingMethodsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query breedingMethodsConnection($pagination: paginationCursorInput)
            { breedingMethodsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query breedingMethodsConnection($pagination: paginationCursorInput)
          { breedingMethodsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
   ...
   * 
   * Get germplasms connection (cursor based) records associated to the given breedingMethod record
  ...
  getGermplasmConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    ...
    var query = (s) ?
      `query readOneBreedingMethod($pagination: paginationCursorInput) {
        readOneBreedingMethod(breedingMethodDbId: "${itemId}") { 
          germplasmConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredGermplasm( ${s} ) 
      } }` :      
      `query readOneBreedingMethod($pagination: paginationCursorInput) {
        readOneBreedingMethod(breedingMethodDbId: "${itemId}") { 
          germplasmConnection( pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredGermplasm 
      } }`;
   ...
   * 
   * Get the germplasm-ids associated (by cursor based connection) to the given breedingMethod record 
   * through association 'Germplasm', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
  ...
  getAssociatedGermplasmConnection(url, itemId) {
    var query = 
      `{ readOneBreedingMethod( breedingMethodDbId: "${itemId}" ){ 
        germplasmConnection{ edges { node { germplasmDbId } } } } }`;
    /**
     * Debug
     */
