...
export default {
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query studiesConnection($pagination: paginationCursorInput)
            { studiesConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
      ...
      if (o != null) {
        ...
        query =
          `query studiesConnection($pagination: paginationCursorInput)
          { studiesConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
    ...
    var query = 
      `mutation
        addStudy(
          ...
          $addStudyToContacts: [ID],
          $addStudyToSeasons: [ID],
          ) { addStudy(
            studyDbId:$studyDbId,
            active:$active,
            ...
            addStudyToContacts: $addStudyToContacts,
            addStudyToSeasons: $addStudyToSeasons,
          ) {
            active,
            commonCropName,
            culturalPractices,
    ...
    var query = 
      `mutation
        updateStudy(
          ...
          $addStudyToContacts: [ID],
          $removeStudyToContacts: [ID],
          $addStudyToSeasons: [ID],
          $removeStudyToSeasons: [ID],
          ) { updateStudy(
            studyDbId: $studyDbId
            active: $active,
            commonCropName: $commonCropName,
            ...
            addStudyToContacts: $addStudyToContacts,
            removeStudyToContacts: $removeStudyToContacts,
            addStudyToSeasons: $addStudyToSeasons,
            removeStudyToSeasons: $removeStudyToSeasons,
          ) {
            active,
            commonCropName,
            culturalPractices,
  ...
  /**
   * getEnvironmentParametersConnection
   * 
   * Get environmentParameters connection (cursor based) records associated to the given study record
   ...
   * @param {String} ops Object with adittional query options.
   */
  getEnvironmentParametersConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    var s = getSearchArgument('environmentParameter', searchText, ops); 
    ...
    var query = (s) ?
      `query readOneStudy($pagination: paginationCursorInput) {
        readOneStudy(studyDbId: "${itemId}") { 
          environmentParametersConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredEnvironmentParameters( ${s} ) 
      } }` :      
      `query readOneStudy($pagination: paginationCursorInput) {
        readOneStudy(studyDbId: "${itemId}") { 
          environmentParametersConnection( pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredEnvironmentParameters 
      } }`;
   ...
   * 
   *  
   * getAssociatedEnvironmentParametersConnection
   * 
   * Get the environmentParameter-ids associated (by cursor based connection) to the given study record 
   * through association 'EnvironmentParameters', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedEnvironmentParametersConnection(url, itemId) {
    var query = 
      `{ readOneStudy( studyDbId: "${itemId}" ){ 
        environmentParametersConnection{ edges { node { environmentParameterDbId } } } } }`;
    /**
     * Debug
     */
   ...
   * getEventsConnection
   * 
   * Get events connection (cursor based) records associated to the given study record
   ...
   * @param {String} ops Object with adittional query options.
   */
  getEventsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
   ...
   * 
   *  
   * getAssociatedEventsConnection
   * 
   * Get the event-ids associated (by cursor based connection) to the given study record 
   * through association 'Events', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedEventsConnection(url, itemId) {
    var query = 
      `{ readOneStudy( studyDbId: "${itemId}" ){ 
        eventsConnection{ edges { node { eventType } } } } }`;
    /**
     * Debug
     */
    console.log("getAssociatedIds.query: gql:\n", query);
   ...
   * 
   * Get locations connection (cursor based) records associated to the given study record
   ...
   * 
   * Get the location-ids associated (by cursor based connection) to the given study record 
   * through association 'Location', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   ...
   * 
   * Get observations connection (cursor based) records associated to the given study record
   ...
   * 
   * Get the observation-ids associated (by cursor based connection) to the given study record 
   * through association 'Observations', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   ...
   * 
   * Get observationUnits connection (cursor based) records associated to the given study record
   ...
   * 
   * Get the observationUnit-ids associated (by cursor based connection) to the given study record 
   * through association 'ObservationUnits', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
  ...
  /**
   * getStudyToContactsConnection
   * 
   * Get study_to_contacts connection (cursor based) records associated to the given study record
   * through association 'StudyToContacts', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   ...
   * @param {String} ops Object with adittional query options.
   */
  getStudyToContactsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    var s = getSearchArgument('study_to_contact', searchText, ops); 
    ...
    var query = (s) ?
      `query readOneStudy($pagination: paginationCursorInput) {
        readOneStudy(studyDbId: "${itemId}") { 
          studyToContactsConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredStudyToContacts( ${s} ) 
      } }` :      
      `query readOneStudy($pagination: paginationCursorInput) {
        readOneStudy(studyDbId: "${itemId}") { 
          studyToContactsConnection( pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredStudyToContacts 
      } }`;
   ...
   * 
   *  
   * getAssociatedStudyToContactsConnection
   * 
   * Get the study_to_contact-ids associated (by cursor based connection) to the given study record 
   * through association 'StudyToContacts', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedStudyToContactsConnection(url, itemId) {
    var query = 
      `{ readOneStudy( studyDbId: "${itemId}" ){ 
        studyToContactsConnection{ edges { node { id } } } } }`;
    /**
     * Debug
     */
    console.log("getAssociatedIds.query: gql:\n", query);
  ...
  /**
   * getStudyToSeasonsConnection
   * 
   * Get study_to_seasons connection (cursor based) records associated to the given study record
   * through association 'StudyToSeasons', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   ...
   * @param {String} ops Object with adittional query options.
   */
  getStudyToSeasonsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    ...
    var query = (s) ?
      `query readOneStudy($pagination: paginationCursorInput) {
        readOneStudy(studyDbId: "${itemId}") { 
          studyToSeasonsConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredStudyToSeasons( ${s} ) 
      } }` :      
      `query readOneStudy($pagination: paginationCursorInput) {
        readOneStudy(studyDbId: "${itemId}") { 
          studyToSeasonsConnection( pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredStudyToSeasons 
      } }`;
   ...
   * 
   *  
   * getAssociatedStudyToSeasonsConnection
   * 
   * Get the study_to_season-ids associated (by cursor based connection) to the given study record 
   * through association 'StudyToSeasons', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedStudyToSeasonsConnection(url, itemId) {
    var query = 
      `{ readOneStudy( studyDbId: "${itemId}" ){ 
        studyToSeasonsConnection{ edges { node { id } } } } }`;
    /**
     * Debug
     */
    console.log("getAssociatedIds.query: gql:\n", query);
   ...
   * getTrialConnection
   * 
   * Get trials connection (cursor based) records associated to the given study record
   ...
   * 
   * Get the trial-ids associated (by cursor based connection) to the given study record 
   * through association 'Trial', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
