...
export default {
  ...
  getCountItems(url, searchText, ops) {
    ...
    //if has search
    if (s !== null) {
      query = `{ countTrial_to_contacts(${s}) }`;
    }
    else {
      query = `{ countTrial_to_contacts }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query trial_to_contactsConnection($pagination: paginationCursorInput)
            { trial_to_contactsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query trial_to_contactsConnection($pagination: paginationCursorInput)
            { trial_to_contactsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query trial_to_contactsConnection($pagination: paginationCursorInput)
          { trial_to_contactsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
        ...
        query =
          `query trial_to_contactsConnection($pagination: paginationCursorInput)
          { trial_to_contactsConnection( pagination: $pagination ) {
            ${qbody}
          }}`
      }
    }
   ...
   * createItem
   * 
   * Add new Trial_to_contact item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Trial_to_contact item. 
   */
   ...
   * updateItem
   * 
   * Update Trial_to_contact item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Trial_to_contact item.  
   */
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Trial_to_contact item. 
   */
   ...
   * 
   * Get contacts connection (cursor based) records associated to the given trial_to_contact record
  ...
  getContactConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    var query = 
      `{ readOneTrial_to_contact(id: ${itemId}) { 
        contact{ 
          contactDbId,
          email,
          instituteName,
   ...
   * 
   * Get the contact-ids associated (by cursor based connection) to the given trial_to_contact record 
   * through association 'Contact', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   ...
   * getTrialConnection
   * 
   * Get trials connection (cursor based) records associated to the given trial_to_contact record
   ...
   * 
   * Get the trial-ids associated (by cursor based connection) to the given trial_to_contact record 
   * through association 'Trial', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
