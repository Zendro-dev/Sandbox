...
app.use(bodyParser.json({
  limit: globals.POST_REQUEST_MAX_BODY_SIZE
}));
...
app.use('/login', cors(), (req, res) => {
  ...
  auth.login(req.body).then((token) => {
    ...
    console.log(err);
    res.status(500).send({
      error: "Wrong email or password. Please check your credentials."
    })
  });
...
app.use('/join', cors(), (req, res) => {
  ...
  if (!req.is('application/json'))
    return res.status(415).send({
      error: "JSON Content-Type expected"
    });
  ...
  if (params.outputFormat === 'TEST') {
    ...
    return res.status(415).send({
      error: "outputFormat = TEST/CSV/JSON is required"
    });
  }
...
app.use('/export', cors(), (req, res, next) => {
  ...
  simpleExport(context, body_info, res).then(() => {
    ...
    if (!res.responseSent) {
      ...
      console.error("ERROR IN EXPORT AFTER SENDING THE RESPONSE:",
        error);
    }
...
function eitherJqOrJsonpath(jqInput, jsonpathInput) {
  ...
  if (helper.isNotUndefinedAndNotNull(jqInput) && helper.isNotUndefinedAndNotNull(
      jsonpathInput)) {
    throw new Error(errorString + " - jq is " + jqInput + " and jsonPath is " +
      jsonpathInput);
  }
  if (!helper.isNotUndefinedAndNotNull(jqInput) && !helper.isNotUndefinedAndNotNull(
      jsonpathInput)) {
  ...
  try {
    ...
    if (req != null) {
      if (await checkAuthorization(context, 'meta_query', '') === true) {
        ...
        let graphQlResponses = await handleGraphQlQueriesForMetaQuery(
          queries, context);
        let output;
        if (helper.isNotUndefinedAndNotNull(jq)) { // jq
          output = await nodejq.run(jq, graphQlResponses, {
            input: 'json',
            output: 'json'
          });
        } else { // JSONPath
    ...
    res.json({
      data: null,
      errors: [graphqlFormatError.formatError(error)]
    });
  }
});
