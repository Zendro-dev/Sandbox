...
async function validForDeletion(id, context) {
    if (await countAssociatedRecordsWithRejectReaction(id, context) > 0) {
        throw new Error(`snpmatrix with id ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`);
    }
...
module.exports = {
    ...
    snpmatricesConnection: async function({
        ...
        //check: adapters
        let registeredAdapters = Object.values(snpmatrix.registeredAdapters);
        if (registeredAdapters.length === 0) {
            throw new Error('No adapters registered for data model "snpmatrix"');
        } //else

        //exclude adapters
        let adapters = helper.removeExcludedAdapters(search, registeredAdapters);
        if (adapters.length === 0) {
            throw new Error('All adapters was excluded for data model "snpmatrix"');
        } //else
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            return await snpmatrix.readAllCursor(search, order, pagination, authorizationCheck.authorizedAdapters, context.benignErrors, searchAuthorizationCheck.authorizedAdapters, token);
        } else { //adapters not auth || errors
            // else new Error
            if (authorizationCheck.authorizationErrors.length > 0) {
                throw new Error(authorizationCheck.authorizationErrors.reduce((a, c) => `${a}, ${c.message}`));
            } else {
                throw new Error('No available adapters for data model "snpmatrix" ');
            }
        }
    },
    ...
    readOneSnpmatrix: async function({
        ...
        //check: adapters auth
        let authorizationCheck = await checkAuthorization(context, snpmatrix.adapterForIri(id), 'read');
        if (authorizationCheck === true) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            return snpmatrix.readById(id, context.benignErrors, token);
    ...
    addSnpmatrix: async function(input, context) {
        ...
        //check: adapters auth
        let authorizationCheck = await checkAuthorization(context, snpmatrix.adapterForIri(input.id), 'create');
        if (authorizationCheck === true) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            let createdRecord = await snpmatrix.addOne(inputSanitized, context.benignErrors, token);
    ...
    deleteSnpmatrix: async function({
        ...
        //check: adapters auth
        let authorizationCheck = await checkAuthorization(context, snpmatrix.adapterForIri(id), 'delete');
        if (authorizationCheck === true) {
    ...
    updateSnpmatrix: async function(input, context) {
        ...
        //check: adapters auth
        let authorizationCheck = await checkAuthorization(context, snpmatrix.adapterForIri(input.id), 'update');
        if (authorizationCheck === true) {
            let inputSanitized = helper.sanitizeAssociationArguments(input, [Object.keys(associationArgsDef)]);
            await helper.checkAuthorizationOnAssocArgs(inputSanitized, context, associationArgsDef, ['read', 'update'], models);
            await helper.checkAndAdjustRecordLimitForCreateUpdate(inputSanitized, context, associationArgsDef);
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            let updatedRecord = await snpmatrix.updateOne(inputSanitized, context.benignErrors, token);
    ...
    countSnpmatrices: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            return await snpmatrix.countRecords(search, authorizationCheck.authorizedAdapters, context.benignErrors, searchAuthorizationCheck.authorizedAdapters, token);
        } else { //adapters not auth || errors
            // else new Error
            if (authorizationCheck.authorizationErrors.length > 0) {
                throw new Error(authorizationCheck.authorizationErrors.reduce((a, c) => `${a}, ${c.message}`));
    ...
    csvTableTemplateSnpmatrix: async function(_, context) {
        if (await checkAuthorization(context, 'snpmatrix', 'read') === true) {
            return snpmatrix.csvTableTemplate(context.benignErrors);
        } else {
    ...
    validateSnpmatrixForCreation: async (input, context) => {
        ...
        let authorization = await checkAuthorization(context, snpmatrix.adapterForIri(input.id), 'read');
    ...
    validateSnpmatrixForDeletion: async ({
        ...
        if ((await checkAuthorization(context, snpmatrix.adapterForIri(id), 'read')) === true) {
