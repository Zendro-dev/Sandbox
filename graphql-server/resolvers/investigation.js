...
module.exports = {
    ...
    investigationsConnection: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            return await investigation.readAllCursor(search, order, pagination, authorizationCheck.authorizedAdapters, benignErrorReporter, searchAuthorizationCheck.authorizedAdapters);
    ...
    readOneInvestigation: async function({
        ...
        if (authorizationCheck === true) {
            ...
            return investigation.readById(id, benignErrorReporter);
    ...
    addInvestigation: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            let createdRecord = await investigation.addOne(inputSanitized, benignErrorReporter);
    ...
    updateInvestigation: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            //construct benignErrors reporter with context
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            let updatedRecord = await investigation.updateOne(inputSanitized, benignErrorReporter);
    ...
    countInvestigations: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let searchAuthorizationCheck = await helper.authorizedAdapters(context, adapters, 'search');
            return await investigation.countRecords(search, authorizationCheck.authorizedAdapters, benignErrorReporter, searchAuthorizationCheck.authorizedAdapters);
        } else { //adapters not auth || errors
            // else new Error
            if (authorizationCheck.authorizationErrors.length > 0) {
                throw new Error(authorizationCheck.authorizationErrors.reduce((a, c) => `${a}, ${c.message}`));
