...
async function validForDeletion(id, context) {
    if (await countAssociatedRecordsWithRejectReaction(id, context) > 0) {
        throw new Error(`snplocus with id ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`);
    }
...
module.exports = {
    ...
    snplocusConnection: async function({
        ...
        //check: adapters
        let registeredAdapters = Object.values(snplocus.registeredAdapters);
        if (registeredAdapters.length === 0) {
            throw new Error('No adapters registered for data model "snplocus"');
        } //else

        //exclude adapters
        let adapters = helper.removeExcludedAdapters(search, registeredAdapters);
        if (adapters.length === 0) {
            throw new Error('All adapters was excluded for data model "snplocus"');
        } //else
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            return await snplocus.readAllCursor(search, order, pagination, authorizationCheck.authorizedAdapters, context.benignErrors, searchAuthorizationCheck.authorizedAdapters, token);
        } else { //adapters not auth || errors
            // else new Error
            if (authorizationCheck.authorizationErrors.length > 0) {
                throw new Error(authorizationCheck.authorizationErrors.reduce((a, c) => `${a}, ${c.message}`));
            } else {
                throw new Error('No available adapters for data model "snplocus" ');
            }
        }
    },
    ...
    readOneSnplocus: async function({
        ...
        //check: adapters auth
        let authorizationCheck = await checkAuthorization(context, snplocus.adapterForIri(id), 'read');
        if (authorizationCheck === true) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            return snplocus.readById(id, context.benignErrors, token);
    ...
    addSnplocus: async function(input, context) {
        ...
        //check: adapters auth
        let authorizationCheck = await checkAuthorization(context, snplocus.adapterForIri(input.id), 'create');
        if (authorizationCheck === true) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            let createdRecord = await snplocus.addOne(inputSanitized, context.benignErrors, token);
    ...
    deleteSnplocus: async function({
        ...
        //check: adapters auth
        let authorizationCheck = await checkAuthorization(context, snplocus.adapterForIri(id), 'delete');
        if (authorizationCheck === true) {
    ...
    updateSnplocus: async function(input, context) {
        ...
        //check: adapters auth
        let authorizationCheck = await checkAuthorization(context, snplocus.adapterForIri(input.id), 'update');
        if (authorizationCheck === true) {
            let inputSanitized = helper.sanitizeAssociationArguments(input, [Object.keys(associationArgsDef)]);
            await helper.checkAuthorizationOnAssocArgs(inputSanitized, context, associationArgsDef, ['read', 'update'], models);
            await helper.checkAndAdjustRecordLimitForCreateUpdate(inputSanitized, context, associationArgsDef);
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            let updatedRecord = await snplocus.updateOne(inputSanitized, context.benignErrors, token);
    ...
    countSnplocus: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            return await snplocus.countRecords(search, authorizationCheck.authorizedAdapters, context.benignErrors, searchAuthorizationCheck.authorizedAdapters, token);
        } else { //adapters not auth || errors
            // else new Error
            if (authorizationCheck.authorizationErrors.length > 0) {
                throw new Error(authorizationCheck.authorizationErrors.reduce((a, c) => `${a}, ${c.message}`));
    ...
    csvTableTemplateSnplocus: async function(_, context) {
        if (await checkAuthorization(context, 'snplocus', 'read') === true) {
            return snplocus.csvTableTemplate(context.benignErrors);
        } else {
    ...
    validateSnplocusForCreation: async (input, context) => {
        ...
        let authorization = await checkAuthorization(context, snplocus.adapterForIri(input.id), 'read');
    ...
    validateSnplocusForDeletion: async ({
        ...
        if ((await checkAuthorization(context, snplocus.adapterForIri(id), 'read')) === true) {
