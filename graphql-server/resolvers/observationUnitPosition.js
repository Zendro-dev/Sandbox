 ...
 */
observationUnitPosition.prototype.handleAssociations = async function(input, benignErrorReporter) {
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
observationUnitPosition.prototype.add_observationUnit = async function(input, benignErrorReporter) {
    await observationUnitPosition.add_observationUnitDbId(this.getIdValue(), input.addObservationUnit, benignErrorReporter);
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
observationUnitPosition.prototype.remove_observationUnit = async function(input, benignErrorReporter) {
    if (input.removeObservationUnit == this.observationUnitDbId) {
        await observationUnitPosition.remove_observationUnitDbId(this.getIdValue(), input.removeObservationUnit, benignErrorReporter);
...
module.exports = {
    ...
    observationUnitPositions: async function({
        ...
        if (await checkAuthorization(context, 'observationUnitPosition', 'read') === true) {
            ...
            return await observationUnitPosition.readAll(search, order, pagination, benignErrorReporter);
        } else {
    ...
    observationUnitPositionsConnection: async function({
        ...
        if (await checkAuthorization(context, 'observationUnitPosition', 'read') === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await observationUnitPosition.readAllCursor(search, order, pagination, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    readOneObservationUnitPosition: async function({
        ...
        if (await checkAuthorization(context, 'observationUnitPosition', 'read') === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await observationUnitPosition.readById(observationUnitPositionDbId, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    countObservationUnitPositions: async function({
        ...
        if (await checkAuthorization(context, 'observationUnitPosition', 'read') === true) {
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await observationUnitPosition.countRecords(search, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    /**
     * vueTableObservationUnitPosition - Return table of records as needed for displaying a vuejs table
     *
     * @param  {string} _       First parameter is not used
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     * @return {object}         Records with format as needed for displaying a vuejs table
     */
    vueTableObservationUnitPosition: async function(_, context) {
        if (await checkAuthorization(context, 'observationUnitPosition', 'read') === true) {
            return helper.vueTable(context.request, observationUnitPosition, ["id", "blockNumber", "entryNumber", "positionCoordinateX", "positionCoordinateY", "replicate", "observationUnitDbId", "observationUnitPositionDbId"]);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    addObservationUnitPosition: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            let createdObservationUnitPosition = await observationUnitPosition.addOne(inputSanitized, benignErrorReporter);
            await createdObservationUnitPosition.handleAssociations(inputSanitized, benignErrorReporter);
     ...
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     */
    bulkAddObservationUnitPositionCsv: async function(_, context) {
        if (await checkAuthorization(context, 'observationUnitPosition', 'create') === true) {
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return observationUnitPosition.bulkAddCsv(context, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    deleteObservationUnitPosition: async function({
        ...
        if (await checkAuthorization(context, 'observationUnitPosition', 'delete') === true) {
            if (await validForDeletion(observationUnitPositionDbId, context)) {
                let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
                return observationUnitPosition.deleteOne(observationUnitPositionDbId, benignErrorReporter);
            }
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    ...
    updateObservationUnitPosition: async function(input, context) {
        ...
        if (authorization === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            let updatedObservationUnitPosition = await observationUnitPosition.updateOne(inputSanitized, benignErrorReporter);
            await updatedObservationUnitPosition.handleAssociations(inputSanitized, benignErrorReporter);
    ...
    bulkAssociateObservationUnitPositionWithObservationUnitDbId: async function(bulkAssociationInput, context) {
        ...
        if (!bulkAssociationInput.skipAssociationsExistenceChecks) {
            ...
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
                observationUnitPositionDbId
            }) => observationUnitPositionDbId)), observationUnitPosition);
        }
        return await observationUnitPosition.bulkAssociateObservationUnitPositionWithObservationUnitDbId(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
    /**
    ...
    bulkDisAssociateObservationUnitPositionWithObservationUnitDbId: async function(bulkAssociationInput, context) {
        ...
        return await observationUnitPosition.bulkDisAssociateObservationUnitPositionWithObservationUnitDbId(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
    ...
    csvTableTemplateObservationUnitPosition: async function(_, context) {
        if (await checkAuthorization(context, 'observationUnitPosition', 'read') === true) {
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return observationUnitPosition.csvTableTemplate(benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    }
