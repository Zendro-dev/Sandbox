...
module.exports = {
    ...
    factorsConnection: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            return await factor.readAllCursor(search, order, pagination, authorizationCheck.authorizedAdapters, benignErrorReporter, searchAuthorizationCheck.authorizedAdapters);
    ...
    readOneFactor: async function({
        ...
        if (authorizationCheck === true) {
            ...
            return factor.readById(id, benignErrorReporter);
    ...
    addFactor: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            let createdRecord = await factor.addOne(inputSanitized, benignErrorReporter);
    ...
    updateFactor: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            //construct benignErrors reporter with context
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            let updatedRecord = await factor.updateOne(inputSanitized, benignErrorReporter);
    ...
    countFactors: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let searchAuthorizationCheck = await helper.authorizedAdapters(context, adapters, 'search');
            return await factor.countRecords(search, authorizationCheck.authorizedAdapters, benignErrorReporter, searchAuthorizationCheck.authorizedAdapters);
        } else { //adapters not auth || errors
            // else new Error
            if (authorizationCheck.authorizationErrors.length > 0) {
                throw new Error(authorizationCheck.authorizationErrors.reduce((a, c) => `${a}, ${c.message}`));
