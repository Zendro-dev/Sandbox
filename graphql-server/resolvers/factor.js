...
factor.prototype.add_fileAttachments = async function(input, benignErrorReporter) {
    ...
    let bulkAssociationInput = input.addFileAttachments.map(associatedRecordId => {
        return {
            factor_id: this.getIdValue(),
            [models.fileAttachment.idAttribute()]: associatedRecordId
        }
    });
    await models.fileAttachment.bulkAssociateFileAttachmentWithFactor_id(bulkAssociationInput, benignErrorReporter);
}
...
factor.prototype.remove_fileAttachments = async function(input, benignErrorReporter) {
    ...
    await models.fileAttachment.bulkDisAssociateFileAttachmentWithFactor_id(bulkAssociationInput, benignErrorReporter);
}
