...
module.exports = {
    ...
    studiesConnection: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            return await study.readAllCursor(search, order, pagination, authorizationCheck.authorizedAdapters, benignErrorReporter, searchAuthorizationCheck.authorizedAdapters);
    ...
    readOneStudy: async function({
        ...
        if (authorizationCheck === true) {
            ...
            return study.readById(id, benignErrorReporter);
    ...
    addStudy: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            let createdRecord = await study.addOne(inputSanitized, benignErrorReporter);
    ...
    updateStudy: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            //construct benignErrors reporter with context
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            let updatedRecord = await study.updateOne(inputSanitized, benignErrorReporter);
    ...
    countStudies: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let searchAuthorizationCheck = await helper.authorizedAdapters(context, adapters, 'search');
            return await study.countRecords(search, authorizationCheck.authorizedAdapters, benignErrorReporter, searchAuthorizationCheck.authorizedAdapters);
        } else { //adapters not auth || errors
            // else new Error
            if (authorizationCheck.authorizationErrors.length > 0) {
                throw new Error(authorizationCheck.authorizationErrors.reduce((a, c) => `${a}, ${c.message}`));
