...
/**
 * taxon.prototype.registro_siagroFilter - Check user authorization and return certain number, specified in pagination argument, of records
 ...
 */
taxon.prototype.registro_siagroFilter = function({
    search,
    order,
    pagination
}, context) {
...
/**
 * taxon.prototype.countFilteredRegistro_siagro - Count number of associated records that holds the conditions specified in the search argument
 *
 * @param  {object} {search} description
 * @param  {object} context  Provided to every resolver holds contextual information like the resquest query and user info.
 * @return {type}          Number of associated records that holds the conditions specified in the search argument
 */
taxon.prototype.countFilteredRegistro_siagro = function({
    search
}, context) {
...
/**
 * taxon.prototype.registro_siagroConnection - Check user authorization and return certain number, specified in pagination argument, of records
 * associated with the current instance, this records should also
 * holds the condition of search argument, all of them sorted as specified by the order argument.
 *
 * @param  {object} search     Search argument for filtering associated records
 ...
 */
taxon.prototype.registro_siagroConnection = function({
    search,
    order,
    pagination
}, context) {
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
taxon.prototype.add_registro_siagro = async function(input, benignErrorReporter) {

    let bulkAssociationInput = input.addRegistro_siagro.map(associatedRecordId => {
        return {
            snib_id: this.getIdValue(),
            [models.registro_siagro.idAttribute()]: associatedRecordId
        }
    });
    await models.registro_siagro.bulkAssociateRegistro_siagroWithSnib_id(bulkAssociationInput, benignErrorReporter);
}
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
taxon.prototype.remove_registro_siagro = async function(input, benignErrorReporter) {
    ...
    await models.registro_siagro.bulkDisAssociateRegistro_siagroWithSnib_id(bulkAssociationInput, benignErrorReporter);
}
...
async function countAllAssociatedRecords(id, context) {
    ...
    promises_to_many.push(taxon.countFilteredRegistro_siagro({}, context));
...
module.exports = {
    ...
    vueTableTaxon: async function(_, context) {
        if (await checkAuthorization(context, 'taxon', 'read') === true) {
            return helper.vueTable(context.request, taxon, ["id", "Genero", "EpitetoEspecifico", "EpitetoSubespecie", "EpitetoVariedad", "EpitetoForma", "EpitetoRaza", "EpitetoCultivar"]);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
