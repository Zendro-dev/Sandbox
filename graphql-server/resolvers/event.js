...
event.prototype.handleAssociations = async function(input, benignErrorReporter) {
    ...
    if (helper.isNonEmptyArray(input.addEventParameters)) {
        promises_add.push(this.add_eventParameters(input, benignErrorReporter));
    }
    ...
    if (helper.isNonEmptyArray(input.removeEventParameters)) {
        promises_remove.push(this.remove_eventParameters(input, benignErrorReporter));
    }
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
event.prototype.add_eventParameters = async function(input, benignErrorReporter) {

    let bulkAssociationInput = input.addEventParameters.map(associatedRecordId => {
        return {
            eventDbId: this.getIdValue(),
            [models.eventParameter.idAttribute()]: associatedRecordId
        }
    });
    await models.eventParameter.bulkAssociateEventParameterWithEventDbId(bulkAssociationInput, benignErrorReporter);
}
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
event.prototype.add_observationUnits = async function(input, benignErrorReporter) {

    await event.add_observationUnitDbIds(this.getIdValue(), input.addObservationUnits, benignErrorReporter);
...
event.prototype.add_study = async function(input, benignErrorReporter) {
    await event.add_studyDbId(this.getIdValue(), input.addStudy, benignErrorReporter);
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
event.prototype.remove_eventParameters = async function(input, benignErrorReporter) {

    let bulkAssociationInput = input.removeEventParameters.map(associatedRecordId => {
        return {
            eventDbId: this.getIdValue(),
            [models.eventParameter.idAttribute()]: associatedRecordId
        }
    });
    await models.eventParameter.bulkDisAssociateEventParameterWithEventDbId(bulkAssociationInput, benignErrorReporter);
}
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
event.prototype.remove_observationUnits = async function(input, benignErrorReporter) {

    await event.remove_observationUnitDbIds(this.getIdValue(), input.removeObservationUnits, benignErrorReporter);
...
event.prototype.remove_study = async function(input, benignErrorReporter) {
    if (input.removeStudy == this.studyDbId) {
        await event.remove_studyDbId(this.getIdValue(), input.removeStudy, benignErrorReporter);
        this.studyDbId = null;
    }
}
...
async function countAllAssociatedRecords(id, context) {
    ...
    promises_to_many.push(event.countFilteredObservationUnits({}, context));
...
module.exports = {
    ...
    readOneEvent: async function({
        ...
        if (await checkAuthorization(context, 'event', 'read') === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await event.readById(eventType, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    vueTableEvent: async function(_, context) {
        if (await checkAuthorization(context, 'event', 'read') === true) {
            return helper.vueTable(context.request, event, ["id", "eventDbId", "eventDescription", "eventType", "studyDbId"]);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    deleteEvent: async function({
        ...
        if (await checkAuthorization(context, 'event', 'delete') === true) {
            if (await validForDeletion(eventType, context)) {
                let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
                return event.deleteOne(eventType, benignErrorReporter);
            }
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    ...
    bulkAssociateEventWithStudyDbId: async function(bulkAssociationInput, context) {
        ...
        return await event.bulkAssociateEventWithStudyDbId(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
    /**
    ...
    bulkDisAssociateEventWithStudyDbId: async function(bulkAssociationInput, context) {
        ...
        return await event.bulkDisAssociateEventWithStudyDbId(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
