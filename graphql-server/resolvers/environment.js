...
module.exports = {
    ...
    environmentsConnection: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            return await environment.readAllCursor(search, order, pagination, authorizationCheck.authorizedAdapters, benignErrorReporter, searchAuthorizationCheck.authorizedAdapters);
    ...
    readOneEnvironment: async function({
        ...
        if (authorizationCheck === true) {
            ...
            return environment.readById(id, benignErrorReporter);
    ...
    addEnvironment: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            let createdRecord = await environment.addOne(inputSanitized, benignErrorReporter);
    ...
    updateEnvironment: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            //construct benignErrors reporter with context
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            let updatedRecord = await environment.updateOne(inputSanitized, benignErrorReporter);
    ...
    countEnvironments: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let searchAuthorizationCheck = await helper.authorizedAdapters(context, adapters, 'search');
            return await environment.countRecords(search, authorizationCheck.authorizedAdapters, benignErrorReporter, searchAuthorizationCheck.authorizedAdapters);
        } else { //adapters not auth || errors
            // else new Error
            if (authorizationCheck.authorizationErrors.length > 0) {
                throw new Error(authorizationCheck.authorizationErrors.reduce((a, c) => `${a}, ${c.message}`));
