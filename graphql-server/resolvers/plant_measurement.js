...
/**
 * plant_measurement.prototype.assay - Return associated record
 *
 * @param  {object} search       Search argument to match the associated record
 * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
 * @return {type}         Associated record
 */
plant_measurement.prototype.assay = async function({
    search
}, context) {

    if (helper.isNotUndefinedAndNotNull(this.plant_id)) {
        if (search === undefined || search === null) {
            return resolvers.readOneAssay({
                [models.assay.idAttribute()]: this.plant_id
            }, context)
        } else {
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
plant_measurement.prototype.add_assay = async function(input, benignErrorReporter) {
    await plant_measurement.add_plant_id(this.getIdValue(), input.addAssay, benignErrorReporter);
    this.plant_id = input.addAssay;
}
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
plant_measurement.prototype.remove_assay = async function(input, benignErrorReporter) {
    if (input.removeAssay == this.plant_id) {
        await plant_measurement.remove_plant_id(this.getIdValue(), input.removeAssay, benignErrorReporter);
        this.plant_id = null;
    }
}
...
async function countAllAssociatedRecords(id, context) {
    ...
    promises_to_one.push(plant_measurement.assay({}, context));
...
async function validForDeletion(id, context) {
    if (await countAllAssociatedRecords(id, context) > 0) {
        throw new Error(`plant_measurement with measurement_id ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`);
    }
    return true;
}
...
module.exports = {
    ...
    /**
     * readOnePlant_measurement - Check user authorization and return one record with the specified measurement_id in the measurement_id argument.
     *
    ...
    readOnePlant_measurement: async function({
        ...
        if (await checkAuthorization(context, 'plant_measurement', 'read') === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await plant_measurement.readById(measurement_id, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    vueTablePlant_measurement: async function(_, context) {
        if (await checkAuthorization(context, 'plant_measurement', 'read') === true) {
            return helper.vueTable(context.request, plant_measurement, ["id", "measurement_id", "plant_id", "measurement", "value", "unit"]);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    /**
     * deletePlant_measurement - Check user authorization and delete a record with the specified measurement_id in the measurement_id argument.
     *
    ...
    deletePlant_measurement: async function({
        ...
        if (await checkAuthorization(context, 'plant_measurement', 'delete') === true) {
            if (await validForDeletion(measurement_id, context)) {
                let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
                return plant_measurement.deleteOne(measurement_id, benignErrorReporter);
            }
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    ...
    /**
     * bulkAssociatePlant_measurementWithPlant_id - bulkAssociaton resolver of given ids
     *
     * @param  {array} bulkAssociationInput Array of associations to add , 
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     * @return {string} returns message on success
     */
    bulkAssociatePlant_measurementWithPlant_id: async function(bulkAssociationInput, context) {
        let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
        // if specified, check existence of the unique given ids
        if (!bulkAssociationInput.skipAssociationsExistenceChecks) {
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
                plant_id
            }) => plant_id)), models.assay);
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
                measurement_id
            }) => measurement_id)), plant_measurement);
        }
        return await plant_measurement.bulkAssociatePlant_measurementWithPlant_id(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
    /**
     * bulkDisAssociatePlant_measurementWithPlant_id - bulkDisAssociaton resolver of given ids
     *
     * @param  {array} bulkAssociationInput Array of associations to remove , 
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     * @return {string} returns message on success
     */
    bulkDisAssociatePlant_measurementWithPlant_id: async function(bulkAssociationInput, context) {
        let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
        // if specified, check existence of the unique given ids
        if (!bulkAssociationInput.skipAssociationsExistenceChecks) {
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
        ...
        return await plant_measurement.bulkDisAssociatePlant_measurementWithPlant_id(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
