...
material.prototype.sourceSetsFilter = function({
    ...
    //return an empty response if the foreignKey Array is empty, no need to query the database
    if (!Array.isArray(this.sourceSet_ids) || this.sourceSet_ids.length === 0) {
        return [];
    }
    let nsearch = helper.addSearchField({
        "search": search,
...
material.prototype.elementsFilter = function({
    ...
    //return an empty response if the foreignKey Array is empty, no need to query the database
    if (!Array.isArray(this.element_ids) || this.element_ids.length === 0) {
        return [];
    }
    let nsearch = helper.addSearchField({
        "search": search,
...
material.prototype.add_fileAttachments = async function(input, benignErrorReporter) {
    ...
    await models.fileAttachment.bulkAssociateFileAttachmentWithMaterial_id(bulkAssociationInput, benignErrorReporter);
}
...
material.prototype.remove_fileAttachments = async function(input, benignErrorReporter) {
    ...
    await models.fileAttachment.bulkDisAssociateFileAttachmentWithMaterial_id(bulkAssociationInput, benignErrorReporter);
}
