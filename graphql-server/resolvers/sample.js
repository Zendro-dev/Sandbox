...
sample.prototype.handleAssociations = async function(input, benignErrorReporter, token) {
    ...
    if (helper.isNotUndefinedAndNotNull(input.addObservation_unit)) {
        promises_add.push(this.add_observation_unit(input, benignErrorReporter, token));
    }
    ...
    if (helper.isNotUndefinedAndNotNull(input.removeObservation_unit)) {
        promises_remove.push(this.remove_observation_unit(input, benignErrorReporter, token));
    }
 ...
 * @param {string} token The token used for authorization
 */
sample.prototype.add_data_files = async function(input, benignErrorReporter, token) {

    await sample.add_data_file_ids(this.getIdValue(), input.addData_files, benignErrorReporter, token);
 ...
 * @param {string} token The token used for authorization
 */
sample.prototype.add_observation_unit = async function(input, benignErrorReporter, token) {
    await sample.add_observation_unit_id(this.getIdValue(), input.addObservation_unit, benignErrorReporter, token);
 ...
 * @param {string} token The token used for authorization
 */
sample.prototype.remove_data_files = async function(input, benignErrorReporter, token) {

    await sample.remove_data_file_ids(this.getIdValue(), input.removeData_files, benignErrorReporter, token);
 ...
 * @param {string} token The token used for authorization
 */
sample.prototype.remove_observation_unit = async function(input, benignErrorReporter, token) {
    if (input.removeObservation_unit == this.observation_unit_id) {
        await sample.remove_observation_unit_id(this.getIdValue(), input.removeObservation_unit, benignErrorReporter, token);
...
async function countAssociatedRecordsWithRejectReaction(id, context) {
    ...
    get_to_many_associated_fk += Array.isArray(sample.data_file_ids) ? sample.data_file_ids.length : 0;
...
module.exports = {
    ...
    samplesConnection: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            return await sample.readAllCursor(search, order, pagination, authorizationCheck.authorizedAdapters, context.benignErrors, searchAuthorizationCheck.authorizedAdapters, token);
    ...
    readOneSample: async function({
        ...
        if (authorizationCheck === true) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            return sample.readById(id, context.benignErrors, token);
    ...
    addSample: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            let createdRecord = await sample.addOne(inputSanitized, context.benignErrors, token);
        ...
        if (authorizationCheck === true) {
            if (await validForDeletion(id, context)) {
                ...
                let token = context.request ?
                    ...
                    undefined :
                    undefined;
                return sample.deleteOne(id, context.benignErrors, token);
            }
        } else { //adapter not auth
            throw new Error("You don't have authorization to perform this action on adapter");
        }
    ...
    updateSample: async function(input, context) {
        ...
        if (authorizationCheck === true) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            let updatedRecord = await sample.updateOne(inputSanitized, context.benignErrors, token);
    ...
    countSamples: async function({
        ...
        if (authorizationCheck.authorizedAdapters.length > 0) {
            ...
            let token = context.request ?
                ...
                undefined :
                undefined;
            return await sample.countRecords(search, authorizationCheck.authorizedAdapters, context.benignErrors, searchAuthorizationCheck.authorizedAdapters, token);
        } else { //adapters not auth || errors
            // else new Error
            if (authorizationCheck.authorizationErrors.length > 0) {
                throw new Error(authorizationCheck.authorizationErrors.reduce((a, c) => `${a}, ${c.message}`));
    ...
    bulkAssociateSampleWithObservation_unit_id: async function(bulkAssociationInput, context) {
        ...
        if (!bulkAssociationInput.skipAssociationsExistenceChecks) {
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
                observation_unit_id
            }) => observation_unit_id)), models.observation_unit, token);
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
                id
            }) => id)), sample, token);
        }
        return await sample.bulkAssociateSampleWithObservation_unit_id(bulkAssociationInput.bulkAssociationInput, context.benignErrors, token);
    },
    /**
    ...
    bulkDisAssociateSampleWithObservation_unit_id: async function(bulkAssociationInput, context) {
        ...
        return await sample.bulkDisAssociateSampleWithObservation_unit_id(bulkAssociationInput.bulkAssociationInput, context.benignErrors, token);
    },
    ...
    csvTableTemplateSample: async function(_, context) {
        if (await checkAuthorization(context, 'sample', 'read') === true) {
            return sample.csvTableTemplate(context.benignErrors);
        } else {
