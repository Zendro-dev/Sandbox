 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
country.prototype.add_cities = async function(input, benignErrorReporter) {
    ...
    await models.city.bulkAssociateCityWithCountry_id(bulkAssociationInput, benignErrorReporter);
}
...
country.prototype.add_capital = async function(input, benignErrorReporter) {
    ...
    if (num > 0) {
        if (num > 1) {
            ...
            const removed = await country.remove_capital_id(country_id, input.addCapital, benignErrorReporter);
            benignErrorReporter.reportError({
                message: `Hint: update ${removed} existing association!`,
            });
        }
    }
    await country.add_capital_id(this.getIdValue(), input.addCapital, benignErrorReporter);
 ...
 * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
 */
country.prototype.remove_cities = async function(input, benignErrorReporter) {
    ...
    await models.city.bulkDisAssociateCityWithCountry_id(bulkAssociationInput, benignErrorReporter);
}
...
country.prototype.remove_capital = async function(input, benignErrorReporter) {
    if (input.removeCapital == this.capital_id) {
        await country.remove_capital_id(this.getIdValue(), input.removeCapital, benignErrorReporter);
...
module.exports = {
    ...
    bulkAssociateCountryWithCapital_id: async function(bulkAssociationInput, context) {
        ...
        if (!bulkAssociationInput.skipAssociationsExistenceChecks) {
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
                capital_id
            }) => capital_id)), models.city);
            await helper.validateExistence(helper.unique(bulkAssociationInput.bulkAssociationInput.map(({
                country_id
            }) => country_id)), country);
        }
        return await country.bulkAssociateCountryWithCapital_id(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
    /**
    ...
    bulkDisAssociateCountryWithCapital_id: async function(bulkAssociationInput, context) {
        ...
        return await country.bulkDisAssociateCountryWithCapital_id(bulkAssociationInput.bulkAssociationInput, benignErrorReporter);
    },
