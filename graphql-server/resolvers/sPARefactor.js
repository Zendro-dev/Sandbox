...
async function validForDeletion(id, context) {
    if (await countAllAssociatedRecords(id, context) > 0) {
        throw new Error(`SPARefactor with spa_refactor ${id} has associated records and is NOT valid for deletion. Please clean up before you delete.`);
    }
    return true;
}
...
module.exports = {
    ...
    /**
     * readOneSPARefactor - Check user authorization and return one record with the specified spa_refactor in the spa_refactor argument.
     *
     * @param  {number} {spa_refactor}    spa_refactor of the record to retrieve
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
     * @return {object}         Record with spa_refactor requested
     */
    readOneSPARefactor: async function({
        ...
        if (await checkAuthorization(context, 'SPARefactor', 'read') === true) {
            ...
            let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
            return await sPARefactor.readById(spa_refactor, benignErrorReporter);
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
    },
    ...
    /**
     * deleteSPARefactor - Check user authorization and delete a record with the specified spa_refactor in the spa_refactor argument.
     *
     * @param  {number} {spa_refactor}    spa_refactor of the record to delete
     * @param  {object} context Provided to every resolver holds contextual information like the resquest query and user info.
    ...
    deleteSPARefactor: async function({
        ...
        if (await checkAuthorization(context, 'SPARefactor', 'delete') === true) {
            if (await validForDeletion(spa_refactor, context)) {
                let benignErrorReporter = new errorHelper.BenignErrorReporter(context);
                return sPARefactor.deleteOne(spa_refactor, benignErrorReporter);
            }
        } else {
            throw new Error("You don't have authorization to perform this action");
        }
