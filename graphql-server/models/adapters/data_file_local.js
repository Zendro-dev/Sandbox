...
module.exports = class data_file_local extends Sequelize.Model {
    /**
    ...
    static async batchReadById(keys) {
        let queryArg = {
            operator: "in",
            field: data_file_local.idAttribute(),
            value: keys.join(),
            valueType: "Array",
        };
        let cursorRes = await data_file_local.readAllCursor(queryArg);
        cursorRes = cursorRes.data_files.reduce(
            (map, obj) => ((map[obj[data_file_local.idAttribute()]] = obj), map), {}
        );
        return keys.map(
            (key) =>
    ...
    static readByIdLoader = new DataLoader(data_file_local.batchReadById, {
        cache: false,
    });
     ...
     * @return {object} The requested record as an object with the type data_file_local, or an error object if the validation after reading fails
     * @throws {Error} If the requested record does not exist
     */
    static async readById(id) {
        return await data_file_local.readByIdLoader.load(id);
    }
    /**
    ...
    static countRecords(search) {
        ...
        if (search !== undefined && search !== null) {
            ...
            let arg_sequelize = arg.toSequelize(data_file_local.definition.attributes);
    ...
    static async readAllCursor(search, order, pagination) {
        ...
        let options = helper.buildCursorBasedSequelizeOptions(search, order, pagination, this.idAttribute(), data_file_local.definition.attributes);
        let records = await super.findAll(options);
        records = records.map(x => data_file_local.postReadCast(x))
        ...
        if (pagination && (pagination.after || pagination.before)) {
            let oppOptions = helper.buildOppositeSearchSequelize(search, order, {
                ...pagination,
                includeCursor: false
            }, this.idAttribute(), data_file_local.definition.attributes);
    ...
    static async addOne(input) {
        input = data_file_local.preWriteCast(input)
        try {
            ...
            data_file_local.postReadCast(result.dataValues)
            data_file_local.postReadCast(result._previousDataValues)
            return result;
        } catch (error) {
            throw error;
        }
    ...
    static bulkAddCsv(context) {
        ...
        return `Bulk import of data_file_local records started. You will be send an email to ${helpersAcl.getTokenFromContext(context).email} informing you about success or errors`;
    }
     ...
     */
    static idAttribute() {
        return data_file_local.definition.id.name;
    }
    ...
    static idAttributeType() {
        return data_file_local.definition.id.type;
    }

    /**
     * getIdValue - Get the value of the idAttribute ("id", or "internalId") for an instance of data_file_local.
     *
     * @return {type} id value
     */
    getIdValue() {
        return this[data_file_local.idAttribute()];
    }
    ...
    /**
     * base64Encode - Encode  data_file_local to a base 64 String
     *
     * @return {string} The data_file_local object, encoded in a base 64 String
     */
    base64Encode() {
    ...
    /**
     * stripAssociations - Instance method for getting all attributes of data_file_local.
     *
     * @return {object} The attributes of data_file_local in object form
     */
    stripAssociations() {
        let attributes = Object.keys(data_file_local.definition.attributes);
        let data_values = _.pick(this, attributes);
        return data_values;
    }

    /**
     * externalIdsArray - Get all attributes of data_file_local that are marked as external IDs.
     *
     * @return {Array<String>} An array of all attributes of data_file_local that are marked as external IDs
     */
    ...
    /**
     * externalIdsObject - Get all external IDs of data_file_local.
     *
