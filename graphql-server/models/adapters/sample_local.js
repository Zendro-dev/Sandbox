...
module.exports = class sample_local extends Sequelize.Model {
    /**
    ...
    static async batchReadById(keys) {
        let queryArg = {
            operator: "in",
            field: sample_local.idAttribute(),
            value: keys.join(),
            valueType: "Array",
        };
        let cursorRes = await sample_local.readAllCursor(queryArg);
        cursorRes = cursorRes.samples.reduce(
            (map, obj) => ((map[obj[sample_local.idAttribute()]] = obj), map), {}
        );
        return keys.map(
            (key) =>
    ...
    static readByIdLoader = new DataLoader(sample_local.batchReadById, {
        cache: false,
    });
     ...
     * @return {object} The requested record as an object with the type sample_local, or an error object if the validation after reading fails
     * @throws {Error} If the requested record does not exist
     */
    static async readById(id) {
        return await sample_local.readByIdLoader.load(id);
    }
    /**
    ...
    static countRecords(search) {
        ...
        if (search !== undefined && search !== null) {
            ...
            let arg_sequelize = arg.toSequelize(sample_local.definition.attributes);
    ...
    static async readAllCursor(search, order, pagination) {
        ...
        let options = helper.buildCursorBasedSequelizeOptions(search, order, pagination, this.idAttribute(), sample_local.definition.attributes);
        let records = await super.findAll(options);
        records = records.map(x => sample_local.postReadCast(x))
        ...
        if (pagination && (pagination.after || pagination.before)) {
            let oppOptions = helper.buildOppositeSearchSequelize(search, order, {
                ...pagination,
                includeCursor: false
            }, this.idAttribute(), sample_local.definition.attributes);
    ...
    static async addOne(input) {
        input = sample_local.preWriteCast(input)
        try {
            ...
            sample_local.postReadCast(result.dataValues)
            sample_local.postReadCast(result._previousDataValues)
            return result;
        } catch (error) {
            throw error;
        }
    ...
    static async add_data_file_ids(id, data_file_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            ...
            data_file_ids.forEach(idx => {
                promises.push(models.data_file.add_sample_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
    ...
    static async remove_data_file_ids(id, data_file_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            let promises = [];
            data_file_ids.forEach(idx => {
                promises.push(models.data_file.remove_sample_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
     ...
     */
    static idAttribute() {
        return sample_local.definition.id.name;
    }
    ...
    static idAttributeType() {
        return sample_local.definition.id.type;
    }

    /**
     * getIdValue - Get the value of the idAttribute ("id", or "internalId") for an instance of sample_local.
     *
     * @return {type} id value
     */
    getIdValue() {
        return this[sample_local.idAttribute()];
    }
    ...
    /**
     * base64Encode - Encode  sample_local to a base 64 String
     *
     * @return {string} The sample_local object, encoded in a base 64 String
     */
    base64Encode() {
    ...
    /**
     * stripAssociations - Instance method for getting all attributes of sample_local.
     *
     * @return {object} The attributes of sample_local in object form
     */
    stripAssociations() {
        let attributes = Object.keys(sample_local.definition.attributes);
        let data_values = _.pick(this, attributes);
        return data_values;
    }

    /**
     * externalIdsArray - Get all attributes of sample_local that are marked as external IDs.
     *
     * @return {Array<String>} An array of all attributes of sample_local that are marked as external IDs
     */
    ...
    /**
     * externalIdsObject - Get all external IDs of sample_local.
     *
