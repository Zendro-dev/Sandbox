...
module.exports = class event_local extends Sequelize.Model {
    ...
    static async add_observation_unit_ids(id, observation_unit_ids, benignErrorReporter, handle_inverse = true) {
        ...
        if (handle_inverse) {
            ...
            observation_unit_ids.forEach(idx => {
                promises.push(models.observation_unit.add_event_ids(idx, [`${id}`], benignErrorReporter, false));
            });
    ...
    static async remove_observation_unit_ids(id, observation_unit_ids, benignErrorReporter, handle_inverse = true) {
        ...
        if (handle_inverse) {
            let promises = [];
            observation_unit_ids.forEach(idx => {
                promises.push(models.observation_unit.remove_event_ids(idx, [`${id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
    ...
    static bulkAddCsv(context) {
        ...
        return `Bulk import of event_local records started. You will be send an email to ${helpersAcl.getTokenFromContext(context).email} informing you about success or errors`;
    }
