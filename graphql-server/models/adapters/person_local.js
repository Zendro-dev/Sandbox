...
module.exports = class person_local extends Sequelize.Model {
    ...
    static async add_investigation_ids(id, investigation_ids, benignErrorReporter, token, handle_inverse = true) {

        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            investigation_ids.forEach(idx => {
                promises.push(models.investigation.add_person_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
    ...
    static async add_study_ids(id, study_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            ...
            study_ids.forEach(idx => {
                promises.push(models.study.add_person_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    static async remove_investigation_ids(id, investigation_ids, benignErrorReporter, token, handle_inverse = true) {

        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            investigation_ids.forEach(idx => {
                promises.push(models.investigation.remove_person_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    static async remove_study_ids(id, study_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            let promises = [];
            study_ids.forEach(idx => {
                promises.push(models.study.remove_person_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
