...
module.exports = class study_local extends Sequelize.Model {
    ...
    static async add_person_ids(id, person_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            ...
            person_ids.forEach(idx => {
                promises.push(models.person.add_study_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
    ...
    static async add_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            ...
            observed_variable_ids.forEach(idx => {
                promises.push(models.observed_variable.add_study_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    static async add_biological_material_ids(id, biological_material_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            ...
            biological_material_ids.forEach(idx => {
                promises.push(models.biological_material.add_study_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    static async remove_person_ids(id, person_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            let promises = [];
            person_ids.forEach(idx => {
                promises.push(models.person.remove_study_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    static async remove_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            let promises = [];
            observed_variable_ids.forEach(idx => {
                promises.push(models.observed_variable.remove_study_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
    ...
    static async remove_biological_material_ids(id, biological_material_ids, benignErrorReporter, token, handle_inverse = true) {
        ...
        if (handle_inverse) {
            let promises = [];
            biological_material_ids.forEach(idx => {
                promises.push(models.biological_material.remove_study_ids(idx, [`${id}`], benignErrorReporter, token, false));
            });
            await Promise.all(promises);
        }
