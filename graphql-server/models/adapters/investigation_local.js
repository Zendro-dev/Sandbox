...
module.exports = class investigation_local extends Sequelize.Model {
    ...
    static async add_person_ids(id, person_ids, benignErrorReporter, handle_inverse = true) {
        ...
        if (handle_inverse) {
            ...
            person_ids.forEach(idx => {
                promises.push(models.person.add_investigation_ids(idx, [`${id}`], benignErrorReporter, false));
            });
    ...
    static async remove_person_ids(id, person_ids, benignErrorReporter, handle_inverse = true) {
        ...
        if (handle_inverse) {
            let promises = [];
            person_ids.forEach(idx => {
                promises.push(models.person.remove_investigation_ids(idx, [`${id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
    ...
    static bulkAddCsv(context) {
        ...
        return `Bulk import of investigation_local records started. You will be send an email to ${helpersAcl.getTokenFromContext(context).email} informing you about success or errors`;
    }
