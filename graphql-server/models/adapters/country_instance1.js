...
module.exports = class country_instance1 extends Sequelize.Model {
    ...
    static async batchReadById(keys) {
        let queryArg = {
            operator: "in",
            field: country_instance1.idAttribute(),
            value: keys.join(),
            valueType: "Array",
        };
        let cursorRes = await country_instance1.readAllCursor(queryArg);
        cursorRes = cursorRes.countries.reduce(
            (map, obj) => ((map[obj[country_instance1.idAttribute()]] = obj), map), {}
        );
        return keys.map(
            (key) =>
    ...
    static readByIdLoader = new DataLoader(country_instance1.batchReadById, {
        cache: false,
    });

    static async readById(id) {
        return await country_instance1.readByIdLoader.load(id);
    }
