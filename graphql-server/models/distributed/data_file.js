...
const definition = {
    ...
    "registry": ["data_file_local", "data_file_public"],
    "attributes": {
        "id": "String",
        "url": "String",
        "description": "String",
...
let registry = ["data_file_local", "data_file_public"];

module.exports = class data_file {
    ...
    static get registeredAdapters() {
        return ["data_file_local", "data_file_public"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readAllCursor(search, order, pagination, authorizedAdapters, benignErrorReporter, searchAuthorizedAdapters, token) {
        ...
        return Promise.allSettled(promises)
            ...
            .then(async nodes => {
                ...
                let graphQLConnection = helper.toGraphQLConnectionObject(paginated_records, this, hasNextPage, hasPreviousPage, "data_files");
    ...
    static async addOne(input, benignErrorReporter, token) {
        ...
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter, token).then(result => new data_file(result));
    }
    ...
    static async updateOne(input, benignErrorReporter, token) {
        ...
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter, token).then(result => new data_file(result));
     ...
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   sample_ids Foreign Key (stored in "Me") of the Association to be updated.
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_sample_ids(id, sample_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_sample_ids(id, sample_ids, benignErrorReporter, token, handle_inverse);
    }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_sample_ids(id, sample_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_sample_ids(id, sample_ids, benignErrorReporter, token, handle_inverse);
    }
     ...
     */
    static async bulkAssociateData_fileWithStudy_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkAssociateData_fileWithStudy_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
     ...
     * @return {string} returns message on success
     */
    static async bulkDisAssociateData_fileWithStudy_id(bulkAssociationInput, benignErrorReporter, token) {
        let mappedBulkAssociateInputToAdapters = this.mapBulkAssociationInputToAdapters(bulkAssociationInput);
        var promises = [];
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkDisAssociateData_fileWithStudy_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
        await Promise.all(promises);
        return "Records successfully updated!";
    }
