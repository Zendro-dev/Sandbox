...
const definition = {
    ...
    "registry": ["person_local", "person_public"],
    "attributes": {
        "id": "String",
        "name": "String",
        "email": "String",
...
let registry = ["person_local", "person_public"];
...
module.exports = class person {
    ...
    constructor({
        ...
        this.investigation_ids = investigation_ids;
    ...
    static get registeredAdapters() {
        return ["person_local", "person_public"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readAllCursor(search, order, pagination, authorizedAdapters, benignErrorReporter, searchAuthorizedAdapters, token) {
        ...
        return Promise.allSettled(promises)
            ...
            .then(async nodes => {
                ...
                let graphQLConnection = helper.toGraphQLConnectionObject(paginated_records, this, hasNextPage, hasPreviousPage, "people");
    ...
    static async addOne(input, benignErrorReporter, token) {
        ...
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter, token).then(result => new person(result));
    }
    ...
    static async updateOne(input, benignErrorReporter, token) {
        ...
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter, token).then(result => new person(result));
     ...
     * @param {Array}   investigation_ids Foreign Key (stored in "Me") of the Association to be updated.
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_investigation_ids(id, investigation_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_investigation_ids(id, investigation_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_investigation_ids(id, investigation_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_investigation_ids(id, investigation_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
