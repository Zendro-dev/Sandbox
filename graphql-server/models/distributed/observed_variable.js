...
const definition = {
    ...
    "registry": ["observed_variable_local", "observed_variable_public"],
    "attributes": {
        "id": "String",
        "name": "String",
...
let registry = ["observed_variable_local", "observed_variable_public"];

module.exports = class observed_variable {
    ...
    constructor({
        ...
        this.trait = trait;
        this.trait_accession_number = trait_accession_number;
        this.method = method;
        this.method_accession_number = method_accession_number;
        this.method_description = method_description;
        this.scale = scale;
        this.scale_accession_number = scale_accession_number;
    ...
    static get registeredAdapters() {
        return ["observed_variable_local", "observed_variable_public"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readById(id, benignErrorReporter, token) {
        if (id !== null) {
            ...
            return adapters[responsibleAdapter[0]].readById(id, benignErrorReporter, token).then(result => {
                let item = new observed_variable(result);
    ...
    static readAllCursor(search, order, pagination, authorizedAdapters, benignErrorReporter, searchAuthorizedAdapters, token) {
        ...
        return Promise.allSettled(promises)
            ...
            .then(async nodes => {
                ...
                let graphQLConnection = helper.toGraphQLConnectionObject(paginated_records, this, hasNextPage, hasPreviousPage, "observed_variables");
    ...
    static async addOne(input, benignErrorReporter, token) {
        ...
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter, token).then(result => new observed_variable(result));
    }
    ...
    static async updateOne(input, benignErrorReporter, token) {
        ...
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter, token).then(result => new observed_variable(result));
