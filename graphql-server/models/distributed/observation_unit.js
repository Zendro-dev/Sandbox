...
const definition = {
    ...
    "registry": ["observation_unit_local", "observation_unit_public"],
    "attributes": {
        "id": "String",
        "type": "String",
        "external_id": "String",
...
let registry = ["observation_unit_local", "observation_unit_public"];

module.exports = class observation_unit {
    ...
    static get registeredAdapters() {
        return ["observation_unit_local", "observation_unit_public"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readById(id, benignErrorReporter, token) {
        if (id !== null) {
            ...
            return adapters[responsibleAdapter[0]].readById(id, benignErrorReporter, token).then(result => {
                let item = new observation_unit(result);
    ...
    static readAllCursor(search, order, pagination, authorizedAdapters, benignErrorReporter, searchAuthorizedAdapters, token) {
        ...
        return Promise.allSettled(promises)
            ...
            .then(async nodes => {
                ...
                let graphQLConnection = helper.toGraphQLConnectionObject(paginated_records, this, hasNextPage, hasPreviousPage, "observation_units");
    ...
    static async addOne(input, benignErrorReporter, token) {
        ...
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter, token).then(result => new observation_unit(result));
    }
    ...
    static async updateOne(input, benignErrorReporter, token) {
        ...
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter, token).then(result => new observation_unit(result));
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_data_file_ids(id, data_file_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_data_file_ids(id, data_file_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
     * add_event_ids - field Mutation (model-layer) for to_many associationsArguments to add
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   event_ids Foreign Key (stored in "Me") of the Association to be updated.
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_event_ids(id, event_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_event_ids(id, event_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
     * add_factor_ids - field Mutation (model-layer) for to_many associationsArguments to add
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   factor_ids Foreign Key (stored in "Me") of the Association to be updated.
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_factor_ids(id, factor_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_factor_ids(id, factor_ids, benignErrorReporter, token, handle_inverse);
    }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_data_file_ids(id, data_file_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_data_file_ids(id, data_file_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_event_ids(id, event_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_event_ids(id, event_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_factor_ids(id, factor_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_factor_ids(id, factor_ids, benignErrorReporter, token, handle_inverse);
    }
    ...
    static async bulkAssociateObservation_unitWithStudy_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkAssociateObservation_unitWithStudy_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
    ...
    static async bulkDisAssociateObservation_unitWithStudy_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        var promises = [];
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkDisAssociateObservation_unitWithStudy_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
        await Promise.all(promises);
        return "Records successfully updated!";
    }
