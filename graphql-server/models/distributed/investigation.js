...
const definition = {
    ...
    "registry": ["investigation_local", "investigation_public"],
    "attributes": {
        "id": "String",
        "title": "String",
        "description": "String",
...
let registry = ["investigation_local", "investigation_public"];

module.exports = class investigation {
    ...
    constructor({
        ...
        this.MIAPPE_version = MIAPPE_version;
    ...
    static get registeredAdapters() {
        return ["investigation_local", "investigation_public"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readById(id, benignErrorReporter, token) {
        if (id !== null) {
            ...
            return adapters[responsibleAdapter[0]].readById(id, benignErrorReporter, token).then(result => {
                let item = new investigation(result);
    ...
    static readAllCursor(search, order, pagination, authorizedAdapters, benignErrorReporter, searchAuthorizedAdapters, token) {
        ...
        return Promise.allSettled(promises)
            ...
            .then(async nodes => {
                ...
                let graphQLConnection = helper.toGraphQLConnectionObject(paginated_records, this, hasNextPage, hasPreviousPage, "investigations");
    ...
    static async addOne(input, benignErrorReporter, token) {
        ...
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter, token).then(result => new investigation(result));
    }
    ...
    static async updateOne(input, benignErrorReporter, token) {
        ...
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter, token).then(result => new investigation(result));
