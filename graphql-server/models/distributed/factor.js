...
let registry = ["factor_local", "factor_public"];
...
module.exports = class factor {
    ...
    static get registeredAdapters() {
        return ["factor_local", "factor_public"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readAllCursor(search, order, pagination, authorizedAdapters, benignErrorReporter, searchAuthorizedAdapters) {
        ...
        return Promise.allSettled(promises)
            ...
            .then(async nodes => {
                ...
                let graphQLConnection = helper.toGraphQLConnectionObject(paginated_records, this, hasNextPage, hasPreviousPage, "factors");
    ...
    static async addOne(input, benignErrorReporter) {
        ...
        //use default BenignErrorReporter if no BenignErrorReporter defined
        benignErrorReporter = errorHelper.getDefaultBenignErrorReporterIfUndef(benignErrorReporter);
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter).then(result => new factor(result));
    }
    ...
    static async updateOne(input, benignErrorReporter) {
        ...
        //use default BenignErrorReporter if no BenignErrorReporter defined
        benignErrorReporter = errorHelper.getDefaultBenignErrorReporterIfUndef(benignErrorReporter);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter).then(result => new factor(result));
     ...
     */
    static async bulkAssociateFactorWithStudy_id(bulkAssociationInput, benignErrorReporter) {
        ...
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkAssociateFactorWithStudy_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter))
        });
     ...
     * @return {string} returns message on success
     */
    static async bulkDisAssociateFactorWithStudy_id(bulkAssociationInput, benignErrorReporter) {
        let mappedBulkAssociateInputToAdapters = this.mapBulkAssociationInputToAdapters(bulkAssociationInput);
        var promises = [];
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkDisAssociateFactorWithStudy_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter))
        });
        await Promise.all(promises);
        return "Records successfully updated!";
    }
