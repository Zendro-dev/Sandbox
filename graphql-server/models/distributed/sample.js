...
const definition = {
    ...
    "registry": ["sample_local", "sample_public"],
    "attributes": {
        "id": "String",
        "value": "String",
...
module.exports = class sample {
    ...
    static async addOne(input, benignErrorReporter, token) {
        ...
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter, token).then(result => new sample(result));
    }
    ...
    static async updateOne(input, benignErrorReporter, token) {
        ...
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter, token).then(result => new sample(result));
     ...
     */
    static async add_observation_unit_id(id, observation_unit_id, benignErrorReporter, token, handle_inverse) {
        try {
            let responsibleAdapter = this.adapterForIri(id);
            return await adapters[responsibleAdapter].add_observation_unit_id(id, observation_unit_id, benignErrorReporter, token, handle_inverse);
        } catch (error) {
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_observation_unit_id(id, observation_unit_id, benignErrorReporter, token, handle_inverse) {
        try {
            let responsibleAdapter = this.adapterForIri(id);
            return await adapters[responsibleAdapter].remove_observation_unit_id(id, observation_unit_id, benignErrorReporter, token, handle_inverse);
        } catch (error) {
            benignErrorReporter.push({
                message: error,
            });
    ...
    static async bulkAssociateSampleWithObservation_unit_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkAssociateSampleWithObservation_unit_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
    ...
    static async bulkDisAssociateSampleWithObservation_unit_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        var promises = [];
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkDisAssociateSampleWithObservation_unit_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
        await Promise.all(promises);
        return "Records successfully updated!";
    }
