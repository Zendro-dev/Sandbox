...
const definition = {
    ...
    "registry": ["snplocus_local", "snplocus_public"],
    "attributes": {
        "snp_matrix_id": "Int",
        "chromsome": "String",
        "pos": "Int",
...
let registry = ["snplocus_local", "snplocus_public"];

module.exports = class snplocus {
    ...
    static get registeredAdapters() {
        return ["snplocus_local", "snplocus_public"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readAllCursor(search, order, pagination, authorizedAdapters, benignErrorReporter, searchAuthorizedAdapters, token) {
        ...
        return Promise.allSettled(promises)
            ...
            .then(async nodes => {
                ...
                let graphQLConnection = helper.toGraphQLConnectionObject(paginated_records, this, hasNextPage, hasPreviousPage, "snplocus");
    ...
    static async addOne(input, benignErrorReporter, token) {
        ...
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter, token).then(result => new snplocus(result));
    }
    ...
    static async updateOne(input, benignErrorReporter, token) {
        ...
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter, token).then(result => new snplocus(result));
    ...
    static async bulkAssociateSnplocusWithSnp_matrix_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkAssociateSnplocusWithSnp_matrix_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
    ...
    static async bulkDisAssociateSnplocusWithSnp_matrix_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        var promises = [];
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkDisAssociateSnplocusWithSnp_matrix_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
        await Promise.all(promises);
        return "Records successfully updated!";
    }
     ...
     */
    static idAttribute() {
        return snplocus.definition.id.name;
    }
    ...
    static idAttributeType() {
        return snplocus.definition.id.type;
    }

    /**
     * getIdValue - Get the value of the idAttribute ("id", or "internalId") for an instance of snplocus.
     *
     * @return {type} id value
     */
    getIdValue() {
        return this[snplocus.idAttribute()];
    }
    ...
    /**
     * base64Encode - Encode  snplocus to a base 64 String
     *
     * @return {string} The snplocus object, encoded in a base 64 String
     */
    base64Encode() {
    ...
    /**
     * stripAssociations - Instance method for getting all attributes of snplocus.
     *
     * @return {object} The attributes of snplocus in object form
     */
    stripAssociations() {
        let attributes = Object.keys(snplocus.definition.attributes);
        attributes.push('id');
    ...
    /**
     * externalIdsArray - Get all attributes of snplocus that are marked as external IDs.
     *
     * @return {Array<String>} An array of all attributes of snplocus that are marked as external IDs
     */
    ...
    /**
     * externalIdsObject - Get all external IDs of snplocus.
     *
