...
const definition = {
    ...
    "registry": ["snpgenotype_local", "snpgenotype_public"],
    "attributes": {
        "snp_matrix_id": "Int",
        "material_id": "String",
        "row_number": "Int"
...
let registry = ["snpgenotype_local", "snpgenotype_public"];

module.exports = class snpgenotype {
    ...
    static get registeredAdapters() {
        return ["snpgenotype_local", "snpgenotype_public"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readById(id, benignErrorReporter, token) {
        if (id !== null) {
            ...
            return adapters[responsibleAdapter[0]].readById(id, benignErrorReporter, token).then(result => {
                let item = new snpgenotype(result);
    ...
    static readAllCursor(search, order, pagination, authorizedAdapters, benignErrorReporter, searchAuthorizedAdapters, token) {
        ...
        return Promise.allSettled(promises)
            ...
            .then(async nodes => {
                ...
                let graphQLConnection = helper.toGraphQLConnectionObject(paginated_records, this, hasNextPage, hasPreviousPage, "snpgenotypes");
    ...
    static async addOne(input, benignErrorReporter, token) {
        ...
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter, token).then(result => new snpgenotype(result));
    }
    ...
    static async updateOne(input, benignErrorReporter, token) {
        ...
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter, token).then(result => new snpgenotype(result));
    ...
    /**
     * add_snp_matrix_id - field Mutation (model-layer) for to_one associationsArguments to add
     *
     ...
     */
    static async add_snp_matrix_id(id, snp_matrix_id, benignErrorReporter, token, handle_inverse) {
        try {
            let responsibleAdapter = this.adapterForIri(id);
            return await adapters[responsibleAdapter].add_snp_matrix_id(id, snp_matrix_id, benignErrorReporter, token, handle_inverse);
        } catch (error) {
    ...
    /**
     * remove_snp_matrix_id - field Mutation (model-layer) for to_one associationsArguments to remove
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Id}   snp_matrix_id Foreign Key (stored in "Me") of the Association to be updated.
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_snp_matrix_id(id, snp_matrix_id, benignErrorReporter, token, handle_inverse) {
        try {
            let responsibleAdapter = this.adapterForIri(id);
            return await adapters[responsibleAdapter].remove_snp_matrix_id(id, snp_matrix_id, benignErrorReporter, token, handle_inverse);
        } catch (error) {
            benignErrorReporter.push({
                message: error,
            });
    ...
    static async bulkAssociateSnpgenotypeWithSnp_matrix_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkAssociateSnpgenotypeWithSnp_matrix_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
    ...
    static async bulkDisAssociateSnpgenotypeWithSnp_matrix_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        var promises = [];
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkDisAssociateSnpgenotypeWithSnp_matrix_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
        await Promise.all(promises);
        return "Records successfully updated!";
    }
     ...
     */
    static idAttribute() {
        return snpgenotype.definition.id.name;
    }
    ...
    static idAttributeType() {
        return snpgenotype.definition.id.type;
    }

    /**
     * getIdValue - Get the value of the idAttribute ("id", or "internalId") for an instance of snpgenotype.
     *
     * @return {type} id value
     */
    getIdValue() {
        return this[snpgenotype.idAttribute()];
    }
    ...
    /**
     * base64Encode - Encode  snpgenotype to a base 64 String
     *
     * @return {string} The snpgenotype object, encoded in a base 64 String
     */
    base64Encode() {
    ...
    /**
     * stripAssociations - Instance method for getting all attributes of snpgenotype.
     *
     * @return {object} The attributes of snpgenotype in object form
     */
    stripAssociations() {
        let attributes = Object.keys(snpgenotype.definition.attributes);
        attributes.push('id');
    ...
    /**
     * externalIdsArray - Get all attributes of snpgenotype that are marked as external IDs.
     *
     * @return {Array<String>} An array of all attributes of snpgenotype that are marked as external IDs
     */
    ...
    /**
     * externalIdsObject - Get all external IDs of snpgenotype.
     *
