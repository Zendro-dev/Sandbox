...
const definition = {
    ...
    "registry": ["study_local", "study_public"],
    "attributes": {
        "id": "String",
        "title": "String",
        "description": "String",
...
let registry = ["study_local", "study_public"];
...
module.exports = class study {
    ...
    constructor({
        ...
        this.experimental_site_name = experimental_site_name;
        this.experimental_design_type = experimental_design_type;
        this.experimental_design_description = experimental_design_description;
        this.experimental_design_map = experimental_design_map;
        this.observation_unit_level_hirarchy = observation_unit_level_hirarchy;
        this.observation_unit_description = observation_unit_description;
        this.growth_facility = growth_facility;
        this.growth_facility_description = growth_facility_description;
        this.cultural_practices = cultural_practices;
        this.investigation_id = investigation_id;
        this.person_ids = person_ids;
        this.observed_variable_ids = observed_variable_ids;
        this.biological_material_ids = biological_material_ids;
    }
    ...
    static get registeredAdapters() {
        return ["study_local", "study_public"].reduce((a, c) => {
            a[c] = adapters[c];
            return a;
        }, {});
    }
    ...
    static readAllCursor(search, order, pagination, authorizedAdapters, benignErrorReporter, searchAuthorizedAdapters, token) {
        ...
        return Promise.allSettled(promises)
            ...
            .then(async nodes => {
                ...
                let graphQLConnection = helper.toGraphQLConnectionObject(paginated_records, this, hasNextPage, hasPreviousPage, "studies");
    ...
    static async addOne(input, benignErrorReporter, token) {
        ...
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter, token).then(result => new study(result));
    }
    ...
    static async updateOne(input, benignErrorReporter, token) {
        ...
        let responsibleAdapter = this.adapterForIri(input.id);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter, token).then(result => new study(result));
     ...
     */
    static async add_investigation_id(id, investigation_id, benignErrorReporter, token, handle_inverse) {
        try {
            let responsibleAdapter = this.adapterForIri(id);
            return await adapters[responsibleAdapter].add_investigation_id(id, investigation_id, benignErrorReporter, token, handle_inverse);
        } catch (error) {
     ...
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   person_ids Foreign Key (stored in "Me") of the Association to be updated.
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_person_ids(id, person_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_person_ids(id, person_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
     * add_observed_variable_ids - field Mutation (model-layer) for to_many associationsArguments to add
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   observed_variable_ids Foreign Key (stored in "Me") of the Association to be updated.
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
     * add_biological_material_ids - field Mutation (model-layer) for to_many associationsArguments to add
     *
     * @param {Id}   id   IdAttribute of the root model to be updated
     * @param {Array}   biological_material_ids Foreign Key (stored in "Me") of the Association to be updated.
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     * @param {string} token The token used for authorization
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async add_biological_material_ids(id, biological_material_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_biological_material_ids(id, biological_material_ids, benignErrorReporter, token, handle_inverse);
    }
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_investigation_id(id, investigation_id, benignErrorReporter, token, handle_inverse) {
        try {
            let responsibleAdapter = this.adapterForIri(id);
            return await adapters[responsibleAdapter].remove_investigation_id(id, investigation_id, benignErrorReporter, token, handle_inverse);
        } catch (error) {
            benignErrorReporter.push({
                message: error,
            });
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_person_ids(id, person_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_person_ids(id, person_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, token, handle_inverse);
    }
    /**
     ...
     * @param {boolean} handle_inverse Handle inverse association
     */
    static async remove_biological_material_ids(id, biological_material_ids, benignErrorReporter, token, handle_inverse) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_biological_material_ids(id, biological_material_ids, benignErrorReporter, token, handle_inverse);
    }
    ...
    static async bulkAssociateStudyWithInvestigation_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkAssociateStudyWithInvestigation_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
    ...
    static async bulkDisAssociateStudyWithInvestigation_id(bulkAssociationInput, benignErrorReporter, token) {
        ...
        var promises = [];
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkDisAssociateStudyWithInvestigation_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter, token))
        });
        await Promise.all(promises);
        return "Records successfully updated!";
    }
