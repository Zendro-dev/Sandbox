...
module.exports = class study {
    ...
    static async addOne(input, benignErrorReporter) {
        ...
        //use default BenignErrorReporter if no BenignErrorReporter defined
        benignErrorReporter = errorHelper.getDefaultBenignErrorReporterIfUndef(benignErrorReporter);
        return adapters[responsibleAdapter].addOne(input, benignErrorReporter).then(result => new study(result));
    }
    ...
    static async updateOne(input, benignErrorReporter) {
        ...
        //use default BenignErrorReporter if no BenignErrorReporter defined
        benignErrorReporter = errorHelper.getDefaultBenignErrorReporterIfUndef(benignErrorReporter);
        return adapters[responsibleAdapter].updateOne(input, benignErrorReporter).then(result => new study(result));
     ...
     */
    static async add_investigation_id(id, investigation_id, benignErrorReporter) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_investigation_id(id, investigation_id, benignErrorReporter);
    }
    ...
    static async add_person_ids(id, person_ids, benignErrorReporter, handle_inverse = true) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_person_ids(id, person_ids, benignErrorReporter, handle_inverse);
    }
    /**
    ...
    static async add_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, handle_inverse = true) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, handle_inverse);
    }
    /**
    ...
    static async add_biological_material_ids(id, biological_material_ids, benignErrorReporter, handle_inverse = true) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].add_biological_material_ids(id, biological_material_ids, benignErrorReporter, handle_inverse);
    }
     ...
     * @param {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     */
    static async remove_investigation_id(id, investigation_id, benignErrorReporter) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_investigation_id(id, investigation_id, benignErrorReporter);
    }
    ...
    static async remove_person_ids(id, person_ids, benignErrorReporter, handle_inverse = true) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_person_ids(id, person_ids, benignErrorReporter, handle_inverse);
    }
    /**
    ...
    static async remove_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, handle_inverse = true) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_observed_variable_ids(id, observed_variable_ids, benignErrorReporter, handle_inverse);
    }
    /**
    ...
    static async remove_biological_material_ids(id, biological_material_ids, benignErrorReporter, handle_inverse = true) {
        let responsibleAdapter = this.adapterForIri(id);
        return await adapters[responsibleAdapter].remove_biological_material_ids(id, biological_material_ids, benignErrorReporter, handle_inverse);
    }
     ...
     */
    static async bulkAssociateStudyWithInvestigation_id(bulkAssociationInput, benignErrorReporter) {
        ...
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkAssociateStudyWithInvestigation_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter))
        });
     ...
     * @return {string} returns message on success
     */
    static async bulkDisAssociateStudyWithInvestigation_id(bulkAssociationInput, benignErrorReporter) {
        let mappedBulkAssociateInputToAdapters = this.mapBulkAssociationInputToAdapters(bulkAssociationInput);
        var promises = [];
        Object.keys(mappedBulkAssociateInputToAdapters).forEach(responsibleAdapter => {
            promises.push(adapters[responsibleAdapter].bulkDisAssociateStudyWithInvestigation_id(mappedBulkAssociateInputToAdapters[responsibleAdapter], benignErrorReporter))
        });
        await Promise.all(promises);
        return "Records successfully updated!";
    }
