...
module.exports = class river extends Sequelize.Model {
     ...
     */
    static async add_country_ids(river_id, country_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            country_ids.forEach(idx => {
                promises.push(models.country.add_river_ids(idx, [`${river_id}`], benignErrorReporter, false));
            });
     ...
     * @param {Array}   country_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async remove_country_ids(river_id, country_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            country_ids.forEach(idx => {
                promises.push(models.country.remove_river_ids(idx, [`${river_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
