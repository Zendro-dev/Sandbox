...
module.exports = class investigation extends Sequelize.Model {
    ...
    static async readById(id) {
        let item = await investigation.findByPk(id);
        if (item === null) {
            throw new Error(`Record with ID = "${id}" does not exist`);
        }
        item = investigation.postReadCast(item)
     ...
     *
     * @param {Id}   investigation_id   IdAttribute of the root model to be updated
     * @param {Array}   contact_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async add_contact_ids(investigation_id, contact_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            contact_ids.forEach(idx => {
                promises.push(models.contact.add_investigation_ids(idx, [`${investigation_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }

        let record = await super.findByPk(investigation_id);
        if (record !== null) {
     ...
     */
    static async add_ontologyAnnotation_ids(investigation_id, ontologyAnnotation_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            ontologyAnnotation_ids.forEach(idx => {
                promises.push(models.ontologyAnnotation.add_investigation_ids(idx, [`${investigation_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
     ...
     */
    static async add_fileAttachment_ids(investigation_id, fileAttachment_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            fileAttachment_ids.forEach(idx => {
                promises.push(models.fileAttachment.add_investigation_ids(idx, [`${investigation_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {Array}   contact_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async remove_contact_ids(investigation_id, contact_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            contact_ids.forEach(idx => {
                promises.push(models.contact.remove_investigation_ids(idx, [`${investigation_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {Array}   ontologyAnnotation_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async remove_ontologyAnnotation_ids(investigation_id, ontologyAnnotation_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            ontologyAnnotation_ids.forEach(idx => {
                promises.push(models.ontologyAnnotation.remove_investigation_ids(idx, [`${investigation_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {Array}   fileAttachment_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async remove_fileAttachment_ids(investigation_id, fileAttachment_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            fileAttachment_ids.forEach(idx => {
                promises.push(models.fileAttachment.remove_investigation_ids(idx, [`${investigation_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
