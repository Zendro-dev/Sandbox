...
module.exports = class contact extends Sequelize.Model {
    ...
    static async batchReadById(keys) {
        let queryArg = {
            operator: "in",
            field: contact.idAttribute(),
            value: keys.join(),
            valueType: "Array",
        };
        let cursorRes = await contact.readAllCursor(queryArg);
        cursorRes = cursorRes.contacts.reduce(
            (map, obj) => ((map[obj[contact.idAttribute()]] = obj), map), {}
        );
        return keys.map(
            (key) =>
    ...
    static readByIdLoader = new DataLoader(contact.batchReadById, {
        cache: false,
    });

    static async readById(id) {
        return await contact.readByIdLoader.load(id);
    }
    ...
    static async readAllCursor(search, order, pagination, benignErrorReporter) {
        ...
        return {
            edges,
            pageInfo,
            contacts: edges.map((edge) => edge.node)
        };
    }
