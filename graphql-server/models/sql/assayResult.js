...
const definition = {
    ...
    associations: {
        assay: {
            ...
            reverseAssociation: 'assayResults',
...
module.exports = class assayResult extends Sequelize.Model {
    ...
    static async batchReadById(keys) {
        let queryArg = {
            operator: "in",
            field: assayResult.idAttribute(),
            value: keys.join(),
            valueType: "Array",
        };
        let cursorRes = await assayResult.readAllCursor(queryArg);
        cursorRes = cursorRes.assayResults.reduce(
            (map, obj) => ((map[obj[assayResult.idAttribute()]] = obj), map), {}
        );
        return keys.map(
            (key) =>
    ...
    static readByIdLoader = new DataLoader(assayResult.batchReadById, {
        cache: false,
    });

    static async readById(id) {
        return await assayResult.readByIdLoader.load(id);
    }
    ...
    static async readAllCursor(search, order, pagination, benignErrorReporter) {
        ...
        return {
            edges,
            pageInfo,
            assayResults: edges.map((edge) => edge.node)
        };
    }
     ...
     */
    static async add_material_ids(assayResult_id, material_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            material_ids.forEach(idx => {
                promises.push(models.material.add_assayResult_ids(idx, [`${assayResult_id}`], benignErrorReporter, false));
            });
     ...
     * @param {Array}   material_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async remove_material_ids(assayResult_id, material_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            material_ids.forEach(idx => {
                promises.push(models.material.remove_assayResult_ids(idx, [`${assayResult_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
