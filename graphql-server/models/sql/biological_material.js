...
module.exports = class biological_material extends Sequelize.Model {
    ...
    static async batchReadById(keys) {
        let queryArg = {
            operator: "in",
            field: biological_material.idAttribute(),
            value: keys.join(),
            valueType: "Array",
        };
        let cursorRes = await biological_material.readAllCursor(queryArg);
        cursorRes = cursorRes.biological_materials.reduce(
            (map, obj) => ((map[obj[biological_material.idAttribute()]] = obj), map), {}
        );
        return keys.map(
            (key) =>
    ...
    static readByIdLoader = new DataLoader(biological_material.batchReadById, {
        cache: false,
    });

    static async readById(id) {
        return await biological_material.readByIdLoader.load(id);
    }
    static async countRecords(search) {
        let options = {}
        options['where'] = helper.searchConditionsToSequelize(search, biological_material.definition.attributes);
    ...
    static async readAll(search, order, pagination, benignErrorReporter) {
        ...
        let options = helper.buildLimitOffsetSequelizeOptions(search, order, pagination, this.idAttribute(), biological_material.definition.attributes);
        let records = await super.findAll(options);
        records = records.map(x => biological_material.postReadCast(x))
    ...
    static async readAllCursor(search, order, pagination, benignErrorReporter) {
        ...
        let options = helper.buildCursorBasedSequelizeOptions(search, order, pagination, this.idAttribute(), biological_material.definition.attributes);
        let records = await super.findAll(options);
        ...
        if (pagination && (pagination.after || pagination.before)) {
            let oppOptions = helper.buildOppositeSearchSequelize(search, order, {
                ...pagination,
                includeCursor: false
            }, this.idAttribute(), biological_material.definition.attributes);
    ...
    static async addOne(input) {
        ...
        input = biological_material.preWriteCast(input)
        try {
            ...
            biological_material.postReadCast(result.dataValues)
            biological_material.postReadCast(result._previousDataValues)
            return result;
        } catch (error) {
            throw error;
        }
    ...
    static bulkAddCsv(context) {
        ...
        return `Bulk import of biological_material records started. You will be send an email to ${helpersAcl.getTokenFromContext(context).email} informing you about success or errors`;
    }
