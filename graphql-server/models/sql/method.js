...
module.exports = class method extends Sequelize.Model {
    ...
    static associate(models) {
        method.belongsTo(models.ontologyReference, {
            as: 'ontologyReference',
            foreignKey: 'ontologyDbId'
        });
        method.hasMany(models.observationVariable, {
    ...
    static async readById(id) {
        let item = await method.findByPk(id);
        if (item === null) {
            throw new Error(`Record with ID = "${id}" does not exist`);
        }
        item = method.postReadCast(item)
    ...
    static bulkAddCsv(context) {
        ...
        return `Bulk import of method records started. You will be send an email to ${helpersAcl.getTokenFromContext(context).email} informing you about success or errors`;
    }
    ...
    /**
     * bulkAssociateMethodWithOntologyDbId - bulkAssociaton of given ids
     *
     * @param  {array} bulkAssociationInput Array of associations to add
     * @param  {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     * @return {string} returns message on success
     */
    static async bulkAssociateMethodWithOntologyDbId(bulkAssociationInput) {
        let mappedForeignKeys = helper.mapForeignKeysToPrimaryKeyArray(bulkAssociationInput, "methodDbId", "ontologyDbId");
        var promises = [];
    ...
    /**
     * bulkDisAssociateMethodWithOntologyDbId - bulkDisAssociaton of given ids
     *
     * @param  {array} bulkAssociationInput Array of associations to remove
     * @param  {BenignErrorReporter} benignErrorReporter Error Reporter used for reporting Errors from remote zendro services
     * @return {string} returns message on success
     */
    static async bulkDisAssociateMethodWithOntologyDbId(bulkAssociationInput) {
        let mappedForeignKeys = helper.mapForeignKeysToPrimaryKeyArray(bulkAssociationInput, "methodDbId", "ontologyDbId");
        var promises = [];
        mappedForeignKeys.forEach(({
            ontologyDbId,
