...
module.exports = class environment extends Sequelize.Model {
    ...
    static associate(models) {
        environment.belongsTo(models.study, {
            as: 'study',
            foreignKey: 'study_id'
        });
    }
    ...
    static async batchReadById(keys) {
        let queryArg = {
            operator: "in",
            field: environment.idAttribute(),
            value: keys.join(),
            valueType: "Array",
        };
        let cursorRes = await environment.readAllCursor(queryArg);
        cursorRes = cursorRes.environments.reduce(
            (map, obj) => ((map[obj[environment.idAttribute()]] = obj), map), {}
        );
        return keys.map(
            (key) =>
    ...
    static readByIdLoader = new DataLoader(environment.batchReadById, {
        cache: false,
    });
     ...
     * @return {object} The requested record as an object with the type environment, or an error object if the validation after reading fails
     * @throws {Error} If the requested record does not exist
     */
    static async readById(id) {
        return await environment.readByIdLoader.load(id);
    }
    /**
    ...
    static async countRecords(search) {
        let options = {}
        options['where'] = helper.searchConditionsToSequelize(search, environment.definition.attributes);
    ...
    static async readAll(search, order, pagination, benignErrorReporter) {
        ...
        let options = helper.buildLimitOffsetSequelizeOptions(search, order, pagination, this.idAttribute(), environment.definition.attributes);
        let records = await super.findAll(options);
        records = records.map(x => environment.postReadCast(x))
    ...
    static async readAllCursor(search, order, pagination, benignErrorReporter) {
        ...
        let options = helper.buildCursorBasedSequelizeOptions(search, order, pagination, this.idAttribute(), environment.definition.attributes);
        let records = await super.findAll(options);
        ...
        if (pagination && (pagination.after || pagination.before)) {
            let oppOptions = helper.buildOppositeSearchSequelize(search, order, {
                ...pagination,
                includeCursor: false
            }, this.idAttribute(), environment.definition.attributes);
    ...
    static async addOne(input) {
        ...
        input = environment.preWriteCast(input)
        try {
            ...
            environment.postReadCast(result.dataValues)
            environment.postReadCast(result._previousDataValues)
            return result;
        } catch (error) {
            throw error;
        }
    ...
    static async add_study_id(id, study_id, benignErrorReporter) {
        try {
            let updated = await environment.update({
                study_id: study_id
            }, {
                where: {
                    id: id
