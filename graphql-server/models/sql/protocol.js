...
module.exports = class protocol extends Sequelize.Model {
    ...
    static async readById(id) {
        let item = await protocol.findByPk(id);
        if (item === null) {
            throw new Error(`Record with ID = "${id}" does not exist`);
        }
        item = protocol.postReadCast(item)
     ...
     */
    static async add_fileAttachment_ids(protocol_id, fileAttachment_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            fileAttachment_ids.forEach(idx => {
                promises.push(models.fileAttachment.add_protocol_ids(idx, [`${protocol_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {Array}   fileAttachment_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async remove_fileAttachment_ids(protocol_id, fileAttachment_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            fileAttachment_ids.forEach(idx => {
                promises.push(models.fileAttachment.remove_protocol_ids(idx, [`${protocol_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
