...
module.exports = class study extends Sequelize.Model {
     ...
     */
    static async add_fileAttachment_ids(study_id, fileAttachment_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            fileAttachment_ids.forEach(idx => {
                promises.push(models.fileAttachment.add_study_ids(idx, [`${study_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
     ...
     * @param {Array}   fileAttachment_ids Array foreign Key (stored in "Me") of the Association to be updated.
     */
    static async remove_fileAttachment_ids(study_id, fileAttachment_ids, benignErrorReporter, handle_inverse = true) {
        //handle inverse association
        if (handle_inverse) {
            let promises = [];
            fileAttachment_ids.forEach(idx => {
                promises.push(models.fileAttachment.remove_study_ids(idx, [`${study_id}`], benignErrorReporter, false));
            });
            await Promise.all(promises);
        }
