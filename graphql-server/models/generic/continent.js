...
module.exports = class continent {
  ...
  static async readAllCursor(search, order, pagination, benignErrorReporter) {
    ...
    let options = helper.buildCursorBasedGenericOptions(
      search,
      order,
      pagination,
      this.idAttribute()
    );
    let records = await continent.readAll(
      options["search"],
      options["order"],
      options["pagination"],
      benignErrorReporter
    ...
    if (pagination && (pagination.after || pagination.before)) {
      let oppOptions = helper.buildOppositeSearchGeneric(
        search,
        order,
        pagination,
        this.idAttribute()
      );
      oppRecords = await continent.readAll(
        oppOptions["search"],
        oppOptions["order"],
        oppOptions["pagination"],
        benignErrorReporter
      );
    }
