...
user.prototype.rolesFilter = async function({
    ...
    if (await checkAuthorization(context, 'role', 'read') === true) {
        await checkCountAndReduceRecordsLimit(search, context, 'rolesFilter', 'role');
...
user.prototype.rolesConnection = async function({
    ...
    if (await checkAuthorization(context, 'role', 'read') === true) {
        await checkCountAndReduceRecordsLimit(search, context, 'rolesConnection', 'role');
 ...
 */
async function checkCountAndReduceRecordsLimit(search, context, resolverName, modelName = 'user') {
