...
export default {
  ...
  tableTemplate(url) {
    let query = `query {csvTableTemplatePlant_variant}`
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('plant_variant', searchText, ops);
    
    var query = '';

    //if has search
    if (s !== null) {
      query = `{ countPlant_variants(${s}) }`;
    }
    else {
      query = `{ countPlant_variants }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query plant_variantsConnection($pagination: paginationCursorInput)
            { plant_variantsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query plant_variantsConnection($pagination: paginationCursorInput)
            { plant_variantsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query plant_variantsConnection($pagination: paginationCursorInput)
          { plant_variantsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
        ...
        query =
          `query plant_variantsConnection($pagination: paginationCursorInput)
          { plant_variantsConnection( pagination: $pagination ) {
            ${qbody}
          }}`
      }
    }
   ...
   * createItem
   * 
   * Add new Plant_variant item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Plant_variant item. 
   */
    ...
    var query = 
      `mutation
        addPlant_variant(
          ...
          $addTomato_measurements: [ID],
          ) { addPlant_variant(
            plant_variant_ID:$plant_variant_ID,
            name:$name,
            genotype:$genotype,
            disease_resistances:$disease_resistances,
            addTomato_measurements: $addTomato_measurements,
          ) {
            name,
            genotype,
            disease_resistances,
   ...
   * updateItem
   * 
   * Update Plant_variant item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Plant_variant item.  
   */
      ...
      `mutation
        updatePlant_variant(
          ...
          $removeTomato_measurements: [ID],
            ...
            disease_resistances: $disease_resistances,
            addTomato_measurements: $addTomato_measurements,
            removeTomato_measurements: $removeTomato_measurements,
          ) {
            name,
            genotype,
            disease_resistances,
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Plant_variant item. 
   */
  ...
  /**
   * getTomato_measurementsConnection
   * 
   * Get tomato_Measurements connection (cursor based) records associated to the given plant_variant record
   * through association 'Tomato_measurements', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  getTomato_measurementsConnection(url, itemId, label, sublabel, searchText, variables, ops) {
    //search
    var s = getSearchArgument('tomato_Measurement', searchText, ops); 
    ...
    var query = (s) ?
      `query readOnePlant_variant($pagination: paginationCursorInput) {
        readOnePlant_variant(plant_variant_ID: "${itemId}") { 
          Tomato_measurementsConnection( ${s}, pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredTomato_measurements( ${s} ) 
      } }` :      
      `query readOnePlant_variant($pagination: paginationCursorInput) {
        readOnePlant_variant(plant_variant_ID: "${itemId}") { 
          Tomato_measurementsConnection( pagination: $pagination ) { 
            ${qbody},
          },
          countFilteredTomato_measurements 
      } }`;
   ...
   * 
   *  
   * getAssociatedTomato_measurementsConnection
   * 
   * Get the tomato_Measurement-ids associated (by cursor based connection) to the given plant_variant record 
   * through association 'Tomato_measurements', from GraphQL Server.
   * 
   * @param {String} url GraphQL Server url
   * @param {Number} itemId Model item internalId.
   */
  getAssociatedTomato_measurementsConnection(url, itemId) {
    var query = 
      `{ readOnePlant_variant( plant_variant_ID: "${itemId}" ){ 
        Tomato_measurementsConnection{ edges { node { tomato_ID } } } } }`;
    /**
     * Debug
     */
