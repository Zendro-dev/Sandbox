...
import TomatoMeasurementsToAddTransferViewToolbar from './components/Tomato_measurementsToAddTransferViewToolbar';
import TomatoMeasurementsToAddTransferViewCursorPagination from './components/Tomato_measurementsToAddTransferViewCursorPagination';
...
export default function TomatoMeasurementsToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.plant_variant.getAssociatedTomato_measurementsConnection(graphqlServerUrl, item.plant_variant_ID));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Plant_variant', association: 'Tomato_measurements', table:'A', query: 'readOnePlant_variant', method: 'getData()', request: 'api.plant_variant.getAssociatedTomato_measurementsConnection'}];
          newError.path=['add', 'Tomato_measurements'];
        ...
        //get items
        let idso = readOnePlant_variant.Tomato_measurementsConnection.edges.map(o => o.node);
        ...
        //set associated ids
        lidsAssociated.current = idso.map(function(item){ return item.tomato_ID});
        ...
        */
        let cancelableApiReqB = makeCancelable(api.tomato_Measurement.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'Plant_variant', association: 'Tomato_measurements', table:'A', query: 'countTomato_Measurements', method: 'getData()', request: 'api.tomato_Measurement.getCountItems'}];
              newError.path=['add', 'Tomato_measurements'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            */
            ...
            let cancelableApiReqC = makeCancelable(api.tomato_Measurement.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'Plant_variant', association: 'Tomato_measurements', table:'A', query: 'tomato_MeasurementsConnection', method: 'getData()', request: 'api.tomato_Measurement.getItemsConnection'}];
                  newError.path=['add', 'Tomato_measurements'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                if(tomato_MeasurementsConnection === null) {
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'Plant_variant', association: 'Tomato_measurements', table:'A', method: 'getData()'}];
                    newError.path=['add', 'Tomato_measurements'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.plant_variant.getAssociatedTomato_measurementsConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(() => {
    ...
    if(lidsToAdd.current !== undefined && lidsToAdd.current.length > 0) {
      ops = {
        only: [{
          type: 'String',
          values: {tomato_ID: lidsToAdd.current}
        }]
      };
    } else {
  ...
  useEffect(() => {
     ...
     * 
     * else if B:
     * - remove the internalId of the 'Tomato_Measurement' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
            ...
            if(entry[1].changedAssociations.Plant_variant.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.tomato_ID;
                  ...
                  handleUntransfer('Tomato_measurements', idAdded);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.tomato_ID) === -1) {
      lidsToAdd.current.push(item.tomato_ID);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.tomato_ID);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <TomatoMeasurementsToAddTransferViewToolbar
              ...
              {/* Pagination */}
              <TomatoMeasurementsToAddTransferViewCursorPagination
                count={count}
...
TomatoMeasurementsToAddTransferView.propTypes = {
