...
import PlantVariantEnhancedTableHead from './components/Plant_variantEnhancedTableHead'
import PlantVariantEnhancedTableToolbar from './components/Plant_variantEnhancedTableToolbar'
import PlantVariantCreatePanel from './components/plant_variant-create-panel/Plant_variantCreatePanel'
import PlantVariantUpdatePanel from './components/plant_variant-update-panel/Plant_variantUpdatePanel'
import PlantVariantDetailPanel from './components/plant_variant-detail-panel/Plant_variantDetailPanel'
import PlantVariantDeleteConfirmationDialog from './components/Plant_variantDeleteConfirmationDialog'
import PlantVariantUploadFileDialog from './components/Plant_variantUploadFileDialog'
import PlantVariantCursorPagination from './components/Plant_variantCursorPagination'
...
//Plotly
import PlantVariantPlotly from '../../../../plots/Plant_variantPlotly';
...
export default function PlantVariantEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('plant_variant_ID');
  ...
  const getData = useCallback(() => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.plant_variant.getCountItems(graphqlServerUrl, search));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Plant_variant', query: 'countPlant_variants', method: 'getData()', request: 'api.plant_variant.getCountItems'}];
          newError.path=['Plant_variants'];
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variant.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'Plant_variant', query: 'plant_variantsConnection', method: 'getData()', request: 'api.plant_variant.getItemsConnection'}];
                newError.path=['Plant_variants'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errors.current.length > 0) {
                ...
                newError.locations=[{model: 'Plant_variant', method: 'getData()'}];
                newError.path=['Plant_variants'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
  ...
  function doDelete(event, item) {
    ...
    let variables = {};
    //set plant_variant_ID (internalId)
    variables.plant_variant_ID = item.plant_variant_ID;

    /*
      API Request: deletePlant_variant
    */
    let cancelableApiReq = makeCancelable(api.plant_variant.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'Plant_variant', query: 'deletePlant_variant', method: 'doDelete()', request: 'api.plant_variant.deleteItem'}];
          newError.path=['Plant_variants', `plant_variant_ID:${item.plant_variant_ID}`, 'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: deletePlant_variant
        let deletePlant_variant = response.data.data.deletePlant_variant;
        if(deletePlant_variant === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'deletePlant_variant ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'Plant_variant', query: 'deletePlant_variant', method: 'doDelete()', request: 'api.plant_variant.deleteItem'}];
          newError.path=['Plant_variants', `plant_variant_ID:${item.plant_variant_ID}` ,'delete'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        /**
         * Type of deletePlant_variant is not validated. Only not null is
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'deletePlant_variant ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'Plant_variant', query: 'deletePlant_variant', method: 'doDelete()', request: 'api.plant_variant.deleteItem'}];
          newError.path=['Plant_variants', `plant_variant_ID:${item.plant_variant_ID}` ,'delete'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
      ...
      //error
      .catch((err) => { //error: on api.plant_variant.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  function getCsvTemplate() {
    ...
    /*
      API Request: csvTableTemplatePlant_variant
    */
    let cancelableApiReq = makeCancelable(api.plant_variant.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'Plant_variant', query: 'csvTableTemplatePlant_variant', method: 'getCsvTemplate()', request: 'api.plant_variant.tableTemplate'}];
          newError.path=['Plant_variants', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
          console.log("Error: ", newError);
        ...
        //check: csvTableTemplatePlant_variant
        let csvTableTemplatePlant_variant = response.data.data.csvTableTemplatePlant_variant;
        if(csvTableTemplatePlant_variant === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplatePlant_variant ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'Plant_variant', query: 'csvTableTemplatePlant_variant', method: 'getCsvTemplate()', request: 'api.plant_variant.tableTemplate'}];
          newError.path=['Plant_variants', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: csvTableTemplatePlant_variant type
        if(!Array.isArray(csvTableTemplatePlant_variant)) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplatePlant_variant ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'Plant_variant', query: 'csvTableTemplatePlant_variant', method: 'getCsvTemplate()', request: 'api.plant_variant.tableTemplate'}];
          newError.path=['Plant_variants', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'csvTableTemplatePlant_variant ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'Plant_variant', query: 'csvTableTemplatePlant_variant', method: 'getCsvTemplate()', request: 'api.plant_variant.tableTemplate'}];
          newError.path=['Plant_variants', 'csvTemplate'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //download
        let file = response.data.data.csvTableTemplatePlant_variant.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "plant_variant-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.plant_variant.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <PlantVariantEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(PlantVariantPlotly)}
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.plant_variant&&Array.isArray(permissions.plant_variant)
                                      &&(permissions.plant_variant.includes('update') || permissions.plant_variant.includes('*')))
                                      &&(
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.plant_variant&&Array.isArray(permissions.plant_variant)
                                      &&(permissions.plant_variant.includes('delete') || permissions.plant_variant.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'plant_variant_ID: ' + item.plant_variant_ID}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.plant_variant_ID}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.genotype!==null)?item.genotype:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.disease_resistances!==null)?item.disease_resistances:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <PlantVariantDeleteConfirmationDialog
          permissions={permissions}
...
PlantVariantEnhancedTable.propTypes = {
