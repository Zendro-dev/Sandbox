...
import PlantVariantUpdatePanel from '../plant_variant-update-panel/Plant_variantUpdatePanel'
import PlantVariantDeleteConfirmationDialog from '../Plant_variantDeleteConfirmationDialog'
...
export default function PlantVariantDetailPanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.Plant_variant[String(itemState.plant_variant_ID)]) {
          
        //updated item
        if(lastModelChanged.Plant_variant[String(itemState.plant_variant_ID)].op === "update"&&
            lastModelChanged.Plant_variant[String(itemState.plant_variant_ID)].newItem) {
              //replace item
              setItemState(lastModelChanged.Plant_variant[String(itemState.plant_variant_ID)].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.Plant_variant[String(itemState.plant_variant_ID)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, itemState.plant_variant_ID]);
  ...
  function doDelete(event, item) {
    ...
    */
    ...
    cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on deletePlant_variant
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('Plant_variant', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('Plant_variant', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Plant_variant | plant_variant_ID: " + itemState.plant_variant_ID}
              </Typography>
              {/*
                Actions:
                - Edit
...
PlantVariantDetailPanel.propTypes = {
