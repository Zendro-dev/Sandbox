...
export default function PlantVariantCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.genotype = null;
    initialValues.disease_resistances = null;
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.genotype = 0;
    initialValueOkStates.disease_resistances = 0;
    initialValueOkStates.plant_variant_ID = 0;
  ...
  function doSave(event) {
    ...
    */
    ...
    /*
      API Request: addPlant_variant
    */
    let cancelableApiReq = makeCancelable(api.plant_variant.createItem(graphqlServerUrl, variables));
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Plant_variant', query: 'addPlant_variant', method: 'doSave()', request: 'api.plant_variant.createItem'}];
          newError.path=['Plant_variants', 'add'];
        ...
        //check: addPlant_variant
        let addPlant_variant = response.data.data.addPlant_variant;
        if(addPlant_variant === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addPlant_variant ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'Plant_variant', query: 'addPlant_variant', method: 'doSave()', request: 'api.plant_variant.createItem'}];
          newError.path=['Plant_variants', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: addPlant_variant type
        if(typeof addPlant_variant !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addPlant_variant ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'Plant_variant', query: 'addPlant_variant', method: 'doSave()', request: 'api.plant_variant.createItem'}];
          newError.path=['Plant_variants', 'add'];
        ...
        onClose(event, true, addPlant_variant);
        return;
      },
      //rejected
      (err) => {
      ...
      //error
      .catch((err) => { //error: on api.plant_variant.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'Tomato_measurements':
        if(tomato_measurementsIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'Tomato_measurements') {
      let iof = tomato_measurementsIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        tomato_measurementsIdsToAdd.current.splice(iof, 1);
      }
      return;
      ...
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Plant_variant'}
          </Typography>
...
PlantVariantCreatePanel.propTypes = {
