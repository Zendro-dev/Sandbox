...
import PlantVariantToAddTransferViewToolbar from './components/Plant_variantToAddTransferViewToolbar';
import PlantVariantToAddTransferViewCursorPagination from './components/Plant_variantToAddTransferViewCursorPagination';
...
export default function PlantVariantToAddTransferView(props) {
  ...
  */
  ...
  const getData = useCallback(() => {
    ...
    /*
      API Request: readOneTomato_Measurement
    */
    let cancelableApiReq = makeCancelable(api.tomato_Measurement.getAssociatedPlant_variantConnection(graphqlServerUrl, item.tomato_ID));  
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'readOneTomato_Measurement', method: 'getData()', request: 'api.tomato_Measurement.getAssociatedPlant_variantConnection'}];
          newError.path=['add', 'Plant_variant'];
        ...
        //check: readOneTomato_Measurement
        let readOneTomato_Measurement = response.data.data.readOneTomato_Measurement;
        if(readOneTomato_Measurement === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneTomato_Measurement ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'readOneTomato_Measurement', method: 'getData()', request: 'api.tomato_Measurement.getAssociatedPlant_variantConnection'}];
          newError.path=['add', 'Plant_variant'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: readOneTomato_Measurement type
        if(typeof readOneTomato_Measurement !== 'object'
        || typeof readOneTomato_Measurement.plant_variant!== 'object' //can be null
        ) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'readOneTomato_Measurement ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'readOneTomato_Measurement', method: 'getData()', request: 'api.tomato_Measurement.getAssociatedPlant_variantConnection'}];
          newError.path=['add', 'Plant_variant'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //get item
        let idso = readOneTomato_Measurement.Plant_variant;

        //check: graphql errors
        if(response.data.errors) {
          let newError = {};
          newError.message = 'readOneTomato_Measurement ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'readOneTomato_Measurement', method: 'getData()', request: 'api.tomato_Measurement.getAssociatedPlant_variantConnection'}];
          newError.path=['add', 'Plant_variant'];
        ...
        //ok
        //set associated id
        lidsAssociated.current = (idso !== null) ? [idso.plant_variant_ID] : [];
        ...
        if(exIds.length > 0) {
          ops = {
            exclude: [{
              type: 'String',
              values: {plant_variant_ID: exIds}
            }]
          };
        }
        /*
          API Request: countPlant_variants
        */
        let cancelableApiReqB = makeCancelable(api.plant_variant.getCountItems(graphqlServerUrl, search, ops));
        ...
        cancelableApiReqB
          ...
          (response)=> {
            ...
            if(!response.data ||!response.data.data) {
              ...
              variant.current='error';
              newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
              newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'countPlant_variants', method: 'getData()', request: 'api.plant_variant.getCountItems'}];
              newError.path=['add', 'Plant_variant'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
              console.log("Error: ", newError);
            ...
            //check: countPlant_variants
            let countPlant_variants = response.data.data.countPlant_variants;
            if(countPlant_variants === null) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countPlant_variants ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
              newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'countPlant_variants', method: 'getData()', request: 'api.plant_variant.getCountItems'}];
              newError.path=['add', 'Plant_variant'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //check: countPlant_variants type
            if(!Number.isInteger(countPlant_variants)) {
              let newError = {};
              let withDetails=true;
              variant.current='error';
              newError.message = 'countPlant_variants ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
              newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'countPlant_variants', method: 'getData()', request: 'api.plant_variant.getCountItems'}];
              newError.path=['add', 'Plant_variant'];
              newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            if(response.data.errors) {
              let newError = {};
              newError.message = 'countPlant_variants ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'countPlant_variants', method: 'getData()', request: 'api.plant_variant.getCountItems'}];
              newError.path=['add', 'Plant_variant'];
              newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
              errors.current.push(newError);
            ...
            //ok
            setCount(countPlant_variants);


            /*
              API Request: plant_variantsConnection
            */
            let variables = {
              pagination: {
            ...
            let cancelableApiReqC = makeCancelable(api.plant_variant.getItemsConnection(
              graphqlServerUrl,
              search,
              null, //orderBy
              null, //orderDirection
            ...
            cancelableApiReqC
              ...
              (response) => {
                ...
                if(!response.data ||!response.data.data) {
                  ...
                  variant.current='error';
                  newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                  newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'plant_variantsConnection', method: 'getData()', request: 'api.plant_variant.getItemsConnection'}];
                  newError.path=['add', 'Plant_variant'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  console.log("Error: ", newError);
                ...
                //check: plant_variantsConnection
                let plant_variantsConnection = response.data.data.plant_variantsConnection;
                if(plant_variantsConnection === null) {
                  let newError = {};
                  let withDetails=true;
                  variant.current='error';
                  newError.message = 'plant_variantsConnection ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
                  newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'plant_variantsConnection', method: 'getData()', request: 'api.plant_variant.getItemsConnection'}];
                  newError.path=['add', 'Plant_variant'];
                  newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                  errors.current.push(newError);
                  ...
                  //check: plant_variantsConnection type
                  if(typeof plant_variantsConnection !== 'object'
                  || !Array.isArray(plant_variantsConnection.edges)
                  || typeof plant_variantsConnection.pageInfo !== 'object' 
                  || plant_variantsConnection.pageInfo === null) {
                    let newError = {};
                    let withDetails=true;
                    variant.current='error';
                    newError.message = 'plant_variantsConnection ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
                    newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'plant_variantsConnection', method: 'getData()', request: 'api.plant_variant.getItemsConnection'}];
                    newError.path=['add', 'Plant_variant'];
                    newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  //get items
                  let its = plant_variantsConnection.edges.map(o => o.node);
                  let pi = plant_variantsConnection.pageInfo;                      

                  //check: graphql errors
                  if(response.data.errors) {
                    let newError = {};
                    newError.message = 'plant_variantsConnection ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
                    newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', query: 'plant_variantsConnection', method: 'getData()', request: 'api.plant_variant.getItemsConnection'}];
                    newError.path=['add', 'Plant_variant'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                  ...
                  */
                  ...
                  if(errors.current.length > 0) {
                    ...
                    newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'A', method: 'getData()'}];
                    newError.path=['add', 'Plant_variant'];
                    newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                    errors.current.push(newError);
                    console.log("Error: ", newError);
                ...
                //error
                .catch((err) => { //error: on api.plant_variant.getItemsConnection
                  if(err.isCanceled) {
                    return;
                  } else {
                    let newError = {};
          ...
          //error
          .catch((err) => { //error: on api.plant_variant.getCountItems
            if(err.isCanceled) {
              return
            } else {
              let newError = {};
      ...
      //error
      .catch((err) => { //error: on api.tomato_Measurement.getAssociatedPlant_variantConnection
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, t, dataTrigger, item.tomato_ID, search, rowsPerPage]);
  ...
  const getDataB = useCallback(() => {
    ...
    if(lidsToAdd.current !== undefined && lidsToAdd.current.length > 0) {
      ops = {
        only: [{
          type: 'String',
          values: {plant_variant_ID: lidsToAdd.current}
        }]
      };
    } else {
    ...
    */
    let cancelableApiReq = makeCancelable(api.plant_variant.getCountItems(graphqlServerUrl, searchB, ops));
    cancelablePromises.current.push(cancelableApiReq);
    cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'B', query: 'countPlant_variants', method: 'getDataB()', request: 'api.plant_variant.getCountItems'}];
          newError.path=['add', 'Plant_variant'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
        ...
        setCountB(countPlant_variants);
          ...
          */
          ...
          let cancelableApiReqB = makeCancelable(api.plant_variant.getItemsConnection(
            graphqlServerUrl,
            searchB,
            null, //orderBy
            null, //orderDirection
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variantB.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'B', query: 'plant_variantsConnection', method: 'getDataB()', request: 'api.plant_variant.getItemsConnection'}];
                newError.path=['add', 'Plant_variant'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errorsB.current.length > 0) {
                ...
                newError.locations=[{model: 'Tomato_Measurement', association: 'Plant_variant', table:'B', method: 'getDataB()'}];
                newError.path=['add', 'Plant_variant'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errorsB.current.push(newError);
                console.log("Error: ", newError);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'Plant_variant' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'plant_variants' (from another place).
     * 
     * Actions:
    ...
    if(lastModelChanged.Tomato_Measurement&&
        ...
        lastModelChanged.Tomato_Measurement[String(item.tomato_ID)].changedAssociations&&
        lastModelChanged.Tomato_Measurement[String(item.tomato_ID)].changedAssociations.Plant_variant&&
        (lastModelChanged.Tomato_Measurement[String(item.tomato_ID)].changedAssociations.Plant_variant.added ||
         lastModelChanged.Tomato_Measurement[String(item.tomato_ID)].changedAssociations.Plant_variant.removed)) {
          
          //remove any dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.Tomato_Measurement[String(item.tomato_ID)].changedAssociations.Plant_variant.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('Plant_variant', idRemoved);
            });
          }
    ...
    /*
     * Case 2: 
     * The peer relation 'Tomato_measurements' for this item was updated.
     * That is to say that this current item was associated or dis-associated with some 'Plant_variant',
     * but this action happened on the peer relation 'Tomato_measurements'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'Plant_variant'.
     * B: the current item internalId is in the addedIds of the updated 'Plant_variant'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'Plant_variant' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.Plant_variant) {
      let oens = Object.entries(lastModelChanged.Plant_variant);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.Tomato_measurements) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.Tomato_measurements.removed) {
              let idsRemoved = entry[1].changedAssociations.Tomato_measurements.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.tomato_ID);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.Tomato_measurements.added) {
              let idsAdded = entry[1].changedAssociations.Tomato_measurements.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.tomato_ID);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.plant_variant_ID;
                  ...
                  handleUntransfer('Plant_variant', idAdded);
    ...
    /*
     * Case 3: 
     * The attributes of some 'Plant_variant' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.Plant_variant) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.plant_variant_ID;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.plant_variant_ID===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.plant_variant_ID===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.plant_variant_ID;

          //lookup item on any tables or associated ids
          let iofA = items.findIndex((item) => item.plant_variant_ID===idRemoved);
          let iofB = itemsB.findIndex((item) => item.plant_variant_ID===idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.tomato_ID, handleUntransfer]);
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnPlant_variantRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.plant_variant_ID) === -1) {
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.plant_variant_ID);
      ...
      //reload B (full)
      resetReloadDataB();
      handleTransfer('Plant_variant', item.plant_variant_ID);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('Plant_variant', item.plant_variant_ID);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <PlantVariantToAddTransferViewToolbar
                title={'Plant_variants'}
                titleIcon={1}
                search={search}
                    ...
                    <List dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.plant_variant_ID===it.plant_variant_ID) ? 'Plant_variant — ' + t('modelPanels.associatedRecord', 'Associated record') : 'Plant_variant '}>
                                <Avatar>
                                  {"plant_variant".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'plant_variant_ID' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.plant_variant_ID}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.plant_variant_ID===it.plant_variant_ID) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.plant_variant_ID===it.plant_variant_ID) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
              ...
              {/* Pagination */}
              <PlantVariantToAddTransferViewCursorPagination
                count={count}
...
PlantVariantToAddTransferView.propTypes = {
