...
import RoleCreatePanel from './components/role-create-panel/RoleCreatePanel'
import RoleUpdatePanel from './components/role-update-panel/RoleUpdatePanel'
import RoleDetailPanel from './components/role-detail-panel/RoleDetailPanel'
...
//Plotly
import RolePlotly from '../../../../plots/RolePlotly';
...
export default function RoleEnhancedTable(props) {
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'role', query: 'countRoles', method: 'getData()', request: 'api.role.getCountItems'}];
          newError.path=['Roles'];
          ...
          cancelableApiReqB
            ...
            (response) => {
              ...
              if(!response.data ||!response.data.data) {
                ...
                variant.current='error';
                newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
                newError.locations=[{model: 'role', query: 'rolesConnection', method: 'getData()', request: 'api.role.getItemsConnection'}];
                newError.path=['Roles'];
                newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
              ...
              */
              ...
              if(errors.current.length > 0) {
                ...
                newError.locations=[{model: 'role', method: 'getData()'}];
                newError.path=['Roles'];
                newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
                errors.current.push(newError);
                console.log("Error: ", newError);
  ...
  function getCsvTemplate() {
    ...
    /*
      API Request: csvTableTemplateRole
    */
    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          variant.current='error';
          newError.message = t('modelPanels.errors.data.e1', 'No data was received from the server.');
          newError.locations=[{model: 'role', query: 'csvTableTemplateRole', method: 'getCsvTemplate()', request: 'api.role.tableTemplate'}];
        ...
        //check: csvTableTemplateRole
        let csvTableTemplateRole = response.data.data.csvTableTemplateRole;
        if(csvTableTemplateRole === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateRole ' + t('modelPanels.errors.data.e2', 'could not be fetched.');
          newError.locations=[{model: 'role', query: 'csvTableTemplateRole', method: 'getCsvTemplate()', request: 'api.role.tableTemplate'}];
          newError.path=['Roles', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(!Array.isArray(csvTableTemplateRole)) {
          ...
          let withDetails=true;
          variant.current='error';
          newError.message = 'csvTableTemplateRole ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'role', query: 'csvTableTemplateRole', method: 'getCsvTemplate()', request: 'api.role.tableTemplate'}];
          newError.path=['Roles', 'csvTemplate'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        if(response.data.errors) {
          ...
          let withDetails=true;
          variant.current='info';
          newError.message = 'csvTableTemplateRole ' + t('modelPanels.errors.data.e3', 'fetched with errors.');
          newError.locations=[{model: 'role', query: 'csvTableTemplateRole', method: 'getCsvTemplate()', request: 'api.role.tableTemplate'}];
          newError.path=['Roles', 'csvTemplate'];
          newError.extensions = {graphQL:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
            ...
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                <RoleEnhancedTableToolbar
                  ...
                  search={search}
                  showToggleButtons={Boolean(RolePlotly)}
