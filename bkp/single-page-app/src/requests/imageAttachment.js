...
export default {
  ...
  tableTemplate(url) {
    let query = `query {csvTableTemplateImageAttachment}`
  ...
  getCountItems(url, searchText, ops) {
    //search
    var s = getSearchArgument('imageAttachment', searchText, ops);
    
    var query = '';

    //if has search
    if (s !== null) {
      query = `{ countImageAttachments(${s}) }`;
    }
    else {
      query = `{ countImageAttachments }`;
    }
  ...
  getItemsConnection(url, searchText, orderBy, orderDirection, variables, ops) {
    ...
    if (s !== null) {
      ...
      if (o != null) {
        ...
        query =
          `query imageAttachmentsConnection($pagination: paginationCursorInput)
            { imageAttachmentsConnection( ${s}, ${o}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
      else {
        ...
        query =
          `query imageAttachmentsConnection($pagination: paginationCursorInput)
            { imageAttachmentsConnection( ${s}, pagination: $pagination ) {
              ${qbody}
            }}`
      }
    }
      ...
      if (o != null) {
        ...
        query =
          `query imageAttachmentsConnection($pagination: paginationCursorInput)
          { imageAttachmentsConnection( ${o}, pagination: $pagination ) {
            ${qbody}
          }}`
      }
      else {
        ...
        query =
          `query imageAttachmentsConnection($pagination: paginationCursorInput)
          { imageAttachmentsConnection( pagination: $pagination ) {
            ${qbody}
          }}`
      }
    }
   ...
   * createItem
   * 
   * Add new ImageAttachment item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new ImageAttachment item. 
   */
   ...
   * updateItem
   * 
   * Update ImageAttachment item on GraphQL Server.
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given ImageAttachment item.  
   */
   ...
   * 
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the ImageAttachment item. 
   */
