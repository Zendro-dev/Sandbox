...
module.exports = {
    ...
    countries: async function({
        ...
        if (await checkAuthorization(context, 'country', 'read' === true)) {
            ...
            let result = await country.readAll(search, order, pagination).catch(err => {throw (err)})
            console.log("country_result: " + JSON.stringify(result));
            //console.log("benignErrs: " + JSON.stringify(context.benignErrors))
            return await country.readAll(search, order, pagination).catch(err => {throw (err)})
        } else {
