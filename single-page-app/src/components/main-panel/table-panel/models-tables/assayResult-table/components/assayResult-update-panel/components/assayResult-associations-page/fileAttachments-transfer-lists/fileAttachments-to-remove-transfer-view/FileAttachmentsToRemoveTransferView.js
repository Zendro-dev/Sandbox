...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assayResult.getFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getFileAttachmentsCount(
      graphqlServerUrl, 
      item.assayResult_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'fileAttachments', method: 'getCount()', request: 'api.assayResult.getFileAttachmentsCount'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'remove', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assayResult.getFileAttachments
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.assayResult.getFileAttachments(
      graphqlServerUrl, 
      item.assayResult_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'fileAttachments', method: 'getData()', request: 'api.assayResult.getFileAttachments'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'remove', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
