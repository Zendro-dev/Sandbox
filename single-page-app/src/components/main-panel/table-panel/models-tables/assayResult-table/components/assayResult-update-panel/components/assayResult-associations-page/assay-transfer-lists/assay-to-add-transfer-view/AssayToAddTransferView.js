...
import AssayToAddTransferViewToolbar from './components/AssayToAddTransferViewToolbar';
import AssayToAddTransferViewCursorPagination from './components/AssayToAddTransferViewCursorPagination';
...
export default function AssayToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.assayResult.getAssay
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getAssay(graphqlServerUrl, item.assayResult_id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assayResult', association: 'assay', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.assayResult.getAssay'}];
            newError.path=['update', `assayResult_id:${currentId}`, 'add', 'assay'];
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getAssay
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"assay_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.assayResult.getNotAssociatedAssayCount
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getNotAssociatedAssayCount(graphqlServerUrl, item.assayResult_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assayResult', association: 'assay', table:'A', method: 'getCount()', request: 'api.assayResult.getNotAssociatedAssayCount'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'add', 'assay'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getNotAssociatedAssayCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assayResult.getNotAssociatedAssay
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getNotAssociatedAssay(
      graphqlServerUrl,
      item.assayResult_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assayResult', association: 'assay', table:'A', method: 'getData()', request: 'api.assayResult.getNotAssociatedAssay'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'add', 'assay'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getNotAssociatedAssay
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getItems
    */
    let cancelableApiReq = makeCancelable(api.assay.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assayResult', association: 'assay', table:'B', method: 'getDataB()', request: 'api.assay.getItems'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'add', 'assay'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'assay' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'assays', 
    ...
    if(lastModelChanged.assayResult&&
        ...
        lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_assay_id&&
        (lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_assay_id.added ||
         lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_assay_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_assay_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('assay', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('assay', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_assay_id.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('assay', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'assay' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'assay',
     * but this action happened on the peer relation, identified by 'assayResult_assay_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'assay'.
     * B: the current item internalId is in the addedIds of the updated 'assay'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'assay' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.assay) {
      let oens = Object.entries(lastModelChanged.assay);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.assayResult_assay_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.assayResult_assay_id.removed) {
              let idsRemoved = entry[1].changedAssociations.assayResult_assay_id.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.assayResult_assay_id.added) {
              let idsAdded = entry[1].changedAssociations.assayResult_assay_id.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.assayResult_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.assay_id;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('assay', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'assay' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.assay) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.assay_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.assay_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.assay_id===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.assay_id===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.assay_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.assay_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('assay', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.assay_id===idRemoved) {
            //update associated item
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnAssayRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.assay_id) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.assay_id);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.assay_id];
        handleDisassociateItem('assay', associatedItem.assay_id);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('assay', item.assay_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('assay', item.assay_id);
    }
  };
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='AssayToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.assay_id;
                        let label = it.measurement;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'AssayToAddTransferView-listA-listItem-'+it.assay_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.assay_id===it.assay_id) ? 'assay — ' + t('modelPanels.associatedRecord', 'Associated record') : 'assay '}>
                                <Avatar>
                                  {"assay".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'assay_id' }>
                                        <Typography 
                                        id={'AssayToAddTransferView-listA-listItem-id-'+it.assay_id}
                                        variant="body1" display="block" noWrap={true}>{it.assay_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.assay_id===it.assay_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.assay_id===it.assay_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'AssayToAddTransferView-listA-listItem-'+it.assay_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <AssayToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='AssayToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.assay_id;
                        let label = it.measurement;
                        ...
                        return (
                          <ListItem 
                            id={'AssayToAddTransferView-listB-listItem-'+it.assay_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'assay_id' }>
                                        <Typography 
                                        id={'AssayToAddTransferView-listB-listItem-id-'+it.assay_id}
                                        variant="body1" display="block" noWrap={true}>{it.assay_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'AssayToAddTransferView-listB-listItem-'+it.assay_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='AssayToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.assay_id;
                        let label = associatedItem.measurement;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'AssayToAddTransferView-associatedItem-list-listItem-'+associatedItem.assay_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'assay — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"assay".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'assay_id' }>
                                        <Typography 
                                        id={'AssayToAddTransferView-listB-listItem-id-'+associatedItem.assay_id}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.assay_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'AssayToAddTransferView-associatedItem-list-listItem-'+associatedItem.assay_id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].assay_id===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('assay', associatedItem.assay_id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.assay_id];
                                      handleDisassociateItem('assay', associatedItem.assay_id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
AssayToAddTransferView.propTypes = {
