...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assayResult.getOntologyAnnotationsCount
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getOntologyAnnotationsCount(
      graphqlServerUrl, 
      item.assayResult_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'ontologyAnnotations', method: 'getCount()', request: 'api.assayResult.getOntologyAnnotationsCount'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'remove', 'ontologyAnnotations'];
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getOntologyAnnotationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assayResult.getOntologyAnnotations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.assayResult.getOntologyAnnotations(
      graphqlServerUrl, 
      item.assayResult_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'ontologyAnnotations', method: 'getData()', request: 'api.assayResult.getOntologyAnnotations'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'remove', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getOntologyAnnotations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.assayResult_id, search, getCount, configurePagination, onEmptyPage]);
