...
import ObservedMaterialToAddTransferViewToolbar from './components/ObservedMaterialToAddTransferViewToolbar';
import ObservedMaterialToAddTransferViewCursorPagination from './components/ObservedMaterialToAddTransferViewCursorPagination';
...
export default function ObservedMaterialToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.assayResult_id;

    /*
      API Request: api.assayResult.getObservedMaterial
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getObservedMaterial(graphqlServerUrl, item.assayResult_id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assayResult', association: 'observedMaterial', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.assayResult.getObservedMaterial'}];
            newError.path=['update', `assayResult_id:${currentId}`, 'add', 'observedMaterial'];
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getObservedMaterial
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.assayResult_id]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"material_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.assayResult.getNotAssociatedObservedMaterialCount
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getNotAssociatedObservedMaterialCount(graphqlServerUrl, item.assayResult_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assayResult', association: 'observedMaterial', table:'A', method: 'getCount()', request: 'api.assayResult.getNotAssociatedObservedMaterialCount'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'add', 'observedMaterial'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getNotAssociatedObservedMaterialCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.assayResult_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assayResult.getNotAssociatedObservedMaterial
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getNotAssociatedObservedMaterial(
      graphqlServerUrl,
      item.assayResult_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assayResult', association: 'observedMaterial', table:'A', method: 'getData()', request: 'api.assayResult.getNotAssociatedObservedMaterial'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'add', 'observedMaterial'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getNotAssociatedObservedMaterial
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.assayResult_id, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.material.getItems
    */
    let cancelableApiReq = makeCancelable(api.material.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assayResult', association: 'observedMaterial', table:'B', method: 'getDataB()', request: 'api.material.getItems'}];
            newError.path=['update', `assayResult_id:${item.assayResult_id}`, 'add', 'observedMaterial'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.assayResult_id, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'observedMaterial' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'materials', 
     ...
     * - return
     */
    if(lastModelChanged.assayResult&&
        lastModelChanged.assayResult[String(item.assayResult_id)]&&
        lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations&&
        lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_material_id&&
        (lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_material_id.added ||
         lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_material_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_material_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('observedMaterial', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('observedMaterial', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.assayResult[String(item.assayResult_id)].changedAssociations.assayResult_material_id.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('observedMaterial', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'observedMaterial' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'material',
     * but this action happened on the peer relation, identified by 'assayResult_material_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'material'.
     * B: the current item internalId is in the addedIds of the updated 'material'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'material' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.material) {
      let oens = Object.entries(lastModelChanged.material);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.assayResult_material_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.assayResult_material_id.removed) {
              let idsRemoved = entry[1].changedAssociations.assayResult_material_id.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.assayResult_id);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('observedMaterial', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.assayResult_material_id.added) {
              let idsAdded = entry[1].changedAssociations.assayResult_material_id.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.assayResult_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.material_id;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('observedMaterial', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'material' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.material) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.material_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.material_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.material_id===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.material_id===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.material_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.material_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('observedMaterial', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.material_id===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.assayResult_id, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnMaterialRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.material_id) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.material_id);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.material_id];
        handleDisassociateItem('observedMaterial', associatedItem.material_id);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('observedMaterial', item.material_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('observedMaterial', item.material_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservedMaterialToAddTransferViewToolbar
                title={'Materials'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservedMaterialToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservedMaterialToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.material_id;
                        let label = it.name;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'ObservedMaterialToAddTransferView-listA-listItem-'+it.material_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.material_id===it.material_id) ? 'material — ' + t('modelPanels.associatedRecord', 'Associated record') : 'material '}>
                                <Avatar>
                                  {"material".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'material_id' }>
                                        <Typography 
                                        id={'ObservedMaterialToAddTransferView-listA-listItem-id-'+it.material_id}
                                        variant="body1" display="block" noWrap={true}>{it.material_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.material_id===it.material_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.material_id===it.material_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ObservedMaterialToAddTransferView-listA-listItem-'+it.material_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ObservedMaterialToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservedMaterialToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservedMaterialToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservedMaterialToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.material_id;
                        let label = it.name;
                        ...
                        return (
                          <ListItem 
                            id={'ObservedMaterialToAddTransferView-listB-listItem-'+it.material_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'material_id' }>
                                        <Typography 
                                        id={'ObservedMaterialToAddTransferView-listB-listItem-id-'+it.material_id}
                                        variant="body1" display="block" noWrap={true}>{it.material_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ObservedMaterialToAddTransferView-listB-listItem-'+it.material_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='ObservedMaterialToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.material_id;
                        let label = associatedItem.name;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'ObservedMaterialToAddTransferView-associatedItem-list-listItem-'+associatedItem.material_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'material — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"material".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'material_id' }>
                                        <Typography 
                                        id={'ObservedMaterialToAddTransferView-listB-listItem-id-'+associatedItem.material_id}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.material_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'ObservedMaterialToAddTransferView-associatedItem-list-listItem-'+associatedItem.material_id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].material_id===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('observedMaterial', associatedItem.material_id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.material_id];
                                      handleDisassociateItem('observedMaterial', associatedItem.material_id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservedMaterialToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
ObservedMaterialToAddTransferView.propTypes = {
