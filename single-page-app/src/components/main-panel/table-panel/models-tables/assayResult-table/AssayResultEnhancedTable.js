...
import AssayResultEnhancedTableHead from './components/AssayResultEnhancedTableHead'
import AssayResultEnhancedTableToolbar from './components/AssayResultEnhancedTableToolbar'
import AssayResultCreatePanel from './components/assayResult-create-panel/AssayResultCreatePanel'
import AssayResultUpdatePanel from './components/assayResult-update-panel/AssayResultUpdatePanel'
import AssayResultDetailPanel from './components/assayResult-detail-panel/AssayResultDetailPanel'
import AssayResultDeleteConfirmationDialog from './components/AssayResultDeleteConfirmationDialog'
import AssayResultUploadFileDialog from './components/AssayResultUploadFileDialog'
import AssayResultCursorPagination from './components/AssayResultCursorPagination'
...
//Plotly
import AssayResultPlotly from '../../../../plots/AssayResultPlotly';
...
export default function AssayResultEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('assayResult_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assayResult', method: 'getCount()', request: 'api.assayResult.getCountItems'}];
            newError.path=['AssayResults'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assayResult', method: 'getData()', request: 'api.assayResult.getItems'}];
            newError.path=['AssayResults'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['assayResult']
    &&(Object.keys(changes.lastModelChanged['assayResult']).length===1)
    &&(changes.lastModelChanged['assayResult'][Object.keys(changes.lastModelChanged['assayResult'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set assayResult_id (internalId)
    variables.assayResult_id = item.assayResult_id;

    /*
      API Request: api.assayResult.deleteItem
    */
    let cancelableApiReq = makeCancelable(api.assayResult.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assayResult', method: 'doDelete()', request: 'api.assayResult.deleteItem'}];
            newError.path=['AssayResults', `assayResult_id:${item.assayResult_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.assayResult.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.assayResult.tableTemplate
    */
    let cancelableApiReq = makeCancelable(api.assayResult.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assayResult', method: 'getCsvTemplate()', request: 'api.assayResult.tableTemplate'}];
            newError.path=['AssayResults', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateAssayResult.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "assayResult-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.assayResult.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <AssayResultEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(AssayResultPlotly)}
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='AssayResultEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='AssayResultEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <AssayResultEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='AssayResultEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'AssayResultEnhancedTable-row-'+item.assayResult_id}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.assayResult_id}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'AssayResultEnhancedTable-row-iconButton-detail-'+item.assayResult_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.assayResult&&Array.isArray(permissions.assayResult)
                                      &&(permissions.assayResult.includes('update') || permissions.assayResult.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'AssayResultEnhancedTable-row-iconButton-edit-'+item.assayResult_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.assayResult&&Array.isArray(permissions.assayResult)
                                      &&(permissions.assayResult.includes('delete') || permissions.assayResult.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'AssayResultEnhancedTable-row-iconButton-delete-'+item.assayResult_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'assayResult_id: ' + item.assayResult_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.assayResult_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.value_as_str!==null)?item.value_as_str:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.value_as_int!==null)?item.value_as_int:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.value_as_num!==null)?item.value_as_num:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.value_as_bool!==null)?item.value_as_bool:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.value_as_float!==null)?item.value_as_float:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.assay_id!==null)?item.assay_id:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.material_id!==null)?item.material_id:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <AssayResultDeleteConfirmationDialog
          permissions={permissions}
...
AssayResultEnhancedTable.propTypes = {
