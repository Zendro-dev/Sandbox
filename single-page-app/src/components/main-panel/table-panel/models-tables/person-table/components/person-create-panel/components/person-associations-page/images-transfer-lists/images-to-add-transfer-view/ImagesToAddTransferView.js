...
const cancelableCountingPromises = useRef([]);
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.imageAttachment.getCountItems(graphqlServerUrl, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Person', association: 'images', table:'A', method: 'getCount()', request: 'api.imageAttachment.getCountItems'}];
            newError.path=['add', 'images'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Person', association: 'images', table:'A', method: 'getData()', request: 'api.imageAttachment.getItems'}];
            newError.path=['add', 'images'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
