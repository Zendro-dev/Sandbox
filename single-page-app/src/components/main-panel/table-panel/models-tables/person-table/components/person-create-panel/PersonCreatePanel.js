...
export default function PersonCreatePanel(props) {
  ...
  async function doSave(event) {
    ...
    */
    let cancelableApiReq = makeCancelable(api.person.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Person', method: 'doSave()', request: 'api.person.createItem'}];
            newError.path=['People', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.person.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  return (
    ...
    <Dialog id='PersonCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='PersonCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Person'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " person" }>
            <Fab
              id='PersonCreatePanel-fabButton-save' 
              color="secondary" 
