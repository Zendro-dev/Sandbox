...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.person.getImagesCount
    */
    let cancelableApiReq = makeCancelable(api.person.getImagesCount(
      graphqlServerUrl, 
      item.id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'images', method: 'getCount()', request: 'api.person.getImagesCount'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'images'];
      ...
      //error
      .catch((err) => { //error: on api.person.getImagesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.person.getImages
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.person.getImages(
      graphqlServerUrl, 
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'images', method: 'getData()', request: 'api.person.getImages'}];
            newError.path=['update', `id:${item.id}`, 'remove', 'images'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.person.getImages
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'ImageAttachment' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ImagesToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ImagesToRemoveTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'ImagesToRemoveTransferView-listA-listItem-'+it.id+'-button-remove'}
                                    color="primary"
                  ...
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ImagesToRemoveTransferView-list-listB'
                      ...
                      {itemsB.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ImagesToRemoveTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'ImagesToRemoveTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
