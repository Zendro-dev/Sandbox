...
import PersonEnhancedTableHead from './components/PersonEnhancedTableHead'
import PersonEnhancedTableToolbar from './components/PersonEnhancedTableToolbar'
import PersonCreatePanel from './components/person-create-panel/PersonCreatePanel'
import PersonUpdatePanel from './components/person-update-panel/PersonUpdatePanel'
import PersonDetailPanel from './components/person-detail-panel/PersonDetailPanel'
import PersonDeleteConfirmationDialog from './components/PersonDeleteConfirmationDialog'
import PersonUploadFileDialog from './components/PersonUploadFileDialog'
import PersonCursorPagination from './components/PersonCursorPagination'
...
//Plotly
import PersonPlotly from '../../../../plots/PersonPlotly';
...
export default function PersonEnhancedTable(props) {
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.person.getCountItems
    */
    let cancelableApiReq = makeCancelable(api.person.getCountItems(graphqlServerUrl, search));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Person', method: 'getCount()', request: 'api.person.getCountItems'}];
            newError.path=['People'];
      ...
      //error
      .catch((err) => { //error: on api.person.getCountItems
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Person', method: 'getData()', request: 'api.person.getItems'}];
            newError.path=['People'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['Person']
    &&(Object.keys(changes.lastModelChanged['Person']).length===1)
    &&(changes.lastModelChanged['Person'][Object.keys(changes.lastModelChanged['Person'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    /*
      API Request: api.person.deleteItem
    */
    let cancelableApiReq = makeCancelable(api.person.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Person', method: 'doDelete()', request: 'api.person.deleteItem'}];
            newError.path=['People', `id:${item.id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.person.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.person.tableTemplate
    */
    let cancelableApiReq = makeCancelable(api.person.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'Person', method: 'getCsvTemplate()', request: 'api.person.tableTemplate'}];
            newError.path=['People', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplatePerson.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "person-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.person.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <PersonEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(PersonPlotly)}
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='PersonEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='PersonEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <PersonEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='PersonEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'PersonEnhancedTable-row-'+item.id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'PersonEnhancedTable-row-iconButton-detail-'+item.id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.person&&Array.isArray(permissions.person)
                                      &&(permissions.person.includes('update') || permissions.person.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'PersonEnhancedTable-row-iconButton-edit-'+item.id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.person&&Array.isArray(permissions.person)
                                      &&(permissions.person.includes('delete') || permissions.person.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'PersonEnhancedTable-row-iconButton-delete-'+item.id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
