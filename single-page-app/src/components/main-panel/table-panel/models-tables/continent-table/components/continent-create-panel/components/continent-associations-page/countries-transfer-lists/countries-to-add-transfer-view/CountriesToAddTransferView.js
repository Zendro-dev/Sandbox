  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.country.getCountItems(graphqlServerUrl, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'continent', association: 'countries', table:'A', method: 'getCount()', request: 'api.country.getCountItems'}];
            newError.path=['add', 'countries'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'continent', association: 'countries', table:'A', method: 'getData()', request: 'api.country.getItems'}];
            newError.path=['add', 'countries'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
