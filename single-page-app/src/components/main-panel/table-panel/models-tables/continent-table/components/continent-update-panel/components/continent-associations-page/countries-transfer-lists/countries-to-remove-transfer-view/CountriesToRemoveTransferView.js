...
import CountriesToRemoveTransferViewToolbar from './components/CountriesToRemoveTransferViewToolbar';
import CountriesToRemoveTransferViewCursorPagination from './components/CountriesToRemoveTransferViewCursorPagination';
...
export default function CountriesToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.continent.getCountriesCount
    */
    ...
    let cancelableApiReq = makeCancelable(api.continent.getCountriesCount(
      graphqlServerUrl, 
      item.continent_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'countries', method: 'getCount()', request: 'api.continent.getCountriesCount'}];
            newError.path=['update', `continent_id:${item.continent_id}`, 'remove', 'countries'];
      ...
      //error
      .catch((err) => { //error: on api.continent.getCountriesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {country_id: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.continent.getCountries
    */
    let variables = {
    ...
    let cancelableApiReq = makeCancelable(api.continent.getCountries(
      graphqlServerUrl, 
      item.continent_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'countries', method: 'getData()', request: 'api.continent.getCountries'}];
            newError.path=['update', `continent_id:${item.continent_id}`, 'remove', 'countries'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.continent.getCountries
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.continent_id, search, getCount, configurePagination, onEmptyPage]);
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'country' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.country_continent_id.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.country_id;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.country_id) === -1) {
      lidsToRemove.current.push(item.country_id);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.country_id);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <CountriesToRemoveTransferViewToolbar
                title={'Countries'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CountriesToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='CountriesToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.country_id;
                        let label = undefined;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(country_id=> country_id=== it.country_id) !== undefined;

                        return (
                          <ListItem 
                            id={'CountriesToRemoveTransferView-listA-listItem-'+it.country_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'country_id' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.country_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'CountriesToRemoveTransferView-listA-listItem-'+it.country_id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <CountriesToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CountriesToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CountriesToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='CountriesToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.country_id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'CountriesToRemoveTransferView-listB-listItem-'+it.country_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'CountriesToRemoveTransferView-listB-listItem-'+it.country_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
CountriesToRemoveTransferView.propTypes = {
