...
import CountriesToAddTransferViewToolbar from './components/CountriesToAddTransferViewToolbar';
import CountriesToAddTransferViewCursorPagination from './components/CountriesToAddTransferViewCursorPagination';
...
export default function CountriesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.continent.getNotAssociatedCountriesCount
    */
    ...
    let cancelableApiReq = makeCancelable(api.continent.getNotAssociatedCountriesCount(graphqlServerUrl, item.continent_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'continent', association: 'countries', table:'A', method: 'getCount()', request: 'api.continent.getNotAssociatedCountriesCount'}];
            newError.path=['update', `continent_id:${item.continent_id}`, 'add', 'countries'];
      ...
      //error
      .catch((err) => { //error: on api.continent.getNotAssociatedCountriesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.continent_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.continent.getNotAssociatedCountries
    */
    let api = await loadApi("continent");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.continent.getNotAssociatedCountries(
      graphqlServerUrl,
      item.continent_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'continent', association: 'countries', table:'A', method: 'getData()', request: 'api.continent.getNotAssociatedCountries'}];
            newError.path=['update', `continent_id:${item.continent_id}`, 'add', 'countries'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.continent.getNotAssociatedCountries
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.continent_id, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.country.getCountItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.country.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'continent', association: 'countries', table:'B', method: 'getCountB()', request: 'api.country.getCountItems'}];
            newError.path=['update', `continent_id:${item.continent_id}`, 'add', 'countries'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsD.current.push(newError);
            console.log("Error: ", newError);
    ...
    //error
    .catch((err) => { //error: on api.country.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.continent_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'continent', association: 'countries', table:'B', method: 'getDataB()', request: 'api.country.getItems'}];
            newError.path=['update', `continent_id:${item.continent_id}`, 'add', 'countries'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsB.current.push(newError);
            console.log("Error: ", newError);
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.continent_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'countries' for this item was updated.
    ...
    if(lastModelChanged.continent&&
        lastModelChanged.continent[String(item.continent_id)]&&
        lastModelChanged.continent[String(item.continent_id)].changedAssociations&&
        lastModelChanged.continent[String(item.continent_id)].changedAssociations.country_continent_id&&
        (lastModelChanged.continent[String(item.continent_id)].changedAssociations.country_continent_id.added ||
         lastModelChanged.continent[String(item.continent_id)].changedAssociations.country_continent_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.continent[String(item.continent_id)].changedAssociations.country_continent_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('countries', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.continent[String(item.continent_id)].changedAssociations.country_continent_id.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'countries' for this item was updated from the target model (in the peer relation).
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.country_continent_id) {
            ...
            if(entry[1].changedAssociations.country_continent_id.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.continent_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.country_continent_id.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.continent_id);
                if(iof !== -1) {
    ...
    if(lastModelChanged.country) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.country_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('countries', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.continent_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.country_id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('countries', item.country_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.country_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('countries', item.country_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <CountriesToAddTransferViewToolbar
                title={'Countries'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CountriesToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='CountriesToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'CountriesToAddTransferView-listA-listItem-'+it.country_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'country' }>
                                <Avatar>{"country".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'country_id' }>
                                        <Typography 
                                        id={'CountriesToAddTransferView-listA-listItem-id-'+it.country_id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'CountriesToAddTransferView-listA-listItem-'+it.country_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <CountriesToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CountriesToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='CountriesToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='CountriesToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.country_id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'CountriesToAddTransferView-listB-listItem-'+it.country_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'country_id' }>
                                        <Typography 
                                        id={'CountriesToAddTransferView-listB-listItem-id-'+it.country_id}
                                        variant="body1" display="block" noWrap={true}>{it.country_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'CountriesToAddTransferView-listB-listItem-'+it.country_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
CountriesToAddTransferView.propTypes = {
