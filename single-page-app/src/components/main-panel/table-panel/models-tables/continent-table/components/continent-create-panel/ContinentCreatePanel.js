...
//lazy loading
const ContinentAttributesPage = lazy(() => retry(() => import(/* webpackChunkName: "Create-AttributesContinent" */ './components/continent-attributes-page/ContinentAttributesPage')));
const ContinentAssociationsPage = lazy(() => retry(() => import(/* webpackChunkName: "Create-AssociationsContinent" */ './components/continent-associations-page/ContinentAssociationsPage')));
const ContinentConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Create-ConfirmationContinent" */ './components/ContinentConfirmationDialog')));
...
export default function ContinentCreatePanel(props) {
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.continent_id = 0;
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.continent.createItem
    */
    ...
    let cancelableApiReq = makeCancelable(api.continent.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'continent', method: 'doSave()', request: 'api.continent.createItem'}];
            newError.path=['Continents', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.continent.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  return (
    ...
    <Dialog id='ContinentCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ContinentCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Continent'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " continent" }>
            <Fab
              id='ContinentCreatePanel-fabButton-save' 
              color="secondary" 
...
ContinentCreatePanel.propTypes = {
