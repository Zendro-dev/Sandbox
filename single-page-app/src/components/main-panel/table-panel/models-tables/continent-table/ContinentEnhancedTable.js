...
import ContinentEnhancedTableHead from './components/ContinentEnhancedTableHead'
import ContinentEnhancedTableToolbar from './components/ContinentEnhancedTableToolbar'
import ContinentUploadFileDialog from './components/ContinentUploadFileDialog'
import ContinentCursorPagination from './components/ContinentCursorPagination'
...
//lazy loading
const ContinentCreatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Create-Continent" */ './components/continent-create-panel/ContinentCreatePanel')));
const ContinentUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Update-Continent" */ './components/continent-update-panel/ContinentUpdatePanel')));
const ContinentDetailPanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Continent" */ './components/continent-detail-panel/ContinentDetailPanel')));
const ContinentDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Delete-Continent" */ './components/ContinentDeleteConfirmationDialog')));
...
export default function ContinentEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('continent_id');
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.continent.getCountItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.continent.getCountItems(graphqlServerUrl, search));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'continent', method: 'getCount()', request: 'api.continent.getCountItems'}];
            newError.path=['Continents'];
      ...
      //error
      .catch((err) => { //error: on api.continent.getCountItems
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'continent', method: 'getData()', request: 'api.continent.getItems'}];
            newError.path=['Continents'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['continent']
    &&(Object.keys(changes.lastModelChanged['continent']).length===1)
    &&(changes.lastModelChanged['continent'][Object.keys(changes.lastModelChanged['continent'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set continent_id (internalId)
    variables.continent_id = item.continent_id;

    /*
      API Request: api.continent.deleteItem
    */
    let api = await loadApi("continent");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.continent.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'continent', method: 'doDelete()', request: 'api.continent.deleteItem'}];
            newError.path=['Continents', `continent_id:${item.continent_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.continent.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.continent.tableTemplate
    */
    let api = await loadApi("continent");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.continent.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'continent', method: 'getCsvTemplate()', request: 'api.continent.tableTemplate'}];
            newError.path=['Continents', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        link.href = url;
        link.setAttribute("download", "continent-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.continent.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    <div className={classes.root}>
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ContinentEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ContinentEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ContinentEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ContinentEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ContinentEnhancedTable-row-'+item.continent_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ContinentEnhancedTable-row-iconButton-detail-'+item.continent_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.continent&&Array.isArray(permissions.continent)
                                      &&(permissions.continent.includes('update') || permissions.continent.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ContinentEnhancedTable-row-iconButton-edit-'+item.continent_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.continent&&Array.isArray(permissions.continent)
                                      &&(permissions.continent.includes('delete') || permissions.continent.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ContinentEnhancedTable-row-iconButton-delete-'+item.continent_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'continent_id: ' + item.continent_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.continent_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}><ErrorBoundary showMessage={true} belowToolbar={true} dialog={true} open={deleteConfirmationDialogOpen} handleClose={handleDeleteConfirmationReject}>
          <ContinentDeleteConfirmationDialog
            permissions={permissions}
...
ContinentEnhancedTable.propTypes = {
