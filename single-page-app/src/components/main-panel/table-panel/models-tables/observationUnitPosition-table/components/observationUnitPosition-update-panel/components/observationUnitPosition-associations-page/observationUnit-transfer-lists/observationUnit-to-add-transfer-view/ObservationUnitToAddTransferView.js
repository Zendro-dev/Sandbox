...
export default function ObservationUnitToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.observationUnitPositionDbId;

    /*
      API Request: api.observationUnitPosition.getObservationUnit
    */
    let api = await loadApi("observationUnitPosition");
    let cancelableApiReq = makeCancelable(api.observationUnitPosition.getObservationUnit(graphqlServerUrl, item.observationUnitPositionDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnitPosition', association: 'observationUnit', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationUnitPosition.getObservationUnit'}];
            newError.path=['update', `observationUnitPositionDbId:${currentId}`, 'add', 'observationUnit'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnitPosition.getObservationUnit
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.observationUnitPositionDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnitPosition.getNotAssociatedObservationUnitCount
    */
    let api = await loadApi("observationUnitPosition");
    let cancelableApiReq = makeCancelable(api.observationUnitPosition.getNotAssociatedObservationUnitCount(graphqlServerUrl, item.observationUnitPositionDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnitPosition', association: 'observationUnit', table:'A', method: 'getCount()', request: 'api.observationUnitPosition.getNotAssociatedObservationUnitCount'}];
            newError.path=['update', `observationUnitPositionDbId:${item.observationUnitPositionDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnitPosition.getNotAssociatedObservationUnitCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.observationUnitPositionDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnitPosition.getNotAssociatedObservationUnit
    */
    let api = await loadApi("observationUnitPosition");
    let cancelableApiReq = makeCancelable(api.observationUnitPosition.getNotAssociatedObservationUnit(
      graphqlServerUrl,
      item.observationUnitPositionDbId,
      search,
      variables,
      ops,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnitPosition', association: 'observationUnit', table:'A', method: 'getData()', request: 'api.observationUnitPosition.getNotAssociatedObservationUnit'}];
            newError.path=['update', `observationUnitPositionDbId:${item.observationUnitPositionDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnitPosition.getNotAssociatedObservationUnit
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.observationUnitPositionDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getItems
    */
    let api = await loadApi("observationUnit");
    let cancelableApiReq = makeCancelable(api.observationUnit.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnitPosition', association: 'observationUnit', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
            newError.path=['update', `observationUnitPositionDbId:${item.observationUnitPositionDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.observationUnitPositionDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'observationUnit' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'observationUnits', 
    ...
    if(lastModelChanged.observationUnitPosition&&
        lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)]&&
        lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations&&
        lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations.observationUnitPosition_observationUnitDbId&&
        (lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations.observationUnitPosition_observationUnitDbId.added ||
         lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations.observationUnitPosition_observationUnitDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations.observationUnitPosition_observationUnitDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('observationUnit', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('observationUnit', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].changedAssociations.observationUnitPosition_observationUnitDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('observationUnit', lidsToRemove.current[0]);
     ...
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'observationUnit'.
     * B: the current item internalId is in the addedIds of the updated 'observationUnit'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'observationUnit' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnitPosition_observationUnitDbId) {
            ...
            if(entry[1].changedAssociations.observationUnitPosition_observationUnitDbId.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.observationUnitPositionDbId);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('observationUnit', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.observationUnitPosition_observationUnitDbId.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.observationUnitPositionDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.observationUnitDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('observationUnit', idAdded);
                  } 
    ...
    if(lastModelChanged.observationUnit) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          ...
          if(associatedItem && associatedItem.observationUnitDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup on associated item
          if(associatedItem && associatedItem.observationUnitDbId===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationUnitPositionDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.observationUnitDbId) === -1) {
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.observationUnitDbId];
        handleDisassociateItem('observationUnit', associatedItem.observationUnitDbId);
      }
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationUnitToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ObservationUnitToAddTransferView-listA-listItem-'+it.observationUnitDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.observationUnitDbId===it.observationUnitDbId) ? 'observationUnit — ' + t('modelPanels.associatedRecord', 'Associated record') : 'observationUnit '}>
                                <Avatar>
                                  {"observationUnit".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationUnitDbId' }>
                                        <Typography 
                                        id={'ObservationUnitToAddTransferView-listA-listItem-id-'+it.observationUnitDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationUnitDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.observationUnitDbId===it.observationUnitDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.observationUnitDbId===it.observationUnitDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ObservationUnitToAddTransferView-listA-listItem-'+it.observationUnitDbId+'-button-add'}
                                  color="primary"
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationUnitToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationUnitDbId;
                        let label = it.observationUnitName;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationUnitToAddTransferView-listB-listItem-'+it.observationUnitDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationUnitDbId' }>
                                        <Typography 
                                        id={'ObservationUnitToAddTransferView-listB-listItem-id-'+it.observationUnitDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationUnitDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ObservationUnitToAddTransferView-listB-listItem-'+it.observationUnitDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='ObservationUnitToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.observationUnitDbId;
                        let label = associatedItem.observationUnitName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'ObservationUnitToAddTransferView-associatedItem-list-listItem-'+associatedItem.observationUnitDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'observationUnit — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"observationUnit".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationUnitDbId' }>
                                        <Typography 
                                        id={'ObservationUnitToAddTransferView-listB-listItem-id-'+associatedItem.observationUnitDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.observationUnitDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'ObservationUnitToAddTransferView-associatedItem-list-listItem-'+associatedItem.observationUnitDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].observationUnitDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('observationUnit', associatedItem.observationUnitDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.observationUnitDbId];
                                      handleDisassociateItem('observationUnit', associatedItem.observationUnitDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
