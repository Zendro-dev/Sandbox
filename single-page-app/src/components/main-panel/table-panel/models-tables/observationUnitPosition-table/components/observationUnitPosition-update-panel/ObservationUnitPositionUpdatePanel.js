...
//lazy loading
const ObservationUnitPositionAttributesPage = lazy(() => import(/* webpackChunkName: "Update-Attributes-ObservationUnitPosition" */ './components/observationUnitPosition-attributes-page/ObservationUnitPositionAttributesPage'));
const ObservationUnitPositionAssociationsPage = lazy(() => import(/* webpackChunkName: "Update-Associations-ObservationUnitPosition" */ './components/observationUnitPosition-associations-page/ObservationUnitPositionAssociationsPage'));
const ObservationUnitPositionConfirmationDialog = lazy(() => import(/* webpackChunkName: "Update-Confirmation-ObservationUnitPosition" */ './components/ObservationUnitPositionConfirmationDialog'));
...
export default function ObservationUnitPositionUpdatePanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged.observationUnitPosition&&
      lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)]) {

        //updated item
        if(lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].op === "update"&&
            lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.observationUnitPosition[String(item.observationUnitPositionDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, item.observationUnitPositionDbId]);
  ...
  function getInitialValues() {
    ...
    initialValues.blockNumber = item.blockNumber;
    initialValues.entryNumber = item.entryNumber;
    initialValues.positionCoordinateX = item.positionCoordinateX;
    initialValues.positionCoordinateY = item.positionCoordinateY;
    initialValues.replicate = item.replicate;
    initialValues.observationUnitDbId = item.observationUnitDbId;
    initialValues.observationUnitPositionDbId = item.observationUnitPositionDbId;
  ...
  initialValueOkStates.blockNumber = (item.blockNumber!==null ? 1 : 0);
  initialValueOkStates.entryNumber = (item.entryNumber!==null ? 1 : 0);
  initialValueOkStates.positionCoordinateX = (item.positionCoordinateX!==null ? 1 : 0);
  initialValueOkStates.positionCoordinateY = (item.positionCoordinateY!==null ? 1 : 0);
  initialValueOkStates.replicate = (item.replicate!==null ? 1 : 0);
    initialValueOkStates.observationUnitDbId = -2; //FK
  initialValueOkStates.observationUnitPositionDbId = (item.observationUnitPositionDbId!==null ? 1 : 0);
  ...
  function areThereChangedFields() {
    if(values.current.blockNumber !== item.blockNumber) { return true;}
    if(values.current.entryNumber !== item.entryNumber) { return true;}
    if(values.current.positionCoordinateX !== item.positionCoordinateX) { return true;}
    if(values.current.positionCoordinateY !== item.positionCoordinateY) { return true;}
    if(values.current.replicate !== item.replicate) { return true;}
    if(values.current.observationUnitDbId !== item.observationUnitDbId) { return true;}
    if(values.current.observationUnitPositionDbId !== item.observationUnitPositionDbId) { return true;}
    return false;
  }
  ...
  function setAddRemoveOneObservationUnit(variables) {
    ...
    if(observationUnitIdsToAdd.current.length>0) {
      ...
      changedAssociations.current.observationUnitPosition_observationUnitDbId.idsAdded = [observationUnitIdsToAdd.current[0]];
    }
    /**
    ...
    if(observationUnitIdsToRemove.current.length>0) {
      ...
      changedAssociations.current.observationUnitPosition_observationUnitDbId.idsRemoved = [observationUnitIdsToRemove.current[0]];
    }
...
function setAjvErrors(err) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.observationUnitPosition.updateItem
    */
    let api = await loadApi("observationUnitPosition");
    let cancelableApiReq = makeCancelable(api.observationUnitPosition.updateItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnitPosition', method: 'doSave()', request: 'api.observationUnitPosition.updateItem'}];
            newError.path=['ObservationUnitPositions', `observationUnitPositionDbId:${item.observationUnitPositionDbId}`, 'update'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnitPosition.updateItem
        if(err.isCanceled) {
          return;
        } else {
          //set ajv errors
  ...
  return (
    <Dialog id='ObservationUnitPositionUpdatePanel-dialog' 
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ObservationUnitPositionUpdatePanel-button-cancel'
              edge="start" 
              color="inherit" 
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": ObservationUnitPosition | observationUnitPositionDbId: " + item.observationUnitPositionDbId}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " observationUnitPosition" }>
              <Fab
                id='ObservationUnitPositionUpdatePanel-fabButton-save' 
                color="secondary" 
...
ObservationUnitPositionUpdatePanel.propTypes = {
