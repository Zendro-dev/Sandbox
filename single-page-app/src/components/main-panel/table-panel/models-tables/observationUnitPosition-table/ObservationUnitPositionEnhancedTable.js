...
import ObservationUnitPositionEnhancedTableHead from './components/ObservationUnitPositionEnhancedTableHead'
import ObservationUnitPositionEnhancedTableToolbar from './components/ObservationUnitPositionEnhancedTableToolbar'
import ObservationUnitPositionUploadFileDialog from './components/ObservationUnitPositionUploadFileDialog'
import ObservationUnitPositionCursorPagination from './components/ObservationUnitPositionCursorPagination'
...
//lazy loading
const ObservationUnitPositionCreatePanel = lazy(() => import(/* webpackChunkName: "Create-ObservationUnitPosition" */ './components/observationUnitPosition-create-panel/ObservationUnitPositionCreatePanel'));
const ObservationUnitPositionUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-ObservationUnitPosition" */ './components/observationUnitPosition-update-panel/ObservationUnitPositionUpdatePanel'));
const ObservationUnitPositionDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-ObservationUnitPosition" */ './components/observationUnitPosition-detail-panel/ObservationUnitPositionDetailPanel'));
const ObservationUnitPositionDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-ObservationUnitPosition" */ './components/ObservationUnitPositionDeleteConfirmationDialog'));
//Plotly
const ObservationUnitPositionPlotly = lazy(() => import(/* webpackChunkName: "Plotly-ObservationUnitPosition" */ '../../../../plots/ObservationUnitPositionPlotly'));
...
export default function ObservationUnitPositionEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('observationUnitPositionDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnitPosition.getCountItems
    */
    let api = await loadApi("observationUnitPosition");
    let cancelableApiReq = makeCancelable(api.observationUnitPosition.getCountItems(graphqlServerUrl, search));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnitPosition', method: 'getCount()', request: 'api.observationUnitPosition.getCountItems'}];
            newError.path=['ObservationUnitPositions'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnitPosition.getCountItems
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnitPosition', method: 'getData()', request: 'api.observationUnitPosition.getItems'}];
            newError.path=['ObservationUnitPositions'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['observationUnitPosition']
    &&(Object.keys(changes.lastModelChanged['observationUnitPosition']).length===1)
    &&(changes.lastModelChanged['observationUnitPosition'][Object.keys(changes.lastModelChanged['observationUnitPosition'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set observationUnitPositionDbId (internalId)
    variables.observationUnitPositionDbId = item.observationUnitPositionDbId;

    /*
      API Request: api.observationUnitPosition.deleteItem
    */
    let api = await loadApi("observationUnitPosition");
    let cancelableApiReq = makeCancelable(api.observationUnitPosition.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnitPosition', method: 'doDelete()', request: 'api.observationUnitPosition.deleteItem'}];
            newError.path=['ObservationUnitPositions', `observationUnitPositionDbId:${item.observationUnitPositionDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.observationUnitPosition.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.observationUnitPosition.tableTemplate
    */
    let api = await loadApi("observationUnitPosition");
    let cancelableApiReq = makeCancelable(api.observationUnitPosition.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnitPosition', method: 'getCsvTemplate()', request: 'api.observationUnitPosition.tableTemplate'}];
            newError.path=['ObservationUnitPositions', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateObservationUnitPosition.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "observationUnitPosition-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.observationUnitPosition.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <ObservationUnitPositionEnhancedTableToolbar
                ...
                >
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ObservationUnitPositionEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ObservationUnitPositionEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ObservationUnitPositionEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ObservationUnitPositionEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ObservationUnitPositionEnhancedTable-row-'+item.observationUnitPositionDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.observationUnitPositionDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ObservationUnitPositionEnhancedTable-row-iconButton-detail-'+item.observationUnitPositionDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.observationUnitPosition&&Array.isArray(permissions.observationUnitPosition)
                                      &&(permissions.observationUnitPosition.includes('update') || permissions.observationUnitPosition.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ObservationUnitPositionEnhancedTable-row-iconButton-edit-'+item.observationUnitPositionDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.observationUnitPosition&&Array.isArray(permissions.observationUnitPosition)
                                      &&(permissions.observationUnitPosition.includes('delete') || permissions.observationUnitPosition.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ObservationUnitPositionEnhancedTable-row-iconButton-delete-'+item.observationUnitPositionDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'observationUnitPositionDbId: ' + item.observationUnitPositionDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.observationUnitPositionDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.blockNumber!==null)?item.blockNumber:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.entryNumber!==null)?item.entryNumber:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.positionCoordinateX!==null)?item.positionCoordinateX:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.positionCoordinateY!==null)?item.positionCoordinateY:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.replicate!==null)?item.replicate:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.observationUnitDbId!==null)?item.observationUnitDbId:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <ObservationUnitPositionCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(ObservationUnitPositionPlotly)) && (
                      <Suspense fallback={<div />}>
                        <ObservationUnitPositionPlotly />
                      </Suspense>
                    )}
                  </div>
      ...
      {(createDialogOpen) && (
        <Suspense fallback={<div />}>
          <ObservationUnitPositionCreatePanel
            permissions={permissions}
      ...
      {(updateDialogOpen) && (
        <Suspense fallback={<div />}>
          <ObservationUnitPositionUpdatePanel
            permissions={permissions}
            item={updateItem}
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <ObservationUnitPositionDeleteConfirmationDialog
            permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <ObservationUnitPositionUploadFileDialog
...
ObservationUnitPositionEnhancedTable.propTypes = {
