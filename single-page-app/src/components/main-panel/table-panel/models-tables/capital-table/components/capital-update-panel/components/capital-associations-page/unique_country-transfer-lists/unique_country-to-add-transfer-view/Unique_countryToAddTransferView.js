...
import UniqueCountryToAddTransferViewToolbar from './components/Unique_countryToAddTransferViewToolbar';
import UniqueCountryToAddTransferViewCursorPagination from './components/Unique_countryToAddTransferViewCursorPagination';
...
export default function UniqueCountryToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.capital_id;

    /*
      API Request: api.capital.getUnique_country
    */
    let api = await loadApi("capital");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.capital.getUnique_country(graphqlServerUrl, item.capital_id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'capital', association: 'unique_country', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.capital.getUnique_country'}];
            newError.path=['update', `capital_id:${currentId}`, 'add', 'unique_country'];
      ...
      //error
      .catch((err) => { //error: on api.capital.getUnique_country
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.capital_id]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"country_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.capital.getNotAssociatedUnique_countryCount
    */
    let api = await loadApi("capital");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.capital.getNotAssociatedUnique_countryCount(graphqlServerUrl, item.capital_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'capital', association: 'unique_country', table:'A', method: 'getCount()', request: 'api.capital.getNotAssociatedUnique_countryCount'}];
            newError.path=['update', `capital_id:${item.capital_id}`, 'add', 'unique_country'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsC.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.capital.getNotAssociatedUnique_countryCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.capital_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.capital.getNotAssociatedUnique_country
    */
    let api = await loadApi("capital");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.capital.getNotAssociatedUnique_country(
      graphqlServerUrl,
      item.capital_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'capital', association: 'unique_country', table:'A', method: 'getData()', request: 'api.capital.getNotAssociatedUnique_country'}];
            newError.path=['update', `capital_id:${item.capital_id}`, 'add', 'unique_country'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.capital.getNotAssociatedUnique_country
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.capital_id, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.country.getItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.country.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'capital', association: 'unique_country', table:'B', method: 'getDataB()', request: 'api.country.getItems'}];
            newError.path=['update', `capital_id:${item.capital_id}`, 'add', 'unique_country'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsB.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.country.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.capital_id, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'unique_country' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'countries', 
     ...
     * - return
     */
    if(lastModelChanged.capital&&
        lastModelChanged.capital[String(item.capital_id)]&&
        lastModelChanged.capital[String(item.capital_id)].changedAssociations&&
        lastModelChanged.capital[String(item.capital_id)].changedAssociations.capital_country_id&&
        (lastModelChanged.capital[String(item.capital_id)].changedAssociations.capital_country_id.added ||
         lastModelChanged.capital[String(item.capital_id)].changedAssociations.capital_country_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.capital[String(item.capital_id)].changedAssociations.capital_country_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('unique_country', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('unique_country', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.capital[String(item.capital_id)].changedAssociations.capital_country_id.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('unique_country', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'unique_country' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'country',
     * but this action happened on the peer relation, identified by 'capital_country_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'country'.
     * B: the current item internalId is in the addedIds of the updated 'country'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'country' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.country) {
      let oens = Object.entries(lastModelChanged.country);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.capital_country_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.capital_country_id.removed) {
              let idsRemoved = entry[1].changedAssociations.capital_country_id.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.capital_id);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('unique_country', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.capital_country_id.added) {
              let idsAdded = entry[1].changedAssociations.capital_country_id.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.capital_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.country_id;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('unique_country', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'country' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.country) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.country_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.country_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.country_id===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.country_id===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.country_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.country_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('unique_country', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.country_id===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.capital_id, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnCountryRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.country_id) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.country_id);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.country_id];
        handleDisassociateItem('unique_country', associatedItem.country_id);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('unique_country', item.country_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('unique_country', item.country_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <UniqueCountryToAddTransferViewToolbar
                title={'Countries'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UniqueCountryToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='UniqueCountryToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.country_id;
                        let label = undefined;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'UniqueCountryToAddTransferView-listA-listItem-'+it.country_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.country_id===it.country_id) ? 'country — ' + t('modelPanels.associatedRecord', 'Associated record') : 'country '}>
                                <Avatar>
                                  {"country".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'country_id' }>
                                        <Typography 
                                        id={'UniqueCountryToAddTransferView-listA-listItem-id-'+it.country_id}
                                        variant="body1" display="block" noWrap={true}>{it.country_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.country_id===it.country_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.country_id===it.country_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'UniqueCountryToAddTransferView-listA-listItem-'+it.country_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <UniqueCountryToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UniqueCountryToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UniqueCountryToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='UniqueCountryToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.country_id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'UniqueCountryToAddTransferView-listB-listItem-'+it.country_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'country_id' }>
                                        <Typography 
                                        id={'UniqueCountryToAddTransferView-listB-listItem-id-'+it.country_id}
                                        variant="body1" display="block" noWrap={true}>{it.country_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'UniqueCountryToAddTransferView-listB-listItem-'+it.country_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='UniqueCountryToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.country_id;
                        let label = undefined;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'UniqueCountryToAddTransferView-associatedItem-list-listItem-'+associatedItem.country_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'country — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"country".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'country_id' }>
                                        <Typography 
                                        id={'UniqueCountryToAddTransferView-listB-listItem-id-'+associatedItem.country_id}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.country_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'UniqueCountryToAddTransferView-associatedItem-list-listItem-'+associatedItem.country_id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].country_id===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('unique_country', associatedItem.country_id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.country_id];
                                      handleDisassociateItem('unique_country', associatedItem.country_id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UniqueCountryToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
UniqueCountryToAddTransferView.propTypes = {
