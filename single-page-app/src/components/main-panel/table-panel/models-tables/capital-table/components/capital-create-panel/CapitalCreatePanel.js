...
//lazy loading
const CapitalAttributesPage = lazy(() => retry(() => import(/* webpackChunkName: "Create-AttributesCapital" */ './components/capital-attributes-page/CapitalAttributesPage')));
const CapitalAssociationsPage = lazy(() => retry(() => import(/* webpackChunkName: "Create-AssociationsCapital" */ './components/capital-associations-page/CapitalAssociationsPage')));
const CapitalConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Create-ConfirmationCapital" */ './components/CapitalConfirmationDialog')));
...
export default function CapitalCreatePanel(props) {
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.capital_id = 0;
  ...
  function setAddUnique_country(variables) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    //delete: fk's
    delete variables.country_id;

    //add: to_one's
    setAddUnique_country(variables);
    
    //add: to_many's

    /*
      API Request: api.capital.createItem
    */
    ...
    let cancelableApiReq = makeCancelable(api.capital.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'capital', method: 'doSave()', request: 'api.capital.createItem'}];
            newError.path=['Capitals', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.capital.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'unique_country':
        if(unique_countryIdsToAdd.current.indexOf(itemId) === -1) {
          unique_countryIdsToAdd.current = [];
          unique_countryIdsToAdd.current.push(itemId);
          setUnique_countryIdsToAddState(unique_countryIdsToAdd.current);
  ...
  return (
    ...
    <Dialog id='CapitalCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='CapitalCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Capital'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " capital" }>
            <Fab
              id='CapitalCreatePanel-fabButton-save' 
              color="secondary" 
