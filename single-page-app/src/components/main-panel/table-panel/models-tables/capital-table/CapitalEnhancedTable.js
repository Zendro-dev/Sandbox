...
import CapitalEnhancedTableHead from './components/CapitalEnhancedTableHead'
import CapitalEnhancedTableToolbar from './components/CapitalEnhancedTableToolbar'
import CapitalUploadFileDialog from './components/CapitalUploadFileDialog'
import CapitalCursorPagination from './components/CapitalCursorPagination'
...
//lazy loading
const CapitalCreatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Create-Capital" */ './components/capital-create-panel/CapitalCreatePanel')));
const CapitalUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Update-Capital" */ './components/capital-update-panel/CapitalUpdatePanel')));
const CapitalDetailPanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Capital" */ './components/capital-detail-panel/CapitalDetailPanel')));
const CapitalDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Delete-Capital" */ './components/CapitalDeleteConfirmationDialog')));
...
export default function CapitalEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('capital_id');
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.capital.getCountItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.capital.getCountItems(graphqlServerUrl, search));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'capital', method: 'getCount()', request: 'api.capital.getCountItems'}];
            newError.path=['Capitals'];
      ...
      //error
      .catch((err) => { //error: on api.capital.getCountItems
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'capital', method: 'getData()', request: 'api.capital.getItems'}];
            newError.path=['Capitals'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['capital']
    &&(Object.keys(changes.lastModelChanged['capital']).length===1)
    &&(changes.lastModelChanged['capital'][Object.keys(changes.lastModelChanged['capital'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set capital_id (internalId)
    variables.capital_id = item.capital_id;

    /*
      API Request: api.capital.deleteItem
    */
    let api = await loadApi("capital");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.capital.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'capital', method: 'doDelete()', request: 'api.capital.deleteItem'}];
            newError.path=['Capitals', `capital_id:${item.capital_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.capital.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.capital.tableTemplate
    */
    let api = await loadApi("capital");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.capital.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'capital', method: 'getCsvTemplate()', request: 'api.capital.tableTemplate'}];
            newError.path=['Capitals', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        link.href = url;
        link.setAttribute("download", "capital-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.capital.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    <div className={classes.root}>
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='CapitalEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='CapitalEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <CapitalEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='CapitalEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'CapitalEnhancedTable-row-'+item.capital_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'CapitalEnhancedTable-row-iconButton-detail-'+item.capital_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.capital&&Array.isArray(permissions.capital)
                                      &&(permissions.capital.includes('update') || permissions.capital.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'CapitalEnhancedTable-row-iconButton-edit-'+item.capital_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.capital&&Array.isArray(permissions.capital)
                                      &&(permissions.capital.includes('delete') || permissions.capital.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'CapitalEnhancedTable-row-iconButton-delete-'+item.capital_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'capital_id: ' + item.capital_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.capital_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.country_id!==null)?item.country_id:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}><ErrorBoundary showMessage={true} belowToolbar={true} dialog={true} open={deleteConfirmationDialogOpen} handleClose={handleDeleteConfirmationReject}>
          <CapitalDeleteConfirmationDialog
            permissions={permissions}
...
CapitalEnhancedTable.propTypes = {
