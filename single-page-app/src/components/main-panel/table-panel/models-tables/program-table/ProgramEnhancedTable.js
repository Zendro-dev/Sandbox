...
import ProgramEnhancedTableHead from './components/ProgramEnhancedTableHead'
import ProgramEnhancedTableToolbar from './components/ProgramEnhancedTableToolbar'
import ProgramUploadFileDialog from './components/ProgramUploadFileDialog'
import ProgramCursorPagination from './components/ProgramCursorPagination'
...
//lazy loading
const ProgramCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Program" */ './components/program-create-panel/ProgramCreatePanel'));
const ProgramUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Program" */ './components/program-update-panel/ProgramUpdatePanel'));
const ProgramDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Program" */ './components/program-detail-panel/ProgramDetailPanel'));
const ProgramDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Program" */ './components/ProgramDeleteConfirmationDialog'));
//Plotly
const ProgramPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Program" */ '../../../../plots/ProgramPlotly'));
...
export default function ProgramEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('programDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.program.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'program', method: 'getCount()', request: 'api.program.getCountItems'}];
            newError.path=['Programs'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', method: 'getData()', request: 'api.program.getItems'}];
            newError.path=['Programs'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['program']
    &&(Object.keys(changes.lastModelChanged['program']).length===1)
    &&(changes.lastModelChanged['program'][Object.keys(changes.lastModelChanged['program'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set programDbId (internalId)
    variables.programDbId = item.programDbId;

    /*
      API Request: api.program.deleteItem
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', method: 'doDelete()', request: 'api.program.deleteItem'}];
            newError.path=['Programs', `programDbId:${item.programDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.program.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.program.tableTemplate
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', method: 'getCsvTemplate()', request: 'api.program.tableTemplate'}];
            newError.path=['Programs', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateProgram.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "program-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.program.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ProgramEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ProgramEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ProgramEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ProgramEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ProgramEnhancedTable-row-'+item.programDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.programDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ProgramEnhancedTable-row-iconButton-detail-'+item.programDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.program&&Array.isArray(permissions.program)
                                      &&(permissions.program.includes('update') || permissions.program.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ProgramEnhancedTable-row-iconButton-edit-'+item.programDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.program&&Array.isArray(permissions.program)
                                      &&(permissions.program.includes('delete') || permissions.program.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ProgramEnhancedTable-row-iconButton-delete-'+item.programDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'programDbId: ' + item.programDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.programDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.abbreviation!==null)?item.abbreviation:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.leadPersonDbId!==null)?item.leadPersonDbId:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.objective!==null)?item.objective:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.programName!==null)?item.programName:'')}
                                    </TableCell>
...
ProgramEnhancedTable.propTypes = {
