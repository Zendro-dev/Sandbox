...
//lazy loading
const ProgramAttributesPage = lazy(() => import(/* webpackChunkName: "Detail-Attributes-Program" */ './components/program-attributes-page/ProgramAttributesPage'));
const ProgramAssociationsPage = lazy(() => import(/* webpackChunkName: "Detail-Associations-Program" */ './components/program-associations-page/ProgramAssociationsPage'));
const ProgramUpdatePanel = lazy(() => import(/* webpackChunkName: "Detail-Update-Program" */ '../program-update-panel/ProgramUpdatePanel'));
const ProgramDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Detail-Delete-Program" */ '../ProgramDeleteConfirmationDialog'));
...
export default function ProgramDetailPanel(props) {
      ...
      lastModelChanged['program'][String(item['programDbId'])]) {
        ...
        if(lastModelChanged['program'][String(item['programDbId'])].op === "update"&&
            lastModelChanged['program'][String(item['programDbId'])].newItem) {
              //replace item
              setItemState(lastModelChanged['program'][String(item['programDbId'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('program', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('program', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='ProgramDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Program | programDbId: " + itemState['programDbId'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='ProgramDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='ProgramDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
