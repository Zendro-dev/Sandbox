...
import TrialsToRemoveTransferViewToolbar from './components/TrialsToRemoveTransferViewToolbar';
import TrialsToRemoveTransferViewCursorPagination from './components/TrialsToRemoveTransferViewCursorPagination';
...
export default function TrialsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.program.getTrialsCount
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getTrialsCount(
      graphqlServerUrl, 
      item.programDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'trials', method: 'getCount()', request: 'api.program.getTrialsCount'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'remove', 'trials'];
      ...
      //error
      .catch((err) => { //error: on api.program.getTrialsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {trialDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.program.getTrials
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getTrials(
      graphqlServerUrl, 
      item.programDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'trials', method: 'getData()', request: 'api.program.getTrials'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'remove', 'trials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.program.getTrials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'trial' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.trial_programDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.trialDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.trialDbId) === -1) {
      lidsToRemove.current.push(item.trialDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.trialDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <TrialsToRemoveTransferViewToolbar
                title={'Trials'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='TrialsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.trialDbId;
                        let label = it.trialName;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(trialDbId=> trialDbId=== it.trialDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'TrialsToRemoveTransferView-listA-listItem-'+it.trialDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'trialDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.trialDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'TrialsToRemoveTransferView-listA-listItem-'+it.trialDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <TrialsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='TrialsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.trialDbId;
                        let label = it.trialName;
                        ...
                        return (
                          <ListItem 
                            id={'TrialsToRemoveTransferView-listB-listItem-'+it.trialDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'TrialsToRemoveTransferView-listB-listItem-'+it.trialDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
TrialsToRemoveTransferView.propTypes = {
