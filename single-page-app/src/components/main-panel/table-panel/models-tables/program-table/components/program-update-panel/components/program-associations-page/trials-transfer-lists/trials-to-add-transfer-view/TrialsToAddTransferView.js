...
import TrialsToAddTransferViewToolbar from './components/TrialsToAddTransferViewToolbar';
import TrialsToAddTransferViewCursorPagination from './components/TrialsToAddTransferViewCursorPagination';
...
export default function TrialsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.program.getNotAssociatedTrialsCount
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getNotAssociatedTrialsCount(graphqlServerUrl, item.programDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'program', association: 'trials', table:'A', method: 'getCount()', request: 'api.program.getNotAssociatedTrialsCount'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'add', 'trials'];
      ...
      //error
      .catch((err) => { //error: on api.program.getNotAssociatedTrialsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.program.getNotAssociatedTrials
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getNotAssociatedTrials(
      graphqlServerUrl,
      item.programDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', association: 'trials', table:'A', method: 'getData()', request: 'api.program.getNotAssociatedTrials'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'add', 'trials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.program.getNotAssociatedTrials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.programDbId, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.trial.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', association: 'trials', table:'B', method: 'getCountB()', request: 'api.trial.getCountItems'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'add', 'trials'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.programDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', association: 'trials', table:'B', method: 'getDataB()', request: 'api.trial.getItems'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'add', 'trials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.programDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'trials' for this item was updated.
    ...
    if(lastModelChanged.program&&
        ...
        lastModelChanged.program[String(item.programDbId)].changedAssociations.trial_programDbId&&
        (lastModelChanged.program[String(item.programDbId)].changedAssociations.trial_programDbId.added ||
         lastModelChanged.program[String(item.programDbId)].changedAssociations.trial_programDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.program[String(item.programDbId)].changedAssociations.trial_programDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('trials', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.program[String(item.programDbId)].changedAssociations.trial_programDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'trials' for this item was updated from the target model (in the peer relation).
    ...
    if(lastModelChanged.trial) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.trialDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('trials', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.programDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.trialDbId) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('trials', item.trialDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.trialDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('trials', item.trialDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <TrialsToAddTransferViewToolbar
                title={'Trials'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='TrialsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'TrialsToAddTransferView-listA-listItem-'+it.trialDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'trialDbId' }>
                                        <Typography 
                                        id={'TrialsToAddTransferView-listA-listItem-id-'+it.trialDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'TrialsToAddTransferView-listA-listItem-'+it.trialDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <TrialsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='TrialsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.trialDbId;
                        let label = it.trialName;
                        ...
                        return (
                          <ListItem 
                            id={'TrialsToAddTransferView-listB-listItem-'+it.trialDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'trialDbId' }>
                                        <Typography 
                                        id={'TrialsToAddTransferView-listB-listItem-id-'+it.trialDbId}
                                        variant="body1" display="block" noWrap={true}>{it.trialDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'TrialsToAddTransferView-listB-listItem-'+it.trialDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
TrialsToAddTransferView.propTypes = {
