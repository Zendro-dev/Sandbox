...
//lazy loading
const ProgramAttributesPage = lazy(() => import(/* webpackChunkName: "Create-AttributesProgram" */ './components/program-attributes-page/ProgramAttributesPage'));
const ProgramAssociationsPage = lazy(() => import(/* webpackChunkName: "Create-AssociationsProgram" */ './components/program-associations-page/ProgramAssociationsPage'));
const ProgramConfirmationDialog = lazy(() => import(/* webpackChunkName: "Create-ConfirmationProgram" */ './components/ProgramConfirmationDialog'));
...
export default function ProgramCreatePanel(props) {
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.abbreviation = 0;
    initialValueOkStates.commonCropName = 0;
    initialValueOkStates.documentationURL = 0;
    initialValueOkStates.leadPersonDbId = -2; //FK
    initialValueOkStates.objective = 0;
    initialValueOkStates.programName = 0;
    initialValueOkStates.programDbId = 0;
  ...
  function setAddLeadPerson(variables) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    //delete: fk's
    delete variables.leadPersonDbId;

    //add: to_one's
    setAddLeadPerson(variables);
    ...
    /*
      API Request: api.program.createItem
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'program', method: 'doSave()', request: 'api.program.createItem'}];
            newError.path=['Programs', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.program.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'leadPerson':
        if(leadPersonIdsToAdd.current.indexOf(itemId) === -1) {
          leadPersonIdsToAdd.current = [];
          leadPersonIdsToAdd.current.push(itemId);
          setLeadPersonIdsToAddState(leadPersonIdsToAdd.current);
      ...
      case 'observationUnits':
        ...
        break;
      case 'trials':
        if(trialsIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    ...
    if(associationKey === 'trials') {
      let iof = trialsIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        trialsIdsToAdd.current.splice(iof, 1);
      }
      return;
    }//end: case 'trials'
  }
  ...
  return (
    ...
    <Dialog id='ProgramCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ProgramCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Program'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " program" }>
            <Fab
              id='ProgramCreatePanel-fabButton-save' 
              color="secondary" 
