...
import LeadPersonToAddTransferViewToolbar from './components/LeadPersonToAddTransferViewToolbar';
import LeadPersonToAddTransferViewCursorPagination from './components/LeadPersonToAddTransferViewCursorPagination';
...
export default function LeadPersonToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.programDbId;

    /*
      API Request: api.program.getLeadPerson
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getLeadPerson(graphqlServerUrl, item.programDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'program', association: 'leadPerson', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.program.getLeadPerson'}];
            newError.path=['update', `programDbId:${currentId}`, 'add', 'leadPerson'];
      ...
      //error
      .catch((err) => { //error: on api.program.getLeadPerson
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.programDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.program.getNotAssociatedLeadPersonCount
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getNotAssociatedLeadPersonCount(graphqlServerUrl, item.programDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', association: 'leadPerson', table:'A', method: 'getCount()', request: 'api.program.getNotAssociatedLeadPersonCount'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'add', 'leadPerson'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.program.getNotAssociatedLeadPersonCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.programDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.program.getNotAssociatedLeadPerson
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getNotAssociatedLeadPerson(
      graphqlServerUrl,
      item.programDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', association: 'leadPerson', table:'A', method: 'getData()', request: 'api.program.getNotAssociatedLeadPerson'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'add', 'leadPerson'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.program.getNotAssociatedLeadPerson
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.programDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getItems
    */
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', association: 'leadPerson', table:'B', method: 'getDataB()', request: 'api.contact.getItems'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'add', 'leadPerson'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.programDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'leadPerson' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'contacts', 
    ...
    if(lastModelChanged.program&&
        ...
        lastModelChanged.program[String(item.programDbId)].changedAssociations&&
        lastModelChanged.program[String(item.programDbId)].changedAssociations.program_leadPersonDbId&&
        (lastModelChanged.program[String(item.programDbId)].changedAssociations.program_leadPersonDbId.added ||
         lastModelChanged.program[String(item.programDbId)].changedAssociations.program_leadPersonDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.program[String(item.programDbId)].changedAssociations.program_leadPersonDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('leadPerson', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('leadPerson', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.program[String(item.programDbId)].changedAssociations.program_leadPersonDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('leadPerson', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'leadPerson' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'contact',
     * but this action happened on the peer relation, identified by 'program_leadPersonDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.contact) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.program_leadPersonDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.program_leadPersonDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.program_leadPersonDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.programDbId);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('leadPerson', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.program_leadPersonDbId.added) {
              let idsAdded = entry[1].changedAssociations.program_leadPersonDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.programDbId);
                if(iof !== -1) {
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('leadPerson', idAdded);
                  } 
    ...
    if(lastModelChanged.contact) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          ...
          if(associatedItem && associatedItem.contactDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('leadPerson', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.contactDbId===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.programDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.contactDbId) === -1) {
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.contactDbId];
        handleDisassociateItem('leadPerson', associatedItem.contactDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('leadPerson', item.contactDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('leadPerson', item.contactDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <LeadPersonToAddTransferViewToolbar
                title={'Contacts'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='LeadPersonToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='LeadPersonToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'LeadPersonToAddTransferView-listA-listItem-'+it.contactDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.contactDbId===it.contactDbId) ? 'contact — ' + t('modelPanels.associatedRecord', 'Associated record') : 'contact '}>
                                <Avatar>
                                  {"contact".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'contactDbId' }>
                                        <Typography 
                                        id={'LeadPersonToAddTransferView-listA-listItem-id-'+it.contactDbId}
                                        variant="body1" display="block" noWrap={true}>{it.contactDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.contactDbId===it.contactDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.contactDbId===it.contactDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'LeadPersonToAddTransferView-listA-listItem-'+it.contactDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <LeadPersonToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='LeadPersonToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='LeadPersonToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='LeadPersonToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.contactDbId;
                        let label = it.name;
                        ...
                        return (
                          <ListItem 
                            id={'LeadPersonToAddTransferView-listB-listItem-'+it.contactDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'contactDbId' }>
                                        <Typography 
                                        id={'LeadPersonToAddTransferView-listB-listItem-id-'+it.contactDbId}
                                        variant="body1" display="block" noWrap={true}>{it.contactDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'LeadPersonToAddTransferView-listB-listItem-'+it.contactDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='LeadPersonToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.contactDbId;
                        let label = associatedItem.name;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'LeadPersonToAddTransferView-associatedItem-list-listItem-'+associatedItem.contactDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'contact — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"contact".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'contactDbId' }>
                                        <Typography 
                                        id={'LeadPersonToAddTransferView-listB-listItem-id-'+associatedItem.contactDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.contactDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'LeadPersonToAddTransferView-associatedItem-list-listItem-'+associatedItem.contactDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].contactDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('leadPerson', associatedItem.contactDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.contactDbId];
                                      handleDisassociateItem('leadPerson', associatedItem.contactDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='LeadPersonToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
LeadPersonToAddTransferView.propTypes = {
