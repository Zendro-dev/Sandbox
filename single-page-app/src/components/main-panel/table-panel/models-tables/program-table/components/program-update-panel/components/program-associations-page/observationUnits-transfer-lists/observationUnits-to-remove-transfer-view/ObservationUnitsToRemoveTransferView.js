...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.program.getObservationUnitsCount
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getObservationUnitsCount(
      graphqlServerUrl, 
      item.programDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationUnits', method: 'getCount()', request: 'api.program.getObservationUnitsCount'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'remove', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.program.getObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.program.getObservationUnits
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getObservationUnits(
      graphqlServerUrl, 
      item.programDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationUnits', method: 'getData()', request: 'api.program.getObservationUnits'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.program.getObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.programDbId, search, getCount, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', association: 'observationUnits', table:'B', method: 'getCountB()', request: 'api.observationUnit.getCountItems'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
        ...
        (response) => {
          ...
          if(response.message === 'ok') {
            ...
            if(response.graphqlErrors) {
              ...
              newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'program', association: 'observationUnits', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
              newError.path=['update', `programDbId:${item.programDbId}`, 'remove', 'observationUnits'];
              newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.program&&
        ...
        lastModelChanged.program[String(item.programDbId)].changedAssociations.observationUnit_programDbId&&
        (lastModelChanged.program[String(item.programDbId)].changedAssociations.observationUnit_programDbId.added ||
         lastModelChanged.program[String(item.programDbId)].changedAssociations.observationUnit_programDbId.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.program[String(item.programDbId)].changedAssociations.observationUnit_programDbId.idsRemoved;
          if(idsRemoved) {
          ...
          let idsAdded = lastModelChanged.program[String(item.programDbId)].changedAssociations.observationUnit_programDbId.idsAdded;
          if(idsAdded) {
            //increment A
