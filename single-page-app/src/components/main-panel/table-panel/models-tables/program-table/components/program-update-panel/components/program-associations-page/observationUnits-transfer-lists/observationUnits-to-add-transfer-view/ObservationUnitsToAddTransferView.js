...
export default function ObservationUnitsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.program.getNotAssociatedObservationUnitsCount
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getNotAssociatedObservationUnitsCount(graphqlServerUrl, item.programDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'program', association: 'observationUnits', table:'A', method: 'getCount()', request: 'api.program.getNotAssociatedObservationUnitsCount'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'add', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.program.getNotAssociatedObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.program.getNotAssociatedObservationUnits
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getNotAssociatedObservationUnits(
      graphqlServerUrl,
      item.programDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'program', association: 'observationUnits', table:'A', method: 'getData()', request: 'api.program.getNotAssociatedObservationUnits'}];
            newError.path=['update', `programDbId:${item.programDbId}`, 'add', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.program.getNotAssociatedObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
