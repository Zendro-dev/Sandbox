...
import ContactEnhancedTableHead from './components/ContactEnhancedTableHead'
import ContactEnhancedTableToolbar from './components/ContactEnhancedTableToolbar'
import ContactCreatePanel from './components/contact-create-panel/ContactCreatePanel'
import ContactUpdatePanel from './components/contact-update-panel/ContactUpdatePanel'
import ContactDetailPanel from './components/contact-detail-panel/ContactDetailPanel'
import ContactDeleteConfirmationDialog from './components/ContactDeleteConfirmationDialog'
import ContactUploadFileDialog from './components/ContactUploadFileDialog'
import ContactCursorPagination from './components/ContactCursorPagination'
...
//Plotly
import ContactPlotly from '../../../../plots/ContactPlotly';
...
export default function ContactEnhancedTable(props) {
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.contact.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', method: 'getCount()', request: 'api.contact.getCountItems'}];
            newError.path=['Contacts'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', method: 'getData()', request: 'api.contact.getItems'}];
            newError.path=['Contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['contact']
    &&(Object.keys(changes.lastModelChanged['contact']).length===1)
    &&(changes.lastModelChanged['contact'][Object.keys(changes.lastModelChanged['contact'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set contact_id (internalId)
    variables.contact_id = item.contact_id;
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['Contacts', `contact_id:${item.contact_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.contact.tableTemplate
    */
    let cancelableApiReq = makeCancelable(api.contact.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', method: 'getCsvTemplate()', request: 'api.contact.tableTemplate'}];
            newError.path=['Contacts', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateContact.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "contact-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.contact.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <ContactEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(ContactPlotly)}
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ContactEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ContactEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ContactEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ContactEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ContactEnhancedTable-row-'+item.contact_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ContactEnhancedTable-row-iconButton-detail-'+item.contact_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ContactEnhancedTable-row-iconButton-edit-'+item.contact_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ContactEnhancedTable-row-iconButton-delete-'+item.contact_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'contact_id: ' + item.contact_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.contact_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.phone!==null)?item.phone:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.address!==null)?item.address:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.affiliation!==null)?item.affiliation:'')}
                                    </TableCell>
...
ContactEnhancedTable.propTypes = {
