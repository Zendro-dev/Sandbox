...
export default function FileAttachmentsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedFileAttachmentsCount(graphqlServerUrl, item.contact_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', association: 'fileAttachments', table:'A', method: 'getCount()', request: 'api.contact.getNotAssociatedFileAttachmentsCount'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedFileAttachments
    */
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedFileAttachments(
      graphqlServerUrl,
      item.contact_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'fileAttachments', table:'A', method: 'getData()', request: 'api.contact.getNotAssociatedFileAttachments'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'fileAttachments', table:'B', method: 'getCountB()', request: 'api.fileAttachment.getCountItems'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'fileAttachments'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'fileAttachments', table:'B', method: 'getDataB()', request: 'api.fileAttachment.getItems'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.contact&&
        ...
        lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_fileAttachment_ids&&
        (lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_fileAttachment_ids.added ||
         lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_fileAttachment_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_fileAttachment_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_fileAttachment_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'contact_fileAttachment_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.fileAttachment) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.contact_fileAttachment_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.contact_fileAttachment_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.contact_fileAttachment_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.contact_fileAttachment_ids.added) {
              let idsAdded = entry[1].changedAssociations.contact_fileAttachment_ids.idsAdded;
              if(idsAdded) {
