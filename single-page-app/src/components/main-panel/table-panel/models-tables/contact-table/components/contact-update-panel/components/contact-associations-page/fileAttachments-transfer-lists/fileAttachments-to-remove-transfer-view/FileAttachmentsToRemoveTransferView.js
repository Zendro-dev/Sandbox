...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.contact.getFileAttachmentsCount(
      graphqlServerUrl, 
      item.contact_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'fileAttachments', method: 'getCount()', request: 'api.contact.getFileAttachmentsCount'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'remove', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getFileAttachments
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.contact.getFileAttachments(
      graphqlServerUrl, 
      item.contact_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'fileAttachments', method: 'getData()', request: 'api.contact.getFileAttachments'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'remove', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
