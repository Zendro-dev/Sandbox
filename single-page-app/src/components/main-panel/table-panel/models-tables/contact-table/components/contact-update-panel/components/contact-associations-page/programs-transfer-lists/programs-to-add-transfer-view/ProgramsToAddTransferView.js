...
import ProgramsToAddTransferViewToolbar from './components/ProgramsToAddTransferViewToolbar';
import ProgramsToAddTransferViewCursorPagination from './components/ProgramsToAddTransferViewCursorPagination';
...
export default function ProgramsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedProgramsCount
    */
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedProgramsCount(graphqlServerUrl, item.contactDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', association: 'programs', table:'A', method: 'getCount()', request: 'api.contact.getNotAssociatedProgramsCount'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'programs'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedProgramsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.contactDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedPrograms
    */
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedPrograms(
      graphqlServerUrl,
      item.contactDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'programs', table:'A', method: 'getData()', request: 'api.contact.getNotAssociatedPrograms'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'programs'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedPrograms
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.contactDbId, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.program.getCountItems
    */
    let api = await loadApi("program");
    let cancelableApiReq = makeCancelable(api.program.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'programs', table:'B', method: 'getCountB()', request: 'api.program.getCountItems'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'programs'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.program.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.contactDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'programs', table:'B', method: 'getDataB()', request: 'api.program.getItems'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'programs'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.contactDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'programs' for this item was updated.
    ...
    if(lastModelChanged.contact&&
        ...
        lastModelChanged.contact[String(item.contactDbId)].changedAssociations&&
        lastModelChanged.contact[String(item.contactDbId)].changedAssociations.program_leadPersonDbId&&
        (lastModelChanged.contact[String(item.contactDbId)].changedAssociations.program_leadPersonDbId.added ||
         lastModelChanged.contact[String(item.contactDbId)].changedAssociations.program_leadPersonDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.contact[String(item.contactDbId)].changedAssociations.program_leadPersonDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('programs', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.contact[String(item.contactDbId)].changedAssociations.program_leadPersonDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'programs' for this item was updated from the target model (in the peer relation).
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.program_leadPersonDbId) {
            ...
            if(entry[1].changedAssociations.program_leadPersonDbId.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.contactDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.program_leadPersonDbId.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.contactDbId);
                if(iof !== -1) {
    ...
    if(lastModelChanged.program) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.programDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('programs', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.contactDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.programDbId) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('programs', item.programDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.programDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('programs', item.programDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ProgramsToAddTransferViewToolbar
                title={'Programs'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ProgramsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ProgramsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ProgramsToAddTransferView-listA-listItem-'+it.programDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'program' }>
                                <Avatar>{"program".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'programDbId' }>
                                        <Typography 
                                        id={'ProgramsToAddTransferView-listA-listItem-id-'+it.programDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ProgramsToAddTransferView-listA-listItem-'+it.programDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ProgramsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ProgramsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ProgramsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ProgramsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.programDbId;
                        let label = it.programName;
                        ...
                        return (
                          <ListItem 
                            id={'ProgramsToAddTransferView-listB-listItem-'+it.programDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'programDbId' }>
                                        <Typography 
                                        id={'ProgramsToAddTransferView-listB-listItem-id-'+it.programDbId}
                                        variant="body1" display="block" noWrap={true}>{it.programDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ProgramsToAddTransferView-listB-listItem-'+it.programDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ProgramsToAddTransferView.propTypes = {
