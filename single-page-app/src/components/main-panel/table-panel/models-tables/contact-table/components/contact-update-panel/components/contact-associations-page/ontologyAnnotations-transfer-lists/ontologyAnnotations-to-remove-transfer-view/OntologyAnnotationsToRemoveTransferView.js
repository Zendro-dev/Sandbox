...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getOntologyAnnotationsCount
    */
    let cancelableApiReq = makeCancelable(api.contact.getOntologyAnnotationsCount(
      graphqlServerUrl, 
      item.contact_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'ontologyAnnotations', method: 'getCount()', request: 'api.contact.getOntologyAnnotationsCount'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'remove', 'ontologyAnnotations'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getOntologyAnnotationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getOntologyAnnotations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.contact.getOntologyAnnotations(
      graphqlServerUrl, 
      item.contact_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'ontologyAnnotations', method: 'getData()', request: 'api.contact.getOntologyAnnotations'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'remove', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getOntologyAnnotations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.contact_id, search, getCount, configurePagination, onEmptyPage]);
