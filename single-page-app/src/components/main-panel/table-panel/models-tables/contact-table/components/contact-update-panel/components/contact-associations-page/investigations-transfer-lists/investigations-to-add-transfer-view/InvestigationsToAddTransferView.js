...
export default function InvestigationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedInvestigationsCount
    */
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedInvestigationsCount(graphqlServerUrl, item.contact_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', association: 'investigations', table:'A', method: 'getCount()', request: 'api.contact.getNotAssociatedInvestigationsCount'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'investigations'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedInvestigationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedInvestigations
    */
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedInvestigations(
      graphqlServerUrl,
      item.contact_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'investigations', table:'A', method: 'getData()', request: 'api.contact.getNotAssociatedInvestigations'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedInvestigations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'investigations', table:'B', method: 'getCountB()', request: 'api.investigation.getCountItems'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'investigations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'investigations', table:'B', method: 'getDataB()', request: 'api.investigation.getItems'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.contact&&
        ...
        lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_investigation_ids&&
        (lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_investigation_ids.added ||
         lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_investigation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_investigation_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_investigation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'contact_investigation_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.investigation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.contact_investigation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.contact_investigation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.contact_investigation_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.contact_investigation_ids.added) {
              let idsAdded = entry[1].changedAssociations.contact_investigation_ids.idsAdded;
              if(idsAdded) {
