...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getTrialsCount
    */
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getTrialsCount(
      graphqlServerUrl, 
      item.contactDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'trials', method: 'getCount()', request: 'api.contact.getTrialsCount'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'remove', 'trials'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getTrialsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getTrials
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getTrials(
      graphqlServerUrl, 
      item.contactDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'trials', method: 'getData()', request: 'api.contact.getTrials'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'remove', 'trials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getTrials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
