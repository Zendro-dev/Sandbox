...
//lazy loading
const ContactAttributesPage = lazy(() => import(/* webpackChunkName: "Create-AttributesContact" */ './components/contact-attributes-page/ContactAttributesPage'));
const ContactAssociationsPage = lazy(() => import(/* webpackChunkName: "Create-AssociationsContact" */ './components/contact-associations-page/ContactAssociationsPage'));
const ContactConfirmationDialog = lazy(() => import(/* webpackChunkName: "Create-ConfirmationContact" */ './components/ContactConfirmationDialog'));
...
export default function ContactCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.studyDbIds = null;
    initialValues.trialDbIds = null;
  ...
  async function doSave(event) {
    ...
    */
    ...
    //delete: fk's
    delete variables.studyDbIds;
    delete variables.trialDbIds;
    ...
    /*
      API Request: api.contact.createItem
    */
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', method: 'doSave()', request: 'api.contact.createItem'}];
            newError.path=['Contacts', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.contact.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'programs':
        if(programsIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'programs') {
      let iof = programsIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        programsIdsToAdd.current.splice(iof, 1);
      }
      return;
    }//end: case 'programs'
  ...
  return (
    ...
    <Dialog id='ContactCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ContactCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Tooltip title={ t('modelPanels.save') + " contact" }>
            <Fab
              id='ContactCreatePanel-fabButton-save' 
              color="secondary" 
