...
import ContactUpdatePanel from '../contact-update-panel/ContactUpdatePanel'
import ContactDeleteConfirmationDialog from '../ContactDeleteConfirmationDialog'
      ...
      lastModelChanged['contact'][String(item['contact_id'])]) {
        ...
        if(lastModelChanged['contact'][String(item['contact_id'])].op === "update"&&
            lastModelChanged['contact'][String(item['contact_id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['contact'][String(item['contact_id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Contact | contact_id: " + itemState['contact_id'] }
              </Typography>
              {/*
                Actions:
                - Edit
