...
export default function StudiesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedStudiesCount(graphqlServerUrl, item.contact_id, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'studies'];
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.contact&&
        ...
        lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_study_ids&&
        (lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_study_ids.added ||
         lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_study_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_study_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_study_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'contact_study_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.contact_study_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.contact_study_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.contact_study_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.contact_study_ids.added) {
              let idsAdded = entry[1].changedAssociations.contact_study_ids.idsAdded;
              if(idsAdded) {
