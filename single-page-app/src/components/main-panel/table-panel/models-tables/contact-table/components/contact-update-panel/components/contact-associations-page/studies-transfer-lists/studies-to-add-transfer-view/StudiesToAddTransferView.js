...
export default function StudiesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedStudiesCount
    */
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedStudiesCount(graphqlServerUrl, item.contactDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', association: 'studies', table:'A', method: 'getCount()', request: 'api.contact.getNotAssociatedStudiesCount'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedStudies
    */
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedStudies(
      graphqlServerUrl,
      item.contactDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'studies', table:'A', method: 'getData()', request: 'api.contact.getNotAssociatedStudies'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'studies', table:'B', method: 'getCountB()', request: 'api.study.getCountItems'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'studies'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'studies', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.contact&&
        ...
        lastModelChanged.contact[String(item.contactDbId)].changedAssociations.study_contactDbIds&&
        (lastModelChanged.contact[String(item.contactDbId)].changedAssociations.study_contactDbIds.added ||
         lastModelChanged.contact[String(item.contactDbId)].changedAssociations.study_contactDbIds.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.contact[String(item.contactDbId)].changedAssociations.study_contactDbIds.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.contact[String(item.contactDbId)].changedAssociations.study_contactDbIds.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'study_contactDbIds'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_contactDbIds) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_contactDbIds.removed) {
              let idsRemoved = entry[1].changedAssociations.study_contactDbIds.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study_contactDbIds.added) {
              let idsAdded = entry[1].changedAssociations.study_contactDbIds.idsAdded;
              if(idsAdded) {
