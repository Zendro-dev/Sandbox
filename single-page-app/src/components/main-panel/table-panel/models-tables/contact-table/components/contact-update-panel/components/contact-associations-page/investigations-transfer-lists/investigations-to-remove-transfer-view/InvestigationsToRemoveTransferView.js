...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getInvestigationsCount
    */
    let cancelableApiReq = makeCancelable(api.contact.getInvestigationsCount(
      graphqlServerUrl, 
      item.contact_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'investigations', method: 'getCount()', request: 'api.contact.getInvestigationsCount'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'remove', 'investigations'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getInvestigationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getInvestigations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.contact.getInvestigations(
      graphqlServerUrl, 
      item.contact_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'investigations', method: 'getData()', request: 'api.contact.getInvestigations'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'remove', 'investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getInvestigations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
