...
import ProgramsToRemoveTransferViewToolbar from './components/ProgramsToRemoveTransferViewToolbar';
import ProgramsToRemoveTransferViewCursorPagination from './components/ProgramsToRemoveTransferViewCursorPagination';
...
export default function ProgramsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getProgramsCount
    */
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getProgramsCount(
      graphqlServerUrl, 
      item.contactDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'programs', method: 'getCount()', request: 'api.contact.getProgramsCount'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'remove', 'programs'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getProgramsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {programDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.contact.getPrograms
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getPrograms(
      graphqlServerUrl, 
      item.contactDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'programs', method: 'getData()', request: 'api.contact.getPrograms'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'remove', 'programs'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getPrograms
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.contactDbId, search, getCount, configurePagination, onEmptyPage]);
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'program' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.program_leadPersonDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.programDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.programDbId) === -1) {
      lidsToRemove.current.push(item.programDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.programDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ProgramsToRemoveTransferViewToolbar
                title={'Programs'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ProgramsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ProgramsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.programDbId;
                        let label = it.programName;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(programDbId=> programDbId=== it.programDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'ProgramsToRemoveTransferView-listA-listItem-'+it.programDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'programDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.programDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'ProgramsToRemoveTransferView-listA-listItem-'+it.programDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <ProgramsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ProgramsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ProgramsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ProgramsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.programDbId;
                        let label = it.programName;
                        ...
                        return (
                          <ListItem 
                            id={'ProgramsToRemoveTransferView-listB-listItem-'+it.programDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'ProgramsToRemoveTransferView-listB-listItem-'+it.programDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ProgramsToRemoveTransferView.propTypes = {
