...
export default function TrialsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedTrialsCount
    */
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedTrialsCount(graphqlServerUrl, item.contactDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', association: 'trials', table:'A', method: 'getCount()', request: 'api.contact.getNotAssociatedTrialsCount'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'trials'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedTrialsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedTrials
    */
    let api = await loadApi("contact");
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedTrials(
      graphqlServerUrl,
      item.contactDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'trials', table:'A', method: 'getData()', request: 'api.contact.getNotAssociatedTrials'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'trials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedTrials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'trials', table:'B', method: 'getCountB()', request: 'api.trial.getCountItems'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'trials'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'trials', table:'B', method: 'getDataB()', request: 'api.trial.getItems'}];
            newError.path=['update', `contactDbId:${item.contactDbId}`, 'add', 'trials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.contact&&
        ...
        lastModelChanged.contact[String(item.contactDbId)].changedAssociations.trial_contactDbIds&&
        (lastModelChanged.contact[String(item.contactDbId)].changedAssociations.trial_contactDbIds.added ||
         lastModelChanged.contact[String(item.contactDbId)].changedAssociations.trial_contactDbIds.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.contact[String(item.contactDbId)].changedAssociations.trial_contactDbIds.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.contact[String(item.contactDbId)].changedAssociations.trial_contactDbIds.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'trial_contactDbIds'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.trial) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.trial_contactDbIds) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.trial_contactDbIds.removed) {
              let idsRemoved = entry[1].changedAssociations.trial_contactDbIds.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.trial_contactDbIds.added) {
              let idsAdded = entry[1].changedAssociations.trial_contactDbIds.idsAdded;
              if(idsAdded) {
