...
export default function ContactUpdatePanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.contact[String(item.contact_id)]) {
        ...
        //updated item
        if(lastModelChanged.contact[String(item.contact_id)].op === "update"&&
            lastModelChanged.contact[String(item.contact_id)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.contact[String(item.contact_id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, item.contact_id]);
  ...
  initialValueOkStates.contact_id = (item.contact_id!==null ? 1 : 0);
  initialValueOkStates.name = (item.name!==null ? 1 : 0);
  initialValueOkStates.email = (item.email!==null ? 1 : 0);
  initialValueOkStates.phone = (item.phone!==null ? 1 : 0);
  initialValueOkStates.address = (item.address!==null ? 1 : 0);
  initialValueOkStates.affiliation = (item.affiliation!==null ? 1 : 0);
  ...
  function getInitialValueAjvStates() {
    ...
    _initialValueAjvStates.contact_id = {errors: []};
    _initialValueAjvStates.name = {errors: []};
    _initialValueAjvStates.email = {errors: []};
    _initialValueAjvStates.phone = {errors: []};
    _initialValueAjvStates.address = {errors: []};
    _initialValueAjvStates.affiliation = {errors: []};
  ...
  function areThereChangedFields() {
    if(values.current.contact_id !== item.contact_id) { return true;}
    if(values.current.name !== item.name) { return true;}
    if(values.current.email !== item.email) { return true;}
    if(values.current.phone !== item.phone) { return true;}
    if(values.current.address !== item.address) { return true;}
    if(values.current.affiliation !== item.affiliation) { return true;}
  ...
  function setAddRemoveManyFileAttachments(variables) {
    ...
    if(!changedAssociations.current.contact_fileAttachment_ids) changedAssociations.current.contact_fileAttachment_ids = {};
    ...
    if(fileAttachmentsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.contact_fileAttachment_ids.added = true;
      if(changedAssociations.current.contact_fileAttachment_ids.idsAdded){
        fileAttachmentsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.contact_fileAttachment_ids.idsAdded.includes(it)) changedAssociations.current.contact_fileAttachment_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.contact_fileAttachment_ids.idsAdded = [...fileAttachmentsIdsToAdd.current];
      }
    }
    /**
    ...
    if(fileAttachmentsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.contact_fileAttachment_ids.removed = true;
      if(changedAssociations.current.contact_fileAttachment_ids.idsRemoved){
        fileAttachmentsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.contact_fileAttachment_ids.idsRemoved.includes(it)) changedAssociations.current.contact_fileAttachment_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.contact_fileAttachment_ids.idsRemoved = [...fileAttachmentsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyInvestigations(variables) {
    //data to notify changes
    if(!changedAssociations.current.contact_investigation_ids) changedAssociations.current.contact_investigation_ids = {};
    ...
    if(investigationsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.contact_investigation_ids.added = true;
      if(changedAssociations.current.contact_investigation_ids.idsAdded){
        investigationsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.contact_investigation_ids.idsAdded.includes(it)) changedAssociations.current.contact_investigation_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.contact_investigation_ids.idsAdded = [...investigationsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(investigationsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.contact_investigation_ids.removed = true;
      if(changedAssociations.current.contact_investigation_ids.idsRemoved){
        investigationsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.contact_investigation_ids.idsRemoved.includes(it)) changedAssociations.current.contact_investigation_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.contact_investigation_ids.idsRemoved = [...investigationsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyOntologyAnnotations(variables) {
    //data to notify changes
    if(!changedAssociations.current.contact_ontologyAnnotation_ids) changedAssociations.current.contact_ontologyAnnotation_ids = {};
    ...
    if(ontologyAnnotationsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.contact_ontologyAnnotation_ids.added = true;
      if(changedAssociations.current.contact_ontologyAnnotation_ids.idsAdded){
        ontologyAnnotationsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.contact_ontologyAnnotation_ids.idsAdded.includes(it)) changedAssociations.current.contact_ontologyAnnotation_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.contact_ontologyAnnotation_ids.idsAdded = [...ontologyAnnotationsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(ontologyAnnotationsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.contact_ontologyAnnotation_ids.removed = true;
      if(changedAssociations.current.contact_ontologyAnnotation_ids.idsRemoved){
        ontologyAnnotationsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.contact_ontologyAnnotation_ids.idsRemoved.includes(it)) changedAssociations.current.contact_ontologyAnnotation_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.contact_ontologyAnnotation_ids.idsRemoved = [...ontologyAnnotationsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyStudies(variables) {
    //data to notify changes
    if(!changedAssociations.current.contact_study_ids) changedAssociations.current.contact_study_ids = {};
    ...
    if(studiesIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.contact_study_ids.added = true;
      if(changedAssociations.current.contact_study_ids.idsAdded){
        studiesIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.contact_study_ids.idsAdded.includes(it)) changedAssociations.current.contact_study_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.contact_study_ids.idsAdded = [...studiesIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(studiesIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.contact_study_ids.removed = true;
      if(changedAssociations.current.contact_study_ids.idsRemoved){
        studiesIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.contact_study_ids.idsRemoved.includes(it)) changedAssociations.current.contact_study_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.contact_study_ids.idsRemoved = [...studiesIdsToRemove.current];
      }
    }
...
function setAjvErrors(err) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.contact.updateItem
    */
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', method: 'doSave()', request: 'api.contact.updateItem'}];
            newError.path=['Contacts', `contact_id:${item.contact_id}`, 'update'];
      ...
      //error
      .catch((err) => { //error: on api.contact.updateItem
        if(err.isCanceled) {
          return;
        } else {
          //set ajv errors
  ...
  return (
    <Dialog id='ContactUpdatePanel-dialog' 
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ContactUpdatePanel-button-cancel'
              edge="start" 
              color="inherit" 
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": Contact | contact_id: " + item.contact_id}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " contact" }>
              <Fab
                id='ContactUpdatePanel-fabButton-save' 
                color="secondary" 
