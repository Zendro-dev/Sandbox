...
//lazy loading
const ContactAttributesPage = lazy(() => import(/* webpackChunkName: "Update-Attributes-Contact" */ './components/contact-attributes-page/ContactAttributesPage'));
const ContactAssociationsPage = lazy(() => import(/* webpackChunkName: "Update-Associations-Contact" */ './components/contact-associations-page/ContactAssociationsPage'));
const ContactConfirmationDialog = lazy(() => import(/* webpackChunkName: "Update-Confirmation-Contact" */ './components/ContactConfirmationDialog'));
const ProgramDetailPanel = lazy(() => import(/* webpackChunkName: "Update-Detail-Program" */ '../../../program-table/components/program-detail-panel/ProgramDetailPanel'));
...
const TrialDetailPanel = lazy(() => import(/* webpackChunkName: "Update-Detail-Trial" */ '../../../trial-table/components/trial-detail-panel/TrialDetailPanel'));
...
export default function ContactUpdatePanel(props) {
  ...
  const [programsIdsToAddState, setProgramsIdsToAddState] = useState([]);
  const programsIdsToAdd = useRef([]);
  const [programsIdsToRemoveState, setProgramsIdsToRemoveState] = useState([]);
  const programsIdsToRemove = useRef([]);
  ...
  function getInitialValues() {
    ...
    initialValues.trialDbIds = item.trialDbIds;
  ...
  function getInitialForeignKeys() {
    ...
    initialForeignKeys.trialDbIds = item.trialDbIds;
  ...
  function areThereChangedFields() {
    ...
    if(values.current.trialDbIds !== item.trialDbIds) { return true;}
    return false;
  }


  function setAddRemoveManyPrograms(variables) {
    //data to notify changes
    if(!changedAssociations.current.program_leadPersonDbId) changedAssociations.current.program_leadPersonDbId = {};

    /**
     * Case: The toAdd list isn't empty.
     */
    if(programsIdsToAdd.current.length>0) {
      //set ids to add
      variables.addPrograms = [ ...programsIdsToAdd.current];
      //changes to nofity
      changedAssociations.current.program_leadPersonDbId.added = true;
      if(changedAssociations.current.program_leadPersonDbId.idsAdded){
        programsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.program_leadPersonDbId.idsAdded.includes(it)) changedAssociations.current.program_leadPersonDbId.idsAdded.push(it);});
      } else {
        changedAssociations.current.program_leadPersonDbId.idsAdded = [...programsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
     */
    if(programsIdsToRemove.current.length>0) {
      //set ids to remove
      variables.removePrograms = [ ...programsIdsToRemove.current];
      //changes to nofity
      changedAssociations.current.program_leadPersonDbId.removed = true;
      if(changedAssociations.current.program_leadPersonDbId.idsRemoved){
        programsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.program_leadPersonDbId.idsRemoved.includes(it)) changedAssociations.current.program_leadPersonDbId.idsRemoved.push(it);});
      } else {
        changedAssociations.current.program_leadPersonDbId.idsRemoved = [...programsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyStudies(variables) {
    //data to notify changes
    if(!changedAssociations.current.study_contactDbIds) changedAssociations.current.study_contactDbIds = {};
    ...
    if(studiesIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.study_contactDbIds.added = true;
      if(changedAssociations.current.study_contactDbIds.idsAdded){
        studiesIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.study_contactDbIds.idsAdded.includes(it)) changedAssociations.current.study_contactDbIds.idsAdded.push(it);});
      } else {
        changedAssociations.current.study_contactDbIds.idsAdded = [...studiesIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(studiesIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.study_contactDbIds.removed = true;
      if(changedAssociations.current.study_contactDbIds.idsRemoved){
        studiesIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.study_contactDbIds.idsRemoved.includes(it)) changedAssociations.current.study_contactDbIds.idsRemoved.push(it);});
      } else {
        changedAssociations.current.study_contactDbIds.idsRemoved = [...studiesIdsToRemove.current];
      }
    }
    
    return;
  }
  function setAddRemoveManyTrials(variables) {
    //data to notify changes
    if(!changedAssociations.current.trial_contactDbIds) changedAssociations.current.trial_contactDbIds = {};
    ...
    if(trialsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.trial_contactDbIds.added = true;
      if(changedAssociations.current.trial_contactDbIds.idsAdded){
        trialsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.trial_contactDbIds.idsAdded.includes(it)) changedAssociations.current.trial_contactDbIds.idsAdded.push(it);});
      } else {
        changedAssociations.current.trial_contactDbIds.idsAdded = [...trialsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(trialsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.trial_contactDbIds.removed = true;
      if(changedAssociations.current.trial_contactDbIds.idsRemoved){
        trialsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.trial_contactDbIds.idsRemoved.includes(it)) changedAssociations.current.trial_contactDbIds.idsRemoved.push(it);});
      } else {
        changedAssociations.current.trial_contactDbIds.idsRemoved = [...trialsIdsToRemove.current];
      }
    }
...
function setAjvErrors(err) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    delete variables.trialDbIds; //FK

    //add & remove: to_one's

    //add & remove: to_many's
    setAddRemoveManyPrograms(variables);
    setAddRemoveManyStudies(variables);
    setAddRemoveManyTrials(variables);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['Contacts', `contactDbId:${item.contactDbId}`, 'update'];
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'programs':
        programsIdsToAdd.current.push(itemId);
        setProgramsIdsToAddState([...programsIdsToAdd.current]);
        break;
      case 'studies':
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'programs') {
      for(let i=0; i<programsIdsToAdd.current.length; ++i)
      {
        if(programsIdsToAdd.current[i] === itemId) {
          programsIdsToAdd.current.splice(i, 1);
          setProgramsIdsToAddState([...programsIdsToAdd.current]);
          return;
        }
      }
  ...
  const handleTransferToRemove = (associationKey, itemId) => {
    switch(associationKey) {
        ...
        programsIdsToRemove.current.push(itemId);
        setProgramsIdsToRemoveState([...programsIdsToRemove.current]);
        break;
        case 'studies':
  ...
  const handleUntransferFromRemove =(associationKey, itemId) => {
    if(associationKey === 'programs') {
      for(let i=0; i<programsIdsToRemove.current.length; ++i)
      {
        if(programsIdsToRemove.current[i] === itemId) {
          programsIdsToRemove.current.splice(i, 1);
          setProgramsIdsToRemoveState([...programsIdsToRemove.current]);
          return;
        }
      }
            ...
            <Grid item xs={12}>
              ...
              <Suspense fallback={<div />}>
                <ContactAssociationsPage
                  ...
                  item={item}
                  programsIdsToAdd={programsIdsToAddState}
                  programsIdsToRemove={programsIdsToRemoveState}
