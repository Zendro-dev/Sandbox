...
export default function OntologyAnnotationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedOntologyAnnotationsCount
    */
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedOntologyAnnotationsCount(graphqlServerUrl, item.contact_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', association: 'ontologyAnnotations', table:'A', method: 'getCount()', request: 'api.contact.getNotAssociatedOntologyAnnotationsCount'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'ontologyAnnotations'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedOntologyAnnotationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.contact_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.contact.getNotAssociatedOntologyAnnotations
    */
    let cancelableApiReq = makeCancelable(api.contact.getNotAssociatedOntologyAnnotations(
      graphqlServerUrl,
      item.contact_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'ontologyAnnotations', table:'A', method: 'getData()', request: 'api.contact.getNotAssociatedOntologyAnnotations'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.contact.getNotAssociatedOntologyAnnotations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.contact_id, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'ontologyAnnotations', table:'B', method: 'getCountB()', request: 'api.ontologyAnnotation.getCountItems'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'ontologyAnnotations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.contact_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'contact', association: 'ontologyAnnotations', table:'B', method: 'getDataB()', request: 'api.ontologyAnnotation.getItems'}];
            newError.path=['update', `contact_id:${item.contact_id}`, 'add', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.contact_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.contact&&
        lastModelChanged.contact[String(item.contact_id)]&&
        lastModelChanged.contact[String(item.contact_id)].changedAssociations&&
        lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_ontologyAnnotation_ids&&
        (lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_ontologyAnnotation_ids.added ||
         lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_ontologyAnnotation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_ontologyAnnotation_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.contact[String(item.contact_id)].changedAssociations.contact_ontologyAnnotation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'contact_ontologyAnnotation_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyAnnotation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.contact_ontologyAnnotation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.contact_ontologyAnnotation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.contact_ontologyAnnotation_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.contact_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.contact_ontologyAnnotation_ids.added) {
              let idsAdded = entry[1].changedAssociations.contact_ontologyAnnotation_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.contact_id);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.contact_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
