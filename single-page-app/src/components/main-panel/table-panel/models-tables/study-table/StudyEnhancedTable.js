...
import StudyEnhancedTableHead from './components/StudyEnhancedTableHead'
import StudyEnhancedTableToolbar from './components/StudyEnhancedTableToolbar'
import StudyUploadFileDialog from './components/StudyUploadFileDialog'
import StudyCursorPagination from './components/StudyCursorPagination'
...
//lazy loading
const StudyCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Study" */ './components/study-create-panel/StudyCreatePanel'));
const StudyUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Study" */ './components/study-update-panel/StudyUpdatePanel'));
const StudyDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Study" */ './components/study-detail-panel/StudyDetailPanel'));
const StudyDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Study" */ './components/StudyDeleteConfirmationDialog'));
//Plotly
const StudyPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Study" */ '../../../../plots/StudyPlotly'));
...
export default function StudyEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('studyDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.study.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', method: 'getCount()', request: 'api.study.getCountItems'}];
            newError.path=['Studies'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', method: 'getData()', request: 'api.study.getItems'}];
            newError.path=['Studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['study']
    &&(Object.keys(changes.lastModelChanged['study']).length===1)
    &&(changes.lastModelChanged['study'][Object.keys(changes.lastModelChanged['study'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.study.tableTemplate
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', method: 'getCsvTemplate()', request: 'api.study.tableTemplate'}];
            newError.path=['Studies', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateStudy.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "study-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.study.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='StudyEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='StudyEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <StudyEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='StudyEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'StudyEnhancedTable-row-'+item.studyDbId}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'StudyEnhancedTable-row-iconButton-detail-'+item.studyDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'StudyEnhancedTable-row-iconButton-edit-'+item.studyDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'StudyEnhancedTable-row-iconButton-delete-'+item.studyDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'studyDbId: ' + item.studyDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.studyDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.culturalPractices!==null)?item.culturalPractices:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.license!==null)?item.license:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.observationUnitsDescription!==null)?item.observationUnitsDescription:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.studyDescription!==null)?item.studyDescription:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.studyName!==null)?item.studyName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.studyType!==null)?item.studyType:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.seasonDbIds!==null)?item.seasonDbIds:'')}
                                    </TableCell>
