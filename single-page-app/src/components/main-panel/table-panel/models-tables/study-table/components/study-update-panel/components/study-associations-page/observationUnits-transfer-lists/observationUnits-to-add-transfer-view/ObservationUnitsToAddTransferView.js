...
export default function ObservationUnitsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedObservationUnitsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedObservationUnitsCount(graphqlServerUrl, item.studyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'observationUnits', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedObservationUnitsCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedObservationUnits
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedObservationUnits(
      graphqlServerUrl,
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'observationUnits', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedObservationUnits'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
