...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getFileAttachmentsCount(
      graphqlServerUrl, 
      item.study_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'fileAttachments', method: 'getCount()', request: 'api.study.getFileAttachmentsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.study.getFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getFileAttachments
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.study.getFileAttachments(
      graphqlServerUrl, 
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'fileAttachments', method: 'getData()', request: 'api.study.getFileAttachments'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
