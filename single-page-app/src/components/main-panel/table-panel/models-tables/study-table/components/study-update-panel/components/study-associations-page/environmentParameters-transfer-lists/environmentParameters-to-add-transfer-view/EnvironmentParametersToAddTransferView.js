...
import EnvironmentParametersToAddTransferViewToolbar from './components/EnvironmentParametersToAddTransferViewToolbar';
import EnvironmentParametersToAddTransferViewCursorPagination from './components/EnvironmentParametersToAddTransferViewCursorPagination';
...
export default function EnvironmentParametersToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"environmentParameterDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.study.getNotAssociatedEnvironmentParametersCount
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedEnvironmentParametersCount(graphqlServerUrl, item.studyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'environmentParameters', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedEnvironmentParametersCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'environmentParameters'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedEnvironmentParametersCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.studyDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedEnvironmentParameters
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedEnvironmentParameters(
      graphqlServerUrl,
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'environmentParameters', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedEnvironmentParameters'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'environmentParameters'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedEnvironmentParameters
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.studyDbId, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.environmentParameter.getCountItems
    */
    let api = await loadApi("environmentParameter");
    let cancelableApiReq = makeCancelable(api.environmentParameter.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'environmentParameters', table:'B', method: 'getCountB()', request: 'api.environmentParameter.getCountItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'environmentParameters'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.environmentParameter.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.studyDbId, searchB]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.environmentParameter.getItems
    */
    let api = await loadApi("environmentParameter");
    let cancelableApiReq = makeCancelable(api.environmentParameter.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'environmentParameters', table:'B', method: 'getDataB()', request: 'api.environmentParameter.getItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'environmentParameters'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.environmentParameter.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.studyDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'environmentParameters' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'environmentParameters', 
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations&&
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.environmentParameter_studyDbId&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.environmentParameter_studyDbId.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.environmentParameter_studyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.studyDbId)].changedAssociations.environmentParameter_studyDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('environmentParameters', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.environmentParameter_studyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'environmentParameters' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'environmentParameter',
     * but this action happened on the peer relation, identified by 'environmentParameter_studyDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'environmentParameter'.
     * B: the current item internalId is in the addedIds of the updated 'environmentParameter'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'environmentParameter' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.environmentParameter) {
      let oens = Object.entries(lastModelChanged.environmentParameter);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.environmentParameter_studyDbId) {
            ...
            if(entry[1].changedAssociations.environmentParameter_studyDbId.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.studyDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.environmentParameter_studyDbId.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.studyDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.environmentParameterDbId;
    ...
    /*
     * Case 3: 
     * The attributes of some 'environmentParameter' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.environmentParameter) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.environmentParameterDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.environmentParameterDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.environmentParameterDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.environmentParameterDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.environmentParameterDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.environmentParameterDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('environmentParameters', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.studyDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnEnvironmentParameterRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.environmentParameterDbId) === -1) {
      lidsToAdd.current.push(item.environmentParameterDbId);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('environmentParameters', item.environmentParameterDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.environmentParameterDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('environmentParameters', item.environmentParameterDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <EnvironmentParametersToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EnvironmentParametersToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='EnvironmentParametersToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.environmentParameterDbId;
                        let label = it.parameterName;
                        let sublabel = it.environmentParametersDbId;
                        
                        return (
                          <ListItem 
                            id={'EnvironmentParametersToAddTransferView-listA-listItem-'+it.environmentParameterDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'environmentParameter' }>
                                <Avatar>{"environmentParameter".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                            ...
                            <ListItemText
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'environmentParameterDbId' }>
                                        <Typography 
                                        id={'EnvironmentParametersToAddTransferView-listA-listItem-id-'+it.environmentParameterDbId}
                                        variant="body1" display="block" noWrap={true}>{it.environmentParameterDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                              ...
                              secondary={
                                <React.Fragment>
                                  ...
                                  {/* Sublabel */}
                                  {(sublabel) && (
                                    <Tooltip title={ 'environmentParametersDbId' }>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'EnvironmentParametersToAddTransferView-listA-listItem-'+it.environmentParameterDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <EnvironmentParametersToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EnvironmentParametersToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EnvironmentParametersToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='EnvironmentParametersToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.environmentParameterDbId;
                        let label = it.parameterName;
                        ...
                        return (
                          <ListItem 
                            id={'EnvironmentParametersToAddTransferView-listB-listItem-'+it.environmentParameterDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'environmentParameterDbId' }>
                                        <Typography 
                                        id={'EnvironmentParametersToAddTransferView-listB-listItem-id-'+it.environmentParameterDbId}
                                        variant="body1" display="block" noWrap={true}>{it.environmentParameterDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'EnvironmentParametersToAddTransferView-listB-listItem-'+it.environmentParameterDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
EnvironmentParametersToAddTransferView.propTypes = {
