...
export default function FactorsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedFactorsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedFactorsCount(graphqlServerUrl, item.study_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'factors', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedFactorsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'factors'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedFactorsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedFactors
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedFactors(
      graphqlServerUrl,
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'factors', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedFactors'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'factors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedFactors
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'factors', table:'B', method: 'getCountB()', request: 'api.factor.getCountItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'factors'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'factors', table:'B', method: 'getDataB()', request: 'api.factor.getItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'factors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.study_id)].changedAssociations.study_factor_ids&&
        (lastModelChanged.study[String(item.study_id)].changedAssociations.study_factor_ids.added ||
         lastModelChanged.study[String(item.study_id)].changedAssociations.study_factor_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.study_id)].changedAssociations.study_factor_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.study[String(item.study_id)].changedAssociations.study_factor_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'study_factor_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.factor) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_factor_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_factor_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.study_factor_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study_factor_ids.added) {
              let idsAdded = entry[1].changedAssociations.study_factor_ids.idsAdded;
              if(idsAdded) {
