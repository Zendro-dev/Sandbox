...
export default function TrialToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.studyDbId;

    /*
      API Request: api.study.getTrial
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getTrial(graphqlServerUrl, item.studyDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'trial', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.study.getTrial'}];
            newError.path=['update', `studyDbId:${currentId}`, 'add', 'trial'];
      ...
      //error
      .catch((err) => { //error: on api.study.getTrial
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.studyDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedTrialCount
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedTrialCount(graphqlServerUrl, item.studyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'trial', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedTrialCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'trial'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedTrialCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedTrial
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedTrial(
      graphqlServerUrl,
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'trial', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedTrial'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'trial'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedTrial
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.studyDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'trial', table:'B', method: 'getDataB()', request: 'api.trial.getItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'trial'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.studyDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_trialDbId&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_trialDbId.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_trialDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_trialDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_trialDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'study_trialDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.trial) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_trialDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_trialDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.study_trialDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study_trialDbId.added) {
              let idsAdded = entry[1].changedAssociations.study_trialDbId.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.studyDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
