...
import SeasonsToAddTransferViewToolbar from './components/SeasonsToAddTransferViewToolbar';
import SeasonsToAddTransferViewCursorPagination from './components/SeasonsToAddTransferViewCursorPagination';
...
export default function SeasonsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedSeasonsCount
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedSeasonsCount(graphqlServerUrl, item.studyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'seasons', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedSeasonsCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'seasons'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedSeasonsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedSeasons
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedSeasons(
      graphqlServerUrl,
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'seasons', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedSeasons'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'seasons'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedSeasons
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.season.getCountItems
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'seasons', table:'B', method: 'getCountB()', request: 'api.season.getCountItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'seasons'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.season.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'seasons', table:'B', method: 'getDataB()', request: 'api.season.getItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'seasons'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'seasons' for this item was updated.
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.season_studyDbIds&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.season_studyDbIds.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.season_studyDbIds.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.studyDbId)].changedAssociations.season_studyDbIds.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('seasons', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.season_studyDbIds.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'seasons' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'season',
     * but this action happened on the peer relation, identified by 'season_studyDbIds'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.season) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.season_studyDbIds) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.season_studyDbIds.removed) {
              let idsRemoved = entry[1].changedAssociations.season_studyDbIds.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.season_studyDbIds.added) {
              let idsAdded = entry[1].changedAssociations.season_studyDbIds.idsAdded;
              if(idsAdded) {
    ...
    if(lastModelChanged.season) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.seasonDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('seasons', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.seasonDbId) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('seasons', item.seasonDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.seasonDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('seasons', item.seasonDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <SeasonsToAddTransferViewToolbar
                title={'Seasons'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='SeasonsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'SeasonsToAddTransferView-listA-listItem-'+it.seasonDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'season' }>
                                <Avatar>{"season".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'seasonDbId' }>
                                        <Typography 
                                        id={'SeasonsToAddTransferView-listA-listItem-id-'+it.seasonDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'SeasonsToAddTransferView-listA-listItem-'+it.seasonDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <SeasonsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='SeasonsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.seasonDbId;
                        let label = it.season;
                        ...
                        return (
                          <ListItem 
                            id={'SeasonsToAddTransferView-listB-listItem-'+it.seasonDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'seasonDbId' }>
                                        <Typography 
                                        id={'SeasonsToAddTransferView-listB-listItem-id-'+it.seasonDbId}
                                        variant="body1" display="block" noWrap={true}>{it.seasonDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'SeasonsToAddTransferView-listB-listItem-'+it.seasonDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
SeasonsToAddTransferView.propTypes = {
