...
export default function ProtocolsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedProtocolsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedProtocolsCount(graphqlServerUrl, item.study_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'protocols', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedProtocolsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'protocols'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedProtocolsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedProtocols
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedProtocols(
      graphqlServerUrl,
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'protocols', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedProtocols'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'protocols'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedProtocols
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'protocols', table:'B', method: 'getCountB()', request: 'api.protocol.getCountItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'protocols'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'protocols', table:'B', method: 'getDataB()', request: 'api.protocol.getItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'protocols'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.study_id)].changedAssociations.study_protocol_ids&&
        (lastModelChanged.study[String(item.study_id)].changedAssociations.study_protocol_ids.added ||
         lastModelChanged.study[String(item.study_id)].changedAssociations.study_protocol_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.study_id)].changedAssociations.study_protocol_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.study[String(item.study_id)].changedAssociations.study_protocol_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'study_protocol_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.protocol) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_protocol_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_protocol_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.study_protocol_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study_protocol_ids.added) {
              let idsAdded = entry[1].changedAssociations.study_protocol_ids.idsAdded;
              if(idsAdded) {
