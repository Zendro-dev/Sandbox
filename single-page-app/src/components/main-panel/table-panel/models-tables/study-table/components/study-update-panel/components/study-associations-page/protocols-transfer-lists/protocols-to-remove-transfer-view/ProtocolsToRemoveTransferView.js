...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getProtocolsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getProtocolsCount(
      graphqlServerUrl, 
      item.study_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'protocols', method: 'getCount()', request: 'api.study.getProtocolsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'protocols'];
      ...
      //error
      .catch((err) => { //error: on api.study.getProtocolsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getProtocols
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.study.getProtocols(
      graphqlServerUrl, 
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'protocols', method: 'getData()', request: 'api.study.getProtocols'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'protocols'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getProtocols
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
