...
export default function OntologyAnnotationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedOntologyAnnotationsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedOntologyAnnotationsCount(graphqlServerUrl, item.study_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'ontologyAnnotations', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedOntologyAnnotationsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'ontologyAnnotations'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedOntologyAnnotationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedOntologyAnnotations
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedOntologyAnnotations(
      graphqlServerUrl,
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'ontologyAnnotations', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedOntologyAnnotations'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedOntologyAnnotations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.study_id, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'ontologyAnnotations', table:'B', method: 'getCountB()', request: 'api.ontologyAnnotation.getCountItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'ontologyAnnotations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.study_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'ontologyAnnotations', table:'B', method: 'getDataB()', request: 'api.ontologyAnnotation.getItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.study_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.study_id)].changedAssociations.study_ontologyAnnotation_ids&&
        (lastModelChanged.study[String(item.study_id)].changedAssociations.study_ontologyAnnotation_ids.added ||
         lastModelChanged.study[String(item.study_id)].changedAssociations.study_ontologyAnnotation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.study_id)].changedAssociations.study_ontologyAnnotation_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.study[String(item.study_id)].changedAssociations.study_ontologyAnnotation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'study_ontologyAnnotation_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyAnnotation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_ontologyAnnotation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_ontologyAnnotation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.study_ontologyAnnotation_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study_ontologyAnnotation_ids.added) {
              let idsAdded = entry[1].changedAssociations.study_ontologyAnnotation_ids.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.study_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
