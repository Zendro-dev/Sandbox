...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'contacts'];
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {contactDbId: lidsToRemove.current}
    //     }]
    //   };
    // }
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.contact_stduyDbIds&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.contact_stduyDbIds.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.contact_stduyDbIds.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.contact_stduyDbIds.idsRemoved;
          if(idsRemoved) {
          ...
          let idsAdded = lastModelChanged.study[String(item.studyDbId)].changedAssociations.contact_stduyDbIds.idsAdded;
          if(idsAdded) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'contact_stduyDbIds'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.contact) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.contact_stduyDbIds) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.contact_stduyDbIds.removed) {
              let idsRemoved = entry[1].changedAssociations.contact_stduyDbIds.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.studyDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToRemove
                  let idRemoved = entry[1].newItem.contactDbId;
            ...
            if(entry[1].changedAssociations.contact_stduyDbIds.added) {
              let idsAdded = entry[1].changedAssociations.contact_stduyDbIds.idsAdded;
              if(idsAdded) {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.contactDbId) === -1) {
      lidsToRemove.current.push(item.contactDbId);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('contacts', item.contactDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.contactDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('contacts', item.contactDbId);
    }
  }
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ContactsToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        let disabled = lidsToRemove.current.find(contactDbId=> contactDbId=== it.contactDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'ContactsToRemoveTransferView-listA-listItem-'+it.contactDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'contactDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.contactDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'ContactsToRemoveTransferView-listA-listItem-'+it.contactDbId+'-button-remove'}
                                    color="primary"
                  ...
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ContactsToRemoveTransferView-list-listB'
                      ...
                      {itemsB.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ContactsToRemoveTransferView-listB-listItem-'+it.contactDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'ContactsToRemoveTransferView-listB-listItem-'+it.contactDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
