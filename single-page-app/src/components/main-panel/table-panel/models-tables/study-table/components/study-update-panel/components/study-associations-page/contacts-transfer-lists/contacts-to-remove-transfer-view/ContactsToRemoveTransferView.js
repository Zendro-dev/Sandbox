...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getContactsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getContactsCount(
      graphqlServerUrl, 
      item.study_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'contacts', method: 'getCount()', request: 'api.study.getContactsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'contacts'];
      ...
      //error
      .catch((err) => { //error: on api.study.getContactsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getContacts
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.study.getContacts(
      graphqlServerUrl, 
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'contacts', method: 'getData()', request: 'api.study.getContacts'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getContacts
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
