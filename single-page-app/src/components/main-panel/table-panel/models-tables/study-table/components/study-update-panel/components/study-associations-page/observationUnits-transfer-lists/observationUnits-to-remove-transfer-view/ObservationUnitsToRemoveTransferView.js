...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getObservationUnitsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getObservationUnitsCount(
      graphqlServerUrl, 
      item.studyDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationUnits', method: 'getCount()', request: 'api.study.getObservationUnitsCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.study.getObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getObservationUnits
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.study.getObservationUnits(
      graphqlServerUrl, 
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationUnits', method: 'getData()', request: 'api.study.getObservationUnits'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'observationUnits', table:'B', method: 'getCountB()', request: 'api.observationUnit.getCountItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
        ...
        (response) => {
          ...
          if(response.message === 'ok') {
            ...
            if(response.graphqlErrors) {
              ...
              newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'study', association: 'observationUnits', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
              newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'observationUnits'];
              newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.observationUnit_studyDbId&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.observationUnit_studyDbId.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.observationUnit_studyDbId.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.observationUnit_studyDbId.idsRemoved;
          if(idsRemoved) {
          ...
          let idsAdded = lastModelChanged.study[String(item.studyDbId)].changedAssociations.observationUnit_studyDbId.idsAdded;
          if(idsAdded) {
            //increment A
