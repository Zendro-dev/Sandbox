...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getFactorsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getFactorsCount(
      graphqlServerUrl, 
      item.study_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'factors', method: 'getCount()', request: 'api.study.getFactorsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'factors'];
      ...
      //error
      .catch((err) => { //error: on api.study.getFactorsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getFactors
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.study.getFactors(
      graphqlServerUrl, 
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'factors', method: 'getData()', request: 'api.study.getFactors'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'factors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getFactors
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
