...
import EventsToRemoveTransferViewToolbar from './components/EventsToRemoveTransferViewToolbar';
import EventsToRemoveTransferViewCursorPagination from './components/EventsToRemoveTransferViewCursorPagination';
...
export default function EventsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getEventsCount
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getEventsCount(
      graphqlServerUrl, 
      item.studyDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'events', method: 'getCount()', request: 'api.study.getEventsCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'events'];
      ...
      //error
      .catch((err) => { //error: on api.study.getEventsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {eventType: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.study.getEvents
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getEvents(
      graphqlServerUrl, 
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'events', method: 'getData()', request: 'api.study.getEvents'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'events'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getEvents
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'event' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.event_studyDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.eventType;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.eventType) === -1) {
      lidsToRemove.current.push(item.eventType);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.eventType);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <EventsToRemoveTransferViewToolbar
                title={'Events'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='EventsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.eventType;
                        let label = undefined;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(eventType=> eventType=== it.eventType) !== undefined;

                        return (
                          <ListItem 
                            id={'EventsToRemoveTransferView-listA-listItem-'+it.eventType}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'eventType' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.eventType}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'EventsToRemoveTransferView-listA-listItem-'+it.eventType+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <EventsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='EventsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.eventType;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'EventsToRemoveTransferView-listB-listItem-'+it.eventType}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'EventsToRemoveTransferView-listB-listItem-'+it.eventType+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
EventsToRemoveTransferView.propTypes = {
