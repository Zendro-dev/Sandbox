...
import EventsToAddTransferViewToolbar from './components/EventsToAddTransferViewToolbar';
import EventsToAddTransferViewCursorPagination from './components/EventsToAddTransferViewCursorPagination';
...
export default function EventsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedEventsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedEventsCount(graphqlServerUrl, item.studyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'events', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedEventsCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'events'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedEventsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedEvents
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedEvents(
      graphqlServerUrl,
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'events', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedEvents'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'events'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedEvents
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.event.getCountItems
    */
    let cancelableApiReq = makeCancelable(api.event.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'events', table:'B', method: 'getCountB()', request: 'api.event.getCountItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'events'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.event.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'events', table:'B', method: 'getDataB()', request: 'api.event.getItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'events'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'events' for this item was updated.
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.event_studyDbId&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.event_studyDbId.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.event_studyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.studyDbId)].changedAssociations.event_studyDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('events', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.event_studyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'events' for this item was updated from the target model (in the peer relation).
    ...
    if(lastModelChanged.event) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.eventType===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('events', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.eventType) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('events', item.eventType);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.eventType);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('events', item.eventType);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <EventsToAddTransferViewToolbar
                title={'Events'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='EventsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'EventsToAddTransferView-listA-listItem-'+it.eventType}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'eventType' }>
                                        <Typography 
                                        id={'EventsToAddTransferView-listA-listItem-id-'+it.eventType}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'EventsToAddTransferView-listA-listItem-'+it.eventType+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <EventsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='EventsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.eventType;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'EventsToAddTransferView-listB-listItem-'+it.eventType}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'eventType' }>
                                        <Typography 
                                        id={'EventsToAddTransferView-listB-listItem-id-'+it.eventType}
                                        variant="body1" display="block" noWrap={true}>{it.eventType}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'EventsToAddTransferView-listB-listItem-'+it.eventType+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
EventsToAddTransferView.propTypes = {
