...
export default function ContactsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedContactsCount(graphqlServerUrl, item.studyDbId, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'contacts'];
...
const handleReloadClickB = (event) => {
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.contactDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    ...
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ContactsToAddTransferView-listA-listItem-'+it.contactDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'contactDbId' }>
                                        <Typography 
                                        id={'ContactsToAddTransferView-listA-listItem-id-'+it.contactDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ContactsToAddTransferView-listA-listItem-'+it.contactDbId+'-button-add'}
                                  color="primary"
                  ...
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ContactsToAddTransferView-list-listB'
                      ...
                      {itemsB.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ContactsToAddTransferView-listB-listItem-'+it.contactDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'contactDbId' }>
                                        <Typography 
                                        id={'ContactsToAddTransferView-listB-listItem-id-'+it.contactDbId}
                                        variant="body1" display="block" noWrap={true}>{it.contactDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ContactsToAddTransferView-listB-listItem-'+it.contactDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
