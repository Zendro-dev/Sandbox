...
export default function ContactsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedContactsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedContactsCount(graphqlServerUrl, item.study_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'contacts', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedContactsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'contacts'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedContactsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedContacts
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedContacts(
      graphqlServerUrl,
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'contacts', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedContacts'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedContacts
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'contacts', table:'B', method: 'getCountB()', request: 'api.contact.getCountItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'contacts'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'contacts', table:'B', method: 'getDataB()', request: 'api.contact.getItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.study_id)].changedAssociations.study_contact_ids&&
        (lastModelChanged.study[String(item.study_id)].changedAssociations.study_contact_ids.added ||
         lastModelChanged.study[String(item.study_id)].changedAssociations.study_contact_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.study_id)].changedAssociations.study_contact_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.study[String(item.study_id)].changedAssociations.study_contact_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'study_contact_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.contact) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_contact_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_contact_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.study_contact_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study_contact_ids.added) {
              let idsAdded = entry[1].changedAssociations.study_contact_ids.idsAdded;
              if(idsAdded) {
