...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getObservationsCount
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getObservationsCount(
      graphqlServerUrl, 
      item.studyDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observations', method: 'getCount()', request: 'api.study.getObservationsCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'observations'];
      ...
      //error
      .catch((err) => { //error: on api.study.getObservationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getObservations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getObservations(
      graphqlServerUrl, 
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observations', method: 'getData()', request: 'api.study.getObservations'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getObservations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
