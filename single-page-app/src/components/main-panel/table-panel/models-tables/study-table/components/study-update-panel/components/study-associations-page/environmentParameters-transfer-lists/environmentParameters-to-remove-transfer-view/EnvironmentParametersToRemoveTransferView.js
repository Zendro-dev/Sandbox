...
import EnvironmentParametersToRemoveTransferViewToolbar from './components/EnvironmentParametersToRemoveTransferViewToolbar';
import EnvironmentParametersToRemoveTransferViewCursorPagination from './components/EnvironmentParametersToRemoveTransferViewCursorPagination';
...
export default function EnvironmentParametersToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getEnvironmentParametersCount
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getEnvironmentParametersCount(
      graphqlServerUrl, 
      item.studyDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'environmentParameters', method: 'getCount()', request: 'api.study.getEnvironmentParametersCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'environmentParameters'];
      ...
      //error
      .catch((err) => { //error: on api.study.getEnvironmentParametersCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {environmentParameterDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.study.getEnvironmentParameters
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getEnvironmentParameters(
      graphqlServerUrl, 
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'environmentParameters', method: 'getData()', request: 'api.study.getEnvironmentParameters'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'environmentParameters'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getEnvironmentParameters
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.studyDbId, search, getCount, configurePagination, onEmptyPage]);
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'environmentParameter' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.environmentParameter_studyDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.environmentParameterDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.environmentParameterDbId) === -1) {
      lidsToRemove.current.push(item.environmentParameterDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.environmentParameterDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <EnvironmentParametersToRemoveTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EnvironmentParametersToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='EnvironmentParametersToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        let disabled = lidsToRemove.current.find(environmentParameterDbId=> environmentParameterDbId=== it.environmentParameterDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'EnvironmentParametersToRemoveTransferView-listA-listItem-'+it.environmentParameterDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'environmentParameterDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.environmentParameterDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'EnvironmentParametersToRemoveTransferView-listA-listItem-'+it.environmentParameterDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <EnvironmentParametersToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EnvironmentParametersToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EnvironmentParametersToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='EnvironmentParametersToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.environmentParameterDbId;
                        let label = it.parameterName;
                        ...
                        return (
                          <ListItem 
                            id={'EnvironmentParametersToRemoveTransferView-listB-listItem-'+it.environmentParameterDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'EnvironmentParametersToRemoveTransferView-listB-listItem-'+it.environmentParameterDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
EnvironmentParametersToRemoveTransferView.propTypes = {
