...
export default function ObservationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedObservationsCount
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedObservationsCount(graphqlServerUrl, item.studyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'observations', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedObservationsCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'observations'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedObservationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedObservations
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedObservations(
      graphqlServerUrl,
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'observations', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedObservations'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedObservations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'observations', table:'B', method: 'getCountB()', request: 'api.observation.getCountItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'observations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'observations', table:'B', method: 'getDataB()', request: 'api.observation.getItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.observation_studyDbId&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.observation_studyDbId.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.observation_studyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.studyDbId)].changedAssociations.observation_studyDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.observation_studyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
