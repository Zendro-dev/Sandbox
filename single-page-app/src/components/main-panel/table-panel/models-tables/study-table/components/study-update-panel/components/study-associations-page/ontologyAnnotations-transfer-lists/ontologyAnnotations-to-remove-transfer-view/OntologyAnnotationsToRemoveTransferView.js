...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getOntologyAnnotationsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getOntologyAnnotationsCount(
      graphqlServerUrl, 
      item.study_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'ontologyAnnotations', method: 'getCount()', request: 'api.study.getOntologyAnnotationsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'ontologyAnnotations'];
      ...
      //error
      .catch((err) => { //error: on api.study.getOntologyAnnotationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getOntologyAnnotations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.study.getOntologyAnnotations(
      graphqlServerUrl, 
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'ontologyAnnotations', method: 'getData()', request: 'api.study.getOntologyAnnotations'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getOntologyAnnotations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.study_id, search, getCount, configurePagination, onEmptyPage]);
