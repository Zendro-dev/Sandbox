...
import SeasonsToRemoveTransferViewToolbar from './components/SeasonsToRemoveTransferViewToolbar';
import SeasonsToRemoveTransferViewCursorPagination from './components/SeasonsToRemoveTransferViewCursorPagination';
...
export default function SeasonsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getSeasonsCount
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getSeasonsCount(
      graphqlServerUrl, 
      item.studyDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'seasons', method: 'getCount()', request: 'api.study.getSeasonsCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'seasons'];
      ...
      //error
      .catch((err) => { //error: on api.study.getSeasonsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {seasonDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.study.getSeasons
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getSeasons(
      graphqlServerUrl, 
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'seasons', method: 'getData()', request: 'api.study.getSeasons'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'remove', 'seasons'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getSeasons
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'season' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.season_studyDbIds.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.seasonDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.seasonDbId) === -1) {
      lidsToRemove.current.push(item.seasonDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.seasonDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <SeasonsToRemoveTransferViewToolbar
                title={'Seasons'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='SeasonsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.seasonDbId;
                        let label = it.season;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(seasonDbId=> seasonDbId=== it.seasonDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'SeasonsToRemoveTransferView-listA-listItem-'+it.seasonDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'seasonDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.seasonDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'SeasonsToRemoveTransferView-listA-listItem-'+it.seasonDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <SeasonsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='SeasonsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.seasonDbId;
                        let label = it.season;
                        ...
                        return (
                          <ListItem 
                            id={'SeasonsToRemoveTransferView-listB-listItem-'+it.seasonDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'SeasonsToRemoveTransferView-listB-listItem-'+it.seasonDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
SeasonsToRemoveTransferView.propTypes = {
