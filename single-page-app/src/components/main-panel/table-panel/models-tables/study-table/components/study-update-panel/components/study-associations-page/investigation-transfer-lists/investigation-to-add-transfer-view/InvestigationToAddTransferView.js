...
import InvestigationToAddTransferViewToolbar from './components/InvestigationToAddTransferViewToolbar';
import InvestigationToAddTransferViewCursorPagination from './components/InvestigationToAddTransferViewCursorPagination';
...
export default function InvestigationToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.study_id;

    /*
      API Request: api.study.getInvestigation
    */
    let cancelableApiReq = makeCancelable(api.study.getInvestigation(graphqlServerUrl, item.study_id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'investigation', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.study.getInvestigation'}];
            newError.path=['update', `study_id:${currentId}`, 'add', 'investigation'];
      ...
      //error
      .catch((err) => { //error: on api.study.getInvestigation
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.study_id]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"investigation_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.study.getNotAssociatedInvestigationCount
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedInvestigationCount(graphqlServerUrl, item.study_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'investigation', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedInvestigationCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'investigation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedInvestigationCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.study_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedInvestigation
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedInvestigation(
      graphqlServerUrl,
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'investigation', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedInvestigation'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'investigation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedInvestigation
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.study_id, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getItems
    */
    let cancelableApiReq = makeCancelable(api.investigation.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'investigation', table:'B', method: 'getDataB()', request: 'api.investigation.getItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'investigation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.investigation.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.study_id, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'investigation' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'investigations', 
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.study_id)].changedAssociations&&
        lastModelChanged.study[String(item.study_id)].changedAssociations.study_investigation_id&&
        (lastModelChanged.study[String(item.study_id)].changedAssociations.study_investigation_id.added ||
         lastModelChanged.study[String(item.study_id)].changedAssociations.study_investigation_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.study_id)].changedAssociations.study_investigation_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('investigation', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('investigation', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.study[String(item.study_id)].changedAssociations.study_investigation_id.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('investigation', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'investigation' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'investigation',
     * but this action happened on the peer relation, identified by 'study_investigation_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'investigation'.
     * B: the current item internalId is in the addedIds of the updated 'investigation'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'investigation' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.investigation) {
      let oens = Object.entries(lastModelChanged.investigation);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_investigation_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_investigation_id.removed) {
              let idsRemoved = entry[1].changedAssociations.study_investigation_id.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.study_id);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('investigation', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.study_investigation_id.added) {
              let idsAdded = entry[1].changedAssociations.study_investigation_id.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.study_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.investigation_id;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('investigation', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'investigation' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.investigation) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.investigation_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.investigation_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.investigation_id===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.investigation_id===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.investigation_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.investigation_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('investigation', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.investigation_id===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.study_id, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnInvestigationRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.investigation_id) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.investigation_id);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.investigation_id];
        handleDisassociateItem('investigation', associatedItem.investigation_id);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('investigation', item.investigation_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('investigation', item.investigation_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <InvestigationToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InvestigationToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='InvestigationToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.investigation_id;
                        let label = it.title;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'InvestigationToAddTransferView-listA-listItem-'+it.investigation_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.investigation_id===it.investigation_id) ? 'investigation — ' + t('modelPanels.associatedRecord', 'Associated record') : 'investigation '}>
                                <Avatar>
                                  {"investigation".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'investigation_id' }>
                                        <Typography 
                                        id={'InvestigationToAddTransferView-listA-listItem-id-'+it.investigation_id}
                                        variant="body1" display="block" noWrap={true}>{it.investigation_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.investigation_id===it.investigation_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.investigation_id===it.investigation_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'InvestigationToAddTransferView-listA-listItem-'+it.investigation_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <InvestigationToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InvestigationToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InvestigationToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='InvestigationToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.investigation_id;
                        let label = it.title;
                        ...
                        return (
                          <ListItem 
                            id={'InvestigationToAddTransferView-listB-listItem-'+it.investigation_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'investigation_id' }>
                                        <Typography 
                                        id={'InvestigationToAddTransferView-listB-listItem-id-'+it.investigation_id}
                                        variant="body1" display="block" noWrap={true}>{it.investigation_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'InvestigationToAddTransferView-listB-listItem-'+it.investigation_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='InvestigationToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.investigation_id;
                        let label = associatedItem.title;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'InvestigationToAddTransferView-associatedItem-list-listItem-'+associatedItem.investigation_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'investigation — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"investigation".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'investigation_id' }>
                                        <Typography 
                                        id={'InvestigationToAddTransferView-listB-listItem-id-'+associatedItem.investigation_id}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.investigation_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'InvestigationToAddTransferView-associatedItem-list-listItem-'+associatedItem.investigation_id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].investigation_id===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('investigation', associatedItem.investigation_id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.investigation_id];
                                      handleDisassociateItem('investigation', associatedItem.investigation_id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InvestigationToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
InvestigationToAddTransferView.propTypes = {
