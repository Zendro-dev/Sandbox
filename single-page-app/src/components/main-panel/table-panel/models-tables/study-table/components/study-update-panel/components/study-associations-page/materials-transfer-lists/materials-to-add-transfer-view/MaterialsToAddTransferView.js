...
export default function MaterialsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedMaterialsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedMaterialsCount(graphqlServerUrl, item.study_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'materials', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedMaterialsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'materials'];
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedMaterialsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedMaterials
    */
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedMaterials(
      graphqlServerUrl,
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'materials', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedMaterials'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedMaterials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'materials', table:'B', method: 'getCountB()', request: 'api.material.getCountItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'materials'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'materials', table:'B', method: 'getDataB()', request: 'api.material.getItems'}];
            newError.path=['update', `study_id:${item.study_id}`, 'add', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.study_id)].changedAssociations.study_material_ids&&
        (lastModelChanged.study[String(item.study_id)].changedAssociations.study_material_ids.added ||
         lastModelChanged.study[String(item.study_id)].changedAssociations.study_material_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.study_id)].changedAssociations.study_material_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.study[String(item.study_id)].changedAssociations.study_material_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'study_material_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.material) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_material_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_material_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.study_material_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study_material_ids.added) {
              let idsAdded = entry[1].changedAssociations.study_material_ids.idsAdded;
              if(idsAdded) {
