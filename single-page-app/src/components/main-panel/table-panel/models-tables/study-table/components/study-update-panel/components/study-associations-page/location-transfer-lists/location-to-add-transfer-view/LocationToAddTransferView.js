...
export default function LocationToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.study.getLocation
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getLocation(graphqlServerUrl, item.studyDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', association: 'location', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.study.getLocation'}];
            newError.path=['update', `studyDbId:${currentId}`, 'add', 'location'];
      ...
      //error
      .catch((err) => { //error: on api.study.getLocation
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedLocationCount
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedLocationCount(graphqlServerUrl, item.studyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'location', table:'A', method: 'getCount()', request: 'api.study.getNotAssociatedLocationCount'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'location'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedLocationCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getNotAssociatedLocation
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getNotAssociatedLocation(
      graphqlServerUrl,
      item.studyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'location', table:'A', method: 'getData()', request: 'api.study.getNotAssociatedLocation'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'location'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getNotAssociatedLocation
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'study', association: 'location', table:'B', method: 'getDataB()', request: 'api.location.getItems'}];
            newError.path=['update', `studyDbId:${item.studyDbId}`, 'add', 'location'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.study&&
        ...
        lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_locationDbId&&
        (lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_locationDbId.added ||
         lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_locationDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_locationDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.study[String(item.studyDbId)].changedAssociations.study_locationDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'study_locationDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.location) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_locationDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_locationDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.study_locationDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study_locationDbId.added) {
              let idsAdded = entry[1].changedAssociations.study_locationDbId.idsAdded;
              if(idsAdded) {
