...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.study.getMaterialsCount
    */
    let cancelableApiReq = makeCancelable(api.study.getMaterialsCount(
      graphqlServerUrl, 
      item.study_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'materials', method: 'getCount()', request: 'api.study.getMaterialsCount'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'materials'];
      ...
      //error
      .catch((err) => { //error: on api.study.getMaterialsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.study.getMaterials
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.study.getMaterials(
      graphqlServerUrl, 
      item.study_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'materials', method: 'getData()', request: 'api.study.getMaterials'}];
            newError.path=['update', `study_id:${item.study_id}`, 'remove', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getMaterials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
