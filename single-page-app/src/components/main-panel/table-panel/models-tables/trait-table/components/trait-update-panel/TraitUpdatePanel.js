...
import TraitTabsA from './components/TraitTabsA';
...
//lazy loading
const TraitAttributesPage = lazy(() => import(/* webpackChunkName: "Update-Attributes-Trait" */ './components/trait-attributes-page/TraitAttributesPage'));
const TraitAssociationsPage = lazy(() => import(/* webpackChunkName: "Update-Associations-Trait" */ './components/trait-associations-page/TraitAssociationsPage'));
const TraitConfirmationDialog = lazy(() => import(/* webpackChunkName: "Update-Confirmation-Trait" */ './components/TraitConfirmationDialog'));
...
export default function TraitUpdatePanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged.trait&&
      lastModelChanged.trait[String(item.traitDbId)]) {

        //updated item
        if(lastModelChanged.trait[String(item.traitDbId)].op === "update"&&
            lastModelChanged.trait[String(item.traitDbId)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.trait[String(item.traitDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, item.traitDbId]);
  ...
  function getInitialValues() {
    ...
    initialValues.attribute = item.attribute;
    initialValues.entity = item.entity;
    initialValues.mainAbbreviation = item.mainAbbreviation;
    initialValues.status = item.status;
    initialValues.traitClass = item.traitClass;
    initialValues.traitDescription = item.traitDescription;
    initialValues.traitName = item.traitName;
  ...
  initialValueOkStates.attribute = (item.attribute!==null ? 1 : 0);
  initialValueOkStates.entity = (item.entity!==null ? 1 : 0);
  initialValueOkStates.mainAbbreviation = (item.mainAbbreviation!==null ? 1 : 0);
  initialValueOkStates.status = (item.status!==null ? 1 : 0);
  initialValueOkStates.traitClass = (item.traitClass!==null ? 1 : 0);
  initialValueOkStates.traitDescription = (item.traitDescription!==null ? 1 : 0);
  initialValueOkStates.traitName = (item.traitName!==null ? 1 : 0);
  initialValueOkStates.xref = (item.xref!==null ? 1 : 0);
  initialValueOkStates.traitDbId = (item.traitDbId!==null ? 1 : 0);
  ...
  function getInitialValueAjvStates() {
    ...
    _initialValueAjvStates.attribute = {errors: []};
    _initialValueAjvStates.entity = {errors: []};
    _initialValueAjvStates.mainAbbreviation = {errors: []};
    _initialValueAjvStates.status = {errors: []};
    _initialValueAjvStates.traitClass = {errors: []};
    _initialValueAjvStates.traitDescription = {errors: []};
    _initialValueAjvStates.traitName = {errors: []};
    _initialValueAjvStates.xref = {errors: []};
    _initialValueAjvStates.traitDbId = {errors: []};
  ...
  function areThereChangedFields() {
    if(values.current.attribute !== item.attribute) { return true;}
    if(values.current.entity !== item.entity) { return true;}
    if(values.current.mainAbbreviation !== item.mainAbbreviation) { return true;}
    if(values.current.status !== item.status) { return true;}
    if(values.current.traitClass !== item.traitClass) { return true;}
    if(values.current.traitDescription !== item.traitDescription) { return true;}
    if(values.current.traitName !== item.traitName) { return true;}
  ...
  function setAddRemoveOneOntologyReference(variables) {
    ...
    if(ontologyReferenceIdsToAdd.current.length>0) {
      ...
      changedAssociations.current.trait_ontologyDbId.idsAdded = [ontologyReferenceIdsToAdd.current[0]];
    }
    /**
    ...
    if(ontologyReferenceIdsToRemove.current.length>0) {
      ...
      changedAssociations.current.trait_ontologyDbId.idsRemoved = [ontologyReferenceIdsToRemove.current[0]];
    }
  ...
  function setAddRemoveManyObservationVariables(variables) {
    ...
    if(observationVariablesIdsToAdd.current.length>0) {
      ...
      if(changedAssociations.current.observationVariable_traitDbId.idsAdded){
        observationVariablesIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.observationVariable_traitDbId.idsAdded.includes(it)) changedAssociations.current.observationVariable_traitDbId.idsAdded.push(it);});
      } else {
        changedAssociations.current.observationVariable_traitDbId.idsAdded = [...observationVariablesIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(observationVariablesIdsToRemove.current.length>0) {
      ...
      if(changedAssociations.current.observationVariable_traitDbId.idsRemoved){
        observationVariablesIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.observationVariable_traitDbId.idsRemoved.includes(it)) changedAssociations.current.observationVariable_traitDbId.idsRemoved.push(it);});
      } else {
        changedAssociations.current.observationVariable_traitDbId.idsRemoved = [...observationVariablesIdsToRemove.current];
      }
    }
...
function setAjvErrors(err) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.trait.updateItem
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.updateItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'trait', method: 'doSave()', request: 'api.trait.updateItem'}];
            newError.path=['Traits', `traitDbId:${item.traitDbId}`, 'update'];
      ...
      //error
      .catch((err) => { //error: on api.trait.updateItem
        if(err.isCanceled) {
          return;
        } else {
          //set ajv errors
  ...
  return (
    <Dialog id='TraitUpdatePanel-dialog' 
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='TraitUpdatePanel-button-cancel'
              edge="start" 
              color="inherit" 
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": Trait | traitDbId: " + item.traitDbId}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " trait" }>
              <Fab
                id='TraitUpdatePanel-fabButton-save' 
                color="secondary" 
