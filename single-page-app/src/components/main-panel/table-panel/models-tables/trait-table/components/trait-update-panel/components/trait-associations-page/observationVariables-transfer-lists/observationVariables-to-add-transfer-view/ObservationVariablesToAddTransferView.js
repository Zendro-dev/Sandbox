...
export default function ObservationVariablesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.trait.getNotAssociatedObservationVariablesCount
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.getNotAssociatedObservationVariablesCount(graphqlServerUrl, item.traitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'trait', association: 'observationVariables', table:'A', method: 'getCount()', request: 'api.trait.getNotAssociatedObservationVariablesCount'}];
            newError.path=['update', `traitDbId:${item.traitDbId}`, 'add', 'observationVariables'];
      ...
      //error
      .catch((err) => { //error: on api.trait.getNotAssociatedObservationVariablesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.traitDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.trait.getNotAssociatedObservationVariables
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.getNotAssociatedObservationVariables(
      graphqlServerUrl,
      item.traitDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trait', association: 'observationVariables', table:'A', method: 'getData()', request: 'api.trait.getNotAssociatedObservationVariables'}];
            newError.path=['update', `traitDbId:${item.traitDbId}`, 'add', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trait.getNotAssociatedObservationVariables
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.traitDbId, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trait', association: 'observationVariables', table:'B', method: 'getCountB()', request: 'api.observationVariable.getCountItems'}];
            newError.path=['update', `traitDbId:${item.traitDbId}`, 'add', 'observationVariables'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.traitDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trait', association: 'observationVariables', table:'B', method: 'getDataB()', request: 'api.observationVariable.getItems'}];
            newError.path=['update', `traitDbId:${item.traitDbId}`, 'add', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.traitDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trait&&
        lastModelChanged.trait[String(item.traitDbId)]&&
        lastModelChanged.trait[String(item.traitDbId)].changedAssociations&&
        lastModelChanged.trait[String(item.traitDbId)].changedAssociations.observationVariable_traitDbId&&
        (lastModelChanged.trait[String(item.traitDbId)].changedAssociations.observationVariable_traitDbId.added ||
         lastModelChanged.trait[String(item.traitDbId)].changedAssociations.observationVariable_traitDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.trait[String(item.traitDbId)].changedAssociations.observationVariable_traitDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.trait[String(item.traitDbId)].changedAssociations.observationVariable_traitDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationVariable_traitDbId) {
            ...
            if(entry[1].changedAssociations.observationVariable_traitDbId.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.traitDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.observationVariable_traitDbId.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.traitDbId);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.traitDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
