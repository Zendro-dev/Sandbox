...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.trait.getObservationVariablesCount
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.getObservationVariablesCount(
      graphqlServerUrl, 
      item.traitDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationVariables', method: 'getCount()', request: 'api.trait.getObservationVariablesCount'}];
            newError.path=['update', `traitDbId:${item.traitDbId}`, 'remove', 'observationVariables'];
      ...
      //error
      .catch((err) => { //error: on api.trait.getObservationVariablesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.trait.getObservationVariables
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.getObservationVariables(
      graphqlServerUrl, 
      item.traitDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationVariables', method: 'getData()', request: 'api.trait.getObservationVariables'}];
            newError.path=['update', `traitDbId:${item.traitDbId}`, 'remove', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trait.getObservationVariables
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.traitDbId, search, getCount, configurePagination, onEmptyPage]);
