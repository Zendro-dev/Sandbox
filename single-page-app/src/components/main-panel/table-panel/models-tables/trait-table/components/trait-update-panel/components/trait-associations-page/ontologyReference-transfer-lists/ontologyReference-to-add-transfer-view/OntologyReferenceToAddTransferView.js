...
export default function OntologyReferenceToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.traitDbId;

    /*
      API Request: api.trait.getOntologyReference
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.getOntologyReference(graphqlServerUrl, item.traitDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'trait', association: 'ontologyReference', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.trait.getOntologyReference'}];
            newError.path=['update', `traitDbId:${currentId}`, 'add', 'ontologyReference'];
      ...
      //error
      .catch((err) => { //error: on api.trait.getOntologyReference
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.traitDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.trait.getNotAssociatedOntologyReferenceCount
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.getNotAssociatedOntologyReferenceCount(graphqlServerUrl, item.traitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trait', association: 'ontologyReference', table:'A', method: 'getCount()', request: 'api.trait.getNotAssociatedOntologyReferenceCount'}];
            newError.path=['update', `traitDbId:${item.traitDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trait.getNotAssociatedOntologyReferenceCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.trait.getNotAssociatedOntologyReference
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.getNotAssociatedOntologyReference(
      graphqlServerUrl,
      item.traitDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trait', association: 'ontologyReference', table:'A', method: 'getData()', request: 'api.trait.getNotAssociatedOntologyReference'}];
            newError.path=['update', `traitDbId:${item.traitDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trait.getNotAssociatedOntologyReference
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.traitDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trait', association: 'ontologyReference', table:'B', method: 'getDataB()', request: 'api.ontologyReference.getItems'}];
            newError.path=['update', `traitDbId:${item.traitDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.traitDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trait&&
        ...
        lastModelChanged.trait[String(item.traitDbId)].changedAssociations.trait_ontologyDbId&&
        (lastModelChanged.trait[String(item.traitDbId)].changedAssociations.trait_ontologyDbId.added ||
         lastModelChanged.trait[String(item.traitDbId)].changedAssociations.trait_ontologyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.trait[String(item.traitDbId)].changedAssociations.trait_ontologyDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.trait[String(item.traitDbId)].changedAssociations.trait_ontologyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'trait_ontologyDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyReference) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.trait_ontologyDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.trait_ontologyDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.trait_ontologyDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.trait_ontologyDbId.added) {
              let idsAdded = entry[1].changedAssociations.trait_ontologyDbId.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.traitDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
