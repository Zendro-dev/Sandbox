...
import TraitEnhancedTableHead from './components/TraitEnhancedTableHead'
import TraitEnhancedTableToolbar from './components/TraitEnhancedTableToolbar'
import TraitUploadFileDialog from './components/TraitUploadFileDialog'
import TraitCursorPagination from './components/TraitCursorPagination'
...
//lazy loading
const TraitCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Trait" */ './components/trait-create-panel/TraitCreatePanel'));
const TraitUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Trait" */ './components/trait-update-panel/TraitUpdatePanel'));
const TraitDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Trait" */ './components/trait-detail-panel/TraitDetailPanel'));
const TraitDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Trait" */ './components/TraitDeleteConfirmationDialog'));
//Plotly
const TraitPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Trait" */ '../../../../plots/TraitPlotly'));
...
export default function TraitEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('traitDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.trait.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'trait', method: 'getCount()', request: 'api.trait.getCountItems'}];
            newError.path=['Traits'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trait', method: 'getData()', request: 'api.trait.getItems'}];
            newError.path=['Traits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['trait']
    &&(Object.keys(changes.lastModelChanged['trait']).length===1)
    &&(changes.lastModelChanged['trait'][Object.keys(changes.lastModelChanged['trait'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set traitDbId (internalId)
    variables.traitDbId = item.traitDbId;

    /*
      API Request: api.trait.deleteItem
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trait', method: 'doDelete()', request: 'api.trait.deleteItem'}];
            newError.path=['Traits', `traitDbId:${item.traitDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.trait.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.trait.tableTemplate
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trait', method: 'getCsvTemplate()', request: 'api.trait.tableTemplate'}];
            newError.path=['Traits', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateTrait.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "trait-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.trait.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='TraitEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='TraitEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <TraitEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='TraitEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'TraitEnhancedTable-row-'+item.traitDbId}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'TraitEnhancedTable-row-iconButton-detail-'+item.traitDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.trait&&Array.isArray(permissions.trait)
                                      &&(permissions.trait.includes('update') || permissions.trait.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'TraitEnhancedTable-row-iconButton-edit-'+item.traitDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.trait&&Array.isArray(permissions.trait)
                                      &&(permissions.trait.includes('delete') || permissions.trait.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'TraitEnhancedTable-row-iconButton-delete-'+item.traitDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'traitDbId: ' + item.traitDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.traitDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.attribute!==null)?item.attribute:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.entity!==null)?item.entity:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.mainAbbreviation!==null)?item.mainAbbreviation:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.traitClass!==null)?item.traitClass:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.traitDescription!==null)?item.traitDescription:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.traitName!==null)?item.traitName:'')}
                                    </TableCell>
