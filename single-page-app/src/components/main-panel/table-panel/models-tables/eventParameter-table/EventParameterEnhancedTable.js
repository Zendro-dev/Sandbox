...
import EventParameterEnhancedTableHead from './components/EventParameterEnhancedTableHead'
import EventParameterEnhancedTableToolbar from './components/EventParameterEnhancedTableToolbar'
import EventParameterUploadFileDialog from './components/EventParameterUploadFileDialog'
import EventParameterCursorPagination from './components/EventParameterCursorPagination'
...
//lazy loading
const EventParameterCreatePanel = lazy(() => import(/* webpackChunkName: "Create-EventParameter" */ './components/eventParameter-create-panel/EventParameterCreatePanel'));
const EventParameterUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-EventParameter" */ './components/eventParameter-update-panel/EventParameterUpdatePanel'));
const EventParameterDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-EventParameter" */ './components/eventParameter-detail-panel/EventParameterDetailPanel'));
const EventParameterDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-EventParameter" */ './components/EventParameterDeleteConfirmationDialog'));
//Plotly
const EventParameterPlotly = lazy(() => import(/* webpackChunkName: "Plotly-EventParameter" */ '../../../../plots/EventParameterPlotly'));
...
export default function EventParameterEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('eventParameterDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.eventParameter.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'eventParameter', method: 'getCount()', request: 'api.eventParameter.getCountItems'}];
            newError.path=['EventParameters'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'eventParameter', method: 'getData()', request: 'api.eventParameter.getItems'}];
            newError.path=['EventParameters'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['eventParameter']
    &&(Object.keys(changes.lastModelChanged['eventParameter']).length===1)
    &&(changes.lastModelChanged['eventParameter'][Object.keys(changes.lastModelChanged['eventParameter'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set eventParameterDbId (internalId)
    variables.eventParameterDbId = item.eventParameterDbId;

    /*
      API Request: api.eventParameter.deleteItem
    */
    let api = await loadApi("eventParameter");
    let cancelableApiReq = makeCancelable(api.eventParameter.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'eventParameter', method: 'doDelete()', request: 'api.eventParameter.deleteItem'}];
            newError.path=['EventParameters', `eventParameterDbId:${item.eventParameterDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.eventParameter.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.eventParameter.tableTemplate
    */
    let api = await loadApi("eventParameter");
    let cancelableApiReq = makeCancelable(api.eventParameter.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'eventParameter', method: 'getCsvTemplate()', request: 'api.eventParameter.tableTemplate'}];
            newError.path=['EventParameters', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateEventParameter.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "eventParameter-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.eventParameter.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <EventParameterEnhancedTableToolbar
                ...
                >
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='EventParameterEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='EventParameterEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <EventParameterEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='EventParameterEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'EventParameterEnhancedTable-row-'+item.eventParameterDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.eventParameterDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'EventParameterEnhancedTable-row-iconButton-detail-'+item.eventParameterDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.eventParameter&&Array.isArray(permissions.eventParameter)
                                      &&(permissions.eventParameter.includes('update') || permissions.eventParameter.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'EventParameterEnhancedTable-row-iconButton-edit-'+item.eventParameterDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.eventParameter&&Array.isArray(permissions.eventParameter)
                                      &&(permissions.eventParameter.includes('delete') || permissions.eventParameter.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'EventParameterEnhancedTable-row-iconButton-delete-'+item.eventParameterDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'eventParameterDbId: ' + item.eventParameterDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.eventParameterDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.rdfValue!==null)?item.rdfValue:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.eventDbId!==null)?item.eventDbId:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <EventParameterCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(EventParameterPlotly)) && (
                      <Suspense fallback={<div />}>
                        <EventParameterPlotly />
                      </Suspense>
                    )}
                  </div>
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <EventParameterDeleteConfirmationDialog
            permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <EventParameterUploadFileDialog
...
EventParameterEnhancedTable.propTypes = {
