...
import EventToAddTransferViewToolbar from './components/EventToAddTransferViewToolbar';
import EventToAddTransferViewCursorPagination from './components/EventToAddTransferViewCursorPagination';
...
export default function EventToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.eventParameterDbId;

    /*
      API Request: api.eventParameter.getEvent
    */
    let api = await loadApi("eventParameter");
    let cancelableApiReq = makeCancelable(api.eventParameter.getEvent(graphqlServerUrl, item.eventParameterDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'eventParameter', association: 'event', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.eventParameter.getEvent'}];
            newError.path=['update', `eventParameterDbId:${currentId}`, 'add', 'event'];
      ...
      //error
      .catch((err) => { //error: on api.eventParameter.getEvent
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.eventParameterDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"eventType": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.eventParameter.getNotAssociatedEventCount
    */
    let api = await loadApi("eventParameter");
    let cancelableApiReq = makeCancelable(api.eventParameter.getNotAssociatedEventCount(graphqlServerUrl, item.eventParameterDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'eventParameter', association: 'event', table:'A', method: 'getCount()', request: 'api.eventParameter.getNotAssociatedEventCount'}];
            newError.path=['update', `eventParameterDbId:${item.eventParameterDbId}`, 'add', 'event'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.eventParameter.getNotAssociatedEventCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.eventParameterDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.eventParameter.getNotAssociatedEvent
    */
    let api = await loadApi("eventParameter");
    let cancelableApiReq = makeCancelable(api.eventParameter.getNotAssociatedEvent(
      graphqlServerUrl,
      item.eventParameterDbId,
      search,
      variables,
      ops,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'eventParameter', association: 'event', table:'A', method: 'getData()', request: 'api.eventParameter.getNotAssociatedEvent'}];
            newError.path=['update', `eventParameterDbId:${item.eventParameterDbId}`, 'add', 'event'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.eventParameter.getNotAssociatedEvent
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.eventParameterDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.event.getItems
    */
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'eventParameter', association: 'event', table:'B', method: 'getDataB()', request: 'api.event.getItems'}];
            newError.path=['update', `eventParameterDbId:${item.eventParameterDbId}`, 'add', 'event'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.event.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.eventParameterDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'event' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'events', 
     ...
     * - return
     */
    if(lastModelChanged.eventParameter&&
        lastModelChanged.eventParameter[String(item.eventParameterDbId)]&&
        lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations&&
        lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations.eventParameter_eventDbId&&
        (lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations.eventParameter_eventDbId.added ||
         lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations.eventParameter_eventDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations.eventParameter_eventDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('event', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('event', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.eventParameter[String(item.eventParameterDbId)].changedAssociations.eventParameter_eventDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('event', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'event' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'event',
     * but this action happened on the peer relation, identified by 'eventParameter_eventDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'event'.
     * B: the current item internalId is in the addedIds of the updated 'event'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'event' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.event) {
      let oens = Object.entries(lastModelChanged.event);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.eventParameter_eventDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.eventParameter_eventDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.eventParameter_eventDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.eventParameterDbId);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('event', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.eventParameter_eventDbId.added) {
              let idsAdded = entry[1].changedAssociations.eventParameter_eventDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.eventParameterDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.eventType;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('event', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'event' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.event) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.eventType;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.eventType===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.eventType===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.eventType===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.eventType;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.eventType===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('event', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.eventType===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.eventParameterDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnEventRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.eventType) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.eventType);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.eventType];
        handleDisassociateItem('event', associatedItem.eventType);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('event', item.eventType);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('event', item.eventType);
    }
  };
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='EventToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.eventType;
                        let label = undefined;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'EventToAddTransferView-listA-listItem-'+it.eventType}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.eventType===it.eventType) ? 'event — ' + t('modelPanels.associatedRecord', 'Associated record') : 'event '}>
                                <Avatar>
                                  {"event".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'eventType' }>
                                        <Typography 
                                        id={'EventToAddTransferView-listA-listItem-id-'+it.eventType}
                                        variant="body1" display="block" noWrap={true}>{it.eventType}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.eventType===it.eventType) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.eventType===it.eventType) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'EventToAddTransferView-listA-listItem-'+it.eventType+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <EventToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='EventToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.eventType;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'EventToAddTransferView-listB-listItem-'+it.eventType}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'eventType' }>
                                        <Typography 
                                        id={'EventToAddTransferView-listB-listItem-id-'+it.eventType}
                                        variant="body1" display="block" noWrap={true}>{it.eventType}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'EventToAddTransferView-listB-listItem-'+it.eventType+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='EventToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.eventType;
                        let label = undefined;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'EventToAddTransferView-associatedItem-list-listItem-'+associatedItem.eventType}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'event — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"event".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'eventType' }>
                                        <Typography 
                                        id={'EventToAddTransferView-listB-listItem-id-'+associatedItem.eventType}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.eventType}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'EventToAddTransferView-associatedItem-list-listItem-'+associatedItem.eventType+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].eventType===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('event', associatedItem.eventType);
                                    } else {
                                      lidsToRemove.current = [associatedItem.eventType];
                                      handleDisassociateItem('event', associatedItem.eventType);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
EventToAddTransferView.propTypes = {
