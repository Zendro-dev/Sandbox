...
const cancelableCountingPromises = useRef([]);
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.person.getCountItems(graphqlServerUrl, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ImageAttachment', association: 'person', table:'A', method: 'getCount()', request: 'api.person.getCountItems'}];
            newError.path=['add', 'person'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ImageAttachment', association: 'person', table:'A', method: 'getData()', request: 'api.person.getItems'}];
            newError.path=['add', 'person'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
                        ...
                        >
                          <ListItemAvatar>
                            <Tooltip title={ 'Person' }>
                              <Avatar>{"person".slice(0,1)}</Avatar>
                            </Tooltip>
                          </ListItemAvatar>
