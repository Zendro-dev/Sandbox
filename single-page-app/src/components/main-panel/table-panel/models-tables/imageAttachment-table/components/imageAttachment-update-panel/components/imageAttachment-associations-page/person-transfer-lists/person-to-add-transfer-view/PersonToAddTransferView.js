...
import PersonToAddTransferViewToolbar from './components/PersonToAddTransferViewToolbar';
import PersonToAddTransferViewCursorPagination from './components/PersonToAddTransferViewCursorPagination';
...
export default function PersonToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.imageAttachment.getPerson
    */
    let cancelableApiReq = makeCancelable(api.imageAttachment.getPerson(graphqlServerUrl, item.id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ImageAttachment', association: 'person', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.imageAttachment.getPerson'}];
            newError.path=['update', `id:${currentId}`, 'add', 'person'];
      ...
      //error
      .catch((err) => { //error: on api.imageAttachment.getPerson
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.imageAttachment.getNotAssociatedPersonCount
    */
    let cancelableApiReq = makeCancelable(api.imageAttachment.getNotAssociatedPersonCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ImageAttachment', association: 'person', table:'A', method: 'getCount()', request: 'api.imageAttachment.getNotAssociatedPersonCount'}];
            newError.path=['update', `id:${item.id}`, 'add', 'person'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.imageAttachment.getNotAssociatedPersonCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.imageAttachment.getNotAssociatedPerson
    */
    let cancelableApiReq = makeCancelable(api.imageAttachment.getNotAssociatedPerson(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ImageAttachment', association: 'person', table:'A', method: 'getData()', request: 'api.imageAttachment.getNotAssociatedPerson'}];
            newError.path=['update', `id:${item.id}`, 'add', 'person'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.imageAttachment.getNotAssociatedPerson
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.person.getItems
    */
    let cancelableApiReq = makeCancelable(api.person.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ImageAttachment', association: 'person', table:'B', method: 'getDataB()', request: 'api.person.getItems'}];
            newError.path=['update', `id:${item.id}`, 'add', 'person'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.person.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'person' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'people', 
     ...
     * - return
     */
    if(lastModelChanged.ImageAttachment&&
        lastModelChanged.ImageAttachment[String(item.id)]&&
        lastModelChanged.ImageAttachment[String(item.id)].changedAssociations&&
        lastModelChanged.ImageAttachment[String(item.id)].changedAssociations.ImageAttachment_personId&&
        (lastModelChanged.ImageAttachment[String(item.id)].changedAssociations.ImageAttachment_personId.added ||
         lastModelChanged.ImageAttachment[String(item.id)].changedAssociations.ImageAttachment_personId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.ImageAttachment[String(item.id)].changedAssociations.ImageAttachment_personId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('person', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('person', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.ImageAttachment[String(item.id)].changedAssociations.ImageAttachment_personId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('person', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'person' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'Person',
     * but this action happened on the peer relation, identified by 'ImageAttachment_personId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'Person'.
     * B: the current item internalId is in the addedIds of the updated 'Person'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'Person' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.Person) {
      let oens = Object.entries(lastModelChanged.Person);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.ImageAttachment_personId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.ImageAttachment_personId.removed) {
              let idsRemoved = entry[1].changedAssociations.ImageAttachment_personId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.ImageAttachment_personId.added) {
              let idsAdded = entry[1].changedAssociations.ImageAttachment_personId.idsAdded;
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('person', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'Person' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.Person) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('person', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnPersonRow(event, item);
  };
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.id) === -1) {
      ...
      if(associatedItem && checked) {
        ...
        handleDisassociateItem('person', associatedItem.id);
      }
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('person', item.id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <PersonToAddTransferViewToolbar
                title={'People'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='PersonToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='PersonToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'PersonToAddTransferView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.id===it.id) ? 'Person — ' + t('modelPanels.associatedRecord', 'Associated record') : 'Person '}>
                                <Avatar>
                                  {"person".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'PersonToAddTransferView-listA-listItem-id-'+it.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'PersonToAddTransferView-listA-listItem-'+it.id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <PersonToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='PersonToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='PersonToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='PersonToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'PersonToAddTransferView-listB-listItem-'+it.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'PersonToAddTransferView-listB-listItem-id-'+it.id}
                                        variant="body1" display="block" noWrap={true}>{it.id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'PersonToAddTransferView-listB-listItem-'+it.id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='PersonToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        ...
                        return (
                          <ListItem 
                            id={'PersonToAddTransferView-associatedItem-list-listItem-'+associatedItem.id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'Person — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"person".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography 
                                        id={'PersonToAddTransferView-listB-listItem-id-'+associatedItem.id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'PersonToAddTransferView-associatedItem-list-listItem-'+associatedItem.id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      lidsToRemove.current = [];
                                      handleReassociateItem('person', associatedItem.id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.id];
                                      handleDisassociateItem('person', associatedItem.id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='PersonToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
PersonToAddTransferView.propTypes = {
