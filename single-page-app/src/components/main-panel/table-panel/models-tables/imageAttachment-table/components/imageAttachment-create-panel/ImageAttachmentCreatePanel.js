...
export default function ImageAttachmentCreatePanel(props) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    await cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on api.imageAttachment.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  return (
    ...
    <Dialog id='ImageAttachmentCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ImageAttachmentCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' ImageAttachment'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " imageAttachment" }>
            <Fab
              id='ImageAttachmentCreatePanel-fabButton-save' 
              color="secondary" 
...
ImageAttachmentCreatePanel.propTypes = {
