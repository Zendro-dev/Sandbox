...
import ObservationVariablesToRemoveTransferViewToolbar from './components/ObservationVariablesToRemoveTransferViewToolbar';
import ObservationVariablesToRemoveTransferViewCursorPagination from './components/ObservationVariablesToRemoveTransferViewCursorPagination';
...
export default function ObservationVariablesToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyReference.getObservationVariablesCount
    */
    let cancelableApiReq = makeCancelable(api.ontologyReference.getObservationVariablesCount(
      graphqlServerUrl, 
      item.ontologyDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationVariables', method: 'getCount()', request: 'api.ontologyReference.getObservationVariablesCount'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'remove', 'observationVariables'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getObservationVariablesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {observationVariableDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.ontologyReference.getObservationVariables
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.ontologyReference.getObservationVariables(
      graphqlServerUrl, 
      item.ontologyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationVariables', method: 'getData()', request: 'api.ontologyReference.getObservationVariables'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'remove', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getObservationVariables
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.ontologyDbId, search, getCount, configurePagination, onEmptyPage]);
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'observationVariable' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.observationVariable_ontologyDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.observationVariableDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.observationVariableDbId) === -1) {
      lidsToRemove.current.push(item.observationVariableDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.observationVariableDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationVariablesToRemoveTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariablesToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationVariablesToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        let disabled = lidsToRemove.current.find(observationVariableDbId=> observationVariableDbId=== it.observationVariableDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'ObservationVariablesToRemoveTransferView-listA-listItem-'+it.observationVariableDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationVariableDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.observationVariableDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'ObservationVariablesToRemoveTransferView-listA-listItem-'+it.observationVariableDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <ObservationVariablesToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariablesToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariablesToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationVariablesToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationVariableDbId;
                        let label = it.ObservationVariableName;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationVariablesToRemoveTransferView-listB-listItem-'+it.observationVariableDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'ObservationVariablesToRemoveTransferView-listB-listItem-'+it.observationVariableDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ObservationVariablesToRemoveTransferView.propTypes = {
