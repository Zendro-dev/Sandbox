...
import TraitsToRemoveTransferViewToolbar from './components/TraitsToRemoveTransferViewToolbar';
import TraitsToRemoveTransferViewCursorPagination from './components/TraitsToRemoveTransferViewCursorPagination';
...
export default function TraitsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyReference.getTraitsCount
    */
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.getTraitsCount(
      graphqlServerUrl, 
      item.ontologyDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'traits', method: 'getCount()', request: 'api.ontologyReference.getTraitsCount'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'remove', 'traits'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getTraitsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {traitDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.ontologyReference.getTraits
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.getTraits(
      graphqlServerUrl, 
      item.ontologyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'traits', method: 'getData()', request: 'api.ontologyReference.getTraits'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'remove', 'traits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getTraits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.trait.getCountItems
    */
    let api = await loadApi("trait");
    let cancelableApiReq = makeCancelable(api.trait.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', association: 'traits', table:'B', method: 'getCountB()', request: 'api.trait.getCountItems'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'remove', 'traits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.trait.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
        ...
        (response) => {
          ...
          if(response.message === 'ok') {
            ...
            if(response.graphqlErrors) {
              ...
              newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'ontologyReference', association: 'traits', table:'B', method: 'getDataB()', request: 'api.trait.getItems'}];
              newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'remove', 'traits'];
              newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'traits' for this item was updated.
    ...
    if(lastModelChanged.ontologyReference&&
        ...
        lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.trait_ontologyDbId&&
        (lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.trait_ontologyDbId.added ||
         lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.trait_ontologyDbId.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.trait_ontologyDbId.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('traits', idRemoved);
          ...
          let idsAdded = lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.trait_ontologyDbId.idsAdded;
          if(idsAdded) {
            //increment A
    ...
    /*
     * Case 2:
     * The relation 'traits' for this item was updated from the target model (in the peer relation).
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'trait' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.trait_ontologyDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.traitDbId;
    ...
    if(lastModelChanged.trait) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.traitDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.traitDbId) === -1) {
      lidsToRemove.current.push(item.traitDbId);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('traits', item.traitDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.traitDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('traits', item.traitDbId);
    }
  }
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <TraitsToRemoveTransferViewToolbar
                title={'Traits'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TraitsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='TraitsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.traitDbId;
                        let label = it.traitName;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(traitDbId=> traitDbId=== it.traitDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'TraitsToRemoveTransferView-listA-listItem-'+it.traitDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'traitDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.traitDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'TraitsToRemoveTransferView-listA-listItem-'+it.traitDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <TraitsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TraitsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TraitsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='TraitsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.traitDbId;
                        let label = it.traitName;
                        ...
                        return (
                          <ListItem 
                            id={'TraitsToRemoveTransferView-listB-listItem-'+it.traitDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'TraitsToRemoveTransferView-listB-listItem-'+it.traitDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
TraitsToRemoveTransferView.propTypes = {
