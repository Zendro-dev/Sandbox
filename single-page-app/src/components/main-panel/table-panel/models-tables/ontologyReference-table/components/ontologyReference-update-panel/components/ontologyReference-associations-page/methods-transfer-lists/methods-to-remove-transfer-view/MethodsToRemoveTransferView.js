...
import MethodsToRemoveTransferViewToolbar from './components/MethodsToRemoveTransferViewToolbar';
import MethodsToRemoveTransferViewCursorPagination from './components/MethodsToRemoveTransferViewCursorPagination';
...
export default function MethodsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyReference.getMethodsCount
    */
    let cancelableApiReq = makeCancelable(api.ontologyReference.getMethodsCount(
      graphqlServerUrl, 
      item.ontologyDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'methods', method: 'getCount()', request: 'api.ontologyReference.getMethodsCount'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'remove', 'methods'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getMethodsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {methodDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.ontologyReference.getMethods
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.ontologyReference.getMethods(
      graphqlServerUrl, 
      item.ontologyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'methods', method: 'getData()', request: 'api.ontologyReference.getMethods'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'remove', 'methods'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getMethods
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'method' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.method_ontologyDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.methodDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.methodDbId) === -1) {
      lidsToRemove.current.push(item.methodDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.methodDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MethodsToRemoveTransferViewToolbar
                title={'Methods'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='MethodsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.methodDbId;
                        let label = it.methodName;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(methodDbId=> methodDbId=== it.methodDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'MethodsToRemoveTransferView-listA-listItem-'+it.methodDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'methodDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.methodDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'MethodsToRemoveTransferView-listA-listItem-'+it.methodDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <MethodsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='MethodsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.methodDbId;
                        let label = it.methodName;
                        ...
                        return (
                          <ListItem 
                            id={'MethodsToRemoveTransferView-listB-listItem-'+it.methodDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'MethodsToRemoveTransferView-listB-listItem-'+it.methodDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
MethodsToRemoveTransferView.propTypes = {
