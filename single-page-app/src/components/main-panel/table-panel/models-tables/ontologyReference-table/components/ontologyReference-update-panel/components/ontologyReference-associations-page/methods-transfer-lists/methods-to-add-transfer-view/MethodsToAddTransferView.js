...
import MethodsToAddTransferViewToolbar from './components/MethodsToAddTransferViewToolbar';
import MethodsToAddTransferViewCursorPagination from './components/MethodsToAddTransferViewCursorPagination';
...
export default function MethodsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyReference.getNotAssociatedMethodsCount
    */
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.getNotAssociatedMethodsCount(graphqlServerUrl, item.ontologyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyReference', association: 'methods', table:'A', method: 'getCount()', request: 'api.ontologyReference.getNotAssociatedMethodsCount'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'methods'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getNotAssociatedMethodsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyReference.getNotAssociatedMethods
    */
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.getNotAssociatedMethods(
      graphqlServerUrl,
      item.ontologyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', association: 'methods', table:'A', method: 'getData()', request: 'api.ontologyReference.getNotAssociatedMethods'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'methods'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getNotAssociatedMethods
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.method.getCountItems
    */
    let api = await loadApi("method");
    let cancelableApiReq = makeCancelable(api.method.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', association: 'methods', table:'B', method: 'getCountB()', request: 'api.method.getCountItems'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'methods'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.method.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', association: 'methods', table:'B', method: 'getDataB()', request: 'api.method.getItems'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'methods'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'methods' for this item was updated.
    ...
    if(lastModelChanged.ontologyReference&&
        ...
        lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.method_ontologyDbId&&
        (lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.method_ontologyDbId.added ||
         lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.method_ontologyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.method_ontologyDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('methods', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.method_ontologyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'methods' for this item was updated from the target model (in the peer relation).
    ...
    if(lastModelChanged.method) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.methodDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('methods', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.methodDbId) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('methods', item.methodDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.methodDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('methods', item.methodDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MethodsToAddTransferViewToolbar
                title={'Methods'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='MethodsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'MethodsToAddTransferView-listA-listItem-'+it.methodDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'method' }>
                                <Avatar>{"method".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'methodDbId' }>
                                        <Typography 
                                        id={'MethodsToAddTransferView-listA-listItem-id-'+it.methodDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'MethodsToAddTransferView-listA-listItem-'+it.methodDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <MethodsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='MethodsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.methodDbId;
                        let label = it.methodName;
                        ...
                        return (
                          <ListItem 
                            id={'MethodsToAddTransferView-listB-listItem-'+it.methodDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'methodDbId' }>
                                        <Typography 
                                        id={'MethodsToAddTransferView-listB-listItem-id-'+it.methodDbId}
                                        variant="body1" display="block" noWrap={true}>{it.methodDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'MethodsToAddTransferView-listB-listItem-'+it.methodDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
MethodsToAddTransferView.propTypes = {
