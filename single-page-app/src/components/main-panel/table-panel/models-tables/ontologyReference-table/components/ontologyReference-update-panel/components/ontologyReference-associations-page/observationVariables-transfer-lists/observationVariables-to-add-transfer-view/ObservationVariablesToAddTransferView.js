...
import ObservationVariablesToAddTransferViewToolbar from './components/ObservationVariablesToAddTransferViewToolbar';
import ObservationVariablesToAddTransferViewCursorPagination from './components/ObservationVariablesToAddTransferViewCursorPagination';
...
export default function ObservationVariablesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyReference.getNotAssociatedObservationVariablesCount
    */
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.getNotAssociatedObservationVariablesCount(graphqlServerUrl, item.ontologyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyReference', association: 'observationVariables', table:'A', method: 'getCount()', request: 'api.ontologyReference.getNotAssociatedObservationVariablesCount'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'observationVariables'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getNotAssociatedObservationVariablesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.ontologyDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyReference.getNotAssociatedObservationVariables
    */
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.getNotAssociatedObservationVariables(
      graphqlServerUrl,
      item.ontologyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', association: 'observationVariables', table:'A', method: 'getData()', request: 'api.ontologyReference.getNotAssociatedObservationVariables'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getNotAssociatedObservationVariables
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.ontologyDbId, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getCountItems
    */
    let api = await loadApi("observationVariable");
    let cancelableApiReq = makeCancelable(api.observationVariable.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', association: 'observationVariables', table:'B', method: 'getCountB()', request: 'api.observationVariable.getCountItems'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'observationVariables'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.observationVariable.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.ontologyDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', association: 'observationVariables', table:'B', method: 'getDataB()', request: 'api.observationVariable.getItems'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.ontologyDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'observationVariables' for this item was updated.
    ...
    if(lastModelChanged.ontologyReference&&
        ...
        lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations&&
        lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.observationVariable_ontologyDbId&&
        (lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.observationVariable_ontologyDbId.added ||
         lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.observationVariable_ontologyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.observationVariable_ontologyDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('observationVariables', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.ontologyReference[String(item.ontologyDbId)].changedAssociations.observationVariable_ontologyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'observationVariables' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'observationVariable',
     * but this action happened on the peer relation, identified by 'observationVariable_ontologyDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationVariable_ontologyDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationVariable_ontologyDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observationVariable_ontologyDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.ontologyDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.observationVariable_ontologyDbId.added) {
              let idsAdded = entry[1].changedAssociations.observationVariable_ontologyDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.ontologyDbId);
                if(iof !== -1) {
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.observationVariableDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('observationVariables', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.ontologyDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.observationVariableDbId) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('observationVariables', item.observationVariableDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.observationVariableDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('observationVariables', item.observationVariableDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationVariablesToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariablesToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationVariablesToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ObservationVariablesToAddTransferView-listA-listItem-'+it.observationVariableDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'observationVariable' }>
                                <Avatar>{"observationVariable".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationVariableDbId' }>
                                        <Typography 
                                        id={'ObservationVariablesToAddTransferView-listA-listItem-id-'+it.observationVariableDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ObservationVariablesToAddTransferView-listA-listItem-'+it.observationVariableDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ObservationVariablesToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariablesToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariablesToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationVariablesToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationVariableDbId;
                        let label = it.ObservationVariableName;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationVariablesToAddTransferView-listB-listItem-'+it.observationVariableDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationVariableDbId' }>
                                        <Typography 
                                        id={'ObservationVariablesToAddTransferView-listB-listItem-id-'+it.observationVariableDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationVariableDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ObservationVariablesToAddTransferView-listB-listItem-'+it.observationVariableDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ObservationVariablesToAddTransferView.propTypes = {
