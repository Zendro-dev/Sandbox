...
import TraitsToAddTransferViewToolbar from './components/TraitsToAddTransferViewToolbar';
import TraitsToAddTransferViewCursorPagination from './components/TraitsToAddTransferViewCursorPagination';
...
export default function TraitsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyReference.getNotAssociatedTraitsCount
    */
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.getNotAssociatedTraitsCount(graphqlServerUrl, item.ontologyDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyReference', association: 'traits', table:'A', method: 'getCount()', request: 'api.ontologyReference.getNotAssociatedTraitsCount'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'traits'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getNotAssociatedTraitsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyReference.getNotAssociatedTraits
    */
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.getNotAssociatedTraits(
      graphqlServerUrl,
      item.ontologyDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', association: 'traits', table:'A', method: 'getData()', request: 'api.ontologyReference.getNotAssociatedTraits'}];
            newError.path=['update', `ontologyDbId:${item.ontologyDbId}`, 'add', 'traits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getNotAssociatedTraits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
          ...
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('traits', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.traitDbId) === -1) {
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.traitDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <TraitsToAddTransferViewToolbar
                title={'Traits'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TraitsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='TraitsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'TraitsToAddTransferView-listA-listItem-'+it.traitDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'traitDbId' }>
                                        <Typography 
                                        id={'TraitsToAddTransferView-listA-listItem-id-'+it.traitDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'TraitsToAddTransferView-listA-listItem-'+it.traitDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <TraitsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TraitsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TraitsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='TraitsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.traitDbId;
                        let label = it.traitName;
                        ...
                        return (
                          <ListItem 
                            id={'TraitsToAddTransferView-listB-listItem-'+it.traitDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'traitDbId' }>
                                        <Typography 
                                        id={'TraitsToAddTransferView-listB-listItem-id-'+it.traitDbId}
                                        variant="body1" display="block" noWrap={true}>{it.traitDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'TraitsToAddTransferView-listB-listItem-'+it.traitDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
TraitsToAddTransferView.propTypes = {
