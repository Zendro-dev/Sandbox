...
import OntologyReferenceEnhancedTableHead from './components/OntologyReferenceEnhancedTableHead'
import OntologyReferenceEnhancedTableToolbar from './components/OntologyReferenceEnhancedTableToolbar'
import OntologyReferenceUploadFileDialog from './components/OntologyReferenceUploadFileDialog'
import OntologyReferenceCursorPagination from './components/OntologyReferenceCursorPagination'
...
//lazy loading
const OntologyReferenceCreatePanel = lazy(() => import(/* webpackChunkName: "Create-OntologyReference" */ './components/ontologyReference-create-panel/OntologyReferenceCreatePanel'));
const OntologyReferenceUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-OntologyReference" */ './components/ontologyReference-update-panel/OntologyReferenceUpdatePanel'));
const OntologyReferenceDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-OntologyReference" */ './components/ontologyReference-detail-panel/OntologyReferenceDetailPanel'));
const OntologyReferenceDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-OntologyReference" */ './components/OntologyReferenceDeleteConfirmationDialog'));
//Plotly
const OntologyReferencePlotly = lazy(() => import(/* webpackChunkName: "Plotly-OntologyReference" */ '../../../../plots/OntologyReferencePlotly'));
...
export default function OntologyReferenceEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('ontologyDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.ontologyReference.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyReference', method: 'getCount()', request: 'api.ontologyReference.getCountItems'}];
            newError.path=['OntologyReferences'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', method: 'getData()', request: 'api.ontologyReference.getItems'}];
            newError.path=['OntologyReferences'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['ontologyReference']
    &&(Object.keys(changes.lastModelChanged['ontologyReference']).length===1)
    &&(changes.lastModelChanged['ontologyReference'][Object.keys(changes.lastModelChanged['ontologyReference'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set ontologyDbId (internalId)
    variables.ontologyDbId = item.ontologyDbId;

    /*
      API Request: api.ontologyReference.deleteItem
    */
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', method: 'doDelete()', request: 'api.ontologyReference.deleteItem'}];
            newError.path=['OntologyReferences', `ontologyDbId:${item.ontologyDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.ontologyReference.tableTemplate
    */
    let api = await loadApi("ontologyReference");
    let cancelableApiReq = makeCancelable(api.ontologyReference.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyReference', method: 'getCsvTemplate()', request: 'api.ontologyReference.tableTemplate'}];
            newError.path=['OntologyReferences', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateOntologyReference.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "ontologyReference-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <OntologyReferenceEnhancedTableToolbar
                ...
                >
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='OntologyReferenceEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='OntologyReferenceEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <OntologyReferenceEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='OntologyReferenceEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'OntologyReferenceEnhancedTable-row-'+item.ontologyDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.ontologyDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'OntologyReferenceEnhancedTable-row-iconButton-detail-'+item.ontologyDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.ontologyReference&&Array.isArray(permissions.ontologyReference)
                                      &&(permissions.ontologyReference.includes('update') || permissions.ontologyReference.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'OntologyReferenceEnhancedTable-row-iconButton-edit-'+item.ontologyDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.ontologyReference&&Array.isArray(permissions.ontologyReference)
                                      &&(permissions.ontologyReference.includes('delete') || permissions.ontologyReference.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'OntologyReferenceEnhancedTable-row-iconButton-delete-'+item.ontologyDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'ontologyDbId: ' + item.ontologyDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.ontologyDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.ontologyName!==null)?item.ontologyName:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <OntologyReferenceCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(OntologyReferencePlotly)) && (
                      <Suspense fallback={<div />}>
                        <OntologyReferencePlotly />
                      </Suspense>
                    )}
                  </div>
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <OntologyReferenceDeleteConfirmationDialog
            permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <OntologyReferenceUploadFileDialog
...
OntologyReferenceEnhancedTable.propTypes = {
