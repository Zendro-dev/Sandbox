...
export default function AssaysToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedAssaysCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedAssaysCount(graphqlServerUrl, item.fileAttachment_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'fileAttachment', association: 'assays', table:'A', method: 'getCount()', request: 'api.fileAttachment.getNotAssociatedAssaysCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'assays'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedAssaysCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedAssays
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedAssays(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'assays', table:'A', method: 'getData()', request: 'api.fileAttachment.getNotAssociatedAssays'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedAssays
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'assays', table:'B', method: 'getCountB()', request: 'api.assay.getCountItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'assays'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'assays', table:'B', method: 'getDataB()', request: 'api.assay.getItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.fileAttachment&&
        ...
        lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_assay_ids&&
        (lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_assay_ids.added ||
         lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_assay_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_assay_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_assay_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'fileAttachment_assay_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.assay) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.fileAttachment_assay_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.fileAttachment_assay_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.fileAttachment_assay_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.fileAttachment_assay_ids.added) {
              let idsAdded = entry[1].changedAssociations.fileAttachment_assay_ids.idsAdded;
              if(idsAdded) {
