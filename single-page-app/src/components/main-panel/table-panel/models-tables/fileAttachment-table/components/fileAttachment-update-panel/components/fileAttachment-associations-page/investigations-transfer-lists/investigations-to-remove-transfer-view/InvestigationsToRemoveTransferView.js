...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getInvestigationsCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getInvestigationsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'investigations', method: 'getCount()', request: 'api.fileAttachment.getInvestigationsCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'investigations'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getInvestigationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getInvestigations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.fileAttachment.getInvestigations(
      graphqlServerUrl, 
      item.fileAttachment_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'investigations', method: 'getData()', request: 'api.fileAttachment.getInvestigations'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getInvestigations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.fileAttachment_id, search, getCount, configurePagination, onEmptyPage]);
