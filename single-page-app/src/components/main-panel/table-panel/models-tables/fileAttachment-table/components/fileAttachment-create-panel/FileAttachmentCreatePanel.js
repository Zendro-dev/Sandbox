    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'FileAttachment', query: 'addFileAttachment', method: 'doSave()', request: 'api.fileAttachment.createItem'}];
        ...
        //check: addFileAttachment
        let addFileAttachment = response.data.data.addFileAttachment;
        if(addFileAttachment === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'addFileAttachment ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'FileAttachment', query: 'addFileAttachment', method: 'doSave()', request: 'api.fileAttachment.createItem'}];
          newError.path=['FileAttachments', 'add'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: addFileAttachment type
        if(typeof addFileAttachment !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'addFileAttachment ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'FileAttachment', query: 'addFileAttachment', method: 'doSave()', request: 'api.fileAttachment.createItem'}];
          newError.path=['FileAttachments', 'add'];
        ...
        onClose(event, true, addFileAttachment);
        return;
      },
      //rejected
      (err) => {
