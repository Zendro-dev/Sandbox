...
export default function InvestigationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedInvestigationsCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedInvestigationsCount(graphqlServerUrl, item.fileAttachment_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'fileAttachment', association: 'investigations', table:'A', method: 'getCount()', request: 'api.fileAttachment.getNotAssociatedInvestigationsCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'investigations'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedInvestigationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.fileAttachment_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedInvestigations
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedInvestigations(
      graphqlServerUrl,
      item.fileAttachment_id,
      search,
      variables,
      ops,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'investigations', table:'A', method: 'getData()', request: 'api.fileAttachment.getNotAssociatedInvestigations'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedInvestigations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.fileAttachment_id, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'investigations', table:'B', method: 'getCountB()', request: 'api.investigation.getCountItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'investigations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.fileAttachment_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'investigations', table:'B', method: 'getDataB()', request: 'api.investigation.getItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.fileAttachment_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
     ...
     * - return
     */
    if(lastModelChanged.fileAttachment&&
        lastModelChanged.fileAttachment[String(item.fileAttachment_id)]&&
        lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations&&
        lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_investigation_ids&&
        (lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_investigation_ids.added ||
         lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_investigation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_investigation_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_investigation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'fileAttachment_investigation_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.investigation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.fileAttachment_investigation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.fileAttachment_investigation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.fileAttachment_investigation_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.fileAttachment_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.fileAttachment_investigation_ids.added) {
              let idsAdded = entry[1].changedAssociations.fileAttachment_investigation_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.fileAttachment_id);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.fileAttachment_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
