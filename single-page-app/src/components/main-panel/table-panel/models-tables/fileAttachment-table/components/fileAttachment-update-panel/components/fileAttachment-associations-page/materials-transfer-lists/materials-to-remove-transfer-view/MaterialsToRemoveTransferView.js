...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getMaterialsCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getMaterialsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'materials', method: 'getCount()', request: 'api.fileAttachment.getMaterialsCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'materials'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getMaterialsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getMaterials
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.fileAttachment.getMaterials(
      graphqlServerUrl, 
      item.fileAttachment_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'materials', method: 'getData()', request: 'api.fileAttachment.getMaterials'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getMaterials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
