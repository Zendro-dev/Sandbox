...
export default function MaterialsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedMaterialsCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedMaterialsCount(graphqlServerUrl, item.fileAttachment_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'fileAttachment', association: 'materials', table:'A', method: 'getCount()', request: 'api.fileAttachment.getNotAssociatedMaterialsCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'materials'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedMaterialsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedMaterials
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedMaterials(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'materials', table:'A', method: 'getData()', request: 'api.fileAttachment.getNotAssociatedMaterials'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedMaterials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'materials', table:'B', method: 'getCountB()', request: 'api.material.getCountItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'materials'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'materials', table:'B', method: 'getDataB()', request: 'api.material.getItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.fileAttachment&&
        ...
        lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_material_ids&&
        (lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_material_ids.added ||
         lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_material_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_material_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_material_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'fileAttachment_material_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.material) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.fileAttachment_material_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.fileAttachment_material_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.fileAttachment_material_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.fileAttachment_material_ids.added) {
              let idsAdded = entry[1].changedAssociations.fileAttachment_material_ids.idsAdded;
              if(idsAdded) {
