...
export default function FactorsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedFactorsCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedFactorsCount(graphqlServerUrl, item.fileAttachment_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'fileAttachment', association: 'factors', table:'A', method: 'getCount()', request: 'api.fileAttachment.getNotAssociatedFactorsCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'factors'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedFactorsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedFactors
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedFactors(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'factors', table:'A', method: 'getData()', request: 'api.fileAttachment.getNotAssociatedFactors'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'factors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedFactors
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'factors', table:'B', method: 'getCountB()', request: 'api.factor.getCountItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'factors'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'factors', table:'B', method: 'getDataB()', request: 'api.factor.getItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'factors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.fileAttachment&&
        ...
        lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_factor_ids&&
        (lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_factor_ids.added ||
         lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_factor_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_factor_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_factor_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'fileAttachment_factor_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.factor) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.fileAttachment_factor_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.fileAttachment_factor_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.fileAttachment_factor_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.fileAttachment_factor_ids.added) {
              let idsAdded = entry[1].changedAssociations.fileAttachment_factor_ids.idsAdded;
              if(idsAdded) {
