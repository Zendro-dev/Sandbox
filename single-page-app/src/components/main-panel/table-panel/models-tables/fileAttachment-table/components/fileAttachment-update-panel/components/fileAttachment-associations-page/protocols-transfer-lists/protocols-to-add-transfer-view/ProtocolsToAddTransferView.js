...
export default function ProtocolsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedProtocolsCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedProtocolsCount(graphqlServerUrl, item.fileAttachment_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'fileAttachment', association: 'protocols', table:'A', method: 'getCount()', request: 'api.fileAttachment.getNotAssociatedProtocolsCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'protocols'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedProtocolsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getNotAssociatedProtocols
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getNotAssociatedProtocols(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'protocols', table:'A', method: 'getData()', request: 'api.fileAttachment.getNotAssociatedProtocols'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'protocols'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getNotAssociatedProtocols
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'protocols', table:'B', method: 'getCountB()', request: 'api.protocol.getCountItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'protocols'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', association: 'protocols', table:'B', method: 'getDataB()', request: 'api.protocol.getItems'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'add', 'protocols'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.fileAttachment&&
        ...
        lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_protocol_ids&&
        (lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_protocol_ids.added ||
         lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_protocol_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_protocol_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.fileAttachment[String(item.fileAttachment_id)].changedAssociations.fileAttachment_protocol_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'fileAttachment_protocol_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.protocol) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.fileAttachment_protocol_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.fileAttachment_protocol_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.fileAttachment_protocol_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.fileAttachment_protocol_ids.added) {
              let idsAdded = entry[1].changedAssociations.fileAttachment_protocol_ids.idsAdded;
              if(idsAdded) {
