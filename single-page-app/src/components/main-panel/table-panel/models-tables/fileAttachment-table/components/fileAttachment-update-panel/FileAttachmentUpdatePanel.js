...
export default function FileAttachmentUpdatePanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged.fileAttachment[String(item.fileAttachment_id)]) {

        //updated item
        if(lastModelChanged.fileAttachment[String(item.fileAttachment_id)].op === "update"&&
            lastModelChanged.fileAttachment[String(item.fileAttachment_id)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.fileAttachment[String(item.fileAttachment_id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, item.fileAttachment_id]);
  ...
  function getInitialValues() {
    ...
    initialValues.fileAttachment_id = item.fileAttachment_id;
    ...
    initialValues.isImage = item.isImage;
    initialValues.smallThumbnailURL = item.smallThumbnailURL;
    initialValues.bigThumbnailURL = item.bigThumbnailURL;
  ...
  initialValueOkStates.fileAttachment_id = (item.fileAttachment_id!==null ? 1 : 0);
  ...
  initialValueOkStates.isImage = (item.isImage!==null ? 1 : 0);
  initialValueOkStates.smallThumbnailURL = (item.smallThumbnailURL!==null ? 1 : 0);
  initialValueOkStates.bigThumbnailURL = (item.bigThumbnailURL!==null ? 1 : 0);
  ...
  function areThereChangedFields() {
    if(values.current.fileAttachment_id !== item.fileAttachment_id) { return true;}
    if(values.current.fileName !== item.fileName) { return true;}
    if(values.current.mimeType !== item.mimeType) { return true;}
    if(Number(values.current.fileSizeKb) !== Number(item.fileSizeKb)) { return true;}
    if(values.current.fileURL !== item.fileURL) { return true;}
    if(values.current.isImage !== item.isImage) { return true;}
    if(values.current.smallThumbnailURL !== item.smallThumbnailURL) { return true;}
    if(values.current.bigThumbnailURL !== item.bigThumbnailURL) { return true;}
  ...
  function setAddRemoveManyAssays(variables) {
    ...
    if(!changedAssociations.current.fileAttachment_assay_ids) changedAssociations.current.fileAttachment_assay_ids = {};
    ...
    if(assaysIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_assay_ids.added = true;
      if(changedAssociations.current.fileAttachment_assay_ids.idsAdded){
        assaysIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_assay_ids.idsAdded.includes(it)) changedAssociations.current.fileAttachment_assay_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.fileAttachment_assay_ids.idsAdded = [...assaysIdsToAdd.current];
      }
    }
    /**
    ...
    if(assaysIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_assay_ids.removed = true;
      if(changedAssociations.current.fileAttachment_assay_ids.idsRemoved){
        assaysIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_assay_ids.idsRemoved.includes(it)) changedAssociations.current.fileAttachment_assay_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.fileAttachment_assay_ids.idsRemoved = [...assaysIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyAssayResults(variables) {
    //data to notify changes
    if(!changedAssociations.current.fileAttachment_assayResult_ids) changedAssociations.current.fileAttachment_assayResult_ids = {};
    ...
    if(assayResultsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_assayResult_ids.added = true;
      if(changedAssociations.current.fileAttachment_assayResult_ids.idsAdded){
        assayResultsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_assayResult_ids.idsAdded.includes(it)) changedAssociations.current.fileAttachment_assayResult_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.fileAttachment_assayResult_ids.idsAdded = [...assayResultsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(assayResultsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_assayResult_ids.removed = true;
      if(changedAssociations.current.fileAttachment_assayResult_ids.idsRemoved){
        assayResultsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_assayResult_ids.idsRemoved.includes(it)) changedAssociations.current.fileAttachment_assayResult_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.fileAttachment_assayResult_ids.idsRemoved = [...assayResultsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyFactors(variables) {
    //data to notify changes
    if(!changedAssociations.current.fileAttachment_factor_ids) changedAssociations.current.fileAttachment_factor_ids = {};
    ...
    if(factorsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_factor_ids.added = true;
      if(changedAssociations.current.fileAttachment_factor_ids.idsAdded){
        factorsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_factor_ids.idsAdded.includes(it)) changedAssociations.current.fileAttachment_factor_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.fileAttachment_factor_ids.idsAdded = [...factorsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(factorsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_factor_ids.removed = true;
      if(changedAssociations.current.fileAttachment_factor_ids.idsRemoved){
        factorsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_factor_ids.idsRemoved.includes(it)) changedAssociations.current.fileAttachment_factor_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.fileAttachment_factor_ids.idsRemoved = [...factorsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyInvestigations(variables) {
    //data to notify changes
    if(!changedAssociations.current.fileAttachment_investigation_ids) changedAssociations.current.fileAttachment_investigation_ids = {};
    ...
    if(investigationsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_investigation_ids.added = true;
      if(changedAssociations.current.fileAttachment_investigation_ids.idsAdded){
        investigationsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_investigation_ids.idsAdded.includes(it)) changedAssociations.current.fileAttachment_investigation_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.fileAttachment_investigation_ids.idsAdded = [...investigationsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(investigationsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_investigation_ids.removed = true;
      if(changedAssociations.current.fileAttachment_investigation_ids.idsRemoved){
        investigationsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_investigation_ids.idsRemoved.includes(it)) changedAssociations.current.fileAttachment_investigation_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.fileAttachment_investigation_ids.idsRemoved = [...investigationsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyMaterials(variables) {
    //data to notify changes
    if(!changedAssociations.current.fileAttachment_material_ids) changedAssociations.current.fileAttachment_material_ids = {};
    ...
    if(materialsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_material_ids.added = true;
      if(changedAssociations.current.fileAttachment_material_ids.idsAdded){
        materialsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_material_ids.idsAdded.includes(it)) changedAssociations.current.fileAttachment_material_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.fileAttachment_material_ids.idsAdded = [...materialsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(materialsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_material_ids.removed = true;
      if(changedAssociations.current.fileAttachment_material_ids.idsRemoved){
        materialsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_material_ids.idsRemoved.includes(it)) changedAssociations.current.fileAttachment_material_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.fileAttachment_material_ids.idsRemoved = [...materialsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyProtocols(variables) {
    //data to notify changes
    if(!changedAssociations.current.fileAttachment_protocol_ids) changedAssociations.current.fileAttachment_protocol_ids = {};
    ...
    if(protocolsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_protocol_ids.added = true;
      if(changedAssociations.current.fileAttachment_protocol_ids.idsAdded){
        protocolsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_protocol_ids.idsAdded.includes(it)) changedAssociations.current.fileAttachment_protocol_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.fileAttachment_protocol_ids.idsAdded = [...protocolsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(protocolsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_protocol_ids.removed = true;
      if(changedAssociations.current.fileAttachment_protocol_ids.idsRemoved){
        protocolsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_protocol_ids.idsRemoved.includes(it)) changedAssociations.current.fileAttachment_protocol_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.fileAttachment_protocol_ids.idsRemoved = [...protocolsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyStudies(variables) {
    //data to notify changes
    if(!changedAssociations.current.fileAttachment_study_ids) changedAssociations.current.fileAttachment_study_ids = {};
    ...
    if(studiesIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_study_ids.added = true;
      if(changedAssociations.current.fileAttachment_study_ids.idsAdded){
        studiesIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_study_ids.idsAdded.includes(it)) changedAssociations.current.fileAttachment_study_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.fileAttachment_study_ids.idsAdded = [...studiesIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(studiesIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.fileAttachment_study_ids.removed = true;
      if(changedAssociations.current.fileAttachment_study_ids.idsRemoved){
        studiesIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.fileAttachment_study_ids.idsRemoved.includes(it)) changedAssociations.current.fileAttachment_study_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.fileAttachment_study_ids.idsRemoved = [...studiesIdsToRemove.current];
      }
    }
...
function setAjvErrors(err) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.fileAttachment.updateItem
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.updateItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'fileAttachment', method: 'doSave()', request: 'api.fileAttachment.updateItem'}];
            newError.path=['FileAttachments', `fileAttachment_id:${item.fileAttachment_id}`, 'update'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.updateItem
        if(err.isCanceled) {
          return;
        } else {
          //set ajv errors
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='FileAttachmentUpdatePanel-button-cancel'
              edge="start" 
              color="inherit" 
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": FileAttachment | fileAttachment_id: " + item.fileAttachment_id}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " fileAttachment" }>
              <Fab
                id='FileAttachmentUpdatePanel-fabButton-save' 
                color="secondary" 
...
FileAttachmentUpdatePanel.propTypes = {
