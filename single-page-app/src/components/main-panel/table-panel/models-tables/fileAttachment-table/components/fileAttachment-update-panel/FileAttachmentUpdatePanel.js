    ...
    cancelableApiReq
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.path=['FileAttachments', `id:${item.id}`, 'update'];
        ...
        if(typeof updateFileAttachment !== 'object') {
          ...
          let withDetails=true;
          variant.current='error';
          newError.message = 'FileAttachment ' + t('modelPanels.errors.data.e4', ' received, does not have the expected format.');
          newError.locations=[{model: 'FileAttachment', query: 'updateFileAttachment', method: 'doSave()', request: 'api.fileAttachment.updateItem'}];
          newError.path=['FileAttachments', `id:${item.id}`, 'update'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        onClose(event, true, updateFileAttachment);
        return;
      },
      //rejected
      (err) => {
      ...
      <AppBar>
        <Toolbar>
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": FileAttachment | id: " + item.id}
          </Typography>
