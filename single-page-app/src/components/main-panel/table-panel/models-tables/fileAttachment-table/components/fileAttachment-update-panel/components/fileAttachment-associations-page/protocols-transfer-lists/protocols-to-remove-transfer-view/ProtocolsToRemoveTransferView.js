...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getProtocolsCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getProtocolsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'protocols', method: 'getCount()', request: 'api.fileAttachment.getProtocolsCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'protocols'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getProtocolsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getProtocols
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.fileAttachment.getProtocols(
      graphqlServerUrl, 
      item.fileAttachment_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'protocols', method: 'getData()', request: 'api.fileAttachment.getProtocols'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'protocols'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getProtocols
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
