...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getAssayResultsCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getAssayResultsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'assayResults', method: 'getCount()', request: 'api.fileAttachment.getAssayResultsCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'assayResults'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getAssayResultsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getAssayResults
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.fileAttachment.getAssayResults(
      graphqlServerUrl, 
      item.fileAttachment_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'assayResults', method: 'getData()', request: 'api.fileAttachment.getAssayResults'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'assayResults'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getAssayResults
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
