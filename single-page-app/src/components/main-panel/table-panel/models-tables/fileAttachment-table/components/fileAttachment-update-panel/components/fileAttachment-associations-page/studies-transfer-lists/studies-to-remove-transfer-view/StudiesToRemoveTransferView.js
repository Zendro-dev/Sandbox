...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getStudiesCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'studies', method: 'getCount()', request: 'api.fileAttachment.getStudiesCount'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getStudies
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.fileAttachment.getStudies(
      graphqlServerUrl, 
      item.fileAttachment_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'studies', method: 'getData()', request: 'api.fileAttachment.getStudies'}];
            newError.path=['update', `fileAttachment_id:${item.fileAttachment_id}`, 'remove', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
