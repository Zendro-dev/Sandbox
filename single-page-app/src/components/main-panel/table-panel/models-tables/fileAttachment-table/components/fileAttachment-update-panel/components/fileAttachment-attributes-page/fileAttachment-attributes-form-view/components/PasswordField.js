...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['FileAttachment'][String(itemId['id'])]) {

        //updated password
        if(lastModelChanged['FileAttachment'][String(itemId['id'])].op === "updatePassword"&&
            lastModelChanged['FileAttachment'][String(itemId['id'])].newItem) {
              //update password value
              setValue(lastModelChanged['FileAttachment'][String(itemId['id'])].newItem[itemKey]);
        }
    }//end: Case 1
  ...
  /**
   ...
   async function doSave() {
    ...
    /*
      API Request: updateFileAttachment
    */
    return await api.fileAttachment.updateItem(graphqlServerUrl, variables)
      .then(
      //resolved
      (response) => {
        //check: response data
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'FileAttachment', query: 'updateFileAttachment', method: 'doSave()', request: 'api.fileAttachment.updateItem'}];
          newError.path=['FileAttachments', itemId, 'update', 'change password'];
        ...
        //check: updateFileAttachment
        let updateFileAttachment = response.data.data.updateFileAttachment;
        if(updateFileAttachment === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'updateFileAttachment ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'FileAttachment', query: 'updateFileAttachment', method: 'doSave()', request: 'api.fileAttachment.updateItem'}];
          newError.path=['FileAttachments', itemId, 'update', 'change password'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: updateFileAttachment type
        if(typeof updateFileAttachment !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'updateFileAttachment ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'FileAttachment', query: 'updateFileAttachment', method: 'doSave()', request: 'api.fileAttachment.updateItem'}];
          newError.path=['FileAttachments', itemId, 'update', 'change password'];
  ...
  return (
    <div>
      <Grid container justify='flex-start' alignItems='center' spacing={0}>
        <Grid item xs={12}>
          <form noValidate autoComplete="off">
            <TextField
              id={'PasswordField-FileAttachment-'+name}
              label={label}
              value={value}
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'PasswordField-FileAttachment-button-showPassword'+name}
                    ...
                    {(valueOk!==undefined&&valueOk===1) 
                      ...
                      <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                        <Typography id={'PasswordField-FileAttachment-exists-'+name} variant="caption" color="primary">
                          &#8707;
                        </Typography>
                      </Tooltip>
                      : 
                      <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                        <Typography id={'PasswordField-FileAttachment-notExists-'+name} variant="caption" color="textSecondary">
                          &#8708;
                        </Typography>
                      </Tooltip>
                    }
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'PasswordField-FileAttachment-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-FileAttachment-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-FileAttachment-ChangePasswordDialog-textField-newPassword2-'+name}
