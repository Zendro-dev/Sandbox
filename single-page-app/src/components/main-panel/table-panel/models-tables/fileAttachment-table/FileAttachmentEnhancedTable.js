...
export default function FileAttachmentEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('fileAttachment_id');
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'fileAttachment', method: 'getCount()', request: 'api.fileAttachment.getCountItems'}];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', method: 'getData()', request: 'api.fileAttachment.getItems'}];
            newError.path=['FileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['fileAttachment']
    &&(Object.keys(changes.lastModelChanged['fileAttachment']).length===1)
    &&(changes.lastModelChanged['fileAttachment'][Object.keys(changes.lastModelChanged['fileAttachment'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set fileAttachment_id (internalId)
    variables.fileAttachment_id = item.fileAttachment_id;

    /*
      API Request: api.fileAttachment.deleteItem
    */
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', method: 'doDelete()', request: 'api.fileAttachment.deleteItem'}];
            newError.path=['FileAttachments', `fileAttachment_id:${item.fileAttachment_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.fileAttachment.tableTemplate
    */
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'fileAttachment', method: 'getCsvTemplate()', request: 'api.fileAttachment.tableTemplate'}];
                          ...
                          <TableBody id='FileAttachmentEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'FileAttachmentEnhancedTable-row-'+item.fileAttachment_id}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.fileAttachment_id}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'FileAttachmentEnhancedTable-row-iconButton-detail-'+item.fileAttachment_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'FileAttachmentEnhancedTable-row-iconButton-edit-'+item.fileAttachment_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'FileAttachmentEnhancedTable-row-iconButton-delete-'+item.fileAttachment_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'fileAttachment_id: ' + item.fileAttachment_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.fileAttachment_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.isImage!==null)?item.isImage:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.smallThumbnailURL!==null)?item.smallThumbnailURL:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.bigThumbnailURL!==null)?item.bigThumbnailURL:'')}
                                    </TableCell>
