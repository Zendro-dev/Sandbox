...
export default function ArrayField(props) {
  ...
  return (
    <Grid container justify='flex-start' alignItems='center' spacing={0}>
      <Grid item xs={12}>
        <TextField
          id={'ArrayField-SPARefactor-'+name}
              ...
              <InputAdornment position="end">
                {(valueOk!==undefined&&valueOk===1) 
                  ...
                  <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                    <Typography id={'ArrayField-SPARefactor-exists-'+name} variant="caption" color="primary">
                      &#8707;
                    </Typography>
                  </Tooltip>
                  : 
                  <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                    <Typography id={'ArrayField-SPARefactor-notExists-'+name} variant="caption" color="textSecondary">
                      &#8708;
                    </Typography>
                  </Tooltip>
                }
      ...
      {(valueAjv !== undefined && valueAjv.errors.length > 0) && (
        <Grid item id={'ArrayField-SPARefactor-ajvError-'+name} xs={12}>
