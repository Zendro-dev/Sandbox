...
import SPARefactorEnhancedTableHead from './components/SPARefactorEnhancedTableHead'
import SPARefactorEnhancedTableToolbar from './components/SPARefactorEnhancedTableToolbar'
import SPARefactorUploadFileDialog from './components/SPARefactorUploadFileDialog'
import SPARefactorCursorPagination from './components/SPARefactorCursorPagination'
...
//lazy loading
const SPARefactorCreatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Create-SPARefactor" */ './components/sPARefactor-create-panel/SPARefactorCreatePanel')));
...
export default function SPARefactorEnhancedTable(props) {
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.sPARefactor.getCountItems
    */
    let api = await loadApi("sPARefactor");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.sPARefactor.getCountItems(graphqlServerUrl, search));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'SPARefactor', method: 'getCount()', request: 'api.sPARefactor.getCountItems'}];
            newError.path=['SPARefactors'];
      ...
      //error
      .catch((err) => { //error: on api.sPARefactor.getCountItems
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.sPARefactor.getItems
    */
    let api = await loadApi("sPARefactor");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.sPARefactor.getItems(
      graphqlServerUrl,
      search,
      orderBy,
      order,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'SPARefactor', method: 'getData()', request: 'api.sPARefactor.getItems'}];
            newError.path=['SPARefactors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.sPARefactor.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['SPARefactor']
    &&(Object.keys(changes.lastModelChanged['SPARefactor']).length===1)
    &&(changes.lastModelChanged['SPARefactor'][Object.keys(changes.lastModelChanged['SPARefactor'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.sPARefactor.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      //error
      .catch((err) => { //error: on api.sPARefactor.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.sPARefactor.tableTemplate
    */
    let api = await loadApi("sPARefactor");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.sPARefactor.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'SPARefactor', method: 'getCsvTemplate()', request: 'api.sPARefactor.tableTemplate'}];
            newError.path=['SPARefactors', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateSPARefactor.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "sPARefactor-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.sPARefactor.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
    ...
    <div className={classes.root}>
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='SPARefactorEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='SPARefactorEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <SPARefactorEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='SPARefactorEnhancedTable-tableBody'>
                            {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.sPARefactor&&Array.isArray(permissions.sPARefactor)
                                      &&(permissions.sPARefactor.includes('update') || permissions.sPARefactor.includes('*')))
                                      &&(
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.sPARefactor&&Array.isArray(permissions.sPARefactor)
                                      &&(permissions.sPARefactor.includes('delete') || permissions.sPARefactor.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.float!==null)?item.float:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.date!==null)?moment(item.date, "YYYY-MM-DD").format("YYYY-MM-DD"):'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.time!==null)?moment(item.time, "HH:mm:ss.SSSZ").format("HH:mm:ss.SSSZ"):'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}><ErrorBoundary showMessage={true} belowToolbar={true} dialog={true} open={deleteConfirmationDialogOpen} handleClose={handleDeleteConfirmationReject}>
          <SPARefactorDeleteConfirmationDialog
            permissions={permissions}
...
SPARefactorEnhancedTable.propTypes = {
