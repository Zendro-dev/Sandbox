  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.capital.getCountItems(graphqlServerUrl, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'country', association: 'unique_capital', table:'A', method: 'getCount()', request: 'api.capital.getCountItems'}];
            newError.path=['add', 'unique_capital'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'unique_capital', table:'A', method: 'getData()', request: 'api.capital.getItems'}];
            newError.path=['add', 'unique_capital'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.capital_id) === -1) {
                        ...
                        >
                          <ListItemAvatar>
                            <Tooltip title={ 'capital' }>
                              <Avatar>{"capital".slice(0,1)}</Avatar>
                            </Tooltip>
                          </ListItemAvatar>
                                ...
                                <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                  <Grid item>
                                    <Tooltip title={ 'capital_id' }>
                                      <Typography variant="body1" display="block" noWrap={true}>{it.capital_id}</Typography>
                                    </Tooltip>
                                  </Grid>
                                  {/*Key icon*/}
                                  <Grid item>
