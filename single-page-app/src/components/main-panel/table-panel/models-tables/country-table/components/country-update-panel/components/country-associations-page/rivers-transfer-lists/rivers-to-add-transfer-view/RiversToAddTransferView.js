...
import RiversToAddTransferViewToolbar from './components/RiversToAddTransferViewToolbar';
import RiversToAddTransferViewCursorPagination from './components/RiversToAddTransferViewCursorPagination';
...
export default function RiversToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"river_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.country.getNotAssociatedRiversCount
    */
    ...
    let cancelableApiReq = makeCancelable(api.country.getNotAssociatedRiversCount(graphqlServerUrl, item.country_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'country', association: 'rivers', table:'A', method: 'getCount()', request: 'api.country.getNotAssociatedRiversCount'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'rivers'];
      ...
      //error
      .catch((err) => { //error: on api.country.getNotAssociatedRiversCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.country.getNotAssociatedRivers
    */
    let api = await loadApi("country");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.country.getNotAssociatedRivers(
      graphqlServerUrl,
      item.country_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'rivers', table:'A', method: 'getData()', request: 'api.country.getNotAssociatedRivers'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'rivers'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.country.getNotAssociatedRivers
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.country_id, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.river.getCountItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.river.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'rivers', table:'B', method: 'getCountB()', request: 'api.river.getCountItems'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'rivers'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsD.current.push(newError);
            console.log("Error: ", newError);
    ...
    //error
    .catch((err) => { //error: on api.river.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.country_id, searchB]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.river.getItems
    */
    let api = await loadApi("river");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.river.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'rivers', table:'B', method: 'getDataB()', request: 'api.river.getItems'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'rivers'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsB.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.river.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.country_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'rivers' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'rivers', 
    ...
    if(lastModelChanged.country&&
        ...
        lastModelChanged.country[String(item.country_id)].changedAssociations.river_country_ids&&
        (lastModelChanged.country[String(item.country_id)].changedAssociations.river_country_ids.added ||
         lastModelChanged.country[String(item.country_id)].changedAssociations.river_country_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.country[String(item.country_id)].changedAssociations.river_country_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('rivers', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.country[String(item.country_id)].changedAssociations.river_country_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'rivers' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'river',
     * but this action happened on the peer relation, identified by 'river_country_ids'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'river'.
     * B: the current item internalId is in the addedIds of the updated 'river'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'river' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.river) {
      let oens = Object.entries(lastModelChanged.river);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.river_country_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.river_country_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.river_country_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.river_country_ids.added) {
              let idsAdded = entry[1].changedAssociations.river_country_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.country_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.river_id;
    ...
    /*
     * Case 3: 
     * The attributes of some 'river' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.river) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.river_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.river_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.river_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.river_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.river_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.river_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('rivers', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.country_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnRiverRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.river_id) === -1) {
      lidsToAdd.current.push(item.river_id);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('rivers', item.river_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.river_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('rivers', item.river_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <RiversToAddTransferViewToolbar
                title={'Rivers'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='RiversToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='RiversToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'RiversToAddTransferView-listA-listItem-'+it.river_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'river_id' }>
                                        <Typography 
                                        id={'RiversToAddTransferView-listA-listItem-id-'+it.river_id}
                                        variant="body1" display="block" noWrap={true}>{it.river_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'RiversToAddTransferView-listA-listItem-'+it.river_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <RiversToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='RiversToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='RiversToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='RiversToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.river_id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'RiversToAddTransferView-listB-listItem-'+it.river_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'river_id' }>
                                        <Typography 
                                        id={'RiversToAddTransferView-listB-listItem-id-'+it.river_id}
                                        variant="body1" display="block" noWrap={true}>{it.river_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'RiversToAddTransferView-listB-listItem-'+it.river_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
RiversToAddTransferView.propTypes = {
