  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.continent.getCountItems(graphqlServerUrl, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'country', association: 'continent', table:'A', method: 'getCount()', request: 'api.continent.getCountItems'}];
            newError.path=['add', 'continent'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'continent', table:'A', method: 'getData()', request: 'api.continent.getItems'}];
            newError.path=['add', 'continent'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.continent_id) === -1) {
                  ...
                  dense component="div" role="list" >
                    {items.map(it => {
                      ...
                      return (
                        ...
                        >
                          <ListItemAvatar>
                            <Tooltip title={ 'continent' }>
                              <Avatar>{"continent".slice(0,1)}</Avatar>
                            </Tooltip>
                          </ListItemAvatar>
                                ...
                                <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                  <Grid item>
                                    <Tooltip title={ 'continent_id' }>
                                      <Typography variant="body1" display="block" noWrap={true}>{it.continent_id}</Typography>
                                    </Tooltip>
                                  </Grid>
                                  {/*Key icon*/}
                                  <Grid item>
