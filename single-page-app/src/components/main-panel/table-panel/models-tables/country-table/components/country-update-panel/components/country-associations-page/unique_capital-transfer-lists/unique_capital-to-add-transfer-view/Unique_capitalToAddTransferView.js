...
import UniqueCapitalToAddTransferViewToolbar from './components/Unique_capitalToAddTransferViewToolbar';
import UniqueCapitalToAddTransferViewCursorPagination from './components/Unique_capitalToAddTransferViewCursorPagination';
...
export default function UniqueCapitalToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.country_id;

    /*
      API Request: api.country.getUnique_capital
    */
    ...
    let cancelableApiReq = makeCancelable(api.country.getUnique_capital(graphqlServerUrl, item.country_id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'country', association: 'unique_capital', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.country.getUnique_capital'}];
            newError.path=['update', `country_id:${currentId}`, 'add', 'unique_capital'];
      ...
      //error
      .catch((err) => { //error: on api.country.getUnique_capital
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.country_id]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"capital_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.country.getNotAssociatedUnique_capitalCount
    */
    let api = await loadApi("country");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.country.getNotAssociatedUnique_capitalCount(graphqlServerUrl, item.country_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'unique_capital', table:'A', method: 'getCount()', request: 'api.country.getNotAssociatedUnique_capitalCount'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'unique_capital'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsC.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.country.getNotAssociatedUnique_capitalCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.country_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.country.getNotAssociatedUnique_capital
    */
    let api = await loadApi("country");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.country.getNotAssociatedUnique_capital(
      graphqlServerUrl,
      item.country_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'unique_capital', table:'A', method: 'getData()', request: 'api.country.getNotAssociatedUnique_capital'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'unique_capital'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.country.getNotAssociatedUnique_capital
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.country_id, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.capital.getItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.capital.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'unique_capital', table:'B', method: 'getDataB()', request: 'api.capital.getItems'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'unique_capital'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsB.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.capital.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.country_id, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'unique_capital' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'capitals', 
    ...
    if(lastModelChanged.country&&
        ...
        lastModelChanged.country[String(item.country_id)].changedAssociations&&
        lastModelChanged.country[String(item.country_id)].changedAssociations.capital_country_id&&
        (lastModelChanged.country[String(item.country_id)].changedAssociations.capital_country_id.added ||
         lastModelChanged.country[String(item.country_id)].changedAssociations.capital_country_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.country[String(item.country_id)].changedAssociations.capital_country_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('unique_capital', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('unique_capital', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.country[String(item.country_id)].changedAssociations.capital_country_id.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('unique_capital', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'unique_capital' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'capital',
     * but this action happened on the peer relation, identified by 'capital_country_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'capital'.
     * B: the current item internalId is in the addedIds of the updated 'capital'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'capital' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.capital) {
      let oens = Object.entries(lastModelChanged.capital);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.capital_country_id) {
            ...
            if(entry[1].changedAssociations.capital_country_id.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.country_id);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('unique_capital', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.capital_country_id.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.country_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.capital_id;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('unique_capital', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'capital' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.capital) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.capital_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.capital_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.capital_id===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.capital_id===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.capital_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.capital_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('unique_capital', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.capital_id===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.country_id, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnCapitalRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.capital_id) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.capital_id);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.capital_id];
        handleDisassociateItem('unique_capital', associatedItem.capital_id);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('unique_capital', item.capital_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('unique_capital', item.capital_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <UniqueCapitalToAddTransferViewToolbar
                title={'Capitals'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UniqueCapitalToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='UniqueCapitalToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.capital_id;
                        let label = undefined;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'UniqueCapitalToAddTransferView-listA-listItem-'+it.capital_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.capital_id===it.capital_id) ? 'capital — ' + t('modelPanels.associatedRecord', 'Associated record') : 'capital '}>
                                <Avatar>
                                  {"capital".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'capital_id' }>
                                        <Typography 
                                        id={'UniqueCapitalToAddTransferView-listA-listItem-id-'+it.capital_id}
                                        variant="body1" display="block" noWrap={true}>{it.capital_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.capital_id===it.capital_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.capital_id===it.capital_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'UniqueCapitalToAddTransferView-listA-listItem-'+it.capital_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <UniqueCapitalToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UniqueCapitalToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UniqueCapitalToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='UniqueCapitalToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.capital_id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'UniqueCapitalToAddTransferView-listB-listItem-'+it.capital_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'capital_id' }>
                                        <Typography 
                                        id={'UniqueCapitalToAddTransferView-listB-listItem-id-'+it.capital_id}
                                        variant="body1" display="block" noWrap={true}>{it.capital_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'UniqueCapitalToAddTransferView-listB-listItem-'+it.capital_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='UniqueCapitalToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.capital_id;
                        let label = undefined;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'UniqueCapitalToAddTransferView-associatedItem-list-listItem-'+associatedItem.capital_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'capital — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"capital".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'capital_id' }>
                                        <Typography 
                                        id={'UniqueCapitalToAddTransferView-listB-listItem-id-'+associatedItem.capital_id}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.capital_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'UniqueCapitalToAddTransferView-associatedItem-list-listItem-'+associatedItem.capital_id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].capital_id===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('unique_capital', associatedItem.capital_id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.capital_id];
                                      handleDisassociateItem('unique_capital', associatedItem.capital_id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='UniqueCapitalToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
UniqueCapitalToAddTransferView.propTypes = {
