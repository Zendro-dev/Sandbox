...
import ContinentToAddTransferViewToolbar from './components/ContinentToAddTransferViewToolbar';
import ContinentToAddTransferViewCursorPagination from './components/ContinentToAddTransferViewCursorPagination';
...
export default function ContinentToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.country.getContinent
    */
    ...
    let cancelableApiReq = makeCancelable(api.country.getContinent(graphqlServerUrl, item.country_id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'country', association: 'continent', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.country.getContinent'}];
            newError.path=['update', `country_id:${currentId}`, 'add', 'continent'];
      ...
      //error
      .catch((err) => { //error: on api.country.getContinent
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"continent_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.country.getNotAssociatedContinentCount
    */
    let api = await loadApi("country");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.country.getNotAssociatedContinentCount(graphqlServerUrl, item.country_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'continent', table:'A', method: 'getCount()', request: 'api.country.getNotAssociatedContinentCount'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'continent'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsC.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.country.getNotAssociatedContinentCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.country.getNotAssociatedContinent
    */
    let api = await loadApi("country");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.country.getNotAssociatedContinent(
      graphqlServerUrl,
      item.country_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'continent', table:'A', method: 'getData()', request: 'api.country.getNotAssociatedContinent'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'continent'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.country.getNotAssociatedContinent
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.continent.getItems
    */
    let api = await loadApi("continent");
    if(!api) {
      let newError = {};
      let withDetails=true;
    ...
    let cancelableApiReq = makeCancelable(api.continent.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', association: 'continent', table:'B', method: 'getDataB()', request: 'api.continent.getItems'}];
            newError.path=['update', `country_id:${item.country_id}`, 'add', 'continent'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsB.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.continent.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'continent' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'continents', 
    ...
    if(lastModelChanged.country&&
        ...
        lastModelChanged.country[String(item.country_id)].changedAssociations.country_continent_id&&
        (lastModelChanged.country[String(item.country_id)].changedAssociations.country_continent_id.added ||
         lastModelChanged.country[String(item.country_id)].changedAssociations.country_continent_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.country[String(item.country_id)].changedAssociations.country_continent_id.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('continent', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('continent', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.country[String(item.country_id)].changedAssociations.country_continent_id.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('continent', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'continent' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'continent',
     * but this action happened on the peer relation, identified by 'country_continent_id'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'continent'.
     * B: the current item internalId is in the addedIds of the updated 'continent'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'continent' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.continent) {
      let oens = Object.entries(lastModelChanged.continent);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.country_continent_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.country_continent_id.removed) {
              let idsRemoved = entry[1].changedAssociations.country_continent_id.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.country_continent_id.added) {
              let idsAdded = entry[1].changedAssociations.country_continent_id.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.country_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.continent_id;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('continent', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'continent' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.continent) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.continent_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.continent_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.continent_id===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.continent_id===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.continent_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.continent_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('continent', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.continent_id===idRemoved) {
            //update associated item
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnContinentRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.continent_id) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.continent_id);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.continent_id];
        handleDisassociateItem('continent', associatedItem.continent_id);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('continent', item.continent_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('continent', item.continent_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ContinentToAddTransferViewToolbar
                title={'Continents'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ContinentToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ContinentToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.continent_id;
                        let label = undefined;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'ContinentToAddTransferView-listA-listItem-'+it.continent_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.continent_id===it.continent_id) ? 'continent — ' + t('modelPanels.associatedRecord', 'Associated record') : 'continent '}>
                                <Avatar>
                                  {"continent".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'continent_id' }>
                                        <Typography 
                                        id={'ContinentToAddTransferView-listA-listItem-id-'+it.continent_id}
                                        variant="body1" display="block" noWrap={true}>{it.continent_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.continent_id===it.continent_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.continent_id===it.continent_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ContinentToAddTransferView-listA-listItem-'+it.continent_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ContinentToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ContinentToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ContinentToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ContinentToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.continent_id;
                        let label = undefined;
                        ...
                        return (
                          <ListItem 
                            id={'ContinentToAddTransferView-listB-listItem-'+it.continent_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'continent_id' }>
                                        <Typography 
                                        id={'ContinentToAddTransferView-listB-listItem-id-'+it.continent_id}
                                        variant="body1" display="block" noWrap={true}>{it.continent_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ContinentToAddTransferView-listB-listItem-'+it.continent_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='ContinentToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.continent_id;
                        let label = undefined;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'ContinentToAddTransferView-associatedItem-list-listItem-'+associatedItem.continent_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'continent — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"continent".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'continent_id' }>
                                        <Typography 
                                        id={'ContinentToAddTransferView-listB-listItem-id-'+associatedItem.continent_id}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.continent_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'ContinentToAddTransferView-associatedItem-list-listItem-'+associatedItem.continent_id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].continent_id===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('continent', associatedItem.continent_id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.continent_id];
                                      handleDisassociateItem('continent', associatedItem.continent_id);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ContinentToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
ContinentToAddTransferView.propTypes = {
