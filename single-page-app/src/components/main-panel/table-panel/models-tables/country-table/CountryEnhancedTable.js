...
import CountryEnhancedTableHead from './components/CountryEnhancedTableHead'
import CountryEnhancedTableToolbar from './components/CountryEnhancedTableToolbar'
import CountryUploadFileDialog from './components/CountryUploadFileDialog'
import CountryCursorPagination from './components/CountryCursorPagination'
...
//lazy loading
const CountryCreatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Create-Country" */ './components/country-create-panel/CountryCreatePanel')));
const CountryUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Update-Country" */ './components/country-update-panel/CountryUpdatePanel')));
const CountryDetailPanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Country" */ './components/country-detail-panel/CountryDetailPanel')));
const CountryDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Delete-Country" */ './components/CountryDeleteConfirmationDialog')));
...
export default function CountryEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('country_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.country.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'country', method: 'getCount()', request: 'api.country.getCountItems'}];
            newError.path=['Countries'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', method: 'getData()', request: 'api.country.getItems'}];
            newError.path=['Countries'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['country']
    &&(Object.keys(changes.lastModelChanged['country']).length===1)
    &&(changes.lastModelChanged['country'][Object.keys(changes.lastModelChanged['country'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.country.tableTemplate
    */
    let api = await loadApi("country");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.country.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'country', method: 'getCsvTemplate()', request: 'api.country.tableTemplate'}];
            newError.path=['Countries', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        link.href = url;
        link.setAttribute("download", "country-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.country.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='CountryEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='CountryEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <CountryEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='CountryEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'CountryEnhancedTable-row-'+item.country_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'CountryEnhancedTable-row-iconButton-detail-'+item.country_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'CountryEnhancedTable-row-iconButton-edit-'+item.country_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'CountryEnhancedTable-row-iconButton-delete-'+item.country_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'country_id: ' + item.country_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.country_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.continent_id!==null)?item.continent_id:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.river_ids!==null)?item.river_ids:'')}
                                    </TableCell>
...
CountryEnhancedTable.propTypes = {
