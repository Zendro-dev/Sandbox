...
import GermplasmToRemoveTransferViewToolbar from './components/GermplasmToRemoveTransferViewToolbar';
import GermplasmToRemoveTransferViewCursorPagination from './components/GermplasmToRemoveTransferViewCursorPagination';
...
export default function GermplasmToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.breedingMethod.getGermplasmCount
    */
    let cancelableApiReq = makeCancelable(api.breedingMethod.getGermplasmCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'germplasm', method: 'getCount()', request: 'api.breedingMethod.getGermplasmCount'}];
            newError.path=['update', `breedingMethodDbId:${item.breedingMethodDbId}`, 'remove', 'germplasm'];
      ...
      //error
      .catch((err) => { //error: on api.breedingMethod.getGermplasmCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {germplasmDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.breedingMethod.getGermplasm
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.breedingMethod.getGermplasm(
      graphqlServerUrl, 
      item.breedingMethodDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'germplasm', method: 'getData()', request: 'api.breedingMethod.getGermplasm'}];
            newError.path=['update', `breedingMethodDbId:${item.breedingMethodDbId}`, 'remove', 'germplasm'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.breedingMethod.getGermplasm
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.breedingMethodDbId, search, getCount, configurePagination, onEmptyPage]);
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'germplasm' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.germplasm_breedingMethodDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.germplasmDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.germplasmDbId) === -1) {
      lidsToRemove.current.push(item.germplasmDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.germplasmDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <GermplasmToRemoveTransferViewToolbar
                title={'Germplasms'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GermplasmToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='GermplasmToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.germplasmDbId;
                        let label = it.germplasmName;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(germplasmDbId=> germplasmDbId=== it.germplasmDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'GermplasmToRemoveTransferView-listA-listItem-'+it.germplasmDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'germplasmDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.germplasmDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'GermplasmToRemoveTransferView-listA-listItem-'+it.germplasmDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <GermplasmToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GermplasmToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GermplasmToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='GermplasmToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.germplasmDbId;
                        let label = it.germplasmName;
                        ...
                        return (
                          <ListItem 
                            id={'GermplasmToRemoveTransferView-listB-listItem-'+it.germplasmDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'GermplasmToRemoveTransferView-listB-listItem-'+it.germplasmDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
GermplasmToRemoveTransferView.propTypes = {
