...
export default function GermplasmToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.breedingMethod.getNotAssociatedGermplasmCount
    */
    let api = await loadApi("breedingMethod");
    let cancelableApiReq = makeCancelable(api.breedingMethod.getNotAssociatedGermplasmCount(graphqlServerUrl, item.breedingMethodDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'breedingMethod', association: 'germplasm', table:'A', method: 'getCount()', request: 'api.breedingMethod.getNotAssociatedGermplasmCount'}];
            newError.path=['update', `breedingMethodDbId:${item.breedingMethodDbId}`, 'add', 'germplasm'];
      ...
      //error
      .catch((err) => { //error: on api.breedingMethod.getNotAssociatedGermplasmCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.breedingMethodDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.breedingMethod.getNotAssociatedGermplasm
    */
    let api = await loadApi("breedingMethod");
    let cancelableApiReq = makeCancelable(api.breedingMethod.getNotAssociatedGermplasm(
      graphqlServerUrl,
      item.breedingMethodDbId,
      search,
      variables,
      ops,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'breedingMethod', association: 'germplasm', table:'A', method: 'getData()', request: 'api.breedingMethod.getNotAssociatedGermplasm'}];
            newError.path=['update', `breedingMethodDbId:${item.breedingMethodDbId}`, 'add', 'germplasm'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.breedingMethod.getNotAssociatedGermplasm
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.breedingMethodDbId, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.germplasm.getCountItems
    */
    let api = await loadApi("germplasm");
    let cancelableApiReq = makeCancelable(api.germplasm.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'breedingMethod', association: 'germplasm', table:'B', method: 'getCountB()', request: 'api.germplasm.getCountItems'}];
            newError.path=['update', `breedingMethodDbId:${item.breedingMethodDbId}`, 'add', 'germplasm'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.germplasm.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.breedingMethodDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'breedingMethod', association: 'germplasm', table:'B', method: 'getDataB()', request: 'api.germplasm.getItems'}];
            newError.path=['update', `breedingMethodDbId:${item.breedingMethodDbId}`, 'add', 'germplasm'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.breedingMethodDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.breedingMethod&&
        ...
        lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations&&
        lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations.germplasm_breedingMethodDbId&&
        (lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations.germplasm_breedingMethodDbId.added ||
         lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations.germplasm_breedingMethodDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations.germplasm_breedingMethodDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.breedingMethod[String(item.breedingMethodDbId)].changedAssociations.germplasm_breedingMethodDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.germplasm_breedingMethodDbId) {
            ...
            if(entry[1].changedAssociations.germplasm_breedingMethodDbId.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.breedingMethodDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.germplasm_breedingMethodDbId.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.breedingMethodDbId);
                if(iof !== -1) {
    ...
    if(lastModelChanged.germplasm) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.germplasmDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.breedingMethodDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.germplasmDbId) === -1) {
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.germplasmDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
                    ...
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'germplasm' }>
                                <Avatar>{"germplasm".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
