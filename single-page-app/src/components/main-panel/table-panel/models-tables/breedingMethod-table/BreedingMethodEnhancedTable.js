...
import BreedingMethodEnhancedTableHead from './components/BreedingMethodEnhancedTableHead'
import BreedingMethodEnhancedTableToolbar from './components/BreedingMethodEnhancedTableToolbar'
import BreedingMethodUploadFileDialog from './components/BreedingMethodUploadFileDialog'
import BreedingMethodCursorPagination from './components/BreedingMethodCursorPagination'
...
//lazy loading
const BreedingMethodCreatePanel = lazy(() => import(/* webpackChunkName: "Create-BreedingMethod" */ './components/breedingMethod-create-panel/BreedingMethodCreatePanel'));
const BreedingMethodUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-BreedingMethod" */ './components/breedingMethod-update-panel/BreedingMethodUpdatePanel'));
const BreedingMethodDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-BreedingMethod" */ './components/breedingMethod-detail-panel/BreedingMethodDetailPanel'));
const BreedingMethodDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-BreedingMethod" */ './components/BreedingMethodDeleteConfirmationDialog'));
//Plotly
const BreedingMethodPlotly = lazy(() => import(/* webpackChunkName: "Plotly-BreedingMethod" */ '../../../../plots/BreedingMethodPlotly'));
...
export default function BreedingMethodEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('breedingMethodDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.breedingMethod.getCountItems
    */
    let api = await loadApi("breedingMethod");
    let cancelableApiReq = makeCancelable(api.breedingMethod.getCountItems(graphqlServerUrl, search));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'breedingMethod', method: 'getCount()', request: 'api.breedingMethod.getCountItems'}];
            newError.path=['BreedingMethods'];
      ...
      //error
      .catch((err) => { //error: on api.breedingMethod.getCountItems
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'breedingMethod', method: 'getData()', request: 'api.breedingMethod.getItems'}];
            newError.path=['BreedingMethods'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['breedingMethod']
    &&(Object.keys(changes.lastModelChanged['breedingMethod']).length===1)
    &&(changes.lastModelChanged['breedingMethod'][Object.keys(changes.lastModelChanged['breedingMethod'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set breedingMethodDbId (internalId)
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.breedingMethod.tableTemplate
    */
    let api = await loadApi("breedingMethod");
    let cancelableApiReq = makeCancelable(api.breedingMethod.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'breedingMethod', method: 'getCsvTemplate()', request: 'api.breedingMethod.tableTemplate'}];
            newError.path=['BreedingMethods', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateBreedingMethod.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "breedingMethod-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.breedingMethod.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <BreedingMethodEnhancedTableToolbar
                ...
                >
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='BreedingMethodEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='BreedingMethodEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <BreedingMethodEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='BreedingMethodEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'BreedingMethodEnhancedTable-row-'+item.breedingMethodDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.breedingMethodDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'BreedingMethodEnhancedTable-row-iconButton-detail-'+item.breedingMethodDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'BreedingMethodEnhancedTable-row-iconButton-edit-'+item.breedingMethodDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'BreedingMethodEnhancedTable-row-iconButton-delete-'+item.breedingMethodDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'breedingMethodDbId: ' + item.breedingMethodDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.breedingMethodDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.breedingMethodName!==null)?item.breedingMethodName:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <BreedingMethodCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(BreedingMethodPlotly)) && (
                      <Suspense fallback={<div />}>
                        <BreedingMethodPlotly />
                      </Suspense>
                    )}
                  </div>
      ...
      {(uploadFileDialogOpen) && (
        <BreedingMethodUploadFileDialog
...
BreedingMethodEnhancedTable.propTypes = {
