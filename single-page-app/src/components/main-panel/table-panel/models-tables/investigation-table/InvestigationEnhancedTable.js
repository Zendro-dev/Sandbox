...
import InvestigationEnhancedTableHead from './components/InvestigationEnhancedTableHead'
import InvestigationEnhancedTableToolbar from './components/InvestigationEnhancedTableToolbar'
import InvestigationCreatePanel from './components/investigation-create-panel/InvestigationCreatePanel'
import InvestigationUpdatePanel from './components/investigation-update-panel/InvestigationUpdatePanel'
import InvestigationDetailPanel from './components/investigation-detail-panel/InvestigationDetailPanel'
import InvestigationDeleteConfirmationDialog from './components/InvestigationDeleteConfirmationDialog'
import InvestigationUploadFileDialog from './components/InvestigationUploadFileDialog'
import InvestigationCursorPagination from './components/InvestigationCursorPagination'
...
//Plotly
import InvestigationPlotly from '../../../../plots/InvestigationPlotly';
...
export default function InvestigationEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('investigation_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.investigation.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'investigation', method: 'getCount()', request: 'api.investigation.getCountItems'}];
            newError.path=['Investigations'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', method: 'getData()', request: 'api.investigation.getItems'}];
            newError.path=['Investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['investigation']
    &&(Object.keys(changes.lastModelChanged['investigation']).length===1)
    &&(changes.lastModelChanged['investigation'][Object.keys(changes.lastModelChanged['investigation'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set investigation_id (internalId)
    variables.investigation_id = item.investigation_id;

    /*
      API Request: api.investigation.deleteItem
    */
    let cancelableApiReq = makeCancelable(api.investigation.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', method: 'doDelete()', request: 'api.investigation.deleteItem'}];
            newError.path=['Investigations', `investigation_id:${item.investigation_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.investigation.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.investigation.tableTemplate
    */
    let cancelableApiReq = makeCancelable(api.investigation.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', method: 'getCsvTemplate()', request: 'api.investigation.tableTemplate'}];
            newError.path=['Investigations', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateInvestigation.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "investigation-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.investigation.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <InvestigationEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(InvestigationPlotly)}
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='InvestigationEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='InvestigationEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <InvestigationEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='InvestigationEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'InvestigationEnhancedTable-row-'+item.investigation_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'InvestigationEnhancedTable-row-iconButton-detail-'+item.investigation_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.investigation&&Array.isArray(permissions.investigation)
                                      &&(permissions.investigation.includes('update') || permissions.investigation.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'InvestigationEnhancedTable-row-iconButton-edit-'+item.investigation_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.investigation&&Array.isArray(permissions.investigation)
                                      &&(permissions.investigation.includes('delete') || permissions.investigation.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'InvestigationEnhancedTable-row-iconButton-delete-'+item.investigation_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'investigation_id: ' + item.investigation_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.investigation_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.title!==null)?item.title:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.startDate!==null)?moment(item.startDate, "YYYY-MM-DD").format("YYYY-MM-DD"):'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.endDate!==null)?moment(item.endDate, "YYYY-MM-DD").format("YYYY-MM-DD"):'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.ontologyAnnotation_ids!==null)?item.ontologyAnnotation_ids:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.contact_ids!==null)?item.contact_ids:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.fileAttachment_ids!==null)?item.fileAttachment_ids:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <InvestigationDeleteConfirmationDialog
          permissions={permissions}
...
InvestigationEnhancedTable.propTypes = {
