...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.investigation.getStudiesCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'studies', method: 'getCount()', request: 'api.investigation.getStudiesCount'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'remove', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.investigation.getStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getStudies
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.investigation.getStudies(
      graphqlServerUrl, 
      item.investigation_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'studies', method: 'getData()', request: 'api.investigation.getStudies'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'remove', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.investigation.getStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
