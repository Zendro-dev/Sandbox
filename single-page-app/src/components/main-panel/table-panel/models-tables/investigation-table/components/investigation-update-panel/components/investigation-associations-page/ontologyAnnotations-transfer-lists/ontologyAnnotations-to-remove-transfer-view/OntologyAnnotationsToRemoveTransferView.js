...
import OntologyAnnotationsToRemoveTransferViewToolbar from './components/OntologyAnnotationsToRemoveTransferViewToolbar';
import OntologyAnnotationsToRemoveTransferViewCursorPagination from './components/OntologyAnnotationsToRemoveTransferViewCursorPagination';
...
export default function OntologyAnnotationsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getOntologyAnnotationsCount
    */
    let cancelableApiReq = makeCancelable(api.investigation.getOntologyAnnotationsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'ontologyAnnotations', method: 'getCount()', request: 'api.investigation.getOntologyAnnotationsCount'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'remove', 'ontologyAnnotations'];
      ...
      //error
      .catch((err) => { //error: on api.investigation.getOntologyAnnotationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {ontologyAnnotation_id: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.investigation.getOntologyAnnotations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.investigation.getOntologyAnnotations(
      graphqlServerUrl, 
      item.investigation_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'ontologyAnnotations', method: 'getData()', request: 'api.investigation.getOntologyAnnotations'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'remove', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.investigation.getOntologyAnnotations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.investigation_id, search, getCount, configurePagination, onEmptyPage]);
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'ontologyAnnotation' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.investigation_ontologyAnnotation_ids.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.ontologyAnnotation_id;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.ontologyAnnotation_id) === -1) {
      lidsToRemove.current.push(item.ontologyAnnotation_id);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.ontologyAnnotation_id);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <OntologyAnnotationsToRemoveTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyAnnotationsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='OntologyAnnotationsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.ontologyAnnotation_id;
                        let label = it.ontology;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(ontologyAnnotation_id=> ontologyAnnotation_id=== it.ontologyAnnotation_id) !== undefined;

                        return (
                          <ListItem 
                            id={'OntologyAnnotationsToRemoveTransferView-listA-listItem-'+it.ontologyAnnotation_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'ontologyAnnotation_id' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.ontologyAnnotation_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'OntologyAnnotationsToRemoveTransferView-listA-listItem-'+it.ontologyAnnotation_id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <OntologyAnnotationsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyAnnotationsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyAnnotationsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='OntologyAnnotationsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.ontologyAnnotation_id;
                        let label = it.ontology;
                        ...
                        return (
                          <ListItem 
                            id={'OntologyAnnotationsToRemoveTransferView-listB-listItem-'+it.ontologyAnnotation_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'OntologyAnnotationsToRemoveTransferView-listB-listItem-'+it.ontologyAnnotation_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
OntologyAnnotationsToRemoveTransferView.propTypes = {
