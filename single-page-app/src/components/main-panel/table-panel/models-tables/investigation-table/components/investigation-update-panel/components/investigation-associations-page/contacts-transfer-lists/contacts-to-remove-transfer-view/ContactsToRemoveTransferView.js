...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getContactsCount
    */
    let cancelableApiReq = makeCancelable(api.investigation.getContactsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'contacts', method: 'getCount()', request: 'api.investigation.getContactsCount'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'remove', 'contacts'];
      ...
      //error
      .catch((err) => { //error: on api.investigation.getContactsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getContacts
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.investigation.getContacts(
      graphqlServerUrl, 
      item.investigation_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'contacts', method: 'getData()', request: 'api.investigation.getContacts'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'remove', 'contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.investigation.getContacts
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
