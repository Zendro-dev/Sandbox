...
import OntologyAnnotationsToAddTransferViewToolbar from './components/OntologyAnnotationsToAddTransferViewToolbar';
import OntologyAnnotationsToAddTransferViewCursorPagination from './components/OntologyAnnotationsToAddTransferViewCursorPagination';
...
export default function OntologyAnnotationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"ontologyAnnotation_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.investigation.getNotAssociatedOntologyAnnotationsCount
    */
    let cancelableApiReq = makeCancelable(api.investigation.getNotAssociatedOntologyAnnotationsCount(graphqlServerUrl, item.investigation_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'investigation', association: 'ontologyAnnotations', table:'A', method: 'getCount()', request: 'api.investigation.getNotAssociatedOntologyAnnotationsCount'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'ontologyAnnotations'];
      ...
      //error
      .catch((err) => { //error: on api.investigation.getNotAssociatedOntologyAnnotationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.investigation_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getNotAssociatedOntologyAnnotations
    */
    let cancelableApiReq = makeCancelable(api.investigation.getNotAssociatedOntologyAnnotations(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'ontologyAnnotations', table:'A', method: 'getData()', request: 'api.investigation.getNotAssociatedOntologyAnnotations'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.investigation.getNotAssociatedOntologyAnnotations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.investigation_id, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getCountItems
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'ontologyAnnotations', table:'B', method: 'getCountB()', request: 'api.ontologyAnnotation.getCountItems'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'ontologyAnnotations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.ontologyAnnotation.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.investigation_id, searchB]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getItems
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'ontologyAnnotations', table:'B', method: 'getDataB()', request: 'api.ontologyAnnotation.getItems'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.investigation_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'ontologyAnnotations' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'ontologyAnnotations', 
    ...
    if(lastModelChanged.investigation&&
        ...
        lastModelChanged.investigation[String(item.investigation_id)].changedAssociations&&
        lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_ontologyAnnotation_ids&&
        (lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_ontologyAnnotation_ids.added ||
         lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_ontologyAnnotation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_ontologyAnnotation_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('ontologyAnnotations', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_ontologyAnnotation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'ontologyAnnotations' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'ontologyAnnotation',
     * but this action happened on the peer relation, identified by 'investigation_ontologyAnnotation_ids'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'ontologyAnnotation'.
     * B: the current item internalId is in the addedIds of the updated 'ontologyAnnotation'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'ontologyAnnotation' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.ontologyAnnotation) {
      let oens = Object.entries(lastModelChanged.ontologyAnnotation);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.investigation_ontologyAnnotation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.investigation_ontologyAnnotation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.investigation_ontologyAnnotation_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.investigation_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.investigation_ontologyAnnotation_ids.added) {
              let idsAdded = entry[1].changedAssociations.investigation_ontologyAnnotation_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.investigation_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.ontologyAnnotation_id;
    ...
    /*
     * Case 3: 
     * The attributes of some 'ontologyAnnotation' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyAnnotation) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.ontologyAnnotation_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.ontologyAnnotation_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.ontologyAnnotation_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.ontologyAnnotation_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.ontologyAnnotation_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.ontologyAnnotation_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('ontologyAnnotations', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.investigation_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnOntologyAnnotationRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.ontologyAnnotation_id) === -1) {
      lidsToAdd.current.push(item.ontologyAnnotation_id);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('ontologyAnnotations', item.ontologyAnnotation_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.ontologyAnnotation_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('ontologyAnnotations', item.ontologyAnnotation_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <OntologyAnnotationsToAddTransferViewToolbar
                title={'OntologyAnnotations'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyAnnotationsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='OntologyAnnotationsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.ontologyAnnotation_id;
                        let label = it.ontology;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'OntologyAnnotationsToAddTransferView-listA-listItem-'+it.ontologyAnnotation_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'ontologyAnnotation' }>
                                <Avatar>{"ontologyAnnotation".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'ontologyAnnotation_id' }>
                                        <Typography 
                                        id={'OntologyAnnotationsToAddTransferView-listA-listItem-id-'+it.ontologyAnnotation_id}
                                        variant="body1" display="block" noWrap={true}>{it.ontologyAnnotation_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'OntologyAnnotationsToAddTransferView-listA-listItem-'+it.ontologyAnnotation_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <OntologyAnnotationsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyAnnotationsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyAnnotationsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='OntologyAnnotationsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.ontologyAnnotation_id;
                        let label = it.ontology;
                        ...
                        return (
                          <ListItem 
                            id={'OntologyAnnotationsToAddTransferView-listB-listItem-'+it.ontologyAnnotation_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'ontologyAnnotation_id' }>
                                        <Typography 
                                        id={'OntologyAnnotationsToAddTransferView-listB-listItem-id-'+it.ontologyAnnotation_id}
                                        variant="body1" display="block" noWrap={true}>{it.ontologyAnnotation_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'OntologyAnnotationsToAddTransferView-listB-listItem-'+it.ontologyAnnotation_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
OntologyAnnotationsToAddTransferView.propTypes = {
