...
const cancelableCountingPromises = useRef([]);
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getCountItems(graphqlServerUrl, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'investigation', association: 'ontologyAnnotations', table:'A', method: 'getCount()', request: 'api.ontologyAnnotation.getCountItems'}];
            newError.path=['add', 'ontologyAnnotations'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'ontologyAnnotations', table:'A', method: 'getData()', request: 'api.ontologyAnnotation.getItems'}];
            newError.path=['add', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
