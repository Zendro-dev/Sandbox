...
export default function ContactsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getNotAssociatedContactsCount
    */
    let cancelableApiReq = makeCancelable(api.investigation.getNotAssociatedContactsCount(graphqlServerUrl, item.investigation_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'investigation', association: 'contacts', table:'A', method: 'getCount()', request: 'api.investigation.getNotAssociatedContactsCount'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'contacts'];
      ...
      //error
      .catch((err) => { //error: on api.investigation.getNotAssociatedContactsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getNotAssociatedContacts
    */
    let cancelableApiReq = makeCancelable(api.investigation.getNotAssociatedContacts(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'contacts', table:'A', method: 'getData()', request: 'api.investigation.getNotAssociatedContacts'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.investigation.getNotAssociatedContacts
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'contacts', table:'B', method: 'getCountB()', request: 'api.contact.getCountItems'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'contacts'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'contacts', table:'B', method: 'getDataB()', request: 'api.contact.getItems'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.investigation&&
        ...
        lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_contact_ids&&
        (lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_contact_ids.added ||
         lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_contact_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_contact_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_contact_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'investigation_contact_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.contact) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.investigation_contact_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.investigation_contact_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.investigation_contact_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.investigation_contact_ids.added) {
              let idsAdded = entry[1].changedAssociations.investigation_contact_ids.idsAdded;
              if(idsAdded) {
