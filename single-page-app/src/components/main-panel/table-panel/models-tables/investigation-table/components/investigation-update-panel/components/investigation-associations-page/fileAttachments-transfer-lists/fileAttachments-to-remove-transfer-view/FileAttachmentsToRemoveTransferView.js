...
import FileAttachmentsToRemoveTransferViewToolbar from './components/FileAttachmentsToRemoveTransferViewToolbar';
import FileAttachmentsToRemoveTransferViewCursorPagination from './components/FileAttachmentsToRemoveTransferViewCursorPagination';
...
export default function FileAttachmentsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.investigation.getFileAttachmentsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'fileAttachments', method: 'getCount()', request: 'api.investigation.getFileAttachmentsCount'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'remove', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.investigation.getFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {fileAttachment_id: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.investigation.getFileAttachments
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.investigation.getFileAttachments(
      graphqlServerUrl, 
      item.investigation_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'fileAttachments', method: 'getData()', request: 'api.investigation.getFileAttachments'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'remove', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.investigation.getFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'fileAttachment' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.investigation_fileAttachment_ids.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.fileAttachment_id;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.fileAttachment_id) === -1) {
      lidsToRemove.current.push(item.fileAttachment_id);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.fileAttachment_id);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <FileAttachmentsToRemoveTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='FileAttachmentsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='FileAttachmentsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.fileAttachment_id;
                        let label = it.fileName;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(fileAttachment_id=> fileAttachment_id=== it.fileAttachment_id) !== undefined;

                        return (
                          <ListItem 
                            id={'FileAttachmentsToRemoveTransferView-listA-listItem-'+it.fileAttachment_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'fileAttachment_id' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.fileAttachment_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'FileAttachmentsToRemoveTransferView-listA-listItem-'+it.fileAttachment_id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <FileAttachmentsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='FileAttachmentsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='FileAttachmentsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='FileAttachmentsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.fileAttachment_id;
                        let label = it.fileName;
                        ...
                        return (
                          <ListItem 
                            id={'FileAttachmentsToRemoveTransferView-listB-listItem-'+it.fileAttachment_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'FileAttachmentsToRemoveTransferView-listB-listItem-'+it.fileAttachment_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
FileAttachmentsToRemoveTransferView.propTypes = {
