...
import FileAttachmentsToAddTransferViewToolbar from './components/FileAttachmentsToAddTransferViewToolbar';
import FileAttachmentsToAddTransferViewCursorPagination from './components/FileAttachmentsToAddTransferViewCursorPagination';
...
export default function FileAttachmentsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"fileAttachment_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.investigation.getNotAssociatedFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.investigation.getNotAssociatedFileAttachmentsCount(graphqlServerUrl, item.investigation_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'investigation', association: 'fileAttachments', table:'A', method: 'getCount()', request: 'api.investigation.getNotAssociatedFileAttachmentsCount'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.investigation.getNotAssociatedFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getNotAssociatedFileAttachments
    */
    let cancelableApiReq = makeCancelable(api.investigation.getNotAssociatedFileAttachments(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'fileAttachments', table:'A', method: 'getData()', request: 'api.investigation.getNotAssociatedFileAttachments'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.investigation.getNotAssociatedFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getCountItems
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'fileAttachments', table:'B', method: 'getCountB()', request: 'api.fileAttachment.getCountItems'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'fileAttachments'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.fileAttachment.getItems
    */
    let cancelableApiReq = makeCancelable(api.fileAttachment.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'investigation', association: 'fileAttachments', table:'B', method: 'getDataB()', request: 'api.fileAttachment.getItems'}];
            newError.path=['update', `investigation_id:${item.investigation_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'fileAttachments' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'fileAttachments', 
    ...
    if(lastModelChanged.investigation&&
        ...
        lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_fileAttachment_ids&&
        (lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_fileAttachment_ids.added ||
         lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_fileAttachment_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_fileAttachment_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('fileAttachments', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.investigation[String(item.investigation_id)].changedAssociations.investigation_fileAttachment_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'fileAttachments' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'fileAttachment',
     * but this action happened on the peer relation, identified by 'investigation_fileAttachment_ids'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'fileAttachment'.
     * B: the current item internalId is in the addedIds of the updated 'fileAttachment'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'fileAttachment' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.fileAttachment) {
      let oens = Object.entries(lastModelChanged.fileAttachment);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.investigation_fileAttachment_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.investigation_fileAttachment_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.investigation_fileAttachment_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.investigation_fileAttachment_ids.added) {
              let idsAdded = entry[1].changedAssociations.investigation_fileAttachment_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.investigation_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.fileAttachment_id;
    ...
    /*
     * Case 3: 
     * The attributes of some 'fileAttachment' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.fileAttachment) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.fileAttachment_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.fileAttachment_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.fileAttachment_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.fileAttachment_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.fileAttachment_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.fileAttachment_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('fileAttachments', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnFileAttachmentRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.fileAttachment_id) === -1) {
      lidsToAdd.current.push(item.fileAttachment_id);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('fileAttachments', item.fileAttachment_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.fileAttachment_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('fileAttachments', item.fileAttachment_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <FileAttachmentsToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='FileAttachmentsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='FileAttachmentsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.fileAttachment_id;
                        let label = it.fileName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'FileAttachmentsToAddTransferView-listA-listItem-'+it.fileAttachment_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'fileAttachment' }>
                                <Avatar>{"fileAttachment".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'fileAttachment_id' }>
                                        <Typography 
                                        id={'FileAttachmentsToAddTransferView-listA-listItem-id-'+it.fileAttachment_id}
                                        variant="body1" display="block" noWrap={true}>{it.fileAttachment_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'FileAttachmentsToAddTransferView-listA-listItem-'+it.fileAttachment_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <FileAttachmentsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='FileAttachmentsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='FileAttachmentsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='FileAttachmentsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.fileAttachment_id;
                        let label = it.fileName;
                        ...
                        return (
                          <ListItem 
                            id={'FileAttachmentsToAddTransferView-listB-listItem-'+it.fileAttachment_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'fileAttachment_id' }>
                                        <Typography 
                                        id={'FileAttachmentsToAddTransferView-listB-listItem-id-'+it.fileAttachment_id}
                                        variant="body1" display="block" noWrap={true}>{it.fileAttachment_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'FileAttachmentsToAddTransferView-listB-listItem-'+it.fileAttachment_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
FileAttachmentsToAddTransferView.propTypes = {
