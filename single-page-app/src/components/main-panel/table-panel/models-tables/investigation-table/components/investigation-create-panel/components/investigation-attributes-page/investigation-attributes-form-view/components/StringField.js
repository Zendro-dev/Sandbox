...
export default function StringField(props) {
  ...
  return (
    <Grid container justify='flex-start' alignItems='center' spacing={0}>
      <Grid item xs={12}>
        <TextField
          id={'StringField-Investigation-'+name}
              ...
              <InputAdornment position="end">
                {(valueOk!==undefined&&valueOk===1) 
                  ...
                  <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                    <Typography id={'StringField-Investigation-exists-'+name} variant="caption" color="primary">
                      &#8707;
                    </Typography>
                  </Tooltip>
                  : 
                  <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                    <Typography id={'StringField-Investigation-notExists-'+name} variant="caption" color="textSecondary">
                      &#8708;
                    </Typography>
                  </Tooltip>
                }
      ...
      {(valueAjv !== undefined && valueAjv.errors.length > 0) && (
        <Grid item id={'StringField-Investigation-ajvError-'+name} xs={12}>
