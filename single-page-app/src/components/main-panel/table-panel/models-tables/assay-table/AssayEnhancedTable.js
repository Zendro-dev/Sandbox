...
import AssayEnhancedTableHead from './components/AssayEnhancedTableHead'
import AssayEnhancedTableToolbar from './components/AssayEnhancedTableToolbar'
import AssayCreatePanel from './components/assay-create-panel/AssayCreatePanel'
import AssayUpdatePanel from './components/assay-update-panel/AssayUpdatePanel'
import AssayDetailPanel from './components/assay-detail-panel/AssayDetailPanel'
import AssayDeleteConfirmationDialog from './components/AssayDeleteConfirmationDialog'
import AssayUploadFileDialog from './components/AssayUploadFileDialog'
import AssayCursorPagination from './components/AssayCursorPagination'
...
//Plotly
import AssayPlotly from '../../../../plots/AssayPlotly';
...
export default function AssayEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('assay_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.assay.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assay', method: 'getCount()', request: 'api.assay.getCountItems'}];
            newError.path=['Assays'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', method: 'getData()', request: 'api.assay.getItems'}];
            newError.path=['Assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['assay']
    &&(Object.keys(changes.lastModelChanged['assay']).length===1)
    &&(changes.lastModelChanged['assay'][Object.keys(changes.lastModelChanged['assay'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set assay_id (internalId)
    variables.assay_id = item.assay_id;

    /*
      API Request: api.assay.deleteItem
    */
    let cancelableApiReq = makeCancelable(api.assay.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', method: 'doDelete()', request: 'api.assay.deleteItem'}];
            newError.path=['Assays', `assay_id:${item.assay_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.assay.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.assay.tableTemplate
    */
    let cancelableApiReq = makeCancelable(api.assay.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', method: 'getCsvTemplate()', request: 'api.assay.tableTemplate'}];
            newError.path=['Assays', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateAssay.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "assay-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.assay.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <AssayEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(AssayPlotly)}
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='AssayEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='AssayEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <AssayEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='AssayEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'AssayEnhancedTable-row-'+item.assay_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'AssayEnhancedTable-row-iconButton-detail-'+item.assay_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.assay&&Array.isArray(permissions.assay)
                                      &&(permissions.assay.includes('update') || permissions.assay.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'AssayEnhancedTable-row-iconButton-edit-'+item.assay_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.assay&&Array.isArray(permissions.assay)
                                      &&(permissions.assay.includes('delete') || permissions.assay.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'AssayEnhancedTable-row-iconButton-delete-'+item.assay_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'assay_id: ' + item.assay_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.assay_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.measurement!==null)?item.measurement:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.technology!==null)?item.technology:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.study_id!==null)?item.study_id:'')}
                                    </TableCell>
