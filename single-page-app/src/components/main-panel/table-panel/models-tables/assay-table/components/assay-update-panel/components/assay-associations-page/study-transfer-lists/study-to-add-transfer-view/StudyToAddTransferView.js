...
export default function StudyToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.assay_id;

    /*
      API Request: api.assay.getStudy
    */
    let cancelableApiReq = makeCancelable(api.assay.getStudy(graphqlServerUrl, item.assay_id));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assay', association: 'study', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.assay.getStudy'}];
            newError.path=['update', `assay_id:${currentId}`, 'add', 'study'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getStudy
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.assay_id]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"study_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.assay.getNotAssociatedStudyCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedStudyCount(graphqlServerUrl, item.assay_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'study', table:'A', method: 'getCount()', request: 'api.assay.getNotAssociatedStudyCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedStudyCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.assay_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedStudy
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedStudy(
      graphqlServerUrl,
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'study', table:'A', method: 'getData()', request: 'api.assay.getNotAssociatedStudy'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedStudy
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.assay_id, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'study', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.assay_id, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.assay&&
        ...
        lastModelChanged.assay[String(item.assay_id)].changedAssociations&&
        lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_study_id&&
        (lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_study_id.added ||
         lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_study_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_study_id.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_study_id.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'assay_study_id'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.assay_study_id) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.assay_study_id.removed) {
              let idsRemoved = entry[1].changedAssociations.assay_study_id.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.assay_id);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('study', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.assay_study_id.added) {
              let idsAdded = entry[1].changedAssociations.assay_study_id.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.assay_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.study_id;
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.study_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.study_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.study_id===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.study_id===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.study_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.study_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup on associated item
          if(associatedItem && associatedItem.study_id===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.assay_id, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.study_id) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.study_id);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.study_id];
        handleDisassociateItem('study', associatedItem.study_id);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('study', item.study_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('study', item.study_id);
    }
  };
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    ...
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'StudyToAddTransferView-listA-listItem-'+it.study_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.study_id===it.study_id) ? 'study — ' + t('modelPanels.associatedRecord', 'Associated record') : 'study '}>
                                <Avatar>
                                  {"study".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'study_id' }>
                                        <Typography 
                                        id={'StudyToAddTransferView-listA-listItem-id-'+it.study_id}
                                        variant="body1" display="block" noWrap={true}>{it.study_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.study_id===it.study_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.study_id===it.study_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'StudyToAddTransferView-listA-listItem-'+it.study_id+'-button-add'}
                                  color="primary"
                  ...
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='StudyToAddTransferView-list-listB'
                      ...
                      {itemsB.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'StudyToAddTransferView-listB-listItem-'+it.study_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'study_id' }>
                                        <Typography 
                                        id={'StudyToAddTransferView-listB-listItem-id-'+it.study_id}
                                        variant="body1" display="block" noWrap={true}>{it.study_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'StudyToAddTransferView-listB-listItem-'+it.study_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
                  ...
                  <Box className={classes.listBox}>
                    ...
                    <List id='StudyToAddTransferView-associatedItem-list'
                      ...
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.study_id;
                        let label = associatedItem.name;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'StudyToAddTransferView-associatedItem-list-listItem-'+associatedItem.study_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'study_id' }>
                                        <Typography 
                                        id={'StudyToAddTransferView-listB-listItem-id-'+associatedItem.study_id}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.study_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'StudyToAddTransferView-associatedItem-list-listItem-'+associatedItem.study_id+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].study_id===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('study', associatedItem.study_id);
                                    } else {
                                      lidsToRemove.current = [associatedItem.study_id];
                                      handleDisassociateItem('study', associatedItem.study_id);
                                    }
