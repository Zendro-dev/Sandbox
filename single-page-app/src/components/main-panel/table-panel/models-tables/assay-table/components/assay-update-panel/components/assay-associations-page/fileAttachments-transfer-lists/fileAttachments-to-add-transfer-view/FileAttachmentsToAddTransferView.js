...
export default function FileAttachmentsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedFileAttachmentsCount(graphqlServerUrl, item.assay_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assay', association: 'fileAttachments', table:'A', method: 'getCount()', request: 'api.assay.getNotAssociatedFileAttachmentsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedFileAttachments
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedFileAttachments(
      graphqlServerUrl,
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'fileAttachments', table:'A', method: 'getData()', request: 'api.assay.getNotAssociatedFileAttachments'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'fileAttachments', table:'B', method: 'getCountB()', request: 'api.fileAttachment.getCountItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'fileAttachments'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'fileAttachments', table:'B', method: 'getDataB()', request: 'api.fileAttachment.getItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.assay&&
        ...
        lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_fileAttachment_ids&&
        (lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_fileAttachment_ids.added ||
         lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_fileAttachment_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_fileAttachment_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_fileAttachment_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'assay_fileAttachment_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.fileAttachment) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.assay_fileAttachment_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.assay_fileAttachment_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.assay_fileAttachment_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.assay_fileAttachment_ids.added) {
              let idsAdded = entry[1].changedAssociations.assay_fileAttachment_ids.idsAdded;
              if(idsAdded) {
