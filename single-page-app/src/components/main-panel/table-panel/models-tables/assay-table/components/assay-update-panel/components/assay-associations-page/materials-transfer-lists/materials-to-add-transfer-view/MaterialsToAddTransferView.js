...
export default function MaterialsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedMaterialsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedMaterialsCount(graphqlServerUrl, item.assay_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assay', association: 'materials', table:'A', method: 'getCount()', request: 'api.assay.getNotAssociatedMaterialsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'materials'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedMaterialsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedMaterials
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedMaterials(
      graphqlServerUrl,
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'materials', table:'A', method: 'getData()', request: 'api.assay.getNotAssociatedMaterials'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedMaterials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'materials', table:'B', method: 'getCountB()', request: 'api.material.getCountItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'materials'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'materials', table:'B', method: 'getDataB()', request: 'api.material.getItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.assay&&
        ...
        lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_material_ids&&
        (lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_material_ids.added ||
         lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_material_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_material_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_material_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'assay_material_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.material) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.assay_material_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.assay_material_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.assay_material_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.assay_material_ids.added) {
              let idsAdded = entry[1].changedAssociations.assay_material_ids.idsAdded;
              if(idsAdded) {
