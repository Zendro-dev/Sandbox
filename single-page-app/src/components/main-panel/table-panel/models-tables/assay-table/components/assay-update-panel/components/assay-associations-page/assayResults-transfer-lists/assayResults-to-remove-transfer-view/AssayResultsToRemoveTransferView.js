...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getAssayResultsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getAssayResultsCount(
      graphqlServerUrl, 
      item.assay_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'assayResults', method: 'getCount()', request: 'api.assay.getAssayResultsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'assayResults'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getAssayResultsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getAssayResults
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.assay.getAssayResults(
      graphqlServerUrl, 
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'assayResults', method: 'getData()', request: 'api.assay.getAssayResults'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'assayResults'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getAssayResults
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
