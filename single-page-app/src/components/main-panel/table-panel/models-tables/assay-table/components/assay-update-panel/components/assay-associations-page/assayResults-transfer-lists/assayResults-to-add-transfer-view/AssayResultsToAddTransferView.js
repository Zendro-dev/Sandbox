...
export default function AssayResultsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedAssayResultsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedAssayResultsCount(graphqlServerUrl, item.assay_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assay', association: 'assayResults', table:'A', method: 'getCount()', request: 'api.assay.getNotAssociatedAssayResultsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'assayResults'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedAssayResultsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedAssayResults
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedAssayResults(
      graphqlServerUrl,
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'assayResults', table:'A', method: 'getData()', request: 'api.assay.getNotAssociatedAssayResults'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'assayResults'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedAssayResults
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'assayResults', table:'B', method: 'getCountB()', request: 'api.assayResult.getCountItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'assayResults'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'assayResults', table:'B', method: 'getDataB()', request: 'api.assayResult.getItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'assayResults'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.assay&&
        ...
        lastModelChanged.assay[String(item.assay_id)].changedAssociations.assayResult_assay_id&&
        (lastModelChanged.assay[String(item.assay_id)].changedAssociations.assayResult_assay_id.added ||
         lastModelChanged.assay[String(item.assay_id)].changedAssociations.assayResult_assay_id.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assayResult_assay_id.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assayResult_assay_id.idsRemoved;
          if(idsRemoved) {
            //increment A
