...
export default function FactorsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedFactorsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedFactorsCount(graphqlServerUrl, item.assay_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assay', association: 'factors', table:'A', method: 'getCount()', request: 'api.assay.getNotAssociatedFactorsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'factors'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedFactorsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedFactors
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedFactors(
      graphqlServerUrl,
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'factors', table:'A', method: 'getData()', request: 'api.assay.getNotAssociatedFactors'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'factors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedFactors
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'factors', table:'B', method: 'getCountB()', request: 'api.factor.getCountItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'factors'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'factors', table:'B', method: 'getDataB()', request: 'api.factor.getItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'factors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.assay&&
        ...
        lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_factor_ids&&
        (lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_factor_ids.added ||
         lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_factor_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_factor_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_factor_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'assay_factor_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.factor) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.assay_factor_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.assay_factor_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.assay_factor_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.assay_factor_ids.added) {
              let idsAdded = entry[1].changedAssociations.assay_factor_ids.idsAdded;
              if(idsAdded) {
