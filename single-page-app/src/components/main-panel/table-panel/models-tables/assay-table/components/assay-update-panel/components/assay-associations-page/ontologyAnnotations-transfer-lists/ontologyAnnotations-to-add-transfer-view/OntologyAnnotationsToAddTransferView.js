...
export default function OntologyAnnotationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedOntologyAnnotationsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedOntologyAnnotationsCount(graphqlServerUrl, item.assay_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assay', association: 'ontologyAnnotations', table:'A', method: 'getCount()', request: 'api.assay.getNotAssociatedOntologyAnnotationsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'ontologyAnnotations'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedOntologyAnnotationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getNotAssociatedOntologyAnnotations
    */
    let cancelableApiReq = makeCancelable(api.assay.getNotAssociatedOntologyAnnotations(
      graphqlServerUrl,
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'ontologyAnnotations', table:'A', method: 'getData()', request: 'api.assay.getNotAssociatedOntologyAnnotations'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getNotAssociatedOntologyAnnotations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.assay_id, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'ontologyAnnotations', table:'B', method: 'getCountB()', request: 'api.ontologyAnnotation.getCountItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'ontologyAnnotations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.assay_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'assay', association: 'ontologyAnnotations', table:'B', method: 'getDataB()', request: 'api.ontologyAnnotation.getItems'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'add', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.assay_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.assay&&
        ...
        lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_ontologyAnnotation_ids&&
        (lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_ontologyAnnotation_ids.added ||
         lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_ontologyAnnotation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_ontologyAnnotation_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.assay[String(item.assay_id)].changedAssociations.assay_ontologyAnnotation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'assay_ontologyAnnotation_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyAnnotation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.assay_ontologyAnnotation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.assay_ontologyAnnotation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.assay_ontologyAnnotation_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.assay_ontologyAnnotation_ids.added) {
              let idsAdded = entry[1].changedAssociations.assay_ontologyAnnotation_ids.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.assay_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
