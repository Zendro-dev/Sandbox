...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getFileAttachmentsCount(
      graphqlServerUrl, 
      item.assay_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'fileAttachments', method: 'getCount()', request: 'api.assay.getFileAttachmentsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getFileAttachments
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.assay.getFileAttachments(
      graphqlServerUrl, 
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'fileAttachments', method: 'getData()', request: 'api.assay.getFileAttachments'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
