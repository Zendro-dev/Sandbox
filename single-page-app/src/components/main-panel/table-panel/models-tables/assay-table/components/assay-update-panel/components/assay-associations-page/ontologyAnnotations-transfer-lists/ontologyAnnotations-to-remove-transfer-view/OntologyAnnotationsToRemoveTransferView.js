...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getOntologyAnnotationsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getOntologyAnnotationsCount(
      graphqlServerUrl, 
      item.assay_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'ontologyAnnotations', method: 'getCount()', request: 'api.assay.getOntologyAnnotationsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'ontologyAnnotations'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getOntologyAnnotationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getOntologyAnnotations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.assay.getOntologyAnnotations(
      graphqlServerUrl, 
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'ontologyAnnotations', method: 'getData()', request: 'api.assay.getOntologyAnnotations'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'ontologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getOntologyAnnotations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.assay_id, search, getCount, configurePagination, onEmptyPage]);
