...
import AssayUpdatePanel from '../assay-update-panel/AssayUpdatePanel'
import AssayDeleteConfirmationDialog from '../AssayDeleteConfirmationDialog'
...
export default function AssayDetailPanel(props) {
      ...
      lastModelChanged['assay'][String(item['assay_id'])]) {
        ...
        if(lastModelChanged['assay'][String(item['assay_id'])].op === "update"&&
            lastModelChanged['assay'][String(item['assay_id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['assay'][String(item['assay_id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('assay', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('assay', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='AssayDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Assay | assay_id: " + itemState['assay_id'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='AssayDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='AssayDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
