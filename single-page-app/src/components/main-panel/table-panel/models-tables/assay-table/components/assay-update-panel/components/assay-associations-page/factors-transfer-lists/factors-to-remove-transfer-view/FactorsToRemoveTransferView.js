...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getFactorsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getFactorsCount(
      graphqlServerUrl, 
      item.assay_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'factors', method: 'getCount()', request: 'api.assay.getFactorsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'factors'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getFactorsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getFactors
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.assay.getFactors(
      graphqlServerUrl, 
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'factors', method: 'getData()', request: 'api.assay.getFactors'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'factors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getFactors
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
