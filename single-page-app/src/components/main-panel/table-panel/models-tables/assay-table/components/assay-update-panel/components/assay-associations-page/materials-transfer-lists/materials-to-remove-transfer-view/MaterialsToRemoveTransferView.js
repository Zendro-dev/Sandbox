...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getMaterialsCount
    */
    let cancelableApiReq = makeCancelable(api.assay.getMaterialsCount(
      graphqlServerUrl, 
      item.assay_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'materials', method: 'getCount()', request: 'api.assay.getMaterialsCount'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'materials'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getMaterialsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.assay.getMaterials
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.assay.getMaterials(
      graphqlServerUrl, 
      item.assay_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'materials', method: 'getData()', request: 'api.assay.getMaterials'}];
            newError.path=['update', `assay_id:${item.assay_id}`, 'remove', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assay.getMaterials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
