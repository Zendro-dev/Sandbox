...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getStudiesCount
    */
    let api = await loadApi("trial");
    let cancelableApiReq = makeCancelable(api.trial.getStudiesCount(
      graphqlServerUrl, 
      item.trialDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'studies', method: 'getCount()', request: 'api.trial.getStudiesCount'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.trial.getStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getStudies
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("trial");
    let cancelableApiReq = makeCancelable(api.trial.getStudies(
      graphqlServerUrl, 
      item.trialDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'studies', method: 'getData()', request: 'api.trial.getStudies'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trial.getStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.trialDbId, search, getCount, configurePagination, onEmptyPage]);
