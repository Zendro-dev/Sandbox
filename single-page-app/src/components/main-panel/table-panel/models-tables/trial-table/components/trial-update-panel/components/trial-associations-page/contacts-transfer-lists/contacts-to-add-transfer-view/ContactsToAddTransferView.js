...
export default function ContactsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getNotAssociatedContactsCount
    */
    let cancelableApiReq = makeCancelable(api.trial.getNotAssociatedContactsCount(graphqlServerUrl, item.trialDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'trial', association: 'contacts', table:'A', method: 'getCount()', request: 'api.trial.getNotAssociatedContactsCount'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'add', 'contacts'];
      ...
      //error
      .catch((err) => { //error: on api.trial.getNotAssociatedContactsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getNotAssociatedContacts
    */
    let cancelableApiReq = makeCancelable(api.trial.getNotAssociatedContacts(
      graphqlServerUrl,
      item.trialDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trial', association: 'contacts', table:'A', method: 'getData()', request: 'api.trial.getNotAssociatedContacts'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'add', 'contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trial.getNotAssociatedContacts
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
