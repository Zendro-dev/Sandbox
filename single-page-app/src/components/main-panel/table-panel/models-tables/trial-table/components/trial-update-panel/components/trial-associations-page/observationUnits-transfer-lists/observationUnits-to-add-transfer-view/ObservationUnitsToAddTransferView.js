...
export default function ObservationUnitsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getNotAssociatedObservationUnitsCount
    */
    let cancelableApiReq = makeCancelable(api.trial.getNotAssociatedObservationUnitsCount(graphqlServerUrl, item.trialDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'trial', association: 'observationUnits', table:'A', method: 'getCount()', request: 'api.trial.getNotAssociatedObservationUnitsCount'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'add', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.trial.getNotAssociatedObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getNotAssociatedObservationUnits
    */
    let cancelableApiReq = makeCancelable(api.trial.getNotAssociatedObservationUnits(
      graphqlServerUrl,
      item.trialDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trial', association: 'observationUnits', table:'A', method: 'getData()', request: 'api.trial.getNotAssociatedObservationUnits'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'add', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trial.getNotAssociatedObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
