...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getContactsCount
    */
    let cancelableApiReq = makeCancelable(api.trial.getContactsCount(
      graphqlServerUrl, 
      item.trialDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'contacts', method: 'getCount()', request: 'api.trial.getContactsCount'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'contacts'];
      ...
      //error
      .catch((err) => { //error: on api.trial.getContactsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getContacts
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.trial.getContacts(
      graphqlServerUrl, 
      item.trialDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'contacts', method: 'getData()', request: 'api.trial.getContacts'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trial.getContacts
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trial', association: 'contacts', table:'B', method: 'getCountB()', request: 'api.contact.getCountItems'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'contacts'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
        ...
        (response) => {
          ...
          if(response.message === 'ok') {
            ...
            if(response.graphqlErrors) {
              ...
              newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'trial', association: 'contacts', table:'B', method: 'getDataB()', request: 'api.contact.getItems'}];
              newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'contacts'];
              newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trial&&
        ...
        lastModelChanged.trial[String(item.trialDbId)].changedAssociations.contact_triaDbIds&&
        (lastModelChanged.trial[String(item.trialDbId)].changedAssociations.contact_triaDbIds.added ||
         lastModelChanged.trial[String(item.trialDbId)].changedAssociations.contact_triaDbIds.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.contact_triaDbIds.idsRemoved;
          if(idsRemoved) {
          ...
          let idsAdded = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.contact_triaDbIds.idsAdded;
          if(idsAdded) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'contact_triaDbIds'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.contact) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.contact_triaDbIds) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.contact_triaDbIds.removed) {
              let idsRemoved = entry[1].changedAssociations.contact_triaDbIds.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.contact_triaDbIds.added) {
              let idsAdded = entry[1].changedAssociations.contact_triaDbIds.idsAdded;
              if(idsAdded) {
