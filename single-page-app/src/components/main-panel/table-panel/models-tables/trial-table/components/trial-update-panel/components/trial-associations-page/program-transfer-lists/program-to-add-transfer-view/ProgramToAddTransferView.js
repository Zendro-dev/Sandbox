...
export default function ProgramToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.trialDbId;

    /*
      API Request: api.trial.getProgram
    */
    let api = await loadApi("trial");
    let cancelableApiReq = makeCancelable(api.trial.getProgram(graphqlServerUrl, item.trialDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'trial', association: 'program', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.trial.getProgram'}];
            newError.path=['update', `trialDbId:${currentId}`, 'add', 'program'];
      ...
      //error
      .catch((err) => { //error: on api.trial.getProgram
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.trialDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getNotAssociatedProgramCount
    */
    let api = await loadApi("trial");
    let cancelableApiReq = makeCancelable(api.trial.getNotAssociatedProgramCount(graphqlServerUrl, item.trialDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trial', association: 'program', table:'A', method: 'getCount()', request: 'api.trial.getNotAssociatedProgramCount'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'add', 'program'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trial.getNotAssociatedProgramCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.trialDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getNotAssociatedProgram
    */
    let api = await loadApi("trial");
    let cancelableApiReq = makeCancelable(api.trial.getNotAssociatedProgram(
      graphqlServerUrl,
      item.trialDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trial', association: 'program', table:'A', method: 'getData()', request: 'api.trial.getNotAssociatedProgram'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'add', 'program'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trial.getNotAssociatedProgram
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.trialDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trial', association: 'program', table:'B', method: 'getDataB()', request: 'api.program.getItems'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'add', 'program'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.trialDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
     ...
     * - return
     */
    if(lastModelChanged.trial&&
        lastModelChanged.trial[String(item.trialDbId)]&&
        lastModelChanged.trial[String(item.trialDbId)].changedAssociations&&
        lastModelChanged.trial[String(item.trialDbId)].changedAssociations.trial_programDbId&&
        (lastModelChanged.trial[String(item.trialDbId)].changedAssociations.trial_programDbId.added ||
         lastModelChanged.trial[String(item.trialDbId)].changedAssociations.trial_programDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.trial_programDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.trial_programDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'trial_programDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.program) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.trial_programDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.trial_programDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.trial_programDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.trialDbId);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('program', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.trial_programDbId.added) {
              let idsAdded = entry[1].changedAssociations.trial_programDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.trialDbId);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.trialDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
