...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getObservationUnitsCount
    */
    let cancelableApiReq = makeCancelable(api.trial.getObservationUnitsCount(
      graphqlServerUrl, 
      item.trialDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationUnits', method: 'getCount()', request: 'api.trial.getObservationUnitsCount'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.trial.getObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.trial.getObservationUnits
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.trial.getObservationUnits(
      graphqlServerUrl, 
      item.trialDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationUnits', method: 'getData()', request: 'api.trial.getObservationUnits'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.trial.getObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'trial', association: 'observationUnits', table:'B', method: 'getCountB()', request: 'api.observationUnit.getCountItems'}];
            newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
        ...
        (response) => {
          ...
          if(response.message === 'ok') {
            ...
            if(response.graphqlErrors) {
              ...
              newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'trial', association: 'observationUnits', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
              newError.path=['update', `trialDbId:${item.trialDbId}`, 'remove', 'observationUnits'];
              newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.trial&&
        ...
        lastModelChanged.trial[String(item.trialDbId)].changedAssociations.observationUnit_trialDbId&&
        (lastModelChanged.trial[String(item.trialDbId)].changedAssociations.observationUnit_trialDbId.added ||
         lastModelChanged.trial[String(item.trialDbId)].changedAssociations.observationUnit_trialDbId.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.observationUnit_trialDbId.idsRemoved;
          if(idsRemoved) {
          ...
          let idsAdded = lastModelChanged.trial[String(item.trialDbId)].changedAssociations.observationUnit_trialDbId.idsAdded;
          if(idsAdded) {
            //increment A
