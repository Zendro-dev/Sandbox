...
export default function ObservationUnitToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.imageDbId;

    /*
      API Request: api.image.getObservationUnit
    */
    let api = await loadApi("image");
    let cancelableApiReq = makeCancelable(api.image.getObservationUnit(graphqlServerUrl, item.imageDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'image', association: 'observationUnit', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.image.getObservationUnit'}];
            newError.path=['update', `imageDbId:${currentId}`, 'add', 'observationUnit'];
      ...
      //error
      .catch((err) => { //error: on api.image.getObservationUnit
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.imageDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.image.getNotAssociatedObservationUnitCount
    */
    let api = await loadApi("image");
    let cancelableApiReq = makeCancelable(api.image.getNotAssociatedObservationUnitCount(graphqlServerUrl, item.imageDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'image', association: 'observationUnit', table:'A', method: 'getCount()', request: 'api.image.getNotAssociatedObservationUnitCount'}];
            newError.path=['update', `imageDbId:${item.imageDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.image.getNotAssociatedObservationUnitCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.image.getNotAssociatedObservationUnit
    */
    let api = await loadApi("image");
    let cancelableApiReq = makeCancelable(api.image.getNotAssociatedObservationUnit(
      graphqlServerUrl,
      item.imageDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'image', association: 'observationUnit', table:'A', method: 'getData()', request: 'api.image.getNotAssociatedObservationUnit'}];
            newError.path=['update', `imageDbId:${item.imageDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.image.getNotAssociatedObservationUnit
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.imageDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'image', association: 'observationUnit', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
            newError.path=['update', `imageDbId:${item.imageDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.imageDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.image&&
        ...
        lastModelChanged.image[String(item.imageDbId)].changedAssociations.image_observationUnitDbId&&
        (lastModelChanged.image[String(item.imageDbId)].changedAssociations.image_observationUnitDbId.added ||
         lastModelChanged.image[String(item.imageDbId)].changedAssociations.image_observationUnitDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.image[String(item.imageDbId)].changedAssociations.image_observationUnitDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.image[String(item.imageDbId)].changedAssociations.image_observationUnitDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.imageDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
