...
//lazy loading
const ImageAttributesPage = lazy(() => import(/* webpackChunkName: "Create-AttributesImage" */ './components/image-attributes-page/ImageAttributesPage'));
const ImageAssociationsPage = lazy(() => import(/* webpackChunkName: "Create-AssociationsImage" */ './components/image-associations-page/ImageAssociationsPage'));
const ImageConfirmationDialog = lazy(() => import(/* webpackChunkName: "Create-ConfirmationImage" */ './components/ImageConfirmationDialog'));
...
export default function ImageCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.copyright = null;
    initialValues.description = null;
    initialValues.imageFileName = null;
    initialValues.imageFileSize = null;
    initialValues.imageHeight = null;
    initialValues.imageName = null;
    initialValues.imageTimeStamp = null;
    initialValues.imageURL = null;
    initialValues.imageWidth = null;
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.copyright = 0;
    initialValueOkStates.description = 0;
    initialValueOkStates.imageFileName = 0;
    initialValueOkStates.imageFileSize = 0;
    initialValueOkStates.imageHeight = 0;
    initialValueOkStates.imageName = 0;
    initialValueOkStates.imageTimeStamp = 0;
    initialValueOkStates.imageURL = 0;
    initialValueOkStates.imageWidth = 0;
    initialValueOkStates.mimeType = 0;
    initialValueOkStates.observationUnitDbId = -2; //FK
    initialValueOkStates.imageDbId = 0;
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.image.createItem
    */
    let api = await loadApi("image");
    let cancelableApiReq = makeCancelable(api.image.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'image', method: 'doSave()', request: 'api.image.createItem'}];
            newError.path=['Images', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.image.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  return (
    ...
    <Dialog id='ImageCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ImageCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Image'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " image" }>
            <Fab
              id='ImageCreatePanel-fabButton-save' 
              color="secondary" 
