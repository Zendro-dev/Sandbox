...
export default function ObservationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.image.getNotAssociatedObservationsCount
    */
    let api = await loadApi("image");
    let cancelableApiReq = makeCancelable(api.image.getNotAssociatedObservationsCount(graphqlServerUrl, item.imageDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'image', association: 'observations', table:'A', method: 'getCount()', request: 'api.image.getNotAssociatedObservationsCount'}];
            newError.path=['update', `imageDbId:${item.imageDbId}`, 'add', 'observations'];
      ...
      //error
      .catch((err) => { //error: on api.image.getNotAssociatedObservationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.imageDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.image.getNotAssociatedObservations
    */
    let api = await loadApi("image");
    let cancelableApiReq = makeCancelable(api.image.getNotAssociatedObservations(
      graphqlServerUrl,
      item.imageDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'image', association: 'observations', table:'A', method: 'getData()', request: 'api.image.getNotAssociatedObservations'}];
            newError.path=['update', `imageDbId:${item.imageDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.image.getNotAssociatedObservations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.imageDbId, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'image', association: 'observations', table:'B', method: 'getCountB()', request: 'api.observation.getCountItems'}];
            newError.path=['update', `imageDbId:${item.imageDbId}`, 'add', 'observations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.imageDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'image', association: 'observations', table:'B', method: 'getDataB()', request: 'api.observation.getItems'}];
            newError.path=['update', `imageDbId:${item.imageDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.imageDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.image&&
        ...
        lastModelChanged.image[String(item.imageDbId)].changedAssociations&&
        lastModelChanged.image[String(item.imageDbId)].changedAssociations.observation_imageDbId&&
        (lastModelChanged.image[String(item.imageDbId)].changedAssociations.observation_imageDbId.added ||
         lastModelChanged.image[String(item.imageDbId)].changedAssociations.observation_imageDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.image[String(item.imageDbId)].changedAssociations.observation_imageDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.image[String(item.imageDbId)].changedAssociations.observation_imageDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observation_imageDbId) {
            ...
            if(entry[1].changedAssociations.observation_imageDbId.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.imageDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.observation_imageDbId.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.imageDbId);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.imageDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
