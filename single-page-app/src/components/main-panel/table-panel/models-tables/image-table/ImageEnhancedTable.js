...
import ImageEnhancedTableHead from './components/ImageEnhancedTableHead'
import ImageEnhancedTableToolbar from './components/ImageEnhancedTableToolbar'
import ImageUploadFileDialog from './components/ImageUploadFileDialog'
import ImageCursorPagination from './components/ImageCursorPagination'
...
//lazy loading
const ImageCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Image" */ './components/image-create-panel/ImageCreatePanel'));
const ImageUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Image" */ './components/image-update-panel/ImageUpdatePanel'));
const ImageDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Image" */ './components/image-detail-panel/ImageDetailPanel'));
const ImageDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Image" */ './components/ImageDeleteConfirmationDialog'));
//Plotly
const ImagePlotly = lazy(() => import(/* webpackChunkName: "Plotly-Image" */ '../../../../plots/ImagePlotly'));
...
export default function ImageEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('imageDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.image.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'image', method: 'getCount()', request: 'api.image.getCountItems'}];
            newError.path=['Images'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'image', method: 'getData()', request: 'api.image.getItems'}];
            newError.path=['Images'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['image']
    &&(Object.keys(changes.lastModelChanged['image']).length===1)
    &&(changes.lastModelChanged['image'][Object.keys(changes.lastModelChanged['image'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.image.tableTemplate
    */
    let api = await loadApi("image");
    let cancelableApiReq = makeCancelable(api.image.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'image', method: 'getCsvTemplate()', request: 'api.image.tableTemplate'}];
            newError.path=['Images', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateImage.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "image-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.image.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ImageEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ImageEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ImageEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ImageEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ImageEnhancedTable-row-'+item.imageDbId}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ImageEnhancedTable-row-iconButton-detail-'+item.imageDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ImageEnhancedTable-row-iconButton-edit-'+item.imageDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ImageEnhancedTable-row-iconButton-delete-'+item.imageDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'imageDbId: ' + item.imageDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.imageDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.copyright!==null)?item.copyright:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.imageFileName!==null)?item.imageFileName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.imageFileSize!==null)?item.imageFileSize:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.imageHeight!==null)?item.imageHeight:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.imageName!==null)?item.imageName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.imageTimeStamp!==null)?moment(item.imageTimeStamp, "YYYY-MM-DDTHH:mm:ss.SSSZ").format("YYYY-MM-DDTHH:mm:ss.SSSZ"):'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.imageURL!==null)?item.imageURL:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.imageWidth!==null)?item.imageWidth:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.mimeType!==null)?item.mimeType:'')}
                                    </TableCell>
