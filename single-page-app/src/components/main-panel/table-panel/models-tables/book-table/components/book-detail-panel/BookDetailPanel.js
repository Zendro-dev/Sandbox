...
//lazy loading
const BookAttributesPage = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Attributes-Book" */ './components/book-attributes-page/BookAttributesPage')));
const BookAssociationsPage = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Associations-Book" */ './components/book-associations-page/BookAssociationsPage')));
const BookUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Update-Book" */ '../book-update-panel/BookUpdatePanel')));
const BookDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Delete-Book" */ '../BookDeleteConfirmationDialog')));
const AuthorDetailPanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Detail-Author" */ '../../../author-table/components/author-detail-panel/AuthorDetailPanel')));
...
export default function BookDetailPanel(props) {
      ...
      lastModelChanged['book'][String(item['book_id'])]) {
        ...
        if(lastModelChanged['book'][String(item['book_id'])].op === "update"&&
            lastModelChanged['book'][String(item['book_id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['book'][String(item['book_id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('book', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('book', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='BookDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Book | book_id: " + itemState['book_id'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='BookDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='BookDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
