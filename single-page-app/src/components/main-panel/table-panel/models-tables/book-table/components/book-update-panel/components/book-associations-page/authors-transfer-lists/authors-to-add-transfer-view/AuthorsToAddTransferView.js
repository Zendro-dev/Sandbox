...
export default function AuthorsToAddTransferView(props) {
  ...
  */
  ...
  }, [graphqlServerUrl, showMessageE, t, item.book_id]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"author_id": lidsToAdd.current}
        }]
      };
    }    
  ...
  }, [graphqlServerUrl, showMessageC, t, item.book_id, search]);
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.book_id, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.author.getItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.author.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      //error
      .catch((err) => { //error: on api.author.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.book_id, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.book&&
        ...
        lastModelChanged.book[String(item.book_id)].changedAssociations&&
        lastModelChanged.book[String(item.book_id)].changedAssociations.book_fk_books_authors&&
        (lastModelChanged.book[String(item.book_id)].changedAssociations.book_fk_books_authors.added ||
         lastModelChanged.book[String(item.book_id)].changedAssociations.book_fk_books_authors.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.book[String(item.book_id)].changedAssociations.book_fk_books_authors.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.book[String(item.book_id)].changedAssociations.book_fk_books_authors.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'author'.
     * B: the current item internalId is in the addedIds of the updated 'author'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'author' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.author) {
      let oens = Object.entries(lastModelChanged.author);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.book_fk_books_authors) {
            ...
            if(entry[1].changedAssociations.book_fk_books_authors.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.book_id);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('authors', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.book_fk_books_authors.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.book_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.author_id;
    ...
    /*
     * Case 3: 
     * The attributes of some 'author' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.author) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.author_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.author_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.author_id===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.author_id===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.author_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.author_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.book_id, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnAuthorRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.author_id) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.author_id);
                    ...
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.author_id===it.author_id) ? 'author — ' + t('modelPanels.associatedRecord', 'Associated record') : 'author '}>
                                <Avatar>
                                  {"author".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    ...
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.author_id===it.author_id) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.author_id===it.author_id) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                    ...
                    <List id='AuthorsToAddTransferView-associatedItem-list'
                      ...
                      {[associatedItem].map(associatedItem => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'author — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"author".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <Checkbox
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].author_id===lidsToAdd.current[0]) {
