...
import BookEnhancedTableHead from './components/BookEnhancedTableHead'
import BookEnhancedTableToolbar from './components/BookEnhancedTableToolbar'
import BookUploadFileDialog from './components/BookUploadFileDialog'
import BookCursorPagination from './components/BookCursorPagination'
...
//lazy loading
const BookCreatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Create-Book" */ './components/book-create-panel/BookCreatePanel')));
const BookUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Update-Book" */ './components/book-update-panel/BookUpdatePanel')));
const BookDetailPanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Book" */ './components/book-detail-panel/BookDetailPanel')));
const BookDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Delete-Book" */ './components/BookDeleteConfirmationDialog')));
...
export default function BookEnhancedTable(props) {
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.book.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'book', method: 'getCount()', request: 'api.book.getCountItems'}];
            newError.path=['Books'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'book', method: 'getData()', request: 'api.book.getItems'}];
            newError.path=['Books'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['book']
    &&(Object.keys(changes.lastModelChanged['book']).length===1)
    &&(changes.lastModelChanged['book'][Object.keys(changes.lastModelChanged['book'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set book_id (internalId)
    variables.book_id = item.book_id;

    /*
      API Request: api.book.deleteItem
    */
    let api = await loadApi("book");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.book.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'book', method: 'doDelete()', request: 'api.book.deleteItem'}];
            newError.path=['Books', `book_id:${item.book_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.book.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.book.tableTemplate
    */
    let api = await loadApi("book");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.book.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'book', method: 'getCsvTemplate()', request: 'api.book.tableTemplate'}];
            newError.path=['Books', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        link.href = url;
        link.setAttribute("download", "book-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.book.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='BookEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='BookEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <BookEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='BookEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'BookEnhancedTable-row-'+item.book_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'BookEnhancedTable-row-iconButton-detail-'+item.book_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.book&&Array.isArray(permissions.book)
                                      &&(permissions.book.includes('update') || permissions.book.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'BookEnhancedTable-row-iconButton-edit-'+item.book_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.book&&Array.isArray(permissions.book)
                                      &&(permissions.book.includes('delete') || permissions.book.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'BookEnhancedTable-row-iconButton-delete-'+item.book_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'book_id: ' + item.book_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.book_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.fk_books_authors!==null)?item.fk_books_authors:'')}
                                    </TableCell>
