...
export default function CountriesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.river.getNotAssociatedCountriesCount
    */
    ...
    let cancelableApiReq = makeCancelable(api.river.getNotAssociatedCountriesCount(graphqlServerUrl, item.river_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'river', association: 'countries', table:'A', method: 'getCount()', request: 'api.river.getNotAssociatedCountriesCount'}];
            newError.path=['update', `river_id:${item.river_id}`, 'add', 'countries'];
      ...
      //error
      .catch((err) => { //error: on api.river.getNotAssociatedCountriesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.river_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.river.getNotAssociatedCountries
    */
    let api = await loadApi("river");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.river.getNotAssociatedCountries(
      graphqlServerUrl,
      item.river_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'river', association: 'countries', table:'A', method: 'getData()', request: 'api.river.getNotAssociatedCountries'}];
            newError.path=['update', `river_id:${item.river_id}`, 'add', 'countries'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.river.getNotAssociatedCountries
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.river_id, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'river', association: 'countries', table:'B', method: 'getCountB()', request: 'api.country.getCountItems'}];
            newError.path=['update', `river_id:${item.river_id}`, 'add', 'countries'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsD.current.push(newError);
            console.log("Error: ", newError);
  ...
  }, [graphqlServerUrl, showMessageD, t, item.river_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'river', association: 'countries', table:'B', method: 'getDataB()', request: 'api.country.getItems'}];
            newError.path=['update', `river_id:${item.river_id}`, 'add', 'countries'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsB.current.push(newError);
            console.log("Error: ", newError);
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.river_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
     ...
     * - return
     */
    if(lastModelChanged.river&&
        lastModelChanged.river[String(item.river_id)]&&
        lastModelChanged.river[String(item.river_id)].changedAssociations&&
        lastModelChanged.river[String(item.river_id)].changedAssociations.country_river_ids&&
        (lastModelChanged.river[String(item.river_id)].changedAssociations.country_river_ids.added ||
         lastModelChanged.river[String(item.river_id)].changedAssociations.country_river_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.river[String(item.river_id)].changedAssociations.country_river_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.river[String(item.river_id)].changedAssociations.country_river_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'country_river_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.country) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.country_river_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.country_river_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.country_river_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.river_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.country_river_ids.added) {
              let idsAdded = entry[1].changedAssociations.country_river_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.river_id);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.river_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
