...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.river.getCountriesCount
    */
    ...
    let cancelableApiReq = makeCancelable(api.river.getCountriesCount(
      graphqlServerUrl, 
      item.river_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'countries', method: 'getCount()', request: 'api.river.getCountriesCount'}];
            newError.path=['update', `river_id:${item.river_id}`, 'remove', 'countries'];
      ...
      //error
      .catch((err) => { //error: on api.river.getCountriesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.river.getCountries
    */
    let variables = {
    ...
    let cancelableApiReq = makeCancelable(api.river.getCountries(
      graphqlServerUrl, 
      item.river_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'countries', method: 'getData()', request: 'api.river.getCountries'}];
            newError.path=['update', `river_id:${item.river_id}`, 'remove', 'countries'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.river.getCountries
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.river_id, search, getCount, configurePagination, onEmptyPage]);
