...
//lazy loading
const RiverAttributesPage = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Attributes-River" */ './components/river-attributes-page/RiverAttributesPage')));
const RiverAssociationsPage = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Associations-River" */ './components/river-associations-page/RiverAssociationsPage')));
const RiverUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Update-River" */ '../river-update-panel/RiverUpdatePanel')));
const RiverDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Delete-River" */ '../RiverDeleteConfirmationDialog')));
...
export default function RiverDetailPanel(props) {
      ...
      lastModelChanged['river'][String(item['river_id'])]) {
        ...
        if(lastModelChanged['river'][String(item['river_id'])].op === "update"&&
            lastModelChanged['river'][String(item['river_id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['river'][String(item['river_id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('river', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('river', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='RiverDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": River | river_id: " + itemState['river_id'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='RiverDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='RiverDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
