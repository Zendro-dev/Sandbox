...
import RiverEnhancedTableHead from './components/RiverEnhancedTableHead'
import RiverEnhancedTableToolbar from './components/RiverEnhancedTableToolbar'
import RiverUploadFileDialog from './components/RiverUploadFileDialog'
import RiverCursorPagination from './components/RiverCursorPagination'
...
//lazy loading
const RiverCreatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Create-River" */ './components/river-create-panel/RiverCreatePanel')));
const RiverUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Update-River" */ './components/river-update-panel/RiverUpdatePanel')));
const RiverDetailPanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-River" */ './components/river-detail-panel/RiverDetailPanel')));
const RiverDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Delete-River" */ './components/RiverDeleteConfirmationDialog')));
...
export default function RiverEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('river_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.river.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'river', method: 'getCount()', request: 'api.river.getCountItems'}];
            newError.path=['Rivers'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'river', method: 'getData()', request: 'api.river.getItems'}];
            newError.path=['Rivers'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['river']
    &&(Object.keys(changes.lastModelChanged['river']).length===1)
    &&(changes.lastModelChanged['river'][Object.keys(changes.lastModelChanged['river'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set river_id (internalId)
    variables.river_id = item.river_id;

    /*
      API Request: api.river.deleteItem
    */
    let api = await loadApi("river");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.river.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'river', method: 'doDelete()', request: 'api.river.deleteItem'}];
            newError.path=['Rivers', `river_id:${item.river_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.river.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.river.tableTemplate
    */
    let api = await loadApi("river");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.river.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'river', method: 'getCsvTemplate()', request: 'api.river.tableTemplate'}];
            newError.path=['Rivers', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        link.href = url;
        link.setAttribute("download", "river-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.river.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='RiverEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='RiverEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <RiverEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='RiverEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'RiverEnhancedTable-row-'+item.river_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'RiverEnhancedTable-row-iconButton-detail-'+item.river_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.river&&Array.isArray(permissions.river)
                                      &&(permissions.river.includes('update') || permissions.river.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'RiverEnhancedTable-row-iconButton-edit-'+item.river_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.river&&Array.isArray(permissions.river)
                                      &&(permissions.river.includes('delete') || permissions.river.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'RiverEnhancedTable-row-iconButton-delete-'+item.river_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'river_id: ' + item.river_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.river_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.length!==null)?item.length:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.country_ids!==null)?item.country_ids:'')}
                                    </TableCell>
