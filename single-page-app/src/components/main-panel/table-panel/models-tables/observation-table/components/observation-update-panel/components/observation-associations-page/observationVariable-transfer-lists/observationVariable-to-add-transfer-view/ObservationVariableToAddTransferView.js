...
import ObservationVariableToAddTransferViewToolbar from './components/ObservationVariableToAddTransferViewToolbar';
import ObservationVariableToAddTransferViewCursorPagination from './components/ObservationVariableToAddTransferViewCursorPagination';
...
export default function ObservationVariableToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.observationDbId;

    /*
      API Request: api.observation.getObservationVariable
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getObservationVariable(graphqlServerUrl, item.observationDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observation', association: 'observationVariable', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observation.getObservationVariable'}];
            newError.path=['update', `observationDbId:${currentId}`, 'add', 'observationVariable'];
      ...
      //error
      .catch((err) => { //error: on api.observation.getObservationVariable
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.observationDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"observationVariableDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observation.getNotAssociatedObservationVariableCount
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedObservationVariableCount(graphqlServerUrl, item.observationDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'observationVariable', table:'A', method: 'getCount()', request: 'api.observation.getNotAssociatedObservationVariableCount'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'observationVariable'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedObservationVariableCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.observationDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getNotAssociatedObservationVariable
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedObservationVariable(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'observationVariable', table:'A', method: 'getData()', request: 'api.observation.getNotAssociatedObservationVariable'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'observationVariable'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedObservationVariable
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.observationDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getItems
    */
    let api = await loadApi("observationVariable");
    let cancelableApiReq = makeCancelable(api.observationVariable.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'observationVariable', table:'B', method: 'getDataB()', request: 'api.observationVariable.getItems'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'observationVariable'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.observationDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'observationVariable' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'observationVariables', 
     ...
     * - return
     */
    if(lastModelChanged.observation&&
        lastModelChanged.observation[String(item.observationDbId)]&&
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations&&
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationVariableDbId&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationVariableDbId.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationVariableDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationVariableDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('observationVariable', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('observationVariable', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationVariableDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('observationVariable', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'observationVariable' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'observationVariable',
     * but this action happened on the peer relation, identified by 'observation_observationVariableDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'observationVariable'.
     * B: the current item internalId is in the addedIds of the updated 'observationVariable'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'observationVariable' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.observationVariable) {
      let oens = Object.entries(lastModelChanged.observationVariable);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observation_observationVariableDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observation_observationVariableDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observation_observationVariableDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observation_observationVariableDbId.added) {
              let idsAdded = entry[1].changedAssociations.observation_observationVariableDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.observationDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.observationVariableDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('observationVariable', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'observationVariable' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.observationVariableDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.observationVariableDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.observationVariableDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.observationVariableDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.observationVariableDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.observationVariableDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('observationVariable', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.observationVariableDbId===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnObservationVariableRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.observationVariableDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.observationVariableDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.observationVariableDbId];
        handleDisassociateItem('observationVariable', associatedItem.observationVariableDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('observationVariable', item.observationVariableDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('observationVariable', item.observationVariableDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationVariableToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariableToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationVariableToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.observationVariableDbId;
                        let label = it.observationVariableName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'ObservationVariableToAddTransferView-listA-listItem-'+it.observationVariableDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.observationVariableDbId===it.observationVariableDbId) ? 'observationVariable — ' + t('modelPanels.associatedRecord', 'Associated record') : 'observationVariable '}>
                                <Avatar>
                                  {"observationVariable".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                            ...
                            <ListItemText
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationVariableDbId' }>
                                        <Typography 
                                        id={'ObservationVariableToAddTransferView-listA-listItem-id-'+it.observationVariableDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationVariableDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.observationVariableDbId===it.observationVariableDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.observationVariableDbId===it.observationVariableDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                              ...
                              secondary={
                                <React.Fragment>
                                  {/* Label */}
                                  {(label) && (
                                    <Tooltip title={ 'observationVariableName' }>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ObservationVariableToAddTransferView-listA-listItem-'+it.observationVariableDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ObservationVariableToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariableToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariableToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationVariableToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationVariableDbId;
                        let label = it.observationVariableName;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationVariableToAddTransferView-listB-listItem-'+it.observationVariableDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationVariableDbId' }>
                                        <Typography 
                                        id={'ObservationVariableToAddTransferView-listB-listItem-id-'+it.observationVariableDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationVariableDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ObservationVariableToAddTransferView-listB-listItem-'+it.observationVariableDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='ObservationVariableToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.observationVariableDbId;
                        let label = associatedItem.observationVariableName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'ObservationVariableToAddTransferView-associatedItem-list-listItem-'+associatedItem.observationVariableDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'observationVariable — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"observationVariable".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationVariableDbId' }>
                                        <Typography 
                                        id={'ObservationVariableToAddTransferView-listB-listItem-id-'+associatedItem.observationVariableDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.observationVariableDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'ObservationVariableToAddTransferView-associatedItem-list-listItem-'+associatedItem.observationVariableDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].observationVariableDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('observationVariable', associatedItem.observationVariableDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.observationVariableDbId];
                                      handleDisassociateItem('observationVariable', associatedItem.observationVariableDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationVariableToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
ObservationVariableToAddTransferView.propTypes = {
