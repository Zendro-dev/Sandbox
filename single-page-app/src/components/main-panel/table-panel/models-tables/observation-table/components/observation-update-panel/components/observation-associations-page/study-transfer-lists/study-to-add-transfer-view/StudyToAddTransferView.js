...
export default function StudyToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getStudy
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getStudy(graphqlServerUrl, item.observationDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observation', association: 'study', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observation.getStudy'}];
            newError.path=['update', `observationDbId:${currentId}`, 'add', 'study'];
      ...
      //error
      .catch((err) => { //error: on api.observation.getStudy
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getNotAssociatedStudyCount
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedStudyCount(graphqlServerUrl, item.observationDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'study', table:'A', method: 'getCount()', request: 'api.observation.getNotAssociatedStudyCount'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedStudyCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getNotAssociatedStudy
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedStudy(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'study', table:'A', method: 'getData()', request: 'api.observation.getNotAssociatedStudy'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedStudy
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'study', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_studyDbId&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_studyDbId.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_studyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_studyDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_studyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'observation_studyDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observation_studyDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observation_studyDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observation_studyDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observation_studyDbId.added) {
              let idsAdded = entry[1].changedAssociations.observation_studyDbId.idsAdded;
              if(idsAdded) {
