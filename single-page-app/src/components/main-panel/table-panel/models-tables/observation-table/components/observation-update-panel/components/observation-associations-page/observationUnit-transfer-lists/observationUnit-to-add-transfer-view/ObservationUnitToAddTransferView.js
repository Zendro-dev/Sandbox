...
export default function ObservationUnitToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.observation.getObservationUnit(graphqlServerUrl, item.observationDbId));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observation', association: 'observationUnit', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observation.getObservationUnit'}];
            newError.path=['update', `observationDbId:${currentId}`, 'add', 'observationUnit'];
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getNotAssociatedObservationUnitCount
    */
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedObservationUnitCount(graphqlServerUrl, item.observationDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'observationUnit', table:'A', method: 'getCount()', request: 'api.observation.getNotAssociatedObservationUnitCount'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedObservationUnitCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getNotAssociatedObservationUnit
    */
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedObservationUnit(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'observationUnit', table:'A', method: 'getData()', request: 'api.observation.getNotAssociatedObservationUnit'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedObservationUnit
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'observationUnit', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationUnitDbId&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationUnitDbId.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationUnitDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationUnitDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_observationUnitDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
