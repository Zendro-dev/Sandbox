...
import ImageToAddTransferViewToolbar from './components/ImageToAddTransferViewToolbar';
import ImageToAddTransferViewCursorPagination from './components/ImageToAddTransferViewCursorPagination';
...
export default function ImageToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getImage
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getImage(graphqlServerUrl, item.observationDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observation', association: 'image', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observation.getImage'}];
            newError.path=['update', `observationDbId:${currentId}`, 'add', 'image'];
      ...
      //error
      .catch((err) => { //error: on api.observation.getImage
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"imageDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observation.getNotAssociatedImageCount
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedImageCount(graphqlServerUrl, item.observationDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'image', table:'A', method: 'getCount()', request: 'api.observation.getNotAssociatedImageCount'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'image'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedImageCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getNotAssociatedImage
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedImage(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'image', table:'A', method: 'getData()', request: 'api.observation.getNotAssociatedImage'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'image'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedImage
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'image', table:'B', method: 'getDataB()', request: 'api.image.getItems'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'image'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'image' for this item was updated.
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_imageDbId&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_imageDbId.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_imageDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_imageDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('image', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('image', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_imageDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('image', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'image' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'image',
     * but this action happened on the peer relation, identified by 'observation_imageDbId'.
     * 
     * Conditions:
     ...
     * 
     * else if B:
     * - remove the internalId of the 'image' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.image) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observation_imageDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observation_imageDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observation_imageDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observation_imageDbId.added) {
              let idsAdded = entry[1].changedAssociations.observation_imageDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.observationDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.imageDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('image', idAdded);
                  } 
    ...
    if(lastModelChanged.image) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          ...
          if(associatedItem && associatedItem.imageDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('image', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.imageDbId===idRemoved) {
            //update associated item
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.imageDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.imageDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.imageDbId];
        handleDisassociateItem('image', associatedItem.imageDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('image', item.imageDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('image', item.imageDbId);
    }
  };
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ImageToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ImageToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ImageToAddTransferView-listA-listItem-'+it.imageDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.imageDbId===it.imageDbId) ? 'image — ' + t('modelPanels.associatedRecord', 'Associated record') : 'image '}>
                                <Avatar>
                                  {"image".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'imageDbId' }>
                                        <Typography 
                                        id={'ImageToAddTransferView-listA-listItem-id-'+it.imageDbId}
                                        variant="body1" display="block" noWrap={true}>{it.imageDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.imageDbId===it.imageDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.imageDbId===it.imageDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ImageToAddTransferView-listA-listItem-'+it.imageDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ImageToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ImageToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ImageToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ImageToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.imageDbId;
                        let label = it.imageName;
                        ...
                        return (
                          <ListItem 
                            id={'ImageToAddTransferView-listB-listItem-'+it.imageDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'imageDbId' }>
                                        <Typography 
                                        id={'ImageToAddTransferView-listB-listItem-id-'+it.imageDbId}
                                        variant="body1" display="block" noWrap={true}>{it.imageDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ImageToAddTransferView-listB-listItem-'+it.imageDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='ImageToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.imageDbId;
                        let label = associatedItem.imageName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'ImageToAddTransferView-associatedItem-list-listItem-'+associatedItem.imageDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'image — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"image".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'imageDbId' }>
                                        <Typography 
                                        id={'ImageToAddTransferView-listB-listItem-id-'+associatedItem.imageDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.imageDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'ImageToAddTransferView-associatedItem-list-listItem-'+associatedItem.imageDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].imageDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('image', associatedItem.imageDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.imageDbId];
                                      handleDisassociateItem('image', associatedItem.imageDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ImageToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
ImageToAddTransferView.propTypes = {
