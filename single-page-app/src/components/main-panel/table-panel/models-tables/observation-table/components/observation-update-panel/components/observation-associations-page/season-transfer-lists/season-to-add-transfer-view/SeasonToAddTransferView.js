...
import SeasonToAddTransferViewToolbar from './components/SeasonToAddTransferViewToolbar';
import SeasonToAddTransferViewCursorPagination from './components/SeasonToAddTransferViewCursorPagination';
...
export default function SeasonToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getSeason
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getSeason(graphqlServerUrl, item.observationDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observation', association: 'season', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observation.getSeason'}];
            newError.path=['update', `observationDbId:${currentId}`, 'add', 'season'];
      ...
      //error
      .catch((err) => { //error: on api.observation.getSeason
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"seasonDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observation.getNotAssociatedSeasonCount
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedSeasonCount(graphqlServerUrl, item.observationDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'season', table:'A', method: 'getCount()', request: 'api.observation.getNotAssociatedSeasonCount'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'season'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedSeasonCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getNotAssociatedSeason
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedSeason(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'season', table:'A', method: 'getData()', request: 'api.observation.getNotAssociatedSeason'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'season'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedSeason
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.season.getItems
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'season', table:'B', method: 'getDataB()', request: 'api.season.getItems'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'season'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.season.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'season' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'seasons', 
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_seasonDbId&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_seasonDbId.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_seasonDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_seasonDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('season', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('season', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_seasonDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('season', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'season' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'season',
     * but this action happened on the peer relation, identified by 'observation_seasonDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'season'.
     * B: the current item internalId is in the addedIds of the updated 'season'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'season' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.season) {
      let oens = Object.entries(lastModelChanged.season);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observation_seasonDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observation_seasonDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observation_seasonDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observation_seasonDbId.added) {
              let idsAdded = entry[1].changedAssociations.observation_seasonDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.observationDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.seasonDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('season', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'season' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.season) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.seasonDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.seasonDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.seasonDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.seasonDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.seasonDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.seasonDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('season', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.seasonDbId===idRemoved) {
            //update associated item
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnSeasonRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.seasonDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.seasonDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.seasonDbId];
        handleDisassociateItem('season', associatedItem.seasonDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('season', item.seasonDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('season', item.seasonDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <SeasonToAddTransferViewToolbar
                title={'Seasons'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='SeasonToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.seasonDbId;
                        let label = it.season;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'SeasonToAddTransferView-listA-listItem-'+it.seasonDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.seasonDbId===it.seasonDbId) ? 'season — ' + t('modelPanels.associatedRecord', 'Associated record') : 'season '}>
                                <Avatar>
                                  {"season".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'seasonDbId' }>
                                        <Typography 
                                        id={'SeasonToAddTransferView-listA-listItem-id-'+it.seasonDbId}
                                        variant="body1" display="block" noWrap={true}>{it.seasonDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.seasonDbId===it.seasonDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.seasonDbId===it.seasonDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'SeasonToAddTransferView-listA-listItem-'+it.seasonDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <SeasonToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='SeasonToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.seasonDbId;
                        let label = it.season;
                        ...
                        return (
                          <ListItem 
                            id={'SeasonToAddTransferView-listB-listItem-'+it.seasonDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'seasonDbId' }>
                                        <Typography 
                                        id={'SeasonToAddTransferView-listB-listItem-id-'+it.seasonDbId}
                                        variant="body1" display="block" noWrap={true}>{it.seasonDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'SeasonToAddTransferView-listB-listItem-'+it.seasonDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='SeasonToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.seasonDbId;
                        let label = associatedItem.season;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'SeasonToAddTransferView-associatedItem-list-listItem-'+associatedItem.seasonDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'season — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"season".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'seasonDbId' }>
                                        <Typography 
                                        id={'SeasonToAddTransferView-listB-listItem-id-'+associatedItem.seasonDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.seasonDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'SeasonToAddTransferView-associatedItem-list-listItem-'+associatedItem.seasonDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].seasonDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('season', associatedItem.seasonDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.seasonDbId];
                                      handleDisassociateItem('season', associatedItem.seasonDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SeasonToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
SeasonToAddTransferView.propTypes = {
