...
export default function GermplasmToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getGermplasm
    */
    let cancelableApiReq = makeCancelable(api.observation.getGermplasm(graphqlServerUrl, item.observationDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observation', association: 'germplasm', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observation.getGermplasm'}];
            newError.path=['update', `observationDbId:${currentId}`, 'add', 'germplasm'];
      ...
      //error
      .catch((err) => { //error: on api.observation.getGermplasm
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getNotAssociatedGermplasmCount
    */
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedGermplasmCount(graphqlServerUrl, item.observationDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'germplasm', table:'A', method: 'getCount()', request: 'api.observation.getNotAssociatedGermplasmCount'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'germplasm'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedGermplasmCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getNotAssociatedGermplasm
    */
    let cancelableApiReq = makeCancelable(api.observation.getNotAssociatedGermplasm(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'germplasm', table:'A', method: 'getData()', request: 'api.observation.getNotAssociatedGermplasm'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'germplasm'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getNotAssociatedGermplasm
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', association: 'germplasm', table:'B', method: 'getDataB()', request: 'api.germplasm.getItems'}];
            newError.path=['update', `observationDbId:${item.observationDbId}`, 'add', 'germplasm'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observation&&
        ...
        lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_germplasmDbId&&
        (lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_germplasmDbId.added ||
         lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_germplasmDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_germplasmDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.observation[String(item.observationDbId)].changedAssociations.observation_germplasmDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'observation_germplasmDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.germplasm) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observation_germplasmDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observation_germplasmDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observation_germplasmDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observation_germplasmDbId.added) {
              let idsAdded = entry[1].changedAssociations.observation_germplasmDbId.idsAdded;
              if(idsAdded) {
