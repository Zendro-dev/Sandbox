...
import ObservationEnhancedTableHead from './components/ObservationEnhancedTableHead'
import ObservationEnhancedTableToolbar from './components/ObservationEnhancedTableToolbar'
import ObservationUploadFileDialog from './components/ObservationUploadFileDialog'
import ObservationCursorPagination from './components/ObservationCursorPagination'
...
//lazy loading
const ObservationCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Observation" */ './components/observation-create-panel/ObservationCreatePanel'));
const ObservationUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Observation" */ './components/observation-update-panel/ObservationUpdatePanel'));
const ObservationDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Observation" */ './components/observation-detail-panel/ObservationDetailPanel'));
const ObservationDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Observation" */ './components/ObservationDeleteConfirmationDialog'));
//Plotly
const ObservationPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Observation" */ '../../../../plots/ObservationPlotly'));
...
export default function ObservationEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('observationDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.observation.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observation', method: 'getCount()', request: 'api.observation.getCountItems'}];
            newError.path=['Observations'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', method: 'getData()', request: 'api.observation.getItems'}];
            newError.path=['Observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['observation']
    &&(Object.keys(changes.lastModelChanged['observation']).length===1)
    &&(changes.lastModelChanged['observation'][Object.keys(changes.lastModelChanged['observation'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set observationDbId (internalId)
    variables.observationDbId = item.observationDbId;

    /*
      API Request: api.observation.deleteItem
    */
    let api = await loadApi("observation");
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', method: 'doDelete()', request: 'api.observation.deleteItem'}];
            newError.path=['Observations', `observationDbId:${item.observationDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.observation.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.observation.tableTemplate
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observation', method: 'getCsvTemplate()', request: 'api.observation.tableTemplate'}];
            newError.path=['Observations', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateObservation.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "observation-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.observation.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <ObservationEnhancedTableToolbar
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ObservationEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ObservationEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ObservationEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ObservationEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ObservationEnhancedTable-row-'+item.observationDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.observationDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ObservationEnhancedTable-row-iconButton-detail-'+item.observationDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.observation&&Array.isArray(permissions.observation)
                                      &&(permissions.observation.includes('update') || permissions.observation.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ObservationEnhancedTable-row-iconButton-edit-'+item.observationDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.observation&&Array.isArray(permissions.observation)
                                      &&(permissions.observation.includes('delete') || permissions.observation.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ObservationEnhancedTable-row-iconButton-delete-'+item.observationDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'observationDbId: ' + item.observationDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.observationDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.collector!==null)?item.collector:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.germplasmDbId!==null)?item.germplasmDbId:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.observationTimeStamp!==null)?moment(item.observationTimeStamp, "YYYY-MM-DDTHH:mm:ss.SSSZ").format("YYYY-MM-DDTHH:mm:ss.SSSZ"):'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.observationVariableDbId!==null)?item.observationVariableDbId:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.uploadedBy!==null)?item.uploadedBy:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.seasonDbId!==null)?item.seasonDbId:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.imageDbId!==null)?item.imageDbId:'')}
                                    </TableCell>
...
ObservationEnhancedTable.propTypes = {
