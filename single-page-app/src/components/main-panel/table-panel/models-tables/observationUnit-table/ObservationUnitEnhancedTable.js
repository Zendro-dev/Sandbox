...
import ObservationUnitEnhancedTableHead from './components/ObservationUnitEnhancedTableHead'
import ObservationUnitEnhancedTableToolbar from './components/ObservationUnitEnhancedTableToolbar'
import ObservationUnitUploadFileDialog from './components/ObservationUnitUploadFileDialog'
import ObservationUnitCursorPagination from './components/ObservationUnitCursorPagination'
...
//lazy loading
const ObservationUnitCreatePanel = lazy(() => import(/* webpackChunkName: "Create-ObservationUnit" */ './components/observationUnit-create-panel/ObservationUnitCreatePanel'));
const ObservationUnitUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-ObservationUnit" */ './components/observationUnit-update-panel/ObservationUnitUpdatePanel'));
const ObservationUnitDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-ObservationUnit" */ './components/observationUnit-detail-panel/ObservationUnitDetailPanel'));
const ObservationUnitDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-ObservationUnit" */ './components/ObservationUnitDeleteConfirmationDialog'));
//Plotly
const ObservationUnitPlotly = lazy(() => import(/* webpackChunkName: "Plotly-ObservationUnit" */ '../../../../plots/ObservationUnitPlotly'));
...
export default function ObservationUnitEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('observationUnitDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.observationUnit.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', method: 'getCount()', request: 'api.observationUnit.getCountItems'}];
            newError.path=['ObservationUnits'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', method: 'getData()', request: 'api.observationUnit.getItems'}];
            newError.path=['ObservationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['observationUnit']
    &&(Object.keys(changes.lastModelChanged['observationUnit']).length===1)
    &&(changes.lastModelChanged['observationUnit'][Object.keys(changes.lastModelChanged['observationUnit'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set observationUnitDbId (internalId)
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.observationUnit.tableTemplate
    */
    let api = await loadApi("observationUnit");
    let cancelableApiReq = makeCancelable(api.observationUnit.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', method: 'getCsvTemplate()', request: 'api.observationUnit.tableTemplate'}];
            newError.path=['ObservationUnits', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateObservationUnit.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "observationUnit-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <ObservationUnitEnhancedTableToolbar
                ...
                >
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ObservationUnitEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ObservationUnitEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ObservationUnitEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ObservationUnitEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ObservationUnitEnhancedTable-row-'+item.observationUnitDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.observationUnitDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ObservationUnitEnhancedTable-row-iconButton-detail-'+item.observationUnitDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ObservationUnitEnhancedTable-row-iconButton-edit-'+item.observationUnitDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ObservationUnitEnhancedTable-row-iconButton-delete-'+item.observationUnitDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'observationUnitDbId: ' + item.observationUnitDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.observationUnitDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.observationLevel!==null)?item.observationLevel:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.observationUnitName!==null)?item.observationUnitName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.observationUnitPUI!==null)?item.observationUnitPUI:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.plantNumber!==null)?item.plantNumber:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.plotNumber!==null)?item.plotNumber:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.locationDbId!==null)?item.locationDbId:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.eventDbIds!==null)?item.eventDbIds:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <ObservationUnitCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(ObservationUnitPlotly)) && (
                      <Suspense fallback={<div />}>
                        <ObservationUnitPlotly />
                      </Suspense>
                    )}
                  </div>
      ...
      {(uploadFileDialogOpen) && (
        <ObservationUnitUploadFileDialog
...
ObservationUnitEnhancedTable.propTypes = {
