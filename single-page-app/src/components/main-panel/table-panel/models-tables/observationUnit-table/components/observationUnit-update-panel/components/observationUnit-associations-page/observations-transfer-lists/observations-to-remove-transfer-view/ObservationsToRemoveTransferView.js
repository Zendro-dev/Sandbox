...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getObservationsCount
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getObservationsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observations', method: 'getCount()', request: 'api.observationUnit.getObservationsCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'remove', 'observations'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getObservationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getObservations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.observationUnit.getObservations(
      graphqlServerUrl, 
      item.observationUnitDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observations', method: 'getData()', request: 'api.observationUnit.getObservations'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'remove', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getObservations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
