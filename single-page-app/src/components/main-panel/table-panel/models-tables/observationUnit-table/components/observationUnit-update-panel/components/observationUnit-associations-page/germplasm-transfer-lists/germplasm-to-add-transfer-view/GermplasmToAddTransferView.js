...
import GermplasmToAddTransferViewToolbar from './components/GermplasmToAddTransferViewToolbar';
import GermplasmToAddTransferViewCursorPagination from './components/GermplasmToAddTransferViewCursorPagination';
...
export default function GermplasmToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getGermplasm
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getGermplasm(graphqlServerUrl, item.observationUnitDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'germplasm', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationUnit.getGermplasm'}];
            newError.path=['update', `observationUnitDbId:${currentId}`, 'add', 'germplasm'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getGermplasm
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"germplasmDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observationUnit.getNotAssociatedGermplasmCount
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedGermplasmCount(graphqlServerUrl, item.observationUnitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'germplasm', table:'A', method: 'getCount()', request: 'api.observationUnit.getNotAssociatedGermplasmCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'germplasm'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedGermplasmCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedGermplasm
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedGermplasm(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'germplasm', table:'A', method: 'getData()', request: 'api.observationUnit.getNotAssociatedGermplasm'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'germplasm'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedGermplasm
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.germplasm.getItems
    */
    let cancelableApiReq = makeCancelable(api.germplasm.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'germplasm', table:'B', method: 'getDataB()', request: 'api.germplasm.getItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'germplasm'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'germplasm' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'germplasms', 
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_germplasmDbId&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_germplasmDbId.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_germplasmDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_germplasmDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('germplasm', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('germplasm', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_germplasmDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('germplasm', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'germplasm' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'germplasm',
     * but this action happened on the peer relation, identified by 'observationUnit_germplasmDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'germplasm'.
     * B: the current item internalId is in the addedIds of the updated 'germplasm'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'germplasm' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.germplasm) {
      let oens = Object.entries(lastModelChanged.germplasm);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnit_germplasmDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnit_germplasmDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnit_germplasmDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnit_germplasmDbId.added) {
              let idsAdded = entry[1].changedAssociations.observationUnit_germplasmDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.observationUnitDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.germplasmDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('germplasm', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'germplasm' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.germplasm) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.germplasmDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.germplasmDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.germplasmDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.germplasmDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.germplasmDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.germplasmDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('germplasm', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.germplasmDbId===idRemoved) {
            //update associated item
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnGermplasmRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.germplasmDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.germplasmDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.germplasmDbId];
        handleDisassociateItem('germplasm', associatedItem.germplasmDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('germplasm', item.germplasmDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('germplasm', item.germplasmDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <GermplasmToAddTransferViewToolbar
                title={'Germplasms'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GermplasmToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='GermplasmToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.germplasmDbId;
                        let label = it.germplasmName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'GermplasmToAddTransferView-listA-listItem-'+it.germplasmDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.germplasmDbId===it.germplasmDbId) ? 'germplasm — ' + t('modelPanels.associatedRecord', 'Associated record') : 'germplasm '}>
                                <Avatar>
                                  {"germplasm".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'germplasmDbId' }>
                                        <Typography 
                                        id={'GermplasmToAddTransferView-listA-listItem-id-'+it.germplasmDbId}
                                        variant="body1" display="block" noWrap={true}>{it.germplasmDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.germplasmDbId===it.germplasmDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.germplasmDbId===it.germplasmDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'GermplasmToAddTransferView-listA-listItem-'+it.germplasmDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <GermplasmToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GermplasmToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GermplasmToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='GermplasmToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.germplasmDbId;
                        let label = it.germplasmName;
                        ...
                        return (
                          <ListItem 
                            id={'GermplasmToAddTransferView-listB-listItem-'+it.germplasmDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'germplasmDbId' }>
                                        <Typography 
                                        id={'GermplasmToAddTransferView-listB-listItem-id-'+it.germplasmDbId}
                                        variant="body1" display="block" noWrap={true}>{it.germplasmDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'GermplasmToAddTransferView-listB-listItem-'+it.germplasmDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='GermplasmToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.germplasmDbId;
                        let label = associatedItem.germplasmName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'GermplasmToAddTransferView-associatedItem-list-listItem-'+associatedItem.germplasmDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'germplasm — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"germplasm".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'germplasmDbId' }>
                                        <Typography 
                                        id={'GermplasmToAddTransferView-listB-listItem-id-'+associatedItem.germplasmDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.germplasmDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'GermplasmToAddTransferView-associatedItem-list-listItem-'+associatedItem.germplasmDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].germplasmDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('germplasm', associatedItem.germplasmDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.germplasmDbId];
                                      handleDisassociateItem('germplasm', associatedItem.germplasmDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='GermplasmToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
GermplasmToAddTransferView.propTypes = {
