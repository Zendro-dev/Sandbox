...
import ObservationTreatmentsToRemoveTransferViewToolbar from './components/ObservationTreatmentsToRemoveTransferViewToolbar';
import ObservationTreatmentsToRemoveTransferViewCursorPagination from './components/ObservationTreatmentsToRemoveTransferViewCursorPagination';
...
export default function ObservationTreatmentsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getObservationTreatmentsCount
    */
    let api = await loadApi("observationUnit");
    let cancelableApiReq = makeCancelable(api.observationUnit.getObservationTreatmentsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationTreatments', method: 'getCount()', request: 'api.observationUnit.getObservationTreatmentsCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'remove', 'observationTreatments'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getObservationTreatmentsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {observationTreatmentDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.observationUnit.getObservationTreatments
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("observationUnit");
    let cancelableApiReq = makeCancelable(api.observationUnit.getObservationTreatments(
      graphqlServerUrl, 
      item.observationUnitDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationTreatments', method: 'getData()', request: 'api.observationUnit.getObservationTreatments'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'remove', 'observationTreatments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getObservationTreatments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'observationTreatment' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.observationTreatment_observationUnitDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.observationTreatmentDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.observationTreatmentDbId) === -1) {
      lidsToRemove.current.push(item.observationTreatmentDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.observationTreatmentDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationTreatmentsToRemoveTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationTreatmentsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationTreatmentsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.observationTreatmentDbId;
                        let label = it.factor;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(observationTreatmentDbId=> observationTreatmentDbId=== it.observationTreatmentDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'ObservationTreatmentsToRemoveTransferView-listA-listItem-'+it.observationTreatmentDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationTreatmentDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.observationTreatmentDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'ObservationTreatmentsToRemoveTransferView-listA-listItem-'+it.observationTreatmentDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <ObservationTreatmentsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationTreatmentsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationTreatmentsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationTreatmentsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationTreatmentDbId;
                        let label = it.factor;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationTreatmentsToRemoveTransferView-listB-listItem-'+it.observationTreatmentDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'ObservationTreatmentsToRemoveTransferView-listB-listItem-'+it.observationTreatmentDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ObservationTreatmentsToRemoveTransferView.propTypes = {
