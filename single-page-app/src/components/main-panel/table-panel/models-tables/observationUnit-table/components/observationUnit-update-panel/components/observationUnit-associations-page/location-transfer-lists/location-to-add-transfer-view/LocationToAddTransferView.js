...
export default function LocationToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getLocation
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getLocation(graphqlServerUrl, item.observationUnitDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'location', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationUnit.getLocation'}];
            newError.path=['update', `observationUnitDbId:${currentId}`, 'add', 'location'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getLocation
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"locationDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observationUnit.getNotAssociatedLocationCount
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedLocationCount(graphqlServerUrl, item.observationUnitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'location', table:'A', method: 'getCount()', request: 'api.observationUnit.getNotAssociatedLocationCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'location'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedLocationCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedLocation
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedLocation(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'location', table:'A', method: 'getData()', request: 'api.observationUnit.getNotAssociatedLocation'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'location'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedLocation
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.location.getItems
    */
    let cancelableApiReq = makeCancelable(api.location.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'location', table:'B', method: 'getDataB()', request: 'api.location.getItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'location'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.location.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'location' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'locations', 
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_locationDbId&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_locationDbId.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_locationDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_locationDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('location', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('location', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_locationDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('location', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'location' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'location',
     * but this action happened on the peer relation, identified by 'observationUnit_locationDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'location'.
     * B: the current item internalId is in the addedIds of the updated 'location'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'location' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.location) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnit_locationDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnit_locationDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnit_locationDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnit_locationDbId.added) {
              let idsAdded = entry[1].changedAssociations.observationUnit_locationDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.observationUnitDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.locationDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('location', idAdded);
                  } 
    ...
    if(lastModelChanged.location) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.locationDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.locationDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.locationDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.locationDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.locationDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.locationDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup on associated item
          if(associatedItem && associatedItem.locationDbId===idRemoved) {
            //update associated item
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.locationDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.locationDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.locationDbId];
        handleDisassociateItem('location', associatedItem.locationDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('location', item.locationDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('location', item.locationDbId);
    }
  };
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='LocationToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='LocationToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.locationDbId;
                        let label = it.locationName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'LocationToAddTransferView-listA-listItem-'+it.locationDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.locationDbId===it.locationDbId) ? 'location — ' + t('modelPanels.associatedRecord', 'Associated record') : 'location '}>
                                <Avatar>
                                  {"location".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'locationDbId' }>
                                        <Typography 
                                        id={'LocationToAddTransferView-listA-listItem-id-'+it.locationDbId}
                                        variant="body1" display="block" noWrap={true}>{it.locationDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.locationDbId===it.locationDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.locationDbId===it.locationDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'LocationToAddTransferView-listA-listItem-'+it.locationDbId+'-button-add'}
                                  color="primary"
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='LocationToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='LocationToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='LocationToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.locationDbId;
                        let label = it.locationName;
                        ...
                        return (
                          <ListItem 
                            id={'LocationToAddTransferView-listB-listItem-'+it.locationDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'locationDbId' }>
                                        <Typography 
                                        id={'LocationToAddTransferView-listB-listItem-id-'+it.locationDbId}
                                        variant="body1" display="block" noWrap={true}>{it.locationDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'LocationToAddTransferView-listB-listItem-'+it.locationDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='LocationToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.locationDbId;
                        let label = associatedItem.locationName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'LocationToAddTransferView-associatedItem-list-listItem-'+associatedItem.locationDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'location — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"location".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'locationDbId' }>
                                        <Typography 
                                        id={'LocationToAddTransferView-listB-listItem-id-'+associatedItem.locationDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.locationDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'LocationToAddTransferView-associatedItem-list-listItem-'+associatedItem.locationDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].locationDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('location', associatedItem.locationDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.locationDbId];
                                      handleDisassociateItem('location', associatedItem.locationDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='LocationToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
