...
import ObservationTreatmentsToAddTransferViewToolbar from './components/ObservationTreatmentsToAddTransferViewToolbar';
import ObservationTreatmentsToAddTransferViewCursorPagination from './components/ObservationTreatmentsToAddTransferViewCursorPagination';
...
export default function ObservationTreatmentsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"observationTreatmentDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observationUnit.getNotAssociatedObservationTreatmentsCount
    */
    let api = await loadApi("observationUnit");
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedObservationTreatmentsCount(graphqlServerUrl, item.observationUnitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'observationTreatments', table:'A', method: 'getCount()', request: 'api.observationUnit.getNotAssociatedObservationTreatmentsCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observationTreatments'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedObservationTreatmentsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedObservationTreatments
    */
    let api = await loadApi("observationUnit");
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedObservationTreatments(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'observationTreatments', table:'A', method: 'getData()', request: 'api.observationUnit.getNotAssociatedObservationTreatments'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observationTreatments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedObservationTreatments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.observationUnitDbId, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.observationTreatment.getCountItems
    */
    let api = await loadApi("observationTreatment");
    let cancelableApiReq = makeCancelable(api.observationTreatment.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'observationTreatments', table:'B', method: 'getCountB()', request: 'api.observationTreatment.getCountItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observationTreatments'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.observationTreatment.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.observationTreatment.getItems
    */
    let api = await loadApi("observationTreatment");
    let cancelableApiReq = makeCancelable(api.observationTreatment.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'observationTreatments', table:'B', method: 'getDataB()', request: 'api.observationTreatment.getItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observationTreatments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationTreatment.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'observationTreatments' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'observationTreatments', 
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationTreatment_observationUnitDbId&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationTreatment_observationUnitDbId.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationTreatment_observationUnitDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationTreatment_observationUnitDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('observationTreatments', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationTreatment_observationUnitDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'observationTreatments' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'observationTreatment',
     * but this action happened on the peer relation, identified by 'observationTreatment_observationUnitDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'observationTreatment'.
     * B: the current item internalId is in the addedIds of the updated 'observationTreatment'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'observationTreatment' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.observationTreatment) {
      let oens = Object.entries(lastModelChanged.observationTreatment);
      oens.forEach( (entry) => {
            ...
            if(entry[1].changedAssociations.observationTreatment_observationUnitDbId.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.observationTreatmentDbId;
    ...
    /*
     * Case 3: 
     * The attributes of some 'observationTreatment' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationTreatment) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.observationTreatmentDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.observationTreatmentDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.observationTreatmentDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.observationTreatmentDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.observationTreatmentDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.observationTreatmentDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('observationTreatments', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnObservationTreatmentRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.observationTreatmentDbId) === -1) {
      lidsToAdd.current.push(item.observationTreatmentDbId);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('observationTreatments', item.observationTreatmentDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.observationTreatmentDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('observationTreatments', item.observationTreatmentDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationTreatmentsToAddTransferViewToolbar
                title={'ObservationTreatments'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationTreatmentsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationTreatmentsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.observationTreatmentDbId;
                        let label = it.factor;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'ObservationTreatmentsToAddTransferView-listA-listItem-'+it.observationTreatmentDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'observationTreatment' }>
                                <Avatar>{"observationTreatment".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationTreatmentDbId' }>
                                        <Typography 
                                        id={'ObservationTreatmentsToAddTransferView-listA-listItem-id-'+it.observationTreatmentDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationTreatmentDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ObservationTreatmentsToAddTransferView-listA-listItem-'+it.observationTreatmentDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ObservationTreatmentsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationTreatmentsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationTreatmentsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationTreatmentsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationTreatmentDbId;
                        let label = it.factor;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationTreatmentsToAddTransferView-listB-listItem-'+it.observationTreatmentDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationTreatmentDbId' }>
                                        <Typography 
                                        id={'ObservationTreatmentsToAddTransferView-listB-listItem-id-'+it.observationTreatmentDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationTreatmentDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ObservationTreatmentsToAddTransferView-listB-listItem-'+it.observationTreatmentDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ObservationTreatmentsToAddTransferView.propTypes = {
