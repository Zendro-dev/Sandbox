...
import TrialToAddTransferViewToolbar from './components/TrialToAddTransferViewToolbar';
import TrialToAddTransferViewCursorPagination from './components/TrialToAddTransferViewCursorPagination';
...
export default function TrialToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getTrial
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getTrial(graphqlServerUrl, item.observationUnitDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'trial', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationUnit.getTrial'}];
            newError.path=['update', `observationUnitDbId:${currentId}`, 'add', 'trial'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getTrial
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"trialDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observationUnit.getNotAssociatedTrialCount
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedTrialCount(graphqlServerUrl, item.observationUnitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'trial', table:'A', method: 'getCount()', request: 'api.observationUnit.getNotAssociatedTrialCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'trial'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedTrialCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedTrial
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedTrial(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'trial', table:'A', method: 'getData()', request: 'api.observationUnit.getNotAssociatedTrial'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'trial'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedTrial
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'trial', table:'B', method: 'getDataB()', request: 'api.trial.getItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'trial'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'trial' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'trials', 
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_trialDbId&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_trialDbId.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_trialDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_trialDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('trial', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('trial', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_trialDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('trial', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'trial' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'trial',
     * but this action happened on the peer relation, identified by 'observationUnit_trialDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'trial'.
     * B: the current item internalId is in the addedIds of the updated 'trial'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'trial' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.trial) {
      let oens = Object.entries(lastModelChanged.trial);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnit_trialDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnit_trialDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnit_trialDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnit_trialDbId.added) {
              let idsAdded = entry[1].changedAssociations.observationUnit_trialDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.observationUnitDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.trialDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('trial', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'trial' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.trial) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.trialDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.trialDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.trialDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.trialDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.trialDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.trialDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('trial', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.trialDbId===idRemoved) {
            //update associated item
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnTrialRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.trialDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.trialDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.trialDbId];
        handleDisassociateItem('trial', associatedItem.trialDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('trial', item.trialDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('trial', item.trialDbId);
    }
  };
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='TrialToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.trialDbId;
                        let label = it.trialName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'TrialToAddTransferView-listA-listItem-'+it.trialDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.trialDbId===it.trialDbId) ? 'trial — ' + t('modelPanels.associatedRecord', 'Associated record') : 'trial '}>
                                <Avatar>
                                  {"trial".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'trialDbId' }>
                                        <Typography 
                                        id={'TrialToAddTransferView-listA-listItem-id-'+it.trialDbId}
                                        variant="body1" display="block" noWrap={true}>{it.trialDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.trialDbId===it.trialDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.trialDbId===it.trialDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'TrialToAddTransferView-listA-listItem-'+it.trialDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <TrialToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='TrialToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.trialDbId;
                        let label = it.trialName;
                        ...
                        return (
                          <ListItem 
                            id={'TrialToAddTransferView-listB-listItem-'+it.trialDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'trialDbId' }>
                                        <Typography 
                                        id={'TrialToAddTransferView-listB-listItem-id-'+it.trialDbId}
                                        variant="body1" display="block" noWrap={true}>{it.trialDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'TrialToAddTransferView-listB-listItem-'+it.trialDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='TrialToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.trialDbId;
                        let label = associatedItem.trialName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'TrialToAddTransferView-associatedItem-list-listItem-'+associatedItem.trialDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'trial — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"trial".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'trialDbId' }>
                                        <Typography 
                                        id={'TrialToAddTransferView-listB-listItem-id-'+associatedItem.trialDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.trialDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'TrialToAddTransferView-associatedItem-list-listItem-'+associatedItem.trialDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].trialDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('trial', associatedItem.trialDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.trialDbId];
                                      handleDisassociateItem('trial', associatedItem.trialDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='TrialToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
TrialToAddTransferView.propTypes = {
