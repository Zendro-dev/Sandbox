...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getEventsCount
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getEventsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'events', method: 'getCount()', request: 'api.observationUnit.getEventsCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'remove', 'events'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getEventsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getEvents
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.observationUnit.getEvents(
      graphqlServerUrl, 
      item.observationUnitDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'events', method: 'getData()', request: 'api.observationUnit.getEvents'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'remove', 'events'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getEvents
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
