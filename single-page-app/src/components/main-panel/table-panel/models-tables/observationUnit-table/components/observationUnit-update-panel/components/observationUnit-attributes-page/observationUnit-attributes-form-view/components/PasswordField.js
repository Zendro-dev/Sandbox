...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['observationUnit'][String(itemId['observationUnitDbId'])]) {

        //updated password
        if(lastModelChanged['observationUnit'][String(itemId['observationUnitDbId'])].op === "updatePassword"&&
            lastModelChanged['observationUnit'][String(itemId['observationUnitDbId'])].newItem) {
              //update password value
              setValue(lastModelChanged['observationUnit'][String(itemId['observationUnitDbId'])].newItem[itemKey]);
        }
    }//end: Case 1
  ...
  /**
    ...
    */
    return await api.observationUnit.updateItem(graphqlServerUrl, variables)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['ObservationUnits', itemId, 'update', 'change password'];
  ...
  return (
    <div>
      <Grid container justify='flex-start' alignItems='center' spacing={0}>
        <Grid item xs={12}>
          <form noValidate autoComplete="off">
            <TextField
              id={'PasswordField-ObservationUnit-'+name}
              label={label}
              value={value}
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'PasswordField-ObservationUnit-button-showPassword'+name}
                    ...
                    {(valueOk!==undefined&&valueOk===1) 
                      ...
                      <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                        <Typography id={'PasswordField-ObservationUnit-exists-'+name} variant="caption" color="primary">
                          &#8707;
                        </Typography>
                      </Tooltip>
                      : 
                      <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                        <Typography id={'PasswordField-ObservationUnit-notExists-'+name} variant="caption" color="textSecondary">
                          &#8708;
                        </Typography>
                      </Tooltip>
                    }
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'PasswordField-ObservationUnit-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-ObservationUnit-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-ObservationUnit-ChangePasswordDialog-textField-newPassword2-'+name}
