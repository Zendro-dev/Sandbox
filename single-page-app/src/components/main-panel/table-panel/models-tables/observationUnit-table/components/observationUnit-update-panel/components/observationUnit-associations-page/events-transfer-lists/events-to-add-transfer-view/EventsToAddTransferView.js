...
export default function EventsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedEventsCount
    */
    let api = await loadApi("observationUnit");
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedEventsCount(graphqlServerUrl, item.observationUnitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'events', table:'A', method: 'getCount()', request: 'api.observationUnit.getNotAssociatedEventsCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'events'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedEventsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedEvents
    */
    let api = await loadApi("observationUnit");
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedEvents(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'events', table:'A', method: 'getData()', request: 'api.observationUnit.getNotAssociatedEvents'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'events'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedEvents
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'events', table:'B', method: 'getCountB()', request: 'api.event.getCountItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'events'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'events', table:'B', method: 'getDataB()', request: 'api.event.getItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'events'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.event_observationUnitDbIds&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.event_observationUnitDbIds.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.event_observationUnitDbIds.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.event_observationUnitDbIds.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.event_observationUnitDbIds.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'event_observationUnitDbIds'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.event) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.event_observationUnitDbIds) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.event_observationUnitDbIds.removed) {
              let idsRemoved = entry[1].changedAssociations.event_observationUnitDbIds.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.event_observationUnitDbIds.added) {
              let idsAdded = entry[1].changedAssociations.event_observationUnitDbIds.idsAdded;
              if(idsAdded) {
