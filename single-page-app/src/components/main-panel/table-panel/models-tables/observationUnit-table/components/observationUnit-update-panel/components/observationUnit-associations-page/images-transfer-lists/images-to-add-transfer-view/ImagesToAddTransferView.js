...
export default function ImagesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedImagesCount
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedImagesCount(graphqlServerUrl, item.observationUnitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'images', table:'A', method: 'getCount()', request: 'api.observationUnit.getNotAssociatedImagesCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'images'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedImagesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedImages
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedImages(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'images', table:'A', method: 'getData()', request: 'api.observationUnit.getNotAssociatedImages'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'images'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedImages
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    ...
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ImagesToAddTransferView-listA-listItem-'+it.imageDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'imageDbId' }>
                                        <Typography 
                                        id={'ImagesToAddTransferView-listA-listItem-id-'+it.imageDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ImagesToAddTransferView-listA-listItem-'+it.imageDbId+'-button-add'}
                                  color="primary"
                  ...
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ImagesToAddTransferView-list-listB'
                      ...
                      {itemsB.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ImagesToAddTransferView-listB-listItem-'+it.imageDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'imageDbId' }>
                                        <Typography 
                                        id={'ImagesToAddTransferView-listB-listItem-id-'+it.imageDbId}
                                        variant="body1" display="block" noWrap={true}>{it.imageDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ImagesToAddTransferView-listB-listItem-'+it.imageDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
