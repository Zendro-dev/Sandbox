...
import ObservationUnitPositionToAddTransferViewToolbar from './components/ObservationUnitPositionToAddTransferViewToolbar';
import ObservationUnitPositionToAddTransferViewCursorPagination from './components/ObservationUnitPositionToAddTransferViewCursorPagination';
...
export default function ObservationUnitPositionToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.observationUnitDbId;

    /*
      API Request: api.observationUnit.getObservationUnitPosition
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getObservationUnitPosition(graphqlServerUrl, item.observationUnitDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'observationUnitPosition', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationUnit.getObservationUnitPosition'}];
            newError.path=['update', `observationUnitDbId:${currentId}`, 'add', 'observationUnitPosition'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getObservationUnitPosition
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.observationUnitDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"observationUnitPositionDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observationUnit.getNotAssociatedObservationUnitPositionCount
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedObservationUnitPositionCount(graphqlServerUrl, item.observationUnitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'observationUnitPosition', table:'A', method: 'getCount()', request: 'api.observationUnit.getNotAssociatedObservationUnitPositionCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observationUnitPosition'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedObservationUnitPositionCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedObservationUnitPosition
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedObservationUnitPosition(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'observationUnitPosition', table:'A', method: 'getData()', request: 'api.observationUnit.getNotAssociatedObservationUnitPosition'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observationUnitPosition'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedObservationUnitPosition
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.observationUnitDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnitPosition.getItems
    */
    let cancelableApiReq = makeCancelable(api.observationUnitPosition.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'observationUnitPosition', table:'B', method: 'getDataB()', request: 'api.observationUnitPosition.getItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observationUnitPosition'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnitPosition.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.observationUnitDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'observationUnitPosition' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'observationUnitPositions', 
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitPosition_observationUnitDbId&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitPosition_observationUnitDbId.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitPosition_observationUnitDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitPosition_observationUnitDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('observationUnitPosition', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('observationUnitPosition', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnitPosition_observationUnitDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('observationUnitPosition', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'observationUnitPosition' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'observationUnitPosition',
     * but this action happened on the peer relation, identified by 'observationUnitPosition_observationUnitDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'observationUnitPosition'.
     * B: the current item internalId is in the addedIds of the updated 'observationUnitPosition'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'observationUnitPosition' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.observationUnitPosition) {
      let oens = Object.entries(lastModelChanged.observationUnitPosition);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnitPosition_observationUnitDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnitPosition_observationUnitDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnitPosition_observationUnitDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnitPosition_observationUnitDbId.added) {
              let idsAdded = entry[1].changedAssociations.observationUnitPosition_observationUnitDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.observationUnitDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.observationUnitPositionDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('observationUnitPosition', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'observationUnitPosition' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationUnitPosition) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.observationUnitPositionDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.observationUnitPositionDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.observationUnitPositionDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.observationUnitPositionDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.observationUnitPositionDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.observationUnitPositionDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('observationUnitPosition', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.observationUnitPositionDbId===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationUnitDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnObservationUnitPositionRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.observationUnitPositionDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.observationUnitPositionDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.observationUnitPositionDbId];
        handleDisassociateItem('observationUnitPosition', associatedItem.observationUnitPositionDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('observationUnitPosition', item.observationUnitPositionDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('observationUnitPosition', item.observationUnitPositionDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationUnitPositionToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitPositionToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationUnitPositionToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.observationUnitPositionDbId;
                        let label = it.observationUnitPositionName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'ObservationUnitPositionToAddTransferView-listA-listItem-'+it.observationUnitPositionDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.observationUnitPositionDbId===it.observationUnitPositionDbId) ? 'observationUnitPosition — ' + t('modelPanels.associatedRecord', 'Associated record') : 'observationUnitPosition '}>
                                <Avatar>
                                  {"observationUnitPosition".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                            ...
                            <ListItemText
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationUnitPositionDbId' }>
                                        <Typography 
                                        id={'ObservationUnitPositionToAddTransferView-listA-listItem-id-'+it.observationUnitPositionDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationUnitPositionDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.observationUnitPositionDbId===it.observationUnitPositionDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.observationUnitPositionDbId===it.observationUnitPositionDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                              ...
                              secondary={
                                <React.Fragment>
                                  {/* Label */}
                                  {(label) && (
                                    <Tooltip title={ 'observationUnitPositionName' }>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ObservationUnitPositionToAddTransferView-listA-listItem-'+it.observationUnitPositionDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ObservationUnitPositionToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitPositionToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitPositionToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationUnitPositionToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationUnitPositionDbId;
                        let label = it.observationUnitPositionName;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationUnitPositionToAddTransferView-listB-listItem-'+it.observationUnitPositionDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationUnitPositionDbId' }>
                                        <Typography 
                                        id={'ObservationUnitPositionToAddTransferView-listB-listItem-id-'+it.observationUnitPositionDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationUnitPositionDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ObservationUnitPositionToAddTransferView-listB-listItem-'+it.observationUnitPositionDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='ObservationUnitPositionToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.observationUnitPositionDbId;
                        let label = associatedItem.observationUnitPositionName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'ObservationUnitPositionToAddTransferView-associatedItem-list-listItem-'+associatedItem.observationUnitPositionDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'observationUnitPosition — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"observationUnitPosition".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationUnitPositionDbId' }>
                                        <Typography 
                                        id={'ObservationUnitPositionToAddTransferView-listB-listItem-id-'+associatedItem.observationUnitPositionDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.observationUnitPositionDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'ObservationUnitPositionToAddTransferView-associatedItem-list-listItem-'+associatedItem.observationUnitPositionDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].observationUnitPositionDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('observationUnitPosition', associatedItem.observationUnitPositionDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.observationUnitPositionDbId];
                                      handleDisassociateItem('observationUnitPosition', associatedItem.observationUnitPositionDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitPositionToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
ObservationUnitPositionToAddTransferView.propTypes = {
