...
export default function ObservationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedObservationsCount
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedObservationsCount(graphqlServerUrl, item.observationUnitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'observations', table:'A', method: 'getCount()', request: 'api.observationUnit.getNotAssociatedObservationsCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observations'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedObservationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedObservations
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedObservations(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'observations', table:'A', method: 'getData()', request: 'api.observationUnit.getNotAssociatedObservations'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedObservations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'observations', table:'B', method: 'getCountB()', request: 'api.observation.getCountItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'observations', table:'B', method: 'getDataB()', request: 'api.observation.getItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observation_observationUnitDbId&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observation_observationUnitDbId.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observation_observationUnitDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observation_observationUnitDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observation_observationUnitDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
