      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'location', table:'A', method: 'getCount()', request: 'api.location.getCountItems'}];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'location', table:'A', method: 'getData()', request: 'api.location.getItems'}];
            newError.path=['add', 'location'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
                                ...
                                <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                  <Grid item>
                                    <Tooltip title={ 'locationDbId' }>
                                      <Typography variant="body1" display="block" noWrap={true}>{it.locationDbId}</Typography>
                                    </Tooltip>
                                  </Grid>
                                  {/*Key icon*/}
                                  <Grid item>
