...
export default function StudyToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getStudy
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getStudy(graphqlServerUrl, item.observationUnitDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', association: 'study', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationUnit.getStudy'}];
            newError.path=['update', `observationUnitDbId:${currentId}`, 'add', 'study'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getStudy
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedStudyCount
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedStudyCount(graphqlServerUrl, item.observationUnitDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'study', table:'A', method: 'getCount()', request: 'api.observationUnit.getNotAssociatedStudyCount'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedStudyCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getNotAssociatedStudy
    */
    let cancelableApiReq = makeCancelable(api.observationUnit.getNotAssociatedStudy(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'study', table:'A', method: 'getData()', request: 'api.observationUnit.getNotAssociatedStudy'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getNotAssociatedStudy
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationUnit', association: 'study', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `observationUnitDbId:${item.observationUnitDbId}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationUnit&&
        ...
        lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_studyDbId&&
        (lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_studyDbId.added ||
         lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_studyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_studyDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.observationUnit[String(item.observationUnitDbId)].changedAssociations.observationUnit_studyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'observationUnit_studyDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnit_studyDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnit_studyDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnit_studyDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnit_studyDbId.added) {
              let idsAdded = entry[1].changedAssociations.observationUnit_studyDbId.idsAdded;
              if(idsAdded) {
