...
import ScaleEnhancedTableHead from './components/ScaleEnhancedTableHead'
import ScaleEnhancedTableToolbar from './components/ScaleEnhancedTableToolbar'
import ScaleUploadFileDialog from './components/ScaleUploadFileDialog'
import ScaleCursorPagination from './components/ScaleCursorPagination'
...
//lazy loading
const ScaleCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Scale" */ './components/scale-create-panel/ScaleCreatePanel'));
const ScaleUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Scale" */ './components/scale-update-panel/ScaleUpdatePanel'));
const ScaleDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Scale" */ './components/scale-detail-panel/ScaleDetailPanel'));
const ScaleDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Scale" */ './components/ScaleDeleteConfirmationDialog'));
//Plotly
const ScalePlotly = lazy(() => import(/* webpackChunkName: "Plotly-Scale" */ '../../../../plots/ScalePlotly'));
...
export default function ScaleEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('scaleDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.scale.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'scale', method: 'getCount()', request: 'api.scale.getCountItems'}];
            newError.path=['Scales'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'scale', method: 'getData()', request: 'api.scale.getItems'}];
            newError.path=['Scales'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['scale']
    &&(Object.keys(changes.lastModelChanged['scale']).length===1)
    &&(changes.lastModelChanged['scale'][Object.keys(changes.lastModelChanged['scale'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set scaleDbId (internalId)
    variables.scaleDbId = item.scaleDbId;

    /*
      API Request: api.scale.deleteItem
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'scale', method: 'doDelete()', request: 'api.scale.deleteItem'}];
            newError.path=['Scales', `scaleDbId:${item.scaleDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.scale.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.scale.tableTemplate
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'scale', method: 'getCsvTemplate()', request: 'api.scale.tableTemplate'}];
            newError.path=['Scales', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateScale.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "scale-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.scale.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ScaleEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ScaleEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ScaleEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ScaleEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ScaleEnhancedTable-row-'+item.scaleDbId}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ScaleEnhancedTable-row-iconButton-detail-'+item.scaleDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.scale&&Array.isArray(permissions.scale)
                                      &&(permissions.scale.includes('update') || permissions.scale.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ScaleEnhancedTable-row-iconButton-edit-'+item.scaleDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.scale&&Array.isArray(permissions.scale)
                                      &&(permissions.scale.includes('delete') || permissions.scale.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ScaleEnhancedTable-row-iconButton-delete-'+item.scaleDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'scaleDbId: ' + item.scaleDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.scaleDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.decimalPlaces!==null)?item.decimalPlaces:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.scaleName!==null)?item.scaleName:'')}
                                    </TableCell>
