...
//lazy loading
const ScaleAttributesPage = lazy(() => import(/* webpackChunkName: "Create-AttributesScale" */ './components/scale-attributes-page/ScaleAttributesPage'));
const ScaleAssociationsPage = lazy(() => import(/* webpackChunkName: "Create-AssociationsScale" */ './components/scale-associations-page/ScaleAssociationsPage'));
const ScaleConfirmationDialog = lazy(() => import(/* webpackChunkName: "Create-ConfirmationScale" */ './components/ScaleConfirmationDialog'));
...
export default function ScaleCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.decimalPlaces = null;
    initialValues.scaleName = null;
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.decimalPlaces = 0;
    initialValueOkStates.scaleName = 0;
    initialValueOkStates.xref = 0;
    initialValueOkStates.scaleDbId = 0;
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.scale.createItem
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'scale', method: 'doSave()', request: 'api.scale.createItem'}];
            newError.path=['Scales', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.scale.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  return (
    ...
    <Dialog id='ScaleCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ScaleCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Scale'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " scale" }>
            <Fab
              id='ScaleCreatePanel-fabButton-save' 
              color="secondary" 
