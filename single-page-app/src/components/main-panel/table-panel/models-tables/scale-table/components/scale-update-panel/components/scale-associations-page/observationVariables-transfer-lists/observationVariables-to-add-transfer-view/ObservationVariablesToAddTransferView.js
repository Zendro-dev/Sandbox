...
export default function ObservationVariablesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.scale.getNotAssociatedObservationVariablesCount
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.getNotAssociatedObservationVariablesCount(graphqlServerUrl, item.scaleDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'scale', association: 'observationVariables', table:'A', method: 'getCount()', request: 'api.scale.getNotAssociatedObservationVariablesCount'}];
            newError.path=['update', `scaleDbId:${item.scaleDbId}`, 'add', 'observationVariables'];
      ...
      //error
      .catch((err) => { //error: on api.scale.getNotAssociatedObservationVariablesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.scaleDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.scale.getNotAssociatedObservationVariables
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.getNotAssociatedObservationVariables(
      graphqlServerUrl,
      item.scaleDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'scale', association: 'observationVariables', table:'A', method: 'getData()', request: 'api.scale.getNotAssociatedObservationVariables'}];
            newError.path=['update', `scaleDbId:${item.scaleDbId}`, 'add', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.scale.getNotAssociatedObservationVariables
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.scaleDbId, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'scale', association: 'observationVariables', table:'B', method: 'getCountB()', request: 'api.observationVariable.getCountItems'}];
            newError.path=['update', `scaleDbId:${item.scaleDbId}`, 'add', 'observationVariables'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.scaleDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'scale', association: 'observationVariables', table:'B', method: 'getDataB()', request: 'api.observationVariable.getItems'}];
            newError.path=['update', `scaleDbId:${item.scaleDbId}`, 'add', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.scaleDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
     ...
     * - return
     */
    if(lastModelChanged.scale&&
        lastModelChanged.scale[String(item.scaleDbId)]&&
        lastModelChanged.scale[String(item.scaleDbId)].changedAssociations&&
        lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.observationVariable_scaleDbId&&
        (lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.observationVariable_scaleDbId.added ||
         lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.observationVariable_scaleDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.observationVariable_scaleDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.observationVariable_scaleDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'observationVariable_scaleDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationVariable_scaleDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationVariable_scaleDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observationVariable_scaleDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.scaleDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.observationVariable_scaleDbId.added) {
              let idsAdded = entry[1].changedAssociations.observationVariable_scaleDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.scaleDbId);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.scaleDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
