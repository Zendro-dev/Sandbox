...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.scale.getObservationVariablesCount
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.getObservationVariablesCount(
      graphqlServerUrl, 
      item.scaleDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationVariables', method: 'getCount()', request: 'api.scale.getObservationVariablesCount'}];
            newError.path=['update', `scaleDbId:${item.scaleDbId}`, 'remove', 'observationVariables'];
      ...
      //error
      .catch((err) => { //error: on api.scale.getObservationVariablesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.scale.getObservationVariables
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.getObservationVariables(
      graphqlServerUrl, 
      item.scaleDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationVariables', method: 'getData()', request: 'api.scale.getObservationVariables'}];
            newError.path=['update', `scaleDbId:${item.scaleDbId}`, 'remove', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.scale.getObservationVariables
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.scaleDbId, search, getCount, configurePagination, onEmptyPage]);
