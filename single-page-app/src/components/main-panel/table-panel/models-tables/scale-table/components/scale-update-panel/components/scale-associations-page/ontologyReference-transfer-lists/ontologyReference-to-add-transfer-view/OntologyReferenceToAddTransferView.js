...
export default function OntologyReferenceToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.scaleDbId;

    /*
      API Request: api.scale.getOntologyReference
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.getOntologyReference(graphqlServerUrl, item.scaleDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'scale', association: 'ontologyReference', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.scale.getOntologyReference'}];
            newError.path=['update', `scaleDbId:${currentId}`, 'add', 'ontologyReference'];
      ...
      //error
      .catch((err) => { //error: on api.scale.getOntologyReference
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.scaleDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.scale.getNotAssociatedOntologyReferenceCount
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.getNotAssociatedOntologyReferenceCount(graphqlServerUrl, item.scaleDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'scale', association: 'ontologyReference', table:'A', method: 'getCount()', request: 'api.scale.getNotAssociatedOntologyReferenceCount'}];
            newError.path=['update', `scaleDbId:${item.scaleDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.scale.getNotAssociatedOntologyReferenceCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.scale.getNotAssociatedOntologyReference
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.getNotAssociatedOntologyReference(
      graphqlServerUrl,
      item.scaleDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'scale', association: 'ontologyReference', table:'A', method: 'getData()', request: 'api.scale.getNotAssociatedOntologyReference'}];
            newError.path=['update', `scaleDbId:${item.scaleDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.scale.getNotAssociatedOntologyReference
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.scaleDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'scale', association: 'ontologyReference', table:'B', method: 'getDataB()', request: 'api.ontologyReference.getItems'}];
            newError.path=['update', `scaleDbId:${item.scaleDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.scaleDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'ontologyReference' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'ontologyReferences', 
    ...
    if(lastModelChanged.scale&&
        ...
        lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.scale_ontologyDbId&&
        (lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.scale_ontologyDbId.added ||
         lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.scale_ontologyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.scale_ontologyDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('ontologyReference', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('ontologyReference', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.scale[String(item.scaleDbId)].changedAssociations.scale_ontologyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('ontologyReference', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'ontologyReference' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'ontologyReference',
     * but this action happened on the peer relation, identified by 'scale_ontologyDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'ontologyReference'.
     * B: the current item internalId is in the addedIds of the updated 'ontologyReference'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'ontologyReference' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.ontologyReference) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.scale_ontologyDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.scale_ontologyDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.scale_ontologyDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.scale_ontologyDbId.added) {
              let idsAdded = entry[1].changedAssociations.scale_ontologyDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.scaleDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.ontologyDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('ontologyReference', idAdded);
                  } 
    ...
    if(lastModelChanged.ontologyReference) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          ...
          if(associatedItem && associatedItem.ontologyDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup on associated item
          if(associatedItem && associatedItem.ontologyDbId===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.scaleDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.ontologyDbId) === -1) {
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.ontologyDbId];
        handleDisassociateItem('ontologyReference', associatedItem.ontologyDbId);
      }
                    ...
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.ontologyDbId===it.ontologyDbId) ? 'ontologyReference — ' + t('modelPanels.associatedRecord', 'Associated record') : 'ontologyReference '}>
                                <Avatar>
                                  {"ontologyReference".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'ontologyDbId' }>
                                        <Typography 
                                        id={'OntologyReferenceToAddTransferView-listA-listItem-id-'+it.ontologyDbId}
                                        variant="body1" display="block" noWrap={true}>{it.ontologyDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.ontologyDbId===it.ontologyDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.ontologyDbId===it.ontologyDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'ontologyDbId' }>
                                        <Typography 
                                        id={'OntologyReferenceToAddTransferView-listB-listItem-id-'+it.ontologyDbId}
                                        variant="body1" display="block" noWrap={true}>{it.ontologyDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='OntologyReferenceToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.ontologyDbId;
                        let label = associatedItem.ontologyName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'OntologyReferenceToAddTransferView-associatedItem-list-listItem-'+associatedItem.ontologyDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'ontologyReference — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"ontologyReference".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'ontologyDbId' }>
                                        <Typography 
                                        id={'OntologyReferenceToAddTransferView-listB-listItem-id-'+associatedItem.ontologyDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.ontologyDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'OntologyReferenceToAddTransferView-associatedItem-list-listItem-'+associatedItem.ontologyDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].ontologyDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('ontologyReference', associatedItem.ontologyDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.ontologyDbId];
                                      handleDisassociateItem('ontologyReference', associatedItem.ontologyDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyReferenceToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
