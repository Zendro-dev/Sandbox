...
import ScaleTabsA from './components/ScaleTabsA';
...
//lazy loading
const ScaleAttributesPage = lazy(() => import(/* webpackChunkName: "Update-Attributes-Scale" */ './components/scale-attributes-page/ScaleAttributesPage'));
const ScaleAssociationsPage = lazy(() => import(/* webpackChunkName: "Update-Associations-Scale" */ './components/scale-associations-page/ScaleAssociationsPage'));
const ScaleConfirmationDialog = lazy(() => import(/* webpackChunkName: "Update-Confirmation-Scale" */ './components/ScaleConfirmationDialog'));
...
export default function ScaleUpdatePanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged.scale&&
      lastModelChanged.scale[String(item.scaleDbId)]) {

        //updated item
        if(lastModelChanged.scale[String(item.scaleDbId)].op === "update"&&
            lastModelChanged.scale[String(item.scaleDbId)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.scale[String(item.scaleDbId)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, item.scaleDbId]);
  ...
  function getInitialValues() {
    ...
    initialValues.decimalPlaces = item.decimalPlaces;
    initialValues.scaleName = item.scaleName;
  ...
  initialValueOkStates.decimalPlaces = (item.decimalPlaces!==null ? 1 : 0);
  initialValueOkStates.scaleName = (item.scaleName!==null ? 1 : 0);
  initialValueOkStates.xref = (item.xref!==null ? 1 : 0);
  initialValueOkStates.scaleDbId = (item.scaleDbId!==null ? 1 : 0);
  ...
  function getInitialValueAjvStates() {
    ...
    _initialValueAjvStates.decimalPlaces = {errors: []};
    _initialValueAjvStates.scaleName = {errors: []};
    _initialValueAjvStates.xref = {errors: []};
    _initialValueAjvStates.scaleDbId = {errors: []};
  ...
  function areThereChangedFields() {
    if(Number(values.current.decimalPlaces) !== Number(item.decimalPlaces)) { return true;}
    if(values.current.scaleName !== item.scaleName) { return true;}
  ...
  function setAddRemoveOneOntologyReference(variables) {
    ...
    if(ontologyReferenceIdsToAdd.current.length>0) {
      ...
      changedAssociations.current.scale_ontologyDbId.idsAdded = [ontologyReferenceIdsToAdd.current[0]];
    }
    /**
    ...
    if(ontologyReferenceIdsToRemove.current.length>0) {
      ...
      changedAssociations.current.scale_ontologyDbId.idsRemoved = [ontologyReferenceIdsToRemove.current[0]];
    }
  ...
  function setAddRemoveManyObservationVariables(variables) {
    ...
    if(observationVariablesIdsToAdd.current.length>0) {
      ...
      if(changedAssociations.current.observationVariable_scaleDbId.idsAdded){
        observationVariablesIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.observationVariable_scaleDbId.idsAdded.includes(it)) changedAssociations.current.observationVariable_scaleDbId.idsAdded.push(it);});
      } else {
        changedAssociations.current.observationVariable_scaleDbId.idsAdded = [...observationVariablesIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(observationVariablesIdsToRemove.current.length>0) {
      ...
      if(changedAssociations.current.observationVariable_scaleDbId.idsRemoved){
        observationVariablesIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.observationVariable_scaleDbId.idsRemoved.includes(it)) changedAssociations.current.observationVariable_scaleDbId.idsRemoved.push(it);});
      } else {
        changedAssociations.current.observationVariable_scaleDbId.idsRemoved = [...observationVariablesIdsToRemove.current];
      }
    }
...
function setAjvErrors(err) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.scale.updateItem
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.updateItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'scale', method: 'doSave()', request: 'api.scale.updateItem'}];
            newError.path=['Scales', `scaleDbId:${item.scaleDbId}`, 'update'];
      ...
      //error
      .catch((err) => { //error: on api.scale.updateItem
        if(err.isCanceled) {
          return;
        } else {
          //set ajv errors
  ...
  return (
    <Dialog id='ScaleUpdatePanel-dialog' 
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ScaleUpdatePanel-button-cancel'
              edge="start" 
              color="inherit" 
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": Scale | scaleDbId: " + item.scaleDbId}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " scale" }>
              <Fab
                id='ScaleUpdatePanel-fabButton-save' 
                color="secondary" 
