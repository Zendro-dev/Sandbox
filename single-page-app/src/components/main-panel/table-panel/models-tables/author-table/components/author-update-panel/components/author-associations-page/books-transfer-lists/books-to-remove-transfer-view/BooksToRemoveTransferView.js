...
import BooksToRemoveTransferViewToolbar from './components/BooksToRemoveTransferViewToolbar';
import BooksToRemoveTransferViewCursorPagination from './components/BooksToRemoveTransferViewCursorPagination';
...
export default function BooksToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.author.getBooksCount
    */
    ...
    let cancelableApiReq = makeCancelable(api.author.getBooksCount(
      graphqlServerUrl, 
      item.author_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'books', method: 'getCount()', request: 'api.author.getBooksCount'}];
            newError.path=['update', `author_id:${item.author_id}`, 'remove', 'books'];
      ...
      //error
      .catch((err) => { //error: on api.author.getBooksCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageC, t, item.author_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {book_id: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.author.getBooks
    */
    let variables = {
    ...
    let cancelableApiReq = makeCancelable(api.author.getBooks(
      graphqlServerUrl, 
      item.author_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'books', method: 'getData()', request: 'api.author.getBooks'}];
            newError.path=['update', `author_id:${item.author_id}`, 'remove', 'books'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.author.getBooks
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.author_id, search, getCount, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.book.getCountItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.book.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'author', association: 'books', table:'B', method: 'getCountB()', request: 'api.book.getCountItems'}];
            newError.path=['update', `author_id:${item.author_id}`, 'remove', 'books'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errorsD.current.push(newError);
            console.log("Error: ", newError);
    ...
    //error
    .catch((err) => { //error: on api.book.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.author_id, searchB]);
  ...
  const getDataB = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.book.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
        ...
        (response) => {
          ...
          if(response.message === 'ok') {
            ...
            if(response.graphqlErrors) {
              ...
              newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'author', association: 'books', table:'B', method: 'getDataB()', request: 'api.book.getItems'}];
              newError.path=['update', `author_id:${item.author_id}`, 'remove', 'books'];
              newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
              errorsB.current.push(newError);
              console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.book.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.author_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
     ...
     * That is to say that the current item was associated or dis-associated with some 'books' 
     * (from another place).
     * 
     * Actions:
     ...
     * - return
     */
    if(lastModelChanged.author&&
        lastModelChanged.author[String(item.author_id)]&&
        lastModelChanged.author[String(item.author_id)].changedAssociations&&
        lastModelChanged.author[String(item.author_id)].changedAssociations.book_fk_books_authors&&
        (lastModelChanged.author[String(item.author_id)].changedAssociations.book_fk_books_authors.added ||
         lastModelChanged.author[String(item.author_id)].changedAssociations.book_fk_books_authors.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.author[String(item.author_id)].changedAssociations.book_fk_books_authors.idsRemoved;
          if(idsRemoved) {
          ...
          let idsAdded = lastModelChanged.author[String(item.author_id)].changedAssociations.book_fk_books_authors.idsAdded;
          if(idsAdded) {
            //increment A
     ...
     * That is to say that this current item was associated or dis-associated with some 'book',
     * but this action happened on the peer relation, identified by 'book_fk_books_authors'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'book'.
     * B: the current item internalId is in the addedIds of the updated 'book'.
     * 
     * Actions:
     * if A:
     * - remove the internalId of the 'book' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
     ...
     * - return
     */
    if(lastModelChanged.book) {
      let oens = Object.entries(lastModelChanged.book);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.book_fk_books_authors) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.book_fk_books_authors.removed) {
              let idsRemoved = entry[1].changedAssociations.book_fk_books_authors.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.author_id);
                if(iof !== -1) {
                  //remove changed item from lidsToRemove
                  let idRemoved = entry[1].newItem.book_id;
            ...
            if(entry[1].changedAssociations.book_fk_books_authors.added) {
              let idsAdded = entry[1].changedAssociations.book_fk_books_authors.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.author_id);
                if(iof !== -1) {
    ...
    /*
     * Case 3: 
     * The attributes of some 'book' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.book) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.book_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.book_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.book_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.book_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.book_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.book_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.author_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnBookRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.book_id) === -1) {
      lidsToRemove.current.push(item.book_id);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('books', item.book_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.book_id);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('books', item.book_id);
    }
  }
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <BooksToRemoveTransferViewToolbar
                title={'Books'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='BooksToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='BooksToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.book_id;
                        let label = it.name;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(book_id=> book_id=== it.book_id) !== undefined;

                        return (
                          <ListItem 
                            id={'BooksToRemoveTransferView-listA-listItem-'+it.book_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'book_id' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.book_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'BooksToRemoveTransferView-listA-listItem-'+it.book_id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <BooksToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='BooksToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='BooksToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='BooksToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.book_id;
                        let label = it.name;
                        ...
                        return (
                          <ListItem 
                            id={'BooksToRemoveTransferView-listB-listItem-'+it.book_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'BooksToRemoveTransferView-listB-listItem-'+it.book_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
BooksToRemoveTransferView.propTypes = {
