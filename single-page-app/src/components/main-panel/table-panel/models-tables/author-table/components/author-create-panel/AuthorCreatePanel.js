...
import AuthorTabsA from './components/AuthorTabsA';
...
//lazy loading
const AuthorAttributesPage = lazy(() => retry(() => import(/* webpackChunkName: "Create-AttributesAuthor" */ './components/author-attributes-page/AuthorAttributesPage')));
const AuthorAssociationsPage = lazy(() => retry(() => import(/* webpackChunkName: "Create-AssociationsAuthor" */ './components/author-associations-page/AuthorAssociationsPage')));
const AuthorConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Create-ConfirmationAuthor" */ './components/AuthorConfirmationDialog')));
const BookDetailPanel = lazy(() => retry(() => import(/* webpackChunkName: "Create-DetailBook" */ '../../../book-table/components/book-detail-panel/BookDetailPanel')));
...
export default function AuthorCreatePanel(props) {
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.author_id = 0;
  ...
  function getInitialValueAjvStates() {
    ...
    _initialValueAjvStates.author_id = {errors: []};
  ...
  async function doSave(event) {
    ...
    */
    ...
    //add: to_many's
    variables.addBooks = [...booksIdsToAdd.current];

    /*
      API Request: api.author.createItem
    */
    ...
    let cancelableApiReq = makeCancelable(api.author.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'author', method: 'doSave()', request: 'api.author.createItem'}];
            newError.path=['Authors', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.author.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'books') {
      let iof = booksIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        booksIdsToAdd.current.splice(iof, 1);
      }
      return;
    }//end: case 'books'
  }
  ...
  return (
    ...
    <Dialog id='AuthorCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='AuthorCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Author'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " author" }>
            <Fab
              id='AuthorCreatePanel-fabButton-save' 
              color="secondary" 
