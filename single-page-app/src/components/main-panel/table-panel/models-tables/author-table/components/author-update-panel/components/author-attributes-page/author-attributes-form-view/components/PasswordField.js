...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['author'][String(itemId['author_id'])]) {

        //updated password
        if(lastModelChanged['author'][String(itemId['author_id'])].op === "updatePassword"&&
            lastModelChanged['author'][String(itemId['author_id'])].newItem) {
              //update password value
              setValue(lastModelChanged['author'][String(itemId['author_id'])].newItem[itemKey]);
        }
    }//end: Case 1
  ...
  /**
   ...
   async function doSave() {
    ...
    /*
      API Request: api.author.updateItem
    */
    ...
    return await api.author.updateItem(graphqlServerUrl, variables)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'author', method: 'doSave()', request: 'api.author.updateItem'}];
            newError.path=['Authors', itemId, 'update', 'change password'];
      ...
      //error
      .catch((err) => { //error: on api.author.updateItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  return (
    <div>
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'PasswordField-Author-button-showPassword'+name}
                    ...
                    {(valueOk!==undefined&&valueOk===1) 
                      ...
                      <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                        <Typography id={'PasswordField-Author-exists-'+name} variant="caption" color="primary">
                          &#8707;
                        </Typography>
                      </Tooltip>
                      : 
                      <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                        <Typography id={'PasswordField-Author-notExists-'+name} variant="caption" color="textSecondary">
                          &#8708;
                        </Typography>
                      </Tooltip>
                    }
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'PasswordField-Author-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-Author-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-Author-ChangePasswordDialog-textField-newPassword2-'+name}
