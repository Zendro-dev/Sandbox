...
export default function BooksToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"book_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.author.getNotAssociatedBooksCount
    */
    ...
    let cancelableApiReq = makeCancelable(api.author.getNotAssociatedBooksCount(graphqlServerUrl, item.author_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'author', association: 'books', table:'A', method: 'getCount()', request: 'api.author.getNotAssociatedBooksCount'}];
            newError.path=['update', `author_id:${item.author_id}`, 'add', 'books'];
      ...
      //error
      .catch((err) => { //error: on api.author.getNotAssociatedBooksCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.author.getNotAssociatedBooks
    */
    let api = await loadApi("author");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.author.getNotAssociatedBooks(
      graphqlServerUrl,
      item.author_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'author', association: 'books', table:'A', method: 'getData()', request: 'api.author.getNotAssociatedBooks'}];
            newError.path=['update', `author_id:${item.author_id}`, 'add', 'books'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.author.getNotAssociatedBooks
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.author_id, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  useEffect(() => {
     ...
     * 
     * else if B:
     * - remove the internalId of the 'book' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
            ...
            if(entry[1].changedAssociations.book_fk_books_authors.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.book_id;
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.book_id) === -1) {
      lidsToAdd.current.push(item.book_id);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.book_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    ...
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'BooksToAddTransferView-listA-listItem-'+it.book_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'book_id' }>
                                        <Typography 
                                        id={'BooksToAddTransferView-listA-listItem-id-'+it.book_id}
                                        variant="body1" display="block" noWrap={true}>{it.book_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'BooksToAddTransferView-listA-listItem-'+it.book_id+'-button-add'}
                                  color="primary"
                  ...
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='BooksToAddTransferView-list-listB'
                      ...
                      {itemsB.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'BooksToAddTransferView-listB-listItem-'+it.book_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'book_id' }>
                                        <Typography 
                                        id={'BooksToAddTransferView-listB-listItem-id-'+it.book_id}
                                        variant="body1" display="block" noWrap={true}>{it.book_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'BooksToAddTransferView-listB-listItem-'+it.book_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
