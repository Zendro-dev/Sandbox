...
import AuthorEnhancedTableHead from './components/AuthorEnhancedTableHead'
import AuthorEnhancedTableToolbar from './components/AuthorEnhancedTableToolbar'
import AuthorUploadFileDialog from './components/AuthorUploadFileDialog'
import AuthorCursorPagination from './components/AuthorCursorPagination'
...
//lazy loading
const AuthorCreatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Create-Author" */ './components/author-create-panel/AuthorCreatePanel')));
const AuthorUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Update-Author" */ './components/author-update-panel/AuthorUpdatePanel')));
const AuthorDetailPanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Author" */ './components/author-detail-panel/AuthorDetailPanel')));
const AuthorDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Delete-Author" */ './components/AuthorDeleteConfirmationDialog')));
...
export default function AuthorEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('author_id');
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.author.getCountItems
    */
    ...
    let cancelableApiReq = makeCancelable(api.author.getCountItems(graphqlServerUrl, search));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'author', method: 'getCount()', request: 'api.author.getCountItems'}];
            newError.path=['Authors'];
      ...
      //error
      .catch((err) => { //error: on api.author.getCountItems
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'author', method: 'getData()', request: 'api.author.getItems'}];
            newError.path=['Authors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['author']
    &&(Object.keys(changes.lastModelChanged['author']).length===1)
    &&(changes.lastModelChanged['author'][Object.keys(changes.lastModelChanged['author'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.author.tableTemplate
    */
    let api = await loadApi("author");
    if(!api) {
      let newError = {};
    ...
    let cancelableApiReq = makeCancelable(api.author.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'author', method: 'getCsvTemplate()', request: 'api.author.tableTemplate'}];
            newError.path=['Authors', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateAuthor.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "author-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.author.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='AuthorEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='AuthorEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <AuthorEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='AuthorEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'AuthorEnhancedTable-row-'+item.author_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'AuthorEnhancedTable-row-iconButton-detail-'+item.author_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'AuthorEnhancedTable-row-iconButton-edit-'+item.author_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'AuthorEnhancedTable-row-iconButton-delete-'+item.author_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'author_id: ' + item.author_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.author_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
