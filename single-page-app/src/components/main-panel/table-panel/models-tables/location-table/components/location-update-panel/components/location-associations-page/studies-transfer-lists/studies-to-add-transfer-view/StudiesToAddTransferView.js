...
export default function StudiesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.location.getNotAssociatedStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.location.getNotAssociatedStudiesCount(graphqlServerUrl, item.locationDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'location', association: 'studies', table:'A', method: 'getCount()', request: 'api.location.getNotAssociatedStudiesCount'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'add', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.location.getNotAssociatedStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.locationDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.location.getNotAssociatedStudies
    */
    let cancelableApiReq = makeCancelable(api.location.getNotAssociatedStudies(
      graphqlServerUrl,
      item.locationDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'location', association: 'studies', table:'A', method: 'getData()', request: 'api.location.getNotAssociatedStudies'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.location.getNotAssociatedStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.locationDbId, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'location', association: 'studies', table:'B', method: 'getCountB()', request: 'api.study.getCountItems'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'add', 'studies'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.locationDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'location', association: 'studies', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.locationDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.location&&
        ...
        lastModelChanged.location[String(item.locationDbId)].changedAssociations&&
        lastModelChanged.location[String(item.locationDbId)].changedAssociations.study_locationDbId&&
        (lastModelChanged.location[String(item.locationDbId)].changedAssociations.study_locationDbId.added ||
         lastModelChanged.location[String(item.locationDbId)].changedAssociations.study_locationDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.location[String(item.locationDbId)].changedAssociations.study_locationDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.location[String(item.locationDbId)].changedAssociations.study_locationDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_locationDbId) {
            ...
            if(entry[1].changedAssociations.study_locationDbId.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.locationDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.study_locationDbId.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.locationDbId);
                if(iof !== -1) {
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.studyDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.locationDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.studyDbId) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('studies', item.studyDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.studyDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('studies', item.studyDbId);
    }
  };
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    ...
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'StudiesToAddTransferView-listA-listItem-'+it.studyDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'studyDbId' }>
                                        <Typography 
                                        id={'StudiesToAddTransferView-listA-listItem-id-'+it.studyDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'StudiesToAddTransferView-listA-listItem-'+it.studyDbId+'-button-add'}
                                  color="primary"
                  ...
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='StudiesToAddTransferView-list-listB'
                      ...
                      {itemsB.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'StudiesToAddTransferView-listB-listItem-'+it.studyDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'studyDbId' }>
                                        <Typography 
                                        id={'StudiesToAddTransferView-listB-listItem-id-'+it.studyDbId}
                                        variant="body1" display="block" noWrap={true}>{it.studyDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'StudiesToAddTransferView-listB-listItem-'+it.studyDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
