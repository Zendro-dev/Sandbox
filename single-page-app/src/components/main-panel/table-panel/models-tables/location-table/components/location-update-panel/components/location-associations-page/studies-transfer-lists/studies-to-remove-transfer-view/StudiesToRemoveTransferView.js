...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.location.getStudiesCount
    */
    let api = await loadApi("location");
    let cancelableApiReq = makeCancelable(api.location.getStudiesCount(
      graphqlServerUrl, 
      item.locationDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'studies', method: 'getCount()', request: 'api.location.getStudiesCount'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'remove', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.location.getStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {studyDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.location.getStudies
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("location");
    let cancelableApiReq = makeCancelable(api.location.getStudies(
      graphqlServerUrl, 
      item.locationDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'studies', method: 'getData()', request: 'api.location.getStudies'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'remove', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.location.getStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.locationDbId, search, getCount, configurePagination, onEmptyPage]);
            ...
            if(entry[1].changedAssociations.study_locationDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.studyDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.studyDbId) === -1) {
      lidsToRemove.current.push(item.studyDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.studyDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
                  ...
                  <Box className={classes.listBox} ref={lref}>
                    <List id='StudiesToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        let disabled = lidsToRemove.current.find(studyDbId=> studyDbId=== it.studyDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'StudiesToRemoveTransferView-listA-listItem-'+it.studyDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'studyDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.studyDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'StudiesToRemoveTransferView-listA-listItem-'+it.studyDbId+'-button-remove'}
                                    color="primary"
                  ...
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='StudiesToRemoveTransferView-list-listB'
                      ...
                      {itemsB.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'StudiesToRemoveTransferView-listB-listItem-'+it.studyDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'StudiesToRemoveTransferView-listB-listItem-'+it.studyDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
