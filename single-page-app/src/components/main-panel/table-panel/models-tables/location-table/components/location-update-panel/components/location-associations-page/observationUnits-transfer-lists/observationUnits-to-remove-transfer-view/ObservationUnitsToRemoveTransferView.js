...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.location.getObservationUnitsCount
    */
    let cancelableApiReq = makeCancelable(api.location.getObservationUnitsCount(
      graphqlServerUrl, 
      item.locationDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationUnits', method: 'getCount()', request: 'api.location.getObservationUnitsCount'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'remove', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.location.getObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.location.getObservationUnits
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.location.getObservationUnits(
      graphqlServerUrl, 
      item.locationDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationUnits', method: 'getData()', request: 'api.location.getObservationUnits'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.location.getObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'location', association: 'observationUnits', table:'B', method: 'getCountB()', request: 'api.observationUnit.getCountItems'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
        ...
        (response) => {
          ...
          if(response.message === 'ok') {
            ...
            if(response.graphqlErrors) {
              ...
              newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'location', association: 'observationUnits', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
              newError.path=['update', `locationDbId:${item.locationDbId}`, 'remove', 'observationUnits'];
              newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.location&&
        ...
        lastModelChanged.location[String(item.locationDbId)].changedAssociations.observationUnit_locationDbId&&
        (lastModelChanged.location[String(item.locationDbId)].changedAssociations.observationUnit_locationDbId.added ||
         lastModelChanged.location[String(item.locationDbId)].changedAssociations.observationUnit_locationDbId.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.location[String(item.locationDbId)].changedAssociations.observationUnit_locationDbId.idsRemoved;
          if(idsRemoved) {
          ...
          let idsAdded = lastModelChanged.location[String(item.locationDbId)].changedAssociations.observationUnit_locationDbId.idsAdded;
          if(idsAdded) {
            //increment A
