...
export default function ObservationUnitsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.location.getNotAssociatedObservationUnitsCount
    */
    let api = await loadApi("location");
    let cancelableApiReq = makeCancelable(api.location.getNotAssociatedObservationUnitsCount(graphqlServerUrl, item.locationDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'location', association: 'observationUnits', table:'A', method: 'getCount()', request: 'api.location.getNotAssociatedObservationUnitsCount'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'add', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.location.getNotAssociatedObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.location.getNotAssociatedObservationUnits
    */
    let api = await loadApi("location");
    let cancelableApiReq = makeCancelable(api.location.getNotAssociatedObservationUnits(
      graphqlServerUrl,
      item.locationDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'location', association: 'observationUnits', table:'A', method: 'getData()', request: 'api.location.getNotAssociatedObservationUnits'}];
            newError.path=['update', `locationDbId:${item.locationDbId}`, 'add', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.location.getNotAssociatedObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
