...
//lazy loading
const LocationCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Location" */ './components/location-create-panel/LocationCreatePanel'));
const LocationUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Location" */ './components/location-update-panel/LocationUpdatePanel'));
const LocationDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Location" */ './components/location-detail-panel/LocationDetailPanel'));
const LocationDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Location" */ './components/LocationDeleteConfirmationDialog'));
//Plotly
const LocationPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Location" */ '../../../../plots/LocationPlotly'));
...
export default function LocationEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('locationDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.location.getCountItems
    */
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'location', method: 'getCount()', request: 'api.location.getCountItems'}];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'location', method: 'getData()', request: 'api.location.getItems'}];
            newError.path=['Locations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['location']
    &&(Object.keys(changes.lastModelChanged['location']).length===1)
    &&(changes.lastModelChanged['location'][Object.keys(changes.lastModelChanged['location'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set locationDbId (internalId)
    variables.locationDbId = item.locationDbId;

    /*
      API Request: api.location.deleteItem
    */
    let api = await loadApi("location");
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'location', method: 'doDelete()', request: 'api.location.deleteItem'}];
            newError.path=['Locations', `locationDbId:${item.locationDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.location.tableTemplate
    */
    let api = await loadApi("location");
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'location', method: 'getCsvTemplate()', request: 'api.location.tableTemplate'}];
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='LocationEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='LocationEnhancedTable-table' stickyHeader size='small' ref={tref}>
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='LocationEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'LocationEnhancedTable-row-'+item.locationDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.locationDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'LocationEnhancedTable-row-iconButton-detail-'+item.locationDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'LocationEnhancedTable-row-iconButton-edit-'+item.locationDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'LocationEnhancedTable-row-iconButton-delete-'+item.locationDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'locationDbId: ' + item.locationDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.locationDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.coordinateDescription!==null)?item.coordinateDescription:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.countryCode!==null)?item.countryCode:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.countryName!==null)?item.countryName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.environmentType!==null)?item.environmentType:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.exposure!==null)?item.exposure:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.instituteAddress!==null)?item.instituteAddress:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.locationName!==null)?item.locationName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.locationType!==null)?item.locationType:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.siteStatus!==null)?item.siteStatus:'')}
                                    </TableCell>
