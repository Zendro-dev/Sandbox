...
import EnvironmentParameterEnhancedTableHead from './components/EnvironmentParameterEnhancedTableHead'
import EnvironmentParameterEnhancedTableToolbar from './components/EnvironmentParameterEnhancedTableToolbar'
import EnvironmentParameterUploadFileDialog from './components/EnvironmentParameterUploadFileDialog'
import EnvironmentParameterCursorPagination from './components/EnvironmentParameterCursorPagination'
...
//lazy loading
const EnvironmentParameterCreatePanel = lazy(() => import(/* webpackChunkName: "Create-EnvironmentParameter" */ './components/environmentParameter-create-panel/EnvironmentParameterCreatePanel'));
const EnvironmentParameterUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-EnvironmentParameter" */ './components/environmentParameter-update-panel/EnvironmentParameterUpdatePanel'));
const EnvironmentParameterDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-EnvironmentParameter" */ './components/environmentParameter-detail-panel/EnvironmentParameterDetailPanel'));
const EnvironmentParameterDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-EnvironmentParameter" */ './components/EnvironmentParameterDeleteConfirmationDialog'));
//Plotly
const EnvironmentParameterPlotly = lazy(() => import(/* webpackChunkName: "Plotly-EnvironmentParameter" */ '../../../../plots/EnvironmentParameterPlotly'));
...
export default function EnvironmentParameterEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('environmentParameterDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.environmentParameter.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'environmentParameter', method: 'getCount()', request: 'api.environmentParameter.getCountItems'}];
            newError.path=['EnvironmentParameters'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'environmentParameter', method: 'getData()', request: 'api.environmentParameter.getItems'}];
            newError.path=['EnvironmentParameters'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['environmentParameter']
    &&(Object.keys(changes.lastModelChanged['environmentParameter']).length===1)
    &&(changes.lastModelChanged['environmentParameter'][Object.keys(changes.lastModelChanged['environmentParameter'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set environmentParameterDbId (internalId)
    variables.environmentParameterDbId = item.environmentParameterDbId;

    /*
      API Request: api.environmentParameter.deleteItem
    */
    let api = await loadApi("environmentParameter");
    let cancelableApiReq = makeCancelable(api.environmentParameter.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'environmentParameter', method: 'doDelete()', request: 'api.environmentParameter.deleteItem'}];
            newError.path=['EnvironmentParameters', `environmentParameterDbId:${item.environmentParameterDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.environmentParameter.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.environmentParameter.tableTemplate
    */
    let api = await loadApi("environmentParameter");
    let cancelableApiReq = makeCancelable(api.environmentParameter.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'environmentParameter', method: 'getCsvTemplate()', request: 'api.environmentParameter.tableTemplate'}];
            newError.path=['EnvironmentParameters', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateEnvironmentParameter.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "environmentParameter-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.environmentParameter.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <EnvironmentParameterEnhancedTableToolbar
                ...
                >
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='EnvironmentParameterEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='EnvironmentParameterEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <EnvironmentParameterEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='EnvironmentParameterEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'EnvironmentParameterEnhancedTable-row-'+item.environmentParameterDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.environmentParameterDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'EnvironmentParameterEnhancedTable-row-iconButton-detail-'+item.environmentParameterDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.environmentParameter&&Array.isArray(permissions.environmentParameter)
                                      &&(permissions.environmentParameter.includes('update') || permissions.environmentParameter.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'EnvironmentParameterEnhancedTable-row-iconButton-edit-'+item.environmentParameterDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.environmentParameter&&Array.isArray(permissions.environmentParameter)
                                      &&(permissions.environmentParameter.includes('delete') || permissions.environmentParameter.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'EnvironmentParameterEnhancedTable-row-iconButton-delete-'+item.environmentParameterDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'environmentParameterDbId: ' + item.environmentParameterDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.environmentParameterDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.parameterName!==null)?item.parameterName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.parameterPUI!==null)?item.parameterPUI:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.unitPUI!==null)?item.unitPUI:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.valuePUI!==null)?item.valuePUI:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.studyDbId!==null)?item.studyDbId:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <EnvironmentParameterCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(EnvironmentParameterPlotly)) && (
                      <Suspense fallback={<div />}>
                        <EnvironmentParameterPlotly />
                      </Suspense>
                    )}
                  </div>
      ...
      {(createDialogOpen) && (
        <Suspense fallback={<div />}>
          <EnvironmentParameterCreatePanel
            permissions={permissions}
      ...
      {(updateDialogOpen) && (
        <Suspense fallback={<div />}>
          <EnvironmentParameterUpdatePanel
            permissions={permissions}
            item={updateItem}
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <EnvironmentParameterDeleteConfirmationDialog
            permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <EnvironmentParameterUploadFileDialog
...
EnvironmentParameterEnhancedTable.propTypes = {
