...
import StudyToAddTransferViewToolbar from './components/StudyToAddTransferViewToolbar';
import StudyToAddTransferViewCursorPagination from './components/StudyToAddTransferViewCursorPagination';
...
export default function StudyToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.environmentParameterDbId;

    /*
      API Request: api.environmentParameter.getStudy
    */
    let api = await loadApi("environmentParameter");
    let cancelableApiReq = makeCancelable(api.environmentParameter.getStudy(graphqlServerUrl, item.environmentParameterDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'environmentParameter', association: 'study', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.environmentParameter.getStudy'}];
            newError.path=['update', `environmentParameterDbId:${currentId}`, 'add', 'study'];
      ...
      //error
      .catch((err) => { //error: on api.environmentParameter.getStudy
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.environmentParameterDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"studyDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.environmentParameter.getNotAssociatedStudyCount
    */
    let api = await loadApi("environmentParameter");
    let cancelableApiReq = makeCancelable(api.environmentParameter.getNotAssociatedStudyCount(graphqlServerUrl, item.environmentParameterDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'environmentParameter', association: 'study', table:'A', method: 'getCount()', request: 'api.environmentParameter.getNotAssociatedStudyCount'}];
            newError.path=['update', `environmentParameterDbId:${item.environmentParameterDbId}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.environmentParameter.getNotAssociatedStudyCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.environmentParameterDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.environmentParameter.getNotAssociatedStudy
    */
    let api = await loadApi("environmentParameter");
    let cancelableApiReq = makeCancelable(api.environmentParameter.getNotAssociatedStudy(
      graphqlServerUrl,
      item.environmentParameterDbId,
      search,
      variables,
      ops,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'environmentParameter', association: 'study', table:'A', method: 'getData()', request: 'api.environmentParameter.getNotAssociatedStudy'}];
            newError.path=['update', `environmentParameterDbId:${item.environmentParameterDbId}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.environmentParameter.getNotAssociatedStudy
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.environmentParameterDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.study.getItems
    */
    let api = await loadApi("study");
    let cancelableApiReq = makeCancelable(api.study.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'environmentParameter', association: 'study', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `environmentParameterDbId:${item.environmentParameterDbId}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.study.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.environmentParameterDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'study' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'studies', 
     ...
     * - return
     */
    if(lastModelChanged.environmentParameter&&
        lastModelChanged.environmentParameter[String(item.environmentParameterDbId)]&&
        lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations&&
        lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations.environmentParameter_studyDbId&&
        (lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations.environmentParameter_studyDbId.added ||
         lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations.environmentParameter_studyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations.environmentParameter_studyDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('study', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('study', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.environmentParameter[String(item.environmentParameterDbId)].changedAssociations.environmentParameter_studyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('study', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'study' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'study',
     * but this action happened on the peer relation, identified by 'environmentParameter_studyDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'study'.
     * B: the current item internalId is in the addedIds of the updated 'study'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'study' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.study) {
      let oens = Object.entries(lastModelChanged.study);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.environmentParameter_studyDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.environmentParameter_studyDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.environmentParameter_studyDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.environmentParameterDbId);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('study', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.environmentParameter_studyDbId.added) {
              let idsAdded = entry[1].changedAssociations.environmentParameter_studyDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.environmentParameterDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.studyDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('study', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'study' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.studyDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.studyDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.studyDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.studyDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.studyDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.studyDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('study', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.studyDbId===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.environmentParameterDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnStudyRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.studyDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.studyDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.studyDbId];
        handleDisassociateItem('study', associatedItem.studyDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('study', item.studyDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('study', item.studyDbId);
    }
  };
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='StudyToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='StudyToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.studyDbId;
                        let label = it.studyName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'StudyToAddTransferView-listA-listItem-'+it.studyDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.studyDbId===it.studyDbId) ? 'study — ' + t('modelPanels.associatedRecord', 'Associated record') : 'study '}>
                                <Avatar>
                                  {"study".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'studyDbId' }>
                                        <Typography 
                                        id={'StudyToAddTransferView-listA-listItem-id-'+it.studyDbId}
                                        variant="body1" display="block" noWrap={true}>{it.studyDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.studyDbId===it.studyDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.studyDbId===it.studyDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'StudyToAddTransferView-listA-listItem-'+it.studyDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <StudyToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='StudyToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='StudyToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='StudyToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.studyDbId;
                        let label = it.studyName;
                        ...
                        return (
                          <ListItem 
                            id={'StudyToAddTransferView-listB-listItem-'+it.studyDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'studyDbId' }>
                                        <Typography 
                                        id={'StudyToAddTransferView-listB-listItem-id-'+it.studyDbId}
                                        variant="body1" display="block" noWrap={true}>{it.studyDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'StudyToAddTransferView-listB-listItem-'+it.studyDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='StudyToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.studyDbId;
                        let label = associatedItem.studyName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'StudyToAddTransferView-associatedItem-list-listItem-'+associatedItem.studyDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'study — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"study".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'studyDbId' }>
                                        <Typography 
                                        id={'StudyToAddTransferView-listB-listItem-id-'+associatedItem.studyDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.studyDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'StudyToAddTransferView-associatedItem-list-listItem-'+associatedItem.studyDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].studyDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('study', associatedItem.studyDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.studyDbId];
                                      handleDisassociateItem('study', associatedItem.studyDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='StudyToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
StudyToAddTransferView.propTypes = {
