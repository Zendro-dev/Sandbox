...
import OntologyAnnotationEnhancedTableHead from './components/OntologyAnnotationEnhancedTableHead'
import OntologyAnnotationEnhancedTableToolbar from './components/OntologyAnnotationEnhancedTableToolbar'
import OntologyAnnotationCreatePanel from './components/ontologyAnnotation-create-panel/OntologyAnnotationCreatePanel'
import OntologyAnnotationUpdatePanel from './components/ontologyAnnotation-update-panel/OntologyAnnotationUpdatePanel'
import OntologyAnnotationDetailPanel from './components/ontologyAnnotation-detail-panel/OntologyAnnotationDetailPanel'
import OntologyAnnotationDeleteConfirmationDialog from './components/OntologyAnnotationDeleteConfirmationDialog'
import OntologyAnnotationUploadFileDialog from './components/OntologyAnnotationUploadFileDialog'
import OntologyAnnotationCursorPagination from './components/OntologyAnnotationCursorPagination'
...
//Plotly
import OntologyAnnotationPlotly from '../../../../plots/OntologyAnnotationPlotly';
...
export default function OntologyAnnotationEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('ontologyAnnotation_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyAnnotation', method: 'getCount()', request: 'api.ontologyAnnotation.getCountItems'}];
            newError.path=['OntologyAnnotations'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', method: 'getData()', request: 'api.ontologyAnnotation.getItems'}];
            newError.path=['OntologyAnnotations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['ontologyAnnotation']
    &&(Object.keys(changes.lastModelChanged['ontologyAnnotation']).length===1)
    &&(changes.lastModelChanged['ontologyAnnotation'][Object.keys(changes.lastModelChanged['ontologyAnnotation'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set ontologyAnnotation_id (internalId)
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.ontologyAnnotation.tableTemplate
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', method: 'getCsvTemplate()', request: 'api.ontologyAnnotation.tableTemplate'}];
            newError.path=['OntologyAnnotations', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateOntologyAnnotation.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "ontologyAnnotation-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <OntologyAnnotationEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(OntologyAnnotationPlotly)}
                ...
                <SwipeableViews index={getSwipeIndex()} disabled={true}>
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='OntologyAnnotationEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='OntologyAnnotationEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <OntologyAnnotationEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='OntologyAnnotationEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'OntologyAnnotationEnhancedTable-row-'+item.ontologyAnnotation_id}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.ontologyAnnotation_id}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'OntologyAnnotationEnhancedTable-row-iconButton-detail-'+item.ontologyAnnotation_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'OntologyAnnotationEnhancedTable-row-iconButton-edit-'+item.ontologyAnnotation_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'OntologyAnnotationEnhancedTable-row-iconButton-delete-'+item.ontologyAnnotation_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'ontologyAnnotation_id: ' + item.ontologyAnnotation_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.ontologyAnnotation_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.ontology!==null)?item.ontology:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.ontologyURL!==null)?item.ontologyURL:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.term!==null)?item.term:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.termURL!==null)?item.termURL:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.investigation_ids!==null)?item.investigation_ids:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.study_ids!==null)?item.study_ids:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.assay_ids!==null)?item.assay_ids:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.assayResult_ids!==null)?item.assayResult_ids:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.factor_ids!==null)?item.factor_ids:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.material_ids!==null)?item.material_ids:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.protocol_ids!==null)?item.protocol_ids:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <OntologyAnnotationCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(OntologyAnnotationPlotly)) && (
                      <OntologyAnnotationPlotly />
                    )}
                  </div>
      ...
      {(uploadFileDialogOpen) && (
        <OntologyAnnotationUploadFileDialog
...
OntologyAnnotationEnhancedTable.propTypes = {
