...
import AssayResultsToAddTransferViewToolbar from './components/AssayResultsToAddTransferViewToolbar';
import AssayResultsToAddTransferViewCursorPagination from './components/AssayResultsToAddTransferViewCursorPagination';
...
export default function AssayResultsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"assayResult_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.ontologyAnnotation.getNotAssociatedAssayResultsCount
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedAssayResultsCount(graphqlServerUrl, item.ontologyAnnotation_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyAnnotation', association: 'assayResults', table:'A', method: 'getCount()', request: 'api.ontologyAnnotation.getNotAssociatedAssayResultsCount'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'assayResults'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedAssayResultsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getNotAssociatedAssayResults
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedAssayResults(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'assayResults', table:'A', method: 'getData()', request: 'api.ontologyAnnotation.getNotAssociatedAssayResults'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'assayResults'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedAssayResults
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.assayResult.getCountItems
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'assayResults', table:'B', method: 'getCountB()', request: 'api.assayResult.getCountItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'assayResults'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.assayResult.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.assayResult.getItems
    */
    let cancelableApiReq = makeCancelable(api.assayResult.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'assayResults', table:'B', method: 'getDataB()', request: 'api.assayResult.getItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'assayResults'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'assayResults' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'assayResults', 
    ...
    if(lastModelChanged.ontologyAnnotation&&
        ...
        lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_assayResult_ids&&
        (lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_assayResult_ids.added ||
         lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_assayResult_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_assayResult_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('assayResults', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_assayResult_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'assayResults' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'assayResult',
     * but this action happened on the peer relation, identified by 'ontologyAnnotation_assayResult_ids'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'assayResult'.
     * B: the current item internalId is in the addedIds of the updated 'assayResult'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'assayResult' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.assayResult) {
      let oens = Object.entries(lastModelChanged.assayResult);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.ontologyAnnotation_assayResult_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.ontologyAnnotation_assayResult_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.ontologyAnnotation_assayResult_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.ontologyAnnotation_assayResult_ids.added) {
              let idsAdded = entry[1].changedAssociations.ontologyAnnotation_assayResult_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.ontologyAnnotation_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.assayResult_id;
    ...
    /*
     * Case 3: 
     * The attributes of some 'assayResult' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.assayResult) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.assayResult_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.assayResult_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.assayResult_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.assayResult_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.assayResult_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.assayResult_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('assayResults', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnAssayResultRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.assayResult_id) === -1) {
      lidsToAdd.current.push(item.assayResult_id);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('assayResults', item.assayResult_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.assayResult_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('assayResults', item.assayResult_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <AssayResultsToAddTransferViewToolbar
                title={'AssayResults'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayResultsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='AssayResultsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.assayResult_id;
                        let label = it.value_as_str;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'AssayResultsToAddTransferView-listA-listItem-'+it.assayResult_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'assayResult' }>
                                <Avatar>{"assayResult".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'assayResult_id' }>
                                        <Typography 
                                        id={'AssayResultsToAddTransferView-listA-listItem-id-'+it.assayResult_id}
                                        variant="body1" display="block" noWrap={true}>{it.assayResult_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'AssayResultsToAddTransferView-listA-listItem-'+it.assayResult_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <AssayResultsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayResultsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayResultsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='AssayResultsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.assayResult_id;
                        let label = it.value_as_str;
                        ...
                        return (
                          <ListItem 
                            id={'AssayResultsToAddTransferView-listB-listItem-'+it.assayResult_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'assayResult_id' }>
                                        <Typography 
                                        id={'AssayResultsToAddTransferView-listB-listItem-id-'+it.assayResult_id}
                                        variant="body1" display="block" noWrap={true}>{it.assayResult_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'AssayResultsToAddTransferView-listB-listItem-'+it.assayResult_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
AssayResultsToAddTransferView.propTypes = {
