...
import InvestigationsToAddTransferViewToolbar from './components/InvestigationsToAddTransferViewToolbar';
import InvestigationsToAddTransferViewCursorPagination from './components/InvestigationsToAddTransferViewCursorPagination';
...
export default function InvestigationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getNotAssociatedInvestigationsCount
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedInvestigationsCount(graphqlServerUrl, item.ontologyAnnotation_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyAnnotation', association: 'investigations', table:'A', method: 'getCount()', request: 'api.ontologyAnnotation.getNotAssociatedInvestigationsCount'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'investigations'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedInvestigationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.ontologyAnnotation_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getNotAssociatedInvestigations
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedInvestigations(
      graphqlServerUrl,
      item.ontologyAnnotation_id,
      search,
      variables,
      ops,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'investigations', table:'A', method: 'getData()', request: 'api.ontologyAnnotation.getNotAssociatedInvestigations'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedInvestigations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.ontologyAnnotation_id, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.investigation.getCountItems
    */
    let cancelableApiReq = makeCancelable(api.investigation.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'investigations', table:'B', method: 'getCountB()', request: 'api.investigation.getCountItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'investigations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.investigation.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.ontologyAnnotation_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'investigations', table:'B', method: 'getDataB()', request: 'api.investigation.getItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.ontologyAnnotation_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'investigations' for this item was updated.
    ...
    if(lastModelChanged.ontologyAnnotation&&
        ...
        lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations&&
        lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_investigation_ids&&
        (lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_investigation_ids.added ||
         lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_investigation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_investigation_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('investigations', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_investigation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'investigations' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'investigation',
     * but this action happened on the peer relation, identified by 'ontologyAnnotation_investigation_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.investigation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.ontologyAnnotation_investigation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.ontologyAnnotation_investigation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.ontologyAnnotation_investigation_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.ontologyAnnotation_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.ontologyAnnotation_investigation_ids.added) {
              let idsAdded = entry[1].changedAssociations.ontologyAnnotation_investigation_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.ontologyAnnotation_id);
                if(iof !== -1) {
    ...
    if(lastModelChanged.investigation) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.investigation_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('investigations', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.ontologyAnnotation_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.investigation_id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('investigations', item.investigation_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.investigation_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('investigations', item.investigation_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <InvestigationsToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InvestigationsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='InvestigationsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'InvestigationsToAddTransferView-listA-listItem-'+it.investigation_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'investigation' }>
                                <Avatar>{"investigation".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'investigation_id' }>
                                        <Typography 
                                        id={'InvestigationsToAddTransferView-listA-listItem-id-'+it.investigation_id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'InvestigationsToAddTransferView-listA-listItem-'+it.investigation_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <InvestigationsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InvestigationsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='InvestigationsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='InvestigationsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.investigation_id;
                        let label = it.title;
                        ...
                        return (
                          <ListItem 
                            id={'InvestigationsToAddTransferView-listB-listItem-'+it.investigation_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'investigation_id' }>
                                        <Typography 
                                        id={'InvestigationsToAddTransferView-listB-listItem-id-'+it.investigation_id}
                                        variant="body1" display="block" noWrap={true}>{it.investigation_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'InvestigationsToAddTransferView-listB-listItem-'+it.investigation_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
InvestigationsToAddTransferView.propTypes = {
