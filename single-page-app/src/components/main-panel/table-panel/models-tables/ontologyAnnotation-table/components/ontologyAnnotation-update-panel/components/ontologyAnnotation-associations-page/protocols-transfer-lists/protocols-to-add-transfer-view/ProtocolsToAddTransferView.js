...
import ProtocolsToAddTransferViewToolbar from './components/ProtocolsToAddTransferViewToolbar';
import ProtocolsToAddTransferViewCursorPagination from './components/ProtocolsToAddTransferViewCursorPagination';
...
export default function ProtocolsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"protocol_id": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.ontologyAnnotation.getNotAssociatedProtocolsCount
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedProtocolsCount(graphqlServerUrl, item.ontologyAnnotation_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyAnnotation', association: 'protocols', table:'A', method: 'getCount()', request: 'api.ontologyAnnotation.getNotAssociatedProtocolsCount'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'protocols'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedProtocolsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getNotAssociatedProtocols
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedProtocols(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'protocols', table:'A', method: 'getData()', request: 'api.ontologyAnnotation.getNotAssociatedProtocols'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'protocols'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedProtocols
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getCountItems
    */
    let cancelableApiReq = makeCancelable(api.protocol.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'protocols', table:'B', method: 'getCountB()', request: 'api.protocol.getCountItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'protocols'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.protocol.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getItems
    */
    let cancelableApiReq = makeCancelable(api.protocol.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'protocols', table:'B', method: 'getDataB()', request: 'api.protocol.getItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'protocols'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.protocol.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'protocols' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'protocols', 
    ...
    if(lastModelChanged.ontologyAnnotation&&
        ...
        lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_protocol_ids&&
        (lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_protocol_ids.added ||
         lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_protocol_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_protocol_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('protocols', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_protocol_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'protocols' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'protocol',
     * but this action happened on the peer relation, identified by 'ontologyAnnotation_protocol_ids'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'protocol'.
     * B: the current item internalId is in the addedIds of the updated 'protocol'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'protocol' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.protocol) {
      let oens = Object.entries(lastModelChanged.protocol);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.ontologyAnnotation_protocol_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.ontologyAnnotation_protocol_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.ontologyAnnotation_protocol_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.ontologyAnnotation_protocol_ids.added) {
              let idsAdded = entry[1].changedAssociations.ontologyAnnotation_protocol_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.ontologyAnnotation_id);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.protocol_id;
    ...
    /*
     * Case 3: 
     * The attributes of some 'protocol' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.protocol) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.protocol_id;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.protocol_id===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.protocol_id===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.protocol_id;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.protocol_id===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.protocol_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('protocols', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnProtocolRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.protocol_id) === -1) {
      lidsToAdd.current.push(item.protocol_id);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('protocols', item.protocol_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.protocol_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('protocols', item.protocol_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ProtocolsToAddTransferViewToolbar
                title={'Protocols'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ProtocolsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ProtocolsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.protocol_id;
                        let label = it.name;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'ProtocolsToAddTransferView-listA-listItem-'+it.protocol_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'protocol' }>
                                <Avatar>{"protocol".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'protocol_id' }>
                                        <Typography 
                                        id={'ProtocolsToAddTransferView-listA-listItem-id-'+it.protocol_id}
                                        variant="body1" display="block" noWrap={true}>{it.protocol_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ProtocolsToAddTransferView-listA-listItem-'+it.protocol_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ProtocolsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ProtocolsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ProtocolsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ProtocolsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.protocol_id;
                        let label = it.name;
                        ...
                        return (
                          <ListItem 
                            id={'ProtocolsToAddTransferView-listB-listItem-'+it.protocol_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'protocol_id' }>
                                        <Typography 
                                        id={'ProtocolsToAddTransferView-listB-listItem-id-'+it.protocol_id}
                                        variant="body1" display="block" noWrap={true}>{it.protocol_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ProtocolsToAddTransferView-listB-listItem-'+it.protocol_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ProtocolsToAddTransferView.propTypes = {
