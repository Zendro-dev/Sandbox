...
import StudiesToAddTransferViewToolbar from './components/StudiesToAddTransferViewToolbar';
import StudiesToAddTransferViewCursorPagination from './components/StudiesToAddTransferViewCursorPagination';
...
export default function StudiesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getNotAssociatedStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedStudiesCount(graphqlServerUrl, item.ontologyAnnotation_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyAnnotation', association: 'studies', table:'A', method: 'getCount()', request: 'api.ontologyAnnotation.getNotAssociatedStudiesCount'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getNotAssociatedStudies
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedStudies(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'studies', table:'A', method: 'getData()', request: 'api.ontologyAnnotation.getNotAssociatedStudies'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.study.getCountItems
    */
    let cancelableApiReq = makeCancelable(api.study.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'studies', table:'B', method: 'getCountB()', request: 'api.study.getCountItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'studies'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.study.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'studies', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'studies' for this item was updated.
    ...
    if(lastModelChanged.ontologyAnnotation&&
        ...
        lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_study_ids&&
        (lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_study_ids.added ||
         lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_study_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_study_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('studies', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_study_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'studies' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'study',
     * but this action happened on the peer relation, identified by 'ontologyAnnotation_study_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.ontologyAnnotation_study_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.ontologyAnnotation_study_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.ontologyAnnotation_study_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.ontologyAnnotation_study_ids.added) {
              let idsAdded = entry[1].changedAssociations.ontologyAnnotation_study_ids.idsAdded;
              if(idsAdded) {
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.study_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('studies', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.study_id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('studies', item.study_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.study_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('studies', item.study_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <StudiesToAddTransferViewToolbar
                title={'Studies'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='StudiesToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='StudiesToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'StudiesToAddTransferView-listA-listItem-'+it.study_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'study_id' }>
                                        <Typography 
                                        id={'StudiesToAddTransferView-listA-listItem-id-'+it.study_id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'StudiesToAddTransferView-listA-listItem-'+it.study_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <StudiesToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='StudiesToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='StudiesToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='StudiesToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.study_id;
                        let label = it.name;
                        ...
                        return (
                          <ListItem 
                            id={'StudiesToAddTransferView-listB-listItem-'+it.study_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'study_id' }>
                                        <Typography 
                                        id={'StudiesToAddTransferView-listB-listItem-id-'+it.study_id}
                                        variant="body1" display="block" noWrap={true}>{it.study_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'StudiesToAddTransferView-listB-listItem-'+it.study_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
StudiesToAddTransferView.propTypes = {
