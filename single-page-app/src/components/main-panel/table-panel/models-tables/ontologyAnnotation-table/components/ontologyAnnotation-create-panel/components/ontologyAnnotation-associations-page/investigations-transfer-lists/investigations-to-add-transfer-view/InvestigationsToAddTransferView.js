...
const cancelableCountingPromises = useRef([]);
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.investigation.getCountItems(graphqlServerUrl, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyAnnotation', association: 'investigations', table:'A', method: 'getCount()', request: 'api.investigation.getCountItems'}];
            newError.path=['add', 'investigations'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'investigations', table:'A', method: 'getData()', request: 'api.investigation.getItems'}];
            newError.path=['add', 'investigations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
