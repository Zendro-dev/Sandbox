...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['ontologyAnnotation'][String(itemId['ontologyAnnotation_id'])]) {

        //updated password
        if(lastModelChanged['ontologyAnnotation'][String(itemId['ontologyAnnotation_id'])].op === "updatePassword"&&
            lastModelChanged['ontologyAnnotation'][String(itemId['ontologyAnnotation_id'])].newItem) {
              //update password value
              setValue(lastModelChanged['ontologyAnnotation'][String(itemId['ontologyAnnotation_id'])].newItem[itemKey]);
        }
    }//end: Case 1
  ...
  /**
    ...
    */
    return await api.ontologyAnnotation.updateItem(graphqlServerUrl, variables)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['OntologyAnnotations', itemId, 'update', 'change password'];
  ...
  return (
    <div>
      <Grid container justify='flex-start' alignItems='center' spacing={0}>
        <Grid item xs={12}>
          <form noValidate autoComplete="off">
            <TextField
              id={'PasswordField-OntologyAnnotation-'+name}
              label={label}
              value={value}
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'PasswordField-OntologyAnnotation-button-showPassword'+name}
                    ...
                    {(valueOk!==undefined&&valueOk===1) 
                      ...
                      <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                        <Typography id={'PasswordField-OntologyAnnotation-exists-'+name} variant="caption" color="primary">
                          &#8707;
                        </Typography>
                      </Tooltip>
                      : 
                      <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                        <Typography id={'PasswordField-OntologyAnnotation-notExists-'+name} variant="caption" color="textSecondary">
                          &#8708;
                        </Typography>
                      </Tooltip>
                    }
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'PasswordField-OntologyAnnotation-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-OntologyAnnotation-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-OntologyAnnotation-ChangePasswordDialog-textField-newPassword2-'+name}
