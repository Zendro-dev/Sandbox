...
import MaterialsToAddTransferViewToolbar from './components/MaterialsToAddTransferViewToolbar';
import MaterialsToAddTransferViewCursorPagination from './components/MaterialsToAddTransferViewCursorPagination';
...
export default function MaterialsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getNotAssociatedMaterialsCount
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedMaterialsCount(graphqlServerUrl, item.ontologyAnnotation_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyAnnotation', association: 'materials', table:'A', method: 'getCount()', request: 'api.ontologyAnnotation.getNotAssociatedMaterialsCount'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'materials'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedMaterialsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getNotAssociatedMaterials
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getNotAssociatedMaterials(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'materials', table:'A', method: 'getData()', request: 'api.ontologyAnnotation.getNotAssociatedMaterials'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getNotAssociatedMaterials
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.material.getCountItems
    */
    let cancelableApiReq = makeCancelable(api.material.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'materials', table:'B', method: 'getCountB()', request: 'api.material.getCountItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'materials'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.material.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'materials', table:'B', method: 'getDataB()', request: 'api.material.getItems'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'add', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'materials' for this item was updated.
    ...
    if(lastModelChanged.ontologyAnnotation&&
        ...
        lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_material_ids&&
        (lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_material_ids.added ||
         lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_material_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_material_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('materials', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.ontologyAnnotation[String(item.ontologyAnnotation_id)].changedAssociations.ontologyAnnotation_material_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'materials' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'material',
     * but this action happened on the peer relation, identified by 'ontologyAnnotation_material_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.material) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.ontologyAnnotation_material_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.ontologyAnnotation_material_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.ontologyAnnotation_material_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.ontologyAnnotation_material_ids.added) {
              let idsAdded = entry[1].changedAssociations.ontologyAnnotation_material_ids.idsAdded;
              if(idsAdded) {
    ...
    if(lastModelChanged.material) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.material_id===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('materials', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.material_id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('materials', item.material_id);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.material_id);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('materials', item.material_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MaterialsToAddTransferViewToolbar
                title={'Materials'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MaterialsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='MaterialsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'MaterialsToAddTransferView-listA-listItem-'+it.material_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'material' }>
                                <Avatar>{"material".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'material_id' }>
                                        <Typography 
                                        id={'MaterialsToAddTransferView-listA-listItem-id-'+it.material_id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'MaterialsToAddTransferView-listA-listItem-'+it.material_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <MaterialsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MaterialsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MaterialsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='MaterialsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.material_id;
                        let label = it.name;
                        ...
                        return (
                          <ListItem 
                            id={'MaterialsToAddTransferView-listB-listItem-'+it.material_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'material_id' }>
                                        <Typography 
                                        id={'MaterialsToAddTransferView-listB-listItem-id-'+it.material_id}
                                        variant="body1" display="block" noWrap={true}>{it.material_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'MaterialsToAddTransferView-listB-listItem-'+it.material_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
MaterialsToAddTransferView.propTypes = {
