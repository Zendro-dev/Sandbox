...
import AssayResultsToRemoveTransferViewToolbar from './components/AssayResultsToRemoveTransferViewToolbar';
import AssayResultsToRemoveTransferViewCursorPagination from './components/AssayResultsToRemoveTransferViewCursorPagination';
...
export default function AssayResultsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.ontologyAnnotation.getAssayResultsCount
    */
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getAssayResultsCount(
      graphqlServerUrl, 
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'assayResults', method: 'getCount()', request: 'api.ontologyAnnotation.getAssayResultsCount'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'remove', 'assayResults'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getAssayResultsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {assayResult_id: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.ontologyAnnotation.getAssayResults
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.ontologyAnnotation.getAssayResults(
      graphqlServerUrl, 
      item.ontologyAnnotation_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'assayResults', method: 'getData()', request: 'api.ontologyAnnotation.getAssayResults'}];
            newError.path=['update', `ontologyAnnotation_id:${item.ontologyAnnotation_id}`, 'remove', 'assayResults'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getAssayResults
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'assayResult' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.ontologyAnnotation_assayResult_ids.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.assayResult_id;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.assayResult_id) === -1) {
      lidsToRemove.current.push(item.assayResult_id);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.assayResult_id);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <AssayResultsToRemoveTransferViewToolbar
                title={'AssayResults'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayResultsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='AssayResultsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.assayResult_id;
                        let label = it.value_as_str;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(assayResult_id=> assayResult_id=== it.assayResult_id) !== undefined;

                        return (
                          <ListItem 
                            id={'AssayResultsToRemoveTransferView-listA-listItem-'+it.assayResult_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'assayResult_id' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.assayResult_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'AssayResultsToRemoveTransferView-listA-listItem-'+it.assayResult_id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <AssayResultsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayResultsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='AssayResultsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='AssayResultsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.assayResult_id;
                        let label = it.value_as_str;
                        ...
                        return (
                          <ListItem 
                            id={'AssayResultsToRemoveTransferView-listB-listItem-'+it.assayResult_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'AssayResultsToRemoveTransferView-listB-listItem-'+it.assayResult_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
AssayResultsToRemoveTransferView.propTypes = {
