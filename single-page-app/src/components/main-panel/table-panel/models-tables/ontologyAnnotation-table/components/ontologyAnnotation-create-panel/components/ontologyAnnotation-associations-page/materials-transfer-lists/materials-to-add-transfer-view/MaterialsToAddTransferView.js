...
const cancelableCountingPromises = useRef([]);
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.material.getCountItems(graphqlServerUrl, search, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyAnnotation', association: 'materials', table:'A', method: 'getCount()', request: 'api.material.getCountItems'}];
            newError.path=['add', 'materials'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'ontologyAnnotation', association: 'materials', table:'A', method: 'getData()', request: 'api.material.getItems'}];
            newError.path=['add', 'materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
