...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.season.getStudiesCount
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getStudiesCount(
      graphqlServerUrl, 
      item.seasonDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'studies', method: 'getCount()', request: 'api.season.getStudiesCount'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'remove', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.season.getStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.season.getStudies
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getStudies(
      graphqlServerUrl, 
      item.seasonDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'studies', method: 'getData()', request: 'api.season.getStudies'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'remove', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.season.getStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
