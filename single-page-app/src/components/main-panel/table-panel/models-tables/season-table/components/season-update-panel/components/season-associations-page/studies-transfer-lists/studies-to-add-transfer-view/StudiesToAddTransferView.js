...
export default function StudiesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.season.getNotAssociatedStudiesCount
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getNotAssociatedStudiesCount(graphqlServerUrl, item.seasonDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'season', association: 'studies', table:'A', method: 'getCount()', request: 'api.season.getNotAssociatedStudiesCount'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'add', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.season.getNotAssociatedStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.season.getNotAssociatedStudies
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getNotAssociatedStudies(
      graphqlServerUrl,
      item.seasonDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'season', association: 'studies', table:'A', method: 'getData()', request: 'api.season.getNotAssociatedStudies'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.season.getNotAssociatedStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'season', association: 'studies', table:'B', method: 'getCountB()', request: 'api.study.getCountItems'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'add', 'studies'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'season', association: 'studies', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.season&&
        ...
        lastModelChanged.season[String(item.seasonDbId)].changedAssociations.study_seasonDbIds&&
        (lastModelChanged.season[String(item.seasonDbId)].changedAssociations.study_seasonDbIds.added ||
         lastModelChanged.season[String(item.seasonDbId)].changedAssociations.study_seasonDbIds.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.season[String(item.seasonDbId)].changedAssociations.study_seasonDbIds.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.season[String(item.seasonDbId)].changedAssociations.study_seasonDbIds.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'study_seasonDbIds'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.study_seasonDbIds) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.study_seasonDbIds.removed) {
              let idsRemoved = entry[1].changedAssociations.study_seasonDbIds.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.study_seasonDbIds.added) {
              let idsAdded = entry[1].changedAssociations.study_seasonDbIds.idsAdded;
              if(idsAdded) {
