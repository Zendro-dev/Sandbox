...
//lazy loading
const SeasonAttributesPage = lazy(() => import(/* webpackChunkName: "Detail-Attributes-Season" */ './components/season-attributes-page/SeasonAttributesPage'));
const SeasonAssociationsPage = lazy(() => import(/* webpackChunkName: "Detail-Associations-Season" */ './components/season-associations-page/SeasonAssociationsPage'));
const SeasonUpdatePanel = lazy(() => import(/* webpackChunkName: "Detail-Update-Season" */ '../season-update-panel/SeasonUpdatePanel'));
const SeasonDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Detail-Delete-Season" */ '../SeasonDeleteConfirmationDialog'));
...
export default function SeasonDetailPanel(props) {
      ...
      lastModelChanged['season'][String(item['seasonDbId'])]) {
        ...
        if(lastModelChanged['season'][String(item['seasonDbId'])].op === "update"&&
            lastModelChanged['season'][String(item['seasonDbId'])].newItem) {
              //replace item
              setItemState(lastModelChanged['season'][String(item['seasonDbId'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('season', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('season', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='SeasonDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Season | seasonDbId: " + itemState['seasonDbId'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='SeasonDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='SeasonDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
