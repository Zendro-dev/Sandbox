...
export default function ObservationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.season.getNotAssociatedObservationsCount
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getNotAssociatedObservationsCount(graphqlServerUrl, item.seasonDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'season', association: 'observations', table:'A', method: 'getCount()', request: 'api.season.getNotAssociatedObservationsCount'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'add', 'observations'];
      ...
      //error
      .catch((err) => { //error: on api.season.getNotAssociatedObservationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.seasonDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.season.getNotAssociatedObservations
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getNotAssociatedObservations(
      graphqlServerUrl,
      item.seasonDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'season', association: 'observations', table:'A', method: 'getData()', request: 'api.season.getNotAssociatedObservations'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.season.getNotAssociatedObservations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.seasonDbId, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'season', association: 'observations', table:'B', method: 'getCountB()', request: 'api.observation.getCountItems'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'add', 'observations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.seasonDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'season', association: 'observations', table:'B', method: 'getDataB()', request: 'api.observation.getItems'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.seasonDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.season&&
        ...
        lastModelChanged.season[String(item.seasonDbId)].changedAssociations&&
        lastModelChanged.season[String(item.seasonDbId)].changedAssociations.observation_seasonDbId&&
        (lastModelChanged.season[String(item.seasonDbId)].changedAssociations.observation_seasonDbId.added ||
         lastModelChanged.season[String(item.seasonDbId)].changedAssociations.observation_seasonDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.season[String(item.seasonDbId)].changedAssociations.observation_seasonDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.season[String(item.seasonDbId)].changedAssociations.observation_seasonDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
        ...
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observation_seasonDbId) {
            ...
            if(entry[1].changedAssociations.observation_seasonDbId.removed) {
              ...
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.seasonDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.observation_seasonDbId.added) {
              ...
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.seasonDbId);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.seasonDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
