...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.season.getObservationsCount
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getObservationsCount(
      graphqlServerUrl, 
      item.seasonDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observations', method: 'getCount()', request: 'api.season.getObservationsCount'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'remove', 'observations'];
      ...
      //error
      .catch((err) => { //error: on api.season.getObservationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.season.getObservations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.getObservations(
      graphqlServerUrl, 
      item.seasonDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observations', method: 'getData()', request: 'api.season.getObservations'}];
            newError.path=['update', `seasonDbId:${item.seasonDbId}`, 'remove', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.season.getObservations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.seasonDbId, search, getCount, configurePagination, onEmptyPage]);
