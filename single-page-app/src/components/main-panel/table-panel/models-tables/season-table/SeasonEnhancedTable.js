...
import SeasonEnhancedTableHead from './components/SeasonEnhancedTableHead'
import SeasonEnhancedTableToolbar from './components/SeasonEnhancedTableToolbar'
import SeasonUploadFileDialog from './components/SeasonUploadFileDialog'
import SeasonCursorPagination from './components/SeasonCursorPagination'
...
//lazy loading
const SeasonCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Season" */ './components/season-create-panel/SeasonCreatePanel'));
const SeasonUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Season" */ './components/season-update-panel/SeasonUpdatePanel'));
const SeasonDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Season" */ './components/season-detail-panel/SeasonDetailPanel'));
const SeasonDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Season" */ './components/SeasonDeleteConfirmationDialog'));
//Plotly
const SeasonPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Season" */ '../../../../plots/SeasonPlotly'));
...
export default function SeasonEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('seasonDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.season.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'season', method: 'getCount()', request: 'api.season.getCountItems'}];
            newError.path=['Seasons'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'season', method: 'getData()', request: 'api.season.getItems'}];
            newError.path=['Seasons'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['season']
    &&(Object.keys(changes.lastModelChanged['season']).length===1)
    &&(changes.lastModelChanged['season'][Object.keys(changes.lastModelChanged['season'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set seasonDbId (internalId)
    variables.seasonDbId = item.seasonDbId;

    /*
      API Request: api.season.deleteItem
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'season', method: 'doDelete()', request: 'api.season.deleteItem'}];
            newError.path=['Seasons', `seasonDbId:${item.seasonDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.season.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.season.tableTemplate
    */
    let api = await loadApi("season");
    let cancelableApiReq = makeCancelable(api.season.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'season', method: 'getCsvTemplate()', request: 'api.season.tableTemplate'}];
            newError.path=['Seasons', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateSeason.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "season-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.season.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='SeasonEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='SeasonEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <SeasonEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='SeasonEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'SeasonEnhancedTable-row-'+item.seasonDbId}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'SeasonEnhancedTable-row-iconButton-detail-'+item.seasonDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.season&&Array.isArray(permissions.season)
                                      &&(permissions.season.includes('update') || permissions.season.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'SeasonEnhancedTable-row-iconButton-edit-'+item.seasonDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.season&&Array.isArray(permissions.season)
                                      &&(permissions.season.includes('delete') || permissions.season.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'SeasonEnhancedTable-row-iconButton-delete-'+item.seasonDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'seasonDbId: ' + item.seasonDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.seasonDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.season!==null)?item.season:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.studyDbIds!==null)?item.studyDbIds:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <SeasonDeleteConfirmationDialog
            permissions={permissions}
