...
import ObservationUnitsToRemoveTransferViewToolbar from './components/ObservationUnitsToRemoveTransferViewToolbar';
import ObservationUnitsToRemoveTransferViewCursorPagination from './components/ObservationUnitsToRemoveTransferViewCursorPagination';
...
export default function ObservationUnitsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.germplasm.getObservationUnitsCount
    */
    let api = await loadApi("germplasm");
    let cancelableApiReq = makeCancelable(api.germplasm.getObservationUnitsCount(
      graphqlServerUrl, 
      item.germplasmDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationUnits', method: 'getCount()', request: 'api.germplasm.getObservationUnitsCount'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'remove', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {observationUnitDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.germplasm.getObservationUnits
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("germplasm");
    let cancelableApiReq = makeCancelable(api.germplasm.getObservationUnits(
      graphqlServerUrl, 
      item.germplasmDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationUnits', method: 'getData()', request: 'api.germplasm.getObservationUnits'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.observationUnit.getCountItems
    */
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'germplasm', association: 'observationUnits', table:'B', method: 'getCountB()', request: 'api.observationUnit.getCountItems'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.observationUnit.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
        ...
        (response) => {
          ...
          if(response.message === 'ok') {
            ...
            if(response.graphqlErrors) {
              ...
              newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'germplasm', association: 'observationUnits', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
              newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'remove', 'observationUnits'];
              newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'observationUnits' for this item was updated.
    ...
    if(lastModelChanged.germplasm&&
        ...
        lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observationUnit_germplasmDbId&&
        (lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observationUnit_germplasmDbId.added ||
         lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observationUnit_germplasmDbId.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observationUnit_germplasmDbId.idsRemoved;
          if(idsRemoved) {
            idsRemoved.forEach( (idRemoved) => {
              ...
              handleUntransfer('observationUnits', idRemoved);
          ...
          let idsAdded = lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.observationUnit_germplasmDbId.idsAdded;
          if(idsAdded) {
            //increment A
    ...
    /*
     * Case 2:
     * The relation 'observationUnits' for this item was updated from the target model (in the peer relation).
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'observationUnit' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.observationUnit_germplasmDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.observationUnitDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.observationUnitDbId) === -1) {
      lidsToRemove.current.push(item.observationUnitDbId);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('observationUnits', item.observationUnitDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.observationUnitDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('observationUnits', item.observationUnitDbId);
    }
  }
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationUnitsToRemoveTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationUnitsToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.observationUnitDbId;
                        let label = it.observationUnitName;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(observationUnitDbId=> observationUnitDbId=== it.observationUnitDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'ObservationUnitsToRemoveTransferView-listA-listItem-'+it.observationUnitDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationUnitDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.observationUnitDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'ObservationUnitsToRemoveTransferView-listA-listItem-'+it.observationUnitDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <ObservationUnitsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationUnitsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationUnitDbId;
                        let label = it.observationUnitName;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationUnitsToRemoveTransferView-listB-listItem-'+it.observationUnitDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'ObservationUnitsToRemoveTransferView-listB-listItem-'+it.observationUnitDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ObservationUnitsToRemoveTransferView.propTypes = {
