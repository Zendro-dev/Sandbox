...
import ObservationUnitsToAddTransferViewToolbar from './components/ObservationUnitsToAddTransferViewToolbar';
import ObservationUnitsToAddTransferViewCursorPagination from './components/ObservationUnitsToAddTransferViewCursorPagination';
...
export default function ObservationUnitsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.germplasm.getNotAssociatedObservationUnitsCount
    */
    let cancelableApiReq = makeCancelable(api.germplasm.getNotAssociatedObservationUnitsCount(graphqlServerUrl, item.germplasmDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'germplasm', association: 'observationUnits', table:'A', method: 'getCount()', request: 'api.germplasm.getNotAssociatedObservationUnitsCount'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'add', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getNotAssociatedObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.germplasm.getNotAssociatedObservationUnits
    */
    let cancelableApiReq = makeCancelable(api.germplasm.getNotAssociatedObservationUnits(
      graphqlServerUrl,
      item.germplasmDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'germplasm', association: 'observationUnits', table:'A', method: 'getData()', request: 'api.germplasm.getNotAssociatedObservationUnits'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'add', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getNotAssociatedObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
          ...
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('observationUnits', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
...
const handleReloadClickB = (event) => {
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.observationUnitDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationUnitsToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationUnitsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ObservationUnitsToAddTransferView-listA-listItem-'+it.observationUnitDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationUnitDbId' }>
                                        <Typography 
                                        id={'ObservationUnitsToAddTransferView-listA-listItem-id-'+it.observationUnitDbId}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ObservationUnitsToAddTransferView-listA-listItem-'+it.observationUnitDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ObservationUnitsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationUnitsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationUnitsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationUnitDbId;
                        let label = it.observationUnitName;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationUnitsToAddTransferView-listB-listItem-'+it.observationUnitDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationUnitDbId' }>
                                        <Typography 
                                        id={'ObservationUnitsToAddTransferView-listB-listItem-id-'+it.observationUnitDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationUnitDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ObservationUnitsToAddTransferView-listB-listItem-'+it.observationUnitDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ObservationUnitsToAddTransferView.propTypes = {
