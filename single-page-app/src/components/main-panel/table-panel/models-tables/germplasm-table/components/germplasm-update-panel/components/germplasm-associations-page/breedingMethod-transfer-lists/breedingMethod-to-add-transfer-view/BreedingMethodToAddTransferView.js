...
import BreedingMethodToAddTransferViewToolbar from './components/BreedingMethodToAddTransferViewToolbar';
import BreedingMethodToAddTransferViewCursorPagination from './components/BreedingMethodToAddTransferViewCursorPagination';
...
export default function BreedingMethodToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.germplasmDbId;

    /*
      API Request: api.germplasm.getBreedingMethod
    */
    let cancelableApiReq = makeCancelable(api.germplasm.getBreedingMethod(graphqlServerUrl, item.germplasmDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'germplasm', association: 'breedingMethod', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.germplasm.getBreedingMethod'}];
            newError.path=['update', `germplasmDbId:${currentId}`, 'add', 'breedingMethod'];
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getBreedingMethod
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.germplasmDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"breedingMethodDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.germplasm.getNotAssociatedBreedingMethodCount
    */
    let cancelableApiReq = makeCancelable(api.germplasm.getNotAssociatedBreedingMethodCount(graphqlServerUrl, item.germplasmDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'germplasm', association: 'breedingMethod', table:'A', method: 'getCount()', request: 'api.germplasm.getNotAssociatedBreedingMethodCount'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'add', 'breedingMethod'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getNotAssociatedBreedingMethodCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.germplasmDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.germplasm.getNotAssociatedBreedingMethod
    */
    let cancelableApiReq = makeCancelable(api.germplasm.getNotAssociatedBreedingMethod(
      graphqlServerUrl,
      item.germplasmDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'germplasm', association: 'breedingMethod', table:'A', method: 'getData()', request: 'api.germplasm.getNotAssociatedBreedingMethod'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'add', 'breedingMethod'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getNotAssociatedBreedingMethod
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.germplasmDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.breedingMethod.getItems
    */
    let cancelableApiReq = makeCancelable(api.breedingMethod.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'germplasm', association: 'breedingMethod', table:'B', method: 'getDataB()', request: 'api.breedingMethod.getItems'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'add', 'breedingMethod'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.breedingMethod.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.germplasmDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'breedingMethod' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'breedingMethods', 
     ...
     * - return
     */
    if(lastModelChanged.germplasm&&
        lastModelChanged.germplasm[String(item.germplasmDbId)]&&
        lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations&&
        lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.germplasm_breedingMethodDbId&&
        (lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.germplasm_breedingMethodDbId.added ||
         lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.germplasm_breedingMethodDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.germplasm_breedingMethodDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('breedingMethod', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('breedingMethod', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.germplasm[String(item.germplasmDbId)].changedAssociations.germplasm_breedingMethodDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('breedingMethod', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'breedingMethod' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'breedingMethod',
     * but this action happened on the peer relation, identified by 'germplasm_breedingMethodDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'breedingMethod'.
     * B: the current item internalId is in the addedIds of the updated 'breedingMethod'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'breedingMethod' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.breedingMethod) {
      let oens = Object.entries(lastModelChanged.breedingMethod);
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.germplasm_breedingMethodDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.germplasm_breedingMethodDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.germplasm_breedingMethodDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.germplasmDbId);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('breedingMethod', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.germplasm_breedingMethodDbId.added) {
              let idsAdded = entry[1].changedAssociations.germplasm_breedingMethodDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.germplasmDbId);
                if(iof !== -1) {
                  //remove changed item from lidsToAdd
                  let idAdded = entry[1].newItem.breedingMethodDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('breedingMethod', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'breedingMethod' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.breedingMethod) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.breedingMethodDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.breedingMethodDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.breedingMethodDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.breedingMethodDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.breedingMethodDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.breedingMethodDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('breedingMethod', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.breedingMethodDbId===idRemoved) {
            //update associated item
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.germplasmDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnBreedingMethodRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.breedingMethodDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.breedingMethodDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.breedingMethodDbId];
        handleDisassociateItem('breedingMethod', associatedItem.breedingMethodDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('breedingMethod', item.breedingMethodDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('breedingMethod', item.breedingMethodDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <BreedingMethodToAddTransferViewToolbar
                title={'BreedingMethods'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='BreedingMethodToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='BreedingMethodToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.breedingMethodDbId;
                        let label = it.breedingMethodName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'BreedingMethodToAddTransferView-listA-listItem-'+it.breedingMethodDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.breedingMethodDbId===it.breedingMethodDbId) ? 'breedingMethod — ' + t('modelPanels.associatedRecord', 'Associated record') : 'breedingMethod '}>
                                <Avatar>
                                  {"breedingMethod".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                            ...
                            <ListItemText
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'breedingMethodDbId' }>
                                        <Typography 
                                        id={'BreedingMethodToAddTransferView-listA-listItem-id-'+it.breedingMethodDbId}
                                        variant="body1" display="block" noWrap={true}>{it.breedingMethodDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.breedingMethodDbId===it.breedingMethodDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.breedingMethodDbId===it.breedingMethodDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                              ...
                              secondary={
                                <React.Fragment>
                                  {/* Label */}
                                  {(label) && (
                                    <Tooltip title={ 'breedingMethodName' }>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'BreedingMethodToAddTransferView-listA-listItem-'+it.breedingMethodDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <BreedingMethodToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='BreedingMethodToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='BreedingMethodToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='BreedingMethodToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.breedingMethodDbId;
                        let label = it.breedingMethodName;
                        ...
                        return (
                          <ListItem 
                            id={'BreedingMethodToAddTransferView-listB-listItem-'+it.breedingMethodDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'breedingMethodDbId' }>
                                        <Typography 
                                        id={'BreedingMethodToAddTransferView-listB-listItem-id-'+it.breedingMethodDbId}
                                        variant="body1" display="block" noWrap={true}>{it.breedingMethodDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'BreedingMethodToAddTransferView-listB-listItem-'+it.breedingMethodDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='BreedingMethodToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.breedingMethodDbId;
                        let label = associatedItem.breedingMethodName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'BreedingMethodToAddTransferView-associatedItem-list-listItem-'+associatedItem.breedingMethodDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'breedingMethod — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"breedingMethod".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'breedingMethodDbId' }>
                                        <Typography 
                                        id={'BreedingMethodToAddTransferView-listB-listItem-id-'+associatedItem.breedingMethodDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.breedingMethodDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'BreedingMethodToAddTransferView-associatedItem-list-listItem-'+associatedItem.breedingMethodDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].breedingMethodDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('breedingMethod', associatedItem.breedingMethodDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.breedingMethodDbId];
                                      handleDisassociateItem('breedingMethod', associatedItem.breedingMethodDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='BreedingMethodToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
BreedingMethodToAddTransferView.propTypes = {
