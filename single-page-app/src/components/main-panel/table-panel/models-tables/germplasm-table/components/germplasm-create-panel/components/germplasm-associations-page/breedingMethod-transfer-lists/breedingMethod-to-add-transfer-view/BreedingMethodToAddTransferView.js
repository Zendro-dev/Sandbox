      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'germplasm', association: 'breedingMethod', table:'A', method: 'getCount()', request: 'api.breedingMethod.getCountItems'}];
            newError.path=['add', 'breedingMethod'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'germplasm', association: 'breedingMethod', table:'A', method: 'getData()', request: 'api.breedingMethod.getItems'}];
            newError.path=['add', 'breedingMethod'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
                  ...
                  dense component="div" role="list" >
                    {items.map(it => {
                      ...
                      return (
                        ...
                        >
                          <ListItemAvatar>
                            <Tooltip title={ 'breedingMethod' }>
                              <Avatar>{"breedingMethod".slice(0,1)}</Avatar>
                            </Tooltip>
                          </ListItemAvatar>
                                ...
                                <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                  <Grid item>
                                    <Tooltip title={ 'breedingMethodDbId' }>
                                      <Typography variant="body1" display="block" noWrap={true}>{it.breedingMethodDbId}</Typography>
                                    </Tooltip>
                                  </Grid>
                                  {/*Key icon*/}
                                  <Grid item>
