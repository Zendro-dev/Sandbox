...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['germplasm'][String(itemId['germplasmDbId'])]) {

        //updated password
        if(lastModelChanged['germplasm'][String(itemId['germplasmDbId'])].op === "updatePassword"&&
            lastModelChanged['germplasm'][String(itemId['germplasmDbId'])].newItem) {
              //update password value
              setValue(lastModelChanged['germplasm'][String(itemId['germplasmDbId'])].newItem[itemKey]);
        }
    }//end: Case 1
  ...
  /**
    ...
    */
    return await api.germplasm.updateItem(graphqlServerUrl, variables)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.path=['Germplasms', itemId, 'update', 'change password'];
  ...
  return (
    <div>
      <Grid container justify='flex-start' alignItems='center' spacing={0}>
        <Grid item xs={12}>
          <form noValidate autoComplete="off">
            <TextField
              id={'PasswordField-Germplasm-'+name}
              label={label}
              value={value}
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'PasswordField-Germplasm-button-showPassword'+name}
                    ...
                    {(valueOk!==undefined&&valueOk===1) 
                      ...
                      <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
                        <Typography id={'PasswordField-Germplasm-exists-'+name} variant="caption" color="primary">
                          &#8707;
                        </Typography>
                      </Tooltip>
                      : 
                      <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
                        <Typography id={'PasswordField-Germplasm-notExists-'+name} variant="caption" color="textSecondary">
                          &#8708;
                        </Typography>
                      </Tooltip>
                    }
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'PasswordField-Germplasm-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-Germplasm-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'PasswordField-Germplasm-ChangePasswordDialog-textField-newPassword2-'+name}
