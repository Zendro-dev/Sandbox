...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.germplasm.getObservationsCount
    */
    let api = await loadApi("germplasm");
    let cancelableApiReq = makeCancelable(api.germplasm.getObservationsCount(
      graphqlServerUrl, 
      item.germplasmDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observations', method: 'getCount()', request: 'api.germplasm.getObservationsCount'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'remove', 'observations'];
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getObservationsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.germplasm.getObservations
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("germplasm");
    let cancelableApiReq = makeCancelable(api.germplasm.getObservations(
      graphqlServerUrl, 
      item.germplasmDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observations', method: 'getData()', request: 'api.germplasm.getObservations'}];
            newError.path=['update', `germplasmDbId:${item.germplasmDbId}`, 'remove', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getObservations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.germplasmDbId, search, getCount, configurePagination, onEmptyPage]);
