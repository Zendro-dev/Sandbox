...
import GermplasmEnhancedTableHead from './components/GermplasmEnhancedTableHead'
import GermplasmEnhancedTableToolbar from './components/GermplasmEnhancedTableToolbar'
import GermplasmUploadFileDialog from './components/GermplasmUploadFileDialog'
import GermplasmCursorPagination from './components/GermplasmCursorPagination'
...
//lazy loading
const GermplasmCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Germplasm" */ './components/germplasm-create-panel/GermplasmCreatePanel'));
const GermplasmUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Germplasm" */ './components/germplasm-update-panel/GermplasmUpdatePanel'));
const GermplasmDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Germplasm" */ './components/germplasm-detail-panel/GermplasmDetailPanel'));
const GermplasmDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Germplasm" */ './components/GermplasmDeleteConfirmationDialog'));
//Plotly
const GermplasmPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Germplasm" */ '../../../../plots/GermplasmPlotly'));
...
export default function GermplasmEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('germplasmDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.germplasm.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'germplasm', method: 'getCount()', request: 'api.germplasm.getCountItems'}];
            newError.path=['Germplasms'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'germplasm', method: 'getData()', request: 'api.germplasm.getItems'}];
            newError.path=['Germplasms'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['germplasm']
    &&(Object.keys(changes.lastModelChanged['germplasm']).length===1)
    &&(changes.lastModelChanged['germplasm'][Object.keys(changes.lastModelChanged['germplasm'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.germplasm.tableTemplate
    */
    let api = await loadApi("germplasm");
    let cancelableApiReq = makeCancelable(api.germplasm.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'germplasm', method: 'getCsvTemplate()', request: 'api.germplasm.tableTemplate'}];
            newError.path=['Germplasms', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateGermplasm.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "germplasm-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.germplasm.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='GermplasmEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='GermplasmEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <GermplasmEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='GermplasmEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'GermplasmEnhancedTable-row-'+item.germplasmDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.germplasmDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'GermplasmEnhancedTable-row-iconButton-detail-'+item.germplasmDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'GermplasmEnhancedTable-row-iconButton-edit-'+item.germplasmDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'GermplasmEnhancedTable-row-iconButton-delete-'+item.germplasmDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'germplasmDbId: ' + item.germplasmDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.germplasmDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.accessionNumber!==null)?item.accessionNumber:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.acquisitionDate!==null)?moment(item.acquisitionDate, "YYYY-MM-DD").format("YYYY-MM-DD"):'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.breedingMethodDbId!==null)?item.breedingMethodDbId:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.countryOfOriginCode!==null)?item.countryOfOriginCode:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.defaultDisplayName!==null)?item.defaultDisplayName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.germplasmGenus!==null)?item.germplasmGenus:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.germplasmName!==null)?item.germplasmName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.germplasmPUI!==null)?item.germplasmPUI:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.germplasmPreprocessing!==null)?item.germplasmPreprocessing:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.germplasmSpecies!==null)?item.germplasmSpecies:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.germplasmSubtaxa!==null)?item.germplasmSubtaxa:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.instituteCode!==null)?item.instituteCode:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.instituteName!==null)?item.instituteName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.pedigree!==null)?item.pedigree:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.seedSource!==null)?item.seedSource:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.seedSourceDescription!==null)?item.seedSourceDescription:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.speciesAuthority!==null)?item.speciesAuthority:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.subtaxaAuthority!==null)?item.subtaxaAuthority:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.biologicalStatusOfAccessionCode!==null)?item.biologicalStatusOfAccessionCode:'')}
                                    </TableCell>
...
GermplasmEnhancedTable.propTypes = {
