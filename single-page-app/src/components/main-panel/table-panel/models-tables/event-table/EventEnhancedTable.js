...
import EventEnhancedTableHead from './components/EventEnhancedTableHead'
import EventEnhancedTableToolbar from './components/EventEnhancedTableToolbar'
import EventUploadFileDialog from './components/EventUploadFileDialog'
import EventCursorPagination from './components/EventCursorPagination'
...
//lazy loading
const EventCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Event" */ './components/event-create-panel/EventCreatePanel'));
const EventUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Event" */ './components/event-update-panel/EventUpdatePanel'));
const EventDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Event" */ './components/event-detail-panel/EventDetailPanel'));
const EventDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Event" */ './components/EventDeleteConfirmationDialog'));
//Plotly
const EventPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Event" */ '../../../../plots/EventPlotly'));
...
export default function EventEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('eventType');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.event.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'event', method: 'getCount()', request: 'api.event.getCountItems'}];
            newError.path=['Events'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', method: 'getData()', request: 'api.event.getItems'}];
            newError.path=['Events'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['event']
    &&(Object.keys(changes.lastModelChanged['event']).length===1)
    &&(changes.lastModelChanged['event'][Object.keys(changes.lastModelChanged['event'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set eventType (internalId)
    variables.eventType = item.eventType;

    /*
      API Request: api.event.deleteItem
    */
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', method: 'doDelete()', request: 'api.event.deleteItem'}];
            newError.path=['Events', `eventType:${item.eventType}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.event.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.event.tableTemplate
    */
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', method: 'getCsvTemplate()', request: 'api.event.tableTemplate'}];
            newError.path=['Events', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateEvent.join("\n");
      ...
      //error
      .catch((err) => { //error: on api.event.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='EventEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='EventEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <EventEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='EventEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'EventEnhancedTable-row-'+item.eventType}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'EventEnhancedTable-row-iconButton-detail-'+item.eventType}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.event&&Array.isArray(permissions.event)
                                      &&(permissions.event.includes('update') || permissions.event.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'EventEnhancedTable-row-iconButton-edit-'+item.eventType}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.event&&Array.isArray(permissions.event)
                                      &&(permissions.event.includes('delete') || permissions.event.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'EventEnhancedTable-row-iconButton-delete-'+item.eventType}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'eventType: ' + item.eventType}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.eventType}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.eventDescription!==null)?item.eventDescription:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.date!==null)?moment(item.date, "YYYY-MM-DDTHH:mm:ss.SSSZ").format("YYYY-MM-DDTHH:mm:ss.SSSZ"):'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.observationUnitDbIds!==null)?item.observationUnitDbIds:'')}
                                    </TableCell>
