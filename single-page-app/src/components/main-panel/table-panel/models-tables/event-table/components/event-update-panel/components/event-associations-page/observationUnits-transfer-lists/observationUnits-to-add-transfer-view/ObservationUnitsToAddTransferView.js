...
export default function ObservationUnitsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.event.getNotAssociatedObservationUnitsCount
    */
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.getNotAssociatedObservationUnitsCount(graphqlServerUrl, item.eventType, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'event', association: 'observationUnits', table:'A', method: 'getCount()', request: 'api.event.getNotAssociatedObservationUnitsCount'}];
            newError.path=['update', `eventType:${item.eventType}`, 'add', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.event.getNotAssociatedObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.event.getNotAssociatedObservationUnits
    */
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.getNotAssociatedObservationUnits(
      graphqlServerUrl,
      item.eventType,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', association: 'observationUnits', table:'A', method: 'getData()', request: 'api.event.getNotAssociatedObservationUnits'}];
            newError.path=['update', `eventType:${item.eventType}`, 'add', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.event.getNotAssociatedObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
