...
//lazy loading
const EventAttributesPage = lazy(() => import(/* webpackChunkName: "Detail-Attributes-Event" */ './components/event-attributes-page/EventAttributesPage'));
const EventAssociationsPage = lazy(() => import(/* webpackChunkName: "Detail-Associations-Event" */ './components/event-associations-page/EventAssociationsPage'));
const EventUpdatePanel = lazy(() => import(/* webpackChunkName: "Detail-Update-Event" */ '../event-update-panel/EventUpdatePanel'));
const EventDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Detail-Delete-Event" */ '../EventDeleteConfirmationDialog'));
const EventParameterDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Detail-EventParameter" */ '../../../eventParameter-table/components/eventParameter-detail-panel/EventParameterDetailPanel'));
...
export default function EventDetailPanel(props) {
      ...
      lastModelChanged['event'][String(item['eventType'])]) {
        ...
        if(lastModelChanged['event'][String(item['eventType'])].op === "update"&&
            lastModelChanged['event'][String(item['eventType'])].newItem) {
              //replace item
              setItemState(lastModelChanged['event'][String(item['eventType'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('event', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('event', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='EventDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Event | eventType: " + itemState['eventType'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='EventDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='EventDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
