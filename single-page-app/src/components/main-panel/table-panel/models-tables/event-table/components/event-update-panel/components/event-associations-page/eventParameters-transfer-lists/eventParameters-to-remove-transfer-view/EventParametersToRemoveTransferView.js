...
import EventParametersToRemoveTransferViewToolbar from './components/EventParametersToRemoveTransferViewToolbar';
import EventParametersToRemoveTransferViewCursorPagination from './components/EventParametersToRemoveTransferViewCursorPagination';
...
export default function EventParametersToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.event.getEventParametersCount
    */
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.getEventParametersCount(
      graphqlServerUrl, 
      item.eventType,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'eventParameters', method: 'getCount()', request: 'api.event.getEventParametersCount'}];
            newError.path=['update', `eventType:${item.eventType}`, 'remove', 'eventParameters'];
      ...
      //error
      .catch((err) => { //error: on api.event.getEventParametersCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    //     exclude: [{
    //       type: 'String',
    //       values: {eventParameterDbId: lidsToRemove.current}
    //     }]
    //   };
    // }

    /*
      API Request: api.event.getEventParameters
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.getEventParameters(
      graphqlServerUrl, 
      item.eventType,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'eventParameters', method: 'getData()', request: 'api.event.getEventParameters'}];
            newError.path=['update', `eventType:${item.eventType}`, 'remove', 'eventParameters'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.event.getEventParameters
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.eventType, search, getCount, configurePagination, onEmptyPage]);
  ...
  useEffect(() => {
     ...
     * Actions:
     * if A:
     * - remove the internalId of the 'eventParameter' from idsToRemove[].
     * - reload both transfer tables.
     * - return
     * 
     * else if B:
            ...
            if(entry[1].changedAssociations.eventParameter_eventDbId.removed) {
              ...
              if(idsRemoved) {
                ...
                if(iof !== -1) {
                  ...
                  let idRemoved = entry[1].newItem.eventParameterDbId;
  ...
  const handleAddItem = (event, item) => {
    if(lidsToRemove.current.indexOf(item.eventParameterDbId) === -1) {
      lidsToRemove.current.push(item.eventParameterDbId);
  ...
  const handleRemoveItem = (event, item) => {
    let iof = lidsToRemove.current.indexOf(item.eventParameterDbId);
    if(iof !== -1) { 
      lidsToRemove.current.splice(iof, 1);
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <EventParametersToRemoveTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventParametersToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='EventParametersToRemoveTransferView-list-listA'
                      dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.eventParameterDbId;
                        let label = it.key;
                        let sublabel = undefined;
                        let disabled = lidsToRemove.current.find(eventParameterDbId=> eventParameterDbId=== it.eventParameterDbId) !== undefined;

                        return (
                          <ListItem 
                            id={'EventParametersToRemoveTransferView-listA-listItem-'+it.eventParameterDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'eventParameterDbId' }>
                                        <Typography variant="body1" display="block" noWrap={true}>{it.eventParameterDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'EventParametersToRemoveTransferView-listA-listItem-'+it.eventParameterDbId+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <EventParametersToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventParametersToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventParametersToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='EventParametersToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.eventParameterDbId;
                        let label = it.key;
                        ...
                        return (
                          <ListItem 
                            id={'EventParametersToRemoveTransferView-listB-listItem-'+it.eventParameterDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'EventParametersToRemoveTransferView-listB-listItem-'+it.eventParameterDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
EventParametersToRemoveTransferView.propTypes = {
