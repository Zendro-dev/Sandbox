...
export default function StudyToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.eventType;

    /*
      API Request: api.event.getStudy
    */
    let cancelableApiReq = makeCancelable(api.event.getStudy(graphqlServerUrl, item.eventType));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'event', association: 'study', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.event.getStudy'}];
            newError.path=['update', `eventType:${currentId}`, 'add', 'study'];
      ...
      //error
      .catch((err) => { //error: on api.event.getStudy
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.eventType]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.event.getNotAssociatedStudyCount
    */
    let cancelableApiReq = makeCancelable(api.event.getNotAssociatedStudyCount(graphqlServerUrl, item.eventType, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', association: 'study', table:'A', method: 'getCount()', request: 'api.event.getNotAssociatedStudyCount'}];
            newError.path=['update', `eventType:${item.eventType}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.event.getNotAssociatedStudyCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.eventType, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.event.getNotAssociatedStudy
    */
    let cancelableApiReq = makeCancelable(api.event.getNotAssociatedStudy(
      graphqlServerUrl,
      item.eventType,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', association: 'study', table:'A', method: 'getData()', request: 'api.event.getNotAssociatedStudy'}];
            newError.path=['update', `eventType:${item.eventType}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.event.getNotAssociatedStudy
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.eventType, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', association: 'study', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `eventType:${item.eventType}`, 'add', 'study'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.eventType, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.event&&
        ...
        lastModelChanged.event[String(item.eventType)].changedAssociations&&
        lastModelChanged.event[String(item.eventType)].changedAssociations.event_studyDbId&&
        (lastModelChanged.event[String(item.eventType)].changedAssociations.event_studyDbId.added ||
         lastModelChanged.event[String(item.eventType)].changedAssociations.event_studyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.event[String(item.eventType)].changedAssociations.event_studyDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.event[String(item.eventType)].changedAssociations.event_studyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'event_studyDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.event_studyDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.event_studyDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.event_studyDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.eventType);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('study', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.event_studyDbId.added) {
              let idsAdded = entry[1].changedAssociations.event_studyDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.eventType);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.eventType, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
