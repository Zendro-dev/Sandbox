...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.event.getObservationUnitsCount
    */
    let cancelableApiReq = makeCancelable(api.event.getObservationUnitsCount(
      graphqlServerUrl, 
      item.eventType,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationUnits', method: 'getCount()', request: 'api.event.getObservationUnitsCount'}];
            newError.path=['update', `eventType:${item.eventType}`, 'remove', 'observationUnits'];
      ...
      //error
      .catch((err) => { //error: on api.event.getObservationUnitsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.event.getObservationUnits
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.event.getObservationUnits(
      graphqlServerUrl, 
      item.eventType,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationUnits', method: 'getData()', request: 'api.event.getObservationUnits'}];
            newError.path=['update', `eventType:${item.eventType}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.event.getObservationUnits
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', association: 'observationUnits', table:'B', method: 'getCountB()', request: 'api.observationUnit.getCountItems'}];
            newError.path=['update', `eventType:${item.eventType}`, 'remove', 'observationUnits'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
        ...
        (response) => {
          ...
          if(response.message === 'ok') {
            ...
            if(response.graphqlErrors) {
              ...
              newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
              newError.locations=[{model: 'event', association: 'observationUnits', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
              newError.path=['update', `eventType:${item.eventType}`, 'remove', 'observationUnits'];
              newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.event&&
        ...
        lastModelChanged.event[String(item.eventType)].changedAssociations.observationUnit_eventDbIds&&
        (lastModelChanged.event[String(item.eventType)].changedAssociations.observationUnit_eventDbIds.added ||
         lastModelChanged.event[String(item.eventType)].changedAssociations.observationUnit_eventDbIds.removed)) {
          
          //remove any dis-associated id from idsToRemove[] & update counts
          let idsRemoved = lastModelChanged.event[String(item.eventType)].changedAssociations.observationUnit_eventDbIds.idsRemoved;
          if(idsRemoved) {
          ...
          let idsAdded = lastModelChanged.event[String(item.eventType)].changedAssociations.observationUnit_eventDbIds.idsAdded;
          if(idsAdded) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'observationUnit_eventDbIds'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationUnit) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationUnit_eventDbIds) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationUnit_eventDbIds.removed) {
              let idsRemoved = entry[1].changedAssociations.observationUnit_eventDbIds.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.observationUnit_eventDbIds.added) {
              let idsAdded = entry[1].changedAssociations.observationUnit_eventDbIds.idsAdded;
              if(idsAdded) {
