...
//lazy loading
const EventAttributesPage = lazy(() => import(/* webpackChunkName: "Create-AttributesEvent" */ './components/event-attributes-page/EventAttributesPage'));
const EventAssociationsPage = lazy(() => import(/* webpackChunkName: "Create-AssociationsEvent" */ './components/event-associations-page/EventAssociationsPage'));
const EventConfirmationDialog = lazy(() => import(/* webpackChunkName: "Create-ConfirmationEvent" */ './components/EventConfirmationDialog'));
const EventParameterDetailPanel = lazy(() => import(/* webpackChunkName: "Create-DetailEventParameter" */ '../../../eventParameter-table/components/eventParameter-detail-panel/EventParameterDetailPanel'));
...
export default function EventCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.eventDescription = null;
    initialValues.eventType = null;
    initialValues.date = null;
    initialValues.observationUnitDbIds = null;
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.eventDbId = 0;
    initialValueOkStates.eventDescription = 0;
    initialValueOkStates.eventType = 0;
  ...
  async function doSave(event) {
    ...
    */
    ...
    //delete: fk's
    delete variables.observationUnitDbIds;
    ...
    /*
      API Request: api.event.createItem
    */
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'event', method: 'doSave()', request: 'api.event.createItem'}];
            newError.path=['Events', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.event.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      case 'eventParameters':
        if(eventParametersIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    if(associationKey === 'eventParameters') {
      let iof = eventParametersIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        eventParametersIdsToAdd.current.splice(iof, 1);
      }
      return;
  ...
  return (
    ...
    <Dialog id='EventCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='EventCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Event'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " event" }>
            <Fab
              id='EventCreatePanel-fabButton-save' 
              color="secondary" 
