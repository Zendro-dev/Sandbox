...
import EventParametersToAddTransferViewToolbar from './components/EventParametersToAddTransferViewToolbar';
import EventParametersToAddTransferViewCursorPagination from './components/EventParametersToAddTransferViewCursorPagination';
...
export default function EventParametersToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"eventParameterDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.event.getNotAssociatedEventParametersCount
    */
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.getNotAssociatedEventParametersCount(graphqlServerUrl, item.eventType, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'event', association: 'eventParameters', table:'A', method: 'getCount()', request: 'api.event.getNotAssociatedEventParametersCount'}];
            newError.path=['update', `eventType:${item.eventType}`, 'add', 'eventParameters'];
      ...
      //error
      .catch((err) => { //error: on api.event.getNotAssociatedEventParametersCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.event.getNotAssociatedEventParameters
    */
    let api = await loadApi("event");
    let cancelableApiReq = makeCancelable(api.event.getNotAssociatedEventParameters(
      graphqlServerUrl,
      item.eventType,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', association: 'eventParameters', table:'A', method: 'getData()', request: 'api.event.getNotAssociatedEventParameters'}];
            newError.path=['update', `eventType:${item.eventType}`, 'add', 'eventParameters'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.event.getNotAssociatedEventParameters
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.eventType, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.eventParameter.getCountItems
    */
    let api = await loadApi("eventParameter");
    let cancelableApiReq = makeCancelable(api.eventParameter.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', association: 'eventParameters', table:'B', method: 'getCountB()', request: 'api.eventParameter.getCountItems'}];
            newError.path=['update', `eventType:${item.eventType}`, 'add', 'eventParameters'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.eventParameter.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.eventType, searchB]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.eventParameter.getItems
    */
    let api = await loadApi("eventParameter");
    let cancelableApiReq = makeCancelable(api.eventParameter.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'event', association: 'eventParameters', table:'B', method: 'getDataB()', request: 'api.eventParameter.getItems'}];
            newError.path=['update', `eventType:${item.eventType}`, 'add', 'eventParameters'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.eventParameter.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.eventType, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'eventParameters' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'eventParameters', 
    ...
    if(lastModelChanged.event&&
        ...
        lastModelChanged.event[String(item.eventType)].changedAssociations.eventParameter_eventDbId&&
        (lastModelChanged.event[String(item.eventType)].changedAssociations.eventParameter_eventDbId.added ||
         lastModelChanged.event[String(item.eventType)].changedAssociations.eventParameter_eventDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.event[String(item.eventType)].changedAssociations.eventParameter_eventDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('eventParameters', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.event[String(item.eventType)].changedAssociations.eventParameter_eventDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'eventParameters' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'eventParameter',
     * but this action happened on the peer relation, identified by 'eventParameter_eventDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'eventParameter'.
     * B: the current item internalId is in the addedIds of the updated 'eventParameter'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'eventParameter' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.eventParameter) {
      let oens = Object.entries(lastModelChanged.eventParameter);
      oens.forEach( (entry) => {
            ...
            if(entry[1].changedAssociations.eventParameter_eventDbId.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.eventParameterDbId;
    ...
    /*
     * Case 3: 
     * The attributes of some 'eventParameter' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.eventParameter) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.eventParameterDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.eventParameterDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.eventParameterDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.eventParameterDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.eventParameterDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.eventParameterDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('eventParameters', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.eventType, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnEventParameterRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.eventParameterDbId) === -1) {
      lidsToAdd.current.push(item.eventParameterDbId);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('eventParameters', item.eventParameterDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.eventParameterDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('eventParameters', item.eventParameterDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <EventParametersToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventParametersToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='EventParametersToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.eventParameterDbId;
                        let label = it.key;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'EventParametersToAddTransferView-listA-listItem-'+it.eventParameterDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'eventParameter' }>
                                <Avatar>{"eventParameter".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'eventParameterDbId' }>
                                        <Typography 
                                        id={'EventParametersToAddTransferView-listA-listItem-id-'+it.eventParameterDbId}
                                        variant="body1" display="block" noWrap={true}>{it.eventParameterDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'EventParametersToAddTransferView-listA-listItem-'+it.eventParameterDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <EventParametersToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventParametersToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='EventParametersToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='EventParametersToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.eventParameterDbId;
                        let label = it.key;
                        ...
                        return (
                          <ListItem 
                            id={'EventParametersToAddTransferView-listB-listItem-'+it.eventParameterDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'eventParameterDbId' }>
                                        <Typography 
                                        id={'EventParametersToAddTransferView-listB-listItem-id-'+it.eventParameterDbId}
                                        variant="body1" display="block" noWrap={true}>{it.eventParameterDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'EventParametersToAddTransferView-listB-listItem-'+it.eventParameterDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
EventParametersToAddTransferView.propTypes = {
