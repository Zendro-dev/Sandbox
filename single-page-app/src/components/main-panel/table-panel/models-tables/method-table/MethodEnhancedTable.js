...
import MethodEnhancedTableHead from './components/MethodEnhancedTableHead'
import MethodEnhancedTableToolbar from './components/MethodEnhancedTableToolbar'
import MethodUploadFileDialog from './components/MethodUploadFileDialog'
import MethodCursorPagination from './components/MethodCursorPagination'
...
//lazy loading
const MethodCreatePanel = lazy(() => import(/* webpackChunkName: "Create-Method" */ './components/method-create-panel/MethodCreatePanel'));
const MethodUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-Method" */ './components/method-update-panel/MethodUpdatePanel'));
const MethodDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Method" */ './components/method-detail-panel/MethodDetailPanel'));
const MethodDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-Method" */ './components/MethodDeleteConfirmationDialog'));
//Plotly
const MethodPlotly = lazy(() => import(/* webpackChunkName: "Plotly-Method" */ '../../../../plots/MethodPlotly'));
...
export default function MethodEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('methodDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.method.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'method', method: 'getCount()', request: 'api.method.getCountItems'}];
            newError.path=['Methods'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'method', method: 'getData()', request: 'api.method.getItems'}];
            newError.path=['Methods'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['method']
    &&(Object.keys(changes.lastModelChanged['method']).length===1)
    &&(changes.lastModelChanged['method'][Object.keys(changes.lastModelChanged['method'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set methodDbId (internalId)
    variables.methodDbId = item.methodDbId;

    /*
      API Request: api.method.deleteItem
    */
    let api = await loadApi("method");
    let cancelableApiReq = makeCancelable(api.method.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'method', method: 'doDelete()', request: 'api.method.deleteItem'}];
            newError.path=['Methods', `methodDbId:${item.methodDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.method.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.method.tableTemplate
    */
    let api = await loadApi("method");
    let cancelableApiReq = makeCancelable(api.method.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'method', method: 'getCsvTemplate()', request: 'api.method.tableTemplate'}];
            newError.path=['Methods', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateMethod.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "method-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.method.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='MethodEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='MethodEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <MethodEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='MethodEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'MethodEnhancedTable-row-'+item.methodDbId}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'MethodEnhancedTable-row-iconButton-detail-'+item.methodDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.method&&Array.isArray(permissions.method)
                                      &&(permissions.method.includes('update') || permissions.method.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'MethodEnhancedTable-row-iconButton-edit-'+item.methodDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.method&&Array.isArray(permissions.method)
                                      &&(permissions.method.includes('delete') || permissions.method.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'MethodEnhancedTable-row-iconButton-delete-'+item.methodDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'methodDbId: ' + item.methodDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.methodDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.formula!==null)?item.formula:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.methodClass!==null)?item.methodClass:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.methodName!==null)?item.methodName:'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <MethodDeleteConfirmationDialog
            permissions={permissions}
