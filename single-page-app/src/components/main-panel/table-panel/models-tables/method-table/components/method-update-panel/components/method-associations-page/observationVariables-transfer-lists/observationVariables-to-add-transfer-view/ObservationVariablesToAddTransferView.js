...
export default function ObservationVariablesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.method.getNotAssociatedObservationVariablesCount
    */
    let cancelableApiReq = makeCancelable(api.method.getNotAssociatedObservationVariablesCount(graphqlServerUrl, item.methodDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'method', association: 'observationVariables', table:'A', method: 'getCount()', request: 'api.method.getNotAssociatedObservationVariablesCount'}];
            newError.path=['update', `methodDbId:${item.methodDbId}`, 'add', 'observationVariables'];
      ...
      //error
      .catch((err) => { //error: on api.method.getNotAssociatedObservationVariablesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.methodDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.method.getNotAssociatedObservationVariables
    */
    let cancelableApiReq = makeCancelable(api.method.getNotAssociatedObservationVariables(
      graphqlServerUrl,
      item.methodDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'method', association: 'observationVariables', table:'A', method: 'getData()', request: 'api.method.getNotAssociatedObservationVariables'}];
            newError.path=['update', `methodDbId:${item.methodDbId}`, 'add', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.method.getNotAssociatedObservationVariables
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.methodDbId, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'method', association: 'observationVariables', table:'B', method: 'getCountB()', request: 'api.observationVariable.getCountItems'}];
            newError.path=['update', `methodDbId:${item.methodDbId}`, 'add', 'observationVariables'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.methodDbId, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'method', association: 'observationVariables', table:'B', method: 'getDataB()', request: 'api.observationVariable.getItems'}];
            newError.path=['update', `methodDbId:${item.methodDbId}`, 'add', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.methodDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
     ...
     * - return
     */
    if(lastModelChanged.method&&
        lastModelChanged.method[String(item.methodDbId)]&&
        lastModelChanged.method[String(item.methodDbId)].changedAssociations&&
        lastModelChanged.method[String(item.methodDbId)].changedAssociations.observationVariable_methodDbId&&
        (lastModelChanged.method[String(item.methodDbId)].changedAssociations.observationVariable_methodDbId.added ||
         lastModelChanged.method[String(item.methodDbId)].changedAssociations.observationVariable_methodDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.method[String(item.methodDbId)].changedAssociations.observationVariable_methodDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.method[String(item.methodDbId)].changedAssociations.observationVariable_methodDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'observationVariable_methodDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationVariable) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationVariable_methodDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationVariable_methodDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observationVariable_methodDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.methodDbId);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.observationVariable_methodDbId.added) {
              let idsAdded = entry[1].changedAssociations.observationVariable_methodDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.methodDbId);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.methodDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
