...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.method.getObservationVariablesCount
    */
    let cancelableApiReq = makeCancelable(api.method.getObservationVariablesCount(
      graphqlServerUrl, 
      item.methodDbId,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'observationVariables', method: 'getCount()', request: 'api.method.getObservationVariablesCount'}];
            newError.path=['update', `methodDbId:${item.methodDbId}`, 'remove', 'observationVariables'];
      ...
      //error
      .catch((err) => { //error: on api.method.getObservationVariablesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.method.getObservationVariables
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.method.getObservationVariables(
      graphqlServerUrl, 
      item.methodDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'observationVariables', method: 'getData()', request: 'api.method.getObservationVariables'}];
            newError.path=['update', `methodDbId:${item.methodDbId}`, 'remove', 'observationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.method.getObservationVariables
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.methodDbId, search, getCount, configurePagination, onEmptyPage]);
