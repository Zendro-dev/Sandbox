...
export default function OntologyReferenceToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.methodDbId;

    /*
      API Request: api.method.getOntologyReference
    */
    let api = await loadApi("method");
    let cancelableApiReq = makeCancelable(api.method.getOntologyReference(graphqlServerUrl, item.methodDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'method', association: 'ontologyReference', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.method.getOntologyReference'}];
            newError.path=['update', `methodDbId:${currentId}`, 'add', 'ontologyReference'];
      ...
      //error
      .catch((err) => { //error: on api.method.getOntologyReference
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.methodDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.method.getNotAssociatedOntologyReferenceCount
    */
    let api = await loadApi("method");
    let cancelableApiReq = makeCancelable(api.method.getNotAssociatedOntologyReferenceCount(graphqlServerUrl, item.methodDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'method', association: 'ontologyReference', table:'A', method: 'getCount()', request: 'api.method.getNotAssociatedOntologyReferenceCount'}];
            newError.path=['update', `methodDbId:${item.methodDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.method.getNotAssociatedOntologyReferenceCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.method.getNotAssociatedOntologyReference
    */
    let api = await loadApi("method");
    let cancelableApiReq = makeCancelable(api.method.getNotAssociatedOntologyReference(
      graphqlServerUrl,
      item.methodDbId,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'method', association: 'ontologyReference', table:'A', method: 'getData()', request: 'api.method.getNotAssociatedOntologyReference'}];
            newError.path=['update', `methodDbId:${item.methodDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.method.getNotAssociatedOntologyReference
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.methodDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'method', association: 'ontologyReference', table:'B', method: 'getDataB()', request: 'api.ontologyReference.getItems'}];
            newError.path=['update', `methodDbId:${item.methodDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.methodDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.method&&
        ...
        lastModelChanged.method[String(item.methodDbId)].changedAssociations.method_ontologyDbId&&
        (lastModelChanged.method[String(item.methodDbId)].changedAssociations.method_ontologyDbId.added ||
         lastModelChanged.method[String(item.methodDbId)].changedAssociations.method_ontologyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.method[String(item.methodDbId)].changedAssociations.method_ontologyDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.method[String(item.methodDbId)].changedAssociations.method_ontologyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'method_ontologyDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyReference) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.method_ontologyDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.method_ontologyDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.method_ontologyDbId.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.method_ontologyDbId.added) {
              let idsAdded = entry[1].changedAssociations.method_ontologyDbId.idsAdded;
              if(idsAdded) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.methodDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
