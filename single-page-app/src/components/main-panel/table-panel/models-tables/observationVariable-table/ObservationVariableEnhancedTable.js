...
import ObservationVariableEnhancedTableHead from './components/ObservationVariableEnhancedTableHead'
import ObservationVariableEnhancedTableToolbar from './components/ObservationVariableEnhancedTableToolbar'
import ObservationVariableUploadFileDialog from './components/ObservationVariableUploadFileDialog'
import ObservationVariableCursorPagination from './components/ObservationVariableCursorPagination'
...
//lazy loading
const ObservationVariableCreatePanel = lazy(() => import(/* webpackChunkName: "Create-ObservationVariable" */ './components/observationVariable-create-panel/ObservationVariableCreatePanel'));
const ObservationVariableUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-ObservationVariable" */ './components/observationVariable-update-panel/ObservationVariableUpdatePanel'));
const ObservationVariableDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-ObservationVariable" */ './components/observationVariable-detail-panel/ObservationVariableDetailPanel'));
const ObservationVariableDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-ObservationVariable" */ './components/ObservationVariableDeleteConfirmationDialog'));
//Plotly
const ObservationVariablePlotly = lazy(() => import(/* webpackChunkName: "Plotly-ObservationVariable" */ '../../../../plots/ObservationVariablePlotly'));
...
export default function ObservationVariableEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('observationVariableDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.observationVariable.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationVariable', method: 'getCount()', request: 'api.observationVariable.getCountItems'}];
            newError.path=['ObservationVariables'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', method: 'getData()', request: 'api.observationVariable.getItems'}];
            newError.path=['ObservationVariables'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['observationVariable']
    &&(Object.keys(changes.lastModelChanged['observationVariable']).length===1)
    &&(changes.lastModelChanged['observationVariable'][Object.keys(changes.lastModelChanged['observationVariable'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set observationVariableDbId (internalId)
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.observationVariable.tableTemplate
    */
    let api = await loadApi("observationVariable");
    let cancelableApiReq = makeCancelable(api.observationVariable.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', method: 'getCsvTemplate()', request: 'api.observationVariable.tableTemplate'}];
            newError.path=['ObservationVariables', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateObservationVariable.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "observationVariable-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <ObservationVariableEnhancedTableToolbar
                ...
                >
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ObservationVariableEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ObservationVariableEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ObservationVariableEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ObservationVariableEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ObservationVariableEnhancedTable-row-'+item.observationVariableDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.observationVariableDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ObservationVariableEnhancedTable-row-iconButton-detail-'+item.observationVariableDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ObservationVariableEnhancedTable-row-iconButton-edit-'+item.observationVariableDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                      ...
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ObservationVariableEnhancedTable-row-iconButton-delete-'+item.observationVariableDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'observationVariableDbId: ' + item.observationVariableDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.observationVariableDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.defaultValue!==null)?item.defaultValue:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.growthStage!==null)?item.growthStage:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.institution!==null)?item.institution:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.language!==null)?item.language:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.scientist!==null)?item.scientist:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.status!==null)?item.status:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.submissionTimestamp!==null)?moment(item.submissionTimestamp, "YYYY-MM-DDTHH:mm:ss.SSSZ").format("YYYY-MM-DDTHH:mm:ss.SSSZ"):'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.xref!==null)?item.xref:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.observationVariableName!==null)?item.observationVariableName:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.methodDbId!==null)?item.methodDbId:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.scaleDbId!==null)?item.scaleDbId:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.traitDbId!==null)?item.traitDbId:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.ontologyDbId!==null)?item.ontologyDbId:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <ObservationVariableCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(ObservationVariablePlotly)) && (
                      <Suspense fallback={<div />}>
                        <ObservationVariablePlotly />
                      </Suspense>
                    )}
                  </div>
      ...
      {(createDialogOpen) && (
        <Suspense fallback={<div />}>
          <ObservationVariableCreatePanel
            permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <ObservationVariableUploadFileDialog
...
ObservationVariableEnhancedTable.propTypes = {
