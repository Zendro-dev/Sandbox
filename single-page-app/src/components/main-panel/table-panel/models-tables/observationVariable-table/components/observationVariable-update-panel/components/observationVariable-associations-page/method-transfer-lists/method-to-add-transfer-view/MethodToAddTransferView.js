...
import MethodToAddTransferViewToolbar from './components/MethodToAddTransferViewToolbar';
import MethodToAddTransferViewCursorPagination from './components/MethodToAddTransferViewCursorPagination';
...
export default function MethodToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getMethod
    */
    let cancelableApiReq = makeCancelable(api.observationVariable.getMethod(graphqlServerUrl, item.observationVariableDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationVariable', association: 'method', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationVariable.getMethod'}];
            newError.path=['update', `observationVariableDbId:${currentId}`, 'add', 'method'];
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getMethod
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"methodDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observationVariable.getNotAssociatedMethodCount
    */
    let cancelableApiReq = makeCancelable(api.observationVariable.getNotAssociatedMethodCount(graphqlServerUrl, item.observationVariableDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'method', table:'A', method: 'getCount()', request: 'api.observationVariable.getNotAssociatedMethodCount'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'method'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getNotAssociatedMethodCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getNotAssociatedMethod
    */
    let cancelableApiReq = makeCancelable(api.observationVariable.getNotAssociatedMethod(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'method', table:'A', method: 'getData()', request: 'api.observationVariable.getNotAssociatedMethod'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'method'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getNotAssociatedMethod
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.method.getItems
    */
    let cancelableApiReq = makeCancelable(api.method.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'method', table:'B', method: 'getDataB()', request: 'api.method.getItems'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'method'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.method.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'method' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'methods', 
    ...
    if(lastModelChanged.observationVariable&&
        ...
        lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_methodDbId&&
        (lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_methodDbId.added ||
         lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_methodDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_methodDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('method', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('method', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_methodDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('method', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'method' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'method',
     * but this action happened on the peer relation, identified by 'observationVariable_methodDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'method'.
     * B: the current item internalId is in the addedIds of the updated 'method'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'method' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.method) {
      let oens = Object.entries(lastModelChanged.method);
      oens.forEach( (entry) => {
            ...
            if(entry[1].changedAssociations.observationVariable_methodDbId.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.methodDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('method', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'method' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.method) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.methodDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.methodDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.methodDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.methodDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.methodDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.methodDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('method', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.methodDbId===idRemoved) {
            //update associated item
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnMethodRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.methodDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.methodDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.methodDbId];
        handleDisassociateItem('method', associatedItem.methodDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('method', item.methodDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('method', item.methodDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <MethodToAddTransferViewToolbar
                title={'Methods'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='MethodToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.methodDbId;
                        let label = it.methodName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'MethodToAddTransferView-listA-listItem-'+it.methodDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.methodDbId===it.methodDbId) ? 'method — ' + t('modelPanels.associatedRecord', 'Associated record') : 'method '}>
                                <Avatar>
                                  {"method".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'methodDbId' }>
                                        <Typography 
                                        id={'MethodToAddTransferView-listA-listItem-id-'+it.methodDbId}
                                        variant="body1" display="block" noWrap={true}>{it.methodDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.methodDbId===it.methodDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.methodDbId===it.methodDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'MethodToAddTransferView-listA-listItem-'+it.methodDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <MethodToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='MethodToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.methodDbId;
                        let label = it.methodName;
                        ...
                        return (
                          <ListItem 
                            id={'MethodToAddTransferView-listB-listItem-'+it.methodDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'methodDbId' }>
                                        <Typography 
                                        id={'MethodToAddTransferView-listB-listItem-id-'+it.methodDbId}
                                        variant="body1" display="block" noWrap={true}>{it.methodDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'MethodToAddTransferView-listB-listItem-'+it.methodDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='MethodToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.methodDbId;
                        let label = associatedItem.methodName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'MethodToAddTransferView-associatedItem-list-listItem-'+associatedItem.methodDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'method — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"method".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'methodDbId' }>
                                        <Typography 
                                        id={'MethodToAddTransferView-listB-listItem-id-'+associatedItem.methodDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.methodDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'MethodToAddTransferView-associatedItem-list-listItem-'+associatedItem.methodDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].methodDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('method', associatedItem.methodDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.methodDbId];
                                      handleDisassociateItem('method', associatedItem.methodDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='MethodToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
MethodToAddTransferView.propTypes = {
