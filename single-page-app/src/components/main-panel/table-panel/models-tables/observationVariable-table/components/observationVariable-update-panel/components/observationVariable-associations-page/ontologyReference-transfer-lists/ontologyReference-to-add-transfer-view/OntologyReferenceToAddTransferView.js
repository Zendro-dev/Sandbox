...
export default function OntologyReferenceToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getOntologyReference
    */
    let cancelableApiReq = makeCancelable(api.observationVariable.getOntologyReference(graphqlServerUrl, item.observationVariableDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationVariable', association: 'ontologyReference', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationVariable.getOntologyReference'}];
            newError.path=['update', `observationVariableDbId:${currentId}`, 'add', 'ontologyReference'];
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getOntologyReference
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getNotAssociatedOntologyReferenceCount
    */
    let cancelableApiReq = makeCancelable(api.observationVariable.getNotAssociatedOntologyReferenceCount(graphqlServerUrl, item.observationVariableDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'ontologyReference', table:'A', method: 'getCount()', request: 'api.observationVariable.getNotAssociatedOntologyReferenceCount'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getNotAssociatedOntologyReferenceCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getNotAssociatedOntologyReference
    */
    let cancelableApiReq = makeCancelable(api.observationVariable.getNotAssociatedOntologyReference(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'ontologyReference', table:'A', method: 'getData()', request: 'api.observationVariable.getNotAssociatedOntologyReference'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'ontologyReference'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getNotAssociatedOntologyReference
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.observationVariable&&
        ...
        lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_ontologyDbId&&
        (lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_ontologyDbId.added ||
         lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_ontologyDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_ontologyDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_ontologyDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
