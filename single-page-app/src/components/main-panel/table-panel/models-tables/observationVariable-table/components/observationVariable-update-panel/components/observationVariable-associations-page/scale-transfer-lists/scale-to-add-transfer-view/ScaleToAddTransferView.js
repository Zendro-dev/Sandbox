...
import ScaleToAddTransferViewToolbar from './components/ScaleToAddTransferViewToolbar';
import ScaleToAddTransferViewCursorPagination from './components/ScaleToAddTransferViewCursorPagination';
...
export default function ScaleToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getScale
    */
    let api = await loadApi("observationVariable");
    let cancelableApiReq = makeCancelable(api.observationVariable.getScale(graphqlServerUrl, item.observationVariableDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationVariable', association: 'scale', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationVariable.getScale'}];
            newError.path=['update', `observationVariableDbId:${currentId}`, 'add', 'scale'];
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getScale
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"scaleDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observationVariable.getNotAssociatedScaleCount
    */
    let api = await loadApi("observationVariable");
    let cancelableApiReq = makeCancelable(api.observationVariable.getNotAssociatedScaleCount(graphqlServerUrl, item.observationVariableDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'scale', table:'A', method: 'getCount()', request: 'api.observationVariable.getNotAssociatedScaleCount'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'scale'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getNotAssociatedScaleCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getNotAssociatedScale
    */
    let api = await loadApi("observationVariable");
    let cancelableApiReq = makeCancelable(api.observationVariable.getNotAssociatedScale(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'scale', table:'A', method: 'getData()', request: 'api.observationVariable.getNotAssociatedScale'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'scale'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getNotAssociatedScale
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.scale.getItems
    */
    let api = await loadApi("scale");
    let cancelableApiReq = makeCancelable(api.scale.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'scale', table:'B', method: 'getDataB()', request: 'api.scale.getItems'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'scale'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.scale.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'scale' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'scales', 
    ...
    if(lastModelChanged.observationVariable&&
        ...
        lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_scaleDbId&&
        (lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_scaleDbId.added ||
         lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_scaleDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_scaleDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('scale', idAdded);
              }
            });
            /**
            ...
            if(lidsToRemove.current && lidsToRemove.current.length>0) {
              ...
              handleDisassociateItem('scale', idsAdded[0]);
            }
          }

          //update count for each dis-associated internalId from idsToAdd[]
          let idsRemoved = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observationVariable_scaleDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
            ...
            if(!idsAdded && lidsToRemove.current && lidsToRemove.current.length>0) {
              handleReassociateItem('scale', lidsToRemove.current[0]);
    ...
    /*
     * Case 2: 
     * The relation 'scale' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'scale',
     * but this action happened on the peer relation, identified by 'observationVariable_scaleDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'scale'.
     * B: the current item internalId is in the addedIds of the updated 'scale'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'scale' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.scale) {
      let oens = Object.entries(lastModelChanged.scale);
      oens.forEach( (entry) => {
            ...
            if(entry[1].changedAssociations.observationVariable_scaleDbId.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.scaleDbId;
                  ...
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    ...
                    handleDisassociateItem('scale', idAdded);
                  } 
    ...
    /*
     * Case 3: 
     * The attributes of some 'scale' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.scale) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.scaleDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.scaleDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.scaleDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
          ...
          if(associatedItem && associatedItem.scaleDbId===idUpdated) {
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.scaleDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.scaleDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          handleUntransfer('scale', idRemoved);

          //lookup on associated item
          if(associatedItem && associatedItem.scaleDbId===idRemoved) {
            //update associated item
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnScaleRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current&&lidsToAdd.current.indexOf(item.scaleDbId) === -1) {
      let hasItem = (lidsToAdd.current.length > 0);
      lidsToAdd.current = [];
      lidsToAdd.current.push(item.scaleDbId);
      ...
      if(associatedItem && checked) {
        setChecked(false);
        lidsToRemove.current = [associatedItem.scaleDbId];
        handleDisassociateItem('scale', associatedItem.scaleDbId);
      }
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('scale', item.scaleDbId);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    if(lidsToAdd.current.length > 0) {
      ...
      //reload B
      reloadDataB();
      handleUntransfer('scale', item.scaleDbId);
    }
  };
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ScaleToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ScaleToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.scaleDbId;
                        let label = it.scaleName;
                        let sublabel = undefined;
                        
                        return (
                          <ListItem 
                            id={'ScaleToAddTransferView-listA-listItem-'+it.scaleDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ (associatedItem&&associatedItem.scaleDbId===it.scaleDbId) ? 'scale — ' + t('modelPanels.associatedRecord', 'Associated record') : 'scale '}>
                                <Avatar>
                                  {"scale".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'scaleDbId' }>
                                        <Typography 
                                        id={'ScaleToAddTransferView-listA-listItem-id-'+it.scaleDbId}
                                        variant="body1" display="block" noWrap={true}>{it.scaleDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                                      <Tooltip title={ (associatedItem&&associatedItem.scaleDbId===it.scaleDbId) ? t('modelPanels.internalId', 'Unique Identifier') + ' — ' + t('modelPanels.associatedRecord', 'Associated record') : t('modelPanels.internalId', 'Unique Identifier')}>
                                        <Key fontSize="small" style={{ marginTop:8, color: (associatedItem&&associatedItem.scaleDbId===it.scaleDbId) ? green[500] : grey[400]}} />
                                      </Tooltip>
                                    </Grid>
                                  </Grid>
                                </React.Fragment>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ScaleToAddTransferView-listA-listItem-'+it.scaleDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ScaleToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ScaleToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ScaleToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ScaleToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.scaleDbId;
                        let label = it.scaleName;
                        ...
                        return (
                          <ListItem 
                            id={'ScaleToAddTransferView-listB-listItem-'+it.scaleDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'scaleDbId' }>
                                        <Typography 
                                        id={'ScaleToAddTransferView-listB-listItem-id-'+it.scaleDbId}
                                        variant="body1" display="block" noWrap={true}>{it.scaleDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ScaleToAddTransferView-listB-listItem-'+it.scaleDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
            ...
            <Paper className={classes.paper} variant='outlined'>
              ...
              {(associatedItem !== undefined && associatedItem !== null) && (
                ...
                >
                  <Box className={classes.listBox}>
                    ...
                    {/* List */}
                    <List id='ScaleToAddTransferView-associatedItem-list'
                      dense component="div" role="list">
                      {[associatedItem].map(associatedItem => {
                        let key = associatedItem.scaleDbId;
                        let label = associatedItem.scaleName;
                        let sublabel = undefined;

                        return (
                          <ListItem 
                            id={'ScaleToAddTransferView-associatedItem-list-listItem-'+associatedItem.scaleDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'scale — ' + t('modelPanels.associatedRecord', 'Associated record') }>
                                <Avatar>
                                  {"scale".slice(0,1)}
                                </Avatar>
                              </Tooltip>
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'scaleDbId' }>
                                        <Typography 
                                        id={'ScaleToAddTransferView-listB-listItem-id-'+associatedItem.scaleDbId}
                                        variant="body1" display="block" noWrap={true}>{associatedItem.scaleDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ checked ? t('modelPanels.uncheckToDissasociate', 'Uncheck to disassociate') : t('modelPanels.checkToReassociate', 'Check to keep associated') }>
                                <Checkbox
                                  id={'ScaleToAddTransferView-associatedItem-list-listItem-'+associatedItem.scaleDbId+'-checkbox'}
                                  ...
                                  onChange={(event) => {
                                    ...
                                    if(event.target.checked) {
                                      ...
                                      && itemsB[0].scaleDbId===lidsToAdd.current[0]) {
                                        handleRemoveItem(null, itemsB[0]);
                                      }
                                      lidsToRemove.current = [];
                                      handleReassociateItem('scale', associatedItem.scaleDbId);
                                    } else {
                                      lidsToRemove.current = [associatedItem.scaleDbId];
                                      handleDisassociateItem('scale', associatedItem.scaleDbId);
                                    }
              ...
              {( associatedItem === null ) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ScaleToAddTransferView-div-noAssociatedItem'>
                    <Grid container>
                      <Grid item xs={12}>
...
ScaleToAddTransferView.propTypes = {
