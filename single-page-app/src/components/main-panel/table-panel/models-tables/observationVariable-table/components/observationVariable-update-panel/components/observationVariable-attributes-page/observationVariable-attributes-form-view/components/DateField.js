  ...
  return (
    <Grid container justify='flex-start' alignItems='center' spacing={0}>
      <Grid item>
        <MuiPickersUtilsProvider libInstance={moment} utils={MomentUtils} locale={i18n.language}>
          <FormControl className={classes.formControl}>
            <KeyboardDatePicker
              id={'DateField-ObservationVariable-'+name}
              ...
              InputAdornmentProps={{
                id: 'DateField-ObservationVariable-input-inputAdornment-'+name,
              }}
              KeyboardButtonProps={{
                id: 'DateField-ObservationVariable-input-inputAdornment-button-'+name,
              }}
              onChange={(date, value) => {
      ...
      <Grid item>
        {(valueOk!==undefined&&valueOk===1) 
          ...
          <Tooltip title={ t('modelPanels.valueEntered', "Value entered") }>
            <Typography id={'DateField-ObservationVariable-exists-'+name} variant="caption" color="primary">
              &#8707;
            </Typography>
          </Tooltip>
          : 
          <Tooltip title={ t('modelPanels.valueNotEntered', "Value not entered") }>
            <Typography id={'DateField-ObservationVariable-notExists-'+name} variant="caption" color="textSecondary">
              &#8708;
            </Typography>
          </Tooltip>
        }
      ...
      {(valueAjv !== undefined && valueAjv.errors.length > 0) && (
        <Grid item id={'DateField-ObservationVariable-ajvError-'+name} xs={12}> 
