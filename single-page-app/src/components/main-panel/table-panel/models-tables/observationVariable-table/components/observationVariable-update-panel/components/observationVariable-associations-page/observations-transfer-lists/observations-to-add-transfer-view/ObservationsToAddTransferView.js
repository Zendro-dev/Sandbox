...
import ObservationsToAddTransferViewToolbar from './components/ObservationsToAddTransferViewToolbar';
import ObservationsToAddTransferViewCursorPagination from './components/ObservationsToAddTransferViewCursorPagination';
...
export default function ObservationsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    if(lidsToAdd.current && lidsToAdd.current.length > 0) {
      ops = {
        exclude: [{
          type: 'String',
          values: {"observationDbId": lidsToAdd.current}
        }]
      };
    }    

    /*
      API Request: api.observationVariable.getNotAssociatedObservationsCount
    */
    let api = await loadApi("observationVariable");
    let cancelableApiReq = makeCancelable(api.observationVariable.getNotAssociatedObservationsCount(graphqlServerUrl, item.observationVariableDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationVariable', association: 'observations', table:'A', method: 'getCount()', request: 'api.observationVariable.getNotAssociatedObservationsCount'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'observations'];
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getNotAssociatedObservationsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationVariable.getNotAssociatedObservations
    */
    let api = await loadApi("observationVariable");
    let cancelableApiReq = makeCancelable(api.observationVariable.getNotAssociatedObservations(
      graphqlServerUrl,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'observations', table:'A', method: 'getData()', request: 'api.observationVariable.getNotAssociatedObservations'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getNotAssociatedObservations
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.observationVariableDbId, dataTrigger, search, configurePagination, onEmptyPage]);
  ...
  const getCountB = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getCountItems
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getCountItems(graphqlServerUrl, searchB, ops));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'observations', table:'B', method: 'getCountB()', request: 'api.observation.getCountItems'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'observations'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
    ...
    //error
    .catch((err) => { //error: on api.observation.getCountItems
      if(err.isCanceled) {
        return;
      } else {
        let newError = {};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.observationVariableDbId, searchB]);
  ...
  const getDataB = useCallback(async () => {
    ...
    /*
      API Request: api.observation.getItems
    */
    let api = await loadApi("observation");
    let cancelableApiReq = makeCancelable(api.observation.getItems(
      graphqlServerUrl,
      searchB,
      null, //orderBy
      null, //orderDirection
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationVariable', association: 'observations', table:'B', method: 'getDataB()', request: 'api.observation.getItems'}];
            newError.path=['update', `observationVariableDbId:${item.observationVariableDbId}`, 'add', 'observations'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observation.getItems
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.observationVariableDbId, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'observations' for this item was updated.
     * That is to say that the current item was associated or dis-associated with some 'observations', 
    ...
    if(lastModelChanged.observationVariable&&
        ...
        lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observation_observationVariableDbId&&
        (lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observation_observationVariableDbId.added ||
         lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observation_observationVariableDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observation_observationVariableDbId.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('observations', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.observationVariable[String(item.observationVariableDbId)].changedAssociations.observation_observationVariableDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'observations' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'observation',
     * but this action happened on the peer relation, identified by 'observation_observationVariableDbId'.
     * 
     * Conditions:
     * A: the current item internalId is in the removedIds of the updated 'observation'.
     * B: the current item internalId is in the addedIds of the updated 'observation'.
     * 
     * Actions:
     * if A:
     * - update associatedIds[].
     ...
     * 
     * else if B:
     * - remove the internalId of the 'observation' from idsToAdd[].
     * - update associatedIds[].
     * - reload both transfer tables.
     * - return
     */
    if(lastModelChanged.observation) {
      let oens = Object.entries(lastModelChanged.observation);
      oens.forEach( (entry) => {
            ...
            if(entry[1].changedAssociations.observation_observationVariableDbId.added) {
              ...
              if(idsAdded) {
                ...
                if(iof !== -1) {
                  ...
                  let idAdded = entry[1].newItem.observationDbId;
    ...
    /*
     * Case 3: 
     * The attributes of some 'observation' were modified or the item was deleted.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observation) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "update"&&entry[1].newItem) {
          let idUpdated = entry[1].item.observationDbId;
          
          //lookup item on table A
          let nitemsA = Array.from(items);
          let iofA = nitemsA.findIndex((item) => item.observationDbId===idUpdated);
          if(iofA !== -1) {
            //set new item
          ...
          let iofB = nitemsB.findIndex((item) => item.observationDbId===idUpdated);
          if(iofB !== -1) {
            //set new item
        ...
        if(entry[1].op === "delete") {
          let idRemoved = entry[1].item.observationDbId;

          //lookup item on table A
          let iofA = items.findIndex((item) => item.observationDbId===idRemoved);
          if(iofA !== -1) {
            //decrement A
            setCount(count-1);
          }
          ...
          //lookup item on table B
          let iofB = itemsB.findIndex((item) => item.observationDbId===idRemoved);
          if(iofB !== -1) {
            //decrement B
            setCountB(countB-1);
          }
          ...
          handleUntransfer('observations', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationVariableDbId, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleRowClicked = (event, item) => {
    handleClickOnObservationRow(event, item);
  };

  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.observationDbId) === -1) {
      lidsToAdd.current.push(item.observationDbId);
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('observations', item.observationDbId);
    }
  };

  const handleRemoveItem = (event, item) => {
    let iof = lidsToAdd.current.indexOf(item.observationDbId);
    if(iof !== -1) { 
      lidsToAdd.current.splice(iof, 1);
      ...
      //reload B
      reloadDataB();
      handleUntransfer('observations', item.observationDbId);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ObservationsToAddTransferViewToolbar
                title={'Observations'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ObservationsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        let key = it.observationDbId;
                        let label = it.observationVariableName;
                        let sublabel = it.value;
                        
                        return (
                          <ListItem 
                            id={'ObservationsToAddTransferView-listA-listItem-'+it.observationDbId}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            <ListItemAvatar>
                              <Tooltip title={ 'observation' }>
                                <Avatar>{"observation".slice(0,1)}</Avatar>
                              </Tooltip>
                            </ListItemAvatar>
                              ...
                              primary={
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationDbId' }>
                                        <Typography 
                                        id={'ObservationsToAddTransferView-listA-listItem-id-'+it.observationDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                                    <Grid item>
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'ObservationsToAddTransferView-listA-listItem-'+it.observationDbId+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <ObservationsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ObservationsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ObservationsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.observationDbId;
                        let label = it.observationVariableName;
                        ...
                        return (
                          <ListItem 
                            id={'ObservationsToAddTransferView-listB-listItem-'+it.observationDbId}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'observationDbId' }>
                                        <Typography 
                                        id={'ObservationsToAddTransferView-listB-listItem-id-'+it.observationDbId}
                                        variant="body1" display="block" noWrap={true}>{it.observationDbId}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'ObservationsToAddTransferView-listB-listItem-'+it.observationDbId+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ObservationsToAddTransferView.propTypes = {
