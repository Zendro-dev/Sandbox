...
//lazy loading
const AlienAttributesPage = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Attributes-Alien" */ './components/alien-attributes-page/AlienAttributesPage')));
const AlienAssociationsPage = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Associations-Alien" */ './components/alien-associations-page/AlienAssociationsPage')));
const AlienUpdatePanel = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Update-Alien" */ '../alien-update-panel/AlienUpdatePanel')));
const AlienDeleteConfirmationDialog = lazy(() => retry(() => import(/* webpackChunkName: "Detail-Delete-Alien" */ '../AlienDeleteConfirmationDialog')));
...
export default function AlienDetailPanel(props) {
      ...
      lastModelChanged['alien'][String(item['idField'])]) {
        ...
        if(lastModelChanged['alien'][String(item['idField'])].op === "update"&&
            lastModelChanged['alien'][String(item['idField'])].newItem) {
              //replace item
              setItemState(lastModelChanged['alien'][String(item['idField'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('alien', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('alien', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='AlienDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Alien | idField: " + itemState['idField'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='AlienDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='AlienDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
