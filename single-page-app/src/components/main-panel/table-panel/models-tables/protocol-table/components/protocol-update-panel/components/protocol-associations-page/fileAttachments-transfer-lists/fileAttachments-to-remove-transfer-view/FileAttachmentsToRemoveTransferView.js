...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.protocol.getFileAttachmentsCount(
      graphqlServerUrl, 
      item.protocol_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'fileAttachments', method: 'getCount()', request: 'api.protocol.getFileAttachmentsCount'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'remove', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.protocol.getFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getFileAttachments
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.protocol.getFileAttachments(
      graphqlServerUrl, 
      item.protocol_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'fileAttachments', method: 'getData()', request: 'api.protocol.getFileAttachments'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'remove', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.protocol.getFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
