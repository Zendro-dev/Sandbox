...
export default function AssaysToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getNotAssociatedAssaysCount
    */
    let cancelableApiReq = makeCancelable(api.protocol.getNotAssociatedAssaysCount(graphqlServerUrl, item.protocol_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'protocol', association: 'assays', table:'A', method: 'getCount()', request: 'api.protocol.getNotAssociatedAssaysCount'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'assays'];
      ...
      //error
      .catch((err) => { //error: on api.protocol.getNotAssociatedAssaysCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getNotAssociatedAssays
    */
    let cancelableApiReq = makeCancelable(api.protocol.getNotAssociatedAssays(
      graphqlServerUrl,
      item.protocol_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'assays', table:'A', method: 'getData()', request: 'api.protocol.getNotAssociatedAssays'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.protocol.getNotAssociatedAssays
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'assays', table:'B', method: 'getCountB()', request: 'api.assay.getCountItems'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'assays'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'assays', table:'B', method: 'getDataB()', request: 'api.assay.getItems'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.protocol&&
        ...
        lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_assay_ids&&
        (lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_assay_ids.added ||
         lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_assay_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_assay_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_assay_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'protocol_assay_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.assay) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.protocol_assay_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.protocol_assay_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.protocol_assay_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.protocol_assay_ids.added) {
              let idsAdded = entry[1].changedAssociations.protocol_assay_ids.idsAdded;
              if(idsAdded) {
