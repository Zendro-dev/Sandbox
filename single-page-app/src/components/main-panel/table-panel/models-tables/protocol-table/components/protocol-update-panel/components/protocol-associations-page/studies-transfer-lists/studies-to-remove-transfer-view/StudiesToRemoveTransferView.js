...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.protocol.getStudiesCount(
      graphqlServerUrl, 
      item.protocol_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'studies', method: 'getCount()', request: 'api.protocol.getStudiesCount'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'remove', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.protocol.getStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getStudies
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.protocol.getStudies(
      graphqlServerUrl, 
      item.protocol_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'studies', method: 'getData()', request: 'api.protocol.getStudies'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'remove', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.protocol.getStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
