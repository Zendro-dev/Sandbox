...
import ProtocolUpdatePanel from '../protocol-update-panel/ProtocolUpdatePanel'
import ProtocolDeleteConfirmationDialog from '../ProtocolDeleteConfirmationDialog'
...
export default function ProtocolDetailPanel(props) {
      ...
      lastModelChanged['protocol'][String(item['protocol_id'])]) {
        ...
        if(lastModelChanged['protocol'][String(item['protocol_id'])].op === "update"&&
            lastModelChanged['protocol'][String(item['protocol_id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['protocol'][String(item['protocol_id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('protocol', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('protocol', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='ProtocolDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Protocol | protocol_id: " + itemState['protocol_id'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='ProtocolDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='ProtocolDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
