...
import OntologyAnnotationToAddTransferViewToolbar from './components/OntologyAnnotationToAddTransferViewToolbar';
import OntologyAnnotationToAddTransferViewCursorPagination from './components/OntologyAnnotationToAddTransferViewCursorPagination';
...
export default function OntologyAnnotationToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getNotAssociatedOntologyAnnotationCount
    */
    let cancelableApiReq = makeCancelable(api.protocol.getNotAssociatedOntologyAnnotationCount(graphqlServerUrl, item.protocol_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'protocol', association: 'ontologyAnnotation', table:'A', method: 'getCount()', request: 'api.protocol.getNotAssociatedOntologyAnnotationCount'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'ontologyAnnotation'];
      ...
      //error
      .catch((err) => { //error: on api.protocol.getNotAssociatedOntologyAnnotationCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.protocol_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getNotAssociatedOntologyAnnotation
    */
    let cancelableApiReq = makeCancelable(api.protocol.getNotAssociatedOntologyAnnotation(
      graphqlServerUrl,
      item.protocol_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'ontologyAnnotation', table:'A', method: 'getData()', request: 'api.protocol.getNotAssociatedOntologyAnnotation'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'ontologyAnnotation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.protocol.getNotAssociatedOntologyAnnotation
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.protocol_id, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'ontologyAnnotation', table:'B', method: 'getCountB()', request: 'api.ontologyAnnotation.getCountItems'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'ontologyAnnotation'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.protocol_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'ontologyAnnotation', table:'B', method: 'getDataB()', request: 'api.ontologyAnnotation.getItems'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'ontologyAnnotation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.protocol_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'ontologyAnnotation' for this item was updated.
     ...
     * - return
     */
    if(lastModelChanged.protocol&&
        lastModelChanged.protocol[String(item.protocol_id)]&&
        lastModelChanged.protocol[String(item.protocol_id)].changedAssociations&&
        lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_ontologyAnnotation_ids&&
        (lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_ontologyAnnotation_ids.added ||
         lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_ontologyAnnotation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_ontologyAnnotation_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('ontologyAnnotation', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_ontologyAnnotation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'ontologyAnnotation' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'ontologyAnnotation',
     * but this action happened on the peer relation, identified by 'protocol_ontologyAnnotation_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyAnnotation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.protocol_ontologyAnnotation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.protocol_ontologyAnnotation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.protocol_ontologyAnnotation_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.protocol_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.protocol_ontologyAnnotation_ids.added) {
              let idsAdded = entry[1].changedAssociations.protocol_ontologyAnnotation_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.protocol_id);
                if(iof !== -1) {
    ...
    if(lastModelChanged.ontologyAnnotation) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('ontologyAnnotation', idRemoved);
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.protocol_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.ontologyAnnotation_id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('ontologyAnnotation', item.ontologyAnnotation_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('ontologyAnnotation', item.ontologyAnnotation_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <OntologyAnnotationToAddTransferViewToolbar
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyAnnotationToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='OntologyAnnotationToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'OntologyAnnotationToAddTransferView-listA-listItem-'+it.ontologyAnnotation_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'ontologyAnnotation_id' }>
                                        <Typography 
                                        id={'OntologyAnnotationToAddTransferView-listA-listItem-id-'+it.ontologyAnnotation_id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'OntologyAnnotationToAddTransferView-listA-listItem-'+it.ontologyAnnotation_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <OntologyAnnotationToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyAnnotationToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='OntologyAnnotationToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='OntologyAnnotationToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.ontologyAnnotation_id;
                        let label = it.ontology;
                        ...
                        return (
                          <ListItem 
                            id={'OntologyAnnotationToAddTransferView-listB-listItem-'+it.ontologyAnnotation_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'ontologyAnnotation_id' }>
                                        <Typography 
                                        id={'OntologyAnnotationToAddTransferView-listB-listItem-id-'+it.ontologyAnnotation_id}
                                        variant="body1" display="block" noWrap={true}>{it.ontologyAnnotation_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'OntologyAnnotationToAddTransferView-listB-listItem-'+it.ontologyAnnotation_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
OntologyAnnotationToAddTransferView.propTypes = {
