...
export default function ProtocolUpdatePanel(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      lastModelChanged.protocol&&
      lastModelChanged.protocol[String(item.protocol_id)]) {

        //updated item
        if(lastModelChanged.protocol[String(item.protocol_id)].op === "update"&&
            lastModelChanged.protocol[String(item.protocol_id)].newItem) {
              //show alert
              setUpdated(true);
        } else {
          //deleted item
          if(lastModelChanged.protocol[String(item.protocol_id)].op === "delete") {
              //show alert
              setDeleted(true);
          }
        }
    }//end: Case 1
  }, [lastModelChanged, lastChangeTimestamp, item.protocol_id]);
  ...
  function getInitialValues() {
    ...
    initialValues.protocol_id = item.protocol_id;
  ...
  initialValueOkStates.protocol_id = (item.protocol_id!==null ? 1 : 0);
  ...
  function areThereChangedFields() {
    if(values.current.protocol_id !== item.protocol_id) { return true;}
  ...
  function setAddRemoveManyAssays(variables) {
    ...
    if(!changedAssociations.current.protocol_assay_ids) changedAssociations.current.protocol_assay_ids = {};
    ...
    if(assaysIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.protocol_assay_ids.added = true;
      if(changedAssociations.current.protocol_assay_ids.idsAdded){
        assaysIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.protocol_assay_ids.idsAdded.includes(it)) changedAssociations.current.protocol_assay_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.protocol_assay_ids.idsAdded = [...assaysIdsToAdd.current];
      }
    }
    /**
    ...
    if(assaysIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.protocol_assay_ids.removed = true;
      if(changedAssociations.current.protocol_assay_ids.idsRemoved){
        assaysIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.protocol_assay_ids.idsRemoved.includes(it)) changedAssociations.current.protocol_assay_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.protocol_assay_ids.idsRemoved = [...assaysIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyFileAttachments(variables) {
    //data to notify changes
    if(!changedAssociations.current.protocol_fileAttachment_ids) changedAssociations.current.protocol_fileAttachment_ids = {};
    ...
    if(fileAttachmentsIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.protocol_fileAttachment_ids.added = true;
      if(changedAssociations.current.protocol_fileAttachment_ids.idsAdded){
        fileAttachmentsIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.protocol_fileAttachment_ids.idsAdded.includes(it)) changedAssociations.current.protocol_fileAttachment_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.protocol_fileAttachment_ids.idsAdded = [...fileAttachmentsIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(fileAttachmentsIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.protocol_fileAttachment_ids.removed = true;
      if(changedAssociations.current.protocol_fileAttachment_ids.idsRemoved){
        fileAttachmentsIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.protocol_fileAttachment_ids.idsRemoved.includes(it)) changedAssociations.current.protocol_fileAttachment_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.protocol_fileAttachment_ids.idsRemoved = [...fileAttachmentsIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyOntologyAnnotation(variables) {
    //data to notify changes
    if(!changedAssociations.current.protocol_ontologyAnnotation_ids) changedAssociations.current.protocol_ontologyAnnotation_ids = {};
    ...
    if(ontologyAnnotationIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.protocol_ontologyAnnotation_ids.added = true;
      if(changedAssociations.current.protocol_ontologyAnnotation_ids.idsAdded){
        ontologyAnnotationIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.protocol_ontologyAnnotation_ids.idsAdded.includes(it)) changedAssociations.current.protocol_ontologyAnnotation_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.protocol_ontologyAnnotation_ids.idsAdded = [...ontologyAnnotationIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(ontologyAnnotationIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.protocol_ontologyAnnotation_ids.removed = true;
      if(changedAssociations.current.protocol_ontologyAnnotation_ids.idsRemoved){
        ontologyAnnotationIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.protocol_ontologyAnnotation_ids.idsRemoved.includes(it)) changedAssociations.current.protocol_ontologyAnnotation_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.protocol_ontologyAnnotation_ids.idsRemoved = [...ontologyAnnotationIdsToRemove.current];
      }
    }
  ...
  function setAddRemoveManyStudies(variables) {
    //data to notify changes
    if(!changedAssociations.current.protocol_study_ids) changedAssociations.current.protocol_study_ids = {};
    ...
    if(studiesIdsToAdd.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.protocol_study_ids.added = true;
      if(changedAssociations.current.protocol_study_ids.idsAdded){
        studiesIdsToAdd.current.forEach((it) => {if(!changedAssociations.current.protocol_study_ids.idsAdded.includes(it)) changedAssociations.current.protocol_study_ids.idsAdded.push(it);});
      } else {
        changedAssociations.current.protocol_study_ids.idsAdded = [...studiesIdsToAdd.current];
      }
    }
    /**
     * Case: The toRemove list isn't empty.
    ...
    if(studiesIdsToRemove.current.length>0) {
      ...
      //changes to nofity
      changedAssociations.current.protocol_study_ids.removed = true;
      if(changedAssociations.current.protocol_study_ids.idsRemoved){
        studiesIdsToRemove.current.forEach((it) => {if(!changedAssociations.current.protocol_study_ids.idsRemoved.includes(it)) changedAssociations.current.protocol_study_ids.idsRemoved.push(it);});
      } else {
        changedAssociations.current.protocol_study_ids.idsRemoved = [...studiesIdsToRemove.current];
      }
    }
...
function setAjvErrors(err) {
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.protocol.updateItem
    */
    let cancelableApiReq = makeCancelable(api.protocol.updateItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'protocol', method: 'doSave()', request: 'api.protocol.updateItem'}];
            newError.path=['Protocols', `protocol_id:${item.protocol_id}`, 'update'];
      ...
      //error
      .catch((err) => { //error: on api.protocol.updateItem
        if(err.isCanceled) {
          return;
        } else {
          //set ajv errors
  ...
  return (
    <Dialog id='ProtocolUpdatePanel-dialog' 
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='ProtocolUpdatePanel-button-cancel'
              edge="start" 
              color="inherit" 
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            { t('modelPanels.editing') +  ": Protocol | protocol_id: " + item.protocol_id}
          </Typography>
          
          {(!deleted)&&(
            <Tooltip title={ t('modelPanels.save') + " protocol" }>
              <Fab
                id='ProtocolUpdatePanel-fabButton-save' 
                color="secondary" 
