...
export default function FileAttachmentsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getNotAssociatedFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.protocol.getNotAssociatedFileAttachmentsCount(graphqlServerUrl, item.protocol_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'protocol', association: 'fileAttachments', table:'A', method: 'getCount()', request: 'api.protocol.getNotAssociatedFileAttachmentsCount'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.protocol.getNotAssociatedFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getNotAssociatedFileAttachments
    */
    let cancelableApiReq = makeCancelable(api.protocol.getNotAssociatedFileAttachments(
      graphqlServerUrl,
      item.protocol_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'fileAttachments', table:'A', method: 'getData()', request: 'api.protocol.getNotAssociatedFileAttachments'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.protocol.getNotAssociatedFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'fileAttachments', table:'B', method: 'getCountB()', request: 'api.fileAttachment.getCountItems'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'fileAttachments'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'fileAttachments', table:'B', method: 'getDataB()', request: 'api.fileAttachment.getItems'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.protocol&&
        ...
        lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_fileAttachment_ids&&
        (lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_fileAttachment_ids.added ||
         lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_fileAttachment_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_fileAttachment_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_fileAttachment_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'protocol_fileAttachment_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.fileAttachment) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.protocol_fileAttachment_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.protocol_fileAttachment_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.protocol_fileAttachment_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.protocol_fileAttachment_ids.added) {
              let idsAdded = entry[1].changedAssociations.protocol_fileAttachment_ids.idsAdded;
              if(idsAdded) {
