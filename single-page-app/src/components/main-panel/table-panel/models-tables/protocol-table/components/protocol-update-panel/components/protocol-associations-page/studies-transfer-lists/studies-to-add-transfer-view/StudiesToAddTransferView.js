...
export default function StudiesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getNotAssociatedStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.protocol.getNotAssociatedStudiesCount(graphqlServerUrl, item.protocol_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'protocol', association: 'studies', table:'A', method: 'getCount()', request: 'api.protocol.getNotAssociatedStudiesCount'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.protocol.getNotAssociatedStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getNotAssociatedStudies
    */
    let cancelableApiReq = makeCancelable(api.protocol.getNotAssociatedStudies(
      graphqlServerUrl,
      item.protocol_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'studies', table:'A', method: 'getData()', request: 'api.protocol.getNotAssociatedStudies'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.protocol.getNotAssociatedStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'studies', table:'B', method: 'getCountB()', request: 'api.study.getCountItems'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'studies'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', association: 'studies', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.protocol&&
        ...
        lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_study_ids&&
        (lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_study_ids.added ||
         lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_study_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_study_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.protocol[String(item.protocol_id)].changedAssociations.protocol_study_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'protocol_study_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.protocol_study_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.protocol_study_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.protocol_study_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.protocol_study_ids.added) {
              let idsAdded = entry[1].changedAssociations.protocol_study_ids.idsAdded;
              if(idsAdded) {
