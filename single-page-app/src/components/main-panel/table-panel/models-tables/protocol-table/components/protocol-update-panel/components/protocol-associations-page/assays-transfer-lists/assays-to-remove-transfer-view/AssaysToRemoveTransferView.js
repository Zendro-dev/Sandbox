...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getAssaysCount
    */
    let cancelableApiReq = makeCancelable(api.protocol.getAssaysCount(
      graphqlServerUrl, 
      item.protocol_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'assays', method: 'getCount()', request: 'api.protocol.getAssaysCount'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'remove', 'assays'];
      ...
      //error
      .catch((err) => { //error: on api.protocol.getAssaysCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.protocol.getAssays
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.protocol.getAssays(
      graphqlServerUrl, 
      item.protocol_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'assays', method: 'getData()', request: 'api.protocol.getAssays'}];
            newError.path=['update', `protocol_id:${item.protocol_id}`, 'remove', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.protocol.getAssays
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
