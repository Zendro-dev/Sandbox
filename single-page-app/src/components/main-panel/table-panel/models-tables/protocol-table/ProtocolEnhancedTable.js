...
import ProtocolEnhancedTableHead from './components/ProtocolEnhancedTableHead'
import ProtocolEnhancedTableToolbar from './components/ProtocolEnhancedTableToolbar'
import ProtocolCreatePanel from './components/protocol-create-panel/ProtocolCreatePanel'
import ProtocolUpdatePanel from './components/protocol-update-panel/ProtocolUpdatePanel'
import ProtocolDetailPanel from './components/protocol-detail-panel/ProtocolDetailPanel'
import ProtocolDeleteConfirmationDialog from './components/ProtocolDeleteConfirmationDialog'
import ProtocolUploadFileDialog from './components/ProtocolUploadFileDialog'
import ProtocolCursorPagination from './components/ProtocolCursorPagination'
...
//Plotly
import ProtocolPlotly from '../../../../plots/ProtocolPlotly';
...
export default function ProtocolEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('protocol_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.protocol.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'protocol', method: 'getCount()', request: 'api.protocol.getCountItems'}];
            newError.path=['Protocols'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', method: 'getData()', request: 'api.protocol.getItems'}];
            newError.path=['Protocols'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['protocol']
    &&(Object.keys(changes.lastModelChanged['protocol']).length===1)
    &&(changes.lastModelChanged['protocol'][Object.keys(changes.lastModelChanged['protocol'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set protocol_id (internalId)
    variables.protocol_id = item.protocol_id;

    /*
      API Request: api.protocol.deleteItem
    */
    let cancelableApiReq = makeCancelable(api.protocol.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', method: 'doDelete()', request: 'api.protocol.deleteItem'}];
            newError.path=['Protocols', `protocol_id:${item.protocol_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.protocol.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.protocol.tableTemplate
    */
    let cancelableApiReq = makeCancelable(api.protocol.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'protocol', method: 'getCsvTemplate()', request: 'api.protocol.tableTemplate'}];
            newError.path=['Protocols', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateProtocol.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "protocol-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.protocol.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <ProtocolEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(ProtocolPlotly)}
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ProtocolEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ProtocolEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ProtocolEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ProtocolEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ProtocolEnhancedTable-row-'+item.protocol_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ProtocolEnhancedTable-row-iconButton-detail-'+item.protocol_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.protocol&&Array.isArray(permissions.protocol)
                                      &&(permissions.protocol.includes('update') || permissions.protocol.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ProtocolEnhancedTable-row-iconButton-edit-'+item.protocol_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.protocol&&Array.isArray(permissions.protocol)
                                      &&(permissions.protocol.includes('delete') || permissions.protocol.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ProtocolEnhancedTable-row-iconButton-delete-'+item.protocol_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'protocol_id: ' + item.protocol_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.protocol_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.type!==null)?moment(item.type, "YYYY-MM-DD").format("YYYY-MM-DD"):'')}
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <ProtocolDeleteConfirmationDialog
          permissions={permissions}
...
ProtocolEnhancedTable.propTypes = {
