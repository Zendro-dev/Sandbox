...
import MaterialEnhancedTableHead from './components/MaterialEnhancedTableHead'
import MaterialEnhancedTableToolbar from './components/MaterialEnhancedTableToolbar'
import MaterialCreatePanel from './components/material-create-panel/MaterialCreatePanel'
import MaterialUpdatePanel from './components/material-update-panel/MaterialUpdatePanel'
import MaterialDetailPanel from './components/material-detail-panel/MaterialDetailPanel'
import MaterialDeleteConfirmationDialog from './components/MaterialDeleteConfirmationDialog'
import MaterialUploadFileDialog from './components/MaterialUploadFileDialog'
import MaterialCursorPagination from './components/MaterialCursorPagination'
...
//Plotly
import MaterialPlotly from '../../../../plots/MaterialPlotly';
...
export default function MaterialEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('material_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.material.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'material', method: 'getCount()', request: 'api.material.getCountItems'}];
            newError.path=['Materials'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', method: 'getData()', request: 'api.material.getItems'}];
            newError.path=['Materials'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['material']
    &&(Object.keys(changes.lastModelChanged['material']).length===1)
    &&(changes.lastModelChanged['material'][Object.keys(changes.lastModelChanged['material'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set material_id (internalId)
    variables.material_id = item.material_id;

    /*
      API Request: api.material.deleteItem
    */
    let cancelableApiReq = makeCancelable(api.material.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', method: 'doDelete()', request: 'api.material.deleteItem'}];
            newError.path=['Materials', `material_id:${item.material_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.material.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.material.tableTemplate
    */
    let cancelableApiReq = makeCancelable(api.material.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', method: 'getCsvTemplate()', request: 'api.material.tableTemplate'}];
            newError.path=['Materials', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateMaterial.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "material-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.material.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <MaterialEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(MaterialPlotly)}
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='MaterialEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='MaterialEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <MaterialEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='MaterialEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'MaterialEnhancedTable-row-'+item.material_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'MaterialEnhancedTable-row-iconButton-detail-'+item.material_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.material&&Array.isArray(permissions.material)
                                      &&(permissions.material.includes('update') || permissions.material.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'MaterialEnhancedTable-row-iconButton-edit-'+item.material_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.material&&Array.isArray(permissions.material)
                                      &&(permissions.material.includes('delete') || permissions.material.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'MaterialEnhancedTable-row-iconButton-delete-'+item.material_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'material_id: ' + item.material_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.material_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.sourceSet_ids!==null)?item.sourceSet_ids:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.element_ids!==null)?item.element_ids:'')}
                                    </TableCell>
...
MaterialEnhancedTable.propTypes = {
