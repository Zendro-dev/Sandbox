...
export default function AssaysToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedAssaysCount
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedAssaysCount(graphqlServerUrl, item.material_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'material', association: 'assays', table:'A', method: 'getCount()', request: 'api.material.getNotAssociatedAssaysCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'assays'];
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedAssaysCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedAssays
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedAssays(
      graphqlServerUrl,
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'assays', table:'A', method: 'getData()', request: 'api.material.getNotAssociatedAssays'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedAssays
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'assays', table:'B', method: 'getCountB()', request: 'api.assay.getCountItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'assays'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'assays', table:'B', method: 'getDataB()', request: 'api.assay.getItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.material&&
        ...
        lastModelChanged.material[String(item.material_id)].changedAssociations.material_assay_ids&&
        (lastModelChanged.material[String(item.material_id)].changedAssociations.material_assay_ids.added ||
         lastModelChanged.material[String(item.material_id)].changedAssociations.material_assay_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.material[String(item.material_id)].changedAssociations.material_assay_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.material[String(item.material_id)].changedAssociations.material_assay_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'material_assay_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.assay) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.material_assay_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.material_assay_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.material_assay_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.material_assay_ids.added) {
              let idsAdded = entry[1].changedAssociations.material_assay_ids.idsAdded;
              if(idsAdded) {
