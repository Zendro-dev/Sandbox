...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getAssayResultsCount
    */
    let cancelableApiReq = makeCancelable(api.material.getAssayResultsCount(
      graphqlServerUrl, 
      item.material_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'assayResults', method: 'getCount()', request: 'api.material.getAssayResultsCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'assayResults'];
      ...
      //error
      .catch((err) => { //error: on api.material.getAssayResultsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getAssayResults
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.material.getAssayResults(
      graphqlServerUrl, 
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'assayResults', method: 'getData()', request: 'api.material.getAssayResults'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'assayResults'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getAssayResults
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
