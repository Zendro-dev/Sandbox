...
import SourceSetsToAddTransferViewToolbar from './components/SourceSetsToAddTransferViewToolbar';
import SourceSetsToAddTransferViewCursorPagination from './components/SourceSetsToAddTransferViewCursorPagination';
...
export default function SourceSetsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedSourceSetsCount
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedSourceSetsCount(graphqlServerUrl, item.material_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'material', association: 'sourceSets', table:'A', method: 'getCount()', request: 'api.material.getNotAssociatedSourceSetsCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'sourceSets'];
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedSourceSetsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedSourceSets
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedSourceSets(
      graphqlServerUrl,
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'sourceSets', table:'A', method: 'getData()', request: 'api.material.getNotAssociatedSourceSets'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'sourceSets'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedSourceSets
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'sourceSets', table:'B', method: 'getCountB()', request: 'api.material.getCountItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'sourceSets'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'sourceSets', table:'B', method: 'getDataB()', request: 'api.material.getItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'sourceSets'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    /*
     * Case 1: 
     * The relation 'sourceSets' for this item was updated.
    ...
    if(lastModelChanged.material&&
        ...
        lastModelChanged.material[String(item.material_id)].changedAssociations.material_sourceSet_ids&&
        (lastModelChanged.material[String(item.material_id)].changedAssociations.material_sourceSet_ids.added ||
         lastModelChanged.material[String(item.material_id)].changedAssociations.material_sourceSet_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.material[String(item.material_id)].changedAssociations.material_sourceSet_ids.idsAdded;
          if(idsAdded) {
            idsAdded.forEach( (idAdded) => {
              ...
              if(iof !== -1) { 
                ...
                handleUntransfer('sourceSets', idAdded);
                //decrement B
                setCountB(countB-1);
              }
              //decrement A
          ...
          let idsRemoved = lastModelChanged.material[String(item.material_id)].changedAssociations.material_sourceSet_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
    ...
    /*
     * Case 2: 
     * The relation 'sourceSets' for this item was updated from the target model (in the peer relation).
     * That is to say that this current item was associated or dis-associated with some 'material',
     * but this action happened on the peer relation, identified by 'material_sourceSet_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.material) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.material_sourceSet_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.material_sourceSet_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.material_sourceSet_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.material_sourceSet_ids.added) {
              let idsAdded = entry[1].changedAssociations.material_sourceSet_ids.idsAdded;
              if(idsAdded) {
    ...
    if(lastModelChanged.material) {
      ...
      oens.forEach( (entry) => {
        ...
        if(entry[1].op === "delete") {
          ...
          handleUntransfer('sourceSets', idRemoved);
...
const handleReloadClickB = (event) => {
  ...
  const handleAddItem = (event, item) => {
    if(lidsToAdd.current.indexOf(item.material_id) === -1) {
      ...
      //reload B
      setDataTriggerB(prevDataTriggerB => !prevDataTriggerB);
      handleTransfer('sourceSets', item.material_id);
    }
  };
  ...
  const handleRemoveItem = (event, item) => {
    ...
    if(iof !== -1) { 
      ...
      //reload B
      reloadDataB();
      handleUntransfer('sourceSets', item.material_id);
    }
  };
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <SourceSetsToAddTransferViewToolbar
                title={'Materials'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SourceSetsToAddTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='SourceSetsToAddTransferView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'SourceSetsToAddTransferView-listA-listItem-'+it.material_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'material_id' }>
                                        <Typography 
                                        id={'SourceSetsToAddTransferView-listA-listItem-id-'+it.material_id}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.transferToAdd') }>
                                <IconButton
                                  id={'SourceSetsToAddTransferView-listA-listItem-'+it.material_id+'-button-add'}
                                  color="primary"
              ...
              {/* Pagination */}
              <SourceSetsToAddTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SourceSetsToAddTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='SourceSetsToAddTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='SourceSetsToAddTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.material_id;
                        let label = it.name;
                        ...
                        return (
                          <ListItem 
                            id={'SourceSetsToAddTransferView-listB-listItem-'+it.material_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'material_id' }>
                                        <Typography 
                                        id={'SourceSetsToAddTransferView-listB-listItem-id-'+it.material_id}
                                        variant="body1" display="block" noWrap={true}>{it.material_id}</Typography>
                                      </Tooltip>
                                    </Grid>
                                    {/*Key icon*/}
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToAdd') }>
                                <IconButton
                                  id={'SourceSetsToAddTransferView-listB-listItem-'+it.material_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
SourceSetsToAddTransferView.propTypes = {
