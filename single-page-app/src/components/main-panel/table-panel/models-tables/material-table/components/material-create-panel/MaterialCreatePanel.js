...
export default function MaterialCreatePanel(props) {
  ...
  function getInitialValues() {
    ...
    initialValues.sourceSet_ids = null;
    initialValues.element_ids = null;
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.material_id = 0;
  ...
  async function doSave(event) {
    ...
    */
    ...
    delete variables.sourceSet_ids;
    delete variables.element_ids;
    ...
    /*
      API Request: api.material.createItem
    */
    let cancelableApiReq = makeCancelable(api.material.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'material', method: 'doSave()', request: 'api.material.createItem'}];
            newError.path=['Materials', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.material.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  const handleTransferToAdd = (associationKey, itemId) => {
    switch(associationKey) {
      ...
      case 'fileAttachments':
        ...
        break;
      case 'sourceSets':
        if(sourceSetsIdsToAdd.current.indexOf(itemId) === -1) {
        ...
        break;
      case 'elements':
        if(elementsIdsToAdd.current.indexOf(itemId) === -1) {
  ...
  const handleUntransferFromAdd =(associationKey, itemId) => {
    ...
    if(associationKey === 'sourceSets') {
      let iof = sourceSetsIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        sourceSetsIdsToAdd.current.splice(iof, 1);
    ...
    if(associationKey === 'elements') {
      let iof = elementsIdsToAdd.current.indexOf(itemId);
      if(iof !== -1) {
        elementsIdsToAdd.current.splice(iof, 1);
  ...
  return (
    ...
    <Dialog id='MaterialCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='MaterialCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Material'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " material" }>
            <Fab
              id='MaterialCreatePanel-fabButton-save' 
              color="secondary" 
