...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getOntologyAnnotationCount
    */
    let cancelableApiReq = makeCancelable(api.material.getOntologyAnnotationCount(
      graphqlServerUrl, 
      item.material_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'ontologyAnnotation', method: 'getCount()', request: 'api.material.getOntologyAnnotationCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'ontologyAnnotation'];
      ...
      //error
      .catch((err) => { //error: on api.material.getOntologyAnnotationCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getOntologyAnnotation
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.material.getOntologyAnnotation(
      graphqlServerUrl, 
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'ontologyAnnotation', method: 'getData()', request: 'api.material.getOntologyAnnotation'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'ontologyAnnotation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getOntologyAnnotation
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.material_id, search, getCount, configurePagination, onEmptyPage]);
