...
export default function FileAttachmentsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedFileAttachmentsCount(graphqlServerUrl, item.material_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'material', association: 'fileAttachments', table:'A', method: 'getCount()', request: 'api.material.getNotAssociatedFileAttachmentsCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedFileAttachments
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedFileAttachments(
      graphqlServerUrl,
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'fileAttachments', table:'A', method: 'getData()', request: 'api.material.getNotAssociatedFileAttachments'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'fileAttachments', table:'B', method: 'getCountB()', request: 'api.fileAttachment.getCountItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'fileAttachments'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'fileAttachments', table:'B', method: 'getDataB()', request: 'api.fileAttachment.getItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.material&&
        ...
        lastModelChanged.material[String(item.material_id)].changedAssociations.material_fileAttachment_ids&&
        (lastModelChanged.material[String(item.material_id)].changedAssociations.material_fileAttachment_ids.added ||
         lastModelChanged.material[String(item.material_id)].changedAssociations.material_fileAttachment_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.material[String(item.material_id)].changedAssociations.material_fileAttachment_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.material[String(item.material_id)].changedAssociations.material_fileAttachment_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'material_fileAttachment_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.fileAttachment) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.material_fileAttachment_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.material_fileAttachment_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.material_fileAttachment_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.material_fileAttachment_ids.added) {
              let idsAdded = entry[1].changedAssociations.material_fileAttachment_ids.idsAdded;
              if(idsAdded) {
