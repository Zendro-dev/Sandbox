...
import ElementsToRemoveTransferViewToolbar from './components/ElementsToRemoveTransferViewToolbar';
import ElementsToRemoveTransferViewCursorPagination from './components/ElementsToRemoveTransferViewCursorPagination';
...
export default function ElementsToRemoveTransferView(props) {
...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getElementsCount
    */
    let cancelableApiReq = makeCancelable(api.material.getElementsCount(
      graphqlServerUrl, 
      item.material_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'elements', method: 'getCount()', request: 'api.material.getElementsCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'elements'];
      ...
      //error
      .catch((err) => { //error: on api.material.getElementsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getElements
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.material.getElements(
      graphqlServerUrl, 
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'elements', method: 'getData()', request: 'api.material.getElements'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'elements'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getElements
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
        ...
        <Grid item xs={12} sm={5} >
          {(item!==undefined) && (
            <Card className={classes.card}>
              ...
              {/* Toolbar */}
              <ElementsToRemoveTransferViewToolbar
                title={'Materials'}
                titleIcon={1}
                search={search}
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ElementsToRemoveTransferView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='ElementsToRemoveTransferView-list-listA'
                      ...
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'ElementsToRemoveTransferView-listA-listItem-'+it.material_id}
                            key={key} 
                            role="listitem" 
                            button 
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={!disabled ? t('modelPanels.transferToRemove') : t('modelPanels.alreadyToRemove') }>
                                <span>
                                  <IconButton
                                    id={'ElementsToRemoveTransferView-listA-listItem-'+it.material_id+'-button-remove'}
                                    color="primary"
              ...
              {/* Pagination */}
              <ElementsToRemoveTransferViewCursorPagination
                count={count}
            ...
            <Card className={classes.card}>
              ...
              {(!thereAreItemsToAdd) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ElementsToRemoveTransferView-div-noItemsB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && (!areItemsReadyB)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='ElementsToRemoveTransferView-div-noDataB'>
                    <Grid container>
                      <Grid item xs={12}>
                        <Grid className={classes.noDataBox} container justify="center" alignItems="center">
                          <Grid item>
              ...
              {(thereAreItemsToAdd && !isOnApiRequestB && areItemsReadyB) && (
                ...
                >
                  <Box className={classes.listBox} ref={lrefB}>
                    <List id='ElementsToRemoveTransferView-list-listB'
                      dense component="div" role="list">
                      {itemsB.map(it => {
                        let key = it.material_id;
                        let label = it.name;
                        ...
                        return (
                          <ListItem 
                            id={'ElementsToRemoveTransferView-listB-listItem-'+it.material_id}
                            key={key} 
                            role="listitem"
                            button
                            className={classes.row}
                          ...
                          >
                            ...
                            <ListItemSecondaryAction>
                              <Tooltip title={ t('modelPanels.untransferToRemove') }>
                                <IconButton
                                  id={'ElementsToRemoveTransferView-listB-listItem-'+it.material_id+'-button-remove'}
                                  color="primary"
                                  onClick={(event) => {
                                    event.stopPropagation();
...
ElementsToRemoveTransferView.propTypes = {
