...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.material.getStudiesCount(
      graphqlServerUrl, 
      item.material_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'studies', method: 'getCount()', request: 'api.material.getStudiesCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.material.getStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getStudies
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.material.getStudies(
      graphqlServerUrl, 
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'studies', method: 'getData()', request: 'api.material.getStudies'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
