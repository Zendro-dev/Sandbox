...
import MaterialUpdatePanel from '../material-update-panel/MaterialUpdatePanel'
...
import MaterialDetailPanelB from '../../../material-table/components/material-detail-panel/MaterialDetailPanel'
...
export default function MaterialDetailPanel(props) {
      ...
      lastModelChanged['material'][String(item['material_id'])]) {
        ...
        if(lastModelChanged['material'][String(item['material_id'])].op === "update"&&
            lastModelChanged['material'][String(item['material_id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['material'][String(item['material_id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('material', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('material', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='MaterialDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Material | material_id: " + itemState['material_id'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='MaterialDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='MaterialDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
