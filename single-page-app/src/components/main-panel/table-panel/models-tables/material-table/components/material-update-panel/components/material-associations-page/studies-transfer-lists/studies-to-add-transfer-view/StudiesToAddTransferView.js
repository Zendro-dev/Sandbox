...
export default function StudiesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedStudiesCount(graphqlServerUrl, item.material_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'material', association: 'studies', table:'A', method: 'getCount()', request: 'api.material.getNotAssociatedStudiesCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedStudies
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedStudies(
      graphqlServerUrl,
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'studies', table:'A', method: 'getData()', request: 'api.material.getNotAssociatedStudies'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'studies', table:'B', method: 'getCountB()', request: 'api.study.getCountItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'studies'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'studies', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.material&&
        ...
        lastModelChanged.material[String(item.material_id)].changedAssociations.material_study_ids&&
        (lastModelChanged.material[String(item.material_id)].changedAssociations.material_study_ids.added ||
         lastModelChanged.material[String(item.material_id)].changedAssociations.material_study_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.material[String(item.material_id)].changedAssociations.material_study_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.material[String(item.material_id)].changedAssociations.material_study_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'material_study_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.material_study_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.material_study_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.material_study_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.material_study_ids.added) {
              let idsAdded = entry[1].changedAssociations.material_study_ids.idsAdded;
              if(idsAdded) {
