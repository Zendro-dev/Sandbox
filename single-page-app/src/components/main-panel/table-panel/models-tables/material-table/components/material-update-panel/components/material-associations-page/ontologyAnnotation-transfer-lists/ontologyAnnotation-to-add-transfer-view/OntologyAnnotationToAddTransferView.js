...
export default function OntologyAnnotationToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedOntologyAnnotationCount
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedOntologyAnnotationCount(graphqlServerUrl, item.material_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'material', association: 'ontologyAnnotation', table:'A', method: 'getCount()', request: 'api.material.getNotAssociatedOntologyAnnotationCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'ontologyAnnotation'];
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedOntologyAnnotationCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.material_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getNotAssociatedOntologyAnnotation
    */
    let cancelableApiReq = makeCancelable(api.material.getNotAssociatedOntologyAnnotation(
      graphqlServerUrl,
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'ontologyAnnotation', table:'A', method: 'getData()', request: 'api.material.getNotAssociatedOntologyAnnotation'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'ontologyAnnotation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getNotAssociatedOntologyAnnotation
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.material_id, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'ontologyAnnotation', table:'B', method: 'getCountB()', request: 'api.ontologyAnnotation.getCountItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'ontologyAnnotation'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.material_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'material', association: 'ontologyAnnotation', table:'B', method: 'getDataB()', request: 'api.ontologyAnnotation.getItems'}];
            newError.path=['update', `material_id:${item.material_id}`, 'add', 'ontologyAnnotation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.material_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.material&&
        ...
        lastModelChanged.material[String(item.material_id)].changedAssociations&&
        lastModelChanged.material[String(item.material_id)].changedAssociations.material_ontologyAnnotation_ids&&
        (lastModelChanged.material[String(item.material_id)].changedAssociations.material_ontologyAnnotation_ids.added ||
         lastModelChanged.material[String(item.material_id)].changedAssociations.material_ontologyAnnotation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.material[String(item.material_id)].changedAssociations.material_ontologyAnnotation_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.material[String(item.material_id)].changedAssociations.material_ontologyAnnotation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'material_ontologyAnnotation_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyAnnotation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.material_ontologyAnnotation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.material_ontologyAnnotation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.material_ontologyAnnotation_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.material_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.material_ontologyAnnotation_ids.added) {
              let idsAdded = entry[1].changedAssociations.material_ontologyAnnotation_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.material_id);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.material_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
