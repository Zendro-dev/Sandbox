...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.material.getAssaysCount
    */
    let cancelableApiReq = makeCancelable(api.material.getAssaysCount(
      graphqlServerUrl, 
      item.material_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'assays', method: 'getCount()', request: 'api.material.getAssaysCount'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'assays'];
      ...
      //error
      .catch((err) => { //error: on api.material.getAssaysCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.material.getAssays
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.material.getAssays(
      graphqlServerUrl, 
      item.material_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'assays', method: 'getData()', request: 'api.material.getAssays'}];
            newError.path=['update', `material_id:${item.material_id}`, 'remove', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.material.getAssays
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
