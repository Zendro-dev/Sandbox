...
import FactorEnhancedTableHead from './components/FactorEnhancedTableHead'
import FactorEnhancedTableToolbar from './components/FactorEnhancedTableToolbar'
import FactorCreatePanel from './components/factor-create-panel/FactorCreatePanel'
import FactorUpdatePanel from './components/factor-update-panel/FactorUpdatePanel'
import FactorDetailPanel from './components/factor-detail-panel/FactorDetailPanel'
import FactorDeleteConfirmationDialog from './components/FactorDeleteConfirmationDialog'
import FactorUploadFileDialog from './components/FactorUploadFileDialog'
import FactorCursorPagination from './components/FactorCursorPagination'
...
//Plotly
import FactorPlotly from '../../../../plots/FactorPlotly';
...
export default function FactorEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('factor_id');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    let cancelableApiReq = makeCancelable(api.factor.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'factor', method: 'getCount()', request: 'api.factor.getCountItems'}];
            newError.path=['Factors'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', method: 'getData()', request: 'api.factor.getItems'}];
            newError.path=['Factors'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['factor']
    &&(Object.keys(changes.lastModelChanged['factor']).length===1)
    &&(changes.lastModelChanged['factor'][Object.keys(changes.lastModelChanged['factor'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set factor_id (internalId)
    variables.factor_id = item.factor_id;

    /*
      API Request: api.factor.deleteItem
    */
    let cancelableApiReq = makeCancelable(api.factor.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', method: 'doDelete()', request: 'api.factor.deleteItem'}];
            newError.path=['Factors', `factor_id:${item.factor_id}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.factor.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.factor.tableTemplate
    */
    let cancelableApiReq = makeCancelable(api.factor.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', method: 'getCsvTemplate()', request: 'api.factor.tableTemplate'}];
            newError.path=['Factors', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateFactor.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "factor-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.factor.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <FactorEnhancedTableToolbar
                  permissions={permissions}
                  search={search}
                  showToggleButtons={Boolean(FactorPlotly)}
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='FactorEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='FactorEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <FactorEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='FactorEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'FactorEnhancedTable-row-'+item.factor_id}
                                    hover
                                  ...
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'FactorEnhancedTable-row-iconButton-detail-'+item.factor_id}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.factor&&Array.isArray(permissions.factor)
                                      &&(permissions.factor.includes('update') || permissions.factor.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'FactorEnhancedTable-row-iconButton-edit-'+item.factor_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.factor&&Array.isArray(permissions.factor)
                                      &&(permissions.factor.includes('delete') || permissions.factor.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'FactorEnhancedTable-row-iconButton-delete-'+item.factor_id}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'factor_id: ' + item.factor_id}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.factor_id}</Typography>
                                      </Tooltip>
                                    </TableCell>
      ...
      {(deleteConfirmationDialogOpen) && (
        <FactorDeleteConfirmationDialog
          permissions={permissions}
