...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.factor.getStudiesCount(
      graphqlServerUrl, 
      item.factor_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'studies', method: 'getCount()', request: 'api.factor.getStudiesCount'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'remove', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.factor.getStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getStudies
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.factor.getStudies(
      graphqlServerUrl, 
      item.factor_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'studies', method: 'getData()', request: 'api.factor.getStudies'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'remove', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.factor.getStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
