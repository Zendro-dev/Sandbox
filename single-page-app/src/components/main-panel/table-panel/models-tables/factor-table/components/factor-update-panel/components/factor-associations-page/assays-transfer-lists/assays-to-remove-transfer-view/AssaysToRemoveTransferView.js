...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getAssaysCount
    */
    let cancelableApiReq = makeCancelable(api.factor.getAssaysCount(
      graphqlServerUrl, 
      item.factor_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'assays', method: 'getCount()', request: 'api.factor.getAssaysCount'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'remove', 'assays'];
      ...
      //error
      .catch((err) => { //error: on api.factor.getAssaysCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getAssays
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.factor.getAssays(
      graphqlServerUrl, 
      item.factor_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'assays', method: 'getData()', request: 'api.factor.getAssays'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'remove', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.factor.getAssays
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
