...
export default function FactorCreatePanel(props) {
  ...
  function getInitialValueOkStates() {
    ...
    */
    ...
    initialValueOkStates.factor_id = 0;
  ...
  async function doSave(event) {
    ...
    */
    ...
    /*
      API Request: api.factor.createItem
    */
    let cancelableApiReq = makeCancelable(api.factor.createItem(graphqlServerUrl, variables));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'factor', method: 'doSave()', request: 'api.factor.createItem'}];
            newError.path=['Factors', 'add'];
      ...
      //error
      .catch((err) => { //error: on api.factor.createItem
        if(err.isCanceled) {
          return
        } else {
          //set ajv errors
  ...
  return (
    ...
    <Dialog id='FactorCreatePanel-dialog'  
      ...
      <AppBar>
        <Toolbar>
          <Tooltip title={ t('modelPanels.cancel') }>
            <IconButton
              id='FactorCreatePanel-button-cancel'
              edge="start" 
              color="inherit"
              onClick={(event) => {
                if(!isCanceling.current){
          ...
          <Typography variant="h6" className={classes.title}>
            {t('modelPanels.new') + ' Factor'}
          </Typography>
          <Tooltip title={ t('modelPanels.save') + " factor" }>
            <Fab
              id='FactorCreatePanel-fabButton-save' 
              color="secondary" 
