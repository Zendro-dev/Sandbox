...
export default function OntologyAnnotationToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getNotAssociatedOntologyAnnotationCount
    */
    let cancelableApiReq = makeCancelable(api.factor.getNotAssociatedOntologyAnnotationCount(graphqlServerUrl, item.factor_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'factor', association: 'ontologyAnnotation', table:'A', method: 'getCount()', request: 'api.factor.getNotAssociatedOntologyAnnotationCount'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'ontologyAnnotation'];
      ...
      //error
      .catch((err) => { //error: on api.factor.getNotAssociatedOntologyAnnotationCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.factor_id, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getNotAssociatedOntologyAnnotation
    */
    let cancelableApiReq = makeCancelable(api.factor.getNotAssociatedOntologyAnnotation(
      graphqlServerUrl,
      item.factor_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'ontologyAnnotation', table:'A', method: 'getData()', request: 'api.factor.getNotAssociatedOntologyAnnotation'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'ontologyAnnotation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.factor.getNotAssociatedOntologyAnnotation
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.factor_id, dataTrigger, search, configurePagination, onEmptyPage]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'ontologyAnnotation', table:'B', method: 'getCountB()', request: 'api.ontologyAnnotation.getCountItems'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'ontologyAnnotation'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageD, t, item.factor_id, searchB]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'ontologyAnnotation', table:'B', method: 'getDataB()', request: 'api.ontologyAnnotation.getItems'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'ontologyAnnotation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.factor_id, dataTriggerB, searchB, getCountB, configurePaginationB, onEmptyPageB]);
  ...
  useEffect(() => {
     ...
     * - return
     */
    if(lastModelChanged.factor&&
        lastModelChanged.factor[String(item.factor_id)]&&
        lastModelChanged.factor[String(item.factor_id)].changedAssociations&&
        lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_ontologyAnnotation_ids&&
        (lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_ontologyAnnotation_ids.added ||
         lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_ontologyAnnotation_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_ontologyAnnotation_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_ontologyAnnotation_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'factor_ontologyAnnotation_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.ontologyAnnotation) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.factor_ontologyAnnotation_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.factor_ontologyAnnotation_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.factor_ontologyAnnotation_ids.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.factor_id);
                if(iof !== -1) {
            ...
            if(entry[1].changedAssociations.factor_ontologyAnnotation_ids.added) {
              let idsAdded = entry[1].changedAssociations.factor_ontologyAnnotation_ids.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.factor_id);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.factor_id, handleUntransfer, getCount, count, getCountB, countB, configurePagination, configurePaginationB]);
