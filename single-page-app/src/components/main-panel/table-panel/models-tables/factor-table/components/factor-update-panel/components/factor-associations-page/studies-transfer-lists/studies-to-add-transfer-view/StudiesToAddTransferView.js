...
export default function StudiesToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getNotAssociatedStudiesCount
    */
    let cancelableApiReq = makeCancelable(api.factor.getNotAssociatedStudiesCount(graphqlServerUrl, item.factor_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'factor', association: 'studies', table:'A', method: 'getCount()', request: 'api.factor.getNotAssociatedStudiesCount'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'studies'];
      ...
      //error
      .catch((err) => { //error: on api.factor.getNotAssociatedStudiesCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getNotAssociatedStudies
    */
    let cancelableApiReq = makeCancelable(api.factor.getNotAssociatedStudies(
      graphqlServerUrl,
      item.factor_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'studies', table:'A', method: 'getData()', request: 'api.factor.getNotAssociatedStudies'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.factor.getNotAssociatedStudies
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'studies', table:'B', method: 'getCountB()', request: 'api.study.getCountItems'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'studies'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'studies', table:'B', method: 'getDataB()', request: 'api.study.getItems'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'studies'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.factor&&
        ...
        lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_study_ids&&
        (lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_study_ids.added ||
         lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_study_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_study_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_study_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'factor_study_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.study) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.factor_study_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.factor_study_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.factor_study_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.factor_study_ids.added) {
              let idsAdded = entry[1].changedAssociations.factor_study_ids.idsAdded;
              if(idsAdded) {
