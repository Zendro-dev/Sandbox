...
import FactorUpdatePanel from '../factor-update-panel/FactorUpdatePanel'
import FactorDeleteConfirmationDialog from '../FactorDeleteConfirmationDialog'
...
export default function FactorDetailPanel(props) {
      ...
      lastModelChanged['factor'][String(item['factor_id'])]) {
        ...
        if(lastModelChanged['factor'][String(item['factor_id'])].op === "update"&&
            lastModelChanged['factor'][String(item['factor_id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['factor'][String(item['factor_id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('factor', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('factor', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='FactorDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Factor | factor_id: " + itemState['factor_id'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='FactorDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='FactorDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
