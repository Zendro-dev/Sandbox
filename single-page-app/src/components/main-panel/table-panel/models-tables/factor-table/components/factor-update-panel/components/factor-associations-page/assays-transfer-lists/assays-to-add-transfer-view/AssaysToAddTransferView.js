...
export default function AssaysToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getNotAssociatedAssaysCount
    */
    let cancelableApiReq = makeCancelable(api.factor.getNotAssociatedAssaysCount(graphqlServerUrl, item.factor_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'factor', association: 'assays', table:'A', method: 'getCount()', request: 'api.factor.getNotAssociatedAssaysCount'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'assays'];
      ...
      //error
      .catch((err) => { //error: on api.factor.getNotAssociatedAssaysCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getNotAssociatedAssays
    */
    let cancelableApiReq = makeCancelable(api.factor.getNotAssociatedAssays(
      graphqlServerUrl,
      item.factor_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'assays', table:'A', method: 'getData()', request: 'api.factor.getNotAssociatedAssays'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.factor.getNotAssociatedAssays
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'assays', table:'B', method: 'getCountB()', request: 'api.assay.getCountItems'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'assays'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'assays', table:'B', method: 'getDataB()', request: 'api.assay.getItems'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'assays'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.factor&&
        ...
        lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_assay_ids&&
        (lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_assay_ids.added ||
         lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_assay_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_assay_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_assay_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'factor_assay_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.assay) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.factor_assay_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.factor_assay_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.factor_assay_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.factor_assay_ids.added) {
              let idsAdded = entry[1].changedAssociations.factor_assay_ids.idsAdded;
              if(idsAdded) {
