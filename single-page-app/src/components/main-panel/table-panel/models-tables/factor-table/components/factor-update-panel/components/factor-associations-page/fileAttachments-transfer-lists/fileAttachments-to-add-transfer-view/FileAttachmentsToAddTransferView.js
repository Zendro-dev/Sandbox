...
export default function FileAttachmentsToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getNotAssociatedFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.factor.getNotAssociatedFileAttachmentsCount(graphqlServerUrl, item.factor_id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'factor', association: 'fileAttachments', table:'A', method: 'getCount()', request: 'api.factor.getNotAssociatedFileAttachmentsCount'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.factor.getNotAssociatedFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getNotAssociatedFileAttachments
    */
    let cancelableApiReq = makeCancelable(api.factor.getNotAssociatedFileAttachments(
      graphqlServerUrl,
      item.factor_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'fileAttachments', table:'A', method: 'getData()', request: 'api.factor.getNotAssociatedFileAttachments'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.factor.getNotAssociatedFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'fileAttachments', table:'B', method: 'getCountB()', request: 'api.fileAttachment.getCountItems'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'fileAttachments'];          
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'factor', association: 'fileAttachments', table:'B', method: 'getDataB()', request: 'api.fileAttachment.getItems'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'add', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
    ...
    if(lastModelChanged.factor&&
        ...
        lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_fileAttachment_ids&&
        (lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_fileAttachment_ids.added ||
         lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_fileAttachment_ids.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_fileAttachment_ids.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.factor[String(item.factor_id)].changedAssociations.factor_fileAttachment_ids.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'factor_fileAttachment_ids'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.fileAttachment) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.factor_fileAttachment_ids) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.factor_fileAttachment_ids.removed) {
              let idsRemoved = entry[1].changedAssociations.factor_fileAttachment_ids.idsRemoved;
              if(idsRemoved) {
            ...
            if(entry[1].changedAssociations.factor_fileAttachment_ids.added) {
              let idsAdded = entry[1].changedAssociations.factor_fileAttachment_ids.idsAdded;
              if(idsAdded) {
