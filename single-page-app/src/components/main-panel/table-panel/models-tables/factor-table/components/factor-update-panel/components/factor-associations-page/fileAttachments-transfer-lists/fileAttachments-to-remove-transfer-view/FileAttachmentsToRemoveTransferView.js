...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getFileAttachmentsCount
    */
    let cancelableApiReq = makeCancelable(api.factor.getFileAttachmentsCount(
      graphqlServerUrl, 
      item.factor_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'fileAttachments', method: 'getCount()', request: 'api.factor.getFileAttachmentsCount'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'remove', 'fileAttachments'];
      ...
      //error
      .catch((err) => { //error: on api.factor.getFileAttachmentsCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getFileAttachments
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.factor.getFileAttachments(
      graphqlServerUrl, 
      item.factor_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'fileAttachments', method: 'getData()', request: 'api.factor.getFileAttachments'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'remove', 'fileAttachments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.factor.getFileAttachments
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
