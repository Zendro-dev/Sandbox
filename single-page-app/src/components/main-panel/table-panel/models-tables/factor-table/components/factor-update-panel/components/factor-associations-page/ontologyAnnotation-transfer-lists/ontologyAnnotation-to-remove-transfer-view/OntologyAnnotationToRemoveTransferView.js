...
const showMessageB = useCallback((message, withDetail) => {
 ...
 /**
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getOntologyAnnotationCount
    */
    let cancelableApiReq = makeCancelable(api.factor.getOntologyAnnotationCount(
      graphqlServerUrl, 
      item.factor_id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'ontologyAnnotation', method: 'getCount()', request: 'api.factor.getOntologyAnnotationCount'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'remove', 'ontologyAnnotation'];
      ...
      //error
      .catch((err) => { //error: on api.factor.getOntologyAnnotationCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.factor.getOntologyAnnotation
    */
    let variables = {
      pagination: {...paginationRef.current}
    };
    let cancelableApiReq = makeCancelable(api.factor.getOntologyAnnotation(
      graphqlServerUrl, 
      item.factor_id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'ontologyAnnotation', method: 'getData()', request: 'api.factor.getOntologyAnnotation'}];
            newError.path=['update', `factor_id:${item.factor_id}`, 'remove', 'ontologyAnnotation'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.factor.getOntologyAnnotation
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, t, dataTrigger, item.factor_id, search, getCount, configurePagination, onEmptyPage]);
