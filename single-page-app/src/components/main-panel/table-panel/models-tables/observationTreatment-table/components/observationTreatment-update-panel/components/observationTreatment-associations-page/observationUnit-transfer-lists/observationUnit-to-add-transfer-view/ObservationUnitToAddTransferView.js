...
export default function ObservationUnitToAddTransferView(props) {
  ...
  */
  ...
  const getAssociatedItem = useCallback(async () => {
    errorsE.current = [];
    let currentId = item.observationTreatmentDbId;

    /*
      API Request: api.observationTreatment.getObservationUnit
    */
    let api = await loadApi("observationTreatment");
    let cancelableApiReq = makeCancelable(api.observationTreatment.getObservationUnit(graphqlServerUrl, item.observationTreatmentDbId));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationTreatment', association: 'observationUnit', table:'Associated Item', method: 'getAssociatedItem()', request: 'api.observationTreatment.getObservationUnit'}];
            newError.path=['update', `observationTreatmentDbId:${currentId}`, 'add', 'observationUnit'];
      ...
      //error
      .catch((err) => { //error: on api.observationTreatment.getObservationUnit
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessageE, t, item.observationTreatmentDbId]);
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.observationTreatment.getNotAssociatedObservationUnitCount
    */
    let api = await loadApi("observationTreatment");
    let cancelableApiReq = makeCancelable(api.observationTreatment.getNotAssociatedObservationUnitCount(graphqlServerUrl, item.observationTreatmentDbId, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationTreatment', association: 'observationUnit', table:'A', method: 'getCount()', request: 'api.observationTreatment.getNotAssociatedObservationUnitCount'}];
            newError.path=['update', `observationTreatmentDbId:${item.observationTreatmentDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationTreatment.getNotAssociatedObservationUnitCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  }, [graphqlServerUrl, showMessageC, t, item.observationTreatmentDbId, search]);
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.observationTreatment.getNotAssociatedObservationUnit
    */
    let api = await loadApi("observationTreatment");
    let cancelableApiReq = makeCancelable(api.observationTreatment.getNotAssociatedObservationUnit(
      graphqlServerUrl,
      item.observationTreatmentDbId,
      search,
      variables,
      ops,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationTreatment', association: 'observationUnit', table:'A', method: 'getData()', request: 'api.observationTreatment.getNotAssociatedObservationUnit'}];
            newError.path=['update', `observationTreatmentDbId:${item.observationTreatmentDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.observationTreatment.getNotAssociatedObservationUnit
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  }, [graphqlServerUrl, showMessage, clearRequestGetData, getCount, t, item.observationTreatmentDbId, dataTrigger, search, configurePagination, onEmptyPage, getAssociatedItem]);
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationTreatment', association: 'observationUnit', table:'B', method: 'getDataB()', request: 'api.observationUnit.getItems'}];
            newError.path=['update', `observationTreatmentDbId:${item.observationTreatmentDbId}`, 'add', 'observationUnit'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  }, [graphqlServerUrl, showMessageB, clearRequestGetDataB, t, item.observationTreatmentDbId, dataTriggerB, searchB]);
  ...
  useEffect(() => {
     ...
     * - return
     */
    if(lastModelChanged.observationTreatment&&
        lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)]&&
        lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations&&
        lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations.observationTreatment_observationUnitDbId&&
        (lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations.observationTreatment_observationUnitDbId.added ||
         lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations.observationTreatment_observationUnitDbId.removed)) {
          
          //remove any associated internalId from idsToAdd[] & update counts
          let idsAdded = lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations.observationTreatment_observationUnitDbId.idsAdded;
          if(idsAdded) {
          ...
          let idsRemoved = lastModelChanged.observationTreatment[String(item.observationTreatmentDbId)].changedAssociations.observationTreatment_observationUnitDbId.idsRemoved;
          if(idsRemoved) {
            //increment A
     ...
     * but this action happened on the peer relation, identified by 'observationTreatment_observationUnitDbId'.
     * 
     * Conditions:
    ...
    if(lastModelChanged.observationUnit) {
      ...
      oens.forEach( (entry) => {
        if(entry[1].changedAssociations&&
          entry[1].changedAssociations.observationTreatment_observationUnitDbId) {

            //case A: this item was removed from peer relation.
            if(entry[1].changedAssociations.observationTreatment_observationUnitDbId.removed) {
              let idsRemoved = entry[1].changedAssociations.observationTreatment_observationUnitDbId.idsRemoved;
              if(idsRemoved) {
                let iof = idsRemoved.indexOf(item.observationTreatmentDbId);
                if(iof !== -1) {
                  //update idsToRemove
                  if(lidsToRemove.current && lidsToRemove.current.length>0) {
                    handleReassociateItem('observationUnit', lidsToRemove.current[0]);
            ...
            if(entry[1].changedAssociations.observationTreatment_observationUnitDbId.added) {
              let idsAdded = entry[1].changedAssociations.observationTreatment_observationUnitDbId.idsAdded;
              if(idsAdded) {
                let iof = idsAdded.indexOf(item.observationTreatmentDbId);
                if(iof !== -1) {
  ...
  }, [lastModelChanged, lastChangeTimestamp, items, itemsB, item.observationTreatmentDbId, handleUntransfer, getCount, count, configurePagination, associatedItem, handleDisassociateItem, handleReassociateItem]);
