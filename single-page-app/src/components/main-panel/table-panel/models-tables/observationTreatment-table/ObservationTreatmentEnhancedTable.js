...
import ObservationTreatmentEnhancedTableHead from './components/ObservationTreatmentEnhancedTableHead'
import ObservationTreatmentEnhancedTableToolbar from './components/ObservationTreatmentEnhancedTableToolbar'
import ObservationTreatmentUploadFileDialog from './components/ObservationTreatmentUploadFileDialog'
import ObservationTreatmentCursorPagination from './components/ObservationTreatmentCursorPagination'
...
//lazy loading
const ObservationTreatmentCreatePanel = lazy(() => import(/* webpackChunkName: "Create-ObservationTreatment" */ './components/observationTreatment-create-panel/ObservationTreatmentCreatePanel'));
const ObservationTreatmentUpdatePanel = lazy(() => import(/* webpackChunkName: "Update-ObservationTreatment" */ './components/observationTreatment-update-panel/ObservationTreatmentUpdatePanel'));
const ObservationTreatmentDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-ObservationTreatment" */ './components/observationTreatment-detail-panel/ObservationTreatmentDetailPanel'));
const ObservationTreatmentDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Delete-ObservationTreatment" */ './components/ObservationTreatmentDeleteConfirmationDialog'));
//Plotly
const ObservationTreatmentPlotly = lazy(() => import(/* webpackChunkName: "Plotly-ObservationTreatment" */ '../../../../plots/ObservationTreatmentPlotly'));
...
export default function ObservationTreatmentEnhancedTable(props) {
  ...
  const [orderBy, setOrderBy] = useState('observationTreatmentDbId');
  ...
  const getCount = useCallback(async () => {
    ...
    */
    ...
    let cancelableApiReq = makeCancelable(api.observationTreatment.getCountItems(graphqlServerUrl, search));
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationTreatment', method: 'getCount()', request: 'api.observationTreatment.getCountItems'}];
            newError.path=['ObservationTreatments'];
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationTreatment', method: 'getData()', request: 'api.observationTreatment.getItems'}];
            newError.path=['ObservationTreatments'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
  ...
  useEffect(() => {
     ...
     */
    if(isNewChange&&!changes.changesCompleted&&changes.lastModelChanged&&changes.lastModelChanged['observationTreatment']
    &&(Object.keys(changes.lastModelChanged['observationTreatment']).length===1)
    &&(changes.lastModelChanged['observationTreatment'][Object.keys(changes.lastModelChanged['observationTreatment'])[0]].op === "delete")
    ) {
      //decrement count
      setCount(count-1);
      //count
  ...
  async function doDelete(event, item) {
    ...
    let variables = {};
    //set observationTreatmentDbId (internalId)
    variables.observationTreatmentDbId = item.observationTreatmentDbId;

    /*
      API Request: api.observationTreatment.deleteItem
    */
    let api = await loadApi("observationTreatment");
    let cancelableApiReq = makeCancelable(api.observationTreatment.deleteItem(graphqlServerUrl, variables));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationTreatment', method: 'doDelete()', request: 'api.observationTreatment.deleteItem'}];
            newError.path=['ObservationTreatments', `observationTreatmentDbId:${item.observationTreatmentDbId}`, 'delete'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
      ...
      //error
      .catch((err) => { //error: on api.observationTreatment.deleteItem
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  async function getCsvTemplate() {
    ...
    /*
      API Request: api.observationTreatment.tableTemplate
    */
    let api = await loadApi("observationTreatment");
    let cancelableApiReq = makeCancelable(api.observationTreatment.tableTemplate(graphqlServerUrl));
    cancelablePromises.current.push(cancelableApiReq);
    await cancelableApiReq
      .promise
      .then(
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            variant.current='info';
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'observationTreatment', method: 'getCsvTemplate()', request: 'api.observationTreatment.tableTemplate'}];
            newError.path=['ObservationTreatments', 'csvTemplate'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
            errors.current.push(newError);
            console.log("Error: ", newError);
        ...
        //download
        let file = response.data.data.csvTableTemplateObservationTreatment.join("\n");
        const url = window.URL.createObjectURL(new Blob([file]));
        const link = document.createElement("a");
        link.href = url;
        link.setAttribute("download", "observationTreatment-template.csv");
      ...
      //error
      .catch((err) => { //error: on api.observationTreatment.tableTemplate
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
  ...
  return (
    <div className={classes.root}>
          ...
          <Grid container justify='center'>
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {/* Toolbar */}
                <ObservationTreatmentEnhancedTableToolbar
                ...
                >
                  ...
                  */}
                  <div>
                    ...
                    <div className={classes.tableWrapper} ref={twref}>
                      ...
                      >
                        <Box
                          id='ObservationTreatmentEnhancedTable-box-noData'
                          className={classes.tableBackdrop}
                          bgcolor='rgba(255, 255, 255, 0.0)'
                      ...
                      {/* Table */}
                      <Table id='ObservationTreatmentEnhancedTable-table' stickyHeader size='small' ref={tref}>

                        {/* Table Head */}
                        <ObservationTreatmentEnhancedTableHead
                          permissions={permissions}
                          order={order}
                          orderBy={orderBy}
                        ...
                        <Fade
                          in={(!isOnApiRequest && items.length > 0)}
                        >
                          <TableBody id='ObservationTreatmentEnhancedTable-tableBody'>
                            {
                              items.map((item, index) => {
                                return ([
                                  ...
                                  */
                                  <TableRow
                                    id={'ObservationTreatmentEnhancedTable-row-'+item.observationTreatmentDbId}
                                    hover
                                    onClick={event => handleClickOnRow(event, item)}
                                    role="checkbox"
                                    tabIndex={-1}
                                    key={item.observationTreatmentDbId}
                                  >
                                    ...
                                    <TableCell padding="checkbox">
                                      <Tooltip title={ t('modelPanels.viewDetails') }>
                                        <IconButton
                                          id={'ObservationTreatmentEnhancedTable-row-iconButton-detail-'+item.observationTreatmentDbId}
                                          color="default"
                                          onClick={event => {
                                    ...
                                    */}
                                    {
                                      /* acl check */
                                      (permissions&&permissions.observationTreatment&&Array.isArray(permissions.observationTreatment)
                                      &&(permissions.observationTreatment.includes('update') || permissions.observationTreatment.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.edit') }>
                                            <IconButton
                                              id={'ObservationTreatmentEnhancedTable-row-iconButton-edit-'+item.observationTreatmentDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
...

                                    {
                                      /* acl check */
                                      (permissions&&permissions.observationTreatment&&Array.isArray(permissions.observationTreatment)
                                      &&(permissions.observationTreatment.includes('delete') || permissions.observationTreatment.includes('*')))
                                      &&(
                                        <TableCell padding='checkbox' align='center'>
                                          <Tooltip title={ t('modelPanels.delete') }>
                                            <IconButton
                                              id={'ObservationTreatmentEnhancedTable-row-iconButton-delete-'+item.observationTreatmentDbId}
                                              color="default"
                                              onClick={(event) => {
                                                event.stopPropagation();
                                    ...
                                    <TableCell
                                      ...
                                      padding="checkbox"
                                    >
                                      <Tooltip title={ 'observationTreatmentDbId: ' + item.observationTreatmentDbId}>
                                        <Typography variant='body2' color='textSecondary' display='block' noWrap={true}>{item.observationTreatmentDbId}</Typography>
                                      </Tooltip>
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.factor!==null)?item.factor:'')}
                                    </TableCell>
                                    ...
                                    <TableCell
                                      ...
                                      padding="default"
                                    >
                                      {String((item.modality!==null)?item.modality:'')}
                                    </TableCell>
                    ...
                    {/*
                      Pagination
                    */}
                    <ObservationTreatmentCursorPagination
                      count={count}
                  ...
                  */}
                  <div>
                    {(Boolean(ObservationTreatmentPlotly)) && (
                      <Suspense fallback={<div />}>
                        <ObservationTreatmentPlotly />
                      </Suspense>
                    )}
                  </div>
      ...
      {(createDialogOpen) && (
        <Suspense fallback={<div />}>
          <ObservationTreatmentCreatePanel
            permissions={permissions}
      ...
      {(updateDialogOpen) && (
        <Suspense fallback={<div />}>
          <ObservationTreatmentUpdatePanel
            permissions={permissions}
            item={updateItem}
      ...
      {(deleteConfirmationDialogOpen) && (
        <Suspense fallback={<div />}>
          <ObservationTreatmentDeleteConfirmationDialog
            permissions={permissions}
      ...
      {(uploadFileDialogOpen) && (
        <ObservationTreatmentUploadFileDialog
...
ObservationTreatmentEnhancedTable.propTypes = {
