...
export default function PasswordField(props) {
  ...
  useEffect(() => {
    ...
    if(lastModelChanged&&
      ...
      lastModelChanged['role_to_user'][String(itemId['id'])]) {

        //updated password
        if(lastModelChanged['role_to_user'][String(itemId['id'])].op === "updatePassword"&&
            lastModelChanged['role_to_user'][String(itemId['id'])].newItem) {
              //update password value
              setValue(lastModelChanged['role_to_user'][String(itemId['id'])].newItem[itemKey]);
        }
    }//end: Case 1
