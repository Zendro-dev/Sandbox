...
import RolesCompactViewToolbar from './components/RolesCompactViewToolbar';
import RolesCompactViewCursorPagination from './components/RolesCompactViewCursorPagination';
...
export default function RolesCompactView(props) {
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.user.getRolesCount
    */
    let api = await loadApi("user");
    let cancelableApiReq = makeCancelable(api.user.getRolesCount(
      graphqlServerUrl, 
      item.id,
      search,
    ));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{association: 'roles', method: 'getCount()', request: 'api.user.getRolesCount'}];
            newError.path=['detail', `id:${item.id}`, 'roles'];
      ...
      //error
      .catch((err) => { //error: on api.user.getRolesCount
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  const getData = useCallback(async () => {
    ...
    */
    let api = await loadApi("user");
    let cancelableApiReq = makeCancelable(api.user.getRoles(
      graphqlServerUrl, 
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{association: 'roles', method: 'getData()', request: 'api.user.getRoles'}];
            newError.path=['detail', `id:${item.id}`, 'roles'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.user.getRoles
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
  ...
  useEffect(() => {
     ...
     * 
     * Conditions:
     * A: the current item 'internalId' attribute is in the removedIds of the updated 'role'.
     * B: the current item 'internalId' attribute is in the addedIds of the updated 'role'.
     * 
     * Actions:
     * if A:
     * - reload table.
            ...
            <Card className={classes.card}>
              ...
              {(!isOnApiRequest && (!areItemsReady)) && (
                ...
                <Fade
                  ...
                  unmountOnExit
                >
                  <div id='RolesCompactView-div-noDataA'>
                    <Grid container>
                      <Grid item xs={12}>
              ...
              {(!isOnApiRequest && areItemsReady) && (
                ...
                >
                  <Box className={classes.listBox} ref={lref}>
                    <List id='RolesCompactView-list-listA'
                    dense component="div" role="list" >
                      {items.map(it => {
                        ...
                        return (
                          <ListItem 
                            id={'RolesCompactView-listA-listItem-'+it.id}
                            key={key} 
                            role="listitem" 
                            button 
                                ...
                                <React.Fragment>
                                  ...
                                  <Grid container alignItems='center' alignContent='center' wrap='nowrap' spacing={1}>
                                    <Grid item>
                                      <Tooltip title={ 'id' }>
                                        <Typography
                                        id={'RolesCompactView-listA-listItem-id-'+it.id}
                              ...
                              secondary={
                                <React.Fragment>
                                  ...
                                  {(label) && (
                                    <Tooltip title={ 'name' }>
                                      <Typography
                                      id={'RolesCompactView-listA-listItem-label-'+it.id}
                                  ...
                                  {(sublabel) && (
                                    <Tooltip title={ 'id' }>
                                      <Typography
                                      id={'RolesCompactView-listA-listItem-sublabel-'+it.id}
              ...
              {/* Pagination */}
              <RolesCompactViewCursorPagination
                count={count}
...
RolesCompactView.propTypes = {
  item: PropTypes.object.isRequired,
  handleClickOnRolesRow: PropTypes.func,
};