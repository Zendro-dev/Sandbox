...
//lazy loading
const UserAttributesPage = lazy(() => import(/* webpackChunkName: "Detail-Attributes-User" */ './components/user-attributes-page/UserAttributesPage'));
const UserAssociationsPage = lazy(() => import(/* webpackChunkName: "Detail-Associations-User" */ './components/user-associations-page/UserAssociationsPage'));
const UserUpdatePanel = lazy(() => import(/* webpackChunkName: "Detail-Update-User" */ '../user-update-panel/UserUpdatePanel'));
const UserDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Detail-Delete-User" */ '../UserDeleteConfirmationDialog'));
const RoleDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Detail-Role" */ '../../../role-table/components/role-detail-panel/RoleDetailPanel'));
...
export default function UserDetailPanel(props) {
      ...
      lastModelChanged['user'][String(item['id'])]) {
        ...
        if(lastModelChanged['user'][String(item['id'])].op === "update"&&
            lastModelChanged['user'][String(item['id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['user'][String(item['id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
        ...
        if(lastModelChanged['user'][String(item['id'])].op === "updatePassword"
          &&lastModelChanged['user'][String(item['id'])].newItem){
            //replace item
            setItemState(lastModelChanged['user'][String(item['id'])].newItem);
        } 
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('user', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('user', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='UserDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": User | id: " + itemState['id'] }
              </Typography>
              {/*
                Actions:
                - Edit
