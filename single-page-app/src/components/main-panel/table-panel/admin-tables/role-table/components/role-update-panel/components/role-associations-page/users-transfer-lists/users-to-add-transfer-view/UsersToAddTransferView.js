...
export default function UsersToAddTransferView(props) {
  ...
  */
  ...
  const getCount = useCallback(async () => {
    ...
    /*
      API Request: api.role.getNotAssociatedUsersCount
    */
    let cancelableApiReq = makeCancelable(api.role.getNotAssociatedUsersCount(graphqlServerUrl, item.id, search, ops));
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'role', association: 'users', table:'A', method: 'getCount()', request: 'api.role.getNotAssociatedUsersCount'}];
            newError.path=['update', `id:${item.id}`, 'add', 'users'];
      ...
      //error
      .catch((err) => { //error: on api.role.getNotAssociatedUsersCount
        if(err.isCanceled) {
          return;
        } else {
          //show error
  ...
  const getData = useCallback(async () => {
    ...
    /*
      API Request: api.role.getNotAssociatedUsers
    */
    let cancelableApiReq = makeCancelable(api.role.getNotAssociatedUsers(
      graphqlServerUrl,
      item.id,
      search,
      variables,
    ...
    await cancelableApiReq
      ...
      (response) => {
        ...
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.message = t('modelPanels.errors.data.e3', 'fetched with errors.');
            newError.locations=[{model: 'role', association: 'users', table:'A', method: 'getData()', request: 'api.role.getNotAssociatedUsers'}];
            newError.path=['update', `id:${item.id}`, 'add', 'users'];
            newError.extensions = {graphQL:{data:response.data, errors:response.graphqlErrors}};
      ...
      //error
      .catch((err) => { //error: on api.role.getNotAssociatedUsers
        if(err.isCanceled) {
          return;
        } else {
          let newError = {};
    ...
    if(lastModelChanged.role&&
        ...
        (lastModelChanged.role[String(item.id)].changedAssociations.role_to_user.added ||
         lastModelChanged.role[String(item.id)].changedAssociations.role_to_user.removed)) {
          ...
          let idsAdded = lastModelChanged.role[String(item.id)].changedAssociations.role_to_user.idsAdded;
          if(idsAdded) {
