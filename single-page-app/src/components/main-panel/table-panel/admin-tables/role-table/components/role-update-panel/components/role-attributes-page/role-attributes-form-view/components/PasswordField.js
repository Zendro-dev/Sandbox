    ...
    return await api.role.updateItem(graphqlServerUrl, variables)
      ...
      (response) => {
        ...
        if(!response.data ||!response.data.data) {
          ...
          newError.locations=[{model: 'role', query: 'updateRole', method: 'doSave()', request: 'api.role.updateItem'}];
        ...
        //check: updateRole
        let updateRole = response.data.data.updateRole;
        if(updateRole === null) {
          let newError = {};
          let withDetails=true;
          variant.current='error';
          newError.message = 'updateRole ' + t('modelPanels.errors.data.e5', 'could not be completed.');
          newError.locations=[{model: 'role', query: 'updateRole', method: 'doSave()', request: 'api.role.updateItem'}];
          newError.path=['Roles', itemId, 'update', 'change password'];
          newError.extensions = {graphqlResponse:{data:response.data.data, errors:response.data.errors}};
          errors.current.push(newError);
        ...
        //check: updateRole type
        if(typeof updateRole !== 'object') {
          let newError = {};
          let withDetails=true;
          variant.current='error';
        ...
        if(response.data.errors) {
          ...
          newError.message = 'updateRole ' + t('modelPanels.errors.data.e6', 'completed with errors.');
          newError.locations=[{model: 'role', query: 'updateRole', method: 'doSave()', request: 'api.role.updateItem'}];
          newError.path=['Roles', itemId, 'update', 'change password'];
  ...
  return (
    <div>
                ...
                endAdornment:
                  <InputAdornment position="end">
                    <Tooltip title={ t('modelPanels.showPassword', 'Show password') }>
                      <IconButton
                        id={'StringField-Role-button-showPassword'+name}
      ...
      {(changePasswordDialogOpen) && (
        <Dialog 
          id={'StringField-Role-ChangePasswordDialog-'+name} 
          ...
          <DialogContent>
            ...
            <form noValidate autoComplete="off">
              <Grid container justify='center' alignItems='center' spacing={2}>
                <Grid item xs={12}>
                  <TextField
                    id={'StringField-Role-ChangePasswordDialog-textField-newPassword-'+name}
                ...
                <Grid item xs={12}>
                  <TextField
                    id={'StringField-Role-ChangePasswordDialog-textField-newPassword2-'+name}
