...
//lazy loading
const RoleAttributesPage = lazy(() => import(/* webpackChunkName: "Detail-Attributes-Role" */ './components/role-attributes-page/RoleAttributesPage'));
const RoleAssociationsPage = lazy(() => import(/* webpackChunkName: "Detail-Associations-Role" */ './components/role-associations-page/RoleAssociationsPage'));
const RoleUpdatePanel = lazy(() => import(/* webpackChunkName: "Detail-Update-Role" */ '../role-update-panel/RoleUpdatePanel'));
const RoleDeleteConfirmationDialog = lazy(() => import(/* webpackChunkName: "Detail-Delete-Role" */ '../RoleDeleteConfirmationDialog'));
const UserDetailPanel = lazy(() => import(/* webpackChunkName: "Detail-Detail-User" */ '../../../user-table/components/user-detail-panel/UserDetailPanel'));
...
export default function RoleDetailPanel(props) {
      ...
      lastModelChanged['role'][String(item['id'])]) {
        ...
        if(lastModelChanged['role'][String(item['id'])].op === "update"&&
            lastModelChanged['role'][String(item['id'])].newItem) {
              //replace item
              setItemState(lastModelChanged['role'][String(item['id'])].newItem);
              //show alert
              setUpdated(true);
        } else {
          //deleted item
  ...
  const handleUpdateDialogClose = (event, status, item, newItem, changedAssociations) => {
    if(status) {
      dispatch(modelChange('role', 'update', item, newItem, changedAssociations))
    }
  ...
  const onSuccessDelete = (event, item) => {
    dispatch(modelChange('role', 'delete', item, null))
          ...
          <AppBar className={classes.appBar}>
            <Toolbar>
              <Tooltip title={ t('modelPanels.close') }>
                <IconButton
                  id='RoleDetailPanel-button-close'
                  edge="start"
                  color="inherit"
                  onClick={(event) => {
                    if(!isCanceling.current){
              ...
              <Typography variant="h6" className={classes.title}>
                { t('modelPanels.detailOf') +  ": Role | id: " + itemState['id'] }
              </Typography>
              {/*
                Actions:
                - Edit
              ...
              */}
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.edit') }>
                      <IconButton
                        id='RoleDetailPanel-button-edit'
                        color='inherit'
                        onClick={(event) => {
              ...
              }
              {
                ...
                &&(!deleted)&&(
                    ...
                    <Tooltip title={ t('modelPanels.delete') }>
                      <IconButton
                        id='RoleDetailPanel-button-delete'
                        color='inherit'
                        onClick={(event) => {
                          event.stopPropagation();
