...
export default function PersonPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.person.getBarchartData
    */
    await api.person.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'Person', method: 'getBarchartData()', request: 'api.person.getBarchartData'}];
            newError.path=['People', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.person.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
