...
export default function EventParameterPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.eventParameter.getBarchartData
    */
    let api = await loadApi("eventParameter");
    await api.eventParameter.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'eventParameter', method: 'getBarchartData()', request: 'api.eventParameter.getBarchartData'}];
            newError.path=['EventParameters', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.eventParameter.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
