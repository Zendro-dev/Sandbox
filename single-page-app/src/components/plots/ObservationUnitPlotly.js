...
export default function ObservationUnitPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.observationUnit.getBarchartData
    */
    let api = await loadApi("observationUnit");
    await api.observationUnit.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnit', method: 'getBarchartData()', request: 'api.observationUnit.getBarchartData'}];
            newError.path=['ObservationUnits', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnit.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
