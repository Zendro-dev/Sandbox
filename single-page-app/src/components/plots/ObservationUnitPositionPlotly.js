...
export default function ObservationUnitPositionPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.observationUnitPosition.getBarchartData
    */
    let api = await loadApi("observationUnitPosition");
    await api.observationUnitPosition.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationUnitPosition', method: 'getBarchartData()', request: 'api.observationUnitPosition.getBarchartData'}];
            newError.path=['ObservationUnitPositions', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.observationUnitPosition.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
