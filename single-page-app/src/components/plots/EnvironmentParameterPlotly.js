...
export default function EnvironmentParameterPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.environmentParameter.getBarchartData
    */
    let api = await loadApi("environmentParameter");
    await api.environmentParameter.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'environmentParameter', method: 'getBarchartData()', request: 'api.environmentParameter.getBarchartData'}];
            newError.path=['EnvironmentParameters', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.environmentParameter.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
