...
export default function OntologyReferencePlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.ontologyReference.getBarchartData
    */
    let api = await loadApi("ontologyReference");
    await api.ontologyReference.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyReference', method: 'getBarchartData()', request: 'api.ontologyReference.getBarchartData'}];
            newError.path=['OntologyReferences', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyReference.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
