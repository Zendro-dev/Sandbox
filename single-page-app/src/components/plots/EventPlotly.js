...
export default function EventPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.event.getBarchartData
    */
    let api = await loadApi("event");
    await api.event.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'event', method: 'getBarchartData()', request: 'api.event.getBarchartData'}];
            newError.path=['Events', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.event.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
