...
export default function RolePlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.role.getBarchartData
    */
    let api = await loadApi("role");
    await api.role.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'role', method: 'getBarchartData()', request: 'api.role.getBarchartData'}];
      ...
      //error
      .catch((err) => { //error: on api.role.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
