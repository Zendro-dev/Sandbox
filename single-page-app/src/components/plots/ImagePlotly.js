...
export default function ImagePlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.image.getBarchartData
    */
    let api = await loadApi("image");
    await api.image.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'image', method: 'getBarchartData()', request: 'api.image.getBarchartData'}];
            newError.path=['Images', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.image.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
