...
export default function ObservationTreatmentPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.observationTreatment.getBarchartData
    */
    let api = await loadApi("observationTreatment");
    await api.observationTreatment.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationTreatment', method: 'getBarchartData()', request: 'api.observationTreatment.getBarchartData'}];
            newError.path=['ObservationTreatments', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.observationTreatment.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
