...
export default function ProgramPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.program.getBarchartData
    */
    let api = await loadApi("program");
    await api.program.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'program', method: 'getBarchartData()', request: 'api.program.getBarchartData'}];
            newError.path=['Programs', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.program.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
