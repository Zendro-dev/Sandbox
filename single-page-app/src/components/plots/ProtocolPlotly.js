...
export default function ProtocolPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.protocol.getBarchartData
    */
    await api.protocol.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'protocol', method: 'getBarchartData()', request: 'api.protocol.getBarchartData'}];
            newError.path=['Protocols', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.protocol.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
