...
export default function TraitPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.trait.getBarchartData
    */
    let api = await loadApi("trait");
    await api.trait.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'trait', method: 'getBarchartData()', request: 'api.trait.getBarchartData'}];
            newError.path=['Traits', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.trait.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
