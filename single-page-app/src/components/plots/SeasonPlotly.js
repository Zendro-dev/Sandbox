...
export default function SeasonPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.season.getBarchartData
    */
    let api = await loadApi("season");
    await api.season.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'season', method: 'getBarchartData()', request: 'api.season.getBarchartData'}];
            newError.path=['Seasons', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.season.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
