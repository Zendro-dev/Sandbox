...
export default function RoleToUserPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.role_to_user.getBarchartData
    */
    let api = await loadApi("role_to_user");
    await api.role_to_user.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'role_to_user', method: 'getBarchartData()', request: 'api.role_to_user.getBarchartData'}];
      ...
      //error
      .catch((err) => { //error: on api.role_to_user.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
