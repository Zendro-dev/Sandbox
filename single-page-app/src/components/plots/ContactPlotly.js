...
export default function ContactPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.contact.getBarchartData
    */
    await api.contact.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'contact', method: 'getBarchartData()', request: 'api.contact.getBarchartData'}];
            newError.path=['Contacts', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.contact.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
