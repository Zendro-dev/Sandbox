...
export default function AssayResultPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.assayResult.getBarchartData
    */
    await api.assayResult.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assayResult', method: 'getBarchartData()', request: 'api.assayResult.getBarchartData'}];
            newError.path=['AssayResults', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.assayResult.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
