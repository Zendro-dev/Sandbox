...
export default function ObservationPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.observation.getBarchartData
    */
    let api = await loadApi("observation");
    await api.observation.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observation', method: 'getBarchartData()', request: 'api.observation.getBarchartData'}];
            newError.path=['Observations', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.observation.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
