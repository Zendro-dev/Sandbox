...
export default function TrialPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.trial.getBarchartData
    */
    let api = await loadApi("trial");
    await api.trial.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'trial', method: 'getBarchartData()', request: 'api.trial.getBarchartData'}];
            newError.path=['Trials', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.trial.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
