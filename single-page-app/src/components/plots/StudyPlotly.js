...
export default function StudyPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.study.getBarchartData
    */
    let api = await loadApi("study");
    await api.study.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'study', method: 'getBarchartData()', request: 'api.study.getBarchartData'}];
            newError.path=['Studies', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.study.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
