...
export default function ScalePlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.scale.getBarchartData
    */
    let api = await loadApi("scale");
    await api.scale.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'scale', method: 'getBarchartData()', request: 'api.scale.getBarchartData'}];
            newError.path=['Scales', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.scale.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
