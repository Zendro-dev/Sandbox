...
export default function FileAttachmentPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.fileAttachment.getBarchartData
    */
    await api.fileAttachment.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'fileAttachment', method: 'getBarchartData()', request: 'api.fileAttachment.getBarchartData'}];
            newError.path=['FileAttachments', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.fileAttachment.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
