...
export default function MaterialPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.material.getBarchartData
    */
    await api.material.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'material', method: 'getBarchartData()', request: 'api.material.getBarchartData'}];
            newError.path=['Materials', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.material.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
