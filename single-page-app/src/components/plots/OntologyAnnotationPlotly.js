...
export default function OntologyAnnotationPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.ontologyAnnotation.getBarchartData
    */
    await api.ontologyAnnotation.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ontologyAnnotation', method: 'getBarchartData()', request: 'api.ontologyAnnotation.getBarchartData'}];
            newError.path=['OntologyAnnotations', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.ontologyAnnotation.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
