...
export default function ObservationVariablePlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.observationVariable.getBarchartData
    */
    let api = await loadApi("observationVariable");
    await api.observationVariable.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'observationVariable', method: 'getBarchartData()', request: 'api.observationVariable.getBarchartData'}];
            newError.path=['ObservationVariables', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.observationVariable.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
