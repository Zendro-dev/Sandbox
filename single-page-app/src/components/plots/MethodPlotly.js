...
export default function MethodPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.method.getBarchartData
    */
    let api = await loadApi("method");
    await api.method.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'method', method: 'getBarchartData()', request: 'api.method.getBarchartData'}];
            newError.path=['Methods', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.method.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
