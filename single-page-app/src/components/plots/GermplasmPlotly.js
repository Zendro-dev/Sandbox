...
export default function GermplasmPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.germplasm.getBarchartData
    */
    let api = await loadApi("germplasm");
    await api.germplasm.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'germplasm', method: 'getBarchartData()', request: 'api.germplasm.getBarchartData'}];
            newError.path=['Germplasms', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.germplasm.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
