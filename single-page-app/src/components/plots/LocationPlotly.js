...
export default function LocationPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.location.getBarchartData
    */
    let api = await loadApi("location");
    await api.location.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'location', method: 'getBarchartData()', request: 'api.location.getBarchartData'}];
            newError.path=['Locations', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.location.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
