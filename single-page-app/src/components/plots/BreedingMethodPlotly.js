...
export default function BreedingMethodPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.breedingMethod.getBarchartData
    */
    let api = await loadApi("breedingMethod");
    await api.breedingMethod.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'breedingMethod', method: 'getBarchartData()', request: 'api.breedingMethod.getBarchartData'}];
            newError.path=['BreedingMethods', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.breedingMethod.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
