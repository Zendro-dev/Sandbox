...
export default function InvestigationPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.investigation.getBarchartData
    */
    await api.investigation.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'investigation', method: 'getBarchartData()', request: 'api.investigation.getBarchartData'}];
            newError.path=['Investigations', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.investigation.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
