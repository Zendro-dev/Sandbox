...
export default function FactorPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.factor.getBarchartData
    */
    await api.factor.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'factor', method: 'getBarchartData()', request: 'api.factor.getBarchartData'}];
            newError.path=['Factors', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.factor.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
