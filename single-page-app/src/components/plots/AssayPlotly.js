...
export default function AssayPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.assay.getBarchartData
    */
    await api.assay.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'assay', method: 'getBarchartData()', request: 'api.assay.getBarchartData'}];
            newError.path=['Assays', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.assay.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
