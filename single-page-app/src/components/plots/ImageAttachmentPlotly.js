...
export default function ImageAttachmentPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.imageAttachment.getBarchartData
    */
    await api.imageAttachment.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'ImageAttachment', method: 'getBarchartData()', request: 'api.imageAttachment.getBarchartData'}];
            newError.path=['ImageAttachments', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.imageAttachment.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
