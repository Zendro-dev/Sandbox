...
export default function SPARefactorPlotly(props) {
  ...
  */
  async function getBarchartData(attribute) {
    ...
    /*
      API Request: api.sPARefactor.getBarchartData
    */
    ...
    await api.sPARefactor.getBarchartData(graphqlServerUrl, attribute)
      .then(
      //resolved
      (response) => {
        //check: response
        if(response.message === 'ok') {
          ...
          if(response.graphqlErrors) {
            ...
            newError.locations=[{model: 'SPARefactor', method: 'getBarchartData()', request: 'api.sPARefactor.getBarchartData'}];
            newError.path=['SPARefactors', 'Plot'];
      ...
      //error
      .catch((err) => { //error: on api.sPARefactor.getBarchartData
        if(err.isCanceled) {
          return
        } else {
          let newError = {};
