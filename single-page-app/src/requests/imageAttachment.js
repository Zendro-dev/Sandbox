 ...
 */
   ...
   * tableTemplate
   *
   * Get imageAttachmenttable template from GraphQL Server.
   * (root query)
   *
  ...
  async tableTemplate(url) {
    ...
    let headers = null;
    //check
    let check = checkResponse(response, graphqlErrors, "csvTableTemplateImageAttachment");
    if(check === 'ok') {
      //check type
      if(!Array.isArray(response.data.data["csvTableTemplateImageAttachment"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      headers = response.data.data["csvTableTemplateImageAttachment"];
 ...
 */
   ...
   * getCountItems
   *
   * Get imageAttachmentsitems count from GraphQL Server.
   * (root query)
   *
   * @param {String} url GraphQL Server url
...
`;
    ...
    //query
    let query =
      `query imageAttachmentsConnection($order: [orderImageAttachmentInput], $search: searchImageAttachmentInput, $pagination: paginationCursorInput!) { 
             imageAttachmentsConnection( order: $order, search: $search, pagination: $pagination ) {
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "addImageAttachment");
    if(check === 'ok') {
        //check type
        if(!response.data.data["addImageAttachment"]
        || typeof response.data.data["addImageAttachment"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["addImageAttachment"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "updateImageAttachment");
    if(check === 'ok') {
        //check type
        if(!response.data.data["updateImageAttachment"]
        || typeof response.data.data["updateImageAttachment"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["updateImageAttachment"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  async deleteItem(url, variables) {
    ...
    let result = null;
    //check
    let check = checkResponse(response, graphqlErrors, "deleteImageAttachment");
    if(check === 'ok') {
      //check type
      if(response.data.data["deleteImageAttachment"] === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      result = response.data.data["deleteImageAttachment"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getPerson   *
   * Get people records associated to the given imageAttachment record
   * through association 'Person', from GraphQL Server.
   *
   *
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  async getPerson(url, itemId, searchText, variables, ops) {
    let graphqlErrors = [];
...
`;
    ...
    //set query
    let query = 
      `query readOneImageAttachment($id:ID!) {
             readOneImageAttachment(id:$id) {
                person{
                  ${qattributes}
                }
             }}`;
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getPerson', query, variables);
    
    //request
    let response = await requestGraphql({ url, query, variables });
    let associatedItem = null;
    //check
    let check = checkResponse(response, graphqlErrors, "readOneImageAttachment");
    if(check === 'ok') {
      //check types
      if(!response.data.data["readOneImageAttachment"]
      || typeof response.data.data["readOneImageAttachment"] !== 'object'
      || typeof response.data.data["readOneImageAttachment"]["person"] !== 'object' //can be null
      ) return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
      
      //get value
      associatedItem = response.data.data["readOneImageAttachment"]["person"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getNotAssociatedPersonCount
   *
   * Get count of not associated Person from GraphQL Server.
   *
   * @param {String} url GraphQL Server url.
   ...
   * @param {String} ops Object with adittional query options.
   */
  async getNotAssociatedPersonCount(url, itemId, searchText, ops) {
    let graphqlErrors = [];
    /**
     * Algorithm:
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedPersonCount.query1', query, variables);
    //request
    let response = await requestGraphql({ url, query, variables });
    let associatedItem = null;
    //check
   ...
   /**
    ...
    if(s) {
      if(associatedItem) {
        ...
        let f1 = {field: "id", valueType: "Int", value: associatedItem["id"], operator: "ne"};
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedPersonCount.query2', query, variables);
    //request
    response = await requestGraphql({ url, query, variables });
    let count = null;
    //check
    check = checkResponse(response, graphqlErrors, "countPeople");
    if(check === 'ok') {
      //check type
...
/**
 * getNotAssociatedPerson
 *
 * Get not associated Person items from GraphQL Server.
 *
 * @param {String} url GraphQL Server url.
 * @param {String} itemId Model item internalId.
 * @param {String} searchText Text string currently on search bar.
 ...
 */
async getNotAssociatedPerson(url, itemId, searchText, variables, ops, batchSize) {
   //internal checks
   if(!variables||typeof variables !== 'object') throw new Error("internal_error: expected object 'variables' argument");
   if(!variables.pagination||typeof variables.pagination !== 'object' ) throw new Error("internal_error: pagination object expected in variables");
    if(!variables.pagination.first&&!variables.pagination.last ) throw new Error("internal_error: pagination first or last positive argument expected");
...
`;
     ...
     */
    let query = 
      `{  readOneImageAttachment(id: ${itemId}) {
            person{
              id            }
          }
        }`;
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedPerson.query1', query);
    //request
    let response = await requestGraphql({ url, query });
    let associatedItem = null;
    //check
    ...
    //make filter to exclude associated item
    let f1 = null;
    if(associatedItem) f1 = {field: "id", valueType: "Int", value: associatedItem["id"], operator: "ne"};
    ...
    query =
      `query peopleConnection($search: searchPersonInput, $pagination: paginationCursorInput!) {
             peopleConnection(search: $search, pagination: $pagination) {
                  ${qbody},
                },
             }`;
    /**
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedPerson.query2', query, variables);

    //request
    response = await requestGraphql({ url, query, variables });
    let items = null;
    //check
    check = checkResponse(response, graphqlErrors, "peopleConnection");
    if(check === 'ok') {
      //check type
