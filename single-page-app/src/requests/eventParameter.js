 ...
 */
   ...
   * tableTemplate
   *
   * Get eventParametertable template from GraphQL Server.
   * (root query)
   *
  ...
  async tableTemplate(url) {
    ...
    let headers = null;
    //check
    let check = checkResponse(response, graphqlErrors, "csvTableTemplateEventParameter");
    if(check === 'ok') {
      //check type
      if(!Array.isArray(response.data.data["csvTableTemplateEventParameter"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      headers = response.data.data["csvTableTemplateEventParameter"];
 ...
 */
   ...
   * getCountItems
   *
   * Get eventParametersitems count from GraphQL Server.
   * (root query)
   *
   * @param {String} url GraphQL Server url
...
`;
    ...
    //query
    let query =
      `query eventParametersConnection($order: [orderEventParameterInput], $search: searchEventParameterInput, $pagination: paginationCursorInput!) { 
             eventParametersConnection( order: $order, search: $search, pagination: $pagination ) {
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "addEventParameter");
    if(check === 'ok') {
        //check type
        if(!response.data.data["addEventParameter"]
        || typeof response.data.data["addEventParameter"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["addEventParameter"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "updateEventParameter");
    if(check === 'ok') {
        //check type
        if(!response.data.data["updateEventParameter"]
        || typeof response.data.data["updateEventParameter"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["updateEventParameter"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  async deleteItem(url, variables) {
    ...
    let result = null;
    //check
    let check = checkResponse(response, graphqlErrors, "deleteEventParameter");
    if(check === 'ok') {
      //check type
      if(response.data.data["deleteEventParameter"] === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      result = response.data.data["deleteEventParameter"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getEvent   *
   * Get events records associated to the given eventParameter record
   ...
   * @param {String} ops Object with adittional query options.
   */
  async getEvent(url, itemId, searchText, variables, ops) {
    let graphqlErrors = [];
...
`;
    ...
    variables = { "eventParameterDbId": itemId };
    //set query
    let query = 
      `query readOneEventParameter($eventParameterDbId:ID!) {
             readOneEventParameter(eventParameterDbId:$eventParameterDbId) {
                event{
                  ${qattributes}
                }
             }}`;
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getEvent', query, variables);
    
    //request
    let response = await requestGraphql({ url, query, variables });
    let associatedItem = null;
    //check
    let check = checkResponse(response, graphqlErrors, "readOneEventParameter");
    if(check === 'ok') {
      //check types
      if(!response.data.data["readOneEventParameter"]
      || typeof response.data.data["readOneEventParameter"] !== 'object'
      || typeof response.data.data["readOneEventParameter"]["event"] !== 'object' //can be null
      ) return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
      
      //get value
      associatedItem = response.data.data["readOneEventParameter"]["event"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getNotAssociatedEventCount
   *
   * Get count of not associated Event from GraphQL Server.
   *
   * @param {String} url GraphQL Server url.
   ...
   * @param {String} ops Object with adittional query options.
   */
  async getNotAssociatedEventCount(url, itemId, searchText, ops) {
    let graphqlErrors = [];
    /**
     * Algorithm:
     ...
     * 
     */
    let variables = {"eventParameterDbId": itemId};
    let query = 
      `query readOneEventParameter($eventParameterDbId:ID!) {
             readOneEventParameter(eventParameterDbId:$eventParameterDbId) {
                event{
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedEventCount.query1', query, variables);
    //request
    let response = await requestGraphql({ url, query, variables });
    let associatedItem = null;
    //check
   ...
   /**
    ...
    if(s) {
      if(associatedItem) {
        ...
        let f1 = {field: "eventType", valueType: "String", value: associatedItem["eventType"], operator: "ne"};
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedEventCount.query2', query, variables);
    //request
    response = await requestGraphql({ url, query, variables });
    let count = null;
    //check
    check = checkResponse(response, graphqlErrors, "countEvents");
    if(check === 'ok') {
      //check type
...
/**
 * getNotAssociatedEvent
 *
 * Get not associated Event items from GraphQL Server.
 *
 * @param {String} url GraphQL Server url.
 * @param {String} itemId Model item internalId.
 * @param {String} searchText Text string currently on search bar.
 ...
 */
async getNotAssociatedEvent(url, itemId, searchText, variables, ops, batchSize) {
   //internal checks
   if(!variables||typeof variables !== 'object') throw new Error("internal_error: expected object 'variables' argument");
   if(!variables.pagination||typeof variables.pagination !== 'object' ) throw new Error("internal_error: pagination object expected in variables");
    if(!variables.pagination.first&&!variables.pagination.last ) throw new Error("internal_error: pagination first or last positive argument expected");
...
`;
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedEvent.query1', query);
    //request
    let response = await requestGraphql({ url, query });
    let associatedItem = null;
    //check
    ...
    //make filter to exclude associated item
    let f1 = null;
    if(associatedItem) f1 = {field: "eventType", valueType: "String", value: associatedItem["eventType"], operator: "ne"};
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedEvent.query2', query, variables);

    //request
    response = await requestGraphql({ url, query, variables });
    let items = null;
    //check
    check = checkResponse(response, graphqlErrors, "eventsConnection");
    if(check === 'ok') {
      //check type
