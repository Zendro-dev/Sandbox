 ...
 */
  ...
  /**
   * getPrograms   *
   * Get programs records associated to the given contact record
   * through association 'Programs', from GraphQL Server.
   *
   *
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  async getPrograms(url, itemId, searchText, variables, ops) {
    //internal checks
    if(!variables||typeof variables !== 'object') throw new Error("internal_error: expected object 'variables' argument");
    if(!variables.pagination||typeof variables.pagination !== 'object' ) throw new Error("internal_error: pagination object expected in variables");
    if(!variables.pagination.first&&!variables.pagination.last ) throw new Error("internal_error: pagination first or last positive argument expected");
...
`;
    ...
    let query =
      `query readOneContact($contactDbId:ID!, $search: searchProgramInput, $pagination: paginationCursorInput!) {
             readOneContact(contactDbId:$contactDbId) {
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getPrograms', query, variables);
...
//request
    ...
    if(check === 'ok') {
      ...
      || !response.data.data["readOneContact"]["programsConnection"]
      || typeof response.data.data["readOneContact"]["programsConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneContact"]["programsConnection"].edges)
      || typeof response.data.data["readOneContact"]["programsConnection"].pageInfo !== 'object' 
      || response.data.data["readOneContact"]["programsConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneContact"]["programsConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * getProgramsCount
   * 
   * Get programs records count associated to the given contact record
   * through association 'Programs', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  async getProgramsCount(url, itemId, searchText, ops) {
    let graphqlErrors = [];

    let variables = {"contactDbId": itemId};
    //search
    let s = getSearchArgument('program', searchText, ops, 'object');
    if(s) variables.search = s.search;
    //query
    let query =
      `query readOneContact($contactDbId:ID!, $search: searchProgramInput) { 
             readOneContact(contactDbId:$contactDbId) {
              countFilteredPrograms(search: $search) 
       }}`

    /**
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getProgramsCount', query, variables);
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !Number.isInteger(response.data.data["readOneContact"]["countFilteredPrograms"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneContact"]["countFilteredPrograms"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getNotAssociatedProgramsCount
   *
   * Get count of not associated Programs from GraphQL Server.
   *
   * @param {String} url GraphQL Server url.
   ...
   * @param {String} ops Object with adittional query options.
   */
  async getNotAssociatedProgramsCount(url, itemId, searchText, ops) {
    let graphqlErrors = [];
    /**
     * Algorithm:
    ...
    let f1 = {field: "leadPersonDbId", valueType: "String", value: itemId, operator: "ne"};
    let f2 = {field: "leadPersonDbId", valueType: "String", value: null, operator: "eq"};
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedProgramsCount', query, variables);
    //request
    let response = await requestGraphql({ url, query, variables });
    let count = null;
    //check
    let check = checkResponse(response, graphqlErrors, "countPrograms");
    if(check === 'ok') {
      //check type
...
/**
 * getNotAssociatedPrograms
 *
 * Get not associated Programs items from GraphQL Server.
 *
 * @param {String} url GraphQL Server url.
 * @param {String} itemId Model item internalId.
 * @param {String} searchText Text string currently on search bar.
 ...
 */
async getNotAssociatedPrograms(url, itemId, searchText, variables, ops, batchSize) {
   //internal checks
   if(!variables||typeof variables !== 'object') throw new Error("internal_error: expected object 'variables' argument");
   if(!variables.pagination||typeof variables.pagination !== 'object' ) throw new Error("internal_error: pagination object expected in variables");
    if(!variables.pagination.first&&!variables.pagination.last ) throw new Error("internal_error: pagination first or last positive argument expected");
...
`;
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedPrograms', query, variables);

    //request
    let response = await requestGraphql({ url, query, variables });
    let items = null;
    //check
    let check = checkResponse(response, graphqlErrors, "programsConnection");
    if(check === 'ok') {
      //check type
...
`;
    ...
    let query =
      `query readOneContact($contactDbId:ID!, $search: searchStudyInput, $pagination: paginationCursorInput!) {
             readOneContact(contactDbId:$contactDbId) {
 ...
 */
  ...
  async getStudiesCount(url, itemId, searchText, ops) {
    ...
    //query
    let query =
      `query readOneContact($contactDbId:ID!, $search: searchStudyInput) { 
             readOneContact(contactDbId:$contactDbId) {
 ...
 */
  ...
  //make filter to exclude itemId on FK & include null's
  let f1 = {field: "contactDbIds", valueType: "Array", value: itemId, operator: "notIn"};
  let f2 = {field: "contactDbIds", valueType: "String", value: null, operator: "eq"};
  let nf = {operator: "or", search: [ f1, f2 ]};
 ...
 */
  ...
  /**
   * getTrials   *
   * Get trials records associated to the given contact record
   ...
   * @param {String} ops Object with adittional query options.
   */
  async getTrials(url, itemId, searchText, variables, ops) {
    //internal checks
    if(!variables||typeof variables !== 'object') throw new Error("internal_error: expected object 'variables' argument");
    if(!variables.pagination||typeof variables.pagination !== 'object' ) throw new Error("internal_error: pagination object expected in variables");
    if(!variables.pagination.first&&!variables.pagination.last ) throw new Error("internal_error: pagination first or last positive argument expected");
...
`;
    ...
    let query =
      `query readOneContact($contactDbId:ID!, $search: searchTrialInput, $pagination: paginationCursorInput!) {
             readOneContact(contactDbId:$contactDbId) {
     ...
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getTrials', query, variables);
...
//request
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !response.data.data["readOneContact"]["trialsConnection"]
      || typeof response.data.data["readOneContact"]["trialsConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneContact"]["trialsConnection"].edges)
      || typeof response.data.data["readOneContact"]["trialsConnection"].pageInfo !== 'object' 
      || response.data.data["readOneContact"]["trialsConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneContact"]["trialsConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * getTrialsCount
   * 
   * Get trials records count associated to the given contact record
   * through association 'Trials', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
   ...
   * @param {String} ops Object with adittional query options.
   */
  async getTrialsCount(url, itemId, searchText, ops) {
    let graphqlErrors = [];
    ...
    //query
    let query =
      `query readOneContact($contactDbId:ID!, $search: searchTrialInput) { 
             readOneContact(contactDbId:$contactDbId) {
              countFilteredTrials(search: $search) 
       }}`

    /**
     * Debug
     */
    if(globals.REQUEST_LOGGER) logRequest('getTrialsCount', query, variables);
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !Number.isInteger(response.data.data["readOneContact"]["countFilteredTrials"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneContact"]["countFilteredTrials"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getNotAssociatedTrialsCount
   *
   * Get count of not associated Trials from GraphQL Server.
   *
   * @param {String} url GraphQL Server url.
   * @param {String} itemId Model item internalId.
   * @param {String} searchText Text string currently on search bar.
   * @param {String} ops Object with adittional query options.
   */
  async getNotAssociatedTrialsCount(url, itemId, searchText, ops) {
    let graphqlErrors = [];
  /**
   * Algorithm:
   *    1. get a filtered count over all items.
   ...
   * Debug
   */
  if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedTrialsCount', query, variables);
  //request
  let response = await requestGraphql({ url, query, variables });
  let count = null;
  //check
...
/**
 * getNotAssociatedTrials
 *
 * Get not associated Trials items from GraphQL Server.
 *
 * @param {String} url GraphQL Server url.
 * @param {String} itemId Model item internalId.
 * @param {String} searchText Text string currently on search bar.
 ...
 * @param {Int}    batchSize Max number of records to fetch in batch from GraphQL Server.
 */
async getNotAssociatedTrials(url, itemId, searchText, variables, ops, batchSize) {
   //internal checks
   if(!variables||typeof variables !== 'object') throw new Error("internal_error: expected object 'variables' argument");
   if(!variables.pagination||typeof variables.pagination !== 'object' ) throw new Error("internal_error: pagination object expected in variables");
    if(!variables.pagination.first&&!variables.pagination.last ) throw new Error("internal_error: pagination first or last positive argument expected");
...
`;
   ...
   * Debug
   */
  if(globals.REQUEST_LOGGER) logRequest('getNotAssociatedTrials', query, variables);
