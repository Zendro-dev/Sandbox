 ...
 */
   ...
   * tableTemplate
   *
   * Get contacttable template from GraphQL Server.
   * (root query)
   *
  ...
  async tableTemplate(url) {
    ...
    let headers = null;
    //check
    let check = checkResponse(response, graphqlErrors, "csvTableTemplateContact");
    if(check === 'ok') {
      //check type
      if(!Array.isArray(response.data.data["csvTableTemplateContact"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      headers = response.data.data["csvTableTemplateContact"];
 ...
 */
   ...
   * getCountItems
   *
   * Get contactsitems count from GraphQL Server.
   * (root query)
   *
   * @param {String} url GraphQL Server url
...
`;
    ...
    //query
    let query =
      `query contactsConnection($order: [orderContactInput], $search: searchContactInput, $pagination: paginationCursorInput!) { 
             contactsConnection( order: $order, search: $search, pagination: $pagination ) {
 ...
 */
   ...
   * createItem
   *
   * Add new Contact item on GraphQL Server.
   * (root mutation)
   *
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Contact item.
   */
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "addContact");
    if(check === 'ok') {
        //check type
        if(!response.data.data["addContact"]
        || typeof response.data.data["addContact"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["addContact"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * updateItem
   *
   * Update Contact item on GraphQL Server.
   * (root mutation)
   *
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Contact item.
   */
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "updateContact");
    if(check === 'ok') {
        //check type
        if(!response.data.data["updateContact"]
        || typeof response.data.data["updateContact"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["updateContact"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   *
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Contact item.
   */
  async deleteItem(url, variables) {
    ...
    let result = null;
    //check
    let check = checkResponse(response, graphqlErrors, "deleteContact");
    if(check === 'ok') {
      //check type
      if(response.data.data["deleteContact"] === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      result = response.data.data["deleteContact"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getFileAttachments   *
   * Get fileAttachments records associated to the given contact record
...
`;
    ...
    let query =
      `query readOneContact($contact_id:ID!, $search: searchFileAttachmentInput, $pagination: paginationCursorInput!) {
             readOneContact(contact_id:$contact_id) {
...
//request
    ...
    let items = null;
    //check
    let check = checkResponse(response, graphqlErrors, "readOneContact");
    if(check === 'ok') {
      //check type
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !response.data.data["readOneContact"]["fileAttachmentsConnection"]
      || typeof response.data.data["readOneContact"]["fileAttachmentsConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneContact"]["fileAttachmentsConnection"].edges)
      || typeof response.data.data["readOneContact"]["fileAttachmentsConnection"].pageInfo !== 'object' 
      || response.data.data["readOneContact"]["fileAttachmentsConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneContact"]["fileAttachmentsConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * 
   * Get fileAttachments records count associated to the given contact record
   * through association 'FileAttachments', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  async getFileAttachmentsCount(url, itemId, searchText, ops) {
    ...
    let variables = {"contact_id": itemId};
    //search
    let s = getSearchArgument('fileAttachment', searchText, ops, 'object');
    if(s) variables.search = s.search;
    //query
    let query =
      `query readOneContact($contact_id:ID!, $search: searchFileAttachmentInput) { 
             readOneContact(contact_id:$contact_id) {
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !Number.isInteger(response.data.data["readOneContact"]["countFilteredFileAttachments"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneContact"]["countFilteredFileAttachments"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  let f1 = {field: "contact_ids", valueType: "Array", value: itemId, operator: "notIn"};
  let f2 = {field: "contact_ids", valueType: "String", value: null, operator: "eq"};
 ...
 */
  ...
  /**
   * getInvestigations   *
   * Get investigations records associated to the given contact record
...
`;
    ...
    let query =
      `query readOneContact($contact_id:ID!, $search: searchInvestigationInput, $pagination: paginationCursorInput!) {
             readOneContact(contact_id:$contact_id) {
...
//request
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !response.data.data["readOneContact"]["investigationsConnection"]
      || typeof response.data.data["readOneContact"]["investigationsConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneContact"]["investigationsConnection"].edges)
      || typeof response.data.data["readOneContact"]["investigationsConnection"].pageInfo !== 'object' 
      || response.data.data["readOneContact"]["investigationsConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneContact"]["investigationsConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * 
   * Get investigations records count associated to the given contact record
   * through association 'Investigations', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  async getInvestigationsCount(url, itemId, searchText, ops) {
    ...
    //query
    let query =
      `query readOneContact($contact_id:ID!, $search: searchInvestigationInput) { 
             readOneContact(contact_id:$contact_id) {
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !Number.isInteger(response.data.data["readOneContact"]["countFilteredInvestigations"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneContact"]["countFilteredInvestigations"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * Get ontologyAnnotations records associated to the given contact record
...
`;
    ...
    let query =
      `query readOneContact($contact_id:ID!, $search: searchOntologyAnnotationInput, $pagination: paginationCursorInput!) {
             readOneContact(contact_id:$contact_id) {
...
//request
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !response.data.data["readOneContact"]["ontologyAnnotationsConnection"]
      || typeof response.data.data["readOneContact"]["ontologyAnnotationsConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneContact"]["ontologyAnnotationsConnection"].edges)
      || typeof response.data.data["readOneContact"]["ontologyAnnotationsConnection"].pageInfo !== 'object' 
      || response.data.data["readOneContact"]["ontologyAnnotationsConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneContact"]["ontologyAnnotationsConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * 
   * Get ontologyAnnotations records count associated to the given contact record
   * through association 'OntologyAnnotations', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  async getOntologyAnnotationsCount(url, itemId, searchText, ops) {
    ...
    //query
    let query =
      `query readOneContact($contact_id:ID!, $search: searchOntologyAnnotationInput) { 
             readOneContact(contact_id:$contact_id) {
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !Number.isInteger(response.data.data["readOneContact"]["countFilteredOntologyAnnotations"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneContact"]["countFilteredOntologyAnnotations"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getStudies   *
   * Get studies records associated to the given contact record
...
`;
    ...
    let query =
      `query readOneContact($contact_id:ID!, $search: searchStudyInput, $pagination: paginationCursorInput!) {
             readOneContact(contact_id:$contact_id) {
...
//request
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !response.data.data["readOneContact"]["studiesConnection"]
      || typeof response.data.data["readOneContact"]["studiesConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneContact"]["studiesConnection"].edges)
      || typeof response.data.data["readOneContact"]["studiesConnection"].pageInfo !== 'object' 
      || response.data.data["readOneContact"]["studiesConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneContact"]["studiesConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * getStudiesCount
   * 
   * Get studies records count associated to the given contact record
   * through association 'Studies', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  async getStudiesCount(url, itemId, searchText, ops) {
    ...
    //query
    let query =
      `query readOneContact($contact_id:ID!, $search: searchStudyInput) { 
             readOneContact(contact_id:$contact_id) {
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneContact"]
      || typeof response.data.data["readOneContact"] !== 'object'
      || !Number.isInteger(response.data.data["readOneContact"]["countFilteredStudies"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneContact"]["countFilteredStudies"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  async getBarchartData(url, attribute) {
    ...
    //query
    let batchQuery = 
          `query contactsConnection($pagination: paginationCursorInput!) {
                 contactsConnection(pagination: $pagination) {
                    pageInfo {startCursor endCursor hasPreviousPage hasNextPage}
