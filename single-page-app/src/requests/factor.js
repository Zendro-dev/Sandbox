 ...
 */
   ...
   * tableTemplate
   *
   * Get factortable template from GraphQL Server.
   * (root query)
   *
  ...
  async tableTemplate(url) {
    ...
    let headers = null;
    //check
    let check = checkResponse(response, graphqlErrors, "csvTableTemplateFactor");
    if(check === 'ok') {
      //check type
      if(!Array.isArray(response.data.data["csvTableTemplateFactor"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      headers = response.data.data["csvTableTemplateFactor"];
 ...
 */
   ...
   * getCountItems
   *
   * Get factorsitems count from GraphQL Server.
   * (root query)
   *
   * @param {String} url GraphQL Server url
...
`;
    ...
    //query
    let query =
      `query factorsConnection($order: [orderFactorInput], $search: searchFactorInput, $pagination: paginationCursorInput!) { 
             factorsConnection( order: $order, search: $search, pagination: $pagination ) {
 ...
 */
   ...
   * createItem
   *
   * Add new Factor item on GraphQL Server.
   * (root mutation)
   *
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to create new Factor item.
   */
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "addFactor");
    if(check === 'ok') {
        //check type
        if(!response.data.data["addFactor"]
        || typeof response.data.data["addFactor"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["addFactor"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * updateItem
   *
   * Update Factor item on GraphQL Server.
   * (root mutation)
   *
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values to update the given Factor item.
   */
...
`;
    ...
    let item = null;
    //check
    let check = checkResponse(response, graphqlErrors, "updateFactor");
    if(check === 'ok') {
        //check type
        if(!response.data.data["updateFactor"]
        || typeof response.data.data["updateFactor"] !== 'object')
        return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

        //get value
        item = response.data.data["updateFactor"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   *
   * @param {String} url GraphQL Server url.
   * @param {Object} variables Object with values needed to delete the Factor item.
   */
  async deleteItem(url, variables) {
    ...
    let result = null;
    //check
    let check = checkResponse(response, graphqlErrors, "deleteFactor");
    if(check === 'ok') {
      //check type
      if(response.data.data["deleteFactor"] === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      result = response.data.data["deleteFactor"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getAssays   *
   * Get assays records associated to the given factor record
...
`;
    ...
    let query =
      `query readOneFactor($factor_id:ID!, $search: searchAssayInput, $pagination: paginationCursorInput!) {
             readOneFactor(factor_id:$factor_id) {
...
//request
    ...
    let items = null;
    //check
    let check = checkResponse(response, graphqlErrors, "readOneFactor");
    if(check === 'ok') {
      //check type
      if(!response.data.data["readOneFactor"]
      || typeof response.data.data["readOneFactor"] !== 'object'
      || !response.data.data["readOneFactor"]["assaysConnection"]
      || typeof response.data.data["readOneFactor"]["assaysConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneFactor"]["assaysConnection"].edges)
      || typeof response.data.data["readOneFactor"]["assaysConnection"].pageInfo !== 'object' 
      || response.data.data["readOneFactor"]["assaysConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneFactor"]["assaysConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * getAssaysCount
   * 
   * Get assays records count associated to the given factor record
   * through association 'Assays', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  async getAssaysCount(url, itemId, searchText, ops) {
    ...
    let variables = {"factor_id": itemId};
    //search
    let s = getSearchArgument('assay', searchText, ops, 'object');
    if(s) variables.search = s.search;
    //query
    let query =
      `query readOneFactor($factor_id:ID!, $search: searchAssayInput) { 
             readOneFactor(factor_id:$factor_id) {
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneFactor"]
      || typeof response.data.data["readOneFactor"] !== 'object'
      || !Number.isInteger(response.data.data["readOneFactor"]["countFilteredAssays"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneFactor"]["countFilteredAssays"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  let f1 = {field: "factor_ids", valueType: "Array", value: itemId, operator: "notIn"};
  let f2 = {field: "factor_ids", valueType: "String", value: null, operator: "eq"};
 ...
 */
  ...
  /**
   * getFileAttachments   *
   * Get fileAttachments records associated to the given factor record
...
`;
    ...
    let query =
      `query readOneFactor($factor_id:ID!, $search: searchFileAttachmentInput, $pagination: paginationCursorInput!) {
             readOneFactor(factor_id:$factor_id) {
...
//request
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneFactor"]
      || typeof response.data.data["readOneFactor"] !== 'object'
      || !response.data.data["readOneFactor"]["fileAttachmentsConnection"]
      || typeof response.data.data["readOneFactor"]["fileAttachmentsConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneFactor"]["fileAttachmentsConnection"].edges)
      || typeof response.data.data["readOneFactor"]["fileAttachmentsConnection"].pageInfo !== 'object' 
      || response.data.data["readOneFactor"]["fileAttachmentsConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneFactor"]["fileAttachmentsConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * 
   * Get fileAttachments records count associated to the given factor record
   * through association 'FileAttachments', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  async getFileAttachmentsCount(url, itemId, searchText, ops) {
    ...
    //query
    let query =
      `query readOneFactor($factor_id:ID!, $search: searchFileAttachmentInput) { 
             readOneFactor(factor_id:$factor_id) {
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneFactor"]
      || typeof response.data.data["readOneFactor"] !== 'object'
      || !Number.isInteger(response.data.data["readOneFactor"]["countFilteredFileAttachments"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneFactor"]["countFilteredFileAttachments"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * Get ontologyAnnotations records associated to the given factor record
...
`;
    ...
    let query =
      `query readOneFactor($factor_id:ID!, $search: searchOntologyAnnotationInput, $pagination: paginationCursorInput!) {
             readOneFactor(factor_id:$factor_id) {
...
//request
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneFactor"]
      || typeof response.data.data["readOneFactor"] !== 'object'
      || !response.data.data["readOneFactor"]["ontologyAnnotationConnection"]
      || typeof response.data.data["readOneFactor"]["ontologyAnnotationConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneFactor"]["ontologyAnnotationConnection"].edges)
      || typeof response.data.data["readOneFactor"]["ontologyAnnotationConnection"].pageInfo !== 'object' 
      || response.data.data["readOneFactor"]["ontologyAnnotationConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneFactor"]["ontologyAnnotationConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * 
   * Get ontologyAnnotations records count associated to the given factor record
   * through association 'OntologyAnnotation', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  async getOntologyAnnotationCount(url, itemId, searchText, ops) {
    ...
    //query
    let query =
      `query readOneFactor($factor_id:ID!, $search: searchOntologyAnnotationInput) { 
             readOneFactor(factor_id:$factor_id) {
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneFactor"]
      || typeof response.data.data["readOneFactor"] !== 'object'
      || !Number.isInteger(response.data.data["readOneFactor"]["countFilteredOntologyAnnotation"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneFactor"]["countFilteredOntologyAnnotation"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  /**
   * getStudies   *
   * Get studies records associated to the given factor record
...
`;
    ...
    let query =
      `query readOneFactor($factor_id:ID!, $search: searchStudyInput, $pagination: paginationCursorInput!) {
             readOneFactor(factor_id:$factor_id) {
...
//request
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneFactor"]
      || typeof response.data.data["readOneFactor"] !== 'object'
      || !response.data.data["readOneFactor"]["studiesConnection"]
      || typeof response.data.data["readOneFactor"]["studiesConnection"] !== 'object'
      || !Array.isArray(response.data.data["readOneFactor"]["studiesConnection"].edges)
      || typeof response.data.data["readOneFactor"]["studiesConnection"].pageInfo !== 'object' 
      || response.data.data["readOneFactor"]["studiesConnection"].pageInfo === null)
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      items = response.data.data["readOneFactor"]["studiesConnection"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
   ...
   * getStudiesCount
   * 
   * Get studies records count associated to the given factor record
   * through association 'Studies', from GraphQL Server.
   * 
   * 
   * @param {String} url GraphQL Server url
  ...
  async getStudiesCount(url, itemId, searchText, ops) {
    ...
    //query
    let query =
      `query readOneFactor($factor_id:ID!, $search: searchStudyInput) { 
             readOneFactor(factor_id:$factor_id) {
    ...
    if(check === 'ok') {
      ...
      if(!response.data.data["readOneFactor"]
      || typeof response.data.data["readOneFactor"] !== 'object'
      || !Number.isInteger(response.data.data["readOneFactor"]["countFilteredStudies"])) 
      return {data: response.data.data, value: null, message: 'bad_type', graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};

      //get value
      count = response.data.data["readOneFactor"]["countFilteredStudies"];
    } else return {data: response.data.data, value: null, message: check, graphqlErrors: (graphqlErrors.length>0) ? graphqlErrors : undefined};
 ...
 */
  ...
  async getBarchartData(url, attribute) {
    ...
    //query
    let batchQuery = 
          `query factorsConnection($pagination: paginationCursorInput!) {
                 factorsConnection(pagination: $pagination) {
                    pageInfo {startCursor endCursor hasPreviousPage hasNextPage}
